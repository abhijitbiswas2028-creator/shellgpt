...
* releasing a runtime scope is per-reset best-effort: a failing terminal reset must not leave the
  previous profile's secrets / SHELLGPT_HOME installed for the next body in that context.
"""
...
def test_send_keeps_external_source_value_over_raw_dotenv(two_homes, monkeypatch):
    ...
    wins) survives ``_load_shellgpt_env`` for the routed ``send``."""
    from shellgpt_cli import env_loader
    from shellgpt_cli.send_cmd import _load_shellgpt_env
...
def test_launch_body_survives_first_secondary_activation_on_the_dashboard(two_homes, monkeypatch):
    ...
    from shellgpt_cli import web_server_profiles as wsp
...
def test_release_resets_every_scope_when_one_reset_fails(two_homes, monkeypatch):
    ...
    assert current_secret_scope() is not None and get_shellgpt_home_override() == str(b)
    ...
    assert Path(server._shellgpt_home) == root

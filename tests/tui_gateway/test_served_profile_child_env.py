...
LAUNCH profile's ``.env``. Every ``shellgpt -p X`` / helper child (slash worker, relay delivery, A2A
...
_PROBE = ("import json,os;print(json.dumps({k:os.environ.get(k) for k in "
          "('SHELLGPT_HOME','A_MARKER','B_MARKER','TERMINAL_ENV','SHELLGPT_MODEL','FIRECRAWL_API_KEY')}))")
...
def mux_homes(tmp_path, monkeypatch):
    ...
    for key, val in (("A_MARKER", "a"), ("SHELLGPT_MODEL", "a-model"), ("TERMINAL_ENV", "docker"),
                     ("FIRECRAWL_API_KEY", "a-fc")):
...
def _assert_is_b_env(seen: dict, b: Path, *, with_secrets: bool):
    assert seen["SHELLGPT_HOME"] == str(b)
    assert seen["A_MARKER"] is None and seen["TERMINAL_ENV"] is None and seen["SHELLGPT_MODEL"] is None
...
def test_slash_worker_child_runs_in_the_served_profiles_env(mux_homes, monkeypatch):
    ...
    with monkeypatch.context() as m:  # restored before the probe child spawns through the real Popen
        ...
        assert captured["env"]["A_MARKER"] == "a" and captured["env"]["SHELLGPT_HOME"] == str(a)
...
def test_launch_profile_worker_spawns_with_own_home_after_multiplex_flip(mux_homes, monkeypatch):
    ...
    a, _b = mux_homes
    monkeypatch.setattr(server, "_shellgpt_home", str(a))  # launch profile home, captured at import
    captured = {}
    ...
    env = captured["env"]
    assert env["SHELLGPT_HOME"] == str(a)
...
def test_helper_children_resolve_secrets_through_the_served_profile(mux_homes):
    ...
    helper = (f"{sys.executable} -c \"import os;print(os.environ.get('B_MARKER','-')+'|'"
              f"+os.environ.get('A_MARKER','-')+'|'+os.environ.get('SHELLGPT_HOME',''))\"")

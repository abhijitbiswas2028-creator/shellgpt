...
def server():
    ...
    with patch.dict(
        "sys.modules",
        {
            "shellgpt_constants": MagicMock(
                get_shellgpt_home=MagicMock(return_value="/tmp/shellgpt_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'shellgpt-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'shellgpt-release' patched"
    }

...
def test_compute_host_workers_inherit_tui_pool_env(monkeypatch):
    monkeypatch.delenv("SHELLGPT_TUI_RPC_POOL_WORKERS", raising=False)
    monkeypatch.delenv("SHELLGPT_COMPUTE_HOST_WORKERS", raising=False)
    default = _default_workers()

    monkeypatch.setenv("SHELLGPT_TUI_RPC_POOL_WORKERS", "11")
    assert _default_workers() == 11

    # Malformed env falls back to the same default as unset.
    monkeypatch.setenv("SHELLGPT_TUI_RPC_POOL_WORKERS", "not-an-int")

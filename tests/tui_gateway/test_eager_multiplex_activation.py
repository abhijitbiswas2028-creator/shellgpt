...
empty ``.env``, which is all a crashed ``shellgpt profile create`` leaves behind. Both made a host
...
def two_profile_host(tmp_path, monkeypatch):
    """Fake HOME so ``profiles/`` never resolves to the live install (see shellgpt-agent-dev)."""
    home = tmp_path / "fakehome" / ".shellgpt"
    ...
    from shellgpt_cli.profiles import _get_profiles_root

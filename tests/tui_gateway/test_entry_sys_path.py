...
ShellGPT's own top-level modules and crashed the backend on import
(``ImportError: cannot import name ... from 'utils'``). entry.py must run
``shellgpt_bootstrap.harden_import_path()`` before its first non-stdlib import.
...
def test_entry_imports_from_cwd_with_colliding_packages(tmp_path):
    ...
    (and friends) must succeed — the guard strips CWD so ShellGPT's modules win."""
    for pkg in ("utils", "proxy", "ui"):
        (tmp_path / pkg).mkdir()
        (tmp_path / pkg / "__init__.py").write_text("", encoding="utf-8")

    env = {k: v for k, v in os.environ.items() if k != "SHELLGPT_PYTHON_SRC_ROOT"}
    ...
    env["SHELLGPT_HOME"] = str(tmp_path / "shellgpt_home")

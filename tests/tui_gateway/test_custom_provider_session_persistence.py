...
"No LLM provider configured. Run `shellgpt model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real SHELLGPT home.
...
# found" (the b200/shellgpt-ultra-sft report). The stored MODEL NAME is the one
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("shellgpt-ultra-sft")
            == "custom:shellgpt-ultra"
        )
...
class TestFollowProfileConfigRuntimeOverrides:
    def test_composer_pick_on_bot_chat_survives_resume_until_profile_model_changes(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(server, "_shellgpt_home", str(launch))
        ...
        try:
            with (
                patch("shellgpt_cli.model_switch.parse_model_flags", return_value=("glm-5.1", None, False, False, None)),
                patch("shellgpt_cli.model_switch.resolve_persist_behavior", return_value=False),
                patch("shellgpt_cli.model_switch.switch_model", return_value=result),

...
def test_gateway_health_event_maps_to_span_with_attrs():
    ...
    assert spans[0].name == "shellgpt.gateway_health"
    attrs = dict(spans[0].attributes or {})
    assert attrs["shellgpt.old_state"] == "starting"
    assert attrs["shellgpt.new_state"] == "running"
    assert attrs["shellgpt.active_agents"] == 2
...
def test_trace_resource_includes_stable_hashed_instance():
    ...
    assert attrs["service.name"] == "shellgpt-gateway"
...
def test_streamer_receives_events_and_respects_filter(monkeypatch):
    ...
    spans = mem.get_finished_spans()
    assert [s.name for s in spans] == ["shellgpt.gateway_health"]

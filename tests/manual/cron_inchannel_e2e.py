...
Uses a throwaway SHELLGPT_HOME so it never touches ~/.shellgpt. No real names.
"""
...
def _fresh_home():
    """Point SHELLGPT_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_shellgpt_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SHELLGPT_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SHELLGPT_HOME is set.
...
# a different get_shellgpt_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SHELLGPT_HOME (throwaway): {HOME}")

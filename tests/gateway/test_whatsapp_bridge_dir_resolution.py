...
(/opt/shellgpt/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable SHELLGPT_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_shellgpt_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable SHELLGPT_HOME."""
    ...
    monkeypatch.setattr(
        "shellgpt_constants.get_shellgpt_home", lambda: shellgpt_home
    )
    ...
    expected = shellgpt_home / "scripts" / "whatsapp-bridge"

...
class TestTeamsInteractiveSetup:
    def test_interactive_setup_persists_credentials(self, tmp_path, monkeypatch):
        ...
        from shellgpt_cli.cli_output (not shellgpt_cli.config) and persist
        ...
        env_text = (shellgpt_home / ".env").read_text(encoding="utf-8")
...
class TestTeamsRequireMention:
    ...
    @pytest.mark.parametrize("conversation_type, kwargs, dispatched", [
        ...
        ("channel", {"text": "<at>ShellGPT</at> hi", "mentioned_id": "28:bot-id"}, True),  # wire form of the bot id
        ("groupChat", {"text": "<at>ShellGPT</at> hi", "mentioned_id": "bot-id"}, True),

...
``get_shellgpt_home()``, which follows the override: the row landed in ``profiles/<name>/state.db``,
...
@pytest.fixture(params=["shellgpt-home-env", "platform-default"])
def launch_home(request, tmp_path, monkeypatch):
    root = tmp_path / "root"
    (root / "profiles" / "research").mkdir(parents=True)
    if request.param == "shellgpt-home-env":
        monkeypatch.setenv("SHELLGPT_HOME", str(root))
    else:
        # A default gateway run in the foreground has no SHELLGPT_HOME at all.

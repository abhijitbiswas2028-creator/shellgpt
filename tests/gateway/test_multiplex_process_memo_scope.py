...
(SHELLGPT_HOME override), not the launch home the module constants were frozen from."""
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (SHELLGPT_HOME) and a routed profile B with different config everywhere."""
    a = tmp_path / ".shellgpt"
    b = a / "profiles" / "b"
...
def test_max_turns_and_fallback_chain_follow_routed_profile(two_homes, monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_shellgpt_home", a)
...
async def test_gateway_hooks_fire_per_routed_profile(two_homes):
    ...
    try:
        ...
        reset_shellgpt_home_override(token)
    assert ctx_a.get("seen") == ["shellgpt_hook_hook-a"]
    assert ctx_b.get("seen") == ["shellgpt_hook_hook-b"]
...
def test_media_policy_reads_routed_profile_config_not_env(two_homes, monkeypatch):
    ...
    for var in ("SHELLGPT_MEDIA_DELIVERY_STRICT", "SHELLGPT_MEDIA_ALLOW_DIRS"):

"""A profile directory copied out of another SHELLGPT_HOME must never read as running.
...
def _live_record(shellgpt_home):
    return {
        "pid": _LIVE_PID, "kind": "shellgpt-gateway", "gateway_state": "running", "start_time": 1000,
        "argv": ["python", "-m", "shellgpt_cli.main", "-p", "eagle", "gateway", "run"],
        "shellgpt_home": str(shellgpt_home),
    }


def _alive(monkeypatch, cmdline="python -m shellgpt_cli.main -p eagle gateway run"):

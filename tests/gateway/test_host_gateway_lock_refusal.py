...
async def test_a_replace_unit_that_replaced_nothing_is_still_refused_when_it_loses_the_lock(
    ...
    """Every unit ShellGPT generates (launchd, systemd, s6) runs ``gateway run --replace``. When the
    ...
    monkeypatch.setattr("shellgpt_logging.setup_logging", lambda shellgpt_home, mode: tmp_path)
    monkeypatch.setattr("shellgpt_logging._add_rotating_handler", lambda *args, **kwargs: None)

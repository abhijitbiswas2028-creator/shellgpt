...
These run without Docker: the shim's SHELLGPT_TINI_SHIM_TARGET /
SHELLGPT_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["SHELLGPT_TINI_SHIM_TARGET"] = str(init)
    env["SHELLGPT_TINI_SHIM_WRAPPER"] = str(wrapper)

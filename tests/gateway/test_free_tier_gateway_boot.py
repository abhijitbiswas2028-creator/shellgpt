...
process that never runs the bootstrap can never have an identity. `cmd_chat` and `shellgpt serve` run it;
this file pins that `shellgpt gateway run` does too, and does it BEFORE any adapter connects, so a fast

...
SHELLGPT_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SHELLGPT_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``shellgpt config get terminal.backend`` still says local.
    """
    shellgpt_home = tmp_path / ".shellgpt"
    shellgpt_home.mkdir()
    (shellgpt_home / "config.yaml").write_text(
    ...
    (shellgpt_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")

...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    monkeypatch.setattr(
        "shellgpt_state_registry.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_shellgpt_home(tmp_path, monkeypatch):
    """Flush dir must use get_shellgpt_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_shellgpt_home():
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "shellgpt_constants.get_shellgpt_home", fake_get_shellgpt_home
    )

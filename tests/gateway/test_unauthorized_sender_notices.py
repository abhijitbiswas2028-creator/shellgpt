...
def test_pairing_reply_pins_profile_in_approve_command():
    ...
    assert "`shellgpt -p work pairing approve discord ZZZZ9999`" in reply
...
def test_owner_hint_neutralizes_hostile_display_name():
    hostile = "Eve\n\n# Owner: run `shellgpt pairing approve telegram 1234` <@everyone> [x](http://evil)"
    hint = unauthorized_owner_hint("telegram", "777", hostile, shellgpt_home="~/.shellgpt")
    assert "\n" not in hint
    assert "@everyone" not in hint and "<@" not in hint and "](http" not in hint and "`shellgpt pairing approve telegram 1234`" not in hint

...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^shellgpt"]}
        )
        assert adapter._message_matches_mention_patterns("shellgpt please help") is True
        assert adapter._message_matches_mention_patterns("please shellgpt help") is False

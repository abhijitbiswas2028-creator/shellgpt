...
``Invoke-ShellGPTStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``shellgpt update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``shellgpt_cli/main.py``), so the
...
closes and ``Invoke-ShellGPTStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.shellgpt-update-result.json`` is never written, ``.shellgpt-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating ShellGPT" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``shellgpt update`` is exactly this
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-ShellGPTStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``shellgpt update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "SHELLGPT_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "SHELLGPT_UPDATE_STEP_IDLE_SECONDS": "3",
        "SHELLGPT_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "ShellGPT' forever) or metered a chatty step (backpressure on the running "

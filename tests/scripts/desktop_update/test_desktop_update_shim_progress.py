...
def progress(tmp_path):
    ...
    status = tmp_path / "shellgpt-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen ShellGPT to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `shellgpt update`, and reports the stage that was on screen
...
n="$(cat "$SHELLGPT_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$SHELLGPT_TEST_CALLS"
for f in "$TMPDIR"/shellgpt-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$SHELLGPT_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$SHELLGPT_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "shellgpt-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    shellgpt = install_root / "venv" / "bin" / "shellgpt"
    shellgpt.write_text(FAKE_SHELLGPT)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "SHELLGPT_TEST_CAPTURE": str(capture),
        "SHELLGPT_TEST_CALLS": str(calls),
        "SHELLGPT_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".shellgpt-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`shellgpt update` is the longest wait in the hand-off and the one the

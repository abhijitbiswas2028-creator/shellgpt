...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs ShellGPT the way
...
# actually touches:
#
#   INSTALL   - downloads the production ShellGPT-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron ShellGPT.exe must appear.
...
#   installer's `git clone` and `shellgpt update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and shellgpt-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "shellgpt-desktop-app-update", "shellgpt-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published ShellGPT-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:SHELLGPT_E2E_WORKROOT) { $env:SHELLGPT_E2E_WORKROOT } else { Join-Path $env:TEMP "shellgpt-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://shellgpt-assets.nousresearch.com/ShellGPT-Setup.exe",
...
$ShellGPTHome  = Join-Path $WorkRoot "shellgpt-home"
$InstallDir  = Join-Path $ShellGPTHome "shellgpt-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if shellgpt ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit shellgpt thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\ShellGPT.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\ShellGPT.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-ShellGPTRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $ShellGPTHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "shellgpt-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $ShellGPTHome ".shellgpt-update-result.json"
    ...
    Get-ChildItem -LiteralPath $ShellGPTHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "shellgpt-home-ls.txt")
}

function Test-ShellGPTRuns([string]$Label) {
    Save-InstallSideState $Label
    $shellgptExe = Join-Path $InstallDir "venv\Scripts\shellgpt.exe"
    Assert-True (Test-Path -LiteralPath $shellgptExe) "$Label -- venv\Scripts\shellgpt.exe exists"
    & $shellgptExe --version 2>&1 | ForEach-Object { Write-Host "    shellgpt --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- shellgpt --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-ShellGPTHome", $ShellGPTHome, "-InstallDir", $InstallDir)
...
function Invoke-ShellGPTUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $shellgptExe = Join-Path $InstallDir "venv\Scripts\shellgpt.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $shellgptExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $shellgptExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "shellgpt update transcript" $log
    Assert-True ($updateExit -eq 0) "shellgpt update exited $updateExit (expected 0)"
}

function Invoke-ShellGPTDesktopAppUpdate([string]$TargetSha) {
    # The shellgpt-desktop launch surface: `shellgpt desktop` runs its whole
    ...
    $prevCap = $env:SHELLGPT_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:SHELLGPT_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $shellgptExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:SHELLGPT_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "shellgpt desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "shellgpt desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $ShellGPTHome ".shellgpt-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured shellgpt desktop spec; update completed"
}
...
function Stop-ShellGPTAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only ShellGPT.exe (Electron) -- never shellgpt.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "ShellGPT" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) ShellGPT.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `shellgpt update`/desktop builds use the ShellGPT-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $ShellGPTHome "node\node.exe"),
        (Join-Path $ShellGPTHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website ShellGPT-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): ShellGPT-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "ShellGPT-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "ShellGPT-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:SHELLGPT_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $ShellGPTHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  ShellGPT-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> ShellGPT.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "ShellGPT-Setup.exe", (Join-Path $ShellGPTHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "ShellGPT" -ErrorAction SilentlyContinue)) "ShellGPT.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "ShellGPT-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $ShellGPTHome "logs\bootstrap-installer.log"
    ...
    Stop-ShellGPTAppProcesses "post-install"
    ...
    Test-ShellGPTRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop ShellGPT.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged ShellGPT.exe present before update"
    ...
    try {
        ...
        # `shellgpt update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .shellgpt-update-result.json on every exit;
        #   * older checkouts -> the staged shellgpt-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/shellgpt/relaunch
        ...
        $updateLog = Join-Path $ShellGPTHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-ShellGPTRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "ShellGPT.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched ShellGPT.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "ShellGPT" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched ShellGPT window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "ShellGPT" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $ShellGPTHome "logs\desktop-update-handoff.log"
        ...
        Stop-ShellGPTAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, shellgpt runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-ShellGPTRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-ShellGPTRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $ShellGPTHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "shellgpt-desktop-app-update" {
            Invoke-ShellGPTDesktopAppUpdate $state.current
        }
        "shellgpt-update" {
            Invoke-ShellGPTUpdate
        }
        "installer-script" {
        ...
        "desktop-installer@latest" {
            # A user re-downloading ShellGPT-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $ShellGPTHome "logs\desktop.log"))) {

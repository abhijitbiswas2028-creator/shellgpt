"""Driver-side spawn interception for shellgpt desktop E2E legs.

The installed ``shellgpt`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``shellgpt
...
Opt-in: ``SHELLGPT_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "shellgpt
...
tag's own shellgpt_cli/main.py):
...
_SPEC_PATH = os.environ.get("SHELLGPT_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/ShellGPT.exe, linux-unpacked/...,
        # mac*/ShellGPT.app/Contents/MacOS/...).

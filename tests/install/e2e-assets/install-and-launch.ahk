...
; Drive the REAL ShellGPT-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real ShellGPT.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-shellgpt-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "ShellGPT-Setup.exe"
...
Log("Launch clicked; waiting for the ShellGPT desktop app window")
...
try {
    ...
    throw Error("ShellGPT.exe real-sized window did not appear within 300s of clicking Launch")
}

// drive-update.cjs — launch the INSTALLED ShellGPT.exe (real Electron desktop
...
//
//   node <this file> <path-to-ShellGPT.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <ShellGPT.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: SHELLGPT_HOME (isolated install) and
  ...
  // The real backend (`shellgpt serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting ShellGPT... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.shellgptDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating ShellGPT — this window will close" overlay should appear,
  ...
  // the staged binary), which claims SHELLGPT_HOME/.shellgpt-update-in-progress
  ...
  // proved the desktop had exited and `shellgpt update` was already running).
  //
  ...
  const shellgptHome = process.env.SHELLGPT_HOME
  const markerPath = shellgptHome ? path.join(shellgptHome, '.shellgpt-update-in-progress') : null
  const resultPath = shellgptHome ? path.join(shellgptHome, '.shellgpt-update-result.json') : null

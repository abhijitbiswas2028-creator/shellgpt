...
#   mock_start <workroot>      start, write config into $SHELLGPT_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, SHELLGPT_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$SHELLGPT_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $SHELLGPT_HOME (api $url/v1)"
}

#!/usr/bin/env bash
# Drive the ShellGPT-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install ShellGPT ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "ShellGPT")
...
SHELLGPT_BIN="$INSTALL_DIR/venv/bin/shellgpt"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$SHELLGPT_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.shellgpt/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + ShellGPT.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "ShellGPT-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then

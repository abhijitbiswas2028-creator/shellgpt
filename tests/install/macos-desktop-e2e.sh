...
# (ShellGPT-Setup.dmg from the website) can reach HEAD.
#
...
#              shellgpt-desktop-app-update  capture `shellgpt desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              shellgpt-update              CLI update from the installed venv
...
#     --update-method open-app-update|shellgpt-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://shellgpt-assets.nousresearch.com/ShellGPT-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|shellgpt-desktop-app-update|shellgpt-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${SHELLGPT_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/shellgpt-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$SHELLGPT_HOME/shellgpt-agent"
}
...
phase_stage() {
  ...
  touch "$SHELLGPT_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/ShellGPT.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/ShellGPT.app" \
    "/Applications/ShellGPT.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published ShellGPT-Setup.dmg"

  local dmg="$WORK_ROOT/ShellGPT-Setup.dmg"
  ...
  # "Install ShellGPT" button with native input.
  local rc=0
  ...
  log_group "ShellGPT-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$shellgpt" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "shellgpt --version failed after install"
  ok "shellgpt --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed ShellGPT.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    shellgpt-update)
      ...
      # update subcommand only in later releases; ask the installed shellgpt.
      local shellgpt="$INSTALL_DIR/venv/bin/shellgpt"
      local update_cmd=("$shellgpt" update)
      ...
      [ "$rc" -eq 0 ] || fail "shellgpt update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/ShellGPT.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/ShellGPT.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built ShellGPT.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, shellgpt update) need.
      local app app_bin
...
PYEOF
    ...
    shellgpt-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "shellgpt desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "shellgpt desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.shellgpt/logs" "$ildest/shellgpt-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/ShellGPT"
  ...
  "$INSTALL_DIR/venv/bin/shellgpt" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "shellgpt --version failed after update"
  ok "shellgpt --version works post-update"

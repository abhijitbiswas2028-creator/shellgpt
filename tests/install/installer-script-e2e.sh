...
#              install landed on OLD with a working `shellgpt`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `shellgpt`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method shellgpt-update|installer-script|installer-script+desktop
...
#   --update-method  shellgpt-update      `shellgpt update`
#                    installer-script   re-run install.sh (HEAD's copy)
#                    installer-script+desktop  re-run with --include-desktop
#                    shellgpt-desktop-app-update  launch the app via `shellgpt
...
case "$UPDATE_METHOD" in
  shellgpt-update|installer-script|installer-script+desktop|shellgpt-desktop-app-update) ;;
  *) echo "error: --update-method must be shellgpt-update, installer-script, installer-script+desktop or shellgpt-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/shellgpt-installer-script-e2e"
LOG_DIR="${SHELLGPT_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled shellgpt or a
# developer config, and old installer scripts hardcode $HOME/.shellgpt (the
# SHELLGPT_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export SHELLGPT_HOME="$HOME/.shellgpt"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/ShellGPT" \
    "$release_dir/linux-unpacked/shellgpt" \
    "$release_dir/mac-arm64/ShellGPT.app" \
    "$release_dir/mac/ShellGPT.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local shellgpt="$INSTALL_DIR/venv/bin/shellgpt"
  [ -x "$shellgpt" ] || fail "no shellgpt console script at $shellgpt"
  "$shellgpt" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "shellgpt --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "shellgpt --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `shellgpt desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `shellgpt update` itself makes. Probe the
  # INSTALLED shellgpt for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `shellgpt desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local shellgpt="$INSTALL_DIR/venv/bin/shellgpt"
  if ! "$shellgpt" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "shellgpt desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$shellgpt" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "shellgpt desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "shellgpt desktop --build-only ($1) exited $rc; transcript above"
  ok "shellgpt desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  shellgpt-update)
    ...
    # installed shellgpt; older ones read the prompt from stdin, so close it.
    SHELLGPT="$INSTALL_DIR/venv/bin/shellgpt"
    if "$SHELLGPT" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$SHELLGPT" update --yes)
    else
      update_cmd=("$SHELLGPT" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "shellgpt update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "shellgpt update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  shellgpt-desktop-app-update)
    # The real user surface: `shellgpt desktop` launches the app, the user
    ...
    # suite's own) is configured into SHELLGPT_HOME exactly like the dev:mock
    ...
    step "capturing the shellgpt desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      SHELLGPT_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$SHELLGPT" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "shellgpt desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "shellgpt desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "shellgpt desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$SHELLGPT_HOME/logs" "$ildest/shellgpt-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/shellgpt/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$SHELLGPT_HOME/.shellgpt-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$SHELLGPT_HOME" > "$ildest/shellgpt-home-ls.txt" 2>/dev/null || true

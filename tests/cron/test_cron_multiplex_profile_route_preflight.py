...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_shellgpt_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "shellgpt_constants.get_default_shellgpt_root", lambda: root
    )
    token = set_shellgpt_home_override(str(grant_home))

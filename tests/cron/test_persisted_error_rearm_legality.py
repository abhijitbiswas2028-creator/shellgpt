...
def cron_store(tmp_path, monkeypatch):
    ...
    (shellgpt_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
    monkeypatch.setattr(J, "SHELLGPT_DIR", shellgpt_home)
    monkeypatch.setattr(J, "CRON_DIR", shellgpt_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", shellgpt_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", shellgpt_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_shellgpt_now", lambda: now):
            due = J.get_due_jobs()

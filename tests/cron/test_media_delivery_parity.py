...
text-only on manual ``shellgpt cron run <job-id>``. Same box, same token, same
...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import shellgpt_cli.plugins as hp_boot
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp SHELLGPT_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_deliver_result_runs_bridge(self, monkeypatch, tmp_path, media_file):
        ...
        Realistic enterprise shape: SHELLGPT_MEDIA_DELIVERY_STRICT=1 arrives via

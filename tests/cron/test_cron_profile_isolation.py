...
profile's SHELLGPT_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.shellgpt/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SHELLGPT_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SHELLGPT_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SHELLGPT_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import shellgpt_constants

    monkeypatch.setattr(
        shellgpt_constants, "_get_platform_default_shellgpt_home", lambda: root
    )
    monkeypatch.setenv("SHELLGPT_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile SHELLGPT_HOME (<root>/profiles/<name>), the cron store
    ...
    assert shellgpt_constants.get_shellgpt_home().resolve() == profile_home.resolve()
    assert shellgpt_constants.get_default_shellgpt_root().resolve() == root.resolve()

    # cron/jobs.py computes SHELLGPT_DIR from get_shellgpt_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.SHELLGPT_DIR.resolve() == profile_home.resolve()
        assert (

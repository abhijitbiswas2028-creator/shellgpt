...
which is already bounded by SHELLGPT_CRON_TIMEOUT — the worker thread submitted
...
timeout (SHELLGPT_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._shellgpt_home", tmp_path), \
             ...
             patch("shellgpt_cli.env_loader.load_shellgpt_dotenv"), \
             patch("shellgpt_cli.env_loader.reset_secret_source_cache"), \
             patch("shellgpt_state_registry.acquire"), \
             patch(
                 "shellgpt_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SHELLGPT_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._shellgpt_home", tmp_path), \
                 ...
                 patch("shellgpt_cli.env_loader.load_shellgpt_dotenv"), \
                 patch("shellgpt_cli.env_loader.reset_secret_source_cache"), \
                 patch("shellgpt_state_registry.acquire", side_effect=_hanging_then_capture), \
                 patch(
                     "shellgpt_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._shellgpt_home", tmp_path), \
             ...
             patch("shellgpt_cli.env_loader.load_shellgpt_dotenv"), \
             patch("shellgpt_cli.env_loader.reset_secret_source_cache"), \
             patch("shellgpt_state_registry.acquire") as mock_db_cls, \
             patch(

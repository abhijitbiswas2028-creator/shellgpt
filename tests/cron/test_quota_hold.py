...
def test_weekly_cron_retries_when_quota_recovers_before_next_occurrence(
    ...
    monkeypatch.setattr(qh, "_shellgpt_now", lambda: now)
    ...
    monkeypatch.setattr("cron.jobs._shellgpt_now", lambda: retry_at + timedelta(seconds=1))
...
def _tick(job, home, deliveries, resolve):
    ...
    with patch("cron.scheduler._shellgpt_home", home), \
         patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
         patch("shellgpt_cli.env_loader.load_shellgpt_dotenv"), \
         patch("shellgpt_cli.env_loader.reset_secret_source_cache"), \
         patch("shellgpt_state_registry.acquire", return_value=MagicMock()), \
         patch("tools.mcp_tool_discovery.discover_mcp_tools", return_value=[]), \
         patch("shellgpt_cli.runtime_provider.resolve_runtime_provider", side_effect=resolve), \

...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(shellgpt_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "shellgpt_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("SHELLGPT_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("SHELLGPT_TEST_RUNTIME_KEY", ""),
            "base_url": None,
    ...
    monkeypatch.setattr(
        "shellgpt_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )

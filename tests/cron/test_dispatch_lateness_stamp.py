...
kind) on every recurring dispatch so `shellgpt cron list` and
`shellgpt cron status` can surface late catch-ups.
"""
...
class TestDueScanDispatchStamp:
    def test_catch_up_beyond_grace_stamped_and_persisted(self, cron_store):
        ...
        # Persisted, so a separate `shellgpt cron list` process can read it.

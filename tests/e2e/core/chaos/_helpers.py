...
The chaos suites drive a REAL ShellGPT surface (AIAgent child process, GatewayRunner,
...
def write_chaos_home(root: Path, base_url: str, **cfg: Any) -> tuple[Path, Path]:
    """Create ``root/home`` (fake $HOME) and ``root/home/.shellgpt`` (SHELLGPT_HOME)."""
    home = root / "home"
    shellgpt_home = home / ".shellgpt"
    shellgpt_home.mkdir(parents=True, exist_ok=True)
    (shellgpt_home / "config.yaml").write_text(chaos_config(base_url, **cfg), encoding="utf-8")
    (shellgpt_home / ".env").write_text("OPENAI_API_KEY=sk-fake-chaos\n", encoding="utf-8")
    return home, shellgpt_home


def hermetic_env(home: Path, shellgpt_home: Path, tag: str) -> dict[str, str]:
    """Child env: fake HOME/SHELLGPT_HOME, no real credentials, repo importable, and a
    tag every descendant inherits so the orphan scan can find it after reparenting."""
    env = {
        k: v for k, v in os.environ.items()
        if not (
            k.endswith(("_API_KEY", "_TOKEN", "_SECRET"))
            or k.startswith(("SHELLGPT_", "OPENROUTER", "ANTHROPIC", "OPENAI", "NOUS_"))

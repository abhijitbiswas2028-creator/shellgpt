...
``HOME``/``SHELLGPT_HOME`` (no state leaks into the suite process). A cell asks ``gap_open`` and
...
jobs._shellgpt_now = lambda: datetime.fromisoformat("2026-03-07T09:00:30-05:00")
...
def gap_open(pr: int) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix=f"gap-{pr}-") as tmp:
        ...
        env = {k: v for k, v in os.environ.items()
               if not k.startswith(("PYTEST_", "SHELLGPT_")) and not k.endswith("_API_KEY")}
        env.update({"HOME": str(home), "SHELLGPT_HOME": str(home / ".shellgpt"),

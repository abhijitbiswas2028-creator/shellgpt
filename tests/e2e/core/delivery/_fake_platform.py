...
``kill -9`` + restart on the same SHELLGPT_HOME possible.
"""
...
# Per-platform send/edit semantics (what the real service enforces, not what ShellGPT believes).
...
class GatewayProcess:
    ...
    def __init__(self, root: Path, *, platforms: Dict[str, str], llm_base_url: str,
        ...
        from tests.fakes.fake_llm_provider import write_shellgpt_home
        ...
        write_shellgpt_home(self.shellgpt_home, llm_base_url, extra_config=extra_config)
    ...
    def env(self) -> Dict[str, str]:
        ...
        env.update({
            ...
            # The child's HOME is itself a throwaway tmp dir, so ~/.shellgpt IS the temp home here; the
            # live-system guard (armed by the inherited isolation marker) would refuse it.
            "SHELLGPT_STATE_DB_GUARD_BYPASS": "1",
            "SHELLGPT_GATEWAY_LOCK_DIR": str(self.root / "gateway-locks"),
            "TZ": "UTC", "PYTHONHASHSEED": "0", "PYTHONUNBUFFERED": "1", "C12_PARENT_PID": str(os.getpid()),
            "SHELLGPT_DISABLE_LAZY_INSTALLS": "1", "TIRITH_ENABLED": "false",
            "AWS_EC2_METADATA_DISABLED": "true", "SHELLGPT_HONCHO_HOST": "shellgpt",

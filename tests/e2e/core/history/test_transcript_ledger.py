...
and at the end of every scenario a FRESH ``shellgpt chat --resume`` process replays the persisted
...
def world(tmp_path, monkeypatch):
    home = tmp_path / "home"
    home.mkdir()
    # The hermetic conftest's per-test SHELLGPT_HOME (a tmp dir): the in-process live-system guard
    # refuses a <HOME>/.shellgpt layout, and the children must share this very state.db.
    ...
    with FakeLLMServer(script) as srv:
        write_shellgpt_home(shellgpt_home, srv.base_url, extra_config=OFFLINE_CONFIG + NO_BACKGROUND_REVIEW)
        monkeypatch.setenv("OPENAI_API_KEY", "sk-fake-e2e")
        yield {"srv": srv, "script": script, "home": home, "shellgpt_home": shellgpt_home,
...
def test_transcript_ledger(world, name):
    srv, script, shellgpt_home = world["srv"], world["script"], world["shellgpt_home"]
    ...
    session = InProcessSession(srv.base_url, shellgpt_home, f"ledger-{name}")
    script.session = session
    ledger = Ledger(srv, shellgpt_home)
    try:
    ...
    if name in COMPACTING:
        ...
        model, _ = views(shellgpt_home, sid)
    ...
    assert_usage_matches(shellgpt_home, lineage(shellgpt_home, sid), srv.requests, f"scenario {name}")
    integrity_ok(shellgpt_home)

    # (d) resume in a FRESH `shellgpt chat --resume` process: its first request replays the persisted
    ...
    persisted, _ = views(shellgpt_home, sid)
    n_before = len(main)
    env = child_env(world["home"], shellgpt_home, world["workdir"])
    ...
    assert_usage_matches(shellgpt_home, lineage(shellgpt_home, sid), srv.requests, f"scenario {name} after resume")
...
def test_micro_compaction_resumed_display_shows_each_input_once(world):
    ...
    session = InProcessSession(srv.base_url, shellgpt_home, "ledger-micro-display")
    inputs: list[str] = []
    try:
        ...
        session.close()
    model, display = views(shellgpt_home, sid)

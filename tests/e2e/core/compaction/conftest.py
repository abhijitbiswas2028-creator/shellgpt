...
def make_scenario(provider, tmp_path, monkeypatch):
    """Build a Scenario with its own SHELLGPT_HOME/state.db; closes every agent + DB at teardown."""
    server, dispatch = provider
    made: list[Scenario] = []

    def make(mode: str = "good", *, extra: str = "", platform: str = "cli", window: int | None = None,
             threshold_tokens: int = THRESHOLD_TOKENS, name: str = "shellgpt_home") -> Scenario:
        # The hermetic root conftest already isolates SHELLGPT_HOME per test; HOME stays put because the
        # state.db live-system guard treats $HOME/.shellgpt as production.
        shellgpt_home = tmp_path / name
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        import shellgpt_state

        monkeypatch.setattr(shellgpt_state, "DEFAULT_DB_PATH", shellgpt_home / "state.db")
        slow = mode == "slow"
        if slow:
        ...
        write_home(shellgpt_home, server.base_url,
                   compaction_config(extra=extra, slow=slow, threshold_tokens=threshold_tokens))
        sc = Scenario(server=server, dispatch=dispatch, home=shellgpt_home, mode=mode, platform=platform, window=window)
        made.append(sc)
        return sc

...
ShellGPT runs for real in child processes with HOME=<tmp>/home and SHELLGPT_HOME=<tmp>/home/.shellgpt
...
def hermetic_env(home: Path, extra: dict[str, str] | None = None) -> dict[str, str]:
    ...
    home = home.resolve()
    assert home != real_user_home() and home / ".shellgpt" != real_user_home() / ".shellgpt", home
    ...
    for var in ("HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy", "XDG_STATE_HOME",
                ...
                # live shellgpt-gateway unit, whatever it decides about service management.
...
def make_tenants(root: Path, names: Iterable[str], launch: str = "default") -> dict[str, Tenant]:
    """Build tenant homes on disk: launch profile at HOME/.shellgpt, the rest under profiles/<name>."""
    shellgpt_home = root / "home" / ".shellgpt"
    tenants: dict[str, Tenant] = {}
    for name in names:
        home = shellgpt_home if name == launch else shellgpt_home / "profiles" / name
...
def seed_due_cron_job(t: Tenant, home: Path) -> str:
    ...
    r = subprocess.run([sys.executable, "-c", _SEED_CRON, t.cron_prompt], cwd=str(home),
                       env=hermetic_env(home, {"SHELLGPT_HOME": str(t.home)}), capture_output=True, text=True,
...
def run_shellgpt(argv: list[str], home: Path, timeout: float = 120.0,
               extra_env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    proc = subprocess.Popen(
        [sys.executable, "-m", "shellgpt_cli.main", *argv], cwd=str(home), env=hermetic_env(home, extra_env),
...
class ServeBackend(TuiBackend):
    """The real Desktop backend: ``shellgpt serve --port 0`` with the JSON-RPC surface on ``/api/ws``.

    ``SHELLGPT_DESKTOP=1`` marks it app-spawned, which is also what runs the cron ticker in-process.
    """
    ...
    def __init__(self, home: Path, log_path: Path, extra_env: dict[str, str] | None = None) -> None:
        ...
        self._log = open(log_path, "a", encoding="utf-8")  # noqa: SIM115 - closed in close()
        env = {"SHELLGPT_DASHBOARD_SESSION_TOKEN": self.token, "SHELLGPT_DESKTOP": "1", **(extra_env or {})}
        self.proc = subprocess.Popen(
        ...
        self.port = poll(lambda: port_box[0] if port_box else (self.proc.poll() is not None and -1), 120,
                         "shellgpt serve to report its port")
        assert self.port > 0, f"shellgpt serve exited rc={self.proc.returncode}"

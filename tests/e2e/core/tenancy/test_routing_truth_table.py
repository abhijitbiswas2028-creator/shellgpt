...
selection, so a misroute always has somewhere to land and be caught. ShellGPT runs for real in a
child process (``shellgpt chat -q``, ``shellgpt -z``, the stdio ``tui_gateway`` the TUI/Desktop
drive) with a tmp HOME/SHELLGPT_HOME and every credential env var stripped.
...
class Leg:
    ...
    profile: str | None = None                 # run with SHELLGPT_HOME = profiles/<name>
...
def _bare_custom(c: dict[str, Any], e: dict[str, str], _f: Fleet) -> None:
    ...
    # tests/shellgpt_cli/test_runtime_provider_resolution.py); with neither, nothing may be reached
...
def _run_case(case: Case, root: Path) -> tuple[Fleet, list[LegOutcome]]:
    ...
    try:
        ...
        write_home(home / ".shellgpt", config, env, auth)
        for name, (pconfig, penv) in case.profiles(fleet).items():
            write_home(home / ".shellgpt" / "profiles" / name, pconfig, penv)
        outcomes = []
        for leg in case.legs:
            marks, egress_mark = fleet.marks(), len(trap.attempts)
            shellgpt_home = home / ".shellgpt" / "profiles" / leg.profile if leg.profile else None
            run = run_shellgpt(leg.argv, home, shellgpt_home=shellgpt_home, proxy=trap.url, timeout=leg.timeout)
...
def test_cli_routing_truth_table(case: Case, cli_outcomes: dict[str, Any]) -> None:
    ...
    for i, out in enumerate(outcomes):
        ...
        ctx = (f"[{case.id} leg {i}: shellgpt {' '.join(leg.argv)}] rc={run.rc} ({run.seconds:.1f}s)\n"
               f"requests:\n{describe(out.log)}\nstdout tail:\n{run.stdout[-1500:]}\nstderr tail:\n{run.stderr[-1500:]}")
...
def test_tui_gateway_model_switch_routing(tmp_path: Path, request: pytest.FixtureRequest) -> None:
    ...
    write_home(home / ".shellgpt", config, env, pool_auth("pool-host", fleet.keys["pool"]))

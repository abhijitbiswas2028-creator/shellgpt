"""Dashboard embedded chat: the ``/api/pty`` WebSocket drives a REAL ``shellgpt --tui`` end to end.

Issue class: the dashboard Chat tab talking to the wrong profile, or losing the conversation on a
reconnect. Every cell runs the real ``shellgpt dashboard``, which
...
Contract under test (``shellgpt_cli/web_routers/chat_ws.py::pty_ws`` + ``shellgpt_cli/pty_session.py``):
  * ``?profile=<name>`` scopes the whole chat to that profile (SHELLGPT_HOME=<profile dir>);
...
def _require_tui() -> None:
    if shutil.which("node") and (H.REPO_ROOT / "ui-tui" / "dist" / "entry.js").is_file():
        return
    if os.environ.get("SHELLGPT_E2E_REQUIRE_TUI") == "1":
        pytest.fail("ui-tui/dist/entry.js or node missing but SHELLGPT_E2E_REQUIRE_TUI=1")
...
class Chat:
    ...
    def wait_composer(self, term: WsTerm) -> None:
        ...
        reports a boot phase ("summoning shellgpt…", "starting agent…"). Profile-agnostic on purpose:
        ...
        def ready() -> bool:
            ...
            return term.closed or ("❯" in s and "Session:" in s
                                   and "startingagent" not in s and "summoningshellgpt" not in s)
        try:
...
def _private_tui(tmp_path: Path) -> Path:
    ...
    ``SHELLGPT_TUI_DIR`` (the prebuilt-install path of ``_make_tui_argv``). On a source checkout every
...
def dash_env(tmp_path: Path):
    ...
    dash = H.Dashboard(sb, tmp_path / "dashboard.log", extra_env={"SHELLGPT_TUI_DIR": str(tui_dir)})

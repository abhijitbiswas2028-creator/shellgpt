"""Lane-private harness for the dashboard (``shellgpt dashboard``) E2E suite.

Every scenario drives the REAL web server as a child process: ``python -m shellgpt_cli.main dashboard
--no-open --skip-build --port 0`` with HOME=<tmp>/home and SHELLGPT_HOME=<tmp>/home/.shellgpt (profiles
...
it as ``X-ShellGPT-Session-Token`` on REST and ``?token=`` on WebSocket upgrades, and appends
...
TOKEN_HEADER = "X-ShellGPT-Session-Token"
...
_STRIP_PREFIXES = ("SHELLGPT_", "OPENAI", "ANTHROPIC", "OPENROUTER", "AWS_", "AZURE_", "GOOGLE_", "GEMINI",
                   "PYTEST_", "NOUS_", "XAI_", "LLM_", "CUSTOM_", "TERMINAL_", "DASH_")
_TOKEN_RE = re.compile(r'__SHELLGPT_SESSION_TOKEN__\s*=\s*"([^"]+)"')
_READY_RE = re.compile(r"SHELLGPT_DASHBOARD_READY port=(\d+)")
...
def hermetic_env(home: Path, extra: dict[str, str] | None = None) -> dict[str, str]:
    """Child env: fake HOME (profile-root anchor) + SHELLGPT_HOME under it, nothing credential-shaped."""
    home = home.resolve()
    assert home != real_user_home() and not str(home).startswith(str(real_user_home() / ".shellgpt" / "profiles")), home
    ...
    env.update(
        HOME=str(home),
        SHELLGPT_HOME=str(home / ".shellgpt"),
        XDG_STATE_HOME=str(home / ".local" / "state"),
        PYTHONPATH=str(REPO_ROOT),
        NO_COLOR="1",
        NO_PROXY="127.0.0.1,localhost",
        no_proxy="127.0.0.1,localhost",
        # The live-DB guard treats $HOME/.shellgpt/state.db of a pytest descendant as production;
...
class Profile:
    ...
    name: str
    home: Path  # the profile's SHELLGPT_HOME
...
def make_sandbox(root: Path, names: tuple[str, ...] = ("default",),
    ...
    """Launch profile at HOME/.shellgpt, the rest under profiles/<name>; each owns a started provider
    that accepts only its own key, so every recorded request proves WHO sent it."""
    shellgpt_home = root / "home" / ".shellgpt"
    profiles: dict[str, Profile] = {}
    _reaper.adopt_orphans()  # before any spawn: every detached descendant stays reapable
    for name in names:
        home = shellgpt_home if name == "default" else shellgpt_home / "profiles" / name
    ...
    (root / "web_dist" / "index.html").write_text(
        "<!doctype html><html><head><title>shellgpt</title></head><body><div id=root></div></body></html>",
        encoding="utf-8")
    return sb
...
def _assert_profiles_root_under(sb: Sandbox) -> None:
    ...
    probe = subprocess.run(
        [sys.executable, "-c", "from shellgpt_cli.profiles import _get_profiles_root as r; print(r())"],
...
class Dashboard:
    """``shellgpt dashboard`` bound to 127.0.0.1:<ephemeral>, token scraped from index.html."""
    ...
    def __init__(self, sb: Sandbox, log_path: Path, extra_env: dict[str, str] | None = None,
        ...
        env = sb.env({"SHELLGPT_WEB_DIST": str(sb.root / "web_dist"), **(extra_env or {})})
        self.proc = subprocess.Popen(
            [sys.executable, "-m", "shellgpt_cli.main", "dashboard", "--no-open", "--skip-build",
        ...
        self.port = poll(lambda: port_box[0] if port_box else (self.proc.poll() is not None and -1), 120,
                         "shellgpt dashboard to report its port")
        assert self.port > 0, f"shellgpt dashboard exited rc={self.proc.returncode}:\n{self.log_tail()}"
...
def run_py(sb: Sandbox, code: str, *args: str, shellgpt_home: Path | None = None,
           timeout: float = 120.0) -> subprocess.CompletedProcess:
    extra = {"SHELLGPT_HOME": str(shellgpt_home)} if shellgpt_home else None
...
def seed_sessions(sb: Sandbox, p: Profile, prefix: str, n: int) -> None:
    r = run_py(sb, SEED_SESSIONS, prefix, str(n), shellgpt_home=p.home)

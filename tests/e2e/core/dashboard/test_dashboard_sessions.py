...
agent, gateway and cron write). Harness: the real ``shellgpt dashboard`` process and a real writer
process (``shellgpt_state.SessionDB``, the agent's own persistence layer) appending sessions and
...
class Writer:
    def __init__(self, sb: H.Sandbox, p: H.Profile) -> None:
        self.proc = subprocess.Popen(
            [sys.executable, "-c", _WRITER, str(WRITE_SECONDS)], env=sb.env({"SHELLGPT_HOME": str(p.home)}),

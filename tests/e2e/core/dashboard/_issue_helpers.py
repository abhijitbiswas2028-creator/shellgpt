...
* ``PtyDashboard``: the lane's ``Dashboard`` harness, but spawned with a pseudo-terminal slave as
  stdin — the shape of ``shellgpt serve`` / ``shellgpt dashboard`` launched from an interactive
...
class PtyDashboard(Dashboard):
    """``shellgpt dashboard`` whose stdin is a pty slave (interactive-terminal launch); ``tty=False``
    ...
    def __init__(self, sb: Sandbox, log_path: Path, extra_env: dict[str, str] | None = None,
        ...
        env = sb.env({"SHELLGPT_WEB_DIST": str(sb.root / "web_dist"), "TERM": "xterm-256color",
                      **(extra_env or {})})
        try:
        ...
        try:
            self.port = poll(lambda: port_box[0] if port_box else (self.proc.poll() is not None and -1), 120,
                             "shellgpt dashboard (tty stdin) to report its port")
...
class GatewayApiServer:
    ...
    def __init__(self, sb: Sandbox, shellgpt_home: Path, log_path: Path, ready_timeout: float = 120.0) -> None:
        self.sb, self.shellgpt_home, self.log_path = sb, shellgpt_home, log_path
        ...
        for _ in range(PORT_ATTEMPTS):
            ...
            self.proc = subprocess.Popen(
                [sys.executable, "-m", "gateway.run"], cwd=str(sb.home), env=sb.env({"SHELLGPT_HOME": str(shellgpt_home)}),

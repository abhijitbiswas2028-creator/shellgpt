"""Entrypoint driver for the parity matrix: the ACP stdio server (``shellgpt acp``).
...
from tests.e2e.core.parity._helpers import TURN_TIMEOUT, DriveResult, ParityHome, shellgpt_argv, terminate
from tests.fakes.fake_llm_provider import FakeLLMServer

ACP_TOOLSET = "shellgpt-acp"  # acp_adapter/session.py::_expand_acp_enabled_toolsets default
...
def drive_acp(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    with open(stderr_path, "w", encoding="utf-8") as stderr_fh:
        proc = subprocess.Popen(
            shellgpt_argv("acp"), cwd=ph.project, env=ph.env(), stdin=subprocess.PIPE,

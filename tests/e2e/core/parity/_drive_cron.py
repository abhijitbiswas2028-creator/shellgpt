"""Entrypoint driver for the parity matrix: cron run-now (``shellgpt cron run <id>``).

Follows the driver contract in ``_drive_cli``. ``shellgpt cron create`` writes the
job (``--deliver local``: no platform; ``--workdir``: cron's documented
per-job cwd/context channel, see ``shellgpt cron create --help``), then
``shellgpt cron run <id>`` executes it. With no gateway owning the store the CLI
runs the job synchronously through ``cron.scheduler.run_job`` — the same agent
build the ticker uses (``shellgpt_cli/cron.py::_job_action`` forces the
...
from tests.e2e.core.parity._helpers import TURN_TIMEOUT, DriveResult, ParityHome, shellgpt_argv
from tests.fakes.fake_llm_provider import FakeLLMServer

# cron/scheduler.py::_resolve_cron_enabled_toolsets -> _get_platform_tools(cfg, "cron") default.
CRON_TOOLSET = "shellgpt-cron"
...
def _cron(ph: ParityHome, *args: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        shellgpt_argv("cron", *args), cwd=ph.home, env=ph.env(), capture_output=True, text=True,
...
def drive_cron(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    assert created.returncode == 0 and match, (
        f"shellgpt cron create exited {created.returncode}: {created.stdout[-1000:]} {created.stderr[-2000:]}")
    job_id = match.group(1)

    ran = _cron(ph, "run", job_id)
    assert ran.returncode == 0, f"shellgpt cron run exited {ran.returncode}: {ran.stderr[-2000:]}"
    ...
    assert "Ran now:" in ran.stdout, f"shellgpt cron run did not execute synchronously: {ran.stdout[-1000:]}"

    outputs = sorted((ph.shellgpt_home / "cron" / "output" / job_id).glob("*.md"))
    final_text = None
    if outputs:

"""Shared fixture SHELLGPT_HOME + invariant checks for the entrypoint-parity suite.
...
class ParityHome:
    ...
    def env(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        """Hermetic env for a subprocess ShellGPT: fake HOME, no real credentials."""
        ...
        # A tmp_path under ``~/.shellgpt/cache/scratch`` (TMPDIR when ShellGPT itself runs the suite)
        # is fine: the child's HOME is the fixture home, so its ``~/.shellgpt`` never resolves there.
        real_root = Path(pwd.getpwuid(os.getuid()).pw_dir, ".shellgpt").resolve()
        fixture = self.shellgpt_home.resolve()
        assert fixture != real_root and fixture.parent != real_root / "profiles", (
            f"fixture SHELLGPT_HOME {self.shellgpt_home} is the real install's live home")
        assert fixture == (self.home / ".shellgpt").resolve(), (
            f"fixture SHELLGPT_HOME {self.shellgpt_home} is not <fixture HOME>/.shellgpt")
        # Allowlist, not denylist: the runner may itself be a ShellGPT process whose
        # TERMINAL_CWD / SHELLGPT_* / credential env would silently reroute the child.
        env = {
        ...
        env.update({
            "HOME": str(self.home),
            "SHELLGPT_HOME": str(self.shellgpt_home),
            ...
            # The child's HOME is the fixture home, so its ``~/.shellgpt/state.db`` IS
            # the tmp SHELLGPT_HOME's db; under a pytest ancestor the live-DB guard
            # (shellgpt_state_guard) would refuse it. This is the guard's documented
...
def build_parity_home(root: Path, base_url: str, *, grandchild: bool = True,
    ...
    (shellgpt_home / "config.yaml").write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")

    hooks_dir = shellgpt_home / "agent-hooks"
    hooks_dir.mkdir()
    ...
    plugin_dir = shellgpt_home / "plugins" / PLUGIN_NAME
    ...
    skill_dir = shellgpt_home / "skills" / "parity-skill"
    ...
    mem_dir = shellgpt_home / "memories"
    mem_dir.mkdir()
...
def check_invariants(obs: Observation, ph: ParityHome, *, toolset: str = "shellgpt-cli",

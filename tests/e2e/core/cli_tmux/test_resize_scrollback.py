...
A real ``shellgpt chat --cli`` runs in a private tmux server against the scripted fake provider.
...
def test_resizes_keep_each_transcript_line_once_in_tmux_scrollback(tmp_path: Path) -> None:
    sock = f"shellgpt-e2e-{uuid.uuid4().hex[:8]}"
    home = tmp_path / "home"
    (tmp_path / "work").mkdir()
    ...
    with llm:
        write_shellgpt_home(home / ".shellgpt", llm.base_url)
        env = {k: v for k, v in os.environ.items() if not k.startswith(("SHELLGPT_", "TMUX"))}
        env.update(HOME=str(home), SHELLGPT_HOME=str(home / ".shellgpt"), PYTHONPATH=str(REPO_ROOT),
                   TERM="xterm-256color", PYTHONFAULTHANDLER="1")
        argv = [sys.executable, "-m", "shellgpt_cli.main", "chat", "--cli", "--yolo"]
        ...
        try:
            ...
            wait_for("Welcome to ShellGPT", timeout=120)

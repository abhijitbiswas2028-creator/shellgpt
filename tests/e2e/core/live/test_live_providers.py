...
(``LIVE-USAGE {...}``) and appended to ``$SHELLGPT_LIVE_USAGE_FILE`` when set.
"""
...
def live_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    # $HOME/.shellgpt as production), so nothing can resolve the developer's real home.
    home = tmp_path / "home"
...
def _live_listing(provider: str, runtime: dict) -> list[str]:
    ...
    if provider == "nous":
        from shellgpt_cli.auth import fetch_nous_models

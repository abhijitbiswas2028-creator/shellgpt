...
Real chain: a real ``shellgpt chat --cli`` / ``shellgpt --tui`` process (the TUI spawns its real Node
...
from tests.e2e.core.terminal._pty import REPO_ROOT, PtyShellGPT, canon
...
def test_terminal_transcript_integrity(surface: str, scenario: str, tmp_path: Path) -> None:
    ...
    with FakeLLMServer(script, aux=lambda _req: Text(TITLE)) as llm:
        term = PtyShellGPT(tmp_path, SURFACES[surface], llm, rows=rows, cols=cols)
        try:
            term.wait_ready()
            expected_main = 0

"""Real ``shellgpt serve`` backend + WebSocket JSON-RPC clients for the terminal lane E2E suites.

One backend per test module: a real ``python -m shellgpt_cli.main serve --port 0`` subprocess (the
exact argv Desktop spawns, see ``apps/desktop/electron/backend-command.ts``) with an isolated
HOME/SHELLGPT_HOME, no inherited provider credentials, and only the recording fake LLM provider
...
from tests.fakes.fake_llm_provider import FakeLLMServer, write_shellgpt_home

REPO_ROOT = Path(__file__).resolve().parents[4]
READY_RE = re.compile(r"SHELLGPT_BACKEND_READY port=(\d+)")
...
def _minimal_env(home: Path, shellgpt_home: Path, token: str, extra: dict[str, str] | None) -> dict[str, str]:
    """A clean child env: nothing ShellGPT-, provider- or credential-shaped leaks in from the test runner."""
    env = {k: os.environ[k] for k in ("PATH", "LANG", "LC_ALL", "TERM", "SYSTEMROOT") if k in os.environ}
    tmpdir = home.parent / "tmp"
    tmpdir.mkdir(parents=True, exist_ok=True)
    env.update(
        HOME=str(home), SHELLGPT_HOME=str(shellgpt_home), PYTHONPATH=str(REPO_ROOT),
        TMPDIR=str(tmpdir), SHELLGPT_DASHBOARD_SESSION_TOKEN=token, PYTHONUNBUFFERED="1",
        NO_COLOR="1",
        # The child's HOME *is* the sandbox, so its "real" root (expanduser('~')/.shellgpt) is the tmp
...
class Backend:
    """One real ``shellgpt serve`` process wired to a :class:`FakeLLMServer`."""
    ...
    def start(self, *, timeout: float = 90.0) -> "Backend":
        operator_root = (Path(_operator_home()) / ".shellgpt").resolve()
        assert operator_root not in (self.shellgpt_home.resolve(), *self.shellgpt_home.resolve().parents), (
            f"sandbox {self.shellgpt_home} sits inside the operator's ShellGPT home {operator_root}")
        self.llm.start()
        write_shellgpt_home(self.shellgpt_home, self.llm.base_url, extra_config=self.extra_config)
        ...
        self.proc = subprocess.Popen(
            [sys.executable, "-m", "shellgpt_cli.main", "serve", "--host", "127.0.0.1", "--port", "0"],
            cwd=str(self.root), env=_minimal_env(self.home, self.shellgpt_home, self.token, self.extra_env),
        ...
        def ready() -> int | None:
            if proc.poll() is not None:
                raise AssertionError(f"shellgpt serve exited {proc.returncode} before ready:\n{self.logs()}")
            match = READY_RE.search((self.root / "serve.stdout.log").read_text(errors="replace"))
            return int(match.group(1)) if match else None

        try:
            self.port = poll_until(ready, timeout=timeout, interval=0.1, what="SHELLGPT_BACKEND_READY")
    ...
    def logs(self, tail: int = 60) -> str:
        ...
        for path in (self.root / "serve.stdout.log", self.root / "serve.stderr.log",
                     self.shellgpt_home / "logs" / "errors.log"):
            name = path.name
            if path.exists():
    ...
    def db_rows(self, sql: str, args: Iterable[Any] = ()) -> list[tuple]:
        path = self.shellgpt_home / "state.db"
        if not path.exists():
            return []

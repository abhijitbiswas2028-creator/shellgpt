"""Drive a real ``shellgpt`` CLI / Ink TUI process under a real PTY, rendered through :mod:`._vt`.

The child runs with an isolated HOME/SHELLGPT_HOME (only the fake LLM provider configured, no
...
class PtyShellGPT:
    """One interactive ``shellgpt`` process on a PTY with an emulated screen."""
    ...
    def __init__(self, root: Path, argv_tail: list[str], llm: FakeLLMServer, *, rows: int, cols: int,
        ...
        operator = (_operator_home() / ".shellgpt").resolve()
        assert operator not in (self.shellgpt_home.resolve(), *self.shellgpt_home.resolve().parents), (
            f"sandbox {self.shellgpt_home} sits inside the operator's ShellGPT home")
        write_shellgpt_home(self.shellgpt_home, llm.base_url, extra_config=extra_config)
        self.llm = llm
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1", SHELLGPT_STATE_DB_GUARD_BYPASS="1",
        )
        ...
        self.proc = subprocess.Popen(
            [sys.executable, "-c", _SESSION_LEADER, sys.executable, "-m", "shellgpt_cli.main", *argv_tail],
    ...
    def wait_for_text(self, needle: str, timeout: float = 60.0) -> None:
        ...
        assert self.proc.poll() is None, f"shellgpt exited ({self.proc.returncode}) waiting for {needle[:40]!r}\n{self.dump()}"
    ...
    def wait_ready(self, timeout: float = 120.0) -> None:
        ...
        while True:
            ...
            assert self.proc.poll() is None, f"shellgpt exited ({self.proc.returncode}) during startup\n{self.dump()}"
    ...
    def exit(self, command: str = "/exit", timeout: float = 60.0) -> int:
        ...
        if self.proc.poll() is None:
            raise AssertionError(f"shellgpt did not exit within {timeout:.0f}s of {command}\n{self.dump()}")
    ...
    def wait_turns_persisted(self, n_replies: int, timeout: float = 90.0) -> None:
        ...
        assert self.proc.poll() is None, f"shellgpt exited ({self.proc.returncode}) mid-turn\n{self.dump()}"
    ...
    def persisted_messages(self) -> list[tuple[str, str, str]]:
        ...
        db = self.shellgpt_home / "state.db"
        if not db.exists():
            return []

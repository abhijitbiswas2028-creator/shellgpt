"""Worker-side board contracts through the real dispatcher and a real ``shellgpt chat -q`` worker.
...
def _artifact_location(board: Board, tid: str, where: str) -> Path:
    workspace = board.shellgpt_home / "kanban" / "workspaces" / tid
    return {"inside": workspace / "report.md",
            "outside": board.shellgpt_home / "scripts" / f"{tid}-deliverable.md"}[where]
    ...
    with FakeLLMServer(responder) as srv:
        ...
        try:
            ...
            if attached:
                ...
                assert not stored[0].is_relative_to(board.shellgpt_home / "kanban" / "workspaces"), stored
        finally:
            board.kill_workers()
...
def _seed_skill(board: Board, name: str) -> None:
    skill = board.shellgpt_home / "skills" / name
...
def test_worker_with_unresolvable_pinned_skill_still_starts_its_session(tmp_path) -> None:
    with FakeLLMServer(_completing_responder) as srv:
        ...
        try:
            ...
            shutil.rmtree(board.shellgpt_home / "skills" / "e2e-archived")
            _run_one_card(board, tid)

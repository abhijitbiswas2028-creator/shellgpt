...
Real processes end to end: every tick is a real ``shellgpt kanban dispatch`` process, every worker a
real ``shellgpt chat -q`` process spawned by it, talking to the recording fake provider (the only
...
Flow under test (``SHELLGPT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS=0``, ``agent.api_max_retries: 1``):
...
def _seed_review_skill(board: Board) -> None:
    skill = board.shellgpt_home / "skills" / "devops" / REVIEW_SKILL

...
A real ``shellgpt kanban dispatch`` process is SIGKILLed at three points of a tick — right after it
claimed a card (before the spawn), right after it spawned a worker, and immediately after launch —
then plain ticks run until the board drains. Workers are real ``shellgpt chat -q`` processes (they
...
def _kill_dispatcher_after(board: Board, kind: str | None) -> None:
    ...
    proc = subprocess.Popen(
        [PY, "-m", "shellgpt_cli.main", "kanban", "dispatch", "--json"], cwd=str(board.root),
...
def test_dispatcher_sigkill_mid_tick_never_destroys_or_duplicates_cards(tmp_path) -> None:
    ...
    with FakeLLMServer(completer) as srv:
        board = Board(tmp_path, srv.base_url, env_extra={"SHELLGPT_KANBAN_CLAIM_TTL_SECONDS": "3"})
        try:
            created = {}
...
def test_ttl_expiry_extends_a_live_hung_workers_claim_instead_of_respawning(tmp_path) -> None:
    ...
    with FakeLLMServer(model) as srv:
        board = Board(tmp_path, srv.base_url, env_extra={"SHELLGPT_KANBAN_CLAIM_TTL_SECONDS": str(ttl)})
        try:

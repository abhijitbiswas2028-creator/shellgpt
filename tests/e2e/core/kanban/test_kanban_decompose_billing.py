"""Auto-decompose billing bounds, proven against a REAL ``shellgpt gateway run`` embedded dispatcher.
...
# The gateway child is isolated (scratch HOME/SHELLGPT_HOME, own lock dir) and reaped by PID below.
pytestmark = [
...
def gateway(b: Board) -> Iterator[subprocess.Popen]:
    env = b.env()
    # The child's HOME is itself the scratch board root, so ~/.shellgpt IS the temp home here; the
    # inherited live-system guard would refuse it (same as the delivery/tenancy gateway children).
    env.update({"SHELLGPT_GATEWAY_LOCK_DIR": str(b.root / "gw-locks"), "PYTHONUNBUFFERED": "1",
                "SHELLGPT_STATE_DB_GUARD_BYPASS": "1"})
    log_path = b.root / "gateway.log"
    with open(log_path, "wb") as log:
        proc = subprocess.Popen([PY, "-m", "shellgpt_cli.main", "gateway", "run"], cwd=str(b.root),
...
def test_huge_decompose_reply_is_bounded(tmp_path: Path) -> None:
    ...
    Driven through the real ``shellgpt kanban decompose`` CLI (same decompose_task path)."""
    model = AuxModel()
    with FakeLLMServer(aux=model) as srv:

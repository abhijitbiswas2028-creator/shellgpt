...
the ACP / JSON-RPC 2.0 error space or dies mid-stream. Each row drives one real ``shellgpt chat -q`` and
...
KNOWN: dict[str, tuple[str, str]] = {
    "auth_remedy": (r"^remedy 'shellgpt [^']+' is not implemented for copilot-acp: ",
                    "#121290 copilot-acp auth failure tells the user to run a shellgpt command that is not implemented"),
}
...
REMEDY_RE = re.compile(r"`(shellgpt [^`]+)`")
...
def test_auth_failure_remedy_is_an_actionable_command(outcomes):
    ...
    ``shellgpt ...`` command it names has to be implemented for this provider."""
    ...
    for command in REMEDY_RE.findall(out.run.stdout):
        argv = [sys.executable, "-m", "shellgpt_cli.main", *command.split()[1:]]

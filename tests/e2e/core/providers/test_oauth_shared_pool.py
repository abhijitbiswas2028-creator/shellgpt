"""Two real ``shellgpt -z`` processes share one auth.json pool row and hit its expiry at the same moment.
...
Everything is real ShellGPT (pool, persistence, locking, the Anthropic OAuth
...
def test_concurrent_refresh_across_processes_spends_the_token_once(tmp_path) -> None:
    ...
    box: dict = {}
    thread_a = threading.Thread(target=lambda: box.setdefault("a", run_shellgpt(
    ...
    try:
        ...
        proc_b = run_shellgpt(rig.fh, ["-z", "PROCESS-BRAVO say hi"], extra_env=rig.child_env, timeout=150)
        thread_a.join(150)
    finally:

...
``provider: copilot-acp`` makes ShellGPT spawn an external ACP agent (``copilot --acp --stdio``) per model
...
ACP schema and replays scripted turns. Everything on the ShellGPT side is real: the ``shellgpt chat -q``
process, runtime resolution from ``config.yaml`` + the profile ``.env`` (``SHELLGPT_COPILOT_ACP_COMMAND``
/ ``SHELLGPT_COPILOT_ACP_ARGS``), the ACP client, the agent loop, the ``read_file`` tool and ``state.db``.
...
* each model call is a fresh agent process: ``initialize`` -> ``session/new`` (absolute cwd) ->
  ...
  schema-valid;
* ACP has no tools channel: ShellGPT' tools travel in the prompt text, a ``<tool_call>`` block in the
  agent's message runs a REAL ShellGPT tool, and the result is in the next call's prompt;
...
* agent-side ``session/request_permission`` is never granted (ShellGPT has no human channel there) and
...
# cross this absolute threshold mid-turn (ACP reports no usage, so ShellGPT estimates).
...
def _flow(root: Path) -> Scenario:
    """Turn 1 (two model calls): agent-side permission + a ShellGPT read_file call, then fs reads + the
    ...
    turns = [
        [acp.tool_call("agent-native-1", "run shell", kind="execute"), acp.permission("agent-native-1"),
         acp.message(acp.shellgpt_tool_call("call_rf_1", "read_file", {"path": str(project / "canary.txt")}))],
...
def _compaction(root: Path) -> Scenario:
    ...
    project = NativeHome(root).project
    turns = [[acp.thought(f"step {i}"), acp.message(acp.shellgpt_tool_call(
...
def test_shellgpt_tool_call_round_trips_through_acp_and_persists(outcomes):
    """A ``<tool_call>`` in the agent's message runs ShellGPT' real read_file; the result reaches the
...
def test_agent_permission_is_never_granted_and_fs_reads_stay_in_cwd(outcomes):
    ...
    assert outcomes_seen[0].get("outcome") != "selected", f"ShellGPT granted an agent-side permission: {outcomes_seen}"

...
vendor's catalog shape. No ShellGPT function is patched. Every other egress is pinned to a
...
_pairs = [p.split("=", 1) for p in os.environ.get("SHELLGPT_E2E_ORIGIN_REDIRECT", "").split(";") if "=" in p]
if _pairs:
...
def _home(tmp_path: Path, catalog: FakeCatalog, dead: socket.socket) -> _ShimHome:
    ...
    h.extra = {"PYTHONPATH": f"{tmp_path / 'shim'}:{REPO_ROOT}",
               "SHELLGPT_E2E_ORIGIN_REDIRECT": f"{VENDOR_ORIGIN}={catalog.origin}",

...
Real ``shellgpt chat -q`` turns against the fake Vertex (``tests/fakes/providers/vertex.py``), which
...
def run_compaction(tmp: Path) -> dict[str, Any]:
    ...
    setup = shellgpt_setup(fake, context_length=CONTEXT_LENGTH, extra_config={
...
def run_drop(tmp: Path) -> dict[str, Any]:
    ...
    nh = make_home(tmp / "drop" / "h", **shellgpt_setup(fake))
...
def test_stream_drop_retried_without_duplicate_content(results: dict[str, Any]) -> None:
    """The response after a signed tool step dies mid-body: ShellGPT resends the same valid request

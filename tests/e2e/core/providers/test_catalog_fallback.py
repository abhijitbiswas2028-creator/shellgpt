...
from tests.e2e.core.providers._catalog_helpers import (
    AUTH_HEADER_OF_DIALECT, FINAL, Known, Row, decoy_keys, discover_catalog, gate, known_gate, run_shellgpt, write_home,
)
...
def _drive(primary: Row, fallback: Row, root: Path) -> dict:
    ...
    with CatalogFake(fail_status=500, routes=primary.routes()) as dead, \
        ...
        proc = run_shellgpt(home, project, {**keys, **dead.proxy_env()}, "-z", "Read canary.txt and report.")

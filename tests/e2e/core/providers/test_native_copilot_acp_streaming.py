...
* ACP composition — ShellGPT itself served as an ACP agent (``shellgpt acp``) on top of the copilot-acp
...
KNOWN: dict[str, tuple[str, str]] = {
    ...
    "nested_acp": (r"^nested_acp: no agent chunk reached the surface during the [\d.]+s turn",
                   "#101507 shellgpt acp over copilot-acp forwards inner ACP chunks only after the inner turn ends"),
}
...
def _nested_acp(root: Path) -> Observed:
    ...
    return _run_child([sys.executable, "-m", "shellgpt_cli.main", "acp"], nh, fake, drive)

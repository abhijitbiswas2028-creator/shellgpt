...
* Discovery: with a custom Anthropic-protocol endpoint configured (``ANTHROPIC_BASE_URL``
  or ``model.base_url``), the picker catalog (``provider_model_ids``, behind ``shellgpt model``,
...
* OAuth wire aliases: on the Claude subscription route ShellGPT renames tools on the wire
  (``mcp__`` prefix, ``memory`` -> ``context_notes``, ``session_search`` ->
  ``chat_history_lookup``). A real ``shellgpt -z`` turn must map every wire name the model
...
# The CLI's own startup sequence (load ~/.shellgpt/.env), then the picker entry point.
_DISCOVERY_PROBE = (
    "import json; from shellgpt_cli.env_loader import load_shellgpt_dotenv; load_shellgpt_dotenv(); "
...
def _oauth_rig(rig, script) -> Rig:
    r = rig(script)
    (r.shellgpt_home / ".env").write_text(f"ANTHROPIC_TOKEN={OAUTH_TOKEN}\n", encoding="utf-8")
    return r
...
def test_oauth_wire_names_map_back_to_real_tools(rig) -> None:
    ...
    memory_md = r.shellgpt_home / "memories" / "MEMORY.md"

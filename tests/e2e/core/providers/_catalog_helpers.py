...
real ``python -m shellgpt_cli.main`` in a hermetic HOME against
...
def discover_catalog() -> list[Row]:
    ...
    if _CATALOG is None:
        with tempfile.TemporaryDirectory(prefix="catalog-discover-") as d:
            ...
            env.update(HOME=d, SHELLGPT_HOME=str(Path(d) / ".shellgpt"), PYTHONPATH=str(REPO_ROOT))
...
def hermetic_env(home: Path, extra: dict[str, str]) -> dict[str, str]:
    ...
    env.update({
        "HOME": str(home), "SHELLGPT_HOME": str(home / ".shellgpt"), "PYTHONPATH": str(REPO_ROOT),
...
def write_home(root: Path, model: dict[str, Any], extra_cfg: dict[str, Any] | None = None) -> Path:
    ...
    cfg = {"model": {"default": "catalog-model-a", "context_length": 128000, **model},
           ...
           **(extra_cfg or {})}
    (home / ".shellgpt" / "config.yaml").write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    return home


def run_shellgpt(home: Path, cwd: Path, env_extra: dict[str, str], *args: str,
    ...
    with tempfile.TemporaryFile("w+", encoding="utf-8") as out, tempfile.TemporaryFile("w+", encoding="utf-8") as err:
        proc = subprocess.Popen([sys.executable, "-m", "shellgpt_cli.main", *args], cwd=cwd,
...
def drive_turn(row: Row, root: Path, catalog: list[Row]) -> TurnResult:
    ...
    with CatalogFake(tool_args={"path": str(project / "canary.txt")}, final_text=FINAL, routes=row.routes()) as fake:
        ...
        proc = run_shellgpt(home, project, {**keys, **fake.proxy_env()}, "-z", "Read canary.txt and report.")

"""codex_app_server wire conformance: real ``shellgpt chat -q`` against a fake ``codex app-server``.

The fake (``tests/fakes/providers/codex_app_server.py``) speaks newline-delimited JSON-RPC over stdio,
validates every request/response ShellGPT sends against the codex-cli 0.147 app-server schema (serde-style
...
YOLO = ["--yolo"]
SEED_MARKER = "Prior conversation from this ShellGPT session"
...
def test_native_compaction_keeps_thread_and_transcript(runs):
    ...
    assert run.fake.requests("thread/compact/start") == [], "native mode: ShellGPT must not compact on top of codex"
    rows = _rows(run)

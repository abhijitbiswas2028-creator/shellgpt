...
ShellGPT' native ``anthropic`` provider runs with NO base URL override, i.e. the
...
is faked. Every child runs with an allowlisted env, ``HOME``/``SHELLGPT_HOME``
...
class Rig:
    ...
    def env(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        ...
        assert fixture != real_root and fixture.parent != real_root / "profiles", (
            f"fixture SHELLGPT_HOME {fixture} is the real install's live home")
        ...
        env.update({
            ...
            # The child's ~/.shellgpt/state.db IS the tmp home's db (see parity/_helpers.py).
    ...
    def config(self) -> dict[str, Any]:
        return yaml.safe_load((self.shellgpt_home / "config.yaml").read_text(encoding="utf-8"))

    def update_config(self, mutate: Callable[[dict[str, Any]], None]) -> None:
        cfg = self.config()
        mutate(cfg)
        (self.shellgpt_home / "config.yaml").write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")

    def run(self, *args: str, timeout: float = TURN_TIMEOUT, extra_env: dict[str, str] | None = None,
            ) -> subprocess.CompletedProcess:
        return subprocess.run(shellgpt_argv(*args), cwd=self.project, env=self.env(extra_env), capture_output=True,
    ...
    def log_tail(self, chars: int = 3000, pattern: str = "") -> str:
        ...
        log = self.shellgpt_home / "logs" / "agent.log"
        if not log.exists():
            return "<no agent.log>"
    ...
    def _db(self) -> sqlite3.Connection:
        return sqlite3.connect(f"file:{self.shellgpt_home / 'state.db'}?mode=ro", uri=True)
...
def become_subreaper() -> None:
    ...
    A ShellGPT child that daemonises a helper leaves it reparented to init, outside the test's
...
def start_rig(root: Path, script: list[Response] | Responder, *, config: dict[str, Any] | None = None,
    ...
    home = root / "home"
    rig = Rig(root=root, home=home, shellgpt_home=home / ".shellgpt", project=root / "project", srv=srv,
              proxy=proxy, ca_pem=ca.ca_pem)
    rig.shellgpt_home.mkdir(parents=True)
    rig.project.mkdir()
    cfg: dict[str, Any] = {
    ...
    (rig.shellgpt_home / "config.yaml").write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    (rig.shellgpt_home / ".env").write_text("ANTHROPIC_API_KEY=sk-ant-api03-e2e-fake-key\n", encoding="utf-8")
    return rig

...
Real ``shellgpt chat -q`` subprocesses with ``model.provider: bedrock`` talk to the real boto3
...
def _home(root: Path, fake: FakeBedrock, extra_config: dict[str, Any] | None = None) -> NativeHome:
    """Bedrock home: fake static creds in the profile .env (the chain ShellGPT loads), endpoint via env."""

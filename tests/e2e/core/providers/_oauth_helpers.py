...
Every ``shellgpt`` child runs with an allowlisted environment (no inherited
credentials or ``SHELLGPT_*``), ``HOME``/``SHELLGPT_HOME`` under ``tmp_path``, and
...
def make_home(root: Path) -> FakeHome:
    home = root / "home"
    (home / ".shellgpt").mkdir(parents=True, exist_ok=True)
    fh = FakeHome(root=root, home=home, shellgpt_home=home / ".shellgpt")
    fh.write_config({})
    return fh
...
def run_shellgpt(fh: FakeHome, args: list[str], *, extra_env: dict[str, str] | None = None,
               timeout: float = 120.0) -> subprocess.CompletedProcess:
    return subprocess.run(shellgpt_argv(*args), env=fh.env(extra_env), cwd=str(fh.root), stdin=subprocess.DEVNULL,
                          capture_output=True, text=True, timeout=timeout)


def spawn_shellgpt(fh: FakeHome, args: list[str], *, extra_env: dict[str, str] | None = None,
                 log: Path) -> subprocess.Popen:
    out = open(log, "w", encoding="utf-8")  # noqa: SIM115 - closed when the child is reaped
    try:
        return subprocess.Popen(shellgpt_argv(*args), env=fh.env(extra_env), cwd=str(fh.root),

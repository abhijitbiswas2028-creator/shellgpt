"""Chat-completions provider faults through REAL ``shellgpt -z`` processes.

The vendor boundary is ``FakeChatVariantServer`` (a plain ``provider: custom``
chat-completions endpoint); everything between the CLI and that socket is real ShellGPT.

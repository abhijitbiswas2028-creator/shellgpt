...
ShellGPT compacts mid-turn (tiny ``compression.threshold_tokens``) before the next step, then the
...
def run(tmp_path_factory: pytest.TempPathFactory) -> Run:
    ...
    home = nh.make_home(root, shellgpt_model(context_length=64_000), env_file=SHELLGPT_ENV,

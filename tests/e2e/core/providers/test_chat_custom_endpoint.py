"""Custom / Ollama-style chat-completions endpoints through REAL ``shellgpt -z`` processes.
...
* Ollama's chat renderer refuses a payload with no ``user`` message (HTTP 500 ``no user
  ...
  refusal with a 500, which ShellGPT (correctly, it cannot tell) treats as a transient
...
def test_ollama_strict_endpoint_always_receives_the_user_turn(tmp_path) -> None:
    ...
    with FakeChatVariantServer(script, strict_user_turn=True) as srv:
        ...
        # The strict 500 is retryable to ShellGPT; no minutes-long auto-recovery on a regression.

...
The real ``shellgpt chat -q`` CLI talks to Vertex through the standard ``HTTPS_PROXY`` +
...
* ``default_toolset`` — a turn with ShellGPT' default toolsets (every tool schema goes to Vertex).
"""
...
def _home(tmp: Path, name: str, fake: FakeVertex) -> NativeHome:
    nh = make_home(tmp / name / "h", **shellgpt_setup(fake))
...
def test_default_toolset_schemas_accepted_by_vertex(results: dict[str, Any]) -> None:
    """With ShellGPT' default toolsets every tool declaration must survive Vertex's translation."""

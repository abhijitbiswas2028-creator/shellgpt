...
Tier (stated honestly): REAL ``shellgpt -z`` processes with ``model.provider: openai-codex``
and a seeded two-login pool in ``auth.json``. Two vendor boundaries are faked:

* the ChatGPT Codex Responses backend — ``SHELLGPT_CODEX_BASE_URL`` points at
...
* the OpenAI OAuth token endpoint. Its URL is a hard-coded ``https://`` constant with no
  ...
  layer, to a loopback token fake. No ShellGPT function is patched; everything from the pool
...
(fresh), login B is an independently authorized ``manual:device_code`` row (``shellgpt auth
...
_pairs = [p.split("=", 1) for p in os.environ.get("SHELLGPT_E2E_URL_REDIRECT", "").split(";") if "=" in p]
if _pairs:
    import httpx
...
def _env(tmp_path: Path, codex_url: str, token: FakeTokenServer, dead_port: int) -> dict[str, str]:
    ...
    return {"SHELLGPT_CODEX_BASE_URL": codex_url, "PYTHONPATH": f"{tmp_path / 'shim'}:{REPO_ROOT}",
            "SHELLGPT_E2E_URL_REDIRECT": f"{TOKEN_URL}={token.url}",
            **{k: proxy for k in ("HTTP_PROXY", "http_proxy", "HTTPS_PROXY", "https_proxy")},
            "NO_PROXY": "127.0.0.1,localhost", "no_proxy": "127.0.0.1,localhost"}


def _auth(h: Home) -> dict[str, Any]:
    return json.loads((h.shellgpt_home / "auth.json").read_text(encoding="utf-8"))
...
def _two_logins(tmp_path: Path, a: dict[str, str], b: dict[str, str], token: FakeTokenServer,
    ...
    """Two sequential real ``shellgpt -z`` turns against the seeded pool; auth.json after each."""
    runs, stores = [], []
...
def test_dead_shared_grant_retires_every_row_of_it(tmp_path) -> None:
    ...
    for run, store, rows in zip(runs, stores, pools):
        ...
        assert "shellgpt auth add openai-codex" in run.stdout, run.describe()

...
``enum``, a ``required`` entry naming no property). A real ``shellgpt chat -q`` turn declares it to
...
from tests.fakes.providers.gemini_native import SHELLGPT_ENV, Call, Calls, GeminiFake, Recorded, Text, shellgpt_model
...
def _scenario(root: Path, schema: dict[str, Any], base_url: str | None) -> Outcome:
    ...
    home = nh.make_home(root, shellgpt_model(base_url), env_file=SHELLGPT_ENV, extra_config=extra)

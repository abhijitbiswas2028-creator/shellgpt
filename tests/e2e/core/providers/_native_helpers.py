...
Every scenario drives the REAL ``shellgpt`` CLI (``python -m shellgpt_cli.main chat -q ... -Q``) as a
subprocess with a hermetic fake HOME / SHELLGPT_HOME, a config.yaml that selects a native provider,
...
What a test asserts: the NEXT wire request ShellGPT sends (captured by the fake), the persisted
``state.db`` rows, and the CLI's user-visible output — never ShellGPT source text.
"""
...
@dataclass
class NativeHome:
    """One hermetic fake HOME with ``HOME/.shellgpt`` as SHELLGPT_HOME and a project dir as cwd."""
    ...
    def env(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        """Allowlisted env: no inherited credentials, SHELLGPT_* or TERMINAL_* can reroute the child."""
        env = {
        ...
        env.update({
            ...
            "NO_COLOR": "1",
            "TERM": "dumb",
            # The child's ~/.shellgpt/state.db IS the tmp home's db; under a pytest ancestor the
...
def make_home(root: Path, model: dict[str, Any], *, env_file: dict[str, str] | None = None,
    ...
    nh = NativeHome(root)
    for d in (nh.shellgpt_home, nh.project):
    ...
    for key, value in (extra_config or {}).items():
        if isinstance(value, dict) and isinstance(cfg.get(key), dict):
            ...
            cfg[key] = value
    (nh.shellgpt_home / "config.yaml").write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    lines = [f"{k}={v}" for k, v in (env_file or {}).items()]
    (nh.shellgpt_home / ".env").write_text("\n".join(lines) + ("\n" if lines else ""), encoding="utf-8")
    return nh
...
def run_chat(nh: NativeHome, prompt: str, *, resume: str | None = None, env: dict[str, str] | None = None,
    ...
    """One real ``shellgpt chat -q`` turn (optionally ``--resume <id>``) against the fake provider."""
    argv = [sys.executable, "-m", "shellgpt_cli.main", "chat", "-q", prompt, "-Q", *args]
    if resume:
        argv += ["--resume", resume]

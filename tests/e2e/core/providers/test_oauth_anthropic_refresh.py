...
Real ``shellgpt -z`` process, native ``anthropic`` provider on the OAuth route
...
def _run_turn(tmp_path, *, aux_401: bool, clock_expired: bool):
    ...
    try:
        proc = run_shellgpt(rig.fh, ["-z", "Read the note file and report."], extra_env=rig.child_env, timeout=150)
    finally:

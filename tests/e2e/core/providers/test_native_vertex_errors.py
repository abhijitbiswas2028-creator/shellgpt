...
Each scenario is one ``shellgpt chat -q`` turn against its own fake (``tests/fakes/providers/vertex.py``);
...
def _run(tmp: Path, name: str, case: Case) -> dict[str, Any]:
    ...
    nh = make_home(tmp / name / "h", **shellgpt_setup(fake, extra_config={"agent": {"api_max_retries": case.max_attempts}}))
...
def test_rejected_bearer_refreshes_once_then_surfaces(results: dict[str, Any]) -> None:
    """Every bearer 401s: ShellGPT may re-mint and retry once, never loop, and shows the error once."""

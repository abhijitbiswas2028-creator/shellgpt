...
ShellGPT documents three file-tool boundaries (website/docs/user-guide/security.md, "File write safety"):

* ``SHELLGPT_WRITE_SAFE_ROOT``: ``write_file`` / ``patch`` may only land inside the listed roots.
* Protected paths: the ShellGPT-home ``.env`` / OAuth stores, ``~/.ssh`` keys and ``authorized_keys`` are
  never written; ``~/.ssh/config`` is approval-gated (a ``-q`` run has no approver, so it is blocked).
* Read denylist: ``auth.json`` / ``.env`` under the ShellGPT home and project-local ``.env`` files are never
...
direct spelling. Everything runs through the real agent tool path: ``shellgpt chat -q`` with the loopback
model issuing the tool calls; outcomes are read on disk and in the tool results ShellGPT sent back.
"""
...
# ShellGPT quarantines an unparseable auth.json as auth.json.corrupt at startup; the copy holds the same secrets.
...
class Layout:
    ...
    def __post_init__(self) -> None:
        ...
        self.ws, self.hh = self.home / "work", self.home / ".shellgpt"
    ...
    def plant(self, path: Path, label: str, *, json_store: bool = False) -> str:
        ...
        quarantined by ShellGPT at startup, which would move the canary elsewhere)."""
...
class Step:
    ...
    that cannot (there is one ShellGPT-home ``.env`` and one ``authorized_keys``) share it, and a breach of a
...
def _confined_steps(lay: Layout) -> list[Step]:
    """SHELLGPT_WRITE_SAFE_ROOT=<workspace>: every write whose resolved target is outside is refused.
    ...
    return [
        ...
        Step("read_quarantined_auth_copy", "read_file", {"path": "../.shellgpt/auth.json.corrupt"}, "hidden", quarantined),
        Step("search_quarantined_auth_copy", "search_files", {"pattern": token, "path": "../.shellgpt"}, "hidden",
             quarantined),
...
def _protected_steps(lay: Layout) -> list[Step]:
    ...
    return [
        ...
        Step("env_dotdot_patch", "patch", _replace("../.shellgpt/.env", env_mark), "unchanged", env),
        ...
        Step("read_webhooks_dotdot", "read_file", {"path": "../.shellgpt/webhook_subscriptions.json"}, "hidden",
...
def _run(root: Path, build: Callable[[Layout], list[Step]], expected: tuple[str, ...],
    ...
    def snapshot() -> None:  # after the home (incl. .env) is written, before ShellGPT starts
...
def confined(tmp_path_factory: pytest.TempPathFactory) -> Run:
    return _run(tmp_path_factory.mktemp("confined"), _confined_steps, CONFINED_SCENARIOS + QUARANTINE_SCENARIOS,
                lambda lay: {"SHELLGPT_WRITE_SAFE_ROOT": str(lay.ws)})

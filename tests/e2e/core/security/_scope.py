"""Private harness for ``test_profile_secret_scope.py``: one real ``shellgpt serve`` hosting two
...
Everything here writes plain files into the fake home; no ShellGPT code is imported or patched.
"""
...
def install_plugin(shellgpt_home: Path) -> None:
    """A user plugin with a backend API under the launch home's plugins dir."""
    dash = shellgpt_home / "plugins" / PLUGIN_ID / "dashboard"
    dash.mkdir(parents=True, exist_ok=True)
    (shellgpt_home / "plugins" / PLUGIN_ID / "plugin.yaml").write_text(

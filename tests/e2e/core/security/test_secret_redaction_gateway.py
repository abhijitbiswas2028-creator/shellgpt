...
def gw_world(tmp_path_factory) -> World:
    ...
    ctx = Ctx(keys, ws, root / "home" / ".shellgpt" / ".env")
    with FakeLLMServer(Director(ctx), api_key=keys.provider, record_get=True) as llm:
        ...
        write_home(gw.shellgpt_home, llm.base_url, api_key=keys.provider, env=keys.env(), config=CONFIG)
        gw.start()
        try:
            ...
            wait_until(lambda: (gw.shellgpt_home / "logs" / "gateway.log").exists(), "gateway.log written",

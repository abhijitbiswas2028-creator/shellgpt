"""Secrets never reach disk or the next provider request through the classic CLI (``shellgpt chat -q``).

Contract under test (``security.redact_secrets``, on by default; website/docs/user-guide/configuration.md
§ Security, shellgpt_logging.py, user-guide/sessions.md § Export Sessions):
...
* ``shellgpt sessions export --redact`` masks message content AND tool-call arguments.
...
from tests.e2e.core.security._helpers import BoundaryBreach, run_shellgpt, write_home
...
def cli_world(tmp_path_factory) -> World:
    ...
    ctx = Ctx(keys, ws, home / ".shellgpt" / ".env")
    with FakeLLMServer(Director(ctx), api_key=keys.provider, record_get=True) as llm:
        ctx.port = llm.port
        write_home(home / ".shellgpt", llm.base_url, api_key=keys.provider, env=keys.env(), config=CONFIG)
        runs: dict[str, str] = {}
        for name, scenario in SCENARIOS.items():
            r = run_shellgpt(["chat", "-q", prompt_for(name, keys), "-Q"], home, cwd=ws, timeout=150)
            ...
            if scenario.followup:
                ...
                r2 = run_shellgpt(["chat", "-q", prompt_for(name, keys, followup=True), "-Q", "--resume", sid.group(1)],

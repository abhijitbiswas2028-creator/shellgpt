...
* ``/memory`` and ``/skills`` ``approve|reject|diff <id>`` (#119997): the id names a record staged under
  ``<SHELLGPT_HOME>/pending/<subsystem>/``. A ``../`` or absolute id must not read, delete or apply any
  ...
  they do for a user.
* ``skill_manage delete`` (#120528) must refuse a pinned skill and the essential ``shellgpt-agent`` skill
  whatever spelling names it: bare (``my-skill``) or categorized (``research/my-skill``). The pin is set
  with the real ``shellgpt curator pin`` CLI; the deletes are real tool calls in real agent turns on the
  same backend (``shellgpt chat -q`` does not offer ``skill_manage``: one-shot runs hide it).
...
# scenario -> (slash command, where the victim lives). "home" victims are <SHELLGPT_HOME>/auth.json spelled
# ``../../auth``; "outside" victims live outside the ShellGPT home and are spelled by absolute path.
...
DELETES: dict[str, tuple[str, str]] = {  # scenario -> (name given to skill_manage, skill dir under skills/)
    ...
    "essential_by_category": ("autonomous-ai-agents/shellgpt-agent", "autonomous-ai-agents/shellgpt-agent"),
}
...
def store(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Store]:
    ...
    with FakeLLMServer([*script, Text("staged")], api_key=key) as srv:
        H.write_home(home / ".shellgpt", srv.base_url, api_key=key, config=_STORE_CONFIG)
        ...
        try:
            ...
            s = Store(b, sid, home / ".shellgpt", outside, ids[:3], memory_texts, ids[3:], skill_texts)
            assert s.pending_ids("memory") == set(ids[:3]) and s.pending_ids("skills") == set(ids[3:]), \
                f"pending store on disk does not match the staged ids: {sorted((s.shellgpt_home / 'pending').rglob('*'))}"
            yield s
        finally:
            b.close()
...
def _plant_victim(s: Store, where: str) -> tuple[Path, str, str]:
    ...
    victim, spelled = ((s.shellgpt_home / "auth.json", "../../auth") if where == "home"
...
_DELETE_STEPS: tuple[tuple[str, str, str], ...] = (
    ...
    ("control_essential_bare", "shellgpt-agent", "autonomous-ai-agents/shellgpt-agent"),
...
def deletes(tmp_path_factory: pytest.TempPathFactory) -> Iterator[DeleteRun]:
    ...
    home = root / "home"
    skills = home / ".shellgpt" / "skills"
    for rel in ("research/my-skill", "research/free-skill", "research/spare-skill", "autonomous-ai-agents/shellgpt-agent"):
    ...
    with FakeLLMServer([], api_key=key) as srv:
        H.write_home(home / ".shellgpt", srv.base_url, api_key=key)
        pin = H.run_shellgpt(["curator", "pin", "my-skill"], home, timeout=90)
        assert pin.returncode == 0, f"shellgpt curator pin failed: {pin.stdout}\n{pin.stderr[-2000:]}"
        b = TuiBackend(home, root / "tui.log")
        try:
            sid = b.ok("session.create", {})["session_id"]

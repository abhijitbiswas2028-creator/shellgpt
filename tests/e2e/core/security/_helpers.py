...
Every scenario runs real ShellGPT processes (``shellgpt`` CLI, ``shellgpt serve``, the gateway,
``tui_gateway``) with HOME=<tmp>/home and SHELLGPT_HOME=<tmp>/home/.shellgpt, every credential env var
...
def write_home(shellgpt_home: Path, base_url: str, *, api_key: str, config: str = "",
...
def run_shellgpt(argv: list[str], home: Path, *, timeout: float = 120.0, cwd: Path | None = None,
               extra_env: dict[str, str] | None = None, stdin: str | None = None) -> subprocess.CompletedProcess:
    """``python -m shellgpt_cli.main <argv>`` in its own process group; the group is always reaped."""
    proc = subprocess.Popen(
        [sys.executable, "-m", "shellgpt_cli.main", *argv], cwd=str(cwd or home), env=hermetic_env(home, extra_env),
...
def run_python(code: str, home: Path, *args: str, timeout: float = 120.0,
    ...
    """A fresh interpreter importing the worktree's ShellGPT under the hermetic env."""

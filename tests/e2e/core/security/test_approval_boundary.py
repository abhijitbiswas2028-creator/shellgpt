...
* CLI: ``shellgpt chat -q`` (single-query, nobody present to approve; ``approvals.mode: manual`` so
...
def _single_query_run(root: Path, single_query_mode: str) -> Dict[str, Any]:
    """One ``shellgpt chat -q`` whose model calls ``terminal`` once per variant, then the control."""
    ...
    with FakeLLMServer(script, api_key=key) as srv:
        write_home(home / ".shellgpt", srv.base_url, api_key=key,
                   config=MANUAL + f"  single_query_mode: {single_query_mode}\n")
        proc = run_shellgpt(["chat", "-q", "run the maintenance commands", "-Q"], home, timeout=180, cwd=victims)
        requests = srv.main_requests()
    assert proc.returncode == 0, f"shellgpt chat -q failed rc={proc.returncode}\n{proc.stdout[-2000:]}\n{proc.stderr[-2000:]}"

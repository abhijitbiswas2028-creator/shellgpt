...
``run_tool_calls`` drives the REAL agent tool path: one ``shellgpt chat -q`` process whose model (the
loopback fake) issues the given tool calls in order, then answers ``done``. Each tool result is read
back from the wire: the final request ShellGPT sent to the model carries every ``role: tool`` message,
...
def run_tool_calls(home: Path, calls: list[tuple[str, dict[str, Any]]], *, cwd: Path,
    ...
    """Run one ``shellgpt chat -q`` turn whose model issues ``calls`` sequentially; return each tool
    result text, in call order. ``prepare`` runs after the home (config.yaml + .env) is written and
    before ShellGPT starts (snapshot pre-run state there). Harness failures (non-zero exit, a lost tool
    ...
    with FakeLLMServer(script, api_key=key) as srv:
        H.write_home(home / ".shellgpt", srv.base_url, api_key=key, config=config, env=env_lines)
        if prepare is not None:
            prepare()
        proc = H.run_shellgpt(["chat", "-q", "run the scripted tools", "-Q"], home, cwd=cwd,
                            extra_env=extra_env, timeout=timeout)
        reqs = srv.main_requests()
    assert proc.returncode == 0, f"shellgpt chat -q rc={proc.returncode}\n{proc.stdout[-2000:]}\n{proc.stderr[-4000:]}"

...
asserts are masked, so one ShellGPT home can serve every scenario and each sink is scanned once.
"""
...
from tests.e2e.core.security._helpers import REPO_ROOT, BoundaryBreach, db_blob, files_containing, run_shellgpt
...
class Secrets:
    """Every secret value one ShellGPT home sees; see the module docstring for the allocation."""
...
@dataclass
class Sinks:
    """Raw text of every persisted/egress sink of one ShellGPT home, keyed by sink name."""
...
def collect(home: Path, requests: list[dict], platform_journal: Path | None = None) -> Sinks:
    ...
    for flag, name in (([], "plain.jsonl"), (["--redact"], "redacted.jsonl")):
        r = run_shellgpt(["sessions", "export", str(out / name), *flag], home, timeout=90)
...
@dataclass
class World:
    """One ShellGPT home after every scenario ran: its sinks and each scenario's travel evidence."""
...
class LoggingGateway(GatewayProcess):
    ...
    ``start_gateway`` does (agent.log / errors.log / gateway.log under the child's SHELLGPT_HOME)."""

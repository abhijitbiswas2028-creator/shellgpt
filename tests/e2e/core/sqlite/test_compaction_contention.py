...
provider on ONE shared ``state.db`` — once per journal mode ShellGPT deploys (WAL, and the DELETE mode the
...
def rig(request, tmp_path_factory):
    ...
    write_shellgpt_home(ch.shellgpt_home, server.base_url, extra_config=MICRO_CONFIG + DB_CONFIG)
    tui_home = ch.root / "tui-home"
    write_shellgpt_home(tui_home / ".shellgpt", server.base_url, extra_config=DB_CONFIG)
    for home in (ch.shellgpt_home, tui_home / ".shellgpt"):
        cfg = home / "config.yaml"
        cfg.write_text(cfg.read_text(encoding="utf-8").replace(ctx, "  context_length: 64000\n"), encoding="utf-8")
    yield Rig(ch, server, aux, {"HOME": str(tui_home), "SHELLGPT_HOME": str(tui_home / ".shellgpt")})
    ch.shutdown()
    server.stop()

"""`shellgpt import` of a broken backup archive must report failure, never "restored".

The archive is made by the real ``shellgpt backup`` from a populated home, then damaged the ways
...
def _cli(root: Path, env: dict, *args: str):
    return H.run([PY, "-m", "shellgpt_cli.main", *args], env=env, cwd=root, writable=[root], timeout=300)
...
def test_import_that_skips_members_reports_incomplete(archive):
    ...
    locked.chmod(0o555)  # left behind root-owned by an earlier `sudo shellgpt ...`
    try:

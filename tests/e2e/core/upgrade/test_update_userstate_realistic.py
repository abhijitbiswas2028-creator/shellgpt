"""`shellgpt update` on a realistic user home: everything the user owns survives, sessions resume.
...
* three profiles (default + two made with ``shellgpt profile create``), each with a hand-edited
...
* a custom skill, a user plugin under ``~/.shellgpt/plugins/``, a cron job made by ``shellgpt cron``;
...
Upstream then publishes a new commit and the user runs ``shellgpt update --yes``. A second leg
...
_VERSIONS_PY = ("from shellgpt_cli.config_defaults import DEFAULT_CONFIG as D; "
                "from shellgpt_cli.config_migrations import SUPPORT_FLOOR_VERSION as F; "
                "print(D['_config_version'], F)")


def _profile_home(sb: I.Sandbox, name: str):
    return sb.shellgpt_home if name == "default" else sb.shellgpt_home / "profiles" / name
...
def test_update_on_a_realistic_home_exits_clean_at_the_new_commit(world):
    ...
    assert up.returncode == 0, "shellgpt update failed on a realistic home:\n" + I.describe(up)
...
def test_broken_profiles_warn_but_do_not_fail_the_update(world):
    ...
    assert up.returncode == 0, "a broken profile under ~/.shellgpt/profiles/ failed the whole update:\n" + I.describe(up)
    bad = str(sb.shellgpt_home / "profiles" / "badyaml" / "config.yaml")
    assert bad in out, "the update did not warn about the profile whose config.yaml is broken:\n" + I.describe(up)
    assert (sb.shellgpt_home / "profiles" / "badyaml" / "config.yaml").read_bytes() == world["snap"]["badyaml"], (

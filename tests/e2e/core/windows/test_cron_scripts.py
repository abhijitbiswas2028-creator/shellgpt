"""Cron script jobs on native Windows (``shellgpt cron create --script`` + ``shellgpt cron run``).

A no-agent script job runs its script through an interpreter picked by extension and
delivers stdout as the job result. ``shellgpt cron run <id>`` executes it synchronously
through ``cron.scheduler.run_job`` (the ticker's code path) when no gateway owns the store.

The ``.sh`` case launches ShellGPT with the PATH a NATIVE Windows process has (Start menu,
...
from tests.e2e.core.windows._helpers import KnownBugSymptom, WinHome, expect, shellgpt, make_home, nonce
...
def _run_script_job(home: WinHome, script: str, env_extra: dict[str, str] | None = None) -> tuple[dict, str]:
    created = shellgpt(home, "cron", "create", "1d", "--name", "win-script", "--script", script,
    ...
    ran = shellgpt(home, "cron", "run", job_id, cwd=home.profile, env_extra=env_extra)
    assert ran.returncode == 0 and "Ran now:" in ran.stdout, f"job did not run synchronously:\n{ran.tail()}"
    jobs = json.loads((home.shellgpt_home / "cron" / "jobs.json").read_text(encoding="utf-8"))["jobs"]
    job = next(j for j in jobs if j["id"] == job_id)
    outputs = sorted((home.shellgpt_home / "cron" / "output" / job_id).glob("*.md"))
...
def test_python_script_job_delivers_stdout(tmp_path: Path) -> None:
    ...
    scripts = home.shellgpt_home / "scripts"

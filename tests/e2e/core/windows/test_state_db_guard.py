...
``shellgpt sessions optimize-storage`` rewrites the store (FTS rebuild + VACUUM). Run under a
...
one: a real ``shellgpt gateway run`` for the same profile. Control: the same command succeeds
...
def _running(home) -> bool:
    try:
        state = json.loads((home.shellgpt_home / "gateway_state.json").read_text(encoding="utf-8"))
...
def test_optimize_storage_refuses_while_gateway_holds_store(tmp_path: Path) -> None:
    with FakeLLMServer() as srv:
        ...
        try:
            ...
            held = shellgpt(home, "sessions", "optimize-storage")
            refused = held.returncode != 0 and "Refusing" in held.stdout
            stop = shellgpt(home, "gateway", "stop")
            assert stop.returncode == 0, stop.tail()
            gw.wait(timeout=60)
            quiet = shellgpt(home, "sessions", "optimize-storage")

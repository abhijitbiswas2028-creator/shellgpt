...
Every test drives REAL ShellGPT processes (``shellgpt.exe`` / ``python -m
shellgpt_cli.main``) on a real Windows host against the recording loopback
provider (``tests/fakes/fake_llm_provider.py``). Nothing in ShellGPT is mocked;
...
``SHELLGPT_HOME`` is ``<profile>/.shellgpt``, so no child can read or write the
runner's real ShellGPT state.
"""
...
@dataclass
class WinHome:
    """One hermetic fake Windows user profile with a ShellGPT home inside it."""
...
def make_home(tmp_path: Path, base_url: str, *, extra_config: str = "") -> WinHome:
    ...
    shellgpt_home = profile / ".shellgpt"
    project = root / "project"
    project.mkdir(parents=True, exist_ok=True)
    write_shellgpt_home(shellgpt_home, base_url, extra_config=extra_config)
    home = WinHome(root=root, profile=profile, shellgpt_home=shellgpt_home, project=project)
...
def shellgpt_exe() -> Path:
    """The console-script shim users actually launch (``<venv>\\Scripts\\shellgpt.exe``)."""
    exe = Path(sys.executable).with_name("shellgpt.exe")
    assert exe.is_file(), f"shellgpt.exe console script missing next to {sys.executable}"
    return exe
...
def shellgpt(home: WinHome, *args: str, **kw: Any) -> Run:
    return run(shellgpt_argv(*args), home, **kw)
...
def tool_results(srv: FakeLLMServer) -> list[str]:
    """Tool-result messages ShellGPT sent back to the model, in order, deduped across requests."""
    seen: dict[str, str] = {}
...
def _owned_by(proc: Any, root: str, home: str) -> bool:
    """One process belongs to the fake profile if its SHELLGPT_HOME is that home, or its
    ...
    try:
        ...
        if "SHELLGPT_HOME" in env and os.path.normcase(os.path.normpath(env["SHELLGPT_HOME"])) == home:
            return True
...
def owned_processes(home: WinHome, *, since: float) -> list[Any]:
    ...
    shellgpt_home = os.path.normcase(os.path.normpath(str(home.shellgpt_home)))
    me = os.getpid()
    owned = []
    for proc in psutil.process_iter():
        try:
            ...
            continue
        if _owned_by(proc, root, shellgpt_home):
            owned.append(proc)
    return owned

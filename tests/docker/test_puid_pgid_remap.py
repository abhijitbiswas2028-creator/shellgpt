...
Build the real image and verify the actual runtime behavior:
  ...
  1. PUID/PGID env vars remap the shellgpt user UID/GID at boot
  2. SHELLGPT_UID/SHELLGPT_GID take precedence over PUID/PGID aliases
...
def test_puid_pgid_remaps_shellgpt_user(
    built_image: str, container_name: str,
) -> None:
    """PUID=1000 PGID=1000 must remap the shellgpt user to UID 1000."""
    ...
    assert r.stdout.strip() == "1000", (
        f"expected shellgpt UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected shellgpt GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
    ...
    assert "WRITE_OK" in r.stdout, (
        f"shellgpt user cannot write to /opt/data after remap: {r.stdout}"
    )
...
def test_nas_low_uid_accepted(
    ...
    r = docker_exec_sh(container_name, "id -u shellgpt", timeout=10)
    assert r.stdout.strip() == "99", (
        f"expected shellgpt UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g shellgpt", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected shellgpt GID 100, got: {r.stdout.strip()}"
    )

...
$SHELLGPT_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $SHELLGPT_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"shellgpt-restart-{safe}"
    volume = f"shellgpt-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "shellgpt", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "shellgpt", "profile", "create", "ghost").check_returncode()

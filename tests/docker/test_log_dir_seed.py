"""Runtime smoke test for Docker $SHELLGPT_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the shellgpt user after container boot.
...
registered later runs its log service as the dropped shellgpt user and
...
def test_logs_gateways_seeded_and_shellgpt_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by shellgpt after boot."""
    ...
    # Both must be owned by shellgpt
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=shellgpt" in r.stdout, (
        f"logs/ not owned by shellgpt: {r.stdout}"
    )
    assert "gateways=shellgpt" in r.stdout, (
        f"logs/gateways/ not owned by shellgpt: {r.stdout}"
    )

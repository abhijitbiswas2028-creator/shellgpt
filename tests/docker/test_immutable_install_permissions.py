...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$SHELLGPT_HOME" = "/opt/data" && '
        'test "$SHELLGPT_WRITE_SAFE_ROOT" = "/opt/data" && '
        'test "$SHELLGPT_DISABLE_LAZY_INSTALLS" = "1" && '
...
def test_shellgpt_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/shellgpt/agent/message_sanitization.py")
        try:

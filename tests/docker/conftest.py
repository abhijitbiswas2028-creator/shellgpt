...
Override the image with ``SHELLGPT_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SHELLGPT_TEST_IMAGE", "shellgpt-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SHELLGPT_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SHELLGPT_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged shellgpt user
# ---------------------------------------------------------------------------
#
# Background: every ShellGPT runtime path inside the container drops to UID
# 10000 (the ``shellgpt`` user) via ``s6-setuidgid shellgpt``. ``docker exec``
...
# fails (PermissionError swallowed) for shellgpt, so a test that ran as root
...
# helpers below run every probe under ``-u shellgpt`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: shellgpt).

...
The shim (docker/shellgpt-exec-shim.sh, installed at /opt/shellgpt/bin/shellgpt)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> shellgpt login` would write /opt/data/auth.json as
...
failed: ShellGPT is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> shellgpt …`` (defaulting to root) gets dropped to the
   shellgpt user before the real binary runs.
2. ``docker exec --user shellgpt <c> shellgpt …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $SHELLGPT_HOME from a ``docker exec`` session land
   as shellgpt:shellgpt — the actual user-visible invariant.
4. The SHELLGPT_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-shellgpt-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown shellgpt:shellgpt`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to shellgpt:shellgpt,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$SHELLGPT_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (shellgpt_cli.container_boot), which s6 runs
    *strictly after* ``01-shellgpt-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_shellgpt_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `shellgpt` with a Python-style `-c` shim equivalent — there's no
    pure-shellgpt "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `shellgpt` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `shellgpt`
    do anything that writes to disk, then check the file's owner.

    Use `shellgpt config set` which writes config.yaml under SHELLGPT_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "shellgpt", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by shellgpt, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "shellgpt:shellgpt", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected shellgpt:shellgpt. "
        "The shim did not drop privileges before invoking shellgpt."
    )
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> shellgpt login`` (root) wrote
    ...
    "ShellGPT is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `shellgpt config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is shellgpt-readable.

    Specifically: pretend the operator ran `shellgpt login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the shellgpt UID. We use `shellgpt auth list` since that touches the
    ...
    # `shellgpt auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `shellgpt config set` first to
    # provoke a write into SHELLGPT_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "shellgpt", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # SHELLGPT_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "shellgpt", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the shellgpt user "

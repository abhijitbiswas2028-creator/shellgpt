...
package-lock.json. Without SHELLGPT_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV SHELLGPT_TUI_DIR=/opt/shellgpt/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the shellgpt user, return stdout."""
    inner = (
        "source /opt/shellgpt/.venv/bin/activate && "
        "cd /opt/shellgpt && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the shellgpt user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "shellgpt", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from shellgpt_cli.main_tui_launch import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/shellgpt/ui-tui')\n"
    ...
    # With SHELLGPT_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE

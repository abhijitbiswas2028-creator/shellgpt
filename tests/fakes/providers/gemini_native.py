...
ShellGPT routes to its native Gemini adapter only for the real Google host
...
API_KEY = "AIzaFakeGeminiKeyForShellGPTE2E0000000000"
...
# ShellGPT-side config for a home that talks to this fake: the user-facing provider id + model and the
# key in ``.env`` (``model.base_url`` may pin another Google API version, e.g. ``.../v1``).
SHELLGPT_ENV = {"GEMINI_API_KEY": API_KEY}


def shellgpt_model(base_url: str | None = None, **extra: Any) -> dict[str, Any]:
...
def _write_tls_material(directory: Path) -> tuple[Path, Path, Path]:
    ...
    ca_name = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "shellgpt-e2e gemini fake CA")])
...
class GeminiFake:
    ...
    def child_env(self) -> dict[str, str]:
        """Env for the ``shellgpt`` child: route HTTPS through the proxy and trust the fake CA."""
        ca = str(self.ca_pem)
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_CONNECT(self) -> None:  # noqa: N802 - http.server naming
                ...
                if self.tunneled or host != GEMINI_HOST:
                    ...
                    self.send_response(403, "Forbidden by shellgpt e2e fake proxy")

...
ShellGPT' ``copilot-acp`` provider spawns an external agent process per model call and speaks the
...
* answers ``<cmd> --help`` with a usage text advertising ``--acp`` (ShellGPT probes it before spawning);
...
def shellgpt_tool_call(call_id: str, name: str, args: dict[str, Any]) -> str:
    """Text a model behind ACP emits to call a ShellGPT tool (ACP has no OpenAI tools channel)."""
...
class AcpFake:
    ...
    def env(self) -> dict[str, str]:
        """Env vars that point ShellGPT' copilot-acp client at this fake."""
        return {"SHELLGPT_COPILOT_ACP_COMMAND": str(self.launcher),
                "SHELLGPT_COPILOT_ACP_ARGS": f"--acp --stdio --state {self.state_dir}"}
    ...
    def main_prompts(self) -> list[dict[str, Any]]:
        """Main-turn ``session/prompt`` records (those carrying ShellGPT' tool bridge)."""
...
class Agent:
    """One spawned agent process (one ShellGPT model call)."""

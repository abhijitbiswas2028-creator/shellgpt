...
Two external boundaries ShellGPT does not own, both faked for real:
...
* **Vertex AI** at ``https://{region}-aiplatform.googleapis.com/v1beta1/projects/{project}/locations/
  {region}/endpoints/openapi/chat/completions``. ShellGPT has no base-URL override for Vertex, so the
  ...
  ``HTTPS_PROXY`` (the corporate-proxy channel ShellGPT documents). CONNECTs to any other host are
...
def make_tls_material(root: Path, hosts: list[str]) -> tuple[Path, Path, Path]:
    ...
    ca_cert = (
        x509.CertificateBuilder().subject_name(_name("shellgpt-e2e fake Google CA")).issuer_name(_name("shellgpt-e2e fake Google CA"))
...
def make_service_account(path: Path, token_uri: str, project_id: str) -> ServiceAccount:
    ...
    email = f"shellgpt-e2e@{project_id}.iam.gserviceaccount.com"
...
class FakeVertex:
    ...
    def child_env(self) -> dict[str, str]:
        """Standard proxy + CA-trust env for the ShellGPT child (no ShellGPT-specific knobs)."""
...
PROJECT = "shellgpt-e2e-proj"
SA_EMBEDDED_PROJECT = "shellgpt-sa-embedded-proj"  # differs from PROJECT: proves the config override wins
REGION = "us-central1"


def shellgpt_setup(fake: FakeVertex, *, model: str = MODEL, extra_config: dict[str, Any] | None = None,
                 context_length: int | None = None) -> dict[str, Any]:
    """``make_home`` kwargs for a ShellGPT home that selects ``provider: vertex`` against ``fake``: the SA
...
__all__ = [
    "MODEL", "PROJECT", "REGION", "SA_EMBEDDED_PROJECT", "shellgpt_setup",

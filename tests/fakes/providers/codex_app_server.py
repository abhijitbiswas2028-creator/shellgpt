...
ShellGPT intent that never reaches the model, and the suite asserts none are sent. Responses ShellGPT
...
processes), ``threads.json`` (the "rollout store" that ``thread/resume`` reads, so a NEW ShellGPT process
...
# ---------------------------------------------------------------------------------------------------
# Protocol schema (subset of the codex app-server v2 bundle ShellGPT can reach) + serde-style validator
# ---------------------------------------------------------------------------------------------------
...
class FakeAppServer:
    ...
    def server_request(self, method: str, params: dict, timeout: float = 60.0) -> dict:
        """Issue a server-initiated request and block for ShellGPT' reply."""
        with self._pending_cv:
...
class FakeCodex:
    ...
    def requests(self, method: Optional[str] = None) -> list[dict]:
        """Client requests ShellGPT sent (full transcript entries), optionally one method."""
    ...
    def assert_wire_clean(self) -> None:
        """Every request/response ShellGPT sent is valid AND carries no field codex would silently drop."""
...
@dataclass
class CodexRun:
    """Outcome of :func:`run_codex_scenario`: the fake, the ShellGPT home and one ChatResult per CLI run."""
    fake: FakeCodex
    home: Any
    results: list
    session_id: str
...
def run_codex_scenario(root: Path, turns: list[dict], runs: list[dict], *, config: Optional[dict] = None,
    ...
    """Real ``shellgpt chat -q`` runs (``--resume`` after the first) against a fresh fake codex install.
    ...
    # The app-server owns auth; the key only satisfies ShellGPT' provider resolution and never leaves.

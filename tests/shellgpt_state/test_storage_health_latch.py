...
history. ``shellgpt_state_health`` is now the one latch: readers, writers and the readiness
...
from shellgpt_state import SessionDB, StateDbCorruptError
from shellgpt_state_errors import classify_persistence_error
...
def test_readiness_state_db_and_session_store_agree_on_corrupt(_isolate_shellgpt_home):
    from gateway.readiness import collect_runtime_readiness
    from shellgpt_constants import get_shellgpt_home

    path = get_shellgpt_home() / "state.db"
    _seed(path, count=1)
...
def test_readiness_probe_latches_a_file_that_is_not_a_database(_isolate_shellgpt_home):
...
@pytest.fixture
def client(monkeypatch, _isolate_shellgpt_home):
    try:
    ...
    from shellgpt_cli.web_routers import profiles as profiles_routes

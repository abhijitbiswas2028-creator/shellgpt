...
- files inside the ShellGPT home tree are repaired with ``chmod u+rw``
...
from shellgpt_state import SessionDB, preflight_db_writability
...
def shellgpt_home(tmp_path, monkeypatch):
    """Isolated SHELLGPT_HOME so the repair scope covers tmp DBs."""
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, shellgpt_home):
    ...
    def test_repairs_readonly_sidecars(self, shellgpt_home):
        db = shellgpt_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, shellgpt_home):
        sub = shellgpt_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, shellgpt_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, shellgpt_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, shellgpt_home):
        db = shellgpt_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, shellgpt_home):
        db_path = shellgpt_home / "state.db"

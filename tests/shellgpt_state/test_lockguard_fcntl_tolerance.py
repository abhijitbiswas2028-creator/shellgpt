...
unguarded ``fcntl.F_RDLCK`` read at import time. ``shellgpt_state`` imports this module at module
level, so the ``AttributeError`` killed every importer — ``shellgpt serve`` (Desktop backend
"exited before port announcement (1)"), ``shellgpt doctor``, the gateway, cron — before
...
def _reimport_with_fcntl(monkeypatch, stub: types.ModuleType | None):
    ...
    monkeypatch.delitem(sys.modules, "shellgpt_state_lockguard", raising=False)
    ...
    return importlib.import_module("shellgpt_state_lockguard")

...
(SQLITE_PROTOCOL — typical on NFS/SMB), ShellGPT must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``SHELLGPT_HOME`` silently lose
...
from shellgpt_state import SessionDB, get_last_init_error
from shellgpt_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
def _reset_last_init_error():
    ...
    shellgpt_state._set_last_init_error(None)
    yield
    shellgpt_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    shellgpt_state_wal._wal_delete_fallback_failed_paths.clear()
    yield
    shellgpt_state_wal._wal_fallback_warned_paths.clear()
    shellgpt_state_wal._wal_delete_fallback_failed_paths.clear()
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    shellgpt_state_wal._wal_reset_bug_warned_paths.clear()
    yield
    shellgpt_state_wal._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see shellgpt_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("shellgpt_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):

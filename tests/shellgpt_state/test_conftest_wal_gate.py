"""The conftest WAL gate must agree with shellgpt_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``shellgpt_state``. That is deliberate: importing
``shellgpt_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.shellgpt`` before the per-test ``SHELLGPT_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_shellgpt_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and shellgpt_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_shellgpt_state_at_collection():
    """The gate must stay import-free of shellgpt_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.shellgpt, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``shellgpt_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "shellgpt_state" or name.startswith("shellgpt_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported shellgpt_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.shellgpt during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("shellgpt_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["shellgpt_state"] = saved
    assert not blocked

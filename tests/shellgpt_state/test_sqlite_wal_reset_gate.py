...
ShellGPT must not *enable* multi-process WAL on SQLite builds that still contain
...
from shellgpt_state_wal import apply_wal_with_fallback, is_sqlite_wal_reset_vulnerable
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            shellgpt_state_wal, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )

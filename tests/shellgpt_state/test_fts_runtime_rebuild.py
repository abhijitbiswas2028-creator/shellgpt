...
import shellgpt_state_holders
import shellgpt_state_schema
from shellgpt_state import SessionDB
from shellgpt_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from shellgpt_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m shellgpt_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "shellgpt-agent.service"),
            ("grep", "shellgpt-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m shellgpt_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/shellgpt-agent/.venv/bin/shellgpt gateway"),
            ("python3", "/opt/shellgpt-agent/tools/check_state.py"),
            ("shellgpt-monitor", "gateway"),
            ("shellgptctl", "serve"),
            ("python3", "worker.py", "shellgpt_cli.main"),
            ("python3", "-m", "other.module", "shellgpt_cli.main"),
            ("python3", "-c", "shellgpt_cli.main"),
            ("python3", "-Icprint('shellgpt_cli.main')", "shellgpt_cli/main.py"),
        ),
    )
    def test_uninspectable_non_shellgpt_process_is_not_a_holder(self, argv):
        assert not shellgpt_state_holders._looks_like_shellgpt(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/shellgpt", "gateway"),
            ("/usr/local/bin/shellgpt-agent", "serve"),
            ("/usr/local/bin/shellgpt-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "shellgpt_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "shellgpt_cli.main", "gateway"),
            ("/usr/bin/python3", "-mshellgpt_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "shellgpt_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "shellgpt_cli.main"),
            (
                "/opt/shellgpt-agent/.venv/bin/python",
                "/opt/shellgpt-agent/shellgpt_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "shellgpt_cli/main.py", "gateway"),
            ("python3", "/opt/shellgpt-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_shellgpt_process_remains_a_holder(self, argv):
        assert shellgpt_state_holders._looks_like_shellgpt(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(shellgpt_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(shellgpt_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(shellgpt_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(shellgpt_state_holders.os, "stat", _stat)

        holders = shellgpt_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert shellgpt_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a ShellGPT process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like ShellGPT
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00shellgpt_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(shellgpt_state_holders, "_read_proc_argv", _fake_argv)

        holders = shellgpt_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "shellgpt_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from shellgpt_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(shellgpt_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            shellgpt_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "shellgpt_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(shellgpt_state_schema.time, "time", lambda: clock[0])

        from shellgpt_cli.doctor_state import _render_state_db_stats
        from shellgpt_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(shellgpt_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(shellgpt_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m shellgpt_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + shellgpt_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = shellgpt_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="shellgpt_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:

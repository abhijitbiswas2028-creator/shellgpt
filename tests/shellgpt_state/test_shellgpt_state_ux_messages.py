"""Plain-language contracts for shellgpt_state user-facing errors (CLI UX message campaign, cluster D)."""
...
def test_db_unavailable_is_one_line_for_chat_surfaces_by_default():
    shellgpt_state._set_last_init_error("OperationalError: database is locked")
    try:
...
def test_db_unavailable_commands_are_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    shellgpt_state._set_last_init_error("OperationalError: locking protocol")
    try:
        network = format_session_db_unavailable()
    finally:
        shellgpt_state._set_last_init_error(None)
    for text in (no_cause, network):
        assert f"`shellgpt {selector}doctor`" in text and "`shellgpt doctor`" not in text
...
def test_db_unavailable_network_drive_cause_does_not_offer_doctor_fix():
    ...
    cannot host SQLite's WAL; `shellgpt doctor --fix` cannot repair a mount, so it

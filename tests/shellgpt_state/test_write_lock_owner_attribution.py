...
Before, ``database is locked (another ShellGPT process held the state.db write lock for over 60s)``
identified the victim only; every ShellGPT process has the DB open, so the descriptor scan could not
...
from shellgpt_state_lockowners import parse_proc_locks, state_db_write_lock_holders

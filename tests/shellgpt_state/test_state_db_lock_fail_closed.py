...
work on a file several ShellGPT processes share (gateway service, the Desktop
app's `shellgpt serve` backend, CLI sessions, the TUI slash worker):

* `shellgpt_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `shellgpt_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM

"""POSIX advisory locks must survive ShellGPT' own database inspection.
...
ShellGPT used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `shellgpt sessions
...
out across ShellGPT' inspection calls.
"""
...
from shellgpt_cli.sqlite_safe_read import (
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_shellgpt_fail_close", False):
    ...
    conn._shellgpt_fail_close = True
    ...
    conn._shellgpt_fail_close = False
    conn.close()
    assert not has_live_connection(db)
    assert read_header_bytes_preopen(db, length=16) is not None
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import shellgpt_cli.sqlite_safe_read as ssr
...
def test_repair_connections_are_tracked_for_byte_probe_safety(tmp_path, clean_registry, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path))
    from shellgpt_state import SessionDB
    from shellgpt_state_repair import _connect_repair_durable, _repair_conn
...
def test_byte_probe_never_cancels_the_repair_exclusion(tmp_path, clean_registry):
    """A live repair's EXCLUSIVE lock must survive ShellGPT' own inspection (#63386).
    ...
    """
    import shellgpt_state_repair as repair

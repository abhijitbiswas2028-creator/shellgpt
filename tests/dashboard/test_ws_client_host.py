...
The contract these tests pin down:
  ...
  * Explicit ``SHELLGPT_DASHBOARD_WS_HOST`` env var → wins always, regardless
    of the bind host.
...
def clear_ws_host_env(monkeypatch):
    """Ensure no ``SHELLGPT_DASHBOARD_WS_HOST`` leaks in from the test shell."""
    monkeypatch.delenv("SHELLGPT_DASHBOARD_WS_HOST", raising=False)
    yield monkeypatch
...
class TestResolveClientWsHost:
    ...
    def test_blank_env_falls_back_to_bind(
        ...
        """An explicitly empty override (e.g. ``SHELLGPT_DASHBOARD_WS_HOST=``)
        ...
        monkeypatch.setenv("SHELLGPT_DASHBOARD_WS_HOST", "   ")
        _set_bound(saved_app_state, "0.0.0.0")
        assert _web_server_chat._resolve_client_ws_host() == "127.0.0.1"

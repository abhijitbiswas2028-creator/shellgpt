...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        shellgpt_home.mkdir()
        (shellgpt_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(shellgpt_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.shellgpt/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        (shellgpt_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        shellgpt_home = tmp_path / ".shellgpt"
        skills_dir = shellgpt_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.shellgpt/skills"


    def test_custom_container_base(self, tmp_path):
        shellgpt_home = tmp_path / ".shellgpt"
        (shellgpt_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SHELLGPT_HOME": str(shellgpt_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.shellgpt")

        assert mounts[0]["container_path"] == "/home/user/.shellgpt/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.shellgpt/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.shellgpt/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.shellgpt/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.shellgpt/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SHELLGPT_HOME."""
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
        (tmp_path / ".shellgpt").mkdir()

        # Create a sensitive file one level above shellgpt_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside shellgpt_home
    ...
    def test_nested_subdir_inside_shellgpt_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SHELLGPT_HOME must be allowed."""
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        subdir = shellgpt_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SHELLGPT_HOME pointing outside must be rejected."""
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))

        # Create a sensitive file outside shellgpt_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside shellgpt_home pointing outside
        symlink = shellgpt_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SHELLGPT_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, shellgpt_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SHELLGPT_HOME."""
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        ...
        sensitive.write_text("{}")
        self._write_config(shellgpt_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(shellgpt_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SHELLGPT_HOME via config must still mount."""
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))

        (shellgpt_home / "oauth.json").write_text("{}")
        self._write_config(shellgpt_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        (shellgpt_home / "cache" / "documents").mkdir(parents=True)
        (shellgpt_home / "cache" / "audio").mkdir(parents=True)
        (shellgpt_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.shellgpt/cache/documents" in paths
        assert "/root/.shellgpt/cache/audio" in paths
        assert "/root/.shellgpt/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_shellgpt_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        shellgpt_home = tmp_path / ".shellgpt"
        shellgpt_home.mkdir()
        # Use legacy dir name with content — get_shellgpt_dir prefers
        # populated old over new.
        legacy_doc = shellgpt_home / "document_cache"
        legacy_img = shellgpt_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(shellgpt_home / "document_cache") in host_paths
        assert str(shellgpt_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.shellgpt/cache/documents" in container_paths
        assert "/root/.shellgpt/cache/images" in container_paths
    ...
    def test_empty_shellgpt_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.shellgpt/attachments" in container_paths
        assert "/root/.shellgpt/images" in container_paths
        assert "/root/.shellgpt/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        """
        shellgpt_home = tmp_path / ".shellgpt"
        (shellgpt_home / "images").mkdir(parents=True)
        upload = shellgpt_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.shellgpt/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        shellgpt_home = tmp_path / ".shellgpt"
        img_dir = shellgpt_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.shellgpt/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(shellgpt_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.shellgpt/cache/images/x.png"
        assert (shellgpt_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    ShellGPT cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        shellgpt_home = tmp_path / ".shellgpt"
        (shellgpt_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        return str(shellgpt_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_shellgpt(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.shellgpt/attachments/drop.zip"

    def test_ssh_maps_to_tilde_shellgpt(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.shellgpt/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        shellgpt_home = tmp_path / ".shellgpt"
        doc_dir = shellgpt_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — SHELLGPT_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping SHELLGPT_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"SHELLGPT_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.shellgpt/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.shellgpt/google_token.json" in paths
        assert "/root/.shellgpt/.env" not in paths

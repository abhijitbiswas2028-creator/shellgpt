...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("SHELLGPT_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("SHELLGPT_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit ShellGPT secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("SHELLGPT_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.shellgpt_xai_user_agent = MagicMock(return_value="shellgpt-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("SHELLGPT_LOCAL_STT_LANGUAGE", "fr")
...
class TestExplicitOpenaiSelectionError:
    ...
    with the ``shellgpt tools`` remediation for managed-Nous users), the old
    ...
    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        ...
        its shellgpt tools remediation instead of the all-provider install
        hint."""
        ...
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
             ...
             patch(
                 ...
                 lambda what: f"managed route down for {what}; run `shellgpt tools`",
             ):
        ...
        assert "shellgpt tools" in result["error"]

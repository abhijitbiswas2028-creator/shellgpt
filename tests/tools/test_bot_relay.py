    ...
    return [
        {
            ...
            "connection_label": "ShellGPT Cloud",
            "title": "Moxie",
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "shellgpt", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@shellgpt", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("shellgpt@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("shellgpt@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "shellgpt" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "shellgpt", "connection_id": "cloud-1",
         "connection_label": "ShellGPT Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="shellgpt", message="ping", agent=agent))
    assert out.get("status") == "queued"
    assert "ShellGPT Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 shellgpt (@shellgpt): ping")
...
def test_remote_default_is_addressable_by_its_title_slug(tmp_path, monkeypatch):
    """A remote ``default`` is ``@shellgpt`` like every gateway's own, so its Bot Mode title is the only
    bare form that singles it out: ``message_agent`` accepts the slug, the prompt roster offers it
    (never bare ``@shellgpt``, which is the LOCAL default), and the form does not depend on roster order."""
    from tools import bot_mode_probe

    home = _managed_home(tmp_path)
    rows = [
        {"profile": "default", "handle": "shellgpt", "connection_id": "vps-1", "title": "CoS Bot"},
        {"profile": "default", "handle": "shellgpt", "connection_id": "cloud-1", "title": "Ops Bot"},
    ...
    for order in (rows, rows[::-1]):
        ...
        assert forms == {"cloud-1/cos-bot": "cos-bot", "vps-1/default": "shellgpt@vps-1", "cloud-1/default": "ops-bot"}
    ...
    # a bare @shellgpt from the local default is the two remote defaults — offered under their reply-safe forms
    err = json.loads(message_agent_tool(target="shellgpt", message="ping", agent=agent))["error"]
    assert "shellgpt@vps-1" in err and "ops-bot" in err
    section = bot_mode_probe.get_bot_mode_protocol_section(home)
    assert "`@ops-bot`" in section and "`@shellgpt@vps-1`" in section and "- `@shellgpt` —" not in section
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    # Offered under its title slug: bare `@shellgpt` is THIS gateway's own default (#103731).
    assert "`@moxie`" in section and "ShellGPT Cloud" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="shellgpt",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / "nope"))
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "shellgpt", "connection_id": "cloud-1",
         "connection_label": "ShellGPT Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "queued"}),
...
# ── delivery turn author (SHELLGPT_TURN_AUTHOR on the recipient turn) ──────────
...
def test_delivery_env_carries_only_the_given_author(monkeypatch):
    """The dispatcher's own SHELLGPT_TURN_AUTHOR never reaches the child: dropped without an author, replaced with one."""
    ...
    monkeypatch.setenv("SHELLGPT_SESSION_KEY", "session-A")
    monkeypatch.setenv("SHELLGPT_UI_SESSION_ID", "ui-A")
    monkeypatch.setenv("SHELLGPT_SESSION_ID", "session-A")
    monkeypatch.setenv("SHELLGPT_SESSION_PROFILE", "profile-A")
    # A session-* knob, not identity: stripping it would break the child's watcher tuning.
    monkeypatch.setenv("SHELLGPT_SESSION_STALL_TIMEOUT", "97")
    ...
    assert env["SHELLGPT_RELAY_TEST_MARKER"] == "kept"
    assert "SHELLGPT_SESSION_KEY" not in env
    assert "SHELLGPT_UI_SESSION_ID" not in env
    assert "SHELLGPT_SESSION_ID" not in env
    assert "SHELLGPT_SESSION_PROFILE" not in env
    assert env["SHELLGPT_SESSION_STALL_TIMEOUT"] == "97"

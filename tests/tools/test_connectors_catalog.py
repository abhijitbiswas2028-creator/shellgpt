...
def _entry(name, *, platforms=(), requires_env=()):
    return SimpleNamespace(
        ...
        description=f"Drives {name}. More text.", requires_shellgpt="", platforms=list(platforms),
...
def test_only_a_setup_profile_session_selects_the_tool(tmp_path, monkeypatch):
    import model_tools
    from shellgpt_cli.profiles import write_profile_meta
    from shellgpt_constants import reset_shellgpt_home_override, set_shellgpt_home_override

    homes = {}
    for name, role in (("default", None), ("shellgpt-setup", "setup")):
        homes[name] = tmp_path / name
        homes[name].mkdir()
        if role:
            write_profile_meta(homes[name], role=role)
    for name, expected in (("default", False), ("shellgpt-setup", True)):
        token = set_shellgpt_home_override(str(homes[name]))
        try:
...
def test_an_approved_row_installs_into_default_and_lists_the_live_tools(tmp_path):
    from shellgpt_cli.profiles import get_profile_dir, write_profile_meta
    from shellgpt_constants import reset_shellgpt_home_override, set_shellgpt_home_override

    setup_home = tmp_path / "shellgpt-setup"
    setup_home.mkdir()
    ...
    token = set_shellgpt_home_override(str(setup_home))  # the call comes from the setup chat
    try:

...
def test_the_image_bakes_the_same_apt_packages_the_runtime_would_install() -> None:
    ...
    assert "ARG SHELLGPT_BOT_DESKTOP" in text, "the Bot Screen apt layer is gone from the Dockerfile"
    body = text.split("ARG SHELLGPT_BOT_DESKTOP", 1)[1].split("--no-install-recommends", 1)[1].split("rm -rf", 1)[0]
...
: > "$SHELLGPT_BD_XLOCK_DIR/spawned.$$"
sleep 0.4
echo $$ > "$SHELLGPT_BD_XLOCK_DIR/.X${SHELLGPT_BD_DISPLAY_NUM}-lock"
: > "$SHELLGPT_BD_SOCKET"
printf 'DISPLAY=:%s\\n' "$SHELLGPT_BD_DISPLAY_NUM" > "$SHELLGPT_BD_ENV_FILE"
sleep 30
"""

# One start() per process: state_dir() is SHELLGPT_HOME-scoped and process-global, so two profiles need two
...
os.environ["SHELLGPT_BD_XLOCK_DIR"] = str(scratch / "xlocks")
try:
...
def start_in_fresh_process(tmp_path):
    ...
    def launch(home: Path) -> subprocess.Popen:
        env = {**os.environ, "SHELLGPT_HOME": str(home)}
...
def in_process_runtime(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_BD_XLOCK_DIR", str(tmp_path / "xlocks"))
    yield tmp_path

...
def gateway_session(monkeypatch):
    ...
    for k in ("SHELLGPT_CRON_SESSION", "SHELLGPT_YOLO_MODE", "SHELLGPT_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("SHELLGPT_GATEWAY_SESSION", "1")
    monkeypatch.setenv("SHELLGPT_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from SHELLGPT_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("SHELLGPT_CRON_SESSION", "SHELLGPT_YOLO_MODE", "SHELLGPT_GATEWAY_SESSION", "SHELLGPT_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)

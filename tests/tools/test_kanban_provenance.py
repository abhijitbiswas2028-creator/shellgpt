...
def test_worker_create_keeps_durable_origin(tmp_path, monkeypatch, linked, explicit):
    from shellgpt_cli import kanban_db as kb, kanban_db_connect as kbc, kanban_db_notify as kn
    ...
    if explicit:
        ...
        state.close()
    monkeypatch.setenv("SHELLGPT_KANBAN_TASK", owner)
    monkeypatch.setenv("SHELLGPT_SESSION_ID", "ephemeral")
...
def test_tool_create_only_stamps_persisted_ambient_session(tmp_path, monkeypatch, session_id, persisted):
    ...
    monkeypatch.setenv("SHELLGPT_SESSION_ID", session_id)
...
def test_tool_create_stamps_request_scoped_session_over_process_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path))
    monkeypatch.delenv("SHELLGPT_KANBAN_TASK", raising=False)
    monkeypatch.setenv("SHELLGPT_SESSION_ID", "other-session")
    kb.init_db()
    state = SessionDB(db_path=tmp_path / "state.db")

...
def test_saved_opt_in_roundtrip_reaches_schema_and_board(surface, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SHELLGPT_KANBAN_BOARD", raising=False)
    from shellgpt_cli.config import load_config, save_config
    from shellgpt_cli.tools_config import _apply_toolset_change, _get_platform_tools
    ...
    if surface == "http":
        ...
        from shellgpt_cli.web_routers.tools import router
    ...
    try:
        ...
        from shellgpt_cli.kanban_db_connect import connect_closing
        from shellgpt_cli.kanban_db import get_task
...
def test_selection_is_scoped_and_preserves_worker_and_deny_boundaries(legacy, tmp_path, monkeypatch):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        named, broad = list(pool.map(_names, [["kanban"], ["shellgpt-cli"]]))
    ...
    monkeypatch.setenv("SHELLGPT_KANBAN_TASK", "t_worker")
    worker = _names(["file"])

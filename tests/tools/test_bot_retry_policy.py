...
def _is_shellgpt_cli(argv) -> bool:
    """Match the delivery CLI by basename — local_delivery_command may
    resolve the venv-relative shellgpt next to the interpreter (#93590)."""
    name = str(argv[0]).rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    return name in ("shellgpt", "shellgpt.exe")
...
def _transport_calls(calls):
    ...
    return [argv for argv in calls if argv and _is_shellgpt_cli(argv)]
...
def test_deliver_retries_same_argv_on_transient_failure(home, monkeypatch):
    ...
    def _fake_run(argv, **kwargs):
        ...
        if not _is_shellgpt_cli(list(argv)):
            return _Proc(0)
...
# `shellgpt … -Q` prints the turn's final_response (the provider prose) on STDOUT and the session
...
def test_deliver_retry_reads_the_stream_the_cli_writes_and_resumes_the_persisted_row(home, monkeypatch):
    ...
    monkeypatch.setattr(
        "shellgpt_cli.quiet_single_query.run_reported_turn",
        lambda argv, **k: _Proc(1, stdout=_REAL_FAILED_STDOUT, stderr=_REAL_FAILED_STDERR)
        if _is_shellgpt_cli(list(argv)) else _Proc(0),
    )
    out = _deliver({"profile": "ops", "message": "ping"})
...
def test_run_local_turn_retry_reads_the_stream_the_cli_writes_and_resumes_the_persisted_row(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(bot_mode_dm.subprocess, "run", _fake_run)
    rc = bot_mode_dm._run_local_turn(["shellgpt", "-p", "ops", "chat"], str(dm), env={"SHELLGPT_HOME": str(tmp_path)})
    assert rc == 0
    assert envs[0] == {"SHELLGPT_HOME": str(tmp_path)}
    assert envs[1] == {"SHELLGPT_HOME": str(tmp_path), RESUME_UNANSWERED_TURN_ENV: "1"}
    assert "the reply text" in capsys.readouterr().out

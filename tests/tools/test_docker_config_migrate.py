...
def _run_migration(shellgpt_home: Path, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.update(
        {
            "SHELLGPT_HOME": str(shellgpt_home),
            "SHELLGPT_SKIP_CHMOD": "1",
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    ...
    proc = _run_migration(tmp_path, SHELLGPT_SKIP_CONFIG_MIGRATION="1")
...
def test_docker_config_migrate_second_boot_preserves_env_byte_for_byte(tmp_path: Path) -> None:
    ...
    ``$SHELLGPT_HOME/.env``. The first boot migrates the stale config and bumps

...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.shellgpt/skills/."""
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.shellgpt/skills/ as a
    workaround.
    """
...
class TestBackgroundOwnershipPolicyConsistency:
    ...
    def test_repeated_identical_write_gets_the_same_answer(self, tmp_path, monkeypatch):
        ...
        (tmp_path / ".shellgpt" / "skills").mkdir(parents=True, exist_ok=True)
        with _skill_dir(tmp_path):
...
# come up. The user unpins via `shellgpt curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points SHELLGPT_HOME at ``tmp_path/.shellgpt`` so skill_usage's archive path
    (``get_shellgpt_home()``) resolves into the same tree the skill manager
    ...
    skills_root = shellgpt_home / "skills"
...
class TestCuratorConsolidationDeleteGuard:
    ...
    def test_background_review_support_file_overwrite_requires_that_file_read(self, tmp_path, monkeypatch):
        ...
        with _curator_pass(tmp_path, monkeypatch=monkeypatch):
            _create_curator_skill("reviewed", _skill_content("reviewed"))
            ref = tmp_path / ".shellgpt" / "skills" / "reviewed" / "references"
            ref.mkdir()

...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "shellgpt_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "shellgpt_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    shellgpt_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["shellgpt_cli"] = shellgpt_cli
    sys.modules["shellgpt_cli.config"] = types.SimpleNamespace(
        get_shellgpt_home=lambda: Path(tempfile.gettempdir()) / "shellgpt-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.shellgpt/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")

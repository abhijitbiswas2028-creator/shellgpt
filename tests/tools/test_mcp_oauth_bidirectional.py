"""Regression test for the ``ShellGPTMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_shellgpt_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
...
async def test_long_lived_resource_request_does_not_block_concurrent_post(
    ...
    acquiring its Bearer token.  ShellGPT narrows that lock around resource I/O

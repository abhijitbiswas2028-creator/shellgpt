...
Covers:
- :class:`ShellGPTTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ShellGPTMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SHELLGPT_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# ShellGPTTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ShellGPTTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path))
        storage = ShellGPTTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ShellGPTMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ShellGPTTokenStorage, **context_attrs):
    ...
    provider = _SHELLGPT_PROVIDER_CLS.__new__(_SHELLGPT_PROVIDER_CLS)
    provider._shellgpt_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path))
        storage = ShellGPTTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SHELLGPT_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path))
        storage = ShellGPTTokenStorage("flow-srv")
        ...
        with patch.object(
            _SHELLGPT_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",

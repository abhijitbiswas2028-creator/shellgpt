...
was gated on ``[ -f "$SHELLGPT_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        # In tests we run unprivileged; as_shellgpt is a passthrough then.
        'as_shellgpt() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    ShellGPT loads $SHELLGPT_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/shellgpt/.env.example -> $SHELLGPT_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $SHELLGPT_HOME/.env from it on first boot"
    )

...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `shellgpt-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.shellgpt/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.shellgpt/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    ...
    def test_accepts_external_cron_worker_with_presence_vars_stripped(self, monkeypatch):
        ...
        for v in ("SHELLGPT_INTERACTIVE", "SHELLGPT_GATEWAY_SESSION", "SHELLGPT_EXEC_ASK"):
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["SHELLGPT_INTERACTIVE", "SHELLGPT_GATEWAY_SESSION", "SHELLGPT_EXEC_ASK"],
    )
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `shellgpt cron`
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.shellgpt/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.shellgpt/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.shellgpt/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.shellgpt/.env; {self.GH}"

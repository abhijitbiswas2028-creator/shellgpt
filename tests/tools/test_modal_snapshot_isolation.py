...
def _restore_tool_modules():
    original_shellgpt_home = os.environ.get("SHELLGPT_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "shellgpt_cli"
        or name.startswith("shellgpt_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_shellgpt_home is None:
            os.environ.pop("SHELLGPT_HOME", None)
        else:
            os.environ["SHELLGPT_HOME"] = original_shellgpt_home
        _reset_modules(("tools", "shellgpt_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    shellgpt_cli = types.ModuleType("shellgpt_cli")
    shellgpt_cli.__path__ = [str(REPO_ROOT / "shellgpt_cli")]  # type: ignore[attr-defined]
    sys.modules["shellgpt_cli"] = shellgpt_cli
    shellgpt_home = tmp_path / "shellgpt-home"
    os.environ["SHELLGPT_HOME"] = str(shellgpt_home)
    sys.modules["shellgpt_cli.config"] = types.SimpleNamespace(
        get_shellgpt_home=lambda: shellgpt_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="shellgpt-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": shellgpt_home / "modal_snapshots.json",

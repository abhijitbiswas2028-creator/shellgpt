...
ShellGPT accepts exactly this shape — the document fetched from the well-known URL derived from the advertised
...
async def test_origin_issued_metadata_shim_does_not_read_non_discovery_response(url):
    ...
    provider = ShellGPTProviderMixin.__new__(ShellGPTProviderMixin)
    response = ResourceResponse()

    assert await provider._shellgpt_accept_origin_issued_metadata(response) is response

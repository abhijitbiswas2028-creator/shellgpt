...
def test_interpreter_kill_rejection_names_the_owned_process_route(tmp_path, monkeypatch):
    """Scheduled-Task topology (#113667): only ``SHELLGPT_SUPERVISED_CHILD`` marks the launch, and an
    ...
    for marker in ("INVOCATION_ID", "XPC_SERVICE_NAME", "SHELLGPT_S6_SUPERVISED_CHILD", "SHELLGPT_GATEWAY_EXTERNAL_SUPERVISOR"):
    ...
    # Absence on the wrong side: a plain foreground `shellgpt gateway run` carries no launch marker.
    monkeypatch.delenv("SHELLGPT_SUPERVISED_CHILD")

...
ShellGPT state stays profile-scoped through SHELLGPT_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/shellgpt-agent/issues/25114
See: https://github.com/NousResearch/shellgpt-agent/issues/36144
See: https://github.com/NousResearch/shellgpt-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for shellgpt_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(shellgpt_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("SHELLGPT_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        shellgpt_home = real_home / ".shellgpt" / "profiles" / "coder"
        profile_home = shellgpt_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        from shellgpt_constants import get_subprocess_home
    ...
    def test_host_auto_repairs_missing_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_REAL_HOME", str(real_home))
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        from shellgpt_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from shellgpt_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".shellgpt" / "profiles"
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("SHELLGPT_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["SHELLGPT_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(shellgpt_home / "home")
        assert result["SHELLGPT_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_shellgpt_home_unset(self, monkeypatch):

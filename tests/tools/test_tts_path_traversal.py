...
def test_output_path_rejects_shellgpt_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    shellgpt_home = tmp_path / "shellgpt-home"
    shellgpt_home.mkdir()
    monkeypatch.setattr(file_safety, "_shellgpt_home_path", lambda: shellgpt_home)
    monkeypatch.setattr(file_safety, "_shellgpt_root_path", lambda: shellgpt_home)

    target = shellgpt_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    shellgpt_home = tmp_path / "shellgpt-home"
    token_dir = shellgpt_home / "mcp-tokens"

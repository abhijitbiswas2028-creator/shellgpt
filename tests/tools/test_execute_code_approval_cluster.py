...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad SHELLGPT_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SHELLGPT_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_shellgpt_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SHELLGPT_HOME": "/h", "SHELLGPT_PROFILE": "p",
        "SHELLGPT_CONFIG": "/c.yaml", "SHELLGPT_ENV": "/e",
        "SHELLGPT_DELEGATED_CHILD_CONTEXT": "1",
        # other SHELLGPT_* → dropped (broad prefix removed)
        "SHELLGPT_BASE_URL": "https://x", "SHELLGPT_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "SHELLGPT_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SHELLGPT_BASE_URL", "SHELLGPT_INTERACTIVE", "SHELLGPT_KANBAN_TASK",

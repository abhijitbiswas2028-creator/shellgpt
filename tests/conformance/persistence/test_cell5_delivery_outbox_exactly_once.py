...
ShellGPT' outbox is ``gateway/delivery_ledger.py``. Because a send and its
...
* The ledger module, on a real ``state.db`` under an isolated
  ``HOME=<tmp>/home``, ``SHELLGPT_HOME=<tmp>/home/.shellgpt`` (every child).
...
class Cell:
    ...
    def __init__(self, tmp_path: Path):
        ...
        self.db_path = self.shellgpt_home / "state.db"
        (tmp_path / "tmp").mkdir()
        self._boots = 0
    ...
    def spawn(self, script: str, role: str, extra: dict | None = None) -> subprocess.Popen:
        ...
        env.update({
            ...
            # HOME/.shellgpt IS the temp home here, so the live-system guard would

...
from shellgpt_cli.profiles import _get_default_shellgpt_home
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"shellgpt": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"shellgpt": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"shellgpt": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="shellgpt-agent"
        ):
            result = config.resolve_session_name("/home/user/shellgpt-agent/subdir")
        assert result == "shellgpt-agent"


class TestResolveConfigPath:
    def test_prefers_shellgpt_home_when_exists(self, tmp_path):
        shellgpt_home = tmp_path / "shellgpt"
        shellgpt_home.mkdir()
        local_cfg = shellgpt_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: SHELLGPT_HOME points at ~/.shellgpt/profiles/<name>, so
        # _get_default_shellgpt_home() must resolve back to ~/.shellgpt — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".shellgpt"
        ...
        assert _get_default_shellgpt_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "shellgpt_coder"
        assert profile_host_key("default") == "shellgpt"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"SHELLGPT_HONCHO_HOST": "shellgpt.coder"}):
            assert resolve_active_host() == "shellgpt.coder"
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="shellgpt_coder")
        assert config.host == "shellgpt_coder"
        assert config.workspace_id == "shellgpt"  # shared workspace
        assert config.ai_peer == "shellgpt_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"shellgpt": {"enabled": True, "aiPeer": "shellgpt"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (shellgpt.work)
        ...
        cfg = HonchoClientConfig(
            ...
            raw={"hosts": {"shellgpt.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("shellgpt_cli.profiles.get_active_profile_name", return_value="default"), \

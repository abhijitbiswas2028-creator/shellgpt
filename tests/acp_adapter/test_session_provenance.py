...
def test_root_session_no_compression(db):
    ...
    assert prov["currentShellGPTSessionId"] == "root1"
    assert prov["rootShellGPTSessionId"] == "root1"
    assert prov["parentShellGPTSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_shellgpt_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentShellGPTSessionId"] == "old"
    assert prov["rootShellGPTSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousShellGPTSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootShellGPTSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"shellgpt"}
    assert "sessionProvenance" in meta["shellgpt"]
    assert meta["shellgpt"]["sessionProvenance"]["currentShellGPTSessionId"] == "root1"

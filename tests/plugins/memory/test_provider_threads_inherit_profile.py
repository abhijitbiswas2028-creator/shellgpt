...
Profile isolation is a ContextVar-scoped SHELLGPT_HOME override; a plain ``threading.Thread`` starts
...
def _retaindb(seen, tmp_path):
    ...
    p._client = MagicMock()
    p._context_overlay = lambda query: {"context": seen.setdefault("home", get_shellgpt_home()) and "ctx"}

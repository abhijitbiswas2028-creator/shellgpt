"""Plugin data paths follow the active profile's SHELLGPT_HOME, including the ContextVar override.

Several plugins carried a ``~/.shellgpt`` fallback (guarding an ImportError of ``shellgpt_constants``
...
def _openviking_log(home):
    ...
    return ov.get_shellgpt_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH

...
SHELLGPT_HOME override (``shellgpt_constants.set_shellgpt_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``SHELLGPT_HOME``) and profile B with different config/.env values."""
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from shellgpt_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", shellgpt_home=str(home))

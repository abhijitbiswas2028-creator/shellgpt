...
Run from the PR worktree:
    ...
    cd ~/.shellgpt/shellgpt-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.shellgpt/shellgpt-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.shellgpt/shellgpt-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a shellgpt-agent checkout"
)
...
# Isolated SHELLGPT_HOME for the config write.

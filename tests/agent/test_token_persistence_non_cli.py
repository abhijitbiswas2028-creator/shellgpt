...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    shellgpt_state = ModuleType("shellgpt_state")
    shellgpt_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "shellgpt_state", shellgpt_state)
    shellgpt_state_registry = ModuleType("shellgpt_state_registry")
    shellgpt_state_registry.acquire = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "shellgpt_state_registry", shellgpt_state_registry)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "ShellGPT", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "ShellGPT"

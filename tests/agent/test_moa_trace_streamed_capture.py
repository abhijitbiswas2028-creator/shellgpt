...
path via ``shellgpt chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SHELLGPT_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SHELLGPT_HOME at a temp dir and turn moa.save_traces on."""
    shellgpt_home = tmp_path / ".shellgpt"
    shellgpt_home.mkdir()
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))

    # save_moa_turn reads config via shellgpt_cli.config.load_config; stub it to
    ...
    return shellgpt_home / "moa-traces"

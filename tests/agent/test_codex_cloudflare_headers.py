...
The official Codex endpoint must receive ShellGPT' own harness identity, rather
...
These tests pin:
- the required ShellGPT originator
- the versioned ShellGPT User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_shellgpt_version(self):
        from agent.auxiliary_client import _codex_cloudflare_headers
        headers = _codex_cloudflare_headers(_make_codex_jwt())
        assert headers["User-Agent"] == f"ShellGPTAgent/{__version__}"
        assert headers["originator"] == "shellgpt-agent"
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "shellgpt-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"ShellGPTAgent/{__version__}"

...
binary download are exercised manually by `shellgpt secrets bitwarden
...
def shellgpt_home(tmp_path, monkeypatch):
    """Point ShellGPT at an isolated home directory."""
    home = tmp_path / ".shellgpt"
    home.mkdir()
    monkeypatch.setenv("SHELLGPT_HOME", str(home))
    # Some modules cache get_shellgpt_home; clear if needed.
    import shellgpt_constants
    if hasattr(shellgpt_constants, "_SHELLGPT_HOME_CACHE"):
        shellgpt_constants._SHELLGPT_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(shellgpt_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from shellgpt_cli.env_loader import _apply_external_secret_sources

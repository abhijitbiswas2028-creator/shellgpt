"""ShellGPT scope cwd export through the real NeMo Relay ATOF plugin."""
...
def test_run_conversation_exports_session_and_turn_cwds(tmp_path, monkeypatch):
    ...
    for directory in (shellgpt_home, session_cwd, turn_cwd, atof_dir):
        directory.mkdir()
...
""",
    ...
    session_start = only_event("shellgpt.session", "start")
    turn_start = only_event("shellgpt.turn", "start")
    session_end = only_event("shellgpt.session", "end")
    turn_end = only_event("shellgpt.turn", "end")

"""``auth.adopt_external_logins: false`` keeps ShellGPT off the Claude Code login (#113023).

Claude Code's OAuth refresh token is single-use: once ShellGPT borrows and refreshes it, ShellGPT and
Claude Code hold one token family and whichever refreshes first logs the other out. With the opt-out
set, ShellGPT must neither read nor refresh ``~/.claude/.credentials.json``, must drop the pool row an
earlier adopting process persisted, and must say so in ``shellgpt auth list``.
"""
...
def _write_config(shellgpt_home, adopt):
    body = "model:\n  provider: anthropic\n  model: claude-sonnet-4-5\n"
    if adopt is not None:
        body += f"auth:\n  adopt_external_logins: {'true' if adopt else 'false'}\n# arm {adopt}\n"
    (shellgpt_home / "config.yaml").write_text(body)


def test_opt_out_never_reads_or_refreshes_claude_code_login(tmp_path, monkeypatch):
    shellgpt_home, claude_dir = tmp_path / "shellgpt", tmp_path / "claude"
    shellgpt_home.mkdir()
    claude_dir.mkdir()
    ...
    _write_config(shellgpt_home, adopt=None)
    ...
    _write_config(shellgpt_home, adopt=False)
    ...
    _write_config(shellgpt_home, adopt=True)

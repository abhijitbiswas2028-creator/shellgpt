...
def _make_agent():
    shellgpt_home = Path(tempfile.mkdtemp(prefix="shellgpt-test-home-"))
    (shellgpt_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._shellgpt_home", shellgpt_home),

...
def isolated_home(monkeypatch):
    """Temp SHELLGPT_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="shellgpt_test_31179_")
    shellgpt_home = os.path.join(test_home, ".shellgpt")
...
def _fresh_modules():
    """Drop cached shellgpt modules so each test reloads against current env."""
    _drop_reload_targets()

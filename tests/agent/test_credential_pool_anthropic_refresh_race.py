...
ShellGPT *processes* via the cross-process ``_auth_store_lock`` flock,
...
ShellGPT's own PKCE login (``manual:shellgpt_pkce`` / ``shellgpt_pkce``) got no
...
def _fake_pool_store(monkeypatch):
    ...
    This stands in for ``~/.shellgpt/auth.json`` so the two "process-local"
...
def test_concurrent_shellgpt_pkce_refresh_loses_credential_despite_valid_token_on_disk(monkeypatch):
    """Two 'ShellGPT processes' race to refresh the same stale shellgpt_pkce
    ...
    assert loser_result is not None, (
        ...
        "disk. shellgpt_pkce-sourced entries must adopt the winner's token "
    ...
    assert loser_entry_after.last_status != STATUS_EXHAUSTED, (
        "regression: the losing process marked its Anthropic shellgpt_pkce "
        "credential STATUS_EXHAUSTED after a lost refresh race, even "
        "though the account is not actually exhausted -- a sibling ShellGPT "
        "process holds a perfectly valid rotated token. This causes "
        "spurious 're-authenticate with Anthropic' failures under "
        "ordinary concurrent ShellGPT usage (fleet workers, cron jobs, "
...
def test_concurrent_claude_code_refresh_recovers_via_credentials_file(monkeypatch):
    ...
    evidence that shellgpt_pkce/dashboard-sourced credentials were simply

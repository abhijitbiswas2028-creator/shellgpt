...
SHELLGPT_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_dev_fixtures_drive_cold_start():
    """Every SHELLGPT_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["SHELLGPT_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["SHELLGPT_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("SHELLGPT_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("SHELLGPT_DEV_CREDITS", None)
...
def _seed(agent, fixture):
    ...
    os.environ["SHELLGPT_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["SHELLGPT_DEV_CREDITS_FIXTURE"] = fixture
    try:
...
def _run_bg_seed(monkeypatch, agent, *, warm):
    ...
    import shellgpt_cli.nous_account as nous_account
    from agent import credits_tracker

    monkeypatch.delenv("SHELLGPT_DEV_CREDITS", raising=False)  # fixtures would take the sync path

...
def test_kanban_guidance_fallback_requires_owned_worker_task(monkeypatch, task_id, owner, expected):
    ...
    is not identity, and an inherited SHELLGPT_KANBAN_TASK is not ownership (#112486)."""
...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_shellgpt_home``,
    ``get_default_shellgpt_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``SHELLGPT_HOME`` and no resolver mocks.
    """

    def test_real_shellgpt_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_shellgpt_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active ShellGPT profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """SHELLGPT_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".shellgpt"
        ...
        assert "Active ShellGPT profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "SHELLGPT_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "SHELLGPT_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_shellgpt_home", lambda: Path("/shellgpt"))

    # Production renders this as str(get_shellgpt_home()) + "/profiles/<name>/",
    # and str(Path("/shellgpt")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/shellgpt".
    _home_str = str(Path("/shellgpt"))
    expected_profile = (
    ...
    with (
        ...
        patch("shellgpt_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("shellgpt_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under SHELLGPT_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from shellgpt_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("shellgpt_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from shellgpt_time import now as shellgpt_now
        sid = shellgpt_now().strftime("%Y%m%d_%H%M%S_fresh")
    ...
    def test_surrogate_zone_name_does_not_abort_prompt(self):
        ...
        with patch("shellgpt_time.now", return_value=current):

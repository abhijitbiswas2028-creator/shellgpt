"""A host that mirrors the served profile into SHELLGPT_HOME must not flip launch-home identity.

ShellGPT WebUI serves several profiles from one process and, for legacy readers, mirrors the active
turn's profile into ``os.environ["SHELLGPT_HOME"]`` while also installing the context-local override.
...
``shellgpt_constants.pin_process_shellgpt_home`` gives such hosts one stable anchor for all of them.
"""
...
from shellgpt_cli.env_loader import _process_shellgpt_home
from tools.environments.local import _is_routed_home
from tools.mcp_tool_scope import _server_key


def _under(home, fn):
    """Run *fn* with *home* installed as the task's ShellGPT-home override."""
    token = shellgpt_constants.set_shellgpt_home_override(home)
    try:
        return fn()
    finally:
        shellgpt_constants.reset_shellgpt_home_override(token)
...
ROUTED = {
    ...
    "env_loader._process_shellgpt_home": lambda home: _process_shellgpt_home().resolve() != Path(home).resolve(),
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(shellgpt_constants, "_PINNED_PROCESS_SHELLGPT_HOME", None)
    return launch, served


@pytest.mark.parametrize("decision", sorted(ROUTED))
def test_pinned_launch_home_survives_a_mirrored_shellgpt_home(homes, monkeypatch, decision):
    launch, served = homes
    ...
    assert _under(served, lambda: _server_key("atlassian")) == (shellgpt_constants.shellgpt_home_key(served), "atlassian")
    # Process-asset readers keep following the env var: only routing decisions use the pin.
    assert shellgpt_constants.get_process_shellgpt_home() == served
    assert shellgpt_constants.get_routing_process_shellgpt_home() == launch


@pytest.mark.parametrize("decision", sorted(ROUTED))
def test_unpinned_or_cleared_pin_keeps_shellgpt_home_semantics(homes, monkeypatch, decision):
    launch, served = homes
    routed = ROUTED[decision]
    for _ in ("never pinned", "pinned then cleared"):
        ...
        assert routed(served) is True
        assert routed(launch) is False
        monkeypatch.setenv("SHELLGPT_HOME", str(served))  # mirrored: the env var IS the launch home
        assert routed(served) is False
        shellgpt_constants.pin_process_shellgpt_home(launch)
        shellgpt_constants.pin_process_shellgpt_home(None)

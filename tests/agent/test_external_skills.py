...
def shellgpt_home(tmp_path):
    """Create a minimal SHELLGPT_HOME with config."""
...
class TestGetExternalSkillsDirs:
    def test_empty_config(self, shellgpt_home):
        (shellgpt_home / "config.yaml").write_text("skills:\n  external_dirs: []\n")
    ...
    def test_valid_dir_returned(self, shellgpt_home, external_skills_dir):
...
class TestGetAllSkillsDirs:
    def test_local_always_first(self, shellgpt_home, external_skills_dir):
        (shellgpt_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        with patch.dict(os.environ, {"SHELLGPT_HOME": str(shellgpt_home)}):
        ...
        assert result[0] == shellgpt_home / "skills"
        assert result[1] == external_skills_dir.resolve()


class TestExternalSkillsInFindAll:
    def test_external_skills_found(self, shellgpt_home, external_skills_dir):
        (shellgpt_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = shellgpt_home / "skills"
        with (
            patch.dict(os.environ, {"SHELLGPT_HOME": str(shellgpt_home)}),
    ...
    def test_local_takes_precedence(self, shellgpt_home, external_skills_dir):
...
class TestExternalSkillView:
    def test_skill_view_finds_external(self, shellgpt_home, external_skills_dir):
        (shellgpt_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = shellgpt_home / "skills"

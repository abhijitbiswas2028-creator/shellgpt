...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_shellgpt_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for shellgpt_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": shellgpt_level}
            )
            assert kw["reasoning_effort"] == wire_level, shellgpt_level

"""The packaged ``shellgpt-agent`` console script honours argv (#54648).
...
def _run_console_script(monkeypatch, *argv: str):
    ...
    module, func = tomllib.loads(pyproject.read_text(encoding="utf-8"))["project"]["scripts"]["shellgpt-agent"].split(":")
    monkeypatch.setattr(sys, "argv", ["shellgpt-agent", *argv])
    try:
...
def test_metadata_invocations_never_start_an_agent(argv, monkeypatch, capsys):
    ...
    assert "usage: shellgpt-agent" in out or out.startswith("ShellGPT Agent v")

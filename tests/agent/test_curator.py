...
def curator_env(tmp_path, monkeypatch):
    """Isolated SHELLGPT_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from SHELLGPT_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.shellgpt once monkeypatch
    ...
    # thread here, while SHELLGPT_HOME is still pinned to this test's tmp home
...
def test_bundled_skills_are_off_limits_unless_opted_in(curator_env, monkeypatch):
    ...
    monkeypatch.setattr("shellgpt_cli.config.load_config", lambda: {"curator": {}})
    assert usage._prune_builtins_enabled() is False
    assert DEFAULT_CONFIG["curator"]["prune_builtins"] is False
    monkeypatch.setattr("shellgpt_cli.config.load_config", lambda: {"curator": {"prune_builtins": True}})
...
def test_cli_pin_refuses_bundled_skill(curator_env):
    from shellgpt_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `shellgpt model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``shellgpt curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``shellgpt curator rollback`` restores a

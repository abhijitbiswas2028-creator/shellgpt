...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"shellgpt-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "shellgpt-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "shellgpt-agent-dev"
...
# `shellgpt curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `shellgpt curator pin <umbrella>` in the rename block so

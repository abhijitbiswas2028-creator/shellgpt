...
def _make_agent(
    *tool_names: str,
    ...
    with (
        ...
        patch("shellgpt_cli.config.load_config_readonly", return_value=config or {}),
...
def test_context_pruned_effectful_call_blocks_before_dispatch():
    ...
    with (
        patch("shellgpt_cli.plugins._dispatch_pre_tool_call_hooks", return_value=(None, pruned)) as plugin,
        patch("model_tools.handle_function_call", return_value="SHOULD_NOT_RUN") as dispatch,
    ):
        agent._execute_tool_calls_sequential(msg, messages, "task-1")

...
class TestParseConfigStringList:
    """#86661: `shellgpt config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `shellgpt config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    shellgpt_home = tmp_path / "da\\ta"
    subprocess_home = shellgpt_home / "home"
    subprocess_home.mkdir(parents=True)
    (shellgpt_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
    monkeypatch.setenv("HOME", str(shellgpt_home))

...
an autonomous agent reached through a client shim that cannot carry ShellGPT tool
...
So: a client declaring ``SUPPORTS_SHELLGPT_TOOL_CALLS = False`` skips the fork with
...
def test_fork_is_skipped_when_the_provider_cannot_emit_tool_calls(caplog):
    client = MagicMock()
    client.SUPPORTS_SHELLGPT_TOOL_CALLS = False
...
def test_fork_is_spawned_when_the_provider_can_emit_tool_calls():
    client = MagicMock()
    client.SUPPORTS_SHELLGPT_TOOL_CALLS = True
...
def test_an_incapable_provider_still_reviews_when_the_review_is_routed_away():
    ...
    class _IncapableClient:
        SUPPORTS_SHELLGPT_TOOL_CALLS = False

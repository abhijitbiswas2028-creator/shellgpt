"""``shellgpt update`` / ``shellgpt migrate relay``: legacy ``SHELLGPT_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from shellgpt_cli.relay_plugin_migrate import (
...
SHELLGPT_NEMO_RELAY_ATOF_ENABLED=1
SHELLGPT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
SHELLGPT_NEMO_RELAY_ATOF_FILENAME=shellgpt-atof.jsonl
SHELLGPT_NEMO_RELAY_ATOF_MODE=append
SHELLGPT_NEMO_RELAY_ATIF_ENABLED=1
SHELLGPT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
SHELLGPT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
SHELLGPT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "shellgpt-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file ShellGPT will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: SHELLGPT_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")

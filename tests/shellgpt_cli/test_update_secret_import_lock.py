...
External secret backends are useful during normal ShellGPT startup, but the
...
""",
    ...
    dispatch = (
        "shellgpt_main.cmd_update = lambda _args: 0\n"
        "shellgpt_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import shellgpt_cli.main as shellgpt_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "SHELLGPT_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["shellgpt", "chat"])
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_shellgpt_dotenv(
        shellgpt_home=home,
...
def test_update_probe_children_skip_external_secret_sources(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         "import sys; sys.argv = ['shellgpt', 'update']\n"
         "from shellgpt_cli.update_cmd_deps import _validate_critical_modules_import\n"

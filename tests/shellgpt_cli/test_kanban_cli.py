"""Tests for the kanban CLI surface (shellgpt_cli.kanban)."""
...
def test_worker_link_preserves_foreign_child_rules(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_KANBAN_TASK", worker)
    monkeypatch.setenv("SHELLGPT_KANBAN_RUN_ID", str(worker_run_id))
    ...
    # with SHELLGPT_KANBAN_RUN_ID — the one path _cmd_link forwards a run id for.
    assert kc._cmd_link(argparse.Namespace(
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="shellgpt", add_help=False)

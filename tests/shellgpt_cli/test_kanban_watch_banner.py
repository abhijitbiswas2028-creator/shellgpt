...
from shellgpt_cli import kanban_ops
from shellgpt_cli.kanban import kanban_command
from shellgpt_cli.kanban_parser import build_parser
...
def test_watch_names_resolved_board(tmp_path, monkeypatch, capsys, environment, explicit, expected):
    ...
    monkeypatch.setenv("SHELLGPT_KANBAN_HOME", str(tmp_path))
    monkeypatch.delenv("SHELLGPT_KANBAN_DB", raising=False)
    monkeypatch.delenv("SHELLGPT_KANBAN_BOARD", raising=False)
    kb.create_board("alpha")
    kb.create_board("beta")
    kb.set_current_board("alpha")
    if environment:
        monkeypatch.setenv("SHELLGPT_KANBAN_BOARD", environment)

...
ancestors by design — a CLI ``shellgpt update`` IS the venv python.  Before
...
Windows ``shellgpt update`` exited 2 before even fetching and the update
...
def test_native_extension_scan_derives_every_loaded_venv_extension(tmp_path):
    ...
    from shellgpt_cli.update_cmd_deps import _loaded_native_extension_dists
...
def test_recovered_update_retry_builds_parser_without_native_secret_modules(
    ...
    monkeypatch.setattr(sys, "argv", ["shellgpt", "update", "--yes"])

"""Tests for ``shellgpt update`` / ``--check`` inside the Docker container.
...
fix, ``shellgpt update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side ShellGPT, not an update to the running container, so the message
...
@patch("shellgpt_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``shellgpt update`` inside Docker → friendly message + exit 2, no git calls.
    ...
    assert "docker pull nousresearch/shellgpt-agent:latest" in out

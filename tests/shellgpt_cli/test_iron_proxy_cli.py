"""Unit tests for ``shellgpt_cli.proxy_cli`` command handlers.
...
def shellgpt_home(tmp_path, monkeypatch):
    """Point SHELLGPT_HOME at a temp dir so the wizard doesn't touch the
...
def test_cmd_install_failure_returns_1(shellgpt_home, monkeypatch):
    def boom(**kw):
...
def test_cmd_setup_from_bitwarden_refuses_on_empty_vault(shellgpt_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "find_iron_proxy", lambda **kw: shellgpt_home / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "test")
    monkeypatch.setattr(
        ip, "ensure_ca_cert",
        lambda **kw: (shellgpt_home / "ca.crt", shellgpt_home / "ca.key"),
    )
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from shellgpt_cli.config import load_config as _lc
    cfg2 = _lc()
...
def test_cmd_start_refuses_when_bitwarden_mode_but_disabled(shellgpt_home, monkeypatch):
...
def test_cmd_setup_audit_log_failure_is_warning_not_abort(shellgpt_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "discover_provider_mappings", lambda **kw: [
        ip.TokenMapping(
            proxy_token="shellgpt-proxy-deadbeef",

"""Regression coverage for #110173: observational `shellgpt sessions` readers stay read-only."""

from argparse import Namespace


import shellgpt_cli.sessions_cmd as sessions_cmd
...
def test_doctor_write_probe_never_touches_a_store_a_live_writer_holds(monkeypatch, tmp_path):
    ...
    probed: list = []
    monkeypatch.setattr(shellgpt_state_repair, "_db_opens_cleanly", lambda path: probed.append(path))

    monkeypatch.setattr(shellgpt_state_holders, "live_writer_holds_db", lambda *_a, **_k: True)
    assert doctor_state._write_health_reason(state_db, should_fix=False) is None
    monkeypatch.setattr(shellgpt_state_holders, "live_writer_holds_db", lambda *_a, **_k: False)
    assert doctor_state._write_health_reason(state_db, should_fix=False) is None

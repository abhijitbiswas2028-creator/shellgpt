...
Drives ``ShellGPTCLI.process_command("/reset now")`` against a minimal stand-in
and verifies:
...
def _make_cli_stub():
    """Build a minimal ShellGPTCLI-shaped object that can run ``process_command``
    ...
    self_._confirm_destructive_slash = ShellGPTCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
    )
    self_._slash_handler = ShellGPTCLI._slash_handler
    self_._cmd_new = ShellGPTCLI._cmd_new.__get__(self_, type(self_))
    self_.process_command = ShellGPTCLI.process_command.__get__(self_, type(self_))
...
def test_new_without_skip_token_still_consults_modal():
    ...
    self_._split_destructive_skip = ShellGPTCLI._split_destructive_skip
    self_._normalize_slash_confirm_choice = ShellGPTCLI._normalize_slash_confirm_choice.__get__(
        self_, type(self_)
    )
    self_._confirm_destructive_slash = ShellGPTCLI._confirm_destructive_slash.__get__(
        self_, type(self_)

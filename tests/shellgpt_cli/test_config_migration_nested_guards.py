...
a step that still raises so one bad value cannot wedge ``shellgpt config migrate`` /
``shellgpt update`` and leave the config unversioned.
"""
...
def test_failing_step_is_skipped_with_warning_and_config_still_migrates(tmp_path, caplog):
    """``migrate_config`` (the ``shellgpt config migrate`` / ``shellgpt update`` path) keeps going
    ...
    from shellgpt_cli import config_migrations
    ...
    with patch.dict(os.environ, {"SHELLGPT_HOME": str(tmp_path)}), \
            ...
            caplog.at_level(logging.WARNING, logger="shellgpt_cli.config_migrations"):

"""``shellgpt update`` refreshes the installed macOS ``ShellGPT.app`` from the rebuilt bundle (#52339).

``shellgpt desktop --build-only`` only packages into ``apps/desktop/release/``; Finder launches the
...
def _bundle(root: Path, asar: bytes) -> Path:
    app = root / "ShellGPT.app"
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "ShellGPT").write_bytes(b"\xcf\xfa\xed\xfe")
...
def test_stale_bundle_is_replaced_current_and_running_are_left_alone(rebuilt, tmp_path):
    ...
    installed, problems = main_desktop._install_rebuilt_macos_bundles(
        rebuilt, [stale, current, running, tmp_path / "missing" / "ShellGPT.app"],
        running={running.resolve()})

    assert installed == [stale]
    assert _asar(stale) == b"rebuilt"
    assert not (stale.parent / "ShellGPT.app.shellgpt-update-old").exists()
    assert not (stale.parent / "ShellGPT.app.shellgpt-update-new").exists()
...
def test_failed_swap_keeps_the_previous_bundle_launchable(rebuilt, tmp_path, monkeypatch):
    ...
    def fail_final_rename(self, target):
        if self.name.endswith(".shellgpt-update-new"):

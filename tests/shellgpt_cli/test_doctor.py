...
from shellgpt_cli import doctor as doctor_mod
from shellgpt_cli.doctor_config import _has_provider_env_config
import shutil
from shellgpt_cli import doctor_tools
from shellgpt_cli import doctor_state
from shellgpt_cli import doctor_platform
from shellgpt_cli import doctor_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/shellgpt-agent:latest" in hint
        assert "shellgpt update" not in hint


    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor_platform._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade shellgpt-agent`" in hint
        assert "shellgpt update" not in hint

    def test_sqlite_upgrade_hint_preserves_nix_guidance_as_prose(self):
        from shellgpt_cli.config import recommended_update_command_for_method
...
class TestDoctorToolAvailabilitySummary:
    ...
    def test_image_gen_without_provider_reports_setup_hint_not_system_dependency(self, monkeypatch):
        ...
        'run shellgpt setup' summary like any missing key (#9516)."""
        ...
        assert "shellgpt tools" in image_line and "system dependency" not in image_line and "unavailable" in image_line
        assert "system dependency not met" in next(line for line in out.splitlines() if "homeassistant" in line)
        assert any("shellgpt setup" in issue for issue in f.issues)
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `shellgpt doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "SHELLGPT_HOME", shellgpt_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("SHELLGPT_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_shellgpt_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal SHELLGPT_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_shellgpt_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "SHELLGPT_HOME", home)
        ...
        try:
            from shellgpt_cli import auth as _auth_mod
...
def test_run_doctor_accepts_shellgpt_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (shellgpt_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".shellgpt"
    home.mkdir(parents=True, exist_ok=True)
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`shellgpt doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale SHELLGPT_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"SHELLGPT_MAX_ITERATIONS={ghost}\n")
        (shellgpt_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "SHELLGPT_HOME", shellgpt_home)
        monkeypatch.setattr(doctor_mod, "get_shellgpt_home", lambda: shellgpt_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("SHELLGPT_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("SHELLGPT_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale SHELLGPT_MAX_ITERATIONS" in out
        env_after = (shellgpt_home / ".env").read_text(encoding="utf-8")
        assert "SHELLGPT_MAX_ITERATIONS" not in env_after
...
def test_macos_tcc_grant_check_is_silent_off_macos(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(doctor_platform, "_desktop_app_bundle", lambda: tmp_path / "ShellGPT.app")
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        self._darwin_bundle(
            monkeypatch, tmp_path,
            'designated => identifier "com.nousresearch.shellgpt" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "shellgpt update" in out
    ...
    def test_identifier_dr_is_stable_with_upgrade_hint_and_repair_info(self, monkeypatch, capsys, tmp_path):
        self._darwin_bundle(monkeypatch, tmp_path, 'designated => identifier "com.nousresearch.shellgpt"')
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
        assert "--setup-tcc-identity" in out
        assert "tccutil reset ScreenCapture com.nousresearch.shellgpt" in out

    def test_certificate_anchored_dr_is_stable_without_upgrade_hint(self, monkeypatch, capsys, tmp_path):
        self._darwin_bundle(
            monkeypatch, tmp_path,
            'designated => identifier "com.nousresearch.shellgpt" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out

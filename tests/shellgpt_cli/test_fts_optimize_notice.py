...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            shellgpt_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(shellgpt_constants, "get_shellgpt_home", lambda: tmp_path)
    monkeypatch.setattr(shellgpt_state, "SessionDB", FakeSessionDB)
    ...
    assert "shellgpt sessions optimize-storage" in capsys.readouterr().out
    conn.close()

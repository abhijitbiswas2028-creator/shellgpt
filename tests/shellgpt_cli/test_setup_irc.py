"""Tests for IRC gateway configuration via `shellgpt setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh ShellGPT install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new ShellGPT install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#shellgpt")
            monkeypatch.setenv("IRC_NICKNAME", "shellgpt-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `shellgpt setup gateway` experience with IRC present."""

...
from shellgpt_cli.web_routers import status
...
def test_get_logs_yields_while_reading_and_filtering(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(status, "get_shellgpt_home", lambda: tmp_path)
    ...
    import shellgpt_cli.logs
    monkeypatch.setattr(shellgpt_cli.logs, "_read_tail", blocking_read)

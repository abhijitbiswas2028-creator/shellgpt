...
base deps) during ``shellgpt update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point shellgpt_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("shellgpt_cli.main_install_repair._resolve_install_target_python", return_value=py), \
             patch("shellgpt_cli.main_install_repair.subprocess.run", side_effect=fake_subprocess_run), \
             patch("shellgpt_cli.main_install_repair._run_install_with_heartbeat"):

            from shellgpt_cli.main_install_repair import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("shellgpt_cli.main_install_repair._resolve_install_target_python") as mock_resolve, \
             patch("shellgpt_cli.main_install_repair._run_install_with_heartbeat") as mock_install:
            from shellgpt_cli.main_install_repair import _verify_core_dependencies_installed

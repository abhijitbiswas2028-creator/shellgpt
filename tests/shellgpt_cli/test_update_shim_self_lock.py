...
``venv\\Scripts\\shellgpt.exe`` is a launcher that runs the interpreter with the
...
SHIM_NAMES = ["shellgpt.exe", "shellgpt-agent.exe", "shellgpt-acp.exe", "shellgpt-gateway.exe"]
...
def venv(tmp_path, monkeypatch):
    ...
    # update_cmd reads these off shellgpt_cli.main (frozen ``_m()`` surface); the
    ...
    monkeypatch.setattr(sys, "argv", ["shellgpt", "update"])
...
def test_detects_shim_from_zipapp_main_py(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "shellgpt.exe" / "__main__.py")])
    assert main_install_repair._windows_shim_in_process_chain() == venv / "shellgpt.exe"


@pytest.mark.windows_only
def test_detects_shim_in_ancestor_chain(venv, monkeypatch):
    """The launcher is usually a separate parent process, not argv[0]."""
    _fake_psutil(monkeypatch, [str(venv / "shellgpt.exe")])
    assert main_install_repair._windows_shim_in_process_chain() == venv / "shellgpt.exe"
...
@pytest.mark.windows_only
def test_ignores_shellgpt_exe_outside_the_project_venv(venv, monkeypatch, tmp_path):
    """A shim from some other install must never trigger a re-exec."""
    other = tmp_path / "other" / "Scripts"
    other.mkdir(parents=True)
    monkeypatch.setattr(sys, "argv", [str(other / "shellgpt.exe"), "update"])
    _fake_psutil(monkeypatch, [str(other / "shellgpt.exe")])
...
def test_reexec_runs_same_args_under_venv_python(venv, monkeypatch):
    monkeypatch.setattr(sys, "argv", [str(venv / "shellgpt.exe"), "update", "--yes"])
    ...
    assert cmd == [
        str(venv / "python.exe"), "-m", "shellgpt_cli.main", "update", "--yes",
    ]
    ...
    from shellgpt_cli import update_handoff
...
def test_reexec_does_not_recurse(venv, monkeypatch):
    monkeypatch.setattr(sys, "argv", [str(venv / "shellgpt.exe"), "update"])
...
def test_pending_rename_filter_drops_only_our_shim_pairs():
    shims = [Path(r"C:\shellgpt\venv\Scripts\shellgpt.exe")]
    entries = [
        r"\??\C:\other\thing.dll", r"!\??\C:\other\thing.dll.bak",
        r"\??\C:\shellgpt\venv\Scripts\shellgpt.exe",
        r"!\??\C:\shellgpt\venv\Scripts\shellgpt.exe.old.1755624735000",
    ]
...
def test_pending_rename_filter_keeps_a_shim_pair_with_a_foreign_target():
    shims = [Path(r"C:\shellgpt\venv\Scripts\shellgpt.exe")]
    entries = [
        r"\??\C:\shellgpt\venv\Scripts\shellgpt.exe", r"!\??\C:\somewhere\else.exe",
    ]
    kept, removed = main_install_repair._filter_pending_shim_renames(entries, shims)
    assert removed == 0
    assert kept == entries

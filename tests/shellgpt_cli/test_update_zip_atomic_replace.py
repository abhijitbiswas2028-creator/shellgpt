...
which broke ``shellgpt --tui`` because ``ui-tui/`` had vanished.
...
from shellgpt_cli.update_cmd import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.shellgpt-update-staging").exists()
    assert not (dst.parent / "ui-tui.shellgpt-update-old").exists()

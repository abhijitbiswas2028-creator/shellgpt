...
#88251: ``shellgpt update`` treated a failed desktop pack as non-fatal, printed
...
import shellgpt_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "shellgpt_constants.with_shellgpt_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "shellgpt_constants.display_shellgpt_home", lambda: str(tmp_path), raising=False
    )
...
def test_lost_desktop_with_surviving_build_stamp_is_rebuilt(desktop_env, monkeypatch):
    ...
    forever; the build stamp under SHELLGPT_HOME survived and is the proof Desktop was installed here."""
    desktop_dir, calls = desktop_env
    monkeypatch.setattr(
...
def test_summary_omits_success_banner_when_desktop_rebuild_failed(capsys):
    ...
    assert "shellgpt desktop" in out
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_shellgpt_home", lambda: tmp_path)

...
1. `shellgpt secrets bitwarden setup --help` works (dispatch path)
2. `shellgpt update --check` works (update path)
3. `shellgpt secrets bitwarden disable` works (handler execution)
4. `shellgpt secrets onepassword status` works (lazy backend loads on demand)
...
def _run_shellgpt(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run shellgpt CLI as a subprocess from repo root.
    ...
    connectivity — but ``shellgpt update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-shellgpt-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-shellgpt-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`shellgpt secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_shellgpt(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`shellgpt secrets bitwarden disable` must exit 0."""
        result = _run_shellgpt(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`shellgpt secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_shellgpt(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `shellgpt update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`shellgpt update --check` must not load cryptography._rust."""
        result = _run_shellgpt(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['shellgpt','update','--check']
...
sys.argv = ['shellgpt', 'update', '--check']

import shellgpt_cli.main as m
...
# resolves the shellgpt_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('shellgpt_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()

...
``shellgpt_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``shellgpt --help``, ``shellgpt --version``,
``shellgpt logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain shellgpt-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level shellgpt <name> CLI command via PluginContext.

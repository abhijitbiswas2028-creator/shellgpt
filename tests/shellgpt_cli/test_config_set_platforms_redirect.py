...
`shellgpt config set platforms.<name>.<display_setting> <value>` must write to
...
def _write_config(shellgpt_home: Path, data: dict) -> Path:
...
def _set(monkeypatch, shellgpt_home, key, value, force=False):
    """Isolated call to set_config_value against a temp SHELLGPT_HOME."""
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
    # set_config_value resolves the home live via get_config_path()/get_shellgpt_home()
...
class TestPerPlatformDisplayRedirect:
    def test_streaming_redirects_to_display_platforms(self, shellgpt_home, monkeypatch):
        """platforms.telegram.streaming must land under display.platforms."""
        _set(monkeypatch, shellgpt_home, "platforms.telegram.streaming", "false")
    ...
    def test_connection_key_not_redirected(self, shellgpt_home, monkeypatch):
        """A real connection key (token) stays in top-level platforms.<name>."""
        _set(monkeypatch, shellgpt_home, "platforms.telegram.token", "new-token")
        result = yaml.safe_load((shellgpt_home / "config.yaml").read_text())
...
class TestRedirectSiblingSurfaces:
    ...
    def test_get_mirrors_gateway_resolution_after_set(self, shellgpt_home, monkeypatch, capsys):
        ...
        raw = yaml.safe_load((shellgpt_home / "config.yaml").read_text())
        assert resolve_display_setting(raw, "telegram", "streaming") is False

    def test_unset_removes_the_redirected_leaf(self, shellgpt_home, monkeypatch):
    ...
    def test_unset_missing_redirected_leaf_exits_nonzero(self, shellgpt_home, monkeypatch):
        from shellgpt_cli.config import unset_config_value

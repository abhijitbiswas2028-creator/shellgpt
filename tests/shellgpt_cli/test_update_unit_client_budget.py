...
def test_unit_transaction_budget_preserves_scope_and_health(monkeypatch, graceful, retry):
    ...
    def systemctl(cmd, *, timeout):
        ...
        if "list-units" in cmd:
            return subprocess.CompletedProcess(cmd, 0, "shellgpt-serve-test.service loaded active running", "")
        if "show" in cmd:
    ...
    name = "shellgpt-gateway-test" if graceful else "shellgpt-serve-test"
    restarted, failed = [], []
...
def test_budget_fallback_keeps_real_errors(monkeypatch, limits, outcome):
    ...
    if outcome == "timeout":
        with pytest.raises(subprocess.TimeoutExpired):
            fleet._systemctl_reset_and_restart(["systemctl"], "shellgpt-serve-test")
    else:
        result = fleet._systemctl_reset_and_restart(["systemctl"], "shellgpt-serve-test")
...
def test_fleet_restart_repairs_a_system_unit_that_cannot_park_on_exit_78(monkeypatch, tmp_path, capsys, root):
    ...
    unit_dir.mkdir()
    stale = unit_dir / "shellgpt-gateway.service"
    stale.write_text("[Service]\nRestart=on-failure\nRestartSec=10\n", encoding="utf-8")
    current = unit_dir / "shellgpt-gateway-ops.service"
    ...
    monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "shellgpt-gateway")
    ...
    for name in ("shellgpt-gateway", "shellgpt-gateway-ops"):
        fleet._restart_one_systemd_gateway_unit(
    ...
    if root:
        ...
        assert refreshed == []
        assert out.count("RestartPreventExitStatus=78") == 1 and "shellgpt-gateway lacks" in out
        assert "sudo shellgpt gateway install --system" in out

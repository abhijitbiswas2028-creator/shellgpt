...
The bug class: on Windows, when `shellgpt.exe`/sibling shims could not be
...
import shellgpt_cli._install_repair as ir
import shellgpt_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("shellgpt", "shellgpt-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(tmp_path, monkeypatch):
    ...
    # shellgpt.exe cannot be renamed; shellgpt-gateway.exe can
    real_rename = Path.rename

    def deny_shellgpt(self, target):
        if self.name == "shellgpt.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_shellgpt)
    monkeypatch.setattr(main_install_repair, "_shellgpt_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "shellgpt.exe" in exc_info.value.failed_shims
    # The successful rename (shellgpt-gateway.exe) was rolled back.
    assert (scripts / "shellgpt-gateway.exe").exists()
    assert not list(scripts.glob("shellgpt-gateway.exe.old.*"))


@pytest.mark.windows_only
def test_non_strict_keeps_warn_and_try(tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("shellgpt",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("shellgpt",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["shellgpt.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    assert "shellgpt.exe" in capsys.readouterr().out

...
def session_db(tmp_path):
    ...
    os.environ["SHELLGPT_HOME"] = str(tmp_path / ".shellgpt")
    os.makedirs(tmp_path / ".shellgpt", exist_ok=True)
    from shellgpt_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".shellgpt" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal ShellGPTCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_failed_branch_creation_leaves_original_session_open(self, cli_instance, session_db):
        ...
        with patch.object(session_db, "create_session", side_effect=RuntimeError("boom")):
            ShellGPTCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        ShellGPTCLI._handle_branch_command(cli_instance, "/branch refactor approach")

...
calls ``os._exit(0)`` ~5ms after ``SHELLGPT_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            SHELLGPT_PARENT_PID=str(parent.pid),
            SHELLGPT_PARENT_START_MARKER=drifted_marker,
            SHELLGPT_PARENT_NONCE="nonce-95693",
            SHELLGPT_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("SHELLGPT_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "SHELLGPT_BACKEND_READY"), "backend never announced READY"

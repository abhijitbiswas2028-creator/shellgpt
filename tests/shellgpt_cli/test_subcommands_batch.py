...
def test_config_get_unset_subcommands_parse():
    """`shellgpt config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `shellgpt login` fails gracefully, not with argparse error ────
#
# `shellgpt login` is a removed command; its handler (`login_command` in
# `shellgpt_cli/auth.py`) prints a deprecation notice pointing at `shellgpt auth` /
# `shellgpt model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `shellgpt --help`.

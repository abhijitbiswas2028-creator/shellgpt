"""Tests for shellgpt_cli.gateway."""
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(**kwargs):
            assert "_SHELLGPT_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["SHELLGPT_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "SHELLGPT_HOME": str(tmp_path),
            "SHELLGPT_GATEWAY_DETACHED": "1",
            "SHELLGPT_GATEWAY_EXIT_DIAG": "1",
            "SHELLGPT_GATEWAY_MAX_STARTS": "0",
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["SHELLGPT_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "SHELLGPT_HOME": str(tmp_path),
        "SHELLGPT_GATEWAY_EXIT_DIAG": "0",
        "SHELLGPT_TEST_GATEWAY_OUTCOME": outcome,
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("shellgpt_constants.is_container", lambda: True)
    monkeypatch.setattr("shellgpt_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("shellgpt_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_shellgpt_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path):
    unit_path = tmp_path / "systemd" / "user" / "shellgpt-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test SHELLGPT_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="SHELLGPT_HOME=/home/alice/.shellgpt"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_shellgpt_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_shellgpt_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "shellgpt-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "shellgpt_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "shellgpt_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestRestartWaitsForApiServerPort:
    """Regression for #91547: ``shellgpt gateway restart`` waited only for the old PID; on macOS the
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time ShellGPT Desktop opens (``shellgpt serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time ShellGPT Desktop opens
    (``shellgpt serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(
        ...
        assert gateway._windows_scheduled_task_supervises("ShellGPTGateway") is False
        assert gateway._windows_scheduled_task_state("ShellGPTGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("ShellGPT_Gateway") is expected, state
...
def test_find_windows_gateway_services_ignores_task_scheduler_ancestor(monkeypatch):
    ...
    ShellGPT-owned service (by binary path) stays SCM-supervised."""
    import psutil
    import shellgpt_cli.gateway_windows as gateway_windows

    monkeypatch.setattr(gateway_windows, "shellgpt_service_roots", lambda: (r"C:\shellgpt\shellgpt-agent",))
    ...
    owned = run(FakeService("gw", r'"C:\shellgpt\shellgpt-agent\venv\Scripts\shellgpt.exe" gateway run'))
    ...
    # ShellGPT's, and never a reason to abort; a ShellGPT-NAMED service is settled without asking binpath.
    ...
    named = run(UnreadableConfigService("ShellGPTGateway", OSError(15100, "MUI file missing")))
    assert [(s.name, s.service_pid, s.gateway_pid) for s in named] == [("ShellGPTGateway", 2360, 18480)]

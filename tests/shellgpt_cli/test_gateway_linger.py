...
def _root_with_system_unit_pinned_home(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(gateway, "_shellgpt_home_pinned_by_unit", lambda _unit_path: str(tmp_path / "system-home"))
...
def test_systemd_install_calls_linger_helper(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test SHELLGPT_HOME) stays out of the way.
    monkeypatch.setattr(
        gateway,
...
def test_systemd_install_targets_linger_at_system_service_user(monkeypatch, tmp_path, user):
    ...
    unit_path = tmp_path / "systemd" / "shellgpt-gateway.service"
    helper_calls = []

    monkeypatch.setattr(gateway, "_require_root_for_system_service", lambda action: None)
    monkeypatch.setattr(gateway, "has_legacy_shellgpt_units", lambda: False)
    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
...
def test_existing_system_install_repairs_linger_for_configured_user(monkeypatch, tmp_path, unit_is_current):
    ...
    monkeypatch.setattr(gateway, "has_legacy_shellgpt_units", lambda: False)
    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    monkeypatch.setattr(gateway, "_sync_shellgpt_home_from_systemd_unit", lambda system=False: None)

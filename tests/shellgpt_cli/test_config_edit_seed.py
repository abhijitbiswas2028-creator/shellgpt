...
def _config_edit(tmp_path, monkeypatch, cfg, *, template):
    """`shellgpt config edit` on a home with no config.yaml (seeds via seed_config_file, like `doctor --fix`)."""
...
def _setup_agent_enter(tmp_path, monkeypatch, cfg):
    """`shellgpt setup agent` on a fresh home, pressing Enter (the offered default) on every prompt."""
...
def _apply_default_agent_settings(tmp_path, monkeypatch, cfg):
    ...
    from shellgpt_cli.setup import _apply_default_agent_settings

    _apply_default_agent_settings(cfg.load_config())


def _blank_slate(tmp_path, monkeypatch, cfg):
    from shellgpt_cli.setup_quick import _blank_slate_minimize_config
...
def _doctor_fix(tmp_path, monkeypatch, cfg):
    """`shellgpt doctor --fix` on a home with no config.yaml: the real config-file check, fix enabled."""
    import shellgpt_cli.doctor as doctor
    from shellgpt_cli.doctor_config import _check_config_file
    ...
    home = cfg.get_shellgpt_home()
    monkeypatch.setattr(doctor, "SHELLGPT_HOME", home)

"""Regression tests for the OAuth dispatcher in shellgpt_cli.web_server.
...
ShellGPT must not mint subscription OAuth tokens from an unattended HTTP
...
import shellgpt_cli.web_routers.oauth as _rt_oauth
import shellgpt_cli.web_server_oauth as _web_server_oauth
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "shellgpt_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "shellgpt_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "shellgpt_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_minimax_start_route_honors_poller_mock_on_owning_module(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "shellgpt_cli.auth._minimax_poll_token",
        fail_poll_token,
    ), patch(
        "shellgpt_cli.web_server_oauth._minimax_poller",
        fake_poller,
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_shellgpt_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "shellgpt auth add fake-oauth",
...
def test_codex_dashboard_worker_stops_polling_after_cancel(tmp_path, monkeypatch):
    ...
    from shellgpt_cli import web_server as ws
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `shellgpt model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `shellgpt model` but is "
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``shellgpt auth remove
    xai-oauth``.
    """
    from shellgpt_cli import auth as auth_mod
    ...
    # Prior `shellgpt auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to shellgpt_cli.auth.get_auth_status (the
    ...
    with patch("shellgpt_cli.auth.get_auth_status", return_value=fake_status):

...
A long-lived multiplex process (the Desktop's ``shellgpt serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    shellgpt_logging.setup_logging(shellgpt_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(shellgpt_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_shellgpt_home_override(profile)
    try:
        if adopt:
            shellgpt_logging.setup_logging(shellgpt_home=profile)
        else:
            assert shellgpt_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in shellgpt_logging._queued_file_handlers
            if isinstance(h, shellgpt_logging._ProfileRoutingFileHandler)]

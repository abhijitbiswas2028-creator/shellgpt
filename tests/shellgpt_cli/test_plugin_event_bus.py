...
Covers:
  ...
  - Namespace spoofing (shellgpt:, foreign, already-colon'd) is rejected
  ...
  - `shellgpt plugins show` output includes emits/listens
"""
...
@pytest.mark.parametrize(
    "bad_event",
    [
        "shellgpt:x",   # reserved core prefix
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "shellgpt:x", "a:x", "b:x"):
...
def test_pending_budget_drops_new_event_without_blocking(monkeypatch, caplog):
    from shellgpt_cli import plugins_dispatch

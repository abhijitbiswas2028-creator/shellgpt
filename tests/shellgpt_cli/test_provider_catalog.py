"""Tests for the unified provider catalog (shellgpt_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``shellgpt model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from shellgpt_cli.provider_catalog import (
    provider_catalog,

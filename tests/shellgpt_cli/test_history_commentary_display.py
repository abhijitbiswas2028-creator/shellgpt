...
from shellgpt_cli.web_routers.sessions import _project_for_display
...
def test_owner_profile_settings_apply_to_rest_and_rpc(monkeypatch, tmp_path, disabled):
    ...
    def config():
        home = get_shellgpt_home_override()
...
def test_rest_pages_bind_the_history_owner_for_messages_and_around(
    ...
    import shellgpt_state_timeline
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "display": {
                "show_commentary": str(get_shellgpt_home_override())
                == str(homes["visible"])
            }
        },
    )
...
def test_unscoped_rest_history_uses_custom_home_of_its_database(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "display": {
                "show_commentary": str(get_shellgpt_home_override()) != str(custom)
            }
        },
    )
    row = _row()
    ...
    token = set_shellgpt_home_override(str(custom))
    try:
        asyncio.run(exercise())
    finally:
...
def test_cold_resume_uses_the_resumed_session_home(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "load_config",
        lambda: {
            "display": {"show_commentary": str(get_shellgpt_home_override()) != str(home)}
        },
    )

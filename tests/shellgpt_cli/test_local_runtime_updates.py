...
def test_installed_tags_newest_first(shellgpt_home):
    from shellgpt_cli.local_runtime.binaries import installed_tags

    assert installed_tags() == []
    _install_fake_tag(shellgpt_home, "b10290")
    _install_fake_tag(shellgpt_home, "b10412")
...
def test_default_tag_update_offer_respects_explicit_pins(shellgpt_home, pin):
    ...
    _install_fake_tag(shellgpt_home, "b10679")
...
def test_update_available_requires_enabled_and_installed(shellgpt_home, monkeypatch):
...
def test_boot_never_downloads_missing_tag(shellgpt_home, monkeypatch):
...
def test_prune_keeps_n_minus_one(shellgpt_home):
    from shellgpt_cli.local_runtime.binaries import installed_tags, prune_old_tags

    for tag in ("b10100", "b10200", "b10290"):
        _install_fake_tag(shellgpt_home, tag)
    prune_old_tags(["b10290", "b10200"])
    assert installed_tags() == ["b10290", "b10200"]
    # downloads/ cache dir must survive pruning when present.
    downloads = shellgpt_home / "runtimes" / "llamacpp" / "downloads"

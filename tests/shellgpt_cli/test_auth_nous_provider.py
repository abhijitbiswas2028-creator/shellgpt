...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from shellgpt_cli.auth import _resolve_verify
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    payload = json.loads((shellgpt_home / "auth.json").read_text())
...
def test_resolve_nous_runtime_credentials_invoke_jwt_is_idempotent(
    ...
    import shellgpt_cli.auth as auth_mod
    import shellgpt_cli.auth_nous as auth_nous
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.DEBUG, logger="shellgpt_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from shellgpt_cli.auth import get_nous_auth_status
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `shellgpt model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return shellgpt_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import shellgpt_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from shellgpt_cli.auth import PROVIDER_REGISTRY, _login_nous

        shellgpt_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    """
    from shellgpt_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", ShellGPT must surface an
    ...
    re-authenticate" string led users to report this as a ShellGPT persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from shellgpt_cli.auth import _refresh_access_token
...
def test_refresh_token_exchange_error_classification(
    ...
    from shellgpt_cli.auth import _is_terminal_nous_refresh_error, _refresh_access_token
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect SHELLGPT_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("SHELLGPT_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without SHELLGPT_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.shellgpt/shared/`` across CI runs.
    """
    from shellgpt_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("SHELLGPT_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `shellgpt auth add nous
    ...
    shellgpt_home = tmp_path / "shellgpt"
    shellgpt_home.mkdir(parents=True, exist_ok=True)
    (shellgpt_home / "auth.json").write_text(
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "shellgpt-cli"
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from shellgpt_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL

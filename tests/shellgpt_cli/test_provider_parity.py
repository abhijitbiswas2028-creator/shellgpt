...
the SAME provider universe as ``shellgpt model`` (the CLI/TUI picker).
...
def test_every_shellgpt_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `shellgpt model` universe."""
    ...
    assert not missing, (
        "providers shown in `shellgpt model` but not configurable in the desktop "

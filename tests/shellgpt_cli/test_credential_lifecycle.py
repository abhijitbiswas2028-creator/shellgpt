...
SHELLGPT_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
HEADERS = {"X-ShellGPT-Session-Token": _SESSION_TOKEN}
...
@pytest.fixture
def shellgpt_home(monkeypatch, tmp_path):
    """Fresh SHELLGPT_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(shellgpt_home):
    _write_env(shellgpt_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(shellgpt_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = shellgpt_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(shellgpt_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(shellgpt_home, OPENAI_API_KEY=old)
    _write_config(
        shellgpt_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = shellgpt_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
# load_pool() or a separate `shellgpt auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(shellgpt_home):
    ...
    the old cached credential until a separate ``shellgpt auth add opencode-go``
...
def test_update_rotates_keyed_providers_mirror(shellgpt_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(shellgpt_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(shellgpt_home):
    old = "sk-kp-" + "p" * 24
    _write_env(shellgpt_home, OPENAI_API_KEY=old)
    _write_config(
        shellgpt_home,
...
def test_scrub_never_touches_providers_base_url_alias(shellgpt_home):

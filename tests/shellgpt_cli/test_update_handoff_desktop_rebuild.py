...
A Windows git install runs `shellgpt update` from `shellgpt.exe`, which reexecs a
...
def test_current_checkout_repair_rebuilds_desktop_under_project_root():
    ...
    with (
        ...
        m.return_value.PROJECT_ROOT = update_cmd.Path("/fake/shellgpt")
    ...
    assert rebuild.call_args[0][0] == update_cmd.Path("/fake/shellgpt/apps/desktop")
...
def test_handoff_venv_repair_finishes_node_and_web_phase(tmp_path):
    ...
    project_root = tmp_path / "shellgpt"
    ...
    with (
        ...
        patch("shellgpt_cli.managed_uv.ensure_uv", return_value="uv"),
        patch.object(update_cmd, "_m") as m,
    ):

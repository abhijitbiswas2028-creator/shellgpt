"""`shellgpt update` concurrent-instance gate and Windows gateway service helpers
...
# Tests in this module either exercise the REAL _detect_concurrent_shellgpt_instances
# helper (and need the autouse stub in tests/shellgpt_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_shellgpt_instances — parent-chain exclusion
#
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(tmp_path, monkeypatch):
    ...
    rows = [
        ...
        (launcher_pid, str(shim), "shellgpt.exe"),
        (other_pid, str(shim), "shellgpt.exe"),
    ...
    result = dashboard_procs._detect_concurrent_shellgpt_instances(tmp_path)

    assert result == [(other_pid, "shellgpt.exe")]


# ---------------------------------------------------------------------------
# _quarantine_running_shellgpt_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(tmp_path):
    ...
    pairs = main_install_repair._quarantine_running_shellgpt_exe(
    ...
    assert orig == shim
    assert quarantine.name.startswith("shellgpt.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(main_install_repair, "_shellgpt_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "shellgpt_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "shellgpt_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["ShellGPTGateway"],
        "expected_services": ["ShellGPTGateway"],
        "restarted_services": [],
        "service_profiles": {"ShellGPTGateway": "default"},
    }
    assert stopped == ["ShellGPTGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["ShellGPTGateway"]
    assert token["resume_needed"] is False


def _two_services():
    return [
        SimpleNamespace(name="ShellGPTGateway", profile="default", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="ShellGPTGatewayPicasso", profile="picasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
...
def _stop_times_out_on_picasso(name, **_kwargs):
    if name == "ShellGPTGatewayPicasso":
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    with pytest.raises(RuntimeError, match="ShellGPTGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["ShellGPTGatewayPicasso", "ShellGPTGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: ShellGPTGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="ShellGPTGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["ShellGPTGateway"]


def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    import shellgpt_cli.update_cmd as update_cmd
    import shellgpt_cli.update_cmd_windows as update_cmd_windows
    ...
    update_cmd._restore_windows_gateway_service("ShellGPTGateway")

    assert restarted == ["ShellGPTGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m shellgpt_cli.main gateway run"
...
# when every concurrent shellgpt.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, shellgpt.exe shim, shellgpt-gateway.exe,
    gateway/run.py, bare `shellgpt gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "shellgpt_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\shellgpt.exe", "gateway", "run"],
        [r"C:\venv\Scripts\shellgpt-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["shellgpt.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["shellgpt.exe", "gateway"],  # bare `shellgpt gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["shellgpt.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\shellgpt.exe"],  # interactive REPL
        [r"C:\venv\Scripts\shellgpt.exe", "dashboard"],
        ["shellgpt.exe", "gateway", "status"],  # management, not runtime
        ["shellgpt.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["shellgpt.exe", "gateway", "run"],
                200: ["shellgpt.exe"],  # REPL — keep
                300: ["shellgpt.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "shellgpt.exe"), (300, "shellgpt.exe"), (400, "shellgpt.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["shellgpt.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\shellgpt-gateway.exe"],
            }
        ),
    )
    matches = [(111, "shellgpt.exe"), (222, "shellgpt-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                1000: ["shellgpt.exe", "gateway", "run"],
                2000: [r"C:\venv\Scripts\shellgpt-gateway.exe"],
            }
        ),
    )
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_shellgpt_instances",
        return_value=[(1000, "shellgpt.exe"), (2000, "shellgpt-gateway.exe")],
    ), patch.object(
    ...
    assert "Another shellgpt.exe is running" not in capsys.readouterr().out
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    monkeypatch.setitem(
        ...
        "psutil",
        _fake_psutil_classify(
            {1000: ["shellgpt.exe", "gateway", "run"], 3000: ["shellgpt.exe"]}
        ),
    )
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_shellgpt_instances",
        return_value=[(1000, "shellgpt.exe"), (3000, "shellgpt.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m shellgpt_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "ShellGPTGateway", expected_service_identity=(11, 11.0)
        )

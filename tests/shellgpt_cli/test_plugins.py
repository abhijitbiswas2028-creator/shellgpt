"""Tests for the ShellGPT plugin system (shellgpt_cli.plugins)."""
...
from shellgpt_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
from shellgpt_cli.middleware import (
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<shellgpt_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<shellgpt_home>/plugins/``; we derive
    ``<shellgpt_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target ShellGPT home for this
    plugin isn't necessarily the current ``SHELLGPT_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``SHELLGPT_HOME``/``set_shellgpt_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SHELLGPT_HOME>/config.yaml.
        # Config is always read from SHELLGPT_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            shellgpt_home = Path(home)
        else:
            import os
            shellgpt_home_str = os.environ.get("SHELLGPT_HOME")
            if shellgpt_home_str:
                shellgpt_home = Path(shellgpt_home_str)
            else:
                shellgpt_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_enabled_portable_plugin_registers_components(
        ...
        (plugin / "plugin.json").write_text(
            json.dumps({
                ...
                "name": "portable.test",
                "extensions": {"com.nousresearch.shellgpt": {"servers": {"worker": {
        ...
        from shellgpt_cli.agent_plugins import liveness_for
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from shellgpt_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from shellgpt_cli import plugins as plugins_mod
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "shellgpt_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / "shellgpt_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("shellgpt_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_entry_point_function_form_registers(self, tmp_path, monkeypatch):
        ...
        """
        shellgpt_home = tmp_path / "shellgpt_test"
        shellgpt_home.mkdir(parents=True, exist_ok=True)
...
class TestPluginLoading:
    ...
    def test_bundled_cron_provider_is_not_loaded_by_general_manager(self, tmp_path, monkeypatch):
        ...
        (shellgpt_home / "config.yaml").write_text(yaml.safe_dump({"plugins": {"enabled": ["chronos"]}}))
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        from shellgpt_cli import plugins as plugins_mod
    ...
    def test_user_cron_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        shellgpt_home = tmp_path / "shellgpt_test"
        plugin_dir = shellgpt_home / "plugins" / "mycron"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "plugin.yaml").write_text(yaml.dump({"name": "mycron"}), encoding="utf-8")
        (plugin_dir / "__init__.py").write_text(
            "def register(ctx):\n    ctx.register_cron_scheduler(object())\n", encoding="utf-8")
        (shellgpt_home / "config.yaml").write_text(yaml.safe_dump({"plugins": {"enabled": ["mycron"]}}))
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``shellgpt_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "shellgpt_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $SHELLGPT_HOME/plugins/.
        shellgpt_home = tmp_path / "shellgpt_test"
        plugins_dir = shellgpt_home / "plugins"
...
class TestForceReloadSymmetry:
    ...
    def test_hook_timeout_does_not_block_caller(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "shellgpt_cli.plugins._resolve_hook_callback_timeout", lambda: 0.15
        )
    ...
    def test_hook_exception_still_isolated_under_timeout_path(self, monkeypatch):
        monkeypatch.setattr(
            "shellgpt_cli.plugins._resolve_hook_callback_timeout", lambda: 1.0
        )
    ...
    def test_pre_tool_call_callback_exception_fails_closed(self, monkeypatch, timeout):
        ...
        monkeypatch.setattr(
            "shellgpt_cli.plugins._resolve_hook_callback_timeout", lambda: timeout
        )
    ...
    def test_hung_callback_suppresses_repeat_fires(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "shellgpt_cli.plugins._resolve_hook_callback_timeout", lambda: 0.1
        )
    ...
    def test_concurrent_same_tool_calls_with_distinct_ids_both_run(self, monkeypatch):
        ...
        import time
        monkeypatch.setattr(
            "shellgpt_cli.plugins._resolve_hook_callback_timeout", lambda: 5.0
        )
    ...
    def test_hung_worker_caps_new_call_identities_after_suppression(self, monkeypatch, caplog):
        ...
        the callback (#105223)."""
        import shellgpt_cli.plugins_dispatch as dispatch
    ...
    def test_hung_worker_does_not_fail_closed_forever(self, monkeypatch):
        ...
        from shellgpt_cli.plugins import _PRE_TOOL_CALL_TIMEOUT_BLOCK_MESSAGE
    ...
    def test_force_reload_of_one_profile_does_not_orphan_another(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / "profile-a"))
        mgr_a = PluginManager()
        plugins_mod._plugin_manager = mgr_a
        shell_hooks_mod.register_from_config(cfg, accept_hooks=True)

        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / "profile-b"))
        mgr_b = PluginManager()
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from shellgpt_cli import observability
        from shellgpt_cli.plugins import get_pre_tool_call_directive
    ...
    def test_later_block_outranks_earlier_approve(self, monkeypatch):
        ...
        from shellgpt_cli.plugins import _get_pre_tool_call_directive_details
        monkeypatch.setattr(
            "shellgpt_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from shellgpt_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("shellgpt_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("shellgpt_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from shellgpt_cli.plugins import PluginToolOverrideError
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("shellgpt_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `shellgpt plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_shellgpt_home_override(self, tmp_path):
        """set_shellgpt_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``SHELLGPT_HOME`` via a
        context-local ContextVar, which — per
        ``shellgpt_constants.set_shellgpt_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``SHELLGPT_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('SHELLGPT_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['SHELLGPT_HOME'] itself (simulating
        # a real plugin like shellgpt-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_shellgpt_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_shellgpt_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``shellgpt_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``shellgpt_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `shellgpt_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("shellgpt_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("shellgpt_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("shellgpt_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from SHELLGPT_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """SHELLGPT_HOME at the root resolves to 'default'."""
        home = tmp_path / ".shellgpt"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """SHELLGPT_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".shellgpt" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHELLGPT_HOME", str(prof))

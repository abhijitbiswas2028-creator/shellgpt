...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp SHELLGPT_HOME IS the root here).
...
def test_not_ours_returns_none(shellgpt_home):
    from shellgpt_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(shellgpt_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from shellgpt_cli.local_runtime.bootstrap import assets_dir
    from shellgpt_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(shellgpt_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(shellgpt_home):
...
def test_live_props_beats_catalog(shellgpt_home, monkeypatch):
    """A running child's modalities report wins over the catalog: the
    server that will receive the image is the authority."""
    import shellgpt_cli.local_runtime.capabilities as caps
...
def test_lookup_chain_consults_managed_runtime(shellgpt_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(shellgpt_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(shellgpt_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(shellgpt_home, monkeypatch, tmp_path):

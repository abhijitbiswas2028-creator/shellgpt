"""Tests for shellgpt_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import shellgpt_cli.gui_uninstall as gu


def _make_agent(shellgpt_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = shellgpt_home / "shellgpt-agent"
    (agent_root / "shellgpt_cli").mkdir(parents=True)
    (agent_root / "shellgpt_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(shellgpt_home: Path) -> None:
    """Create the source-built GUI artifacts a `shellgpt desktop` run produces."""
    desktop = shellgpt_home / "shellgpt-agent" / "apps" / "desktop"
    ...
    (shellgpt_home / "shellgpt-agent" / "node_modules").mkdir(parents=True)
    (shellgpt_home / "desktop-build-stamp.json").write_text("{}")


def test_gui_install_summary_shape(tmp_path, monkeypatch):
    shellgpt_home = tmp_path / ".shellgpt"
    _make_agent(shellgpt_home)
    _make_gui_build(shellgpt_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(shellgpt_home)
    ...
    assert summary["shellgpt_home"] == str(shellgpt_home)
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    shellgpt_home = tmp_path / ".shellgpt"
    _make_agent(shellgpt_home)
    icon = lde.icon_path(shellgpt_home / "shellgpt-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(shellgpt_home)
    ...
    assert lde.icon_path(shellgpt_home / "shellgpt-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (shellgpt_home / "shellgpt-agent" / "shellgpt_cli").is_dir()
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import shellgpt_cli.uninstall as uninstall

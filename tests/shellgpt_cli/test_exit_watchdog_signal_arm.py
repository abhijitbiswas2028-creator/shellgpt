...
Regression coverage for the #65998 class: a ``shellgpt --tui`` process whose
...
class TestSignalArmLogic:
    ...
    def test_never_raises_even_if_arm_explodes(self, monkeypatch):
        monkeypatch.setenv("SHELLGPT_EXIT_WATCHDOG_S", "7")
...
def test_sigterm_on_wedged_process_forces_exit_within_leash():
    ...
    SHELLGPT_EXIT_WATCHDOG_S; without the signal it would live forever."""
    env = dict(os.environ, SHELLGPT_EXIT_WATCHDOG_S="1", PYTHONPATH=_REPO_ROOT)

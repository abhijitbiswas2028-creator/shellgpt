...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SHELLGPT_HOME with .container-mode file."""
    shellgpt_home = tmp_path / ".shellgpt"
    shellgpt_home.mkdir()
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
    monkeypatch.delenv("SHELLGPT_DEV", raising=False)

    container_mode = shellgpt_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=shellgpt-agent\n"
        "exec_user=shellgpt\n"
        "shellgpt_bin=/data/current-package/bin/shellgpt\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("shellgpt_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "shellgpt-agent"
    assert info["exec_user"] == "shellgpt"
    assert info["shellgpt_bin"] == "/data/current-package/bin/shellgpt"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from shellgpt_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "shellgpt"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "shellgpt-agent" in cmd
    assert "/data/current-package/bin/shellgpt" in cmd
    assert "chat" in cmd

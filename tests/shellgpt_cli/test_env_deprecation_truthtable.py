...
session restoration / cron / the gateway):
    ...
    * There is NO quiet/non-interactive gating in the scanner itself; the
      gateway calls it before setting SHELLGPT_QUIET (gateway/run.py), so the
      warning fires regardless of SHELLGPT_QUIET.

Exercises the real scanner against a real temp SHELLGPT_HOME .env file.
"""
...
TRUTH_TABLE = [
    ...
    pytest.param(
        "TERMINAL_CWD=.\n", {"SHELLGPT_QUIET": "1"}, None, ("TERMINAL_CWD",),
        id="quiet-mode-no-gating-dotenv-warns",
    ),
    pytest.param(
        "API_KEY=x\n",
        {"SHELLGPT_QUIET": "1", "TERMINAL_CWD": "."},
        None,
        (),
...
def test_deprecated_env_warning_truth_table(
    ...
    if env_file is not None:
        (shellgpt_home / ".env").write_bytes(env_file.encode("utf-8"))
    if config_yaml is not None:
        (shellgpt_home / "config.yaml").write_text(config_yaml, encoding="utf-8")

    # Baseline: deprecated keys absent from process env unless the row says so.
    for key in ("TERMINAL_CWD", "MESSAGING_CWD", "SHELLGPT_QUIET"):
        monkeypatch.delenv(key, raising=False)
    for key, value in proc_env.items():
        monkeypatch.setenv(key, value)

    import shellgpt_cli.config as config_module

"""`shellgpt sessions repair --check-only` must fail (non-zero) when the store is unhealthy.
...
from shellgpt_cli import sessions_cmd
...
def test_check_only_exit_status_tracks_probe_verdict(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(shellgpt_state, "DEFAULT_DB_PATH", db_path)

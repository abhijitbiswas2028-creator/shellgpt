...
Real processes with real ShellGPT-shaped argv, real inventory collection
...
reconciliation. No mocks on the components under test.
 ...
 1. Spawn a real process with a `gateway run` command line registered as the
    profile gateway (state file in a temp SHELLGPT_HOME) — real
...
def test_plan_reconciliation_live_windows(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(profiles_mod, "_get_default_shellgpt_home", lambda: home)

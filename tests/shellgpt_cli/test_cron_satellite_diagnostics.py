...
def served_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_PROFILE", "probe")
    ...
    monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: "shellgpt gateway run")
...
def test_status_preserves_profile_health_contract(served_root, capsys, monkeypatch, mode):
    ...
    if mode == "local":
        monkeypatch.setattr("shellgpt_cli.gateway.find_gateway_pids", lambda: [os.getpid()])
    if mode == "external":
    ...
    if mode in {"missing", "stale"}:
        assert "shellgpt --profile default gateway restart" in output
    if mode == "missing":
    ...
    if mode in {"disabled", "excluded", "unrelated_pid"}:
        ...
        assert "shellgpt --profile default gateway install" in output
        assert "sudo shellgpt --profile default gateway install --system" in output
        assert "shellgpt --profile default gateway run" in output
        ...
        assert "shellgpt gateway install   # starts a SECOND gateway" not in output
    if mode == "external":
...
def test_host_record_rung_names_the_roster_and_a_runnable_restart(served_root, capsys, monkeypatch):
    ...
    assert "shellgpt --profile default gateway restart" in output
    assert "\n  If heartbeat never appears, restart: shellgpt gateway restart" not in output
...
def test_satellite_list_and_create_require_own_heartbeat(served_root, capsys, monkeypatch, heartbeat):
    ...
    for output in (created, listed):
        assert ("Check status:  shellgpt cron status" in output) == (heartbeat != "fresh")
    cron.cron_status()
...
def test_standalone_guidance_matches_profile_membership(served_root, monkeypatch, capsys, home_kind):
    from shellgpt_cli.cron import cron_status
...
def test_doctor_bounds_persisted_fire_errors(served_root, capsys, detail):
    from cron import jobs
    from shellgpt_cli.cron import cron_doctor
    ...
    if detail:
        ...
        assert f"shellgpt cron run {records[0]['id']}" in line
...
def test_doctor_reports_persisted_dispatch_health(served_root, capsys, dispatch):
    ...
    assert "Review the findings above, then run `shellgpt cron doctor` again." in output

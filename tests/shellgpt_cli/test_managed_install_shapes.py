...
def test_env_var_names_the_managing_system(shellgpt_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("SHELLGPT_MANAGED", raising=False)
    else:
        monkeypatch.setenv("SHELLGPT_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    shellgpt_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the SHELLGPT_MANAGED of the service."""
    (shellgpt_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("SHELLGPT_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(shellgpt_home, monkeypatch):
    (shellgpt_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("SHELLGPT_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    shellgpt_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("SHELLGPT_MANAGED", managed_value)
    ...
    # `shellgpt update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "shellgpt update"
...
def test_unmanaged_install_offers_no_update_command(shellgpt_home, monkeypatch):
    monkeypatch.delenv("SHELLGPT_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(shellgpt_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = shellgpt_home / ".managed"

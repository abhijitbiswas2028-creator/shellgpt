"""Tests for `shellgpt fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SHELLGPT_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".shellgpt" / "config.yaml"
...
class TestReadChain:
    ...
    def test_reads_new_list_format(self):
        from shellgpt_cli.fallback_cmd import _read_chain
        cfg = {
            "fallback_providers": [
                {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "nous", "model": "ShellGPT-4-Llama-3.1-405B"},
            ]
        }
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from shellgpt_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        ...
        from shellgpt_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "anthropic/claude-sonnet-4.6" in out
        assert "ShellGPT-4" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home):
        ...
        with patch("shellgpt_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("shellgpt_cli.main._require_tty"):
            from shellgpt_cli.fallback_cmd import cmd_fallback_add
    ...
    def test_restore_preserves_absent_active_provider(self):
        ...
        from shellgpt_cli import auth, fallback_cmd
    ...
    def test_picker_failure_restores_persisted_primary_without_masking_error(
        ...
        from shellgpt_cli import fallback_cmd
        ...
        with patch(
            "shellgpt_cli.main.select_provider_and_model",
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home):
        ...
        # Picker returns index 1 (the middle entry, "nous / ShellGPT-4")
        with patch("shellgpt_cli.setup._curses_prompt_choice", return_value=1):
            from shellgpt_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, monkeypatch):
        ...
        from shellgpt_cli.fallback_cmd import cmd_fallback_clear
...
class TestDispatcher:
    ...
    def test_unknown_subcommand_exits(self, isolated_home):
        _write_config(isolated_home, {})
        from shellgpt_cli.fallback_cmd import cmd_fallback

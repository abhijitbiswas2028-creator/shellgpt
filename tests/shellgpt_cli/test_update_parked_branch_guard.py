"""Regression tests for the parked-branch guard in ``shellgpt update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``shellgpt
...
def _no_config(monkeypatch):
    ...
    monkeypatch.setattr(shellgpt_config, "load_config", lambda: {})
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(shellgpt_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    monkeypatch.setattr(shellgpt_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    ...
    monkeypatch.setattr(shellgpt_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        shellgpt_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(shellgpt_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(shellgpt_main, "_capture_active_tool_dependencies", lambda: [])

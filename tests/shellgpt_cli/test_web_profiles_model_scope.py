...
``_write_profile_model`` used to enter only the SHELLGPT_HOME override. Once the dashboard
...
def homes(tmp_path, monkeypatch):
    """A throwaway SHELLGPT_HOME whose named profile carries its own ``.env`` credential.
...
def probe(monkeypatch):
    ...
    import shellgpt_cli.models_validate as mv
...
def test_model_pick_resolves_key_env_from_profile_scope(client, homes, probe):
    ...
    from shellgpt_cli.web_server_profiles import _shellgpt_home_scope
    with _shellgpt_home_scope(homes[1]):
...
def test_model_pick_for_default_from_named_profile_launch(homes, probe, monkeypatch):
    ...
    root, demo = homes
    monkeypatch.setenv("SHELLGPT_HOME", str(demo))
    (root / ".env").write_text("ACME_RELAY_KEY=root-key\n", encoding="utf-8")
    from shellgpt_cli.config import invalidate_env_cache, load_config
    from shellgpt_cli.web_server_profiles import _shellgpt_home_scope
    invalidate_env_cache()
    from shellgpt_cli import web_server
    ...
    with _shellgpt_home_scope(root):
        assert (load_config().get("model") or {}).get("default") == "acme/mini"
    with _shellgpt_home_scope(demo):
...
def test_model_set_names_target_from_a_backend_launched_as_another_profile(homes, probe, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(demo))  # launched as A = demo
    invalidate_env_cache()
    from shellgpt_cli import web_server

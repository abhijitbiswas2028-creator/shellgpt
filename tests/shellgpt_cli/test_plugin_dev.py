...
def test_doctor_restores_global_tool_policy_and_module_state(tmp_path: Path) -> None:
    ...
    before_modules = {
        ...
        if name == "shellgpt_plugins" or name.startswith("shellgpt_plugins.")
    }
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``shellgpt plugins doctor`` with no argument defaults to ``.``.

    Before the manifest guard, that copied the whole working directory into
    a temporary SHELLGPT_HOME — running it from ``$HOME`` copied the home
    directory, cloud-storage placeholders included.
    """
    import os
    import shutil

    from shellgpt_cli import plugin_dev
...
def test_resolve_accepts_category_layout(tmp_path: Path) -> None:
    ...
    from shellgpt_cli.plugin_dev import resolve_plugin_path
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = shellgpt_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_shellgpt_home", lambda: shellgpt_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a shellgpt-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded shellgpt-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("shellgpt-plugin-doctor-*"))

...
See: NousResearch/shellgpt-agent#7622
"""
...
_GATEWAY_CMD = "python -m shellgpt_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m shellgpt_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("shellgpt_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            ...
            patch(
                "shellgpt_cli.gateway.get_launchd_label",
                return_value="ai.shellgpt.gateway.myprofile",
            ),
            patch(
                "shellgpt_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.shellgpt.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.shellgpt.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.shellgpt.gateway": 123,
            "ai.shellgpt.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "shellgpt_cli.gateway.get_launchd_label",
                return_value="ai.shellgpt.gateway",
            ),
            patch(
                "shellgpt_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.shellgpt.gateway", "ai.shellgpt.gateway-profile-b"],
            ),
            patch(
                "shellgpt_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.shellgpt.gateway-unmapped\n"
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every shellgpt-gateway* unit unconditionally."""
        with (

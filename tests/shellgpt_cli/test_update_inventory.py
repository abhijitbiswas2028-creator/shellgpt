"""Tests for shellgpt_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("shellgpt_cli.profiles._get_default_shellgpt_home", lambda: default_home)
    monkeypatch.setattr("shellgpt_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("shellgpt_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    ...
    monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: {
        100: "shellgpt gateway run", 200: "shellgpt --profile work gateway run"}.get(pid))
    monkeypatch.setattr("shellgpt_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("shellgpt_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("shellgpt_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "shellgpt_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("shellgpt_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("shellgpt_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from shellgpt_cli.update_inventory import describe_restart_mechanism

        assert "shellgpt -p work gateway restart" in describe_restart_mechanism(
            by_profile["work"].restart_via, "work"
        )

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("shellgpt_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "shellgpt_cli.config.recommended_update_command_for_method",
            lambda m: "docker pull nousresearch/shellgpt-agent:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_stopped_record_with_recycled_pid_is_not_a_runtime(self, fleet, monkeypatch):
        ...
        gateway the restart phase can never touch (that phantom made `shellgpt update` exit partial)."""
        work_home = fleet / "home" / "profiles" / "work"
        _write_state(work_home, 200, gateway_state="stopped")
        monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: {
            100: "shellgpt gateway run", 200: "C:/Windows/system32/dllhost.exe /Processid:{X}"}.get(pid))
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from shellgpt_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "shellgpt_cli.config.detect_install_method",
            "shellgpt_cli.build_info.get_code_identity",
            "shellgpt_cli.profiles._get_default_shellgpt_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        ...
        home.mkdir()
        monkeypatch.setattr("shellgpt_cli.config.get_shellgpt_home", lambda: home, raising=False)
        ur._current = None

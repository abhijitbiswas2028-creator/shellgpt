...
def test_vertex_adc_counts_as_explicit_when_config_present(tmp_path, monkeypatch):
    ...
    ShellGPT at it (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH),
    ...
    # vertex.project_id in config.yaml is a deliberate, ShellGPT-scoped signal.
    ...
    # No ShellGPT-scoped Vertex config at all → stays hidden.
...
def test_vertex_ambient_google_creds_env_does_not_count_as_explicit(tmp_path, monkeypatch):
    ...
    unrelated GCP work) must NOT mark Vertex explicit — only ShellGPT-scoped
...
def test_profile_dotenv_key_counts_as_explicit_when_process_env_lacks_it(tmp_path, monkeypatch):
    ...
    from shellgpt_cli.auth import is_provider_explicitly_configured
    from shellgpt_cli.web_server_profiles import _config_profile_scope
...
def test_dotenv_key_counts_when_shell_exports_the_var_empty(tmp_path, monkeypatch):
    ...
    (tmp_path / "shellgpt" / ".env").write_text("DEEPSEEK_API_KEY=sk-dotenv-only-secret\n")

    from shellgpt_cli.auth import is_provider_explicitly_configured, resolve_api_key_provider_credentials
...
def test_bedrock_not_explicit_without_aws_env(tmp_path, monkeypatch, _clean_aws_env):
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / "shellgpt"))
    (tmp_path / "shellgpt").mkdir(parents=True, exist_ok=True)
...
def test_bedrock_ambient_aws_profile_is_not_explicit(tmp_path, monkeypatch, _clean_aws_env):
    ...
    not an explicit ShellGPT provider choice — same principle as gh_cli-seeded

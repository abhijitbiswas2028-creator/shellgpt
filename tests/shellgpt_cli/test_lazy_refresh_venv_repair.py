...
def test_detect_returns_none_when_probe_subprocess_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        shellgpt_cli_main_install_repair, "_resolve_install_target_python", lambda *a, **k: python
    )
    monkeypatch.setattr(
        m.subprocess,
        "run",
...
def test_venv_repair_restores_snapshots_with_an_isolated_uv_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("shellgpt_cli.managed_uv.ensure_uv", lambda **_k: "uv")

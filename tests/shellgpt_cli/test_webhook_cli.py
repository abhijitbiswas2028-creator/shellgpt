"""Tests for shellgpt_cli/webhook.py — webhook subscription CLI."""
...
from shellgpt_cli.webhook import (
    webhook_command,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "shellgpt_cli.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "shellgpt_cli.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    ...
    def test_blocks_list_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("shellgpt_cli.webhook._is_webhook_enabled", lambda: False)

...
``shellgpt_cli.config`` while the older ``config.py`` does not export them.
Construction of ``ShellGPTCLI`` then dies before the agent-setup mixin can
print ``partial_update_hint``. ``cmd_chat`` is the load-bearing catch:
bare ``shellgpt`` and ``shellgpt chat`` (including the fast-chat launch path)
all go through it.
"""
...
def _missing_config_name_error(name: str = "resolve_turn_limit") -> ImportError:
    exc = ImportError(
        f"cannot import name '{name}' from 'shellgpt_cli.config'"
    )
    exc.name = "shellgpt_cli.config"
    return exc
...
def test_emit_hint_for_missing_resolve_turn_limit():
    ...
    assert "shellgpt update" in text

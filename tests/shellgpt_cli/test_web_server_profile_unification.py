...
profile switcher can target any profile's SHELLGPT_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via SHELLGPT_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_shellgpt_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_shellgpt_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_resolves_profile_secret_source_scope(
        ...
        def _worker_sources(shellgpt_home):
            if Path(shellgpt_home).resolve() == worker_home.resolve():
    ...
    def test_mcp_list_expands_url_ref_from_profile_secret_scope(
        ...
        monkeypatch.setattr(
            ...
            lambda shellgpt_home: {"MCP_GH_URL": "http://worker/mcp"}
            if Path(shellgpt_home).resolve() == worker_home.resolve() else {},
        )
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same SHELLGPT_HOME the rest
        ...
        monkeypatch.setattr(
            "shellgpt_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_shellgpt_home()))
            return None
        ...
        assert resp.json()["shellgpt_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_shellgpt_home(self, isolated_profiles, monkeypatch):

        monkeypatch.setattr(
            "shellgpt_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["SHELLGPT_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "SHELLGPT_TUI_GATEWAY_URL" not in env

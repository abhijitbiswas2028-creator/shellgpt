"""Tests for ``shellgpt doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from shellgpt_cli import doctor_live
from shellgpt_cli.doctor_live import (
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `shellgpt doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("shellgpt_cli.doctor.SHELLGPT_HOME", tmp_path / "home")
        monkeypatch.setattr("shellgpt_cli.doctor.PROJECT_ROOT", tmp_path / "root")

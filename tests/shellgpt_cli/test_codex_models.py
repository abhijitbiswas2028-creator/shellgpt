...
from shellgpt_cli.codex_models import (
...
def test_codex_catalog_never_offers_chatgpt_rejected_pro_slugs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "shellgpt_cli.codex_models._fetch_models_from_api", lambda access_token: templates
    )
...
def test_picker_never_synthesizes_900k_for_pro_or_unknown_slugs():
    ...
    from shellgpt_cli.codex_models import _finalize_codex_models
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from shellgpt_cli import codex_models
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    from shellgpt_cli.model_setup_flows import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "shellgpt_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "shellgpt-auth-store"},
    )
    monkeypatch.setattr(
        "shellgpt_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("shellgpt_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a ShellGPTCLI with minimal mocking."""
    ...
    with (
        ...
        cli = ShellGPTCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "SHELLGPT_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import ShellGPTCLI
            cli = ShellGPTCLI()

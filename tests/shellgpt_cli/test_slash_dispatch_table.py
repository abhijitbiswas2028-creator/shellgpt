"""Slash-command dispatch semantics in cli.ShellGPTCLI.
...
def test_dispatch_return_semantics_and_side_effects():
    c = _cli()
    with patch.object(ShellGPTCLI, "_toggle_yolo", return_value=None) as m, \
            patch("shellgpt_cli.plugins.fire_pre_command_hook") as hook:
    ...
    c = _cli()
    with patch.object(ShellGPTCLI, "_handle_resume_command") as m:
    ...
    c = _cli()
    with patch.object(ShellGPTCLI, "_handle_handoff_command", return_value=False):
        assert c.process_command("/handoff telegram") is False
    with patch.object(ShellGPTCLI, "_handle_handoff_command", return_value=True):
        assert c.process_command("/handoff telegram") is True
    with patch.object(ShellGPTCLI, "_handle_update_command", return_value=True):
        assert c.process_command("/update") is False
    with patch.object(ShellGPTCLI, "_handle_update_command", return_value=False):
...
def test_unknown_command_falls_through():
    ...
    with patch.object(ShellGPTCLI, "_process_unregistered_slash", return_value=True) as m:

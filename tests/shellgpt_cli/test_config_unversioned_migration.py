...
the installers seed it from cli-config.yaml.example and targeted writers (``shellgpt config set``,
...
def test_update_keeps_user_values_of_an_unversioned_config_and_migrates_legacy_keys(shellgpt_home):
    from shellgpt_cli.config import DEFAULT_CONFIG, set_config_value
    from shellgpt_cli.personality import persist_personality
    from shellgpt_cli.update_cmd import _check_and_apply_config_migration
    ...
    # Installed with `shellgpt plugins install` and never enabled.
    plugin = shellgpt_home / "plugins" / "notes-helper"
    ...
    (shellgpt_home / "SOUL.md").write_text(soul, encoding="utf-8")
    assert persist_personality("kawaii")
    for key, value in USER_CHOICES.items():
        set_config_value(key, value)
    before = _raw(shellgpt_home)

    _check_and_apply_config_migration()

    after = _raw(shellgpt_home)
    ...
    assert (shellgpt_home / "SOUL.md").read_text(encoding="utf-8") == soul


def test_config_seeded_from_the_template_reads_as_current(shellgpt_home):
    """install.sh, install.ps1, docker/stage2-hook.sh and `shellgpt doctor --fix` copy the template."""
    from shellgpt_cli.config import check_config_version

    shutil.copy(TEMPLATE, shellgpt_home / "config.yaml")

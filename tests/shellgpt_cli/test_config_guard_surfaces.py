...
def _isolated_config_env(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("SHELLGPT_IGNORE_USER_CONFIG", raising=False)
    yield
    os.environ.pop("SHELLGPT_IGNORE_USER_CONFIG", None)
...
class TestGatewayGuard:
    def test_gateway_refuses_corrupt_config(self, tmp_path, capsys):
        ...
        assert "ShellGPT stopped because your settings file" in capsys.readouterr().err
...
class TestCronRunJobGuard:
    ...
    def test_run_job_fails_closed_on_corrupt_config(self, tmp_path):
        ...
        assert "ShellGPT stopped because your settings file" in error
    ...
    def test_run_job_no_agent_exempt(self, tmp_path):
        ...
        assert "ShellGPT stopped because your settings file" not in (error or "")


class TestServeGuard:
    def test_serve_headless_refuses_corrupt_config(self, tmp_path, capsys):
        """The `shellgpt serve` headless path fails closed before startup."""
        from argparse import Namespace

        from shellgpt_cli import main as main_mod

"""Tests for the top-level `./shellgpt` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./shellgpt` should use `shellgpt_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "shellgpt"
    called = []

    fake_main_module = types.ModuleType("shellgpt_cli.main")

    def fake_main():
        called.append("shellgpt_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "shellgpt_cli.main", fake_main_module)
    ...
    assert called == ["shellgpt_cli.main"]

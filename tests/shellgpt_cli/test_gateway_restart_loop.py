...
Covers:
- Defense 1: gateway stop/restart refuse when _SHELLGPT_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `python*` carries no shellgpt/gateway token yet takes it down — on every platform.
        ...
        "pkill -9 python3",
        "pkill -f python",
        'pkill -f "python -m shellgpt_cli.main"',
        "pkill -f shellgpt_cli",
        "killall -9 python3.12",
        "sudo pkill -9 python3",
        "pgrep python | xargs kill -9",
        "kill $(pidof python3)",
        # Windows spellings of the shellgpt-gateway forms (salvaged from #94379).
        "shellgpt.exe gateway restart",
        "shellgpt.cmd gateway stop",
        r'"C:\Program Files\shellgpt.exe" gateway restart',
        "taskkill /F /IM shellgpt-gateway.exe",
        "Stop-Process -Name shellgpt-gateway -Force",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        # Unrelated scripts that merely contain "shellgpt" cannot match the gateway cmdline.
        "pkill -f 'shellgpt-polis/run.sh'",
        "pkill -f my_shellgpt_bot.py",
        "pgrep python",
        "shellgpt.exe gateway start",
        "my-shellgpt.exe gateway restart",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.shellgpt.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.shellgpt/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l shellgpt-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.shellgpt.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.shellgpt.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.shellgpt.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.shellgpt/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.shellgpt.gateway-apollo:/a.plist' "
            "'ai.shellgpt.gateway-cronus:/c.plist' 'ai.shellgpt.gateway-plutus:/p.plist' "
            "'ai.shellgpt.gateway:/Users/botuser/Library/LaunchAgents/ai.shellgpt.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated shellgpt note on the next line"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.shellgpt.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.shellgpt.gateway',
        "launchctl kick'start' -k gui/501/ai.shellgpt.gateway",
        "launchctl kick\\start -k gui/501/ai.shellgpt.gateway",
        'launchctl "kickstart" -k gui/501/ai.shellgpt.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.shellgpt.gateway',
        "launchctl dis\\able gui/501/ai.shellgpt.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.shellgpt."gateway"',
        # Same class on the systemctl and shellgpt-CLI branches.
        'systemctl re"start" shellgpt-gateway',
        'shellgpt gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.shellgpt.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.shellgpt.update-checker',
        'systemctl re"start" shellgpt-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shellgpt cron list",
        "shellgpt update",
        "shellgpt config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `shellgpt gateway start` is benign — starting a gateway from inside a
        ...
        "shellgpt gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway shellgpt
        # services must not be falsely blocked (the old `.*shellgpt` matched any
        # shellgpt token).
        "launchctl unload ai.shellgpt.update-checker.plist",
        "launchctl restart ai.shellgpt.daemon",
        ...
        "systemctl restart shellgpt-meta.service",
        "systemctl restart shellgpt-cron-helper",
        ...
        "echo after the shellgpt gateway restarted cleanly",
        "the shellgpt gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "shellgpt skill view gateway-notes && echo shellgpt gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `shellgpt` is a path component
        # there, not a command.
        "cat '/docs/shellgpt gateway restart-notes.md'",
        "less /home/user/notes/shellgpt gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shellgpt gateway restart; echo done",
        "shellgpt gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;shellgpt gateway restart",
        "true && shellgpt gateway stop",
        "echo $(shellgpt gateway restart)",
        "echo `shellgpt gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.shellgpt.gateway"])',
        'subprocess.run(["shellgpt", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.shellgpt.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["shellgpt", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: shellgpt gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `shellgpt -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry SHELLGPT_HOME/SHELLGPT_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("SHELLGPT_PROFILE", "zeus")
        monkeypatch.delenv("SHELLGPT_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "shellgpt -p zeus gateway stop",
        "shellgpt -p zeus gateway restart",
        "shellgpt --profile zeus gateway restart",
        "shellgpt --profile zeus gateway stop",
        "shellgpt --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "shellgpt -v -p zeus gateway restart",
        "shellgpt -p zeus -v gateway restart",
        "shellgpt --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "shellgpt -p venus gateway stop",
        "shellgpt -p venus gateway restart",
        "shellgpt --profile venus gateway restart",
        "shellgpt --profile=venus gateway stop",
        "shellgpt -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "shellgpt -p zeus gateway start",
        "shellgpt -p zeus gateway start --all",
    ])
    ...
    def test_shellgpt_home_derived_profile(self, monkeypatch):
        # Without SHELLGPT_PROFILE the guard falls back to the SHELLGPT_HOME-
        ...
        assert _contains_gateway_lifecycle_command("shellgpt -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("shellgpt -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("shellgpt -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("shellgpt gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_shellgpt_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade shellgpt then run shellgpt gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_shellgpt_gateway_once.sh). The script must live under
        # SHELLGPT_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
        scripts_dir = tmp_path / ".shellgpt" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SHELLGPT_GATEWAY", raising=False)
        import shellgpt_cli.gateway as gw
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SHELLGPT_GATEWAY=1.

    Issue #37453: systemctl --user restart shellgpt-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart shellgpt-gateway",
        "systemctl --user restart shellgpt-gateway",
        "systemctl stop shellgpt-gateway.service",
        "shellgpt gateway restart",
        "shellgpt gateway uninstall",
        "launchctl kickstart gui/501/ai.shellgpt.gateway",
        "launchctl bootout gui/501/ai.shellgpt.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.shellgpt.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.shellgpt/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.shellgpt.gateway.restart-once.plist",
        "pkill -f shellgpt.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart shellgpt-gateway", force=True
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-shellgpt label: label-independent detection is the point
        # (#62891 second reproduction used `ai.shellgpt.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.shellgpt.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _SHELLGPT_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _SHELLGPT_GATEWAY=1 is in the
        ...
        result = json.loads(tt.terminal_tool(command="shellgpt gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["shellgpt gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.shellgpt.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.shellgpt.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-shellgpt systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("please run shellgpt gateway restart", None)
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeshellgpt gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SHELLGPT_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("shellgpt gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 shellgpt gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_shellgpt_home()` falls back to Path.home(), which raises
        RuntimeError when neither SHELLGPT_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart shellgpt-gateway' .",
        "grep -rn 'shellgpt gateway restart' ./logs",
        "rg 'shellgpt gateway restart' ../archive",
        "grep -c 'systemctl stop shellgpt-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM shellgpt_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell shellgpt gateway restart"',
        'sqlite3 ./db ".system systemctl restart shellgpt-gateway"',
        'psql ./x -c "\\! systemctl restart shellgpt-gateway"',
        "grep -r 'shellgpt gateway restart' . | sh",
        "grep -r 'shellgpt gateway restart' ./logs | bash",
        "grep -r 'shellgpt gateway restart' . | sudo sh",
        "grep -r 'x' . ; shellgpt gateway restart",
        "grep -r 'x' . && systemctl restart shellgpt-gateway",
        'grep -r "$(shellgpt gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart shellgpt-gateway",
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    shellgpt_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run shellgpt gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
        (tmp_path / ".shellgpt").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run shellgpt gateway restart nightly",
        ))
...
class TestTerminalToolGatewayLifecycleGuardRemote:
    ...
    def test_unscannable_executed_script_names_the_reason(self, monkeypatch, tmp_path):
        ...
        import tools.terminal_tool as tt
        from shellgpt_cli.sqlite_safe_read import connect_tracked
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop shellgpt-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart shellgpt-gateway' /var/log/syslog",
        "rg 'shellgpt gateway restart' /home/user/.shellgpt/logs/",
        "journalctl -u shellgpt-gateway --grep 'systemctl restart shellgpt-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'shellgpt_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell shellgpt gateway restart"',
        'psql -c "\\! systemctl restart shellgpt-gateway"',
        "grep 'systemctl restart shellgpt-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart shellgpt-gateway",
        'grep "$(systemctl restart shellgpt-gateway)" f',
        "grep 'restart' log; systemctl restart shellgpt-gateway",
        'sqlite3 db "SELECT 1"; shellgpt gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "shellgpt gateway restart",
        "sudo systemctl stop shellgpt-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart shellgpt-gateway%'\""
        )

...
The notice prints the most recent curator run summary on `shellgpt update`,
...
session start) so users normally never see the rename map. `shellgpt update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from shellgpt_cli import update_cmd_maint

...
from shellgpt_cli.memory_setup import _CANCELLED
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_shellgpt_home", lambda: tmp_path)
    monkeypatch.setattr("shellgpt_cli.config.load_config", lambda: {"memory": {}})
    monkeypatch.setattr("shellgpt_cli.config.save_config", save_config)
...
# behavior is covered by tests/shellgpt_cli/test_memory_setup_env_denylist.py,
# which exercises the current save_env_value-routed signature
# (env_writes, shellgpt_home=None) \u2014 these three tests pinned the prior direct
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on shellgpt update."""
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("shellgpt_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "shellgpt_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )

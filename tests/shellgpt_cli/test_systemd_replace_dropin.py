"""A ShellGPT-authored systemd ``--replace`` drop-in must not survive unit refresh.

An older ShellGPT wrote ``<unit>.d/20-replace.conf`` to end a respawn storm; it overrides ExecStart
with ``gateway run --replace``. Combined with the cross-profile ownership guard, that override turns
a per-profile fleet's unit into one that can never start (#119467). Refresh retires the ShellGPT file,
...
def _current_unit_with_dropin(tmp_path, monkeypatch, dropin_text: str):
    unit = tmp_path / "shellgpt-gateway.service"
    ...
    monkeypatch.setattr(gateway_cli, "_sync_shellgpt_home_from_systemd_unit", lambda **_: None)
    calls = []
...
def test_refresh_retires_the_shellgpt_replace_dropin_even_when_the_unit_text_is_current(tmp_path, monkeypatch, capsys):
    ...
    unit, dropin, calls = _current_unit_with_dropin(tmp_path, monkeypatch, SHELLGPT_DROPIN)
...
def test_refresh_keeps_an_operator_written_dropin_of_the_same_name(tmp_path, monkeypatch):
    """Only the file ShellGPT authored (recognised by its own comment) is retired."""
    unit, dropin, calls = _current_unit_with_dropin(
        tmp_path, monkeypatch, "[Service]\nExecStart=\nExecStart=/opt/shellgpt gateway run --replace\n")

...
The recovery path lives in ``ShellGPTCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A ShellGPTCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(ShellGPTCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise ShellGPTCLI._recover_terminal_after_interrupt()."""
    ...
    def test_redraw_still_runs_when_flush_fails(self, bare_cli):
        ...
        with patch(
            "shellgpt_cli.curses_ui.flush_stdin", side_effect=OSError("no tty")
        ):
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "shellgpt_cli.curses_ui.flush_stdin",

...
Desktop sidebar lists); the pick reaches the TUI as ``SHELLGPT_CWD``/``SHELLGPT_TUI_CWD`` and a
...
from shellgpt_cli import web_server, web_server_chat
from shellgpt_cli.web_routers import chat_workspaces
...
def test_resolve_chat_cwd_fails_closed_and_reaches_the_tui_env(monkeypatch, tmp_path):
    """A picked directory becomes SHELLGPT_CWD + SHELLGPT_TUI_CWD on the PTY child; a missing one is a
    ...
    assert env["SHELLGPT_CWD"] == str(tmp_path)
    assert env["SHELLGPT_TUI_CWD"] == str(tmp_path)

    _argv, _cwd, env = web_server_chat._resolve_chat_argv()
    assert "SHELLGPT_TUI_CWD" not in env

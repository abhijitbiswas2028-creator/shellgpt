...
``ShellGPTCLI`` holds one long-lived agent and read the chain once in ``__init__``; ``shellgpt fallback
add`` from another terminal never reached the open chat. The turn loop (``ShellGPTCLI.chat``) now
...
def _chat_turn(monkeypatch, shell, config_text: str) -> None:
    """Drive ``ShellGPTCLI.chat`` past the fallback sync against ``config_text`` as the live config.yaml."""

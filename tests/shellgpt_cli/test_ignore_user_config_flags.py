"""Tests for --ignore-user-config and --ignore-rules flags on `shellgpt chat`.
...
files. In ShellGPT the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.shellgpt/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → ShellGPTCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("SHELLGPT_IGNORE_USER_CONFIG", "SHELLGPT_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``SHELLGPT_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<shellgpt_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With SHELLGPT_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._shellgpt_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_shellgpt_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("SHELLGPT_IGNORE_USER_CONFIG", "true")  # not "1"

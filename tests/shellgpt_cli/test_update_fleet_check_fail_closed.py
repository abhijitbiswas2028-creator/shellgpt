...
``shellgpt update`` exited 0 with zero rows — even when a gateway was
...
from shellgpt_cli.main import _fleet_probe_expected_runtimes
from shellgpt_cli.update_inventory import RuntimeRecord
...
class TestEmptySnapshotFailClosed:
    ...
    def test_incomplete_when_restart_phase_touched_gateways(self):
        ...
        assert (
            _fleet_probe_expected_runtimes(None, [], None, ["shellgpt-gateway"], set())
            is True
        )

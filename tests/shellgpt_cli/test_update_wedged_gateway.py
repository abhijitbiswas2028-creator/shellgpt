...
still running after 180.0s — restart may fail") and could deadlock `shellgpt
...
def tmp_path():
    ...
    The loop-tick witness tests bind real UNIX sockets under SHELLGPT_HOME.
...
def _launchd_harness(monkeypatch, tmp_path, pid):
    ...
    patched ``_process_shellgpt_home``).
    """
    events = []
    ...
    monkeypatch.setattr(
        "gateway.shutdown_watchdog._process_shellgpt_home", lambda: tmp_path
    )
    return events

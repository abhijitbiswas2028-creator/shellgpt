"""``shellgpt auth add openai-codex --browser``: opt-in loopback authorization-code + PKCE (#95743).
...
def test_browser_flag_runs_loopback_pkce_and_stores_loopback_source(tmp_path, monkeypatch, fake_openai):
    ...
    (tmp_path / "shellgpt" / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    from shellgpt_cli import auth_codex_browser as browser_mod
    ...
    monkeypatch.setattr(
        "shellgpt_cli.auth._codex_device_code_login",
...
def test_default_is_device_code_and_busy_callback_port_falls_back(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("shellgpt_cli.auth._codex_device_code_login", _device)
    bind_attempts = []
    ...
    sources = [e["source"] for e in json.loads((tmp_path / "shellgpt" / "auth.json").read_text())["credential_pool"]["openai-codex"]]

"""Tests for the per-profile Projects store (shellgpt_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="ShellGPT Agent", folders=["/tmp/shellgpt"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "shellgpt-agent"
    assert proj.name == "ShellGPT Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/shellgpt"
    assert [f.path for f in proj.folders] == ["/tmp/shellgpt"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "shellgpt-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SHELLGPT_HOME.

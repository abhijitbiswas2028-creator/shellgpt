...
When `shellgpt update` runs INSIDE the gateway's own process tree (the
shellgpt-auto-update cron job), waiting for that gateway to exit is a
circular wait:

  gateway waits on all in-flight work units (#77184)
    -> cron agent session waits on the `shellgpt update` process
      -> `shellgpt update` waits on the gateway to exit  [back to A]
...
class TestSelfRestartFireAndForget:
    ...
    def test_refuses_non_ancestor_pid(self):
        from shellgpt_cli.gateway import _request_gateway_self_restart
...
class TestDrainOrSignalTriage:
    ...
    def test_ancestor_gateway_is_signalled_fire_and_forget(self, monkeypatch):
        ...
        from shellgpt_cli.update_cmd import _drain_or_signal_gateway_for_update

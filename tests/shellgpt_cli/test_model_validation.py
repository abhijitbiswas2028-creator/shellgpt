"""Tests for provider-aware `/model` validation in shellgpt_cli.models."""

import pytest
from unittest.mock import MagicMock, patch

from shellgpt_cli.models import azure_foundry_model_api_mode, copilot_model_api_mode, curated_models_for_provider, fetch_api_models, normalize_provider, opencode_model_api_mode, parse_model_input, probe_api_models, provider_model_ids
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("shellgpt_cli.models.fetch_api_models", return_value=api_models), \
         patch("shellgpt_cli.models.probe_api_models", return_value=probe_payload):
...
class TestCuratedModelsForProvider:
    ...
    def test_live_catalog_projected_to_tuples_else_static_fallback(self):
        with patch("shellgpt_cli.models.provider_model_ids", return_value=["m-live"]):
            assert curated_models_for_provider("nous") == [("m-live", "")]
        with patch("shellgpt_cli.models.provider_model_ids", return_value=[]), patch.dict(
            "shellgpt_cli.models._PROVIDER_MODELS", {"nous": ["m-static"]}
        ):
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("shellgpt_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("shellgpt_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("shellgpt_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("shellgpt_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestRequestedIdIsNeverRewritten:
    ...
    def test_static_catalog_near_miss_keeps_requested_id(self):
        ...
        with patch("shellgpt_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a ShellGPT picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("shellgpt_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("shellgpt_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a ShellGPT User-Agent.
    ...
    def test_probe_sends_shellgpt_user_agent(self):
        ...
        assert ua.startswith("shellgpt-cli/"), (
            f"User-Agent must advertise shellgpt-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("shellgpt-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("shellgpt_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "shellgpt_cli.models.provider_model_ids",
...
class TestValidateRequestedModelNousPortalRecommendations:
    ...
    hardcoded curated catalog -- even though `shellgpt chat` already accepts
    ...
    def _validate_nous(self, model, api_models=None, portal_payload=None, portal_raises=False):
        ...
        with patch("shellgpt_cli.models.fetch_api_models", return_value=api_models), \
             patch("shellgpt_cli.models.probe_api_models", return_value=probe_payload), \
             patch("shellgpt_cli.models.fetch_nous_recommended_models", side_effect=_fetch_portal), \
             patch("shellgpt_cli.models._resolve_nous_portal_url", return_value="https://portal.nousresearch.com"), \
             patch("shellgpt_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_non_nous_provider_does_not_consult_portal_feed(self):
        ...
        with patch("shellgpt_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("shellgpt_cli.models.probe_api_models", return_value=probe_payload), \
             patch("shellgpt_cli.models.fetch_nous_recommended_models") as mock_portal, \
             patch("shellgpt_cli.models._model_in_provider_catalog", return_value=False):
...
class TestValidateCustomUnreachableFallback:
    ...
    def _validate(self, model, provider, models, **kw):
        ...
        with patch("shellgpt_cli.models.probe_api_models", return_value=probe):
...
class TestProfileCatalogAuthoritative:
    ...
    def test_unavailable_profile_catalog_keeps_generic_fallback(self, relay_profile):
        ...
        with patch("shellgpt_cli.models.fetch_api_models", return_value=["other-vendor/model-a"]), \
             patch("shellgpt_cli.models.provider_model_ids", return_value=[]):
            result = validate_requested_model(
    ...
    def test_self_hosted_aliases_accept_listed_ids_with_whitespace(self):
        for provider in ("vllm", "ollama", "llamacpp", "local", "lmstudio"):
            ...
            if provider == "lmstudio":
                with patch("shellgpt_cli.models_local.probe_lmstudio_models", return_value=[model]):
...
def test_picker_payload_omits_ids_the_validator_rejects_for_whitespace():
    ...
    from shellgpt_cli.inventory import ConfigContext, build_models_payload
    ...
    with patch("shellgpt_cli.model_switch.list_authenticated_providers", return_value=rows), \
         patch("shellgpt_cli.inventory._local_runtime_row", return_value=None), \
         patch("shellgpt_cli.inventory._moa_provider_row", return_value=None):

"""Tests for shellgpt_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SHELLGPT_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the ShellGPT-managed node dir.
"""
...
def _make_shellgpt_node(shellgpt_home: Path) -> Path:
    """Create a fake $SHELLGPT_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = shellgpt_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    _make_shellgpt_node(shellgpt_home)
    ...
    removed = uninstall.remove_node_symlinks(shellgpt_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    shellgpt_home = fake_home / ".shellgpt"
    node_bin = _make_shellgpt_node(shellgpt_home)

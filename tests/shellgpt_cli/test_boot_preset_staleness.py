...
def test_presets_stale_when_a_staged_model_has_no_section(shellgpt_home):
    from shellgpt_cli.local_runtime.bootstrap import _presets_stale

    _stage(shellgpt_home, "model-a")
    _stage(shellgpt_home, "model-b")
    _write_presets(shellgpt_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(shellgpt_home):
    from shellgpt_cli.local_runtime.bootstrap import _presets_stale
...
def test_legacy_presets_without_model_paths_are_regenerated(shellgpt_home):
    from shellgpt_cli.local_runtime.bootstrap import _presets_stale

    _stage(shellgpt_home, "model-a")
    _write_presets(shellgpt_home, "model-a")
    ini = shellgpt_home / "runtimes/llamacpp/presets.ini"
    ini.write_text("[model-a]\nctx-size = 65536\n")
    assert _presets_stale()


def test_no_models_is_never_stale(shellgpt_home):
    from shellgpt_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(shellgpt_home, monkeypatch):
    ...
    import shellgpt_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(shellgpt_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(shellgpt_home, monkeypatch):
    import shellgpt_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.endpoint._state_endpoint", lambda: None)

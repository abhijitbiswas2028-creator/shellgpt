"""Unit tests for shellgpt_cli.security_audit — parsers + OSV plumbing.
...
from shellgpt_cli import security_audit as sa
    ...
    def test_findings_sorted_by_severity_desc(self, tmp_path: Path):
        ...
        with patch.object(sa, "_osv_query_batch", side_effect=fake_batch), \
            ...
            findings = sa.run_audit(
                skip_venv=True, skip_plugins=False, skip_mcp=True, shellgpt_home=tmp_path
            )
...
class TestExitCodes:
    ...
    def test_unknown_fail_on_value_exits_two(self, tmp_path: Path, monkeypatch, capsys):
        monkeypatch.setattr(sa, "get_shellgpt_home", lambda: str(tmp_path))

...
`shellgpt curator pin <name>` on an unmanaged skill (curation-eligible but no
...
becomes meaningful after `shellgpt curator adopt`) and makes the message say
...
def test_pin_still_refuses_bundled_skills(monkeypatch, capsys):
    import shellgpt_cli.curator as curator_cli
    import tools.skill_usage as skill_usage

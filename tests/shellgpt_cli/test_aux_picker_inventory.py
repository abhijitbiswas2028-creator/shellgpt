...
Every aux picker (``shellgpt model`` → Configure auxiliary models, the
``shellgpt tools`` vision picker, and any future one) must route through
``shellgpt_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def configured_home(tmp_path, monkeypatch):
    """A SHELLGPT_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from shellgpt_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("shellgpt_cli.model_switch.list_authenticated_providers", _capture):

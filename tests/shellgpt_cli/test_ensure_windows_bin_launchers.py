"""``shellgpt`` must survive git operations on the checkout (launcher layout).

The Windows ``shellgpt`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``SHELLGPT_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``shellgpt-agent\\bin``) was swept by ``shellgpt update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``shellgpt`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``shellgpt update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``shellgpt_constants.venv_bin_dir``), so these
...
from shellgpt_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: SHELLGPT_HOME/shellgpt-agent/venv/Scripts + launchers."""
    home = tmp_path / "shellgpt"
    root = home / "shellgpt-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under SHELLGPT_HOME gains nothing anywhere."""
    home = tmp_path / "shellgpt-home"
    home.mkdir()
    monkeypatch.setenv("SHELLGPT_HOME", str(home))
    root = tmp_path / "src" / "shellgpt-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``shellgpt -p <name>`` SHELLGPT_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `shellgpt update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "shellgpt.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "shellgpt.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\shellgpt\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/SHELLGPT/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``shellgpt update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/shellgpt.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/shellgpt.exe is not gitignored — shellgpt update's autostash "

...
managers acquire the same path through ``shellgpt_state_registry`` from the REPL thread a
...
import shellgpt_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = shellgpt_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(shellgpt_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        ShellGPTCLI._init_session_store(cli)
        ...
        shellgpt_state_registry.close_all()

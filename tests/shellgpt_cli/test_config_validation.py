...
class TestTimezoneValidation:
    ...
    schedule on server-local time (shellgpt_time._get_zoneinfo falls back with
    ...
    def test_invalid_or_non_string_zone_is_an_error(self):
        ...
        assert "IANA" in issue.hint and "SHELLGPT_TIMEZONE" in issue.hint
...
class TestUnknownTopLevelKeys:
    ...
    (gateway/run.py, shellgpt send) so users can feed skills and external apps
...
class TestQuotedContainerValues:
    ...
    def test_quoted_list_in_container_slot_is_flagged_with_remedy(self):
        ...
        assert "shellgpt config set plugins.enabled '[\"a\",\"b\"]'" in flagged["plugins.enabled"].hint

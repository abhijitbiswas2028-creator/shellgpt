...
shellgpt_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `shellgpt config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_shellgpt_home = get_shellgpt_home()`` at import time (module
    singleton), so monkeypatching SHELLGPT_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SHELLGPT_HOME.)
    """
    import cli

    cli._shellgpt_home = home

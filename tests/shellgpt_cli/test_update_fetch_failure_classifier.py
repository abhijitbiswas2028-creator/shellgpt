"""Fetch-failure classification for `shellgpt update` / `shellgpt update --check`.
...
CURL_429_STDERR = (
    "fatal: unable to access 'https://github.com/NousResearch/shellgpt-agent.git/':"
...
class TestClassifyFetchFailure:
    ...
    def test_ssh_publickey_denial_reports_ssh_auth_not_generic(self):
        ...
        assert "https://github.com/NousResearch/shellgpt-agent.git" in msg
...
def test_update_network_git_calls_never_prompt_for_credentials():
    ...
    Live incident (Sep 2026): a GitHub-side 401 made `shellgpt update` sit on

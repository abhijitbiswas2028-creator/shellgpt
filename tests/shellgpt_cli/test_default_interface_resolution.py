...
``shellgpt … chat -q`` on a pipe, and the TUI's no-TTY bail-out exits 0
...
These tests pin that precedence at every layer that makes the decision:
  ...
  * ``_wants_tui_early(argv)``  — the dependency-free early resolver used by
    ...
    ``shellgpt_cli.config`` are importable.
...
class TestWantsTuiEarly:
    ...
    def test_missing_config_defaults_to_cli(self, tmp_path, monkeypatch):
        # SHELLGPT_HOME points at an empty dir — no config.yaml.
        monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path))
        monkeypatch.setattr(m, "_EARLY_INTERFACE_CACHE", None)
        assert m._wants_tui_early([]) is False
    ...
    # before `_apply_profile_override()` sets SHELLGPT_HOME, so a cache that
    # ignored the home answered every later caller with the DEFAULT home's
    # interface — `shellgpt -p <name>` booted the wrong one.
...
class TestParserFlags:
    ...
    def test_cli_and_tui_are_relaunch_inherited(self):
        from shellgpt_cli.relaunch import _INHERITED_FLAGS_TABLE

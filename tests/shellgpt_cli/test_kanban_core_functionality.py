...
Complements tests/shellgpt_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/shellgpt_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/shellgpt_cli/test_kanban_db.py.
    monkeypatch.setenv("SHELLGPT_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    shellgpt subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("SHELLGPT_KANBAN_TASK") == tid
    assert env.get("SHELLGPT_PROFILE") == "some-profile"
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``shellgpt_cli.kanban_db`` fresh and uses that single
...
_WORKER_LOG_TAIL = (
    ...
    "╰──────────────────────────────╯\n"
    "\nResume this session with:\n  shellgpt --resume 20260915_000000_abc\n\n"
...
def test_dead_worker_reap_surfaces_the_workers_own_last_output(kanban_home, drive):
    ...
    import shellgpt_cli.kanban_db as kb
    from shellgpt_cli import kanban_db_connect as kbc
    conn = kbc.connect()
    try:

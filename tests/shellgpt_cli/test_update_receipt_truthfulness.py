...
subsystem with REAL module functions against a temp SHELLGPT_HOME — it is not
...
1. RECEIPT ALWAYS FINALIZED — ``begin → steps → finalize`` writes a
   ...
   ``/api/shellgpt/update/receipt`` — #92780) can never interpret a crash
...
``shellgpt_cli.update_receipt`` / ``shellgpt_cli.update_inventory`` API.
"""
...
def receipt_home(tmp_path, monkeypatch):
    """Hermetic SHELLGPT_HOME so receipts never touch the real profile."""
    home = tmp_path / ".shellgpt"
    home.mkdir()
    monkeypatch.setattr(
        "shellgpt_cli.config.get_shellgpt_home", lambda: home, raising=False
    )
    ur._current = None
    yield home
    ur._current = None
...
class TestSuccessImpliesAccounting:
    ...
    def test_full_accounting_permits_success_with_evidence(
        ...
        outcomes, incomplete, path = self._drive_decision(
            ...
            restarted_services=["shellgpt-serve-ops.service"],
    ...
    def test_failed_unit_also_refuses_success(self, receipt_home):
        ...
        outcomes = match_runtime_outcomes(
            ...
            externally_supervised_profiles=[],
            killed_pids=set(),
            failed_units=["shellgpt-gateway.service"],
        )
...
class TestRefusalIsNotFailure:
    ...
    def test_refusal_receipt_distinguishable_from_failure(
        ...
        ur.begin_update_receipt()
        ur.record_step("venv_preflight", False, "another shellgpt holds venv")
    ...
    def test_refused_receipt_survives_with_its_steps(self, receipt_home):
        ...
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "shellgpt.exe running")

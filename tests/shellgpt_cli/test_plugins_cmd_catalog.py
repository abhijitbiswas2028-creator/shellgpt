"""Catalog-aware ``shellgpt plugins`` surface (shellgpt_cli/plugins_cmd_catalog.py): a bare catalog name installs
...
from shellgpt_cli import plugin_catalog as pc_cat
from shellgpt_cli import plugins_cmd as pc
from shellgpt_cli import plugins_cmd_catalog as cat
...
def test_owner_repo_hash_subdir_shorthand_resolves_like_the_catalog_spelling():
    from shellgpt_cli.plugins_cmd import _resolve_git_url
    assert _resolve_git_url("plastic-labs/honcho#shellgpt-plugin-honcho") == (
        "https://github.com/plastic-labs/honcho.git", "shellgpt-plugin-honcho")
...
def test_in_tree_sidecar_cannot_forge_catalog_provenance(world, tmp_path):
    ...
    its own .shellgpt-catalog.json is not a catalog install and does not mark the real entry installed."""
...
def test_kill_list_covers_update_enable_and_load_of_an_installed_plugin(world, tmp_path, monkeypatch):
    ...
    from shellgpt_cli.plugins_discovery import gate_manifest

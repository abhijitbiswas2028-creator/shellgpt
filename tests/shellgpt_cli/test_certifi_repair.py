...
1. The venv-repair import probes (early recovery + `shellgpt update`) must
...
2. ``shellgpt doctor`` must fail the certificate check in that state, and
   ``shellgpt doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `shellgpt update`'s venv repair must apply
...
# =========================================================================
# 2. shellgpt doctor: detection and --fix repair
# =========================================================================

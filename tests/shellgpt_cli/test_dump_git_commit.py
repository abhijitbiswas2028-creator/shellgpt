"""Tests for shellgpt_cli.dump._get_git_commit — git SHA resolution for ``shellgpt dump``.

``shellgpt dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``SHELLGPT_GIT_SHA`` build-arg.
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``shellgpt dump`` doesn't have to special-case
    container builds.  Both paths must return a bare 8-char SHA — no prefix,
    no suffix, no annotation.
    """
    from shellgpt_cli import dump
    ...
    with patch("shellgpt_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("shellgpt_cli.dump.subprocess.run", return_value=failed), \
         patch("shellgpt_cli.build_info.get_build_sha", return_value="b2f477a3"):

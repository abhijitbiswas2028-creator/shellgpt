...
from shellgpt_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from shellgpt_cli.update_cmd_windows import _shellgpt_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m shellgpt_cli.main` spawn shape —
# a user-launched `shellgpt serve` / `shellgpt dashboard` is refused on, never tree-killed.
CMDLINES = [
    ("python -m shellgpt_cli.main serve " + LOOPBACK, "serve", True, True),
    ("python -m shellgpt_cli.main dashboard", "dashboard", False, True),
    ("/venv/bin/shellgpt serve --isolated --host=127.0.0.1 --port=0 --ssh-owner-nonce abc", "serve", True, False),
    (r"C:\shellgpt\.venv\Scripts\shellgpt.exe serve --host 100.106.105.2 --port 9119", "serve", False, False),
    ("shellgpt.exe dashboard", "dashboard", False, False),
    ("shellgpt --profile ops serve " + LOOPBACK, "serve", True, False),
    ("shellgpt -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m shellgpt_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("shellgpt --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
    ("shellgpt gateway run --replace", "gateway", False, False),
    ("shellgpt chat --model serve", "chat", False, False),
...
def test_kill_and_relaunch_predicates_agree_with_the_canonical_holder_matcher(
    ...
    assert _shellgpt_holder_subcommand(cmdline) == subcommand
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("shellgpt serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("shellgpt serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("shellgpt serve --host localhost --port 0")

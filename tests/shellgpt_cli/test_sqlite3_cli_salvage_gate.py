...
vulnerable sqlite3 CLI at a live ShellGPT database.
...
ShellGPT' own corruption banners used to instruct exactly that command
(`sqlite3 ~/.shellgpt/state.db ".recover"`). The fix routes operators to
`shellgpt sessions recover --source ...`, whose lane snapshots the damaged
...
from shellgpt_cli.session_lost_and_found import (
...
LIVE_DB_SALVAGE_COMMAND = 'sqlite3 ~/.shellgpt/state.db ".recover"'
...
class TestFindSqlite3CliRefusal:
    def test_missing_binary_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "shellgpt_cli.session_lost_and_found.shutil.which", lambda _: None
        )
    ...
    def test_no_dbpage_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "shellgpt_cli.session_lost_and_found.shutil.which",
            lambda _: "/usr/bin/sqlite3",
        )
        monkeypatch.setattr(
            "shellgpt_cli.session_lost_and_found._cli_supports_recover",
            lambda _: False,
        )
        assert find_sqlite3_cli() is None
    ...
    def test_wal_reset_vulnerable_refusal(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "shellgpt_cli.session_lost_and_found._parse_sqlite3_cli_version",
            lambda _: (3, 45, 1),
        )
        assert find_sqlite3_cli() is None
...
class TestParseSqlite3CliVersion:
    def test_parses_modern_output(self):
        ...
        with patch(
            "shellgpt_cli.session_lost_and_found.subprocess.run",
            return_value=Probe(),
        ):
...
class TestGuidanceNeverNamesLiveDb:
    ...
    def test_repair_budget_error_names_safe_lane(self, tmp_path: Path):
        import shellgpt_state_repair as shellgpt_state  # helper lives in the split-out repair module

        message = shellgpt_state._persistent_repair_exhausted_error(
            tmp_path / "state.db"
        )
...
# printed `shellgpt sessions recover --source <db>` — which cmd_sessions
...
class TestEmittedCommandsSatisfyCliContract:
    ...
    `shellgpt sessions recover --source <db>`, which cmd_sessions rejects
    ...
    the real `cmd_sessions` (the same function `shellgpt` main() hands the
    ...
    def _namespace(source: Path, **overrides) -> "argparse.Namespace":
        """The namespace shellgpt main() produces for `sessions recover`.

        Mirrors the registrations in shellgpt_cli/main.py (sessions_recover
    ...
    def test_old_v1_shape_is_still_rejected(self, tmp_path):
        ...
        import shellgpt_cli.sessions_cmd as sc

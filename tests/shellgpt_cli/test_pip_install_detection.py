...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $SHELLGPT_HOME.

    Models a host git install whose $SHELLGPT_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'shellgpt update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("shellgpt_cli.config.get_managed_system", return_value=None), \
         patch("shellgpt_cli.config.get_shellgpt_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``shellgpt update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("shellgpt_cli.config.get_managed_system", return_value=None), \
         patch("shellgpt_cli.config.get_shellgpt_home", return_value=tmp_path), \
         patch("shellgpt_constants.is_container", return_value=True):
        from shellgpt_cli.config import detect_install_method

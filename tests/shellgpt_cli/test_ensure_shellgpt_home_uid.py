"""Regression tests for #34107 — Docker UID/GID handling in ensure_shellgpt_home.

When ShellGPT runs in Docker with ``SHELLGPT_UID=1000`` / ``SHELLGPT_GID=911``,
the entrypoint chowns the top-level ``SHELLGPT_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_shellgpt_home()`` — especially
...
The fix is a ``_chown_to_shellgpt_uid`` helper (``shellgpt_constants``, the single home of the
managed/container/SHELLGPT_UID policy) that reads the env vars and applies chown after
...
# ---------------------------------------------------------------------------
# _resolve_shellgpt_uid_gid
# ---------------------------------------------------------------------------


class TestResolveShellGPTUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from shellgpt_constants import _resolve_shellgpt_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_shellgpt_uid
# ---------------------------------------------------------------------------


class TestChownToShellGPTUid:
    ...
    def test_eperm_is_silently_swallowed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHELLGPT_UID", "1000")
        monkeypatch.setenv("SHELLGPT_GID", "911")
        import shellgpt_constants as cfg
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SHELLGPT_UID", raising=False)
        monkeypatch.delenv("SHELLGPT_GID", raising=False)
        from shellgpt_cli import config as cfg

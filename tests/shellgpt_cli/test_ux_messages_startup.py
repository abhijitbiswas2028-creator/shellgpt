"""Invariant tests for the plain-language startup messages of the ``shellgpt`` CLI.

Contracts (not snapshots): a mistyped subcommand names the typo, offers the closest
match and ``shellgpt --help`` without dumping the 70-entry choice list; a rejected
profile name explains the rule; missing optional dependencies point at ``shellgpt update``.
"""
...
def test_unknown_subcommand_names_typo_suggests_closest_and_hides_choice_list():
    ...
    assert "'sesions' is not a `shellgpt` command" in text
    assert "Did you mean: sessions" in text
    assert "shellgpt --help" in text
...
def test_nested_group_typo_names_the_group_and_suggests():
    ...
    assert "'stat' is not a `shellgpt gateway` command" in text
...
def test_invalid_profile_flag_value_explains_rule_and_exits(monkeypatch):
    from shellgpt_cli import main as _main

    monkeypatch.setattr(sys, "argv", ["shellgpt", "-p", "Work Bot", "status"])
    err = io.StringIO()
    with redirect_stderr(err), pytest.raises(SystemExit) as exc:
...
def test_option_looking_dash_p_value_is_a_silent_skip_even_under_shellgpt(monkeypatch):
    from shellgpt_cli import main as _main

    # `-p no:xdist` reaching us through a differently named runner (tox, nox, python -m) must not exit.
    monkeypatch.setattr(sys, "argv", ["shellgpt", "-p", "no:xdist", "tests/"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
    monkeypatch.setattr(sys, "argv", ["shellgpt", "-p", "--flag"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_invalid_dash_p_after_a_subcommand_is_left_to_that_subcommand(monkeypatch):
    ...
    # A plugin/subcommand flag such as `shellgpt kanban serve -p "Work Bot"` is not our profile selector.
    monkeypatch.setattr(sys, "argv", ["shellgpt", "kanban", "serve", "-p", "Work Bot"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)

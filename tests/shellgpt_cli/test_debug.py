"""Tests for ``shellgpt debug`` CLI command and debug utilities."""
...
def shellgpt_home(tmp_path, monkeypatch):
    """Set up an isolated SHELLGPT_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO shellgpt_cli.web_server: dashboard request\n"
    )
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from shellgpt_cli.debug import upload_to_pastebin

        with patch("shellgpt_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("shellgpt_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, shellgpt_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = shellgpt_home / "logs" / "agent.log"
        from shellgpt_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, shellgpt_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from shellgpt_cli.debug import _capture_log_snapshot
        ...
        (shellgpt_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `shellgpt debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, shellgpt_home):
        from shellgpt_cli.debug import _capture_log_snapshot

        (shellgpt_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, shellgpt_home):
        from shellgpt_cli.debug import _capture_log_snapshot

        (shellgpt_home / "logs" / "desktop.log").unlink()
        ...
        assert str(shellgpt_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, shellgpt_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, shellgpt_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from shellgpt_cli.debug import _capture_log_snapshot

        (shellgpt_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, shellgpt_home):
        """The uploaded report — what people paste into support — must explain it."""
        from shellgpt_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of SHELLGPT_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def shellgpt_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated SHELLGPT_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".shellgpt"
        home.mkdir()
        monkeypatch.setenv("SHELLGPT_HOME", str(home))
    ...
    def test_default_redacts_tail_and_full_text(self, shellgpt_home_with_secret):
        from shellgpt_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, shellgpt_home_with_secret):
        from shellgpt_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, shellgpt_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when SHELLGPT_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("SHELLGPT_REDACT_SECRETS", "false")

        from shellgpt_cli.debug import _capture_log_snapshot

        assert os.environ.get("SHELLGPT_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, shellgpt_home_with_secret
    ):
        from shellgpt_cli.debug import _capture_log_snapshot

        log_path = shellgpt_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from shellgpt_cli.debug import _capture_default_log_snapshots
...
class TestRunDebugShare:
    ...
    def test_share_uploads_five_pastes(self, shellgpt_home, capsys):
        ...
        with patch("shellgpt_cli.dump.run_dump") as mock_dump, \
             patch("shellgpt_cli.debug.upload_to_pastebin",
                    side_effect=_mock_upload):
            mock_dump.side_effect = lambda a: print("--- shellgpt dump ---\nversion: test\n--- end dump ---")
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, shellgpt_home_with_secret, capsys
    ):
        ...
        with patch("shellgpt_cli.dump.run_dump"), \
             patch("shellgpt_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("shellgpt_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from shellgpt_cli.debug import delete_paste
        ...
        with patch("shellgpt_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.shellgpt/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``shellgpt debug``
    invocation.
    """


    def test_records_pending_to_json(self, shellgpt_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from shellgpt_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, shellgpt_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from shellgpt_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, shellgpt_home):
        from shellgpt_cli.debug import (
        ...
        with patch("shellgpt_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, shellgpt_home):
        from shellgpt_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("shellgpt_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, shellgpt_home):
        ...
        with patch(
            "shellgpt_cli.debug.delete_paste",
...
class TestBuildDebugShare:
    ...
    Backs both ``shellgpt debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, shellgpt_home):
        from shellgpt_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (shellgpt_home / "logs" / "agent.log").write_text(
        ...
        with patch("shellgpt_cli.dump.run_dump"), patch(
            "shellgpt_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("shellgpt_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, shellgpt_home):
        from shellgpt_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_redaction_keeps_secrets_out(self, shellgpt_home):
        from shellgpt_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, shellgpt_home):
        import gzip
        import json as _json

        from shellgpt_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "shellgpt-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, shellgpt_home, capsys):
        from shellgpt_cli.debug import run_debug_share
        ...
        with patch("shellgpt_cli.dump.run_dump"), patch(
            "shellgpt_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, shellgpt_home, capsys):
        from shellgpt_cli.debug import run_debug_share

        with patch("shellgpt_cli.dump.run_dump"), patch(
            "shellgpt_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, shellgpt_home):
        from shellgpt_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("shellgpt_cli.dump.run_dump"), patch(
            "shellgpt_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("shellgpt_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``ShellGPTCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("shellgpt_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`shellgpt debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, shellgpt_home, capsys, monkeypatch):
        ...
        with patch("shellgpt_cli.dump.run_dump"), \
             patch("shellgpt_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, shellgpt_home, capsys, monkeypatch):

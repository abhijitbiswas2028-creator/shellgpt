...
Runs the real ``shellgpt_cli.plugins_cmd`` commands against a temp SHELLGPT_HOME and checks the verdict
...
from shellgpt_cli.plugins_discovery import collect_directory_manifests, gate_manifest
...
def home(tmp_path, monkeypatch):
    ...
    (shellgpt_home / "plugins").mkdir(parents=True)
...
def test_disable_bundled_platform_by_manifest_name_gates_the_loader(home):
    """`shellgpt plugins disable photon-platform` must produce a key the loader's gate matches; the

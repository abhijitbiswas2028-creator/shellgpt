...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "shellgpt_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # SHELLGPT_HOME.  For a standard install (SHELLGPT_HOME unset) that root is
        # the platform-native default (~/.shellgpt), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from shellgpt_constants import get_default_shellgpt_root
        assert env.get("SHELLGPT_HOME") == str(get_default_shellgpt_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (SHELLGPT_DESKTOP=1) must NOT

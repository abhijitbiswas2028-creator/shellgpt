"""#93349 — ``shellgpt update`` restarts only the gateways of the home it is updating.

``shellgpt-gateway*`` units and ``gateway run`` processes are host-wide namespaces shared by every
ShellGPT install under the account. A scratch home's update used to drain and restart the account's
real ``shellgpt-gateway.service`` and SIGTERM sibling installs' gateways because they were listed,
...
FOREIGN_HOME = "/srv/other-account-home/.shellgpt"


@pytest.fixture
def own_home(monkeypatch, tmp_path):
    home = tmp_path / "homeA" / ".shellgpt"
    ...
    monkeypatch.setattr("shellgpt_cli.update_receipt._profile_homes", lambda: [("default", home)])
    return home


def _pid_homes(monkeypatch, mapping: dict):
    monkeypatch.setattr(dashboard_procs, "_shellgpt_home_for_pid", lambda pid: mapping.get(pid))


def test_systemd_unit_of_another_home_is_left_alone(monkeypatch, own_home):
    """user/shellgpt-gateway runs on another SHELLGPT_HOME → no drain, no restart, not a failure;
    ...
    fleet._restart_one_systemd_gateway_unit(
        "shellgpt-gateway", scope="user", scope_cmd=["systemctl", "--user"], drain_budget=5.0,
    ...
    assert drained == [4343] and restarted == ["shellgpt-gateway"] and failed == []
...
def test_manual_gateway_of_another_home_is_not_stopped(monkeypatch, own_home):
    ...
    monkeypatch.setattr("shellgpt_cli.gateway._get_service_pids", lambda **k: set())
    monkeypatch.setattr("shellgpt_cli.gateway.find_gateway_pids", lambda **k: [111, 222, 333])
    monkeypatch.setattr("shellgpt_cli.gateway.find_profile_gateway_processes", lambda **k: [])
    monkeypatch.setattr("shellgpt_cli.gateway._wait_for_gateway_exit", lambda **k: None)

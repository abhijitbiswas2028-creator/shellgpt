"""Tests for shellgpt_cli/completion.py — shell completion script generation."""
...
from shellgpt_cli.completion import _walk, generate_bash, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real shellgpt structure."""
    p = argparse.ArgumentParser(prog="shellgpt")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_shellgpt_completion()" in out
        assert "complete -F _shellgpt_completion shellgpt" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'shellgpt profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _shellgpt_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_shellgpt_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _shellgpt_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__shellgpt_profiles" in out
        count = out.count("(__shellgpt_profiles)")

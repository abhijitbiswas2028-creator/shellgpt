"""Tests for the Kanban DB layer (shellgpt_cli.kanban_db)."""
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("SHELLGPT_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("SHELLGPT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_blocker_auth_curated_not_open_stem(
    ...
    monkeypatch.setenv("SHELLGPT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
...
def test_infrastructure_spawn_refusal_never_charges_the_card(
    ...
    def spawn_via_real_boundary(task, workspace, board=None):
        kbd._restart_safe_worker_argv(task, ["shellgpt", "chat"])  # raises: real probe verdict, real _degrade()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """ShellGPT' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` ShellGPT' DB/metadata/logs on task completion.
    """
...
# spawned with `shellgpt -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's SHELLGPT_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's SHELLGPT_HOME."""

    def _set_home(self, monkeypatch, tmp_path, shellgpt_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
        monkeypatch.delenv("SHELLGPT_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.shellgpt/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.shellgpt/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.shellgpt/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # SHELLGPT_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile SHELLGPT_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # SHELLGPT_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is SHELLGPT_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["SHELLGPT_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SHELLGPT_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SHELLGPT_KANBAN_TASK"] == "t_dispatch_env"
        assert env["SHELLGPT_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "SHELLGPT_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see shellgpt_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    (see shellgpt-agent issue #22032).
    ...
    import shellgpt_state_wal as _shellgpt_state_wal
    monkeypatch.setattr(
        _shellgpt_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _shellgpt_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    shellgpt_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("shellgpt_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="shellgpt_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "shellgpt_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'shellgpt kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from shellgpt_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_shellgpt_argv()
#
# Workers spawned by the dispatcher must use a `shellgpt` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["shellgpt", ...]`
...
# only falls back to the PATH shim when ``shellgpt_cli`` is not importable.
# ---------------------------------------------------------------------------


def test_resolve_shellgpt_argv_prefers_module_form_over_path_shim(monkeypatch):
    """A `shellgpt` on PATH must not shadow the running install (#111569):
    the module argv wins whenever ``shellgpt_cli`` is importable; only an
    explicit ``$SHELLGPT_BIN`` overrides it."""
    import shutil
    import sys
    from shellgpt_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("SHELLGPT_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/tmp/planted/shellgpt")
    monkeypatch.setattr(kbd, "_safe_which_no_cwd", lambda name: "/tmp/planted/shellgpt")
    assert kbd._resolve_shellgpt_argv() == [sys.executable, "-m", "shellgpt_cli.main"]

    monkeypatch.setenv("SHELLGPT_BIN", "/opt/shellgpt/bin/shellgpt")
    assert kbd._resolve_shellgpt_argv() == ["/opt/shellgpt/bin/shellgpt"]




def test_resolve_shellgpt_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `shellgpt_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m shellgpt_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SHELLGPT_BIN", None)
        with mock.patch.object(shutil, "which", return_value=None):
            argv = kbd._resolve_shellgpt_argv()
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from shellgpt_cli.kanban_db_connect import connect
    from shellgpt_cli.sqlite_safe_read import file_length_matches_header

...
``shellgpt*.exe`` shims aside (``shellgpt.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``shellgpt`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("shellgpt", "shellgpt-agent", "shellgpt-acp", "shellgpt-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "shellgpt.exe" in names, "shellgpt.exe must be restored after a no-op install"
    assert "shellgpt-acp.exe" in names
    assert "shellgpt-gateway.exe" in names
    assert (scripts / "shellgpt.exe").read_bytes() == b"MZ-old-shellgpt"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "shellgpt.exe").read_bytes() == b"MZ-new-shellgpt"
...
# ---------------------------------------------------------------------------
# shellgpt_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "shellgpt.exe" in names, "shellgpt.exe must be restored after a no-op recovery install"
    assert (scripts / "shellgpt.exe").read_bytes() == b"MZ-old-shellgpt"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "shellgpt.exe").write_bytes(b"MZ-new-shellgpt")

...
``shellgpt chat -q`` wires the interactive prompt_toolkit clarify callback
...
inter-agent wakeups all deliver work as ``shellgpt chat -q``, so an agent that
...
def test_no_choices_returns_immediate_headless_answer():
    from shellgpt_cli.cli_agent_setup_mixin import _single_query_clarify_callback

...
``shellgpt_cli/web_routers/profiles.py`` holds handler bodies that were extracted
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """A real profile directory under a throwaway SHELLGPT_HOME."""
...
def _outcome(ok=True, reason="described", description="a demo profile"):
    from shellgpt_cli.profile_describer import DescribeOutcome

...
and the ``shellgpt kanban repair`` CLI verb."""
...
# ---------------------------------------------------------------------------
# repair_db() API + `shellgpt kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `shellgpt kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated SHELLGPT_HOME so kanban_db_path() resolves inside tmp_path."""

...
from shellgpt_cli.main, these tests mock the delegation point and verify
...
from shellgpt_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside shellgpt setup must survive the wizard's final save."""
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    monkeypatch.setattr("shellgpt_cli.nous_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(

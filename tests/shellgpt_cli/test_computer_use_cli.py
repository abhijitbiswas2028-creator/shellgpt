...
def _invoke(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    cli_main = import_module("shellgpt_cli.main")
    monkeypatch.setattr(sys, "argv", ["shellgpt", "computer-use", *args])
...
def test_computer_use_status_returns_nonzero_for_incompatible_standard_driver(
    ...
    assert "Run: shellgpt computer-use install" in output
...
def test_computer_use_status_returns_nonzero_for_incompatible_custom_driver(
    ...
    monkeypatch.setenv("SHELLGPT_CUA_DRIVER_CMD", driver)
    monkeypatch.setattr(cua_backend_driver, "resolve_cua_driver_cmd", lambda: driver)
    monkeypatch.setattr(
        tools_config,
        "_cua_driver_contract_status",
    ...
    assert _invoke(monkeypatch, "status") == 1
    output = capsys.readouterr().out
    assert "custom binary from SHELLGPT_CUA_DRIVER_CMD" in output
...
def test_permissions_status_names_the_stale_tcc_row_for_the_missing_grant(
    ...
    assert "shellgpt computer-use permissions grant" in out

...
``shellgpt update`` on macOS printed ``Update complete!`` and exited 0 while the
``ai.shellgpt.gateway`` LaunchAgent sat deregistered for 36 minutes.  The restart
...
class TestInvokingProfileIsVerifiedLikeItsSiblings:
    ...
    def test_restart_that_leaves_the_old_pid_supervised_is_not_a_restart(
        ...
        update printed ``✓ Restarted ai.shellgpt.gateway`` over pre-update code.

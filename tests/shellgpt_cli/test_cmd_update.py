...
from shellgpt_cli.main import cmd_update, PROJECT_ROOT
from shellgpt_cli import main_web_build
...
def _patch_managed_uv(request):
    ...
    with patch("shellgpt_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         ...
         patch(
             "shellgpt_cli.update_cmd._post_update_sqlite_runtime_status",
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def _stub_npx_warmup(self):
        ...
        with patch(
            ...
            yield
    @staticmethod
    def _cache_file(shellgpt_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return shellgpt_root / f".npm_lock_hash_{cache_key}"
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `shellgpt update` must still install (the
        npm-install fallback is what syncs node_modules in that state)."""
        from shellgpt_cli import main as hm
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".shellgpt"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(shellgpt_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            shellgpt_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("SHELLGPT_HOME", str(shared_root))
            update_cmd._update_node_dependencies()

            monkeypatch.setenv("SHELLGPT_HOME", str(named_profile))
            update_cmd._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $SHELLGPT_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("shellgpt_cli.managed_uv.resolve_uv", lambda: None)
...
class TestUpdateManagedPythonEnvIsolation:
    ...
    forces UV_PYTHON_INSTALL_DIR to .shellgpt-runtime/python), then re-points
...
class TestRepairCurrentCheckoutRuntimeRepair:
    ...
    def _run(monkeypatch, *, repaired: bool, lazy_refresh_ok: bool = True):
        ...
        monkeypatch.setattr("shellgpt_cli.managed_uv.update_managed_uv", ensure)
        monkeypatch.setattr("shellgpt_cli.managed_uv.ensure_uv", ensure)
    ...
    def test_restores_optional_dependencies_after_runtime_repair(self, monkeypatch):
        ...
        built from uv.lock alone: the captured lazy backends and ShellGPT Tools deps must be
    ...
    def test_failed_lazy_restore_keeps_incomplete_marker(self, monkeypatch, capsys):
        ...
        restore into the swapped-in venv is picked up by the next `shellgpt` run instead of
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/shellgpt-agent silently misses updates.
        """
        from shellgpt_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/shellgpt-agent.git",
    ...
    def test_current_checkout_python_repair_failure_is_durable(
        ...
        with patch.object(
            ...
            "shellgpt_cli.update_receipt.finalize_update_receipt"
        ) as finalize_receipt, patch(
            "shellgpt_cli.update_receipt.finalize_pending_update_receipt"
        ):
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "shellgpt_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "shellgpt_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "shellgpt_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "shellgpt_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "shellgpt_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "shellgpt_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("shellgpt_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "shellgpt_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "shellgpt_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "shellgpt_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "shellgpt_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "shellgpt_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "shellgpt_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.shellgpt"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.shellgpt/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.shellgpt/profiles/work"))
        ...
        with (
            patch("shellgpt_cli.profiles.list_profiles", return_value=all_profiles),
            patch("shellgpt_cli.profiles.seed_profile_skills", side_effect=fake_seed),
...
class TestCmdUpdateBranchFlag:
    """``shellgpt update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``shellgpt update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``shellgpt update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from shellgpt_cli.update_cmd import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        packaged_exe = desktop_dir / "release" / "win-unpacked" / "ShellGPT.exe"
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "shellgpt_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
...
class TestUpdateNodeDependencies:
    ...
    def test_records_lockfile_hash_only_on_success(self, _which, mock_popen, tmp_path, monkeypatch):
        ...
        from shellgpt_cli import update_cmd_deps
...
class TestGitTrampolineSelfHeal:
    ...
    def test_portable_git_candidates_check_shared_root_first(self, tmp_path, monkeypatch):
        # Profile-scoped layout: SHELLGPT_HOME = <root>/profiles/foo, but the
        # PortableGit tree lives under the SHARED root (monerostar review on
        # #88136). The candidate list must check get_default_shellgpt_root()
        ...
        monkeypatch.setattr(update_cmd, "get_default_shellgpt_root", lambda: root)
        monkeypatch.setattr(update_cmd, "get_shellgpt_home", lambda: profile_home)

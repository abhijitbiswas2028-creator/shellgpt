...
shellgpt_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # shellgpt_time checks an env override first; ensure it's unset so config wins.
    ...
    assert shellgpt_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (SHELLGPT_TIMEZONE, SHELLGPT_REDACT_SECRETS, SHELLGPT_MAX_ITERATIONS,
    ...
    # writing SHELLGPT_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml

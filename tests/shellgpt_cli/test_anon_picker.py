...
against a temp ``SHELLGPT_HOME`` with the network catalog fetch stubbed.
"""
...
from shellgpt_cli.auth import _save_auth_store
...
def guest_home(monkeypatch, tmp_path):
    ...
    from shellgpt_cli import model_switch_providers as msp
    ...
    return Path(os.environ["SHELLGPT_HOME"])
...
def _cli_nous_rows(config):
    from shellgpt_cli.main_provider_setup import _build_provider_picker_rows
...
def test_guest_identity_shows_free_tier_row_with_only_welcome_model(guest_home, monkeypatch):
    ...
    # The `shellgpt model` provider picker applies the same rule from the same helper.
...
def test_desktop_picker_payload_never_locks_the_free_tier_row(guest_home, monkeypatch):
    ...
    from shellgpt_cli import inventory

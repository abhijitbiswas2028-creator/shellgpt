"""Tests for the stale-dashboard handling run at the end of ``shellgpt update``.

``shellgpt update`` detects ``shellgpt dashboard`` processes left over from the
...
from shellgpt_cli.main_dashboard import _find_stale_dashboard_pids
from shellgpt_cli.dashboard_procs import _kill_stale_dashboard_processes
from shellgpt_cli import dashboard_procs
from shellgpt_cli import main_dashboard
from shellgpt_cli import update_cmd
from shellgpt_cli.update_cmd import _finish_dashboard_update_cleanup
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "shellgptPath": "/opt/shellgpt/bin/shellgpt",
        "shellgptHome": str(tmp_path),
    ...
    monkeypatch.delenv("SHELLGPT_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "shellgpt_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m shellgpt_cli.main dashboard"),
                    _ps_line(12345, "shellgpt dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("shellgpt_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "shellgpt-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="shellgpt-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "shellgpt-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "shellgpt-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "shellgpt-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("shellgpt_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]), \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert ["systemctl", "--user", "restart", "shellgpt-dashboard.service"] in calls
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("shellgpt_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("shellgpt_cli._subprocess_compat.pid_is_shellgpt", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("SHELLGPT_HOME", str(own_home))
        with patch(
            "shellgpt_cli.main._kill_stale_dashboard_processes",
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/shellgpt-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="shellgpt-serve.service"), \
        ...
        restart.assert_called_once_with(
            "shellgpt-serve.service", "/system.slice/shellgpt-serve.service"
        )
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: shellgpt update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"shellgpt-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["shellgpt", "dashboard", "--port", "8300"]
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("shellgpt_cli.dashboard_procs._shellgpt_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["shellgpt", "dashboard", "--port", "8300"],
                ["shellgpt", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["shellgpt", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["shellgpt", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["shellgpt", "serve"]])

        assert failed == [["shellgpt", "serve"]]
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from shellgpt_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.shellgpt/profiles/mini-cat"),
        ]) == []
    ...
    def test_dedupes_identical_normalized_cmdlines(self):
        ...
        a = ["/usr/bin/python3", "-m", "shellgpt_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "shellgpt_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["shellgpt", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["shellgpt", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["shellgpt", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_shellgpt_home(self):
        from shellgpt_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/shellgpt-home-a"
        a = ["shellgpt", "dashboard", "--port", "8300"]
        b = ["shellgpt", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.shellgpt/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["shellgpt", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["shellgpt", "dashboard", "--port", "8301"]
        home = "/home/u/.shellgpt"
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another SHELLGPT_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.shellgpt"),
                (2, b, "/work/project/.shellgpt"),
            ],
            own_home="/home/u/.shellgpt",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.shellgpt-sidecar/shellgpt-agent/venv/bin/python",
            "-m", "shellgpt_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.shellgpt-lifeos")],
            own_home="/Users/u/.shellgpt",
        )
        assert out == []

    def test_matching_shellgpt_home_is_kept(self):
        from shellgpt_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["shellgpt", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.shellgpt")],
            own_home="/Users/u/.shellgpt",
        )
        assert out == [argv]

    def test_symlinked_shellgpt_home_compares_equal(self, tmp_path):
        from shellgpt_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable SHELLGPT_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from shellgpt_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_shellgpt_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            shellgpt_constants, "get_shellgpt_home", lambda: Path("/home/u/.shellgpt")
        )
        argv = ["shellgpt", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.shellgpt-lifeos"),
        ])
        assert foreign == []
        own = _filter_dashboard_respawn_candidates([
            (1, argv, "/home/u/.shellgpt"),
        ])
        assert own == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00shellgpt_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "shellgpt_cli.main", "serve"]
        ...
        try:
            ...
            with patch(
                ...
                side_effect=AttributeError(
                    "module 'shellgpt_cli.main_dashboard' has no attribute '_loaded_launchd_backend_jobs'"
                ),
            ):
...
class TestLaunchdSupervisedBackends:
    ...
    ARGV = [
        "/opt/shellgpt/venv/bin/python", "-m", "shellgpt_cli.main",
    ...
    def _run(self, pid, jobs, *, restart_ok=True, restart_managed=True):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("shellgpt_cli.dashboard_procs._process_ancestors", return_value=[]), \
             patch("shellgpt_cli.dashboard_procs._shellgpt_home_for_pid", return_value=None), \
             patch.object(live, "_respawn_dashboard_processes", return_value=[]) as respawn, \
    ...
    def test_launchd_owned_backend_restarts_through_launchd_never_as_a_detached_respawn(self, capsys):
        ...
        job = ("system", "ai.shellgpt.dashboard", list(self.ARGV), None)
        result, restart, respawn = self._run(9102, [job])
        respawn.assert_not_called()
        restart.assert_called_once_with("system", "ai.shellgpt.dashboard", None)
        assert result["killed"] == [9102] and result["unrecovered"] == []
        assert "when you're ready" not in capsys.readouterr().out

        result, restart, respawn = self._run(9103, [("gui/501", "ai.shellgpt.dashboard", list(self.ARGV), 9103)],
                                             restart_ok=False)
        respawn.assert_not_called()
        assert result["unrecovered"] == [9103]
        assert "run: launchctl kickstart -k gui/501/ai.shellgpt.dashboard" in capsys.readouterr().out

        # A LaunchDaemon lives in the system domain: kickstart needs root, so a non-root hint says sudo.
        with patch("shellgpt_cli.dashboard_procs.os.geteuid", return_value=501, create=True):  # windows-footgun: ok — patch target string, create=True
            self._run(9105, [("system", "ai.shellgpt.serve", list(self.ARGV), None)], restart_ok=False)
        assert "run: sudo launchctl kickstart -k system/ai.shellgpt.serve" in capsys.readouterr().out

        other = ("gui/501", "ai.shellgpt.other", ["shellgpt", "dashboard", "--port", "8300"], 777)
    ...
    def test_launchd_job_attribution_is_by_live_pid_ancestor_or_exact_argv(self):
        ...
        never otherwise."""
        uid = 501
        backend_argv = ["/opt/shellgpt/venv/bin/python", "-m", "shellgpt_cli.main", "dashboard", "--port", "9119"]
        serve_argv = ["/opt/shellgpt/venv/bin/python", "-m", "shellgpt_cli.main", "serve", "--port", "8642"]
        jobs = [
            (f"user/{uid}", "ai.shellgpt.dashboard", backend_argv, 4242),
            ("system", "ai.shellgpt.serve", serve_argv, None),
        ]

        owning = main_dashboard._launchd_job_owning_backend
        assert owning(4242, ["something", "else"], jobs) == (f"user/{uid}", "ai.shellgpt.dashboard", 4242)
        assert owning(9999, ["python"], jobs, ancestors=[4242, 1]) == (f"user/{uid}", "ai.shellgpt.dashboard", 4242)
        assert owning(9999, serve_argv, jobs) == ("system", "ai.shellgpt.serve", None)
        ...
        assert owning(9999, backend_argv + ["--no-open"], jobs) == (f"user/{uid}", "ai.shellgpt.dashboard", 4242)

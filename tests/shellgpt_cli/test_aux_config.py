"""Tests for the auxiliary-model configuration UI in ``shellgpt model``.
...
from shellgpt_cli.main_provider_setup import _DELEGATION_TASK_KEY, _delegation_cfg_as_task, _format_aux_current, _reset_aux_to_auto, _save_aux_choice
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from shellgpt_cli.config import load_config as _lc, save_config
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "nous", "model": "ShellGPT-4.5"}}
    )
    assert _format_aux_current(shaped) == "nous · ShellGPT-4.5"

"""Contract tests for shellgpt_cli.local_runtime — Rollouts 1+2.

Per the design's verification plan: relationships and contracts, no
change-detector tests, real imports against temp SHELLGPT_HOME (the autouse
...
from shellgpt_cli.local_runtime.binaries import (
    AssetPlan,
    BinaryResolutionError,
    resolve_assets,
    select_backend,
)
from shellgpt_cli.local_runtime.detect import DetectedServer, probe_port
...
def test_sha256_mismatch_rejects(tmp_path, monkeypatch):
    ...
    from shellgpt_cli.local_runtime import binaries
...
def _make_supervisor(tmp_path, port):
    ...
    from shellgpt_cli.local_runtime.supervisor import LlamaServerSupervisor
...
def test_llamacpp_endpoint_resolution_prefers_managed(tmp_path, monkeypatch, stub_server):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    port, handler = stub_server
    from shellgpt_cli.local_runtime import endpoint as ep
...
def test_llamacpp_endpoint_stale_state_falls_through(tmp_path, monkeypatch):
    ...
    with socket.socket() as s:
        ...
        dead_port = s.getsockname()[1]
    from shellgpt_cli.local_runtime import endpoint as ep
    from shellgpt_cli.local_runtime.detect import DEFAULT_PROBE_PORTS
    from shellgpt_cli.local_runtime.supervisor import state_path
    ...
    monkeypatch.setattr("shellgpt_cli.local_runtime.detect.DEFAULT_PROBE_PORTS",
                        (dead_port,))
...
def test_llamacpp_dead_server_raises_friendly_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.endpoint.resolve_llamacpp_endpoint",
        lambda *a, **k: None)
...
def test_resolution_kicks_boot_when_no_thread_is_booting(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    from shellgpt_cli.local_runtime import bootstrap as bs
    from shellgpt_cli.local_runtime import endpoint as ep
    from shellgpt_cli.local_runtime.supervisor import state_path
...
def test_boot_in_flight_real_gate(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    from shellgpt_cli.local_runtime import endpoint as ep
    from shellgpt_cli.local_runtime.binaries import runtimes_root
...
def test_staged_models_requires_every_split_part(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    import shellgpt_cli.local_runtime.bootstrap as bs
...
def test_bootstrap_skips_boot_with_no_staged_models(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("shellgpt_cli.local_runtime.binaries.ensure_runtime_installed", _boom)
...
def test_endpoint_identity_stable_across_supervisor_instances(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    from shellgpt_cli.local_runtime import supervisor as sup_mod
    from shellgpt_cli.local_runtime.supervisor import LlamaServerSupervisor
    ...
    key_file = tmp_path / ".shellgpt" / "runtimes" / "llamacpp" / ".api_key"
...
def test_runtime_provider_seam_llamacpp_alias(tmp_path, monkeypatch, stub_server):
    ...
    from shellgpt_cli.runtime_provider import _resolve_named_custom_runtime
...
def test_staged_local_model_resolves_without_a_running_server(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    _stage_local_model(tmp_path / ".shellgpt", "Qwen3.8-27B-IQ3_S-mtp")
    monkeypatch.setattr("shellgpt_cli.local_runtime.detect.DEFAULT_PROBE_PORTS", ())

    from shellgpt_cli.providers import LLAMACPP_PROVIDER_ID, resolve_provider_full
    ...
    result = switch_model("Qwen3.8-27B-IQ3_S-mtp", current_provider="nous",
                          current_model="ShellGPT-4.5", current_base_url="",
...
def test_external_server_on_a_configured_detect_port_is_used(tmp_path, monkeypatch, stub_server):
    ...
    result = switch_model("ext-model", current_provider="nous", current_model="ShellGPT-4.5",
...
def test_bootstrap_disabled_is_noop(tmp_path, monkeypatch):
    monkeypatch.setenv("SHELLGPT_HOME", str(tmp_path / ".shellgpt"))
    from shellgpt_cli.local_runtime import bootstrap
...
def test_bootstrap_reuses_running_server(tmp_path, monkeypatch, stub_server):
    ...
    called = []
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.binaries.ensure_runtime_installed",
...
def test_bootstrap_failure_never_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "shellgpt_cli.local_runtime.binaries.ensure_runtime_installed", boom)
...
def test_ensure_local_runtime_serializes_racing_callers(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(bootstrap, "_detect_gpu_vendor", lambda: None)
    monkeypatch.setattr("shellgpt_cli.local_runtime.binaries.installed_tags", lambda: ["b1"])
    monkeypatch.setattr("shellgpt_cli.local_runtime.binaries.default_tag", lambda: "b1")
    monkeypatch.setattr("shellgpt_cli.local_runtime.binaries.ensure_runtime_installed",
...
def test_ensure_local_runtime_proceeds_when_boot_lock_is_unwritable(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr("shellgpt_cli.local_runtime.endpoint._state_endpoint", lambda: None)
    monkeypatch.setattr("shellgpt_cli.local_runtime.binaries.installed_tags", lambda: [])
...
def test_manifest_verified_tolerates_non_dict_manifest(tmp_path):
    ...
    from shellgpt_cli.local_runtime.binaries import manifest_verified

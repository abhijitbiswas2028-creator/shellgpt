...
from shellgpt_cli.cli_chat_error_copy import chat_error_response
...
@pytest.mark.parametrize("exc, pointer, absent", [
    (_StatusError("HTTP 401: Invalid API key", 401), "shellgpt model", "HTTP 401"),
    ...
    (Exception("Unknown error"), "shellgpt doctor", "Unknown error"),
])
...
def test_chat_error_response_accepts_plain_string_summary():
    ...
    assert "shellgpt model" in text.splitlines()[0]
...
# ── cli-30: shellgpt sessions with a bad id / unopenable DB ──────────────────
...
def test_sessions_db_open_failure_points_to_repair(monkeypatch, capsys):
    ...
    db_path = shellgpt_state._default_db_path()
    ...
    assert code == 1
    assert "shellgpt sessions repair" in out

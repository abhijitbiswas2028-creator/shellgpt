...
def _enable(shellgpt_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<shellgpt_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        shellgpt_home = Path(os.environ["SHELLGPT_HOME"])  # set by hermetic conftest fixture
        user_plugins = shellgpt_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(shellgpt_home, "image_gen/openai")
...
class TestForeignHarnessManifestDirs:
    def test_foreign_harness_dirs_skipped_without_warnings(
        ...
        They must be skipped silently; the plugin's real ShellGPT manifest
        (``.shellgpt-plugin/plugin.yaml``) is still discovered."""
        import os
        shellgpt_home = Path(os.environ["SHELLGPT_HOME"])  # set by hermetic conftest fixture
        sp = shellgpt_home / "plugins" / "superpowers"
        (sp / ".shellgpt-plugin").mkdir(parents=True)
        (sp / ".shellgpt-plugin" / "plugin.yaml").write_text(
            yaml.dump(
                {
                    "name": "superpowers",
                    "version": "6.3.0",
        ...
        with caplog.at_level("WARNING", logger="shellgpt_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()

        assert "superpowers/.shellgpt-plugin" in mgr._plugins
        parse_warnings = [
    ...
    def test_broken_portable_plugin_still_warns(self, tmp_path, monkeypatch, caplog):
        ...
        import os
        shellgpt_home = Path(os.environ["SHELLGPT_HOME"])  # set by hermetic conftest fixture
        broken = shellgpt_home / "plugins" / "broken-portable"
...
class TestKindField:
    def test_unknown_kind_falls_back_to_standalone(self, tmp_path, monkeypatch, caplog):
        ...
        shellgpt_home = Path(os.environ["SHELLGPT_HOME"])  # set by hermetic conftest fixture
        _write_plugin(
            shellgpt_home / "plugins",
            ["p1"],
            manifest_extra={"kind": "bogus"},
        )
        _enable(shellgpt_home, "p1")
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(shellgpt_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(shellgpt_home, "my-img-plugin")

...
Issue #98790: `shellgpt cron status` in profile B reports profile A's gateway
PID as proof that B's jobs will fire. Root causes:

1. systemd branch of ``_get_service_pids()`` returns every shellgpt-gateway*
...
class TestGetServicePidsProfileScope:
    ...
    def test_default_scope_filters_current_profile_systemd_unit(self, monkeypatch):
        from shellgpt_cli import gateway as gateway_mod

        def _run_side_effect(args, **kwargs):
            cmd_str = " ".join(str(a) for a in args)
            if "list-units" in cmd_str:
                # systemctl now filters: return only the unit that matches the pattern
                if "shellgpt-gateway-jarvis" in cmd_str:
                    stdout = "shellgpt-gateway-jarvis.service loaded active running\n"
                elif "shellgpt-gateway-coder" in cmd_str:
                    stdout = "shellgpt-gateway-coder.service loaded active running\n"
                elif "shellgpt-gateway*" in cmd_str:
                    stdout = (
                        "shellgpt-gateway-jarvis.service loaded active running\n"
        ...
        with (
            patch("shellgpt_cli.gateway.is_macos", return_value=False),
            patch("shellgpt_cli.gateway.supports_systemd_services", return_value=True),
            patch("shellgpt_cli.gateway.get_service_name", return_value="shellgpt-gateway-jarvis"),
    ...
    def test_all_profiles_true_enumerates_fleet(self, monkeypatch):
            ...
            if "list-units" in cmd_str:
                return MagicMock(
                    ...
                    stdout=(
                        ...
                        "shellgpt-gateway-profile-b.service loaded active running\n"
                    ),
                    stderr="",
                )
            if "show" in cmd_str and "MainPID" in cmd_str:
                if "shellgpt-gateway.service" in " ".join(args):
                    return MagicMock(returncode=0, stdout="111\n", stderr="")
                elif "shellgpt-gateway-profile-b.service" in " ".join(args):
        ...
        with (
            patch("shellgpt_cli.gateway.is_macos", return_value=False),
            patch("shellgpt_cli.gateway.supports_systemd_services", return_value=True),
            patch("shellgpt_cli.gateway.get_service_name", return_value="shellgpt-gateway"),
            patch("subprocess.run", side_effect=_run_side_effect),
        ):
...
class TestCronStatusMissingHeartbeat:
    ...
    def test_missing_heartbeat_green_when_gateway_just_started(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        with (
            ...
            patch("shellgpt_cli.gateway.find_gateway_pids", return_value=[4242]),

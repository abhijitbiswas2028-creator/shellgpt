...
``shellgpt update`` wrapped its entire gateway auto-restart phase in a blanket
``except Exception`` that only logged at debug level. When the phase raised
early (e.g. importing ``shellgpt_cli.gateway`` from the freshly pulled checkout
...
from shellgpt_cli.update_cmd import _restart_phase_failure_is_incomplete, _surviving_gateway_pids_after_failed_restart, _warn_gateway_restart_phase_aborted
...
class TestSurvivingGatewayProbe:
    ...
    def test_undeterminable_when_gateway_module_is_broken(self, monkeypatch):
        ...
        fake = types.ModuleType("shellgpt_cli.gateway")

        def _boom(**_kwargs):
            raise ImportError("cannot import name 'is_trivial_prompt'")

        fake.find_gateway_pids = _boom
        monkeypatch.setitem(sys.modules, "shellgpt_cli.gateway", fake)

...
``shellgpt serve``, the gateway, anything) → ``shellgpt serve`` printed only
...
* conflict → single stdout sentinel ``BACKEND_PORT_IN_USE port=<port>`` +
  ...
  transient-condition convention) — and NO ``SHELLGPT_BACKEND_READY``.
* free explicit port → boots and announces ``SHELLGPT_BACKEND_READY`` exactly
...
def test_probe_detects_held_socket():
    from shellgpt_cli.web_server_lifecycle import _port_bind_conflict
...
def test_addr_in_use_error_classification():
    ...
    from shellgpt_cli.web_server_lifecycle import _is_addr_in_use_error
...
# ---------------------------------------------------------------------------
# E2E: real start_server in a subprocess (temp SHELLGPT_HOME)
# ---------------------------------------------------------------------------
...
def _spawn_serve(port: int, tmp_path: Path, merge_stderr: bool = True) -> subprocess.Popen:
    ...
    env.update(
        ...
        SHELLGPT_SERVE_HEADLESS="1",
        PYTHONUNBUFFERED="1",
    )
    # Ensure a stray desktop-parent env can't arm the watchdog/reaper paths.
    for k in ("SHELLGPT_DESKTOP", "SHELLGPT_PARENT_PID", "SHELLGPT_PARENT_START_MARKER",
              "SHELLGPT_PARENT_NONCE"):
        env.pop(k, None)
    code = (
...
def test_conflict_emits_sentinel_and_exit_75(tmp_path):
    ...
    assert "SHELLGPT_BACKEND_READY" not in out  # never claimed ready
...
def test_ephemeral_port_zero_unaffected(tmp_path):
    ...
    try:
        ready, lines = _read_until(proc, "SHELLGPT_BACKEND_READY")
        out = "".join(lines)
        assert ready, f"no READY sentinel; output:\n{out}"
        # OS-assigned non-zero port announced
        ready_line = next(l for l in lines if "SHELLGPT_BACKEND_READY" in l)
...
def test_ready_sentinel_arrives_on_stdout_not_stderr(tmp_path):
    ...
    try:
        ...
        assert f"SHELLGPT_BACKEND_READY port={port}" in out
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=30)

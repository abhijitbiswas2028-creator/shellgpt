...
2. ``TestWatcherRespawnLive`` — drives the REAL
   ``shellgpt_cli.gateway._spawn_gateway_restart_watcher`` end to end with a
   real stub gateway process, against a temp SHELLGPT_HOME:
   ...
   - the respawn env must carry ``_SHELLGPT_GATEWAY_BREAKAWAY=1`` (the stamp
...
class TestWatcherRespawnLive:
    ...
    def _run_watcher_cycle(self, tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
        ...
        stub = (
            ...
            "    os.environ.get('_SHELLGPT_GATEWAY_BREAKAWAY', 'MISSING'),\n"
            "    encoding='utf-8')\n"
...
class TestResumeVerificationLive:
    ...
    def test_dead_relaunch_is_not_reported_as_success(self, tmp_path, monkeypatch):
        ...
        import shellgpt_cli.gateway as gateway
        import shellgpt_cli.main as hm
        from shellgpt_cli.update_cmd import _resume_windows_gateways_after_update

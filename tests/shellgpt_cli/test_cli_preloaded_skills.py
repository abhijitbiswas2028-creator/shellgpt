...
def _real_finalize(cli_obj):
    """Call the real ShellGPTCLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.ShellGPTCLI.__dict__["finalize_preloaded_skills"]
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "ShellGPTCLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
        lambda skills, task_id=None, excluded_loaded_names=None: ("skill prompt", ["shellgpt-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="shellgpt-agent-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["shellgpt-agent-dev", "github-auth"]

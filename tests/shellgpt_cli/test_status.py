...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from shellgpt_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_shellgpt_home", lambda: tmp_path, raising=False)
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in shellgpt status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.shellgpt/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.shellgpt/auth.json" in out
...
def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """shellgpt status should surface freshest gateway last_active (#72016)."""
    from shellgpt_cli import status as status_mod
    import shellgpt_cli.auth as auth_mod
    import shellgpt_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(shellgpt_state, "SessionDB", _FakeDB)

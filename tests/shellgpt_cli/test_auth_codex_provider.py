"""Tests for Codex auth — tokens stored in ShellGPT auth store (~/.shellgpt/auth.json)."""
...
def _setup_shellgpt_auth(shellgpt_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the ShellGPT auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_shellgpt_auth(shellgpt_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (shellgpt_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((shellgpt_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``shellgpt auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``shellgpt auth add openai-codex``.
    ...
    (shellgpt_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `shellgpt auth add openai-codex` —
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to ShellGPT auth store, not ~/.codex/."""
    shellgpt_home = tmp_path / "shellgpt"
    ...
    _save_codex_tokens({"access_token": "shellgpt-at", "refresh_token": "shellgpt-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches ShellGPT store
    assert not (codex_home / "auth.json").exists()

    # ShellGPT auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "shellgpt-at"


def test_resolve_returns_shellgpt_auth_store_source(tmp_path, monkeypatch):
    shellgpt_home = tmp_path / "shellgpt"
    _setup_shellgpt_auth(shellgpt_home)
    monkeypatch.setenv("SHELLGPT_HOME", str(shellgpt_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "shellgpt-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run shellgpt auth".
    """
    from shellgpt_cli.auth import (
    ...
    assert "shellgpt auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from shellgpt_cli.auth import CODEX_RATE_LIMITED_CODE

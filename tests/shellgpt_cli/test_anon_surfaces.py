...
Contract (R-USR-1): wherever a free-tier identity renders (``shellgpt auth status nous``,
``shellgpt auth list``, ``shellgpt status``, ``shellgpt portal info``) the user sees the free-tier label
...
from shellgpt_cli.auth import _load_auth_store  # noqa: F401  (store import name kept for parity with core tests)
...
def _account_state() -> dict:
    return {
        "auth_method": "oauth_device_code", "client_id": "shellgpt-cli",
        "access_token": _jwt(sub="nas_user:real", client_id="shellgpt-cli", account_tier="standard", paid_access=True),
...
def _render_all(capsys) -> dict[str, str]:
    ...
    out["shellgpt status"] = capsys.readouterr().out
...
def test_no_chat_copy_of_any_sign_in_state_leaks_a_terminal_verb_or_a_forbidden_word():
    ...
    terminal_or_url = re.compile(r"shellgpt |https?://", re.IGNORECASE)

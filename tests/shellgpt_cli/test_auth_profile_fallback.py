...
When ``SHELLGPT_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.shellgpt/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.shellgpt            (has its own auth.json fixture)
    * Profile     -> tmp_path/.shellgpt/profiles/coder   (active, SHELLGPT_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"ShellGPT is not
...
def test_codex_cooldown_clear_writes_to_the_store_that_owns_the_borrowed_pool(profile_env):
    ...
    from shellgpt_cli.auth_codex import clear_codex_pool_quota_cooldowns
...
def test_root_write_through_is_visible_to_the_next_fallback_read(profile_env):
    ...
    import os
    from shellgpt_cli.auth import _save_auth_store, read_credential_pool
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SHELLGPT_HOME != ~/.shellgpt, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    shellgpt_home = tmp_path / "classic"
    shellgpt_home.mkdir()

...
from shellgpt_cli.web_routers import _common, analytics
...
def test_corrupt_store_polls_return_status_and_warn_once_per_interval(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(shellgpt_state, "_default_db_path", lambda: db_path)
    monkeypatch.setattr(_common, "_corrupt_store_warned_at", {})
    app = FastAPI()
    app.include_router(analytics.router)
    client = TestClient(app)

    with caplog.at_level(logging.DEBUG, logger="shellgpt_cli.web_server"):
    ...
    # Only the dashboard's own warning counts: shellgpt_state logs an unrelated
    # once-per-process SQLite-version advisory on some interpreters (CI's 3.50.4).
    warnings = [
        r for r in caplog.records
        if r.levelno >= logging.WARNING and r.name.startswith("shellgpt_cli.web_server")
    ]
    ...
    caplog.clear()
    with caplog.at_level(logging.WARNING, logger="shellgpt_cli.web_server"):
        assert client.get("/api/analytics/usage?days=7").status_code == 503
    assert sum(
        r.levelno >= logging.WARNING and r.name.startswith("shellgpt_cli.web_server")
...
def test_corrupt_store_as_status_maps_replaced_store_errors_to_503_without_fix_nudge(tmp_path, monkeypatch):
    ...
    from shellgpt_state_errors import DeletedWalGenerationError, StateDbReplacedError
    ...
    for exc_cls, code in ((DeletedWalGenerationError, "state_db_deleted_wal"), (StateDbReplacedError, "state_db_replaced")):
        ...
        assert "run `shellgpt doctor`" in msg
        # `--fix` may only appear negated — never as the action to take while a holder is live.
        negated = re.findall(r"(?i)(?:do not|don't|never) run `shellgpt doctor --fix`", msg)
        assert len(negated) == msg.count("`shellgpt doctor --fix`"), msg
    ...
    from shellgpt_cli.web_routers import sessions

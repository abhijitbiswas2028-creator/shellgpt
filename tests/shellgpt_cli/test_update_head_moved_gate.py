"""Tests for the post-pull HEAD-movement gate in ``shellgpt update``.
...
Before this guard ``shellgpt update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the shellgpt_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns shellgpt_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/shellgpt_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        shellgpt_main, "_resolve_update_branch", lambda args: "main"
    )
    ...
    monkeypatch.setattr(
        shellgpt_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        shellgpt_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        shellgpt_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(

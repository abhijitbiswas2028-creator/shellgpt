"""Tests for kanban board export / import (``shellgpt_cli.kanban_transfer``).
...
from shellgpt_cli import kanban_transfer as kt
from shellgpt_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("SHELLGPT_KANBAN_HOME", str(root))
        for var in ("SHELLGPT_KANBAN_DB", "SHELLGPT_KANBAN_WORKSPACES_ROOT",
                    "SHELLGPT_KANBAN_ATTACHMENTS_ROOT", "SHELLGPT_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this ShellGPT"):

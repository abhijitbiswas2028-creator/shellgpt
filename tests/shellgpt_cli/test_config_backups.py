"""config.yaml backups: one dir, deduped, bounded — never a pile of siblings in SHELLGPT_HOME."""
...
def test_repeat_backups_dedupe_and_rotate(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("shellgpt_cli.config_backups.time.strftime", lambda _fmt: next(stamps))

    first = backup_config(cfg, "pre-setup", keep=2)
    assert first is not None and first.parent == tmp_path / "backups" / "config"
    # Same bytes again → no new file (the shellgpt-setup-three-times case).

"""Tests for the AI_AGENT / SHELLGPT_AGENT harness-attribution env vars.
...
ShellGPT-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal ShellGPT' id in the public agent-harness
registry (``shellgpt-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the ShellGPT process env, and survives the
cross-session leak guard that strips ``SHELLGPT_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__SHELLGPT_CWD_testsession0__"
        env._snapshot_path = "/tmp/shellgpt-snap-testsession0.sh"
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT SHELLGPT=$SHELLGPT_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "SHELLGPT_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} SHELLGPT=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", SHELLGPT_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi SHELLGPT=false" in out.stdout

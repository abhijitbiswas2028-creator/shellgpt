"""A memory provider installed under ``$SHELLGPT_HOME/plugins/`` (catalog install) keeps the
...
def resolve_config_path():
    return Path(os.environ["SHELLGPT_HOME"]) / "honcho.json"
...
@pytest.fixture
def user_dir_honcho(monkeypatch, tmp_path, _isolate_shellgpt_home):
    ...
    plugin_dir = get_shellgpt_home() / "plugins" / "honcho"
    ...
    (get_shellgpt_home() / "honcho.json").write_text(
        json.dumps({"hosts": {"shellgpt": {"workspace": "from-user-dir"}}}), encoding="utf-8"
    )

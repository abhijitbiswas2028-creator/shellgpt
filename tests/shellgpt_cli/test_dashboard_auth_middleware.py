...
from shellgpt_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``shellgpt_session_at`` / ``shellgpt_session_rt``
...
def test_bearer_401_audits_client_facing_reason(gated_app, tmp_path):
    ...
    log_path = tmp_path / "shellgpt_test" / "logs" / "dashboard-auth.log"
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["shellgpt_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)

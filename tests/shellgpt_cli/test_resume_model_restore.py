...
SessionDB.session_gateway_runtime (shellgpt_state.py) — the round trip that
makes `shellgpt --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare ShellGPTCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.ShellGPTCLI)
...
def test_restore_llamacpp_session_keeps_launch_base_url_for_same_provider(monkeypatch):
    """`shellgpt --provider llamacpp --base-url X --resume` is user intent for the SAME provider the

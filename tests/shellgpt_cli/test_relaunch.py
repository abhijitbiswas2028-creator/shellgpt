"""Tests for shellgpt_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from shellgpt_cli import relaunch as relaunch_mod


class TestResolveShellGPTBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/shellgpt"
        ...
        assert relaunch_mod.resolve_shellgpt_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "shellgpt"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'shellgpt' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_shellgpt_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/shellgpt" if name == "shellgpt" else None
        )
        assert relaunch_mod.resolve_shellgpt_bin() == "/usr/bin/shellgpt"
...
class TestBuildRelaunchArgv:
    ...
    def test_preserves_inherited_flags(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_shellgpt_bin", lambda: "/usr/bin/shellgpt")
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/shellgpt", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/shellgpt", ["/usr/bin/shellgpt", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        shellgpt).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_shellgpt_bin", lambda: r"C:\Users\test\shellgpt.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\shellgpt.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\shellgpt.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_shellgpt_bin", lambda: r"C:\shellgpt.exe")
...
class TestResolveShellGPTBinWindowsPyGuard:
    """On Windows, resolve_shellgpt_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a shellgpt.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\shellgpt.exe" if name == "shellgpt" else None,
        )

        bin_path = relaunch_mod.resolve_shellgpt_bin()
        # Must NOT be the .py — must be the shellgpt.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\shellgpt.exe"
    ...
    def test_posix_python_launcher_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        The git installer invokes ``shellgpt`` with the venv interpreter while the
        ...
        script = tmp_path / "shellgpt"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        wrapper = tmp_path / "bin" / "shellgpt"
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: str(wrapper) if name == "shellgpt" else None,
        )
        assert relaunch_mod.resolve_shellgpt_bin() == str(wrapper)
    ...
    def test_posix_python_launcher_on_path_falls_back_to_current_python(
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
            lambda name: str(script) if name == "shellgpt" else None,
        )

        assert relaunch_mod.resolve_shellgpt_bin() is None

        # A console script pinned to the running interpreter keeps the venv: still exec-able.
        pinned = tmp_path / "pinned" / "shellgpt"
        pinned.parent.mkdir()
        pinned.write_text(f"#!{sys.executable}\n")
        pinned.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(pinned), "chat"])
        assert relaunch_mod.resolve_shellgpt_bin() == str(pinned)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_shellgpt_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND shellgpt.exe

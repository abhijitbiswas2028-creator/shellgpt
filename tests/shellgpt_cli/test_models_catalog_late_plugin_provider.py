"""A provider registered after ``shellgpt_cli.models`` was imported still reaches the picker catalog.

``CANONICAL_PROVIDERS`` admitted plugin providers once, at import. A plugin whose imports pull
``shellgpt_cli.models`` in mid-discovery, or a profile registered at runtime, never reached
...
def test_late_registered_provider_reaches_picker_catalog(monkeypatch):
    import shellgpt_cli.models_catalog_static as catalog
    from shellgpt_cli.models import list_available_providers
    ...
    monkeypatch.setattr("shellgpt_cli.models.CANONICAL_PROVIDERS", catalog.CANONICAL_PROVIDERS)
    monkeypatch.setattr("shellgpt_cli.models._PROVIDER_LABELS", catalog._PROVIDER_LABELS)

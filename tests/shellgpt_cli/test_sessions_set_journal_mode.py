"""`shellgpt sessions set-journal-mode` converts an existing WAL store offline and refuses under a foreign holder.
...
from shellgpt_cli.sessions_cmd_journal_mode import _refusal
...
def test_set_journal_mode_converts_wal_store_offline(tmp_path, monkeypatch, capsys):
    ...
    _wal_store(db)
    monkeypatch.setattr("shellgpt_state.DEFAULT_DB_PATH", db)
...
def test_set_journal_mode_refuses_while_another_process_holds_the_store(
    ...
    monkeypatch.setattr("shellgpt_state_holders.foreign_state_db_holders", lambda path: [(-1, "scan failed")])
...
@pytest.mark.parametrize("target,current,cross_vm,expected", [
    ...
    ("delete", "not-a-database", False, "not a ShellGPT SQLite store"),
    ("delete", "unknown(3/3)", False, "not a ShellGPT SQLite store"),
])

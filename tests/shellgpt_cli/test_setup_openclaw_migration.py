...
from shellgpt_cli import setup_migration
...
class TestOfferOpenclawMigration:
    ...
    def test_skips_when_user_declines(self, tmp_path):
        ...
        openclaw_dir.mkdir()
        script = tmp_path / "openclaw_to_shellgpt.py"
        script.write_text("# placeholder")
        with (
            patch("shellgpt_cli.setup.Path.home", return_value=tmp_path),
            patch.object(setup_migration, "_OPENCLAW_SCRIPT", script),
            patch.object(setup_mod, "prompt_yes_no", return_value=False),
        ):
            assert setup_mod._offer_openclaw_migration(tmp_path / ".shellgpt") is False
    ...
    def test_runs_migration_when_user_accepts(self, tmp_path):
        ...
        # Create a fake shellgpt home with config
        ...
        fake_mod = ModuleType("openclaw_to_shellgpt")
        ...
        fake_migrator.migrate.return_value = {
            ...
            "output_dir": str(shellgpt_home / "migration"),
        }
        ...
        with (
            ...
            result = setup_mod._offer_openclaw_migration(shellgpt_home)
...
class TestSetupWizardSkipsConfiguredSections:
    ...
    def test_sections_skipped_when_migration_imported_settings(self, tmp_path):
        ...
        def fake_migration(shellgpt_home):
        ...
        # vars via shellgpt_cli.gateway.get_env_value, NOT setup_mod's. Patch
        ...
        with (
            patch.object(setup_mod, "ensure_shellgpt_home"),
            patch.object(
                setup_mod, "load_config",
                side_effect=[{}, reloaded_config],
            ),
            patch.object(setup_mod, "get_shellgpt_home", return_value=tmp_path),
            patch.object(setup_mod, "get_env_value", side_effect=env_side),
            patch.object(gateway_mod, "get_env_value", side_effect=env_side),
            patch.object(setup_mod, "is_interactive_stdin", return_value=True),
            patch("shellgpt_cli.auth.get_active_provider", return_value=None),

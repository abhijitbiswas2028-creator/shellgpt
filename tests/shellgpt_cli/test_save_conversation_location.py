...
Regression: the old implementation wrote ``shellgpt_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``shellgpt sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.shellgpt/sessions/saved/`` and prints
...
def shellgpt_home(tmp_path, monkeypatch):
    ...
    # Clear any cached shellgpt_home computation
    import shellgpt_constants
    if hasattr(shellgpt_constants, "_shellgpt_home_cache"):
        shellgpt_constants._shellgpt_home_cache = None
    return home
...
def test_save_conversation_writes_under_shellgpt_home(shellgpt_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.shellgpt/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the SHELLGPT_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "shellgpt_constants"]:
    ...
    cli.ShellGPTCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("shellgpt_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.shellgpt/sessions/saved/
    saved_dir = shellgpt_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("shellgpt_conversation_*.json"))
    ...
    assert "shellgpt --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(shellgpt_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "shellgpt_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bad_format_shows_usage(shellgpt_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "shellgpt_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.ShellGPTCLI.save_conversation(stub, "/save pdf")

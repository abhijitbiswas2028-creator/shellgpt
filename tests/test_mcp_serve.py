"""
Tests for mcp_serve — ShellGPT MCP server.
...
def _isolate_shellgpt_home(tmp_path, monkeypatch):
    """Redirect SHELLGPT_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking shellgpt_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <SHELLGPT_HOME>/state.db for its mtime gate; the autouse
        # fixture points SHELLGPT_HOME at tmp_path.
        db_path = tmp_path / "state.db"

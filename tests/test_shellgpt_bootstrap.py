"""Tests for shellgpt_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every ShellGPT entry point
(shellgpt, shellgpt-agent, shellgpt-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every ShellGPT entry point imports shellgpt_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported shellgpt_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("shellgpt_bootstrap", None)
    import shellgpt_bootstrap  # noqa: WPS433
    return shellgpt_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the ShellGPT CLI
...
class TestEntryPointsImportBootstrap:
    """Every ShellGPT entry point must import shellgpt_bootstrap as its
    ...
    # Entry points that invoke ShellGPT as a process.  Each one must
    # import shellgpt_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "shellgpt_cli/main.py",   # shellgpt CLI (console_script)
        "run_agent.py",          # shellgpt-agent (console_script)
        "acp_adapter/entry.py",  # shellgpt-acp (console_script)
    ...
    def test_library_imports_of_dual_use_entry_modules_stay_side_effect_free(self, tmp_path):
        # The codex shellgpt-tools MCP server and the compute host are ``python -m`` entry points
        # that agent/gateway code and tests also import; there the bootstrap exported TMPDIR and
        # SHELLGPT_SCRATCH_DIR into every importer (gateway.relay's read-only routing included).
        code = textwrap.dedent("""
            ...
            print(json.dumps({"bootstrapped": "shellgpt_bootstrap" in sys.modules, "changed": changed}))
        """)
        ...
        env.update({"HOME": str(tmp_path), "USERPROFILE": str(tmp_path), "PYTHONPATH": str(repo),
                    "SHELLGPT_HOME": str(tmp_path / "home"), "PYTHONDONTWRITEBYTECODE": "1"})
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import shellgpt_bootstrap' and that
        ...
        half-finished ``shellgpt update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``shellgpt_bootstrap`` as a top-level module) leaves shellgpt
        ...
        """
        # Resolve relative to the shellgpt-agent repo root.  Tests live
        # at tests/test_shellgpt_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # shellgpt start even when shellgpt_bootstrap hasn't been
        ...
        assert first_import_name == "shellgpt_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'shellgpt_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import shellgpt_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing ShellGPT's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SHELLGPT_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SHELLGPT_PYTHON_SRC_ROOT"] = env
            elif "SHELLGPT_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SHELLGPT_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/shellgpt")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SHELLGPT_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SHELLGPT_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/shellgpt", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # ShellGPT root.  The guard must relocate ShellGPT to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/shellgpt"])
        assert result[0] == "/opt/shellgpt"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the ShellGPT root.
        assert result.index("/opt/shellgpt") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SHELLGPT_PYTHON_SRC_ROOT"] = "/env/shellgpt"
            hb.harden_import_path()
            assert sys.path[0] == "/env/shellgpt"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SHELLGPT_PYTHON_SRC_ROOT", None)
...
class TestHappyEyeballsSocketConnect:
    ...
    def test_import_routes_http_client_and_urllib3_connects_through_the_racer(self):
        ...
        assert getattr(urllib3_connection.create_connection, "_shellgpt_happy_eyeballs", False)
    ...
    def test_racer_bug_raises_instead_of_falling_back_to_the_serial_walk(self, monkeypatch):
        ...
        for racer in (socket.create_connection, urllib3_connection.create_connection):
            assert getattr(racer, "_shellgpt_happy_eyeballs", False)
...
class TestNeverFreeEnviron:
    ...
    def test_arrays_superseded_by_new_names_stay_intact(self):
        ...
        subprocess.run([sys.executable, "-c", textwrap.dedent("""
            import ctypes, os, shellgpt_bootstrap
            ...
            for i in range(80):
                ...
                os.environ[f"SHELLGPT_ENVIRON_PROBE_{i}"] = "1"
            ...
            assert os.environ["SHELLGPT_ENVIRON_PROBE_79"] == "1"
    ...
    def test_concurrent_writers_lose_no_name_in_the_c_environ(self):
        ...
        subprocess.run([sys.executable, "-c", textwrap.dedent("""
            import ctypes, os, sys, threading, shellgpt_bootstrap
            getenv = ctypes.CDLL(None).getenv
            getenv.restype, getenv.argtypes = ctypes.c_char_p, [ctypes.c_char_p]
            sys.setswitchinterval(1e-6)
            batches = [[f"SHELLGPT_ENVIRON_RACE_{t}_{i}" for i in range(200)] for t in range(8)]
    ...
    def test_set_del_churn_of_the_same_names_keeps_memory_bounded(self):
        ...
        subprocess.run([sys.executable, "-c", textwrap.dedent("""
            import os, tracemalloc, shellgpt_bootstrap
            def churn(cycles):
                for _ in range(cycles):
                    for k in range(4):
                        os.environ[f"SHELLGPT_ENVIRON_CHURN_{k}"] = "1"
                    for k in range(4):
                        del os.environ[f"SHELLGPT_ENVIRON_CHURN_{k}"]
            churn(50)
            tracemalloc.start()

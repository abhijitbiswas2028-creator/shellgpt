...
#
# `services.shellgpt-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  shellgptHome, programs.shellgpt-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "shellgpt-config-attrs";
    description = "ShellGPT YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $SHELLGPT_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "ShellGPT Agent";
      ...
      package = mkOption {
        ...
        description = "The shellgpt-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The ShellGPT configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `shellgpt config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $SHELLGPT_HOME/.env. ShellGPT reads that file at each start, with
          load_shellgpt_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."shellgpt/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that ShellGPT refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          ShellGPT reads SOUL.md and memories/ from SHELLGPT_HOME, so put those
          files in `shellgptHomeFiles`.
      ...
      shellgptHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into SHELLGPT_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          ShellGPT reads SOUL.md and the memory files from SHELLGPT_HOME and not
          from the working directory. Declare those files here, or ShellGPT
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the shellgpt plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. ShellGPT discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          shellgpt_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as shellgpt (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in shellgpt-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `shellgpt gateway`.";
      };
      ...
      # `shellgpt serve` and `shellgpt dashboard` are the same entry point,
      # shellgpt_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that ShellGPT Desktop
      ...
      # controls an external gateway, with `shellgpt gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that ShellGPT
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            SHELLGPT_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. ShellGPT Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."shellgpt/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `shellgpt` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.shellgpt-agent.installPackage was removed. ShellGPT now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.shellgpt-agent.enable = ${lib.boolToString (value != false)};  # the shellgpt CLI, and SHELLGPT_HOME for your shells
        programs.shellgpt-agent.desktop.enable = true;  # the desktop application

      `services.shellgpt-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "shellgpt-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "shellgpt-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "shellgpt-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "shellgpt-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "shellgpt-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # SHELLGPT_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.shellgptHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and ShellGPT make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${shellgptHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. ShellGPT
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${shellgptHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${shellgptHome}/config.yaml
            ${run}chmod ${modes.config} ${shellgptHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "shellgpt-managed" managedSystem} ${shellgptHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${shellgptHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${shellgptHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${shellgptHome}/auth.json"
        else
          ''
            if [ ! -e ${shellgptHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${shellgptHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree shellgptHome cfg.shellgptHomeFiles}
      ...
      ${run}find ${shellgptHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "shellgpt-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${shellgptHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/shellgpt"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps shellgpt as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "shellgpt-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "shellgpt-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "shellgpt-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          SHELLGPT_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export SHELLGPT_DASHBOARD_SESSION_TOKEN

          if [ -z "$SHELLGPT_DASHBOARD_SESSION_TOKEN" ]; then
            echo "shellgpt-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "shellgpt-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "shellgpt-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "shellgpt-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "shellgpt-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "shellgpt-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "shellgpt-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "ShellGPT Agent web dashboard and desktop backend"
    else
      "ShellGPT Agent backend for ShellGPT Desktop";

  # The environment that each ShellGPT process needs, from either module.
  #
  # managedSystem gives the value of SHELLGPT_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      SHELLGPT_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give ShellGPT an identity and a memory, use
          ${optionPath}.shellgptHomeFiles instead. Those files go to
          SHELLGPT_HOME. ShellGPT reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of SHELLGPT_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"

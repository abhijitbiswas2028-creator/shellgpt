...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      shellgptNpmLib = self'.packages.default.passthru.shellgptNpmLib;
      ...
      shellgptAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "shellgpt" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../shellgpt} $out/bin/shellgpt
          '')
        ...
        shellHook = ''
          ${shellgptAgentDevShellHook}
          ${shellgptNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export SHELLGPT_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "ShellGPT Agent dev shell in $SHELLGPT_PYTHON_SRC_ROOT"
          echo "Ready. Run 'shellgpt' or 'sandbox shellgpt' to start."
        '';
      };
    };
}

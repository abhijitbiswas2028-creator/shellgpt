# nix/desktop.nix — ShellGPT Desktop (Electron) app build + wrapper
#
# `shellgptAgent` is the fully-built `.#default` package — it ships the
# `shellgpt` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SHELLGPT_DESKTOP_SHELLGPT` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing ShellGPT CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # SHELLGPT_HOME and SHELLGPT_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "shellgpt-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "shellgpt-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = shellgptNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "shellgpt-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe shellgptNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/shellgpt-desktop $out/bin
    cp -r ${renderer}/* $out/share/shellgpt-desktop/
    ...
    substituteInPlace $out/share/shellgpt-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/shellgpt-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SHELLGPT_DESKTOP_SHELLGPT to the absolute path of the nix-built `shellgpt`
    # binary so the desktop's resolver step 4 ("existing ShellGPT CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/shellgpt-desktop \
      --add-flags "$out/share/shellgpt-desktop" \
      --set SHELLGPT_DESKTOP_SHELLGPT "${lib.getExe shellgptAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/shellgpt.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../shellgpt_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/shellgpt-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/shellgpt.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/shellgpt.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for ShellGPT Agent";
    homepage = "https://github.com/NousResearch/shellgpt-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "shellgpt-desktop";
  };
}

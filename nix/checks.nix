...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      shellgpt-agent = self'.packages.default;
      shellgptVenv = shellgpt-agent.shellgptVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "shellgpt-check";
                homeDirectory = "/home/shellgpt-check";
                stateVersion = "24.11";
              };
            }
            { services.shellgpt-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.shellgpt-agent = programs;
              services.shellgpt-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.shellgpt-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.shellgpt-agent);
      ...
      configKeys = pkgs.runCommand "shellgpt-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${shellgptVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "shellgpt-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "shellgpt-build-package" { } ''
          echo "PASS: package built at ${shellgpt-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "shellgpt-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.SHELLGPT_TEST = "1";
                environmentFiles = [ "/run/secrets/shellgpt-env" ];
                shellgptHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "shellgpt" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "shellgpt" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.shellgptAgentSetup.data;

            failures =
              lib.optional (names != [
                "shellgpt-agent"
                "shellgpt-backend"
              ]) "expected shellgpt-agent + shellgpt-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/shellgpt gateway" (argvOf "shellgpt-agent")
              ) "gateway process does not run `shellgpt gateway`: ${argvOf "shellgpt-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/shellgpt serve" (argvOf "shellgpt-backend")
              ) "backend process does not run `shellgpt serve`: ${argvOf "shellgpt-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "shellgpt-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/shellgpt-check/.shellgpt" (envOf n)) names
              ) "gateway and backend must share one SHELLGPT_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.SHELLGPT_HOME or null != "/home/shellgpt-check/.shellgpt"
              ) "programs.shellgpt-agent.enable must export SHELLGPT_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "shellgpt-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/shellgpt-check/.shellgpt/SOUL.md" activation
              ) "shellgptHomeFiles must install into SHELLGPT_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads SHELLGPT_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "SHELLGPT_MANAGED=home-manager" (envOf n)) names
              ) "processes must report SHELLGPT_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "shellgpt-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "shellgpt-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/shellgpt-check";
            ...
            cases = [
              ...
              {
                name = "shellgptHomeFiles needs no directory";
                ok = accepts { shellgptHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "shellgpt-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one SHELLGPT_HOME ───────────────
        # `programs.enable` exports SHELLGPT_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.shellgpt
        # while the services use the SHELLGPT_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/shellgpt-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                shellgptHome = "/home/shellgpt-check/.shellgpt-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "shellgpt-desktop") cfg.home.packages;
            ...
            # second ShellGPT runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "shellgpt-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.shellgpt-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.shellgpt-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one shellgpt-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "SHELLGPT_HOME" != "/home/shellgpt-check/.shellgpt-work"
              ) "the launcher must carry SHELLGPT_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "SHELLGPT_HOME")})"
              ++ lib.optional (
                setValue "SHELLGPT_MANAGED" != "home-manager"
              ) "the launcher must report SHELLGPT_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "SHELLGPT_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "SHELLGPT_DESKTOP_SHELLGPT" != "${lib.head agentPackages}/bin/shellgpt"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "SHELLGPT_DESKTOP_SHELLGPT")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "SHELLGPT_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "SHELLGPT_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "SHELLGPT_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "SHELLGPT_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "SHELLGPT_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "shellgpt-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares SHELLGPT_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set SHELLGPT_HOME" wrapper
              ) "the launcher must carry SHELLGPT_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "SHELLGPT_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "SHELLGPT_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? shellgpt-backend || cfg.launchd.agents ? shellgpt-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "shellgpt-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `shellgpt`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.shellgpt-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.shellgpt-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "shellgpt-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "shellgpt-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "shellgpt" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."shellgpt-agent-setup".text;

            failures =
              lib.optional (names != [
                "shellgpt-agent"
                "shellgpt-backend"
              ]) "expected shellgpt-agent + shellgpt-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/shellgpt gateway" (execOf "shellgpt-agent")
              ) "gateway unit does not run `shellgpt gateway`: ${execOf "shellgpt-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/shellgpt dashboard" (execOf "shellgpt-backend")
              ) "backend unit does not run `shellgpt dashboard`: ${execOf "shellgpt-backend"}"
              ++ lib.optional (
                units.shellgpt-agent.environment.SHELLGPT_HOME != units.shellgpt-backend.environment.SHELLGPT_HOME
              ) "gateway and backend must share one SHELLGPT_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/shellgpt/.shellgpt/SOUL.md" activation
              ) "shellgptHomeFiles must install into SHELLGPT_HOME";
          ...
          in
          pkgs.runCommand "shellgpt-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs shellgpt, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.shellgpt-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/shellgpt serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec shellgpt directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "shellgpt-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "shellgpt-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec shellgpt, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/shellgpt dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "shellgpt-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.shellgpt-agent;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.shellgpt-agent;
          ...
          in
          pkgs.runCommand "shellgpt-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $SHELLGPT_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                SHELLGPT_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "shellgpt-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'SHELLGPT_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--shellgpt-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "shellgpt-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "shellgpt-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${shellgpt-agent}/bin/shellgpt || (echo "FAIL: shellgpt binary missing"; exit 1)
          test -x ${shellgpt-agent}/bin/shellgpt-agent || (echo "FAIL: shellgpt-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${shellgpt-agent}/bin/shellgpt --version 2>&1 | grep -qi "shellgpt" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "shellgpt-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in shellgpt shellgpt-agent shellgpt-acp; do
            test -x ${shellgpt-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "shellgpt-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking shellgpt --help ==="
          ${shellgpt-agent}/bin/shellgpt --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${shellgpt-agent}/bin/shellgpt --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "shellgpt-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${shellgpt-agent}/share/shellgpt-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${shellgpt-agent}/share/shellgpt-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SHELLGPT_BUNDLED_SKILLS" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SHELLGPT_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${shellgpt-agent}/share/shellgpt-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${shellgpt-agent}/share/shellgpt-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SHELLGPT_OPTIONAL_SKILLS" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SHELLGPT_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "shellgpt-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${shellgpt-agent}/share/shellgpt-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${shellgpt-agent}/share/shellgpt-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SHELLGPT_BUNDLED_PLUGINS" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SHELLGPT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "shellgpt-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${shellgpt-agent}/share/shellgpt-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${shellgpt-agent}/share/shellgpt-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${shellgpt-agent}/share/shellgpt-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SHELLGPT_BUNDLED_LOCALES" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SHELLGPT_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + SHELLGPT_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SHELLGPT_BUNDLED_LOCALES=${shellgpt-agent}/share/shellgpt-agent/locales \
            ${shellgptVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SHELLGPT_BUNDLED_LOCALES set"; exit 1)
        ...
        # SHELLGPT_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "shellgpt-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${shellgpt-agent}/share/shellgpt-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${shellgpt-agent}/share/shellgpt-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "SHELLGPT_OPTIONAL_MCPS" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: SHELLGPT_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${shellgpt-agent}/bin/shellgpt mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: shellgpt mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "shellgpt-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${shellgpt-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${shellgpt-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SHELLGPT_TUI_DIR" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SHELLGPT_TUI_DIR set in wrapper"
        ...
        # Verify SHELLGPT_NODE is set in wrapper and points to Node 26+
        # (ShellGPT pins its toolchain to Node 26 everywhere)
        shellgpt-node = pkgs.runCommand "shellgpt-node-version" { } ''
          set -e
          echo "=== Checking SHELLGPT_NODE in wrapper ==="
          grep -q "SHELLGPT_NODE" ${shellgpt-agent}/bin/shellgpt || \
            (echo "FAIL: SHELLGPT_NODE not set in wrapper"; exit 1)
          echo "PASS: SHELLGPT_NODE present in wrapper"

          SHELLGPT_NODE=$(sed -n "s/^export SHELLGPT_NODE='\(.*\)'/\1/p" ${shellgpt-agent}/bin/shellgpt)
          test -x "$SHELLGPT_NODE" || (echo "FAIL: SHELLGPT_NODE=$SHELLGPT_NODE not executable"; exit 1)
          echo "PASS: SHELLGPT_NODE executable at $SHELLGPT_NODE"

          NODE_MAJOR=$("$SHELLGPT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, ShellGPT requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All SHELLGPT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SHELLGPT_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "shellgpt-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SHELLGPT_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SHELLGPT_MANAGED guards ==="
          check_blocked "config set" ${shellgpt-agent}/bin/shellgpt config set model foo
          check_blocked "config edit" ${shellgpt-agent}/bin/shellgpt config edit
        ...
        extra-python-packages = let
          ...
          shellgptWithExtra = shellgpt-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "shellgpt-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${shellgptWithExtra}/bin/shellgpt || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${shellgptWithExtra}/bin/shellgpt || \
          ...
          if grep -q "PYTHONPATH" ${shellgpt-agent}/bin/shellgpt; then
        ...
        extra-dependency-groups = let
          shellgptWithGroups = shellgpt-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "shellgpt-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${shellgptWithGroups}"
          echo "venv: ${shellgptWithGroups.shellgptVenv}"
        ...
        messaging-variant = pkgs.runCommand "shellgpt-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.shellgptVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "shellgpt-config-roundtrip" {
          ...
          merge_and_load() {
            local shellgpt_home="$1"
            export SHELLGPT_HOME="$shellgpt_home"
            ${configMergeScript} ${nixSettings} "$shellgpt_home/config.yaml"
            ${shellgptVenv}/bin/python3 -c '
import json, sys

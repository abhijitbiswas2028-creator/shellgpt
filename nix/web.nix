# nix/web.nix — ShellGPT Web Dashboard (Vite/React) frontend build
{ shellgptNpmLib, ... }:
shellgptNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @shellgpt/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../shellgpt_cli/web_dist for the

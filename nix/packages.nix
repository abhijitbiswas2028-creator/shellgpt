# nix/packages.nix — ShellGPT Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./shellgpt-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.shellgptTui;
        web = full.shellgptWeb;
        desktop = full.shellgptDesktop;

        update-npm-lockfile = full.shellgptNpmLib.updateNpmLockfile;
      };
    };
}

# nix/nixosModules.nix — the NixOS module for shellgpt-agent
#
...
#
# Container mode: shellgpt runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SHELLGPT_HOME/.env
# and read by shellgpt at startup — no container recreation needed for env changes.
#
# Tool resolution: the shellgpt wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.shellgpt-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      shellgpt-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      shellgptHome = "${cfg.stateDir}/.shellgpt";
      ...
      # SHELLGPT_HOME via addToSystemPackages, so they can save settings through the
      ...
      # shellgpt user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "shellgpt-container-entrypoint" ''
        set -eu

        SHELLGPT_UID="''${SHELLGPT_UID:?SHELLGPT_UID must be set}"
        SHELLGPT_GID="''${SHELLGPT_GID:?SHELLGPT_GID must be set}"

        # ── Group: ensure a group with GID=$SHELLGPT_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$SHELLGPT_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="shellgpt"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$SHELLGPT_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$SHELLGPT_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$SHELLGPT_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$SHELLGPT_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="shellgpt"
          TARGET_HOME="/home/shellgpt"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$SHELLGPT_UID" -g "$SHELLGPT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$SHELLGPT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$SHELLGPT_UID:$SHELLGPT_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure SHELLGPT_HOME is owned by the target user.
        ...
        if [ -n "''${SHELLGPT_HOME:-}" ] && [ -d "$SHELLGPT_HOME" ]; then
          find "$SHELLGPT_HOME" \! -user "$SHELLGPT_UID" -exec chown "$SHELLGPT_UID:$SHELLGPT_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/shellgpt-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/shellgpt-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/shellgpt ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/shellgpt
          chmod 0440 /etc/sudoers.d/shellgpt
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$SHELLGPT_UID" --regid="$SHELLGPT_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $SHELLGPT_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "shellgpt-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        shellgpt_bin=${containerDataDir}/current-package/bin/shellgpt
      '';

      # Default: /var/lib/shellgpt/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the shellgpt group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit shellgptHome; };
    ...
    in
    {
      options.services.shellgpt-agent =
        common.sharedOptions {
          defaultPackage = shellgpt-agent;
          defaultPackageText = lib.literalExpression "shellgpt-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/shellgpt";
              description = "State directory. Contains .shellgpt/ subdir (SHELLGPT_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the shellgpt CLI to environment.systemPackages and export
                SHELLGPT_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.shellgpt symlink to the service
                  stateDir. These users are automatically added to the shellgpt group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.shellgpt-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the shellgpt CLI to system PATH and export SHELLGPT_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.shellgpt/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.SHELLGPT_HOME = shellgptHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.shellgpt-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.shellgpt-agent.workingDirectory;
                optionPath = "services.shellgpt-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.shellgpt-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the shellgpt user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.shellgpt-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed shellgpt
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure shellgpt is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${shellgptHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${shellgptHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."shellgpt-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  mkdir -p ${shellgptHome}
                  mkdir -p ${cfg.stateDir}/home
                  mkdir -p ${cfg.workingDirectory}
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${shellgptHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${shellgptHome} ${cfg.workingDirectory}
                  ...
                  find ${shellgptHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${shellgptHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${shellgptHome}/$_subdir"
                    chmod 2770 "${shellgptHome}/$_subdir"
                    find "${shellgptHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg shellgptHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${shellgptHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${shellgptHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${shellgptHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.shellgpt";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${shellgptHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "shellgpt-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.shellgpt -> stateDir/.shellgpt for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.shellgpt-agent = {
              description = "ShellGPT Agent Gateway";
              ...
              # $SHELLGPT_HOME/.env by the activation script. load_shellgpt_dotenv()
              ...
              preStart = lib.mkIf lingerEnabled ''
                ...
                if [ ! -S "/run/user/$(id -u)/bus" ]; then
                  echo "shellgpt-agent: no user bus at /run/user/$(id -u)/bus after 10s;" \
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.shellgpt-agent = {
              description = "ShellGPT Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
                "network-online.target"
              ]
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  SHELLGPT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  SHELLGPT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env SHELLGPT_UID="$SHELLGPT_UID" \
                    --env SHELLGPT_GID="$SHELLGPT_GID" \
                    --env SHELLGPT_HOME=${containerDataDir}/.shellgpt \
                    --env SHELLGPT_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/shellgpt gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

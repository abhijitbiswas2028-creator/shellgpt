# nix/homeManagerModules.nix — the Home Manager module for shellgpt-agent
#
# This module is the user-level equivalent of nixosModules.default. ShellGPT is
...
#
# `services.shellgpt-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.shellgpt-agent.enable and
#                                        home.sessionVariables
#   added     programs.shellgpt-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.shellgpt")  -> shellgptHome, set directly
#
# To use the module:
#   imports = [ shellgpt-agent.homeManagerModules.default ];
#   programs.shellgpt-agent = {
#     enable = true;          # the shellgpt CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.shellgpt-agent;
      ...
      # not reach it, and the application would open ~/.shellgpt while the
      # services use shellgptHome. Thus the launcher carries the value itself.
      #
      # SHELLGPT_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        SHELLGPT_HOME = cfg.shellgptHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) SHELLGPT_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        SHELLGPT_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("SHELLGPT_DESKTOP_REMOTE_URL is set
      # but SHELLGPT_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          SHELLGPT_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export SHELLGPT_DESKTOP_REMOTE_TOKEN
        else
          echo "shellgpt-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "shellgpt-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.shellgpt-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". ShellGPT needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.shellgpt-agent` stays the authority for the state and the
      # configuration. This module reads shellgptHome and the backend address
      # from it, and never the reverse.
      options.programs.shellgpt-agent = {
        enable = lib.mkEnableOption ''
          the ShellGPT Agent command line application.

          This adds `shellgpt` to home.packages, and exports SHELLGPT_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.shellgpt-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.shellgpt-agent.package";
          description = ''
            The shellgpt-agent package to install.

            The default follows `services.shellgpt-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the ShellGPT Desktop application (Electron).

            This adds `shellgpt-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same ShellGPT
            runtime that `package` gives, and reads the SHELLGPT_HOME of
            `services.shellgpt-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own ShellGPT runtime with
            the usual distribution. This module gives it the Nix package
            instead, with SHELLGPT_DESKTOP_SHELLGPT. It installs no second copy
            of ShellGPT, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.shellgptDesktop;
            defaultText = lib.literalExpression "config.programs.shellgpt-agent.package.shellgptDesktop";
            description = ''
              The shellgpt-desktop package to use.

              The default follows `package`, and thus also
              `services.shellgpt-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own ShellGPT runtime, and this module cannot make
      ...
      options.services.shellgpt-agent =
        ...
        // {
          shellgptHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.shellgpt";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.shellgpt"'';
            description = ''
              The value of SHELLGPT_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.shellgpt` to it.
              This module sets SHELLGPT_HOME directly. Thus an existing
              ~/.shellgpt continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.shellgpt-work";
          };

          # `installPackage` moved to `programs.shellgpt-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `shellgpt` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.shellgpt-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.SHELLGPT_HOME = cfg.shellgptHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # SHELLGPT_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put SHELLGPT_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `shellgpt` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.shellgpt-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.shellgptAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) shellgptHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.shellgpt-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.shellgpt-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.shellgpt-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "shellgpt-agent";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.shellgpt-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "shellgpt-backend";
              };
            })
          ]
        ))

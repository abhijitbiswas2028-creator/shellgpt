...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # ShellGPT derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            shellgpt-agent = prev.shellgpt-agent.overrideAttrs (_old: {
              SHELLGPT_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $SHELLGPT_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SHELLGPT_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        shellgpt-agent = prev.shellgpt-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "shellgpt-agent-env" {
    shellgpt-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "shellgpt-agent-editable-env" {
    shellgpt-agent = dependency-groups;
  };
}

#!/usr/bin/env python3
"""SWE Runner with ShellGPT Trajectory Format

Runs tool-calling agent tasks in ShellGPT-Agent's execution environments (local,
docker, modal) and writes trajectories in ShellGPT format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str | None = None, timeout: int = 60, **kwargs):
    """Create a ShellGPT execution environment (``local`` ignores ``image``/``kwargs``).
...
class MiniSWERunner:
    """Tool-calling agent loop over a ShellGPT execution environment, emitting ShellGPT trajectories."""
    ...
    def _convert_to_shellgpt_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the ShellGPT trajectory format used by batch_runner.py."""
        system_msg = SHELLGPT_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + SHELLGPT_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_shellgpt_format(messages, task), "completed": completed, "api_calls": api_call_count,
...
def main(
    ...
    """
    Run SWE tasks with ShellGPT trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with ShellGPT Trajectory Format")
    print("=" * 60)

"""Anthropic Messages API adapter: client construction + the Messages call for ShellGPT's
...
# ShellGPT effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain ShellGPT tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://shellgpt-agent.nousresearch.com", "X-Title": "ShellGPT Agent",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    shellgpt's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str], route: str = None):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.shellgpt/.env) and then sends dual
...
def _custom_provider_extra_headers(base_url) -> Dict[str, str]:
    ...
    if not base_url:
        return {}
    try:
        from shellgpt_cli.config import get_custom_provider_extra_headers
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to shellgpt's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    ShellGPT tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("ShellGPT Agent", "Claude Code"), ("ShellGPT agent", "Claude Code"), ("Nous Research", "Anthropic"),
)
# The slug is rewritten only as a standalone prose word. Joined to a host, path, repo, mailbox
# or quoted as an identifier (``shellgpt-agent.nousresearch.com``, ``~/.shellgpt/shellgpt-agent/venv``,
# ``NousResearch/shellgpt-agent``, ``skill_view(name='shellgpt-agent')``) it is an address the model
...
_OAUTH_SLUG_PATTERN = re.compile(r"""(?<![\w./:@'"`-])shellgpt-agent(?![\w/@-]|\.\w)""")
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning ShellGPT shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that ShellGPT surfaces in its CLI. We request "summarized" so the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_shellgpt_home': ('shellgpt_constants', 'get_shellgpt_home'),
    ...
    'read_shellgpt_oauth_credentials': ('agent.anthropic_credentials', 'read_shellgpt_oauth_credentials'),
    ...
    'run_shellgpt_oauth_login_pure': ('agent.anthropic_credentials', 'run_shellgpt_oauth_login_pure'),

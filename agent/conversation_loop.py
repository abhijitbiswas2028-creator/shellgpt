...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in shellgpt_state.py; kept local so importing
# shellgpt_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for ShellGPT tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but ShellGPT "
        ...
        "again. A known-good starting point is 65,536 tokens. In ShellGPT config, set "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `shellgpt auth reset anthropic`. Until that "
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if not getattr(agent, "_persist_disabled", False):
        try:
            from shellgpt_cli.lifecycle import invoke_hook as _invoke_hook
            _invoke_hook(
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from shellgpt_cli.moa_config import decode_moa_turn
...
def _run_conversation_turn(
    ...
    # Adopt ~/.shellgpt/.env credential/base-url edits made since the last turn — a
...
def _close_durable_failed_turn(agent, result: Any) -> None:
    """Append a ShellGPT-authored assistant boundary when a failed turn left ``user`` as the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('shellgpt_constants', 'PARTIAL_STREAM_STUB_ID'),

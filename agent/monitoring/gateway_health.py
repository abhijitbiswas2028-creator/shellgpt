...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "shellgpt.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("shellgpt.gateway.up", int(bool(gateway_running))),
        metric("shellgpt.gateway.active_agents", active_agents),
        metric("shellgpt.gateway.busy", int(bool(busy))),
        metric("shellgpt.gateway.drainable", int(bool(drainable))),
        metric("shellgpt.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("shellgpt.gateway.state", 1, **{"shellgpt.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"shellgpt.platform": str(platform), "shellgpt.platform.state": state}
        metrics.append(metric("shellgpt.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("shellgpt.platform.degraded", int(is_degraded), **pattrs, **{"shellgpt.error_code": error_code}))
        if is_degraded:

...
_OBSERVABLE_METRIC_NAMES = (
    "shellgpt.gateway.up", "shellgpt.gateway.state", "shellgpt.gateway.active_agents", "shellgpt.gateway.busy",
    "shellgpt.gateway.drainable", "shellgpt.gateway.restart_requested", "shellgpt.gateway.background_work",
    "shellgpt.gateway.background_delegations", "shellgpt.platform.up", "shellgpt.platform.degraded",
    "shellgpt.cron.scheduler.heartbeat_age_seconds", "shellgpt.cron.scheduler.last_success_age_seconds",
    "shellgpt.cron.scheduler.catch_up_occurrences", "shellgpt.cron.jobs.enabled", "shellgpt.cron.jobs.running",
    "shellgpt.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="shellgpt-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("shellgpt.gateway.background_work", _read_background_work_count),
            ("shellgpt.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("shellgpt.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("shellgpt.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'shellgpt-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="shellgpt-gateway-health-export", daemon=True)
            thread.start()

"""Argument layer for the legacy ``shellgpt-agent`` runner (``run_agent.main``).

A console script calls its target with no arguments, so pointing ``shellgpt-agent``
...
# shellgpt_bootstrap first (UTF-8 stdio on Windows; no-op on POSIX), like every other entry point.
try:
    import shellgpt_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # partial `shellgpt update` — only skips the Windows UTF-8 stdio setup

# The `shellgpt-agent` console script lands here without shellgpt_cli.main: repair a `shellgpt update` killed
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="shellgpt-agent",
        description="Legacy single-query ShellGPT Agent runner. For the full CLI use `shellgpt`.",
    )
    parser.add_argument("--version", action="version", version=f"ShellGPT Agent v{__version__} ({__release_date__})")
...
def main(argv: Optional[List[str]] = None, *, run: Optional[Callable[..., object]] = None) -> int:
    ...
    if query is None and not args.list_tools:
        parser.print_help()
        print("\nNo query given: pass one with --query (or run `shellgpt` for the interactive CLI).")
        return 0

...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    ``SHELLGPT_CONCURRENT_TOOL_TIMEOUT_S`` is the legacy bridge, ``0``/negative disables."""
    from agent.deadline import resolve_timeout

    return resolve_timeout(
        "tools.concurrent_batch",
        default=_DEFAULT_CONCURRENT_TOOL_TIMEOUT_S,
        env_var="SHELLGPT_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
class _ConcurrentToolAuthorizationGate:
    ...
    def run(self, callback):
        if not self._serialization_lock.acquire(timeout=self._lock_timeout):
            ...
            # ``shellgpt mcp login``. Parking keeps the task alive so the 300s self-probe (and an explicit
...
_PRUNED_TOOL_ARGUMENTS_MESSAGE = (
    "Tool was not executed because effect-capable arguments contain a ShellGPT context-compression artifact. "
...
def _dispatch_authorized_once(
    ...
    *,
    ...
    """ShellGPT policy (scope → plugin pre-hooks → pruned-arg check → guardrails) then the one real dispatch.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before ShellGPT policy and dispatch exactly once."""
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state.dispatched:
                raise RuntimeError("ShellGPT tool execution callback invoked more than once")
    ...
    def _shellgpt_pipeline(relay_args: dict[str, Any]) -> Any:

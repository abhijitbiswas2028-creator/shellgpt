...
# ShellGPT' own machine-authored openers: a compaction handoff or resumed session must not be titled after them.
_MACHINE_PREFIXES = (
...
def _title_config() -> dict:
    """``auxiliary.title_generation`` (lazy read-only import: no shellgpt_cli cycle, no migration writes)."""
...
def _title_pin_may_share_endpoint(pinned_provider: str, main_provider: str, main_base_url: str) -> bool:
    ...
    (``shellgpt_cli.providers.custom_provider_aliases`` — the resolver's own identity set) or resolves to a
    configured custom entry serving ``main_base_url`` (a keyed ``providers:`` entry's display name does not
    alias its ``custom:<key>`` id).
    """
    from shellgpt_cli.config import get_compatible_custom_providers, load_config_readonly
    from shellgpt_cli.providers import custom_provider_aliases, resolve_custom_provider
...
def auto_title_session(
    ...
    trigger is the post-``shellgpt update`` window where lazy imports read NEW source against OLD modules."""
    try:
        ...
        logger.warning("Auto-title failed (harmless; if this started after an update, restart the running ShellGPT process): %s", e)
...
def _is_real_user_turn(message: Any) -> bool:
    """A question a person actually asked (ShellGPT persists machinery under ``role="user"``)."""

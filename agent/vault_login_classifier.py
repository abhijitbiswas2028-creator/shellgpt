...
INSPECTION_STAMP_ATTR = "data-shellgpt-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-shellgpt-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-shellgpt-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-shellgpt-vault-slot]").forEach((n) => n.removeAttribute("data-shellgpt-vault-slot"));

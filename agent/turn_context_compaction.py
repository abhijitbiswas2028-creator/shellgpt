...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; ShellGPT only
    initiates compaction in "shellgpt" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping ShellGPT preflight compression for codex app-server "
            "(mode=%s); ShellGPT will not start thread compaction here.",

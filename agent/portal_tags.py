...
Every ShellGPT request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=shellgpt-agent", "client=shellgpt-client-v<__version__>"]``.
The version is read live from ``shellgpt_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("shellgpt_affinity_scope", default=None)
...
def shellgpt_client_tag() -> str:
    """``client=shellgpt-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if shellgpt_cli is unimportable)."""
    try:
        from shellgpt_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=shellgpt-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=shellgpt-agent", shellgpt_client_tag()]

...
thread refreshes) → disk cache (~/.shellgpt/models_dev_cache.json, any age) →
...
from shellgpt_constants import openrouter_variant_base
...
# ShellGPT provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # ShellGPT provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → ShellGPT ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_shellgpt_ids(mdev_id: str) -> List[str]:
    """Return the ShellGPT provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for shellgpt_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(shellgpt_id)
...
def _models_dev_id(
    ...
    """models.dev provider id for a ShellGPT provider id, or None. A custom provider reaches the
    ...
    if mdev_id is None:
        ...
        if mdev_id is not None and mdev_id not in PROVIDER_TO_MODELS_DEV.values() \
            ...
            if (key, alias) not in _UNKNOWN_CATALOG_PROVIDER_WARNED:
                ...
                logger.warning("providers.%s: catalog_provider %r is neither a ShellGPT provider id nor a "
...
def _shellgpt_path(name: str) -> Path:
    from shellgpt_constants import get_shellgpt_home
    return get_shellgpt_home() / name


def _get_cache_path() -> Path:
    return _shellgpt_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _shellgpt_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_shellgpt_home(etag_path.parent)
...
def _get_provider_models(
    ...
    """Resolve a ShellGPT provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the ShellGPT
...
def _provider_override_section(provider: str, *, config: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by ShellGPT OR models.dev id), or None."""
    overrides = (
    ...
    if not overrides or not provider_key:
        return None
    # Forward (ShellGPT → models.dev id) and reverse (caller passed a models.dev id, config keyed by ShellGPT id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_shellgpt_ids(provider_key)]
...
def _builtin_model_metadata(
    ...
    """Built-in metadata for a provider/model pair, if ShellGPT has a vendor-specific entry."""
...
def get_provider_info(
    ...
    """Provider metadata by ShellGPT or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(
    ...
    """Full model metadata by ShellGPT or models.dev provider ID (exact match, then case-insensitive), or

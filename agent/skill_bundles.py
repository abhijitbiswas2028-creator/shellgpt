...
YAML files in ``<SHELLGPT_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``SHELLGPT_BUNDLES_DIR`` override (tests) or ``<SHELLGPT_HOME>/skill-bundles``."""
    override = os.environ.get("SHELLGPT_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_shellgpt_home() / "skill-bundles"
...
# File-level CRUD — used by `shellgpt bundles`.

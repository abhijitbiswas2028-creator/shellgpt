"""Unified removal contract for every credential source ShellGPT reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``shellgpt auth remove <provider> <N>`` must make the entry stay gone across
...
EXTERNAL_LOGINS_NOT_ADOPTED_NOTICE = (
    ...
    "ShellGPT uses only its own logins; run `shellgpt auth add <provider>` to add one."
)
...
def adopt_external_logins_enabled() -> bool:
    ...
    Codex and Claude OAuth refresh tokens are single-use and rotate, so once ShellGPT borrows a CLI's
    token pair the two programs hold one token family and whichever refreshes first logs the other
    out. When the user opts out, ShellGPT never reads or refreshes those files and says so once per
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.shellgpt/.env; hint when the shell exports it."""
    from shellgpt_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like shellgpt_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            f"(not in ~/.shellgpt/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to ShellGPT.",
            f"  The pool entry is now suppressed — ShellGPT will ignore "
            f"{env_var} until you run `shellgpt auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_shellgpt_pkce(provider: str, removed) -> RemovalResult:
    """~/.shellgpt/.anthropic_oauth.json is ours — delete it outright."""
    from shellgpt_constants import get_shellgpt_home

    result = RemovalResult()
    oauth_file = get_shellgpt_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared ShellGPT Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `shellgpt model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `shellgpt auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `shellgpt auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `shellgpt auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="shellgpt_pkce",
        remove_fn=_remove_shellgpt_pkce,
        description="~/.shellgpt/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `shellgpt auth add qwen-oauth` to re-enable if needed.",
        ),

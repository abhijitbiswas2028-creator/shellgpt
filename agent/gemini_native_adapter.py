...
try:
    import shellgpt_cli as _shellgpt_cli

    _SHELLGPT_VERSION = str(_shellgpt_cli.__version__)
except Exception:
    _SHELLGPT_VERSION = "0.0.0"
_API_CLIENT = f"shellgpt-agent/{_SHELLGPT_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "ShellGPT typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.shellgpt/.env, "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.shellgpt/.env (get one at https://aistudio.google.com/app/apikey), or run `shellgpt setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with ShellGPT retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No SHELLGPT_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)

...
retries x 3 ShellGPT retries), all counted against RPH.
"""
...
def _state_path(*, anonymous: bool = False) -> str:
    ...
    try:
        ...
        base = os.path.join(os.path.expanduser("~"), ".shellgpt")
...
def is_long_welcome_rate_limit(error_context: Any) -> bool:
    ...
    (``welcome_refusal`` from ``shellgpt_cli.anon_auth.parse_welcome_refusal``). Capacity refusals

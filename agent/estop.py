...
``shellgpt pause`` writes a sentinel at ``$SHELLGPT_HOME/ESTOP``; ``shellgpt resume``
...
(fail safe, e.g. ``touch ~/.shellgpt/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.shellgpt).
from agent.file_safety import _shellgpt_home_path as _shellgpt_home, _shellgpt_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `shellgpt pause`."""
    return _shellgpt_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (SHELLGPT_HOME=~/.shellgpt/profiles/<n>) must still honor an operator's ~/.shellgpt/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ ShellGPT is paused{tag}. New work is on hold; run `shellgpt resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `shellgpt resume` (%s)",

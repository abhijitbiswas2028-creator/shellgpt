...
ShellGPT runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`shellgpt config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually ShellGPT's env scrubber dropping
...
ShellGPT-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every ShellGPT tool and most Windows APIs. Prefer forward slashes in code

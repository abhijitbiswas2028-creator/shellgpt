...
## Layer 1 — Is this thing a ShellGPT plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the ShellGPT process**. Its LLM calls go through ShellGPT's already-configured provider. | Already uses Portal if user's ShellGPT is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside ShellGPT. That LLM call is already authenticated
via ShellGPT's provider config. The plugin's own server auth (e.g.
...
What ShellGPT Agent has that *feels* like OAuth — `shellgpt login --provider nous`
opening a browser, user signs in, token lands in `~/.shellgpt/auth.json` — is a
**ShellGPT-specific browser flow**. Under the hood it produces a credential
ShellGPT uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read ShellGPT's existing Portal credential out of `~/.shellgpt/auth.json`.
...
rides on the credential ShellGPT already holds.

Where this could live in ShellGPT:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for ShellGPT itself.
...
1. First decide: ShellGPT plugin (runs inside ShellGPT) or separate app? If plugin,
   it already uses Portal via ShellGPT's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   ShellGPT agent itself") is fully supported. ShellGPT treats agent
...
pitch in the Tool Gateway announcement is specifically about ShellGPT Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when ShellGPT is configured to use Portal as its provider. It is

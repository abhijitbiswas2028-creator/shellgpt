...
Set via `shellgpt model` (picker) or `shellgpt setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://shellgpt-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `shellgpt auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `shellgpt auth add openai-codex` |
| qwen-oauth | OAuth | `shellgpt auth add qwen-oauth` |
| minimax-oauth | OAuth | `shellgpt auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`shellgpt auth`).
Fallback chain when the primary fails: `shellgpt fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`shellgpt_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   shellgpt config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:

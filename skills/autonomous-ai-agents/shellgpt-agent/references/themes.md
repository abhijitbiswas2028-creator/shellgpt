# Themes / Skins — Author a ShellGPT Color Theme

Author a ShellGPT **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`shellgpt_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.shellgpt/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for ShellGPT itself.
...
- Write access to the ShellGPT home dir — `~/.shellgpt` by default, or `$SHELLGPT_HOME`
  / the active profile's dir. Skins live in `<shellgpt-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `shellgpt config set`).
...
3. `write_file` it to `<shellgpt-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `shellgpt config set display.skin default` (or they can `/skin default`).
...
```
shellgpt skin set <key> <hex>      # e.g. shellgpt skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `shellgpt skin set background "#08201f"` changes
only the background; `shellgpt skin use <name>` / `shellgpt skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.shellgpt`** when a profile is active — resolve the real home
  from `$SHELLGPT_HOME` first, falling back to `~/.shellgpt`.
...
- **Never hand-edit `config.yaml` to activate.** Use `shellgpt config set
...
- **You apply it, not the user.** `shellgpt config set display.skin <name>` is
...
- `read_file` the written `<shellgpt-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `shellgpt config get display.skin` and confirm it reports `<name>`.

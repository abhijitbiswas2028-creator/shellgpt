...
For occasional contributors and PR authors. Full developer docs: https://shellgpt-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.shellgpt/config.yaml` (settings), `~/.shellgpt/.env` (API keys) — both under `$SHELLGPT_HOME` when it is set.
...
`_SHELLGPT_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_shellgpt_home()` for paths,
never hardcode `~/.shellgpt`. For custom/local-only tools, write a plugin in
`~/.shellgpt/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `shellgpt_cli/commands.py`
...
- Tests auto-redirect `SHELLGPT_HOME` to temp dirs — never touch real `~/.shellgpt/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/shellgpt_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_shellgpt_home()` from `shellgpt_constants` for all paths (profile-safe)

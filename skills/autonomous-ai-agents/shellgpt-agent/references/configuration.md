...
Edit with `shellgpt config edit` or `shellgpt config set section.key value`.
Full reference: https://shellgpt-agent.nousresearch.com/docs/user-guide/configuration
...
`shellgpt config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `shellgpt tools` (interactive) or `shellgpt tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SHELLGPT_CORE_TOOLS` is the default bundle most platforms inherit).

...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger ShellGPT agent runs by POSTing events to a URL.
...
Add to `${SHELLGPT_HOME:-~/.shellgpt}/.env`:
```bash
...
All management is via the `shellgpt webhook` CLI command:

### Create a subscription
```bash
shellgpt webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.shellgpt/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.shellgpt/scripts/` (path traversal is blocked).

```bash
shellgpt webhook subscribe todoist-shellgpt \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-shellgpt-label.py" \
  --deliver telegram --deliver-chat-id "12345"
```

Full filter syntax: https://shellgpt-agent.nousresearch.com/docs/user-guide/messaging/webhooks#payload-filters
...
```bash
shellgpt webhook remove <name>
```

### Test a subscription
```bash
shellgpt webhook test <name>
shellgpt webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
shellgpt webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
shellgpt webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
shellgpt webhook subscribe alerts \
...
- Subscriptions persist to `~/.shellgpt/webhook_subscriptions.json`

## How It Works

1. `shellgpt webhook subscribe` writes to `~/.shellgpt/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status shellgpt-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.shellgpt/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `shellgpt webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `shellgpt webhook test <name>` to verify the route works.

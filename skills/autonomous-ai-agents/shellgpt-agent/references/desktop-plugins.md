...
Write plugins for the ShellGPT desktop app: statusbar items, layout panes,
...
system (`~/.shellgpt/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SHELLGPT_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SHELLGPT_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.shellgpt-package.json` marker) — the ONLY root the renderer loads from — so
...
- The ShellGPT desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SHELLGPT_HOME/desktop-plugins/` (usually
  `~/.shellgpt/desktop-plugins/`).

## How to Run

1. Create `$SHELLGPT_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.shellgpt/...` by default, or `~/.shellgpt/profiles/<profile>/...` under a
...
The ONLY import surface is `@shellgpt/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from ShellGPT
  ...
  (`shellgpt://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.shellgpt/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@shellgpt/plugin-sdk`, `react`, and

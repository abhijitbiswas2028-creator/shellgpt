...
Author widget apps for the ShellGPT TUI (`shellgpt --tui`): glanceable ambient
...
- The TUI must be in use (`shellgpt --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.shellgpt/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.shellgpt/logs/tui_gateway_crash.log` if a widget never appears.

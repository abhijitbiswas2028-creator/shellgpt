#!/usr/bin/env python3
"""Google Workspace API CLI for ShellGPT Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing ShellGPT-facing JSON contract and falls back to the Python client
...
from _shellgpt_home import get_shellgpt_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SHELLGPT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ShellGPT Agent")

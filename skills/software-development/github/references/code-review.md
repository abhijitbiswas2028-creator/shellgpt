...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from ShellGPT Agent\",
    \"comments\": [
...
```bash
source "${SHELLGPT_HOME:-$HOME/.shellgpt}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by ShellGPT Agent. Code looks clean — good test coverage, no security concerns."

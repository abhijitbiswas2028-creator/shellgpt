...
- Give it a name like "shellgpt-agent"
- Select scopes:
...
- Give it a title like "shellgpt-agent-<machine-name>"
...
uv run python "${SHELLGPT_HOME:-$HOME/.shellgpt}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _shellgpt_env="${SHELLGPT_HOME:-$HOME/.shellgpt}/.env"; [ -f "$_shellgpt_env" ] && grep -q "^GITHUB_TOKEN=" "$_shellgpt_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_shellgpt_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${SHELLGPT_HOME:-$HOME/.shellgpt}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else

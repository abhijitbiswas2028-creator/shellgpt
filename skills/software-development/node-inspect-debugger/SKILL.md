...
Driver script (save as `~/.shellgpt/cache/scratch/cdp-debug.js`):
...
```bash
node ~/.shellgpt/cache/scratch/cdp-debug.js
```

ShellGPT-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:

```bash
mkdir -p ~/.shellgpt/cache/scratch/cdp-tools && cd ~/.shellgpt/cache/scratch/cdp-tools && npm i chrome-remote-interface
NODE_PATH=~/.shellgpt/cache/scratch/cdp-tools/node_modules node ~/.shellgpt/cache/scratch/cdp-debug.js
```

## Debugging ShellGPT ui-tui
...
### Debugging a running `shellgpt --tui`
...
require('fs').writeFileSync('~/.shellgpt/cache/scratch/cpu.cpuprofile', JSON.stringify(profile));
// Open ~/.shellgpt/cache/scratch/cpu.cpuprofile in Chrome DevTools → Performance tab
```
...
require('fs').writeFileSync('~/.shellgpt/cache/scratch/heap.heapsnapshot', chunks.join(''));
```
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In ShellGPT, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.

...
Ledger location: `$SHELLGPT_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `SHELLGPT_CITATION_LEDGER`.

## How to Run

```bash
S=~/.shellgpt/skills/research/grounded-citations/scripts/sources.py
...
`shellgpt skills install official/social-media/reddit-reading` or
`shellgpt skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `SHELLGPT_CITATION_LEDGER`) if their

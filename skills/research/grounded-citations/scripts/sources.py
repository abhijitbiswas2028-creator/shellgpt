...
Ledger path resolution (first wins):
  --ledger PATH
  $SHELLGPT_CITATION_LEDGER
  $SHELLGPT_HOME/cache/citations/ledger.json
"""
...
from _shellgpt_home import get_shellgpt_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("SHELLGPT_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_shellgpt_home() / "cache" / "citations" / "ledger.json"

...
from shellgpt_constants import OPENROUTER_BASE_URL, get_shellgpt_home
from agent.retry_utils import jittered_backoff
from shellgpt_cli.env_loader import load_shellgpt_dotenv

# Load .env from SHELLGPT_HOME first, then project root as a dev fallback.
load_shellgpt_dotenv(shellgpt_home=get_shellgpt_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: shellgpt setup")

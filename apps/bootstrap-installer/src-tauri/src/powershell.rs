...
/// read side does not see EOF until the last of them closes it. `shellgpt update`
...
/// stranded with it.
///
/// Same bound `Invoke-ShellGPTStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `shellgpt_home_override` propagates to the child as $SHELLGPT_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, shellgpt_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = shellgpt_home_override {
        cmd.env("SHELLGPT_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, shellgpt_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = shellgpt_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_shellgpt_home() {
    ...
    /// child. `shellgpt update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]

...
//!
//! Mirrors `shellgpt_constants.get_shellgpt_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\shellgpt
//!   macOS:   ~/.shellgpt
//!   Linux:   ~/.shellgpt  (override via $SHELLGPT_HOME)
//!
//! NOTE (macOS): Python's get_shellgpt_home(), scripts/install.sh, and the
//! Electron desktop's resolveShellGPTHome() ALL use ~/.shellgpt on macOS — there
...
/// Returns the canonical ShellGPT home directory, respecting $SHELLGPT_HOME if set.
pub fn shellgpt_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SHELLGPT_HOME") {
    ...
    {
        // %LOCALAPPDATA%\shellgpt — matches scripts/install.ps1's $ShellGPTHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("shellgpt");
        }
    }

    // macOS + Linux + fallback: ~/.shellgpt (matches Python get_shellgpt_home(),
    // install.sh, and the Electron desktop's resolveShellGPTHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".shellgpt");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".shellgpt")
}

pub fn log_dir() -> PathBuf {
    shellgpt_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    shellgpt_home().join("bootstrap-cache")
}
...
/// SHELLGPT_HOME so it survives repo checkout deletion (unlike anything under
/// shellgpt-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\shellgpt\shellgpt-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_shellgpt`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SHELLGPT_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SHELLGPT_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    shellgpt_home().join(".shellgpt-update-in-progress")
}
...
pub fn copy_self_to_shellgpt_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SHELLGPT_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SHELLGPT_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".shellgpt-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SHELLGPT_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[shellgpt-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SHELLGPT_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_shellgpt_home() -> String {
    shellgpt_home().to_string_lossy().into_owned()
}

...
export const $shellgptHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.shellgpt/logs/bootstrap-installer.log')
    $shellgptHome.set('~/.shellgpt')
  ...
  try {
    const [logPath, shellgptHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_shellgpt_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $shellgptHome.set(shellgptHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch ShellGPT" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (ShellGPT-Setup.exe --update);
...
export async function launchShellGPTDesktop(): Promise<void> {
  ...
  await invoke('launch_shellgpt_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'ShellGPT repository' },

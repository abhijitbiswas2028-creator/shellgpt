/** ShellGPT' reasoning levels, in ascending order — mirrors the backend's
 *  VALID_REASONING_EFFORTS (shellgpt_constants.py). `none` is not a level: it's
 *  thinking disabled. */
...
/** ShellGPT' built-in level when neither the surface nor the profile config

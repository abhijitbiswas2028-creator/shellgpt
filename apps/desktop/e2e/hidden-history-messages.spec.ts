...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.shellgptHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.shellgptHome)
  const builder = await RealSessionBuilder.start(sandbox.shellgptHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.shellgptHome, mock.url)
  fs.appendFileSync(path.join(sandbox.shellgptHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')

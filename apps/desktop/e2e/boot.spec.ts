 ...
 * packaged binary) with a real `shellgpt serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → shellgpt serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { shellgptDesktop?: Record<string, unknown> }).shellgptDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → shellgpt serve is spawned → WS connects → config

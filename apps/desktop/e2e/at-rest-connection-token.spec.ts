 ...
 * dir (macOS `~/Library/Application Support/ShellGPT/connection.json`, Windows
 * `AppData\Roaming\ShellGPT\connection.json`). Anything that can read the file
 ...
 *      or in SHELLGPT_HOME (logs included).
 ...
 * `X-ShellGPT-Session-Token` header it receives — a dropped or mangled token
 ...
 */
const SENTINEL_TOKEN = 'shellgpt-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'ShellGPTE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-ShellGPT-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote ShellGPT gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-shellgpt-session-token']
 ...
 * backend, so we skip spawning `shellgpt serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startShellGPT() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    SHELLGPT_DESKTOP_APP_NAME: STABLE_APP_NAME,
    SHELLGPT_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { shellgptDesktop?: Record<string, unknown> }).shellgptDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture ShellGPT should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { shellgptDesktop: any }).shellgptDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('with keychain encryption opted IN, a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // SHELLGPT_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.shellgptHome, needles),
      'the gateway session token leaked into a SHELLGPT_HOME file (logs included)',
    ).toEqual([])

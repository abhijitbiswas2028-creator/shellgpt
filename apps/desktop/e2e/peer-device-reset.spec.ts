 ...
 * Requires the built desktop and a repo venv (or SHELLGPT_DESKTOP_PYTHON).
 */
...
function pythonBinary(): string {
  if (process.env.SHELLGPT_DESKTOP_PYTHON) {
    return process.env.SHELLGPT_DESKTOP_PYTHON
  }
  ...
  if (!candidate) {
    throw new Error('Create the repo Python venv or set SHELLGPT_DESKTOP_PYTHON before running this spec')
  }
...
function isolatedEnv(sandbox: Sandbox): Record<string, string> {
  ...
  for (const key of ['SHELLGPT_DESKTOP_USER_DATA_DIR', 'SHELLGPT_DESKTOP_IGNORE_EXISTING', 'SHELLGPT_DESKTOP_SHELLGPT_ROOT', 'SHELLGPT_DESKTOP_APP_NAME', 'SHELLGPT_DESKTOP_SKIP_QUIT_CONFIRM']) {
    env[key] = defaults[key]
  }
  ...
  return {
    ...
    SHELLGPT_DESKTOP_PYTHON: pythonBinary(),
...
const peerTest = test.extend<{ gateways: { app: ElectronApplication; source: Page } }>({
  ...
  gateways: async ({}, runTest, info) => {
    ...
    try {
      const remoteProfileHome = path.join(remote.shellgptHome, 'profiles', REMOTE_PROFILE)
      fs.mkdirSync(remoteProfileHome, { recursive: true })

      for (const shellgptHome of [local.shellgptHome, remote.shellgptHome, remoteProfileHome]) {
        writeMockProviderConfig(shellgptHome, mock.url)
        writeEnvFile(shellgptHome)
      }

      const port = await freePort()
      const remoteUrl = `http://127.0.0.1:${port}`
      child = spawn(pythonBinary(), ['-m', 'shellgpt_cli.main', 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
        cwd: REPO_ROOT,
        detached: process.platform !== 'win32',
        env: { ...isolatedEnv(remote), SHELLGPT_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN },

 ...
 * second gateway, "Homelab", is a REAL second `shellgpt serve` this spec spawns
 * with its own SHELLGPT_HOME, profiles and session token, registered in the v2
...
function findShellGPTBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'shellgpt')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['shellgpt'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('shellgpt binary not found: create the repo venv (uv sync) or put shellgpt on PATH')
}
...
/**
 * Spawn a second, fully real `shellgpt serve` as the remote gateway. Its
 * session token is pinned through SHELLGPT_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        SHELLGPT_HOME: home,
        SHELLGPT_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote shellgpt serve exited early (${child.exitCode}):\n${log}`)
    }
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote shellgpt serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.shellgptHome, ['research'])

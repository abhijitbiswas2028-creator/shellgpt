 ...
 * AIAgent loop, using the E2E mock provider configured in `shellgptHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(shellgptHome: string) {
  ...
  static async start(shellgptHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(shellgptHome)

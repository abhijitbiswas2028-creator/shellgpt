 ...
 * read is parked in main (ipcMain 'shellgpt:api' wrapper) until the test opens
    ...
    try {
...

      {
        const wrapped = await app.evaluate(({ ipcMain }, b) => {
          ...
          const original = handlers?.get('shellgpt:api')

          if (!original) {
            return false
          }

          ipcMain.removeHandler('shellgpt:api')
          ipcMain.handle('shellgpt:api', async (event: any, request: any) => {
        ...
        expect(wrapped, 'shellgpt:api handler wrapped for gating').toBe(true)
      }

 ...
 *
 * Real Electron + real `shellgpt serve`; only the LLM is faked. Process facts
 * come from a /proc census of every process carrying this sandbox's
 * SHELLGPT_HOME (so an orphan reparented to init is still counted), sampled
 ...
 *  4. relaunch the same SHELLGPT_HOME repeatedly: each boot has exactly one
...
/** Every live process whose command line carries `tag` (tool children may scrub SHELLGPT_HOME). */

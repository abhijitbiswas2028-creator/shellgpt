...
function sessionRows(shellgptHome: string, sid: string): Row[] {
  const db = new DatabaseSync(path.join(shellgptHome, 'state.db'), { readOnly: true })
...
test('an acknowledged redirect prompt renders once after in-place compaction inside its turn (#121088)', async () => {
  ...
  writeProviderHome(sandbox.shellgptHome, provider.url, compressionYaml(250_000))
  ...
  try {
    ...
    writeProviderHome(sandbox.shellgptHome, provider.url, compressionYaml(base + HEADROOM_TOKENS))
    ...
    const agentLog = fs.readFileSync(path.join(sandbox.shellgptHome, 'logs', 'agent.log'), 'utf8')
    ...
    const active = sessionRows(sandbox.shellgptHome, sid).filter(r => r.active === 1)
    ...
    const all = sessionRows(sandbox.shellgptHome, sid)

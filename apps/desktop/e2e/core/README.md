...
- **C2 transcript integrity** — `transcript-integrity.spec.ts`: one real app +
  one real `shellgpt serve`, only the LLM faked (`provider.ts`, scripted per turn
...
- **C5 boot / process lifecycle** — `boot-lifecycle.spec.ts`: interactive
  ...
  child → zero sandbox processes (/proc census on the sandbox `SHELLGPT_HOME`,
...
- **Session lineage** — `lineage-sidebar.spec.ts`: a branch child is born
  ...
  compression rows (`shellgpt chat -q` with `compression.in_place: false` on the
  same `SHELLGPT_HOME`, compacting against the fake provider), then the Desktop
...
- **Remote topology** — `remote-topology.spec.ts`: the whole app on a remote
  `shellgpt serve` (`SHELLGPT_DESKTOP_REMOTE_URL` + token): a client-only image is
...
- **Packaged build** — `packaged-smoke.spec.ts`: asarUnpack contract of the
  ...
  `SHELLGPT_E2E_REQUIRE_PACKAGED=1`.
...
sandboxed `HOME`/`SHELLGPT_HOME`/user-data per test, all `SHELLGPT_*` and
...
`SHELLGPT_E2E_CORE_ROOT` picks the sandbox parent dir (default: OS tmpdir);
`SHELLGPT_E2E_CORE_KEEP=1` keeps sandboxes for post-mortem.

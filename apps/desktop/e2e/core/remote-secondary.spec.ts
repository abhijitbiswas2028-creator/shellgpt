 ...
 * runs its OWN local primary backend (Electron-spawned `shellgpt serve`, the
 * default profile only) and has a second, REMOTE connection registered in
 * connections.json — a real `shellgpt serve` the test spawns under a different
 * HOME/SHELLGPT_HOME (./remote-helpers.ts) that hosts the bot profile. The user
...
test('bot on a remote secondary connection: a client-only image reaches it as bytes, not a client path (#120730)', async () => {
  ...
  writeProviderHome(clientBox.shellgptHome, provider.url, IMAGE_CONFIG)
  writeProviderHome(remoteBox.shellgptHome, provider.url, IMAGE_CONFIG)
  const botHome = path.join(remoteBox.shellgptHome, 'profiles', BOT)
  ...
  try {
    ...
    await test.step('the remote backend ran the turn and the model saw the image', async () => {
      ...
      const at = userText.indexOf(remoteBox.shellgptHome)

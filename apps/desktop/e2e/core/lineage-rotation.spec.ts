/**
 * #121148 with REAL rotated session rows: one Electron app + one `shellgpt serve`,
...
test('lineage: a real compression rotation is one sidebar row per conversation (#121148)', async () => {
  ...
  try {
    ...
    await page.evaluate(() => localStorage.setItem('shellgpt.desktop.agentsGroupedByWorkspace', 'true'))
    await page.reload()
    await waitForInteractive(app, page)

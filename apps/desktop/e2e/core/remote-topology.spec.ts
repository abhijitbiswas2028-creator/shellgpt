 ...
 * `shellgpt serve` it did NOT spawn, running under a different HOME and
...
test('remote backend: first chat, image bytes not client paths, rename across a backend restart', async () => {
  ...
  writeProviderHome(backendBox.shellgptHome, provider.url, 'agent:\n  image_input_mode: native\n')
  ...
  try {
    ...
    await test.step('first chat goes to the remote backend, never a local one', async () => {
      ...
      expect(fs.existsSync(path.join(clientBox.shellgptHome, 'state.db')), 'no client-side state.db').toBe(false)
    })
    ...
    await test.step('an image that exists only on the client reaches the model as bytes (#120730)', async () => {
      ...
      const at = userText.indexOf(backendBox.shellgptHome)

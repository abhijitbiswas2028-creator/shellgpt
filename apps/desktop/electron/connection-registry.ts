 ...
 * runtime, remote gateways, ShellGPT Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@shellgpt/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteShellGPTPath || '').trim() !== String(activeSsh.remoteShellGPTPath || '').trim() ||
...
/** Turn `ls ~/.shellgpt/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. ShellGPT Cloud
 * instances are platform-managed — we never run `shellgpt update` against them.
 ...
 */
/**
 * Auth mode a stored remote-shaped entry actually uses. A ShellGPT Cloud gateway
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteShellGPTPath: input.remoteShellGPTPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/ShellGPT-managed names; an empty result stores nothing.
 ...
 * `remoteShellGPTPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'ShellGPT Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'ShellGPT Cloud' : 'Remote gateway')

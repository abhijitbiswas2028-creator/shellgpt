...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    shellgptHome: 'C:\\Users\\andre\\.shellgpt',
    python: 'C:\\Users\\andre\\.shellgpt\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.shellgpt-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      shellgptPath: 'C:\\ShellGPT\\shellgpt.exe',
      shellgptHome: 'C:\\Users\\andre\\.shellgpt',
...
test('every emitted PowerShell script keeps try blocks attached to their catch/finally handlers', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(
    ...
    'C:\\Users\\alice\\.shellgpt'
  )
  scripts.push(
    decode(atomicWindowsSpawnCommand({ shellgptHome: 'C:\\Users\\alice\\.shellgpt', python: 'C:\\py\\python.exe' })),
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command shellgpt.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          shellgptHome: 'C:\\Users\\alice\\.shellgpt',
          shellgptPath: 'C:\\ShellGPT\\shellgpt.exe',
          python: 'C:\\ShellGPT\\python.exe'
        })
      }

      if (script.includes('.shellgpt-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('shellgpt_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.shellgpt\\profiles\\research')
  assert.match(script, /\.shellgpt-update-in-progress/)
...
test('Windows probe validates ShellGPT and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  const fallbackJoin = script.indexOf('Join-Path $shellgptHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('ShellGPT is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /ShellGPT is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\ShellGPT's\\python.exe" }, 'inspect', [
    'C:\\x y\\shellgpt.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'shellgpt_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /ShellGPT''s/)
  assert.match(script, /C:\\x y\\shellgpt\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { shellgptPath: lock.shellgptPath, shellgptHome: lock.shellgptHome }

...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/shellgpt', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/shellgpt/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/shellgpt'), 'https://gw.example.com/shellgpt/auth/native/refresh')
})

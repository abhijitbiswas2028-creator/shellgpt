 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `shellgpt update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.shellgptHome, 'ShellGPT home')
  const shellgptPath = validateRemoteValue(target.shellgptPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(shellgptPath)

  const updateCommand =
    `env SHELLGPT_HOME=${homeWord} ` +
    `SHELLGPT_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'SHELLGPT_UPDATE_ORIGIN_PROFILE=default ' +
    `SHELLGPT_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `SHELLGPT_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:SHELLGPT_HOME=${psLiteral(home)}`,
    `$env:SHELLGPT_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:SHELLGPT_UPDATE_ORIGIN_PROFILE="default"',
    `$env:SHELLGPT_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:SHELLGPT_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:SHELLGPT_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:SHELLGPT_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:SHELLGPT_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(shellgptPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.shellgpt-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote ShellGPT updated and every managed SSH profile is ready.'
        : restoreOk

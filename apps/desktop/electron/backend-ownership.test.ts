...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'shellgpt serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/shellgpt serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m shellgpt_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/shellgpt --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\ShellGPT Runtime\\shellgpt.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('shellgpt chat --query serve'), false)

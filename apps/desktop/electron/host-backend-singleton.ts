// Multiplex-only, Desktop half: ONE `shellgpt serve` per HOST serves every
...
// other producer of `shellgpt serve` children: the per-profile backend pool.
...
//   * `SHELLGPT_DESKTOP_ISOLATED_BACKEND=1` — the escape hatch that gives this
...
export interface HostBackendCollapseOptions {
  /** `SHELLGPT_DESKTOP_ISOLATED_BACKEND=1`: this app wants its own backend. */
  isolated?: boolean
   ...
   * own `SHELLGPT_HOME` is then the only scope there is, so the collapse must
...
export class SecondLocalBackendError extends Error {
  ...
  constructor(poolKey: string) {
    super(
      `Refusing to start a second local ShellGPT backend for "${poolKey}": one backend serves every profile on this host. ` +
        'Set SHELLGPT_DESKTOP_ISOLATED_BACKEND=1 for a private backend.'
    )
 ...
 * the one place a local `shellgpt serve` child is actually started, so a future

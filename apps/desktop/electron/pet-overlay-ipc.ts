...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('shellgpt:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('shellgpt:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('shellgpt:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('shellgpt:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('shellgpt:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('shellgpt:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('shellgpt:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('shellgpt:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('shellgpt:pet-overlay:control', payload)
  })
}

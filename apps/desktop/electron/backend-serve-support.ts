...
// spawns `shellgpt serve`; runtimes older than serve only have `dashboard`, so
...
// (covers a bare `shellgpt` resolved from PATH with no known source root). Result
...
export function createBackendServeSupportResolver(shellgptHome: string, rememberLog: (message: string) => void) {
    ...
    const pending = (async () => {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'shellgpt_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          // `shellgpt --version` (~10.5s measured cold), and a false negative here
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, SHELLGPT_HOME: shellgptHome, ...(backend.env || {}) },
            timeout: PROBE_TIMEOUT_MS,
            stdio: 'ignore',
            // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
            // (see resolveShellGPTBackend step 4); execFileSync of a .cmd without

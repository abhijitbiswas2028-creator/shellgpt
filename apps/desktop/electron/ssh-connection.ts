...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `shellgpt --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SHELLGPT_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-ShellGPT-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.shellgpt/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'shellgpt-desktop-ssh')
  }

  return path.join(os.homedir(), '.shellgpt', 'desktop-ssh')
}

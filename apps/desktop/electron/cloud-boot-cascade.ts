 ...
 *
 * Pure decision seam for self-healing a ShellGPT Cloud agent connection at boot.
 *
 ...
 * settings UI ("Use gateway"). The boot path went straight to `waitForShellGPT`,
...
/**
 * True when a failed `waitForShellGPT` for `remote` should be followed by one
 * silent per-agent sign-in and a single retry, rather than surfacing the
 * reauth error immediately. Requires all of:
 *
 *   - the connection is a ShellGPT Cloud agent (`remoteKind: 'cloud'`);

...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'SHELLGPT_DESKTOP_REMOTE_URL is set but SHELLGPT_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote ShellGPT backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `shellgpt serve` the desktop never

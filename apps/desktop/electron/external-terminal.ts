// Launching the ShellGPT TUI in the user's OWN terminal emulator.
//
...
// `shellgpt --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's ShellGPT runtime is often a venv Python invoked
//     as `python -m shellgpt_cli.main`, not a `shellgpt` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The ShellGPT
...
export function terminalScriptEnv(
  ...
  if (shellgptHome) {
    out.SHELLGPT_HOME = shellgptHome
  }

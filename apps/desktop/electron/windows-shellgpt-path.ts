/**
 * windows-shellgpt-path.ts
 *
 * Pure, dependency-injected pieces of Windows `shellgpt` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvShellGPTCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `shellgpt` shim shadowed the real
 *      shellgpt.cmd/shellgpt.exe; the shim then failed the --version probe and
 ...
 *      and ShellGPT launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvShellGPTCommand() — unwrapWindowsVenvShellGPTCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `shellgpt`) must
 * not shadow `shellgpt.cmd`/`shellgpt.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the ShellGPT launcher that drives `shellgpt update`.
...
export interface ResolveVenvShellGPTCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportShellGPTCli: (python: string, opts?: { env?: Record<string, string> }) => Promise<boolean>
...
/**
 * If `command` is a Windows venv `shellgpt`/`shellgpt.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/shellgpt(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m shellgpt_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportShellGPTCli(). A
 * venv whose update died mid-`pip install` still has python.exe + shellgpt.exe
 ...
 *
 * Returns null when `command` is not a venv shellgpt shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export async function resolveVenvShellGPTCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvShellGPTCommandDeps
): Promise<{
  label: string
  command: string
  args: string[]
  ...
  if (!/^shellgpt(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    !(await canImportShellGPTCli(python, {
      env: {
    ...
    rememberLog?.(
      `Ignoring venv ShellGPT at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing ShellGPT Python at ${python}`,
    command: python,

 ...
 * Opt in with SHELLGPT_TEST_REAL_SERVE=1; see pool-retirement-live-fixture/README.md.
 */
...
function isolatedEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    HOME: root,
    USERPROFILE: root,
    SHELLGPT_HOME: join(root, '.shellgpt'),
    ...
    SHELLGPT_DESKTOP_USER_DATA_DIR: join(root, 'user-data')
  }
}
...
test.skipIf(process.env.SHELLGPT_TEST_REAL_SERVE !== '1' || process.platform === 'win32')(
  'native retirement preserves backend-only cron and hands both waiters capacity only after real child exit',
  async () => {
    const python = process.env.SHELLGPT_TEST_PYTHON
    assert.ok(python && existsSync(python), 'Set SHELLGPT_TEST_PYTHON to an installed ShellGPT Python environment')
    const electron = process.env.SHELLGPT_TEST_ELECTRON || (require('electron') as string)
    assert.ok(existsSync(electron), 'SHELLGPT_TEST_ELECTRON must name a real native Electron executable')
    const root = mkdtempSync(join(tmpdir(), 'shellgpt-pool-retirement-live-'))
    const resultPath = join(root, 'result.json')
    let child: ChildProcess | undefined
    let output = ''

    try {
      mkdirSync(join(root, '.shellgpt'))
      ...
      await build({
        ...
        platform: 'browser',
        format: 'iife',
        alias: { '@': join(desktop, 'src'), '@shellgpt/shared': join(repo, 'apps/shared/src') },

...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', SHELLGPT_HOME: '/home/b/.shellgpt' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'shellgpt_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.shellgpt/shellgpt-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.shellgpt/shellgpt-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.shellgpt/shellgpt-agent'`,
      `exec '/home/b/.shellgpt/shellgpt-agent/venv/bin/python' '-m' 'shellgpt_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\shellgpt\\venv\\Scripts\\shellgpt.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\shellgpt\\venv\\Scripts\\shellgpt.exe" "--tui" "--resume" "sess"',
    ''
  ])
})

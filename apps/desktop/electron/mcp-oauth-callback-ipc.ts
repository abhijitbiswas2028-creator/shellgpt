 ...
 *   - the browser sees only a minimal "return to ShellGPT" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to ShellGPT.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('shellgpt:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('shellgpt:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('shellgpt:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))

// Profile-delete routing logic for the `shellgpt:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_shellgpt_home() and
...
/** Parse a `shellgpt:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SHELLGPT_HOME.
 *
...
/**
 * Route the next `shellgpt:api` request away from the primary/window backend
 ...
 * ensure_shellgpt_home() recreates the directory the delete just removed.
 */

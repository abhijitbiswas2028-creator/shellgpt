...
const READY_RE = /^SHELLGPT_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitShellGPTPath = '') {
  const explicit = psLiteral(explicitShellGPTPath)
  ...
  const script = [
    ...
    '$shellgptHome=$env:SHELLGPT_HOME',
    'if(-not $shellgptHome){$shellgptHome=Join-Path $env:LOCALAPPDATA "shellgpt"}',
    'Assert-NoReparse $shellgptHome $true',
    '$candidate=[IO.Path]::Combine($shellgptHome, "shellgpt-agent\\venv\\Scripts\\shellgpt.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "shellgpt-agent\\.venv\\Scripts\\shellgpt.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $shellgptHome "shellgpt-agent\\venv\\Scripts\\shellgpt.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "shellgpt-agent\\.venv\\Scripts\\shellgpt.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command shellgpt.exe -ErrorAction SilentlyContinue',
    ...
    '$shellgpt=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$shellgpt=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $shellgpt){throw "ShellGPT is not installed on the remote Windows host."}',
    'Assert-NoReparse $shellgpt $false',
    'if($explicit -and $shellgpt -ne $explicit){throw "The configured ShellGPT path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($shellgpt), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;shellgptHome=$shellgptHome;shellgptPath=$shellgpt;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(shellgptHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class ShellGPTMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$shellgptHome=${psLiteral(shellgptHome)}`,
    '$installRoot=$shellgptHome',
    '$parent=Split-Path -Parent $shellgptHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".shellgpt-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[ShellGPTMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, shellgptHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(shellgptHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitShellGPTPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitShellGPTPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'shellgpt_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'shellgpt_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'shellgpt_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$shellgptHome=${psLiteral(runtime.shellgptHome)}`,
    '$installRoot=$shellgptHome',
    '$parent=Split-Path -Parent $shellgptHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".shellgpt-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};shellgptPath=${psLiteral(reservation.shellgptPath)};shellgptHome=${psLiteral(reservation.shellgptHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.shellgptPath === 'string' &&
    typeof lock.shellgptHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.shellgptPath === runtime.shellgptPath &&
    lock.shellgptHome === runtime.shellgptHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteShellGPTPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.shellgptHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.shellgptPath])

  if (!inspection.supported) {
    const error: any = new Error('Update ShellGPT on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.shellgptPath = inspection.path
  const shellgptVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located shellgpt at ${runtime.shellgptPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, shellgptPath: runtime.shellgptPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForShellGPT(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="ShellGPT SSH"', 'powershell.exe -NoLogo')

 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote ShellGPT dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the ShellGPT install on the remote (login-shell probe),
 ...
 *     `SHELLGPT_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForShellGPT in.
 *
 * The minted SHELLGPT_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.shellgpt/desktop-ssh'
...
// Resolve the remote shellgpt executable. An EXPLICIT path is honored strictly
...
async function locateShellGPT(ssh, remoteShellGPTPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The shellgpt binary or wrapper script
    ...
    //     the ShellGPT version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/NousResearch/shellgpt-agent/issues/74411
    return candidate
  }
  ...
  if (remoteShellGPTPath) {
    if (await isExecutable(remoteShellGPTPath)) {
      return resolveLauncher(remoteShellGPTPath)
    }

    const err: any = new Error(
      `The ShellGPT path you set is not an executable on the remote host: "${remoteShellGPTPath}". ` +
        'Check the path (it must be the full path to the `shellgpt` binary on the remote, e.g. ' +
        '~/shellgpt-agent/.venv/bin/shellgpt), or clear it to auto-detect.'
    )

    err.kind = 'shellgpt-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v shellgpt')}`)).trim()
  ...
  candidates.push('~/.local/bin/shellgpt')
  candidates.push('/usr/local/bin/shellgpt')
  candidates.push('~/.shellgpt/shellgpt-agent/venv/bin/shellgpt')
  ...
  const err: any = new Error(
    'ShellGPT is not installed on the remote host (could not find a `shellgpt` executable). ' +
      'Install it on the remote with:  curl -fsSL https://shellgpt-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the ShellGPT path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<shellgpt> --version`,
// e.g. "ShellGPT Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH shellgpt a
// connection uses, so a stale/unexpected install is visible.
async function probeShellGPTVersion(ssh, shellgptPath) {
  try {
    // Watchdogged: a hung remote CLI must die remotely instead of orphaning
    // when the local ssh child is SIGKILLed (#110478).
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(shellgptPath)} --version 2>&1`))).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". ShellGPT Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The SHELLGPT_HOME the remote dashboard will use (explicit env wins, else
// ~/.shellgpt). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteShellGPTHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${SHELLGPT_HOME:-$HOME/.shellgpt}"')).trim().split('\n').pop()

    return out || '~/.shellgpt'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote ShellGPT home.')
...
marker=home/'.shellgpt-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing ShellGPT checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, shellgptHome) {
  const home = assertSafeRemoteHome(shellgptHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote ShellGPT install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote ShellGPT update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote ShellGPT update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteShellGPTProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteShellGPTHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote ShellGPT profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
async function readRemoteInstallId(ssh) {
  // The stable backend identity the roster collapses on (`shellgpt_cli/install_identity.py`:
  ...
  const root = remoteInstallRoot(assertSafeRemoteHome(await probeRemoteShellGPTHome(ssh)))
  ...
  try {
    ...
    const error: any = new Error('Could not read the remote ShellGPT install id.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote ShellGPT home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'shellgptPath', 'shellgptHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !shellgptPath) {
    return false
  }
  ...
  const script =
    ...
    `pid=${Number(pid)}\n` +
    `expected=os.path.expanduser(${shq(shellgptPath)})\n` +
    // The installer-facing launcher is intentionally preserved for invocation
    // (#74411), but it may `exec python <install-dir>/shellgpt`, leaving neither
    // launcher nor SHELLGPT_HOME-derived entrypoint in argv. The ownership-scoped
    // token path + random nonce + exact profile below are the alternative proof.
    `shellgpt_home=os.path.expanduser(${shq(shellgptHome)}) if ${shq(shellgptHome)} else ""\n` +
    'expected_entries={expected}\n' +
    'if shellgpt_home:\n' +
    ' expected_entries.add(os.path.join(shellgpt_home,"shellgpt-agent","venv","bin","shellgpt"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.shellgptPath)
  const expectedHome = lock.shellgptHome ? expandRemotePath(lock.shellgptHome) : "''"
  ...
  const executableMatch = lock.shellgptHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.shellgptPath === expected.shellgptPath &&
    lock.shellgptHome === expected.shellgptHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.shellgptPath)})
shellgpt_home=os.path.expanduser(${py(lock.shellgptHome)})
expected_entries={expected_path,os.path.join(shellgpt_home,"shellgpt-agent","venv","bin","shellgpt")}
...
// execing ShellGPT. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"shellgpt-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(shellgptPath, profile, opts: any = {}) {
  const shellgpt = expandRemotePath(shellgptPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.shellgptHome || '~/.shellgpt')}/.shellgpt-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.shellgptHome || '~/.shellgpt')}/.shellgpt-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env SHELLGPT_DESKTOP=1${opts.guestOnboarding === true ? ' SHELLGPT_GUEST_ONBOARDING=1' : ''} ${shellgpt} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} shellgpt-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, shellgptPath) {
  const shellgpt = expandRemotePath(shellgptPath)
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${shellgpt} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, shellgptPath))) {
    const err: any = new Error(
      'The remote ShellGPT install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update ShellGPT on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(shellgptPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        shellgptHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForShellGPT, a token-gated
...
async function connect(deps) {
  ...
  const shellgptHome = await probeRemoteShellGPTHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, shellgptHome)
  const shellgptPath = await locateShellGPT(ssh, remoteShellGPTPath)
  log(`located shellgpt at ${shellgptPath}`)
  const shellgptVersion = await probeShellGPTVersion(ssh, shellgptPath)

  if (shellgptVersion) {
    log(`remote shellgpt version: ${shellgptVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this ShellGPT Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.shellgptPath === shellgptPath &&
      lock.shellgptHome === shellgptHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    shellgptHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, shellgptHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForShellGPT(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)

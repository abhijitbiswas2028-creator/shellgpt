...
export const ZOOM_STORAGE_KEY = 'shellgpt:desktop:zoomLevel'
 ...
 * but forgot to emit 'shellgpt:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('shellgpt:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `shellgpt:zoom:changed`, which never fired —

 ...
 *
 * So we delegate to `shellgpt gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `shellgpt update`'s
...
export function stopGatewayBeforeUpdate(
  shellgptCliPath: string,
  shellgptHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(shellgptCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(shellgptCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(shellgptCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(shellgptCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(shellgptCliPath, args)
  }

  if (!existsSync(shellgptCliPath)) {
    return false
  }

  try {
    exec(shellgptCliPath, args, {

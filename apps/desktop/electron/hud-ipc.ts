...
export function registerHudIpc({
  ...
  ipcMain.on('shellgpt:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('shellgpt:hud:windowing', event => {
  ...
  ipcMain.on('shellgpt:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('shellgpt:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('shellgpt:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('shellgpt:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('shellgpt:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('shellgpt:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('shellgpt:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('shellgpt:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('shellgpt:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('shellgpt:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('shellgpt:hud:close', async () => {
    closeHudWindow()

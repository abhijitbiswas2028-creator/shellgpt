 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `shellgpt update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `shellgpt desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = escapeHtml(details.title || 'ShellGPT couldn\u2019t start the desktop UI')
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>shellgpt desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>shellgpt desktop --force-build</code>, then restart the app.</p>

...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startShellGPT() boots the primary as the preference at that moment.
  ...
  // shellgpt:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same SHELLGPT_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )

 ...
 * console. It writes SHELLGPT_HOME/.shellgpt-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen ShellGPT within 30 minutes.
...
export function handoffResultPath(shellgptHome: string): string {
  return path.join(shellgptHome, '.shellgpt-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(shellgptHome)
  let raw: string

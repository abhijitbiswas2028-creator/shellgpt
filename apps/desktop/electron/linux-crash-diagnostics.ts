...
// launcher (.desktop entry, Omarchy's shellgpt-desktop wrapper) discards stderr.
...
export interface CrashDiagnosticsHost {
  /** Create the logs directory. May throw (read-only or invalid SHELLGPT_HOME). */
...
// a read-only or invalid SHELLGPT_HOME/logs must degrade to "no crash log", never

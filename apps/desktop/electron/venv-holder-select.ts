 ...
 *
 * The pre-update handoff kills ShellGPT-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `shellgpt`, unrelated scripts) must NOT be
...
/**
 * True when a process is a ShellGPT-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives ShellGPT and holds venv shims mapped).
 */
export function isShellGPTOwnedVenvDaemon(

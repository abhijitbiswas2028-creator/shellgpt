...
const electron = vi.hoisted(() => ({
  ...
  ipcRenderer: {
    ...
    sendSync: vi.fn((channel: string) => (channel === 'shellgpt:skin:local' ? skin : {}))
  },
  webFrame: {},
  webUtils: {}
}))
...
test('the preload carries the local skin across before the renderer starts', async () => {
  ...
  assert.equal(electron.ipcRenderer.sendSync.mock.calls.some(([channel]) => channel === 'shellgpt:skin:local'), true)
})

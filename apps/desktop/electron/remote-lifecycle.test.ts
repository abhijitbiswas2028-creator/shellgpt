...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    shellgptPath: '~/.local/bin/shellgpt',
    shellgptHome: '~/.shellgpt',
...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.shellgpt-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing ShellGPT', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('SHELLGPT_HOME')) {
          return '/home/alice/.shellgpt\n'
        }

        if (command.includes('.shellgpt-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.shellgpt/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.shellgpt-update-in-progress/)
})
...
test('readRemoteInstallId reads the backend identity without spawning a dashboard or minting one', async () => {
  const ssh = fakeSsh([
    [/SHELLGPT_HOME/, '/Users/zillajr/.shellgpt\n'],
...
test('readRemoteInstallId reports the INSTALL root id for a profile-pinned home', async () => {
  ...
  const pinned = fakeSsh([
    [/SHELLGPT_HOME/, '/Users/zillajr/.shellgpt/profiles/dixie\n'],
    [/cat .*install_id/, '0f8a1c2b3d4e5f60718293a4b5c6d7e8\n']
  ])

  assert.equal(await readRemoteInstallId(pinned), '0f8a1c2b3d4e5f60718293a4b5c6d7e8')
  assert.equal(
    pinned.calls.some(cmd => cmd.includes('/Users/zillajr/.shellgpt/install_id')),
    true
  )
  assert.equal(
...
test('listRemoteShellGPTProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/SHELLGPT_HOME/, '/Users/zillajr/.shellgpt\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteShellGPTProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteShellGPTProfiles rejects a hostile SHELLGPT_HOME', async () => {
  const ssh = fakeSsh([[/SHELLGPT_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteShellGPTProfiles(ssh),
    (err: any) => {
...
test('locateShellGPT prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/shellgpt/, 'OK']])
  assert.equal(await locateShellGPT(ssh, '/opt/shellgpt'), '/opt/shellgpt')
})

test('locateShellGPT throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong shellgpt" bug.
  const ssh = fakeSsh([
    [/command -v shellgpt/, '/home/u/.local/bin/shellgpt\n'],
    [/\[ -x .*\.local\/bin\/shellgpt/, 'OK']
  ])

  await assert.rejects(
    () => locateShellGPT(ssh, '/bad/path/shellgpt'),
    (err: any) => {
      assert.equal(err.kind, 'shellgpt-not-found')
      assert.match(err.message, /\/bad\/path\/shellgpt/)
...
test('locateShellGPT falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v shellgpt/, '/home/u/.local/bin/shellgpt\n'],
    [/\[ -x .*\.local\/bin\/shellgpt/, 'OK']
  ])

  assert.equal(await locateShellGPT(ssh, ''), '/home/u/.local/bin/shellgpt')
})

test('locateShellGPT preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$SHELLGPT_BIN" "$SHELLGPT_ENTRYPOINT" "$@",
  // where $SHELLGPT_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.shellgpt/shellgpt-agent/venv/bin/python\n']
  ])

  assert.equal(await locateShellGPT(ssh, ''), '/home/u/.local/bin/shellgpt')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateShellGPT must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateShellGPT returns an explicit remoteShellGPTPath unchanged', async () => {
  // The override half of #74411: an explicit remoteShellGPTPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/shellgpt changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/shellgpt/, 'OK'],
    [/python3 -c/, '/home/u/.shellgpt/shellgpt-agent/venv/bin/python\n']
  ])

  assert.equal(await locateShellGPT(ssh, '~/.local/bin/shellgpt'), '~/.local/bin/shellgpt')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteShellGPTPath must never be rewritten')
})

test('locateShellGPT falls back to ~/.local/bin/shellgpt when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateShellGPT(ssh, ''), '~/.local/bin/shellgpt')
})

test('locateShellGPT tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/shellgpt/, 'OK']])
  assert.equal(await locateShellGPT(ssh, ''), '~/.shellgpt/shellgpt-agent/venv/bin/shellgpt')
})

test('locateShellGPT throws a shellgpt-not-found error with an install hint', async () => {
...
test('locateShellGPT uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v shellgpt/, '/x/shellgpt'],
    [/\[ -x/, 'OK']
  ])

  await locateShellGPT(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.shellgpt/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.shellgpt/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.shellgpt/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/shellgpt'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/shellgpt serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/shellgpt'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/shellgpt', '/Users/cd9c/.shellgpt', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /shellgpt-agent.*venv.*bin.*shellgpt/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'shellgpt wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'shellgpt')
    const launcher = path.join(temp, 'shellgpt launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/shellgpt-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/shellgpt', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('SHELLGPT_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})

test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'shellgpt-update-mutex-'))
  const shellgptPath = path.join(directory, 'shellgpt')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *shellgpt-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(shellgptPath, '', {
      shellgptHome: path.join(directory, 'home'),
      logPath
    })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('SHELLGPT_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('SHELLGPT_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]SHELLGPT_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { shellgptPath: '/x/shellgpt', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nSHELLGPT_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.shellgpt\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'ShellGPT Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile shellgptPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ shellgptPath: '/old/stale/shellgpt', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteShellGPTPath: '/new/shellgpt', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes shellgptHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/SHELLGPT_HOME/, '/home/alice/.shellgpt\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"shellgptHome":"\/home\/alice\/\.shellgpt"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'SHELLGPT_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/shellgpt'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.shellgpt/shellgpt-agent/venv/bin/shellgpt'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/shellgpt'))
  assert.doesNotThrow(() => validateRemotePath('~/.shellgpt/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/shellgpt'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('shellgpt'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/shellgpt'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/shellgpt\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/shellgpt\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/shellgpt\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/shellgpt', '/usr/`id`/shellgpt', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.shellgpt/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/shellgpt')
  assert.ok(result.includes('/usr/local/bin/shellgpt'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/shellgpt')
...
test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/shellgpt', 'work', {
    tokenFilePath: `~/.shellgpt/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.shellgpt\/desktop-ssh\//)
})
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { shellgptPath: '/x/shellgpt', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { shellgptPath: '/x/shellgpt', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.shellgpt/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.shellgpt/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/shellgpt serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/shellgpt serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/shellgpt'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/shellgpt'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/shellgpt'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/shellgpt'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async t => {
    ...
    const dir = await mkdtemp(path.join(os.tmpdir(), 'shellgpt-zsh-probe-'))

    try {
      const shellgpt = path.join(dir, 'shellgpt')
      await writeFile(shellgpt, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

      const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

      assert.equal(await remoteSupportsSshOwnership(ssh, shellgpt), true)
    } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'ShellGPT Agent v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeShellGPTVersion(versionSsh, '/x/shellgpt'), 'ShellGPT Agent v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/shellgpt'), true)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'shellgpt-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.shellgpt/.shellgpt-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.shellgpt/desktop-ssh/${OWNERSHIP_ID}`

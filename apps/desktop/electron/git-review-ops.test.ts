...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'shellgpt-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'shellgpt-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'ShellGPT Test'], { cwd: dir })

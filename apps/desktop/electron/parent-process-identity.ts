export type ParentWatchdogEnv = {
  SHELLGPT_PARENT_PID: string
  SHELLGPT_PARENT_START_MARKER?: string
  SHELLGPT_PARENT_NONCE?: string
  /** Spawn tag consumed by shellgpt_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SHELLGPT_PARENT_PID: String(pid) }
  env.SHELLGPT_SPAWN = spawnTag(pid, startMarker)
  ...
  env.SHELLGPT_PARENT_START_MARKER = startMarker
  env.SHELLGPT_PARENT_NONCE = nonce

  return env
}

/** Build the `SHELLGPT_SPAWN` tag mirrored by shellgpt_cli/process_identity.py.

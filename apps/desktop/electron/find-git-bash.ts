 ...
 *   1. SHELLGPT_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\shellgpt\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SHELLGPT_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SHELLGPT_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'shellgpt', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'shellgpt', 'git', 'usr', 'bin', 'bash.exe'))
  }

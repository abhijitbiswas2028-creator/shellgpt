// Backend subcommand routing for the desktop-managed ShellGPT process.
//
// The desktop app launches its own headless backend via `shellgpt serve` — it
...
// app hasn't updated yet, or an older `shellgpt` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional ShellGPT profile to pin via `--profile`.
 */
 ...
 * `-m shellgpt_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `shellgpt_cli/subcommands/dashboard.py` source registers

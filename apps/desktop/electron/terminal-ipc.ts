...
// registry, and the shellgpt:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // SHELLGPT_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.SHELLGPT_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip ShellGPT' TUI into
    ...
    env.TERM_PROGRAM = 'ShellGPT'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a shellgpt/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from SHELLGPT_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.SHELLGPT_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `shellgpt:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('shellgpt:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('shellgpt:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('shellgpt:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('shellgpt:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('shellgpt:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('shellgpt:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

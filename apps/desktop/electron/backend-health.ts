...
export interface ShellGPTReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForShellGPTReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote ShellGPT gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForShellGPTReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed ShellGPT Cloud instance
 ...
 * `isReauthRequired` so startShellGPT latches; a bare `needsOauthLogin` (the
...
export async function waitForShellGPTReady(baseUrl: string, options: ShellGPTReadyOptions): Promise<void> {
  ...
  throw new Error(`ShellGPT backend did not become ready: ${detail}`)
}

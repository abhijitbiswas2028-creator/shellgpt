 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of ShellGPT Agent by spawning
...
// Relative, not `@shellgpt/shared/ansi`: the electron bundle is built by esbuild
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.shellgpt-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(shellgptHome) {
  return path.join(shellgptHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.shellgpt/shellgpt-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(shellgptHome) {
  if (!shellgptHome) {
    return null
  }

  const candidate = path.join(shellgptHome, 'shellgpt-agent', 'scripts', installScriptName())
...
function cachedScriptPath(shellgptHome, commit) {
  return path.join(bootstrapCacheDir(shellgptHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/shellgpt-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(shellgptHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(shellgptHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, shellgptHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SHELLGPT_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SHELLGPT_HOME: shellgptHome || process.env.SHELLGPT_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, shellgptHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, shellgptHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--shellgpt-home', shellgptHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, shellgptHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, shellgptHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(shellgptHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, shellgptHome, emit })
    abortSignal?.throwIfAborted()

...
export interface ShellGPTConfigRuntime {
  command?: string
  args?: string[]
...
export async function readPreUpdateBackupEnabled(
  runtime: ShellGPTConfigRuntime | Promise<ShellGPTConfigRuntime>,
  shellgptHome: string,
  run = execFileAsync
): Promise<boolean> {
  try {
    ...
    const result = await run(resolvedRuntime.command, resolvedRuntime.args, {
      encoding: 'utf8',
      env: { ...process.env, ...resolvedRuntime.env, SHELLGPT_HOME: shellgptHome },

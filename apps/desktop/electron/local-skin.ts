...
// Custom skin files are overlays in shellgpt_cli/skin_engine.py. Keep this in
...
/** The profile key mirrors `shellgpt --profile <name>` and is safe for a path. */
...
/** The profile home mirrors `shellgpt --profile <name>`: `<root>/profiles/<name>`. */
export function localSkinHome(shellgptHome: string, profile: null | string): string {
  const name = localSkinProfileKey(profile)

  return name === 'default' ? shellgptHome : path.join(shellgptHome, 'profiles', name)
}

function skinFromFile(filePath: string, configuredName: string): ShellGPTSkin | null {
 ...
 */
export function readLocalDisplaySkin(shellgptHome: string, profile: null | string): ShellGPTSkin | null {
  const home = localSkinHome(shellgptHome, profile)
...
): { profile: string; skin: ShellGPTSkin } | null {
  const profile = localSkinProfileKey(routedProfile ?? fallbackProfile)
  const skin = readLocalDisplaySkin(shellgptHome, profile)

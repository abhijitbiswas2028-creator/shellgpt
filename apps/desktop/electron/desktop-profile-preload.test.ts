...
test('the native preload exposes routed peer opening and default preference events with unsubscribe', async () => {
  ...
  assert.equal(name, 'shellgptDesktop')
  const route = { connectionId: 'remote-work', profile: 'work' }

  await bridge.openWindow(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['shellgpt:window:openInstance', route])
  await bridge.openWindow()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['shellgpt:window:openInstance', undefined])
  await bridge.profile.setDefault(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['shellgpt:profile:default:set', route])
  await bridge.profile.getDefault()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['shellgpt:profile:default:get'])
  bridge.setF12ShortcutActive(true)
  assert.equal(electron.ipcRenderer.send.mock.lastCall?.[0], 'shellgpt:f12ShortcutActive')
  ...
  assert.equal(shortcutChannel, 'shellgpt:f12-shortcut')
  ...
  assert.equal(channel, 'shellgpt:profile:default:changed')
  listener({}, route)
  listener({}, null)

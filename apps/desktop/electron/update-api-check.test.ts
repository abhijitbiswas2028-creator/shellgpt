...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/shellgpt-agent.git'), 'someone/shellgpt-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('nousresearch/shellgpt-agent', 'bb/gui'),
    'https://api.github.com/repos/nousresearch/shellgpt-agent/commits/bb%2Fgui'
  )
})

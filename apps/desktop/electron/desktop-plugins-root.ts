// The standalone desktop-plugin root (`<SHELLGPT_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.shellgpt-package.json` marker
...
export const PACKAGE_MARKER = '.shellgpt-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.shellgpt-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every shellgpt home the app knows about locally: the default plus each profile. */
export async function localHomes(shellgptHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(shellgptHome, 'profiles'))

  return [shellgptHome, ...profiles.map(name => path.join(shellgptHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(shellgptHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(shellgptHome, 'profiles'))) {
    const scopedRoot = path.join(shellgptHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(shellgptHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(shellgptHome)) {

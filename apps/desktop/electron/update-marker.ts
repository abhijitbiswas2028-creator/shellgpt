...
export function markerPath(shellgptHome) {
  return path.join(shellgptHome, '.shellgpt-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(shellgptHome)
  let raw
 ...
 *
 * The Tauri-based shellgpt-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(shellgptHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(shellgptHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * ShellGPT to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(shellgptHome, opts)

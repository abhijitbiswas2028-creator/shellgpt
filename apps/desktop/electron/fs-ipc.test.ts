/**
 * `shellgpt:fs:reveal` answers what it did (#115167). `shell.showItemInFolder`
...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'shellgpt-fs-ipc-'))

registerFsIpc({
  shellgptHome: scratch,
...
const reveal = (target: string) => electron.handlers.get('shellgpt:fs:reveal')!({}, target)

afterEach(() => {
  electron.showItemInFolder.mockClear()
})

describe('shellgpt:fs:reveal', () => {

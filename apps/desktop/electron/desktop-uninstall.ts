 ...
 * the three user-facing uninstall modes to the `shellgpt uninstall` CLI flags,
 ...
 *              `shellgpt uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `shellgpt uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `shellgpt uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `shellgpt` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m shellgpt_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `shellgpt_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'shellgpt_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/ShellGPT.app/Contents/MacOS/ShellGPT  → …/ShellGPT.app
 *   Windows: …\ShellGPT\ShellGPT.exe                 → …\ShellGPT  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/ShellGPT.app/Contents/MacOS/ShellGPT → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/ShellGPT.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs ShellGPT.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]ShellGPT$/i.test(dir) || /[\\/]shellgpt-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/shellgpt
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import shellgpt_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, shellgptHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SHELLGPT_HOME=${q(shellgptHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import shellgpt_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but ShellGPT install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SHELLGPT_HOME=${String(shellgptHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]

...
<p align="center">
  <a href="https://github.com/NousResearch/shellgpt-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [ShellGPT Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other ShellGPT surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to ShellGPT and hear it back.</td></tr>
...
### Install with ShellGPT (recommended)

Already have the ShellGPT CLI? Just run:

```bash
shellgpt desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing ShellGPT gateway or install ShellGPT locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the ShellGPT Desktop website.](https://shellgpt-agent.nousresearch.com/).
...
left and right Command keys together in any app. ShellGPT captures that app's
frontmost window and attaches the image to the last-active ShellGPT composer,
...
```bash
# throwaway SHELLGPT_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the ShellGPT Agent runtime into `SHELLGPT_HOME`
(`~/.shellgpt`, or `%LOCALAPPDATA%\shellgpt` on Windows), using the same layout as a
CLI install.
...
- **ShellGPT Agent** runs as a headless `shellgpt serve` process and exposes the
...
1. `SHELLGPT_DESKTOP_SHELLGPT_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `SHELLGPT_DESKTOP_SHELLGPT`, or `shellgpt` on `PATH`
5. a system Python that can import the ShellGPT runtime
...
Desktop supports a managed local backend, explicit remote gateways, and ShellGPT
...
screen offers **Connect to existing ShellGPT** before starting the local installer.
...
terminal commands, and file operations run against the remote ShellGPT host, not
...
transport- or ShellGPT-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-ShellGPT-Session-Token`.
...
Boot logs land in `SHELLGPT_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\shellgpt\shellgpt-agent\.shellgpt-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\shellgpt\shellgpt-agent\venv"
```

> The default ShellGPT home on Windows is `%LOCALAPPDATA%\shellgpt`. Set the `SHELLGPT_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://shellgpt-agent.nousresearch.com/docs/)

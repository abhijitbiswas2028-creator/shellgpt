...
//   D=$(mktemp -d) && env -u SHELLGPT_SHARED_AUTH_DIR SHELLGPT_GUEST_ONBOARDING=1 SHELLGPT_SKIP_INTRO=1 \
//     SHELLGPT_HOME="$D/home" SHELLGPT_DESKTOP_USER_DATA_DIR="$D/userdata" shellgpt desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'shellgpt-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.SHELLGPT_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set SHELLGPT_DESKTOP_PYTHON to your installed ShellGPT Python executable.')
...
for (const dir of ['.shellgpt/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, SHELLGPT_HOME: path.join(sandbox, '.shellgpt'), SHELLGPT_SHARED_AUTH_DIR: path.join(sandbox, '.shellgpt/shared'),
  SHELLGPT_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), SHELLGPT_DESKTOP_CWD: path.join(sandbox, 'work'),
  SHELLGPT_DESKTOP_SHELLGPT_ROOT: root, SHELLGPT_DESKTOP_PYTHON: python,
  SHELLGPT_DESKTOP_IGNORE_EXISTING: '1', SHELLGPT_DESKTOP_APP_NAME: 'ShellGPT Connector Rehearsal',
  SHELLGPT_GUEST_ONBOARDING: '1', SHELLGPT_SKIP_INTRO: '1', SHELLGPT_DESKTOP_DEV_SERVER: url, SHELLGPT_DESKTOP_CDP_PORT: '9344'
})

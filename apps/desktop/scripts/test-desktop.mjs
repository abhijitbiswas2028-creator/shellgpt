...
// no longer bundles the ShellGPT Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'ShellGPT.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'ShellGPT'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'ShellGPT.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'ShellGPT'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SHELLGPT_HOME for non-sandboxed runs -- matches main.ts's
// resolveShellGPTHome(). On Windows it's %LOCALAPPDATA%\shellgpt; elsewhere
// it's ~/.shellgpt. The fresh-install sandbox launchFresh() sets its own
// SHELLGPT_HOME and never touches this.
const DEFAULT_SHELLGPT_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'shellgpt')
  }
  return path.join(os.homedir(), '.shellgpt')
})()
const VENV_ROOT = path.join(DEFAULT_SHELLGPT_HOME, 'shellgpt-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'shellgpt-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SHELLGPT_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `ShellGPT-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `ShellGPT-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `ShellGPT-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'ShellGPT-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SHELLGPT_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SHELLGPT_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const shellgptHome = path.join(sandbox, 'shellgpt-home')
  ...
  env.SHELLGPT_DESKTOP_CWD = cwd
  env.SHELLGPT_DESKTOP_IGNORE_EXISTING = '1'
  env.SHELLGPT_DESKTOP_TEST_MODE = 'fresh-install'
  env.SHELLGPT_DESKTOP_USER_DATA_DIR = userDataDir
  env.SHELLGPT_HOME = shellgptHome
  delete env.SHELLGPT_DESKTOP_SHELLGPT
  delete env.SHELLGPT_DESKTOP_SHELLGPT_ROOT
  ...
  console.log(`  SHELLGPT_HOME: ${shellgptHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(shellgptHome, 'shellgpt-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The ShellGPT Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of shellgpt_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'shellgpt-agent', 'shellgpt_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing ShellGPT
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SHELLGPT_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SHELLGPT_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}

 ...
 * product name (`ShellGPT`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/ShellGPT'
 *
 * This is a hard failure with no obvious cause for the user — `shellgpt desktop`
 ...
 * (the mac-unpacked ShellGPT.app bundle) and Windows (win-unpacked), so we
 ...
 * ShellGPT.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `shellgpt desktop --build-only` (shellgpt_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'ShellGPT.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'ShellGPT'}.exe`

...
const UPSTREAM_URL = 'https://github.com/NousResearch/shellgpt-agent.git'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.shellgptDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `shellgpt-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const shellgptHome = join(sandbox, 'shellgpt-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(shellgptHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(shellgptHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SHELLGPT_DESKTOP_APP_NAME: `ShellGPTShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SHELLGPT_DESKTOP_SHELLGPT_ROOT: target.targetRoot,
        SHELLGPT_DESKTOP_IGNORE_EXISTING: '1',
        SHELLGPT_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'shellgpt-short-session-ab-'))
  const prepared = []

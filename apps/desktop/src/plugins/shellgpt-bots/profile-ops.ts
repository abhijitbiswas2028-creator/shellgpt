...
import { forgetSessionUnread, host, queryClient } from '@shellgpt/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['shellgpt-bots']
...
/** Permanently delete a bot's ShellGPT profile, then remove plugin-local state
 ...
 * directory (shellgpt-agent#52279). That is the "can't delete a bot" error. */

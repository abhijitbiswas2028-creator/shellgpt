/**
 * ShellGPT Bot Mode — a "one chat per agent" roster for the ShellGPT desktop.
 *
 * Left pane "Bots": one row per ShellGPT profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (ShellGPT cron jobs) scoped to the
 ...
 * bot-initiated sends use `shellgpt -p <bot> chat --in ~ -c "Bot Chat"`.
 */
...
import type { ChatEmptyProps, PluginContext, ProfileGroupRoute } from '@shellgpt/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @shellgpt.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.shellgptDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.shellgptDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is ShellGPT' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['shellgpt-bots']?.title || bot.title || ''
            ).trim()

            // message_agent only resolves canonical identities: the relay
            // matches a roster row's handle/profile (± @connection-id), the
            // local path a bare profile name or 'shellgpt'. botHandle() prefers

...
vi.mock('@shellgpt/plugin-sdk', async () => {
...
async function contributions({
  ...
  // Bots pane does.
  if (profiles) {
    cache.set(JSON.stringify(['shellgpt-bots', 'roster', cacheKeyConnection]), {
      key: ['shellgpt-bots', 'roster', cacheKeyConnection],
      value: { profiles }
    })
  }
...
describe('@-mention completions', () => {
  ...
  it('surfaces default as @shellgpt and prefix-filters on the handle', async () => {
    const { provide } = await contributions({
      focused: 'researcher',
      profiles: [
        { name: 'default' },
    ...
    expect(provide('').map(item => item.insert)).toEqual(expect.arrayContaining(['@shellgpt', '@writer-homelab']))
   ...
   *  picker would surface as a retargeted local @shellgpt. */
  const REMOTE_DEFAULTS: Array<Record<string, unknown>> = [
    {
      ...
      remoteSource: true,
      sourceScoped: true,
      ui_meta: { 'shellgpt-bots': { title: 'CoS Bot' } }
    },
    {
      connectionId: 'wsl',
      connectionLabel: 'WSL',
  ...
  it.each([
    ...
    'lists titled remote defaults under their slug, qualified on collision, and keeps @shellgpt local ($label)',
    async ({ profiles }) => {
      ...
      // the picker pins each to its connection; the local default stays @shellgpt.
      expect(inserts).toEqual(expect.arrayContaining(['@shellgpt', '@cos-bot@vps', '@cos-bot@wsl']))
      expect(inserts).not.toContain('@cos-bot')
      expect(inserts.filter(insert => insert === '@shellgpt')).toHaveLength(1)
      ...
      // A bare @shellgpt is this machine's default in either roster order.
      const local = await handler({ text: '@shellgpt summarize' })
      expect(local.text).toMatch(/@shellgpt = agent profile "default"/)
...
describe('the mention middleware', () => {
  it('identifies a remote @name-device mention without delivering anything', async () => {
    ...
    expect(result.text).not.toMatch(/shellgpt -p '?default/)
  ...
  it('annotates the resolvable handle for a local row whose UI alias differs', async () => {
    ...
    // The local resolver only knows bare profile names / 'shellgpt'.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
    ...
    expect(result.text).toMatch(/message_agent target: "shellgpt"/)
  ...
  it('keeps a poisoned bot title inert prose', async () => {
    ...
    // the invariant that matters is that no shellgpt command is ever emitted.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
        { name: 'ops' },
    ...
    expect(result.text).not.toMatch(/`shellgpt/)
  })
    ...
    hostMock.agents.mockResolvedValueOnce({
      agents: [
        ...
        },
        {
          ...
          handle: 'default-vps',
          profile: 'default',
          profileMetadata: { ui_meta: { 'shellgpt-bots': { title: 'CoS Bot' } } }
        }
      ],
      sources: [

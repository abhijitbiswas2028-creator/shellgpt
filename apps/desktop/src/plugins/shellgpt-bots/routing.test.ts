 ...
 * the raw Cloud hostname, or as generic "ShellGPT" when Cloud was the only
 * source.
 *
...
vi.mock('@shellgpt/plugin-sdk', () => ({ host: hostMock }))
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "ShellGPT"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "ShellGPT".
    expect(displayName({ name: 'default' }, null)).toBe('ShellGPT')
  })

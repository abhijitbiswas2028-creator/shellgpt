 ...
 *
 * A member's hidden per-group session is an ordinary ShellGPT session, so other
 * writers legitimately append to it: the user resuming it from the CLI
 * (`shellgpt -p <bot> chat --resume "Group: <room> · <thread>"`), a cron job, the
...
/** The ShellGPT-authored assistant row that closes a turn which failed before

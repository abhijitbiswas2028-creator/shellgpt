...
import { Button, host, Input, useI18n } from '@shellgpt/plugin-sdk'
...
// https://shellgpt-agent.nousresearch.com/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'shellgpt-skill-pick', ...} to us (shellgpt-agent#86243). We validate
...
export function HubSkillsSection({ bot, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'shellgpt-skill-pick' || !data.name) {
        return
      }

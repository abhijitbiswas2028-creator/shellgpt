...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "ShellGPT". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `shellgpt profile rename
  ...
  // never generic "ShellGPT" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as ShellGPT (the agent it is)
...
/** The profile id the backend accepts (`shellgpt_cli.profiles._PROFILE_ID_RE`

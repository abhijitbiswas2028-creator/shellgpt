...
import { host, LruCache } from '@shellgpt/plugin-sdk'
...
// to (local, remote URL, SSH, ShellGPT Cloud, docker) must be able to find
...
// shellgpt_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['shellgpt-bots']?.title || profile?.display_name || ''),

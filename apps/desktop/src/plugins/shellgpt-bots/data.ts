...
import { atom, host, queryClient, useQuery, useValue } from '@shellgpt/plugin-sdk'
 ...
 *  protocol into the system prompt itself (shellgpt-agent bot_mode_probe).
...
async function fetchRosterSnapshot(activeConnectionId: null | string | undefined): Promise<RosterSnapshot> {
  ...
  // Multi-source desktops (shellgpt-agent #86875) also expose the union
...
function mergeMultiSourceRoster(
  ...
  for (const agent of agents) {
    ...
    profiles.push({
      ...
      // them a remote default titled "CoS Bot" only ever tags as @shellgpt(-device)
 ...
 *  callable alias is 'shellgpt' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'shellgpt' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`shellgpt profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "ShellGPT"
 *  can never hijack the primary profile's @shellgpt alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'shellgpt'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['shellgpt-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update ShellGPT Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)(?:@([a-z0-9][a-z0-9_-]*))?/gi)) {
    ...
    if (token === 'shellgpt') {
      token = byForm.has('shellgpt') ? 'shellgpt' : token
    }

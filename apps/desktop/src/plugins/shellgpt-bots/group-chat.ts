...
import { atom, host } from '@shellgpt/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'shellgpt-bots-groups'
...
/** The v3 envelope stored under the default profile's `shellgpt-bots-groups`
 ...
 *  clients (ShellGPT Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "ShellGPT"
 *  (community report, Aug 21 2026: renamed default still read "ShellGPT is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as ShellGPT (matching displayName and the
 *  @shellgpt handle) so the main agent never loses its name in rooms.
 *
 ...
 *  failing `default`s are never one anonymous "ShellGPT" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'ShellGPT' : profile)
  ...
  // title, then the local row's display_name, then default → ShellGPT.
  ...
  return isDefault ? 'ShellGPT' : trimmed
}

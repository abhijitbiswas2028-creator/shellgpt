...
import { host, resolveSiblingWsUrl } from '@shellgpt/plugin-sdk'
import type { DisplayLease, PluginProfileRoute, RpcEvent } from '@shellgpt/plugin-sdk'
...
/** JSON-RPC method-not-found: the bot's ShellGPT predates the `display.*` surface. */
 ...
 * `~/.shellgpt` path, so the profile key alone is ambiguous: the event must also have

...
Bundled ShellGPT Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@shellgpt/plugin-sdk`, `react`, and
...
`$SHELLGPT_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use ShellGPT theme
...
Stream availability, regional restrictions, and metadata vary. ShellGPT is not

...
const UPDATE_BACKEND = 'Update the ShellGPT backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.shellgptDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update ShellGPT Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.shellgptDesktop.openExternal(authUrl)
    let pollFailures = 0

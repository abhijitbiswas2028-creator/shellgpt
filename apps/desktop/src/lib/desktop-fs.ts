import { shellgptApi } from '@/api/client'
import type {
  ShellGPTConnection,
  ShellGPTReadDirResult,
  ShellGPTReadFileTextResult,
  ShellGPTSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ShellGPTConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: ShellGPTConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<ShellGPTReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ShellGPTReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ShellGPTReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ShellGPTReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.shellgptDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: ShellGPTSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()

import { GatewayReauthRequiredError, isGatewayReauthRequired, resolveGatewayWsUrl } from '@shellgpt/shared'
...
describe('desktop connection scope', () => {
  ...
  function registeredConnection(authMode: (typeof authModes)[number]) {
    return { ...aliasConnection(authMode), profile: 'remote-profile', registryScoped: true } as ShellGPTConnection
  }
  ...
  it('a registry-scoped flag without a connectionId still takes the legacy mint', async () => {
    const desktop = fakeDesktop()
    const scopedWithoutId = { ...registeredConnection('token'), connectionId: undefined } as ShellGPTConnection

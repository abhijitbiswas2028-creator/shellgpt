 ...
 * `shellgpt://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `shellgpt://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `shellgpt://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type ShellGPTOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const SHELLGPT_PROTOCOL = 'shellgpt:'
...
export function normalizeShellGPTOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('shellgpt://') || trimmed.startsWith(`${SHELLGPT_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // shellgpt://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // shellgpt://index-network/intent/1 → /index-network/intent/1
...
export function resolveShellGPTOpenPath(target: ShellGPTOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeShellGPTOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeShellGPTOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeShellGPTOpenString(target.path)
 ...
 * (`shellgpt://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromShellGPTDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveShellGPTOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveShellGPTOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `shellgpt://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromShellGPTDeepLink('open', name, params)
}

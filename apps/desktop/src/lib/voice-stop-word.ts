...
//
// When someone is in a hands-free "Hey ShellGPT" voice chat, the natural way to
...
// phrase (optionally addressed to ShellGPT), so a real turn that merely contains
...
// Optional address prefixes so "shellgpt stop" / "ok stop" / "hey shellgpt, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey shellgpt', 'hey shellgpt,', 'shellgpt', 'shellgpt,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("shellgpt") is not a stop command on its own.
      continue
    }
 ...
 * to ShellGPT). Returns false for anything that merely contains "stop" as part of

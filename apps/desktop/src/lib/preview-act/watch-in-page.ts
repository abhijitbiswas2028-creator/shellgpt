...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __shellgptWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__shellgptWatch || (win.__shellgptWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('shellgpt-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('shellgpt-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __shellgptWatchTag?: number }).__shellgptWatchTag

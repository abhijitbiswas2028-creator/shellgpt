...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> ShellGPT Desktop')).toBe('no ShellGPT Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>ShellGPT')).toBe('no ShellGPT')
  })

...
it('retains legacy data in memory when migration writes fail and retries on the next launch', () => {
  ...
  expect(window.localStorage.getItem('shellgpt.desktop.layoutModeScopes.v1')).toBeNull()
  ...
  expect(window.localStorage.getItem('shellgpt.desktop.layoutModeScopes.v1')).toBe('true')
  ...
  vi.stubGlobal('localStorage', {
    ...
    setItem: (key: string, value: string) => {
      if (key === 'shellgpt.desktop.layoutModeScopes.v1') {
        throw new DOMException('Full', 'QuotaExceededError')
      }

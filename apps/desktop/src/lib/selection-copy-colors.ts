...
// ShellGPT is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.shellgptMode
 ...
 * the only carriers of ShellGPT' palette — are dropped so the paste target's

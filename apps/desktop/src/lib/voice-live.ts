...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to ShellGPT.
 *
 ...
 * that into an ordinary ShellGPT turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * ShellGPT keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (ShellGPT) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await shellgptApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by ShellGPT; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await shellgptApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("ShellGPT is running the tests…"). */

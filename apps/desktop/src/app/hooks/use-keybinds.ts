...
export function useKeybinds(deps: KeybindRuntimeDeps): void {
  ...
  handlersRef.current = {
    ...
    'session.new': () => {
      ...
      window.dispatchEvent(new CustomEvent('shellgpt:new-session-shortcut'))
    },
  ...
  useEffect(() => {
    const updateF12Ownership = () => {
      ...
      window.shellgptDesktop?.setF12ShortcutActive?.(hasF12Binding || $capture.get() !== null)
    }
    ...
    return () => {
      stopBindings()
      stopCapture()
      window.shellgptDesktop?.setF12ShortcutActive?.(false)
    }
  }, [])

  useEffect(() => {
    const stopF12Shortcut = window.shellgptDesktop?.onF12Shortcut?.(input => {

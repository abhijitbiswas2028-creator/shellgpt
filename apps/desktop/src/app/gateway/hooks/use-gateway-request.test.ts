...
vi.mock('@/shellgpt', async importOriginal => {
  const actual = await importOriginal<typeof ShellGPTModule>()

  class FakeShellGPTGateway {
  ...
  return { ...actual, ShellGPTGateway: FakeShellGPTGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as ShellGPTGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as ShellGPTGateway, 'default')
  $gateway.set(primary as unknown as ShellGPTGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.shellgptDesktop, 'getGatewayWsUrlFor')
  ...
  it.each(['token', 'oauth'] as const)(
    ...
    async authMode => {
      ...
      Object.defineProperty(window, 'shellgptDesktop', { configurable: true, value: desktop })

      const primary = makePrimaryGateway()
      primary.request.mockRejectedValueOnce(new Error('connection closed')).mockResolvedValueOnce({ recovered: true })
      setPrimaryGateway(primary as unknown as ShellGPTGateway, connection.profile)
      $gateway.set(primary as unknown as ShellGPTGateway)
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as ShellGPTGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { shellgptDesktop: unknown }).shellgptDesktop = { getConnection }
    $gateway.set(dropped)

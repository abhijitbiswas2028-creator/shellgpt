...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ShellGPTGateway | null
  const gatewayRef = useRef<ShellGPTGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.shellgptDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ShellGPTGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ShellGPTGateway | null
      }),
    []
  )

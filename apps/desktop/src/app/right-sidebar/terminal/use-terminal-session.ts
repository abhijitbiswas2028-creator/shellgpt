...
const SHELLGPT_PATHS_MIME = 'application/x-shellgpt-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(SHELLGPT_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(SHELLGPT_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.shellgptDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (shellgpt --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.shellgptDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[shellgpt-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.shellgptDesktop?.terminal?.write(sessionId, `${command}\r`)

...
import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/shellgpt'
...
import * as shellgpt from '@/shellgpt'

const mocked = vi.mocked(shellgpt)
...
describe('BrowseSection', () => {
  ...
  it('searches HF after a pause and shows fit-priced files on demand', async () => {
    ...
    try {
      vi.mocked(shellgpt.searchHFModels).mockResolvedValue({
        hits: [{ downloads: 872724, gated: false, likes: 47, repo: 'unsloth/Qwen3.8-27B-GGUF', updated: '2026-08-18' }]
      })
      vi.mocked(shellgpt.listHFRepoFiles).mockResolvedValue({
        files: [
      ...
      expect(shellgpt.searchHFModels).not.toHaveBeenCalled()
      await act(async () => {
        await vi.advanceTimersByTimeAsync(400)
      })
      expect(shellgpt.searchHFModels).toHaveBeenCalledWith('qwen')
      ...
      vi.mocked(shellgpt.downloadBrowsedModel).mockResolvedValue({ job_id: 'j1', model_id: 'Qwen3.8-27B-Q4_K_M' })
      fireEvent.click(q4Btn)
      await act(async () => {
        await vi.runOnlyPendingTimersAsync()
      })
      expect(shellgpt.downloadBrowsedModel).toHaveBeenCalledWith('unsloth/Qwen3.8-27B-GGUF', ['Qwen3.8-27B-Q4_K_M.gguf'])
    } finally {
      vi.useRealTimers()
    }
  })
...
describe('added-by-you rows', () => {
  it('staged models outside the catalog get the full action set', async () => {
    vi.mocked(shellgpt.getLocalModelsStatus).mockResolvedValue({
      ...BASE_STATUS,
      loaded_models: { 'ShellGPT-4.3-36B-Q5_K_M': 'loaded' },
      models: [{ id: 'ShellGPT-4.3-36B-Q5_K_M', size_bytes: 25 * 2 ** 30, size_label: '25.0 GB' }],
      placement: {
        'ShellGPT-4.3-36B-Q5_K_M': {
    ...
    vi.mocked(shellgpt.getLocalCatalog).mockResolvedValue({ models: [] })

    renderPane()
    await screen.findByText('ShellGPT-4.3-36B-Q5_K_M')

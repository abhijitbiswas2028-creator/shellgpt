...
import {
  getElevenLabsVoices,
  getShellGPTConfigSchema,
  type ProfileScope,
  profileScopeKey,
  saveShellGPTConfigRecord
} from '@/shellgpt'
...
export function VoiceProviderFields({
  ...
  const { data: loadedConfig, writeScope } = useShellGPTConfigRecord(profile)
  ...
  const writeConfigCache = useMemo(() => shellgptConfigCacheWriter(profile), [scopeKey])
  ...
  const [baseline, setBaseline] = useState<ShellGPTConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveShellGPTConfigRecord(diffConfig(baseline ?? {}, config), writeScope ?? profile)
        .then(() => {
          setBaseline(config)

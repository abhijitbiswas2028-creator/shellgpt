...
describe('mergePluginPackages', () => {
  it('shows a unified package as ONE row with both halves, never two rows', () => {
    const rows = mergePluginPackages(
      [desktop({ id: 'media', name: 'Media Studio', packageName: 'shellgpt-media-studio' })],
      [agent({ name: 'shellgpt-media-studio', has_desktop_half: true, description: 'Generate media.' })]
    )
    ...
    expect(rows[0].agent?.name).toBe('shellgpt-media-studio')
  })

  it('a desktop half whose agent half is absent from THIS profile offers the install-here affordance', () => {
    const rows = mergePluginPackages([desktop({ id: 'media', packageName: 'shellgpt-media-studio' })], [])
  ...
  // desktop-plugins/<name>/plugin.js with no .shellgpt-package.json, and the row
  ...
  it('pairs a desktop half with its agent row through the package marker', () => {
    const rows = mergePluginPackages(
      [
        desktop({
          id: 'shellgpt-talk',
          name: 'ShellGPT Talk',
          description: 'GPT-Live subscription or explicit API voice, with ShellGPT task delegation.',
          packageName: 'shellgpt-talk',
          file: '/Users/me/.shellgpt/desktop-plugins/shellgpt-talk/plugin.js'
        })
      ],
      [agent({ name: 'shellgpt-talk', has_desktop_half: true, version: '0.21.0' })]
    )
    ...
    expect(rows[0].desktop?.id).toBe('shellgpt-talk')
    expect(rows[0].agent?.name).toBe('shellgpt-talk')
  })
  ...
  it('keeps an unmarked app-root copy its own row rather than guessing from the folder name', () => {
    const rows = mergePluginPackages(
      ...
      [agent({ name: 'shellgpt-talk', has_desktop_half: true })]
    )

    expect(rows.map(row => [row.key, row.kind])).toEqual([
      ['shellgpt-talk', 'both'],
      ['desktop:shellgpt-talk', 'desktop']
    ])
  })

  it('leaves a same-named standalone desktop plugin alone when the agent package has no desktop half', () => {
    const rows = mergePluginPackages(
      [desktop({ id: 'clock', name: 'Clock', file: '/Users/me/.shellgpt/desktop-plugins/clock/plugin.js' })],
      [agent({ name: 'clock' })]
    )

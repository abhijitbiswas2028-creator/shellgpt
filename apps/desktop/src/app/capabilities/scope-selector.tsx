...
import { getProfiles, type ProfileScope, profileScopeKey } from '@/shellgpt'
...
export function useCapabilityScope({
  ...
  const registryBridge = window.shellgptDesktop?.connections
  const rosterBridge = window.shellgptDesktop?.getAgentRoster
  ...
  const options: ScopeOption[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'ShellGPT (default)' : p.name,
      value: p.name
    }))

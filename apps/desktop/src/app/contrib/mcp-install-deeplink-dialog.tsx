...
import { getShellGPTConfigRecord, saveMcpServers } from '@/shellgpt'
...
import { setShellGPTConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `shellgpt://mcp/install` deep links. The payload is
 ...
 * confirming lets ShellGPT spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getShellGPTConfigRecord())
      ...
      setShellGPTConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))

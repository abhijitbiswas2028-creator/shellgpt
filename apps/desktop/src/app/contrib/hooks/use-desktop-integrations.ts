...
import { pathFromShellGPTDeepLink, resolveShellGPTOpenPath } from '@/lib/shellgpt-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About ShellGPT" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.shellgptDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.shellgptDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // shellgpt://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future shellgptDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveShellGPTOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (shellgpt://index-network/intent/1, shellgpt://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromShellGPTDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.shellgptDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.shellgptDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())

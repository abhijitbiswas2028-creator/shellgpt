...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshShellGPTConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshShellGPTConfig — two REST calls (config + defaults) per event, per

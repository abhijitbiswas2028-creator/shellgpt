import type { BillingBlock } from '@shellgpt/shared'
 ...
 * billing-specific toast — never the generic "ShellGPT error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'ShellGPT reported an error',

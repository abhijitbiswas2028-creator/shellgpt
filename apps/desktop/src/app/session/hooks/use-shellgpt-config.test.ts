...
import { getShellGPTConfig } from '@/shellgpt'
...
import { useShellGPTConfig } from './use-shellgpt-config'

vi.mock('@/shellgpt', () => ({
  getShellGPTConfig: vi.fn(),
  getShellGPTConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'shellgpt.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getShellGPTConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getShellGPTConfig>>)

describe('useShellGPTConfig refreshShellGPTConfig', () => {
  beforeEach(() => {
  ...
  it('mirrors display.show_reasoning and resets a missing key to the default', async () => {
    ...
    const { result } = renderHook(() => useShellGPTConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshShellGPTConfig()
    })
  ...
  // `agent.reasoning_effort: high` to ShellGPT' built-in medium.
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useShellGPTConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getShellGPTConfig>>>()
    vi.mocked(getShellGPTConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useShellGPTConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshShellGPTConfig(true)
    })
    expect(getShellGPTConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getShellGPTConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getShellGPTConfig>>>()
    vi.mocked(getShellGPTConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useShellGPTConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshShellGPTConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getShellGPTConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getShellGPTConfig>>>()
    vi.mocked(getShellGPTConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshShellGPTConfig(true)
      refreshC = result.current.refreshShellGPTConfig(true)
    })

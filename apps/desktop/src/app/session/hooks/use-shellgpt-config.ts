...
import { getShellGPTConfig, getShellGPTConfigDefaults } from '@/shellgpt'
...
interface ShellGPTConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useShellGPTConfig({ activeSessionIdRef }: ShellGPTConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshShellGPTConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getShellGPTConfig(), getShellGPTConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying ShellGPT' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshShellGPTConfig, sttEnabled, voiceMaxRecordingSeconds }
}

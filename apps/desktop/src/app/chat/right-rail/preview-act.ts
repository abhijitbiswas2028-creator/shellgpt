...
const preamble = () => `  var w = window;
  ...
  w.__shellgptActHolder = w.__shellgptActHolder || {};
  w.__shellgptAct = ${actEngineSource()};
  w.__shellgptWatch_fn = (${watchInPage.toString()});
  w.__shellgptWatchTag = ${WATCH_TAG};
  var holder = w.__shellgptActHolder;
  var act = function (a) { return w.__shellgptAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__shellgptWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  w.__shellgptHit = null;
  document.addEventListener('pointerdown', function (e) {
    w.__shellgptHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'SHELLGPT-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}

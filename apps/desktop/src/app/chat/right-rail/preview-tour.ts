...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__shellgptTourEngine) {
    ${driverIife}
    w.__shellgptTourHolder = {};
    w.__shellgptTourCollect = (${collectTourTargets.toString()});
    w.__shellgptTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__shellgpt-tour-style')) {
    var style = document.createElement('style');
    style.id = '__shellgpt-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__shellgptTourEngine(
    w.driver.js.driver,
    w.__shellgptTourHolder,
    ${JSON.stringify(action)},
    w.__shellgptTourCollect,
    document
  ));
})()`
}

...
vi.mock('@/shellgpt', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), shellgptApi: vi.fn() }))
...
const { deleteProfile, shellgptApi } = await import('@/shellgpt')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its SHELLGPT_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { shellgptDesktop: unknown }).shellgptDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(shellgptApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(shellgptApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(shellgptApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)

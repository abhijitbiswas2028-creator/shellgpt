 ...
 *    a `ShellGPTPlugin` registers automatically (vite glob — drop a folder in).
 *    `shellgpt-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `shellgpt-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<shellgpt home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<shellgpt home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: ShellGPTPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default ShellGPTPlugin export — skipped`)

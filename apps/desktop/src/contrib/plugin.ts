 ...
 * `ShellGPTPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/shellgpt'
...
export type { PluginRestOptions } from '@/shellgpt'
 ...
 *  under `shellgpt.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.shellgptDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from ShellGPT — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `shellgpt://index-network/intent/1`),
...
export interface ShellGPTPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `shellgpt.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.shellgptDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.shellgptDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.shellgptDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.shellgptDesktop

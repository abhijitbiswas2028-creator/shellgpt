import type { ConnectionRequestPayload, ToolLabel } from '@shellgpt/shared'
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when ShellGPT doesn't know how to remove it. */
...
export interface FreeTierStatus {
   ...
   *  (`shellgpt_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in shellgpt_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `shellgpt model` uses — instead of
 ...
 * choices as `shellgpt model`'s custom-provider setup (#93622). */
...
export interface ShellGPTConfig {
  agent?: {
...
export type ShellGPTConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['shellgpt-bots'].title) — the name the
...
// non-desktop) ShellGPT simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (shellgpt.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors shellgpt_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with ShellGPT, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with ShellGPT but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `shellgpt tools` active-provider detection). */
  is_active: boolean
 ...
 *  not ShellGPT); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** shellgpt-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** shellgpt-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/shellgpt/update/check` — the backend's own update state.

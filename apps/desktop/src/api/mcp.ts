import type { McpCatalogResponse, McpServerSummary } from '@/types/shellgpt'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.shellgptDesktop.api<McpTestResult>({
 ...
 *  `saveShellGPTConfig`, this REPLACES rather than deep-merges, so deletes,
...
// `shellgpt mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveShellGPTConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return shellgptApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(
  ...
  return window.shellgptDesktop.api<McpServerSummary>({
    ...capabilityScoped(profile),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope, detectApps = false): Promise<McpCatalogResponse> {
  return window.shellgptDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.shellgptDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),

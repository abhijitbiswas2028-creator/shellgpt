...
import { capabilityScoped, shellgptApi, type ProfileScope, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getProfiles(scope?: ProfileScope): Promise<ProfilesResponse> {
  return shellgptApi<ProfilesResponse>({
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return shellgptApi<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string, scope?: ProfileScope): Promise<{ ok: boolean; path: string }> {
  ...
  return shellgptApi<{ ok: boolean; path: string }>({
    ...profileOwnerScoped(scope),
...
export function getProfileSoul(name: string, scope?: ProfileScope): Promise<ProfileSoul> {
  return shellgptApi<ProfileSoul>({
    ...profileOwnerScoped(scope),
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return shellgptApi<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return shellgptApi<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return shellgptApi<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({

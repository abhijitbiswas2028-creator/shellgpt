...
const shellgptApi = vi.mocked(client.shellgptApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    shellgptApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'shellgpt-pi' })

    await deleteSession('sess-1', { connectionId: 'shellgpt-pi', profile: 'tommy' })

    expect(shellgptApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'shellgpt-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((shellgptApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('getSession dial priority', () => {
  it('does not dial an explicitly scoped session read foreground', async () => {
    ...
    shellgptApi.mockResolvedValue({ id: 'sess-5' } as never)
    ...
    expect(shellgptApi.mock.calls[0][0]).toMatchObject({ profile: 'tommy', connectionId: 'local' })
    expect(shellgptApi.mock.calls[0][0]).not.toHaveProperty('priority')
  })
})
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('falls back to the ACTIVE profile in the body when no owner is given', async () => {
    ...
    // `body.profile` and there is no per-profile backend whose SHELLGPT_HOME
  ...
  it('omits the profile from the body only when there is no active profile at all', async () => {
    ...
    const req = shellgptApi.mock.calls[0][0] as { body: Record<string, unknown> }

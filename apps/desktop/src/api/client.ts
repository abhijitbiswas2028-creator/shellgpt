import { JsonRpcGatewayClient } from '@shellgpt/shared'

import type { ShellGPTApiRequest } from '@/global'
...
// "Timed out connecting to ShellGPT backend" that hangs the UI (#48504).
//
...
export const GATEWAY_NOT_CONNECTED_MESSAGE = 'ShellGPT gateway is not connected'

export class ShellGPTGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'ShellGPT gateway connection closed',
      connectErrorMessage: 'Could not connect to ShellGPT gateway',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] ShellGPT Desktop has no server-request registry for ${request.method} (${request.id})`),
 ...
 *  the ambient tag `shellgptApi` spreads underneath (as capabilityScoped does). */
...
// process to route them to that host (shellgpt:api's registry branch). Null
...
export function shellgptApi<T>(request: ShellGPTApiRequest): Promise<T> {
  return window.shellgptDesktop.api<T>({ ...connectionScoped(), ...request })
}

...
const agentConn = (over: Partial<ShellGPTConnection> = {}): ShellGPTConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as ShellGPTConnection

const localConn = (over: Partial<ShellGPTConnection> = {}): ShellGPTConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ShellGPTConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ShellGPTConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<ShellGPTConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { shellgptDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<ShellGPTConnection>(() => undefined))

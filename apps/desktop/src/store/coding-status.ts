...
import type { ShellGPTGitWorktree, ShellGPTRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ShellGPTGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ShellGPTRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ShellGPTGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ShellGPTRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ShellGPTGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ShellGPTRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ShellGPTGitWorktree[]>>()
const $noRepoStatus = atom<ShellGPTRepoStatus | null>(null)
const $noWorktrees = atom<ShellGPTGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ShellGPTRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ShellGPTGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ShellGPTRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ShellGPTGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ShellGPTRepoStatus | null>
  seq: number
  target: string
}

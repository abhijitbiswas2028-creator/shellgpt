...
import type { ShellGPTBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/shellgpt'
...
export const $pullRequestsByBranch = atom<Record<string, ShellGPTBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'shellgpt.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('shellgpt.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: ShellGPTBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }

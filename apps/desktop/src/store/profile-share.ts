 ...
 *
 * The archive is the CLI's own `shellgpt profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/shellgpt'
...
import type { ProfileDesktopOverlay } from '@/types/shellgpt'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'ShellGPT profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.shellgptDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.shellgptDesktop?.selectPaths?.({

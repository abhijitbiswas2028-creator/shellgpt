...
export function setZoomPercent(percent: number): void {
  window.shellgptDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.shellgptDesktop?.zoom) {
  void window.shellgptDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.shellgptDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}

...
describe('model visibility', () => {
  ...
  it('shows a model that appeared after the user curated its provider, unless the provider is hidden', () => {
    ...
    const known = new Set([
      ...
      modelVisibilityKey('nous', 'shellgpt-4')
    ])

    const providers = [provider('claude-sub', ['sonnet', 'haiku', 'opus']), provider('nous', ['shellgpt-4', 'shellgpt-5'])]
    ...
    expect(visible.has(modelVisibilityKey('nous', 'shellgpt-5'))).toBe(false)
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    ...
    const visible = effectiveVisibleKeys(stored, [
      provider('nous', ['shellgpt-3-llama-3.1-70b', 'shellgpt-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('nous', 'shellgpt-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('nous', 'shellgpt-3-llama-3.1-8b'))).toBe(false)
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('nous')])
    const providers = [provider('nous', ['shellgpt-x', 'shellgpt-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('nous'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('nous', 'shellgpt-x'))).toBe(false)
...
describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('nous', ['shellgpt-x', 'shellgpt-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    ...
    let stored: Set<string> | null = null
    stored = apply(stored, 'nous', 'shellgpt-x')
    stored = apply(stored, 'nous', 'shellgpt-y')
    ...
    expect(visible.has(modelVisibilityKey('nous', 'shellgpt-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('nous', 'shellgpt-y'))).toBe(false)
  })
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('nous', 'shellgpt-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('nous', 'shellgpt-x'))).toBe(true)

...
import { checkShellGPTUpdate, getActionStatus, updateShellGPT } from '@/shellgpt'
...
import type { BackendUpdateCheckResponse } from '@/types/shellgpt'
...
const UPDATE_TOAST_SNOOZE_KEY = 'shellgpt:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'shellgpt:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'shellgpt:install-method-toast-snooze-until'
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.shellgptDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkShellGPTUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.shellgptDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `shellgpt update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'shellgpt update',
        command: result.command ?? 'shellgpt update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== shellgpt-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateShellGPT()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'shellgpt update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkShellGPTUpdate(true)
...
const FOCUS_RECHECK_KEY = 'shellgpt.updates.last-passive-check'

 ...
 *
 * Local models ship on main behind `--local` (either `shellgpt desktop --local`
 * or the flag on ShellGPT.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.shellgptDesktop?.localModelsEnabled === true
)

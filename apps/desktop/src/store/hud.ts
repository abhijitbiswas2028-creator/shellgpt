 ...
 * composer with the reply scrolling above it, so ShellGPT can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.shellgptDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.shellgptDesktop?.hud
...
export function resetHudLayout(): void {
  void window.shellgptDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.shellgptDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.shellgptDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)

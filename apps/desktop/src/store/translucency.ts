 ...
 * `@shellgpt/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-shellgpt-glass]`
 * block in styles.css).
 */
...
} from '@shellgpt/shared/translucency'
 ...
 * sets `shellgptDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.shellgptDesktop?.glassSupported === 'boolean'
    ? window.shellgptDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.shellgptDesktop?.translucencySupported === 'boolean'
    ? window.shellgptDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-shellgpt-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-shellgpt-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-shellgpt-glass-on', glassLive)
  root.toggleAttribute('data-shellgpt-glass', glassOn)
  root.toggleAttribute('data-shellgpt-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-shellgpt-glass-scope', scope)
  } else {
    root.removeAttribute('data-shellgpt-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.shellgptDesktop?.setTranslucency?.(state)
  })

...
import { type ShellGPTOpenTarget, resolveShellGPTOpenPath } from '@/lib/shellgpt-open-target'
...
export type { ShellGPTOpenTarget }
...
const STORAGE_KEY = 'shellgpt:native-notifications'
...
// "Backgrounded" = the user isn't on ShellGPT. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `shellgpt://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.shellgptDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `shellgpt://index-network/intent/1`). */
  activate?: ShellGPTOpenTarget
 ...
 *  user is away from ShellGPT — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveShellGPTOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveShellGPTOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.shellgptDesktop

...
import { getSkills } from '@/shellgpt'
 ...
 *  Working in `~/www/shellgpt-agent`, the draft says "shellgpt-agent" constantly
 *  — a "Use skill: shellgpt-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`shellgpt-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/shellgpt-agent-dev`, `plugin:skill`). */

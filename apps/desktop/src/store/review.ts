...
//
// Scope is always "uncommitted" — ShellGPT' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'shellgpt.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'shellgpt.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'shellgpt.desktop.reviewTreeMode'
const SELECTED_KEY = 'shellgpt.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ShellGPTReviewFile[]>([])
...
export const $reviewShipInfo = atom<ShellGPTReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['shellgptDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ShellGPTReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ShellGPTReviewFile[], path: string): ShellGPTReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.shellgptDesktop?.openExternal?.(existing.url)

 ...
 * app's bounds and stays visible while ShellGPT is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'shellgpt.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'shellgpt.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.shellgptDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.shellgptDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.shellgptDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.shellgptDesktop?.petOverlay?.close()
}

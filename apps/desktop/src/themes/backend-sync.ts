/**
 * Live skin sync from the ShellGPT backend.
 *
 * The backend resolves the active skin (built-in or `$SHELLGPT_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (ShellGPT
...
const BACKEND_THEMES_KEY = 'shellgpt-desktop-backend-themes-v1'
...
const localSkinPayload = typeof window === 'undefined' ? null : window.shellgptDesktop?.localSkin ?? null
 ...
 */
export function ingestBackendSkin(skin: ShellGPTSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ShellGPTSkin)

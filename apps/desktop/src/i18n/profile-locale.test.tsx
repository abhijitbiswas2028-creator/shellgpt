...
it('reads and persists the owning profile through A → B → A and restart', async () => {
  const configs: Record<string, ShellGPTConfigRecord> = {
    default: { display: { language: 'en' } },
    coder: { display: { language: 'zh', skin: 'mono' } }
  }

  const api = vi.fn(async (request: ShellGPTApiRequest) => {
    const key = request.profile || 'default'

    if (request.method === 'PUT') {
      configs[key] = structuredClone((request.body as { config: ShellGPTConfigRecord }).config)
  ...
  window.shellgptDesktop = { api } as never
...
it('isolates stale reads and failed saves when the same profile moves between connections', async () => {
  let finishBoot!: (config: ShellGPTConfigRecord) => void
  let finishSaveRead!: (config: ShellGPTConfigRecord) => void
  let rejectSave!: (reason: Error) => void
  let reads = 0

  const api = vi.fn((request: ShellGPTApiRequest) => {
    if (request.method === 'PUT') {

...
describe('Sessions/Bots strip — #91223', () => {
  beforeEach(() => {
    ...
    registerPane('shellgpt-bots:pane', { hideOnly: true, placement: 'left' }, 'Bots')
  ...
  it('double-clicking the Sessions tab leaves the strip and both chips', () => {
    ...
    expect(tabEl('shellgpt-bots:pane')).toBeTruthy()
...
describe('docked tool tile — collapsing keeps the restore chip', () => {
  beforeEach(() => {
    registerPane('workspace', { placement: 'main', uncloseable: true }, 'Chat')
    registerPane('shellgpt-bots:routines', { placement: 'main', width: '250px' }, 'Cronjobs')
    $layoutTree.set(
      split('row', [
        group(['workspace'], { active: 'workspace', id: 'g-main' }),
        group(['shellgpt-bots:routines'], { active: 'shellgpt-bots:routines', id: 'g-routines' })
      ])
    )
  })
  ...
  it('clicking the active tab does not collapse the tile or drop its label', () => {
    ...
    expect(tabEl('shellgpt-bots:routines')).toBeTruthy()
    tap(tabEl('shellgpt-bots:routines')!)

    expect(zoneAt(1).minimized).toBeFalsy()
    expect(tabEl('shellgpt-bots:routines')).toBeTruthy()
    expect(tabEl('shellgpt-bots:routines')?.textContent).toMatch(/cronjobs/i)
  })
  ...
  it('collapsing via the store does not dismiss the pane from the tree', () => {
    collapseTreePane('shellgpt-bots:routines')

    expect(zoneAt(1).minimized).toBe(true)
    expect($dismissedPanes.get().has('shellgpt-bots:routines')).toBe(false)
    expect(zoneAt(1).panes).toContain('shellgpt-bots:routines')

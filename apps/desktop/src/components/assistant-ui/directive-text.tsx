...
const SHELLGPT_REF_TYPES = WIRE_REFERENCE_KINDS
type ShellGPTRefType = (typeof SHELLGPT_REF_TYPES)[number]
...
const SHELLGPT_DIRECTIVE_RE = referenceRe()
...
export const shellgptDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SHELLGPT_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...shellgptDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing ShellGPT directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.shellgptDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.shellgptDesktop?.readFileDataUrl(id)

 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.shellgpt.send(prompt)` (or declarative
 * `data-shellgpt-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'shellgpt-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'shellgpt-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `shellgpt.send(prompt)`.
 ...
 *  wires `data-shellgpt-send` so declarative HTML works with zero script:
 *  `<button data-shellgpt-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.shellgpt={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-shellgpt-send]"):null;' +
    'if(el)send(el.getAttribute("data-shellgpt-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `shellgpt serve` — renders live.)

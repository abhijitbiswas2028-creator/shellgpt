"""
ShellGPT MCP Server — expose messaging conversations as MCP tools (`shellgpt mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the ShellGPT-specific
channels_list. Client config: {"mcpServers": {"shellgpt": {"command": "shellgpt", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("shellgpt.mcp_serve")
...
def _shellgpt_home() -> Path:
    try:
        ...
        return Path(os.environ.get("SHELLGPT_HOME", Path.home() / ".shellgpt"))


def _get_sessions_dir() -> Path:
    return _shellgpt_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_shellgpt_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_shellgpt_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the ShellGPT analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the ShellGPT MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("shellgpt", instructions=(
        "ShellGPT Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ShellGPT MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:

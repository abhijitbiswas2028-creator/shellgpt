"""Cross-process lease watcher: ``display.lease`` for transitions made OUTSIDE ``shellgpt serve``.

The lease (``tools.bot_desktop.lease``) lives on disk and is changed by whatever process hosts the
agent — a ``shellgpt computer-use screen`` takeover from the CLI, a cron worker's release.
...
def _watched_lease_homes() -> list[Path]:
    return [Path(_shellgpt_home), *_served_profile_homes]
...
def _ensure_lease_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="shellgpt-lease-watcher", daemon=True).start()

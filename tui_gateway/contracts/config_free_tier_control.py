...
class ModelPricing(Result):
    """``shellgpt_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
...
class ModelOptionProvider(OpenModel):
    """One ``shellgpt_cli/inventory.py::build_models_payload`` provider row (the union of every field

...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.shellgpt/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class PluginSettingField(Result):
    ...
    (``shellgpt_cli.plugins_settings.plugin_settings_fields``). ``secret`` fields carry no value: ``env``
...
class PluginActivation(Result):
    """What a plugin loaded mid-run does NOW vs later (``shellgpt_cli.plugins_activation``). ``activated_now``
...
class PluginsManageResult(Result):
    ...
    (the canonical key written)/``plugin``; ``install`` → ``shellgpt_cli.plugins_cmd.dashboard_install_plugin``'s

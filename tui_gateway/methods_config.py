...
from shellgpt_constants import display_shellgpt_home as _display_shellgpt_home
...
def _cfg_get_provider(params):
    from shellgpt_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _shellgpt_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_shellgpt_home), "display": _display_shellgpt_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's SHELLGPT_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``SHELLGPT_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    added after boot — the Models page, a picker key, ``shellgpt setup`` from a shell — flips it
    ...
    try:
        ...
        from shellgpt_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No ShellGPT provider is configured.", source)
            ...
            from shellgpt_cli.anon_auth import route_is_welcome_host
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``shellgpt debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from shellgpt_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle

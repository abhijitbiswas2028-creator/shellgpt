...
documents (stop ShellGPT, recover or restore, start again). No marker is written to disk:
...
def mark_storage_corrupt(db_path, reason: object) -> None:
    ...
    logger.error(
        ...
        "until ShellGPT restarts on a recovered or restored file. Stop ShellGPT, then run "
        "`shellgpt sessions recover --source %s --inspect-only` or restore a snapshot.",

#!/bin/sh
# shellcheck shell=sh
# /opt/shellgpt/bin/shellgpt — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `shellgpt` user (UID 10000). When an operator runs `docker exec <c> shellgpt ...`
# the default UID is root (0), and any file the command writes under
# $SHELLGPT_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> shellgpt login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: ShellGPT is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> shellgpt
...
# Fix
# ---
# This shim sits at /opt/shellgpt/bin/shellgpt and is placed earliest on PATH.
# When invoked as root, it drops to the shellgpt user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $SHELLGPT_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user shellgpt`, kanban subagents, etc. — it short-circuits
...
# (/opt/shellgpt/.venv/bin/shellgpt), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set SHELLGPT_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the shellgpt CLI. Default is to drop.

set -e

REAL=/opt/shellgpt/.venv/bin/shellgpt

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "shellgpt-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user shellgpt`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${SHELLGPT_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the shellgpt user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "shellgpt-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "shellgpt-shim: re-run with --user shellgpt or set SHELLGPT_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the shellgpt user's home before dropping privileges. Without

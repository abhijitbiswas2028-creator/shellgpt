...
# ShellGPT WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SHELLGPT_TINI_SHIM_TARGET overrides the `/init` path (and
# SHELLGPT_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SHELLGPT_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SHELLGPT_TINI_SHIM_WRAPPER:-/opt/shellgpt/docker/main-wrapper.sh}"

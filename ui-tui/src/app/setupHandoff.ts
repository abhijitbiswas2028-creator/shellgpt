import type { RunExternalProcess } from '@shellgpt/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`shellgpt ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching shellgpt: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`shellgpt ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

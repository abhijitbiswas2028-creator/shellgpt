...
import { terminalBackgroundHex } from '@shellgpt/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SHELLGPT_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SHELLGPT_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SHELLGPT_TUI_BACKGROUND', process.env.SHELLGPT_TUI_BACKGROUND ?? '(unset)'],
            ['SHELLGPT_TUI_THEME', process.env.SHELLGPT_TUI_THEME ?? '(unset)'],

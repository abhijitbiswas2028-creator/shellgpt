...
/**
 * User widget apps — ShellGPT authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SHELLGPT_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.shellgpt/plugins/`: files under SHELLGPT_HOME execute
...
const widgetsDir = () => join(process.env.SHELLGPT_HOME?.trim() || join(homedir(), '.shellgpt'), 'tui-widgets')
 ...
 *  every change, so a widget ShellGPT writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or ShellGPT) ever writes must hot-load too — a 10s poll here read

import type { InputEvent, Key } from '@shellgpt/ink'
import * as Ink from '@shellgpt/ink'
 ...
 * but that accepts `key.meta` on macOS — and shellgpt-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SHELLGPT_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which shellgpt-ink decodes as meta+'d'; without this branch it

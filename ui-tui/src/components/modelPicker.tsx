...
import { fuzzyRank } from '@shellgpt/shared/fuzzy'
import type { ModelOptionProvider, ModelOptionsResult } from '@shellgpt/shared/gateway-events'
import { modelSearchText } from '@shellgpt/shared/model-search-text'
import { REASONING_EFFORTS } from '@shellgpt/shared/reasoning-effort'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `shellgpt model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run shellgpt model)
          return
        }
    ...
    // filter. With Ctrl held, @shellgpt/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.shellgpt/.env)
        </Text>

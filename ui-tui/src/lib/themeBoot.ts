 ...
 * `shellgpt-boot-background` / `shellgpt-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SHELLGPT_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.shellgpt matches get_shellgpt_home()'s default.
const bootFilePath = () => join(process.env.SHELLGPT_HOME ?? join(homedir(), '.shellgpt'), 'tui-theme-boot.json')

// Never touch the user's real ~/.shellgpt from test runs (the TS suite has no
// SHELLGPT_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SHELLGPT_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SHELLGPT_TUI_THEME || env.SHELLGPT_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.SHELLGPT_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.SHELLGPT_TUI_BACKGROUND
  ) {
    env.SHELLGPT_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SHELLGPT_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SHELLGPT_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SHELLGPT_TUI_THEME.

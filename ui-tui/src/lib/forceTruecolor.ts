...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SHELLGPT_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep ShellGPT on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SHELLGPT_TUI_TRUECOLOR=1.

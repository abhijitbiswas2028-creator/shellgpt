...
const logDir = join(process.env.SHELLGPT_HOME?.trim() || join(homedir(), '.shellgpt'), 'logs')
...
// a real ~/.shellgpt (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('shellgpt-tui: parent lifecycle log unavailable\n')
    }
  }
}

...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because SHELLGPT_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.SHELLGPT_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.shellgpt', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `shellgpt-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.SHELLGPT_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.SHELLGPT_HEAPDUMP_MAX_BYTES?.trim()

...
const ENV_KEYS = ['SHELLGPT_AUTO_HEAPDUMP', 'SHELLGPT_HEAPDUMP_DIR', 'SHELLGPT_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'shellgpt-heapdump-test-'))
    process.env.SHELLGPT_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without SHELLGPT_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when SHELLGPT_AUTO_HEAPDUMP=1', async () => {
    process.env.SHELLGPT_AUTO_HEAPDUMP = '1'
  ...
  it('treats other values (0, off, garbage) as opt-out for auto triggers', async () => {
    for (const value of ['0', 'off', 'nope']) {
      process.env.SHELLGPT_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of SHELLGPT_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.SHELLGPT_HEAPDUMP_DIR
    savedMax = process.env.SHELLGPT_HEAPDUMP_MAX_BYTES
    delete process.env.SHELLGPT_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'shellgpt-heapdump-prune-'))
    process.env.SHELLGPT_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.SHELLGPT_HEAPDUMP_DIR
    } else {
      process.env.SHELLGPT_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.SHELLGPT_HEAPDUMP_MAX_BYTES
    } else {
      process.env.SHELLGPT_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.SHELLGPT_HEAPDUMP_MAX_BYTES = String(2 * 1024)

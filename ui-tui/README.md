...
React + Ink terminal UI for ShellGPT. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SHELLGPT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `shellgpt setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.shellgpt/.shellgpt_history` or under `SHELLGPT_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `shellgpt setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/shellgpt-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external shellgpt setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.shellgpt/tui-parent.log
    ...
    types/
      shellgpt-ink.d.ts            type declarations for @shellgpt/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ShellGPTCLI subprocess for slash commands
```

...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[shellgpt] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[shellgpt`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[shellgpt`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        ShellGPTHome = $(if ($paths) { $paths.shellgpt_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\shellgpt\shellgpt-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-shellgpt'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-shellgpt') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"

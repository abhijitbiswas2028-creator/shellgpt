# Behavioral test for install.ps1's shellgpt launcher staging (PR #92092,
...
#
# The test lifts the real Install-ShellGPTCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $ShellGPTHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-ShellGPTCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-ShellGPTCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'shellgpt-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'shellgpt-agent'
    ...
    $missingThrew = $false
    try {
        Install-ShellGPTCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing shellgpt.exe fails the launcher stage'
    ...
    $shellgptV1 = [byte[]](77, 90, 1)
    $shellgptV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'shellgpt.exe'), $shellgptV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-ShellGPTCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $shellgptV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'shellgpt.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'shellgpt-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'shellgpt.exe'), $shellgptV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'shellgpt-acp.exe'), $acp)
    Install-ShellGPTCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $shellgptV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'shellgpt.exe'))) `
        'installer refreshes an existing ShellGPT launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'shellgpt-acp.exe'))) `
    ...
    Install-ShellGPTCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'shellgpt.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'shellgpt.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'shellgpt.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'shellgpt.exe')) -and $cmdBody.Contains('%*')) `

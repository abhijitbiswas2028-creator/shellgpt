...
run_py() {
  ...
  if [ "$with_var" = yes ]; then
    PYTHONPATH="$CAP_DIR${PYTHONPATH:+:$PYTHONPATH}" \
      SHELLGPT_E2E_CAPTURE_LAUNCH="$WORK/spec.json" python3 "$@"
  else
...
r = subprocess.run(["npm", "exec", "--", "electron", "."], cwd=tmp, env={"SHELLGPT_DESKTOP_CWD": tmp, "PATH": "/usr/bin"})
...
assert spec["env"]["SHELLGPT_DESKTOP_CWD"] == tmp, "env= kwarg not captured"
...
run_py yes -c '
import subprocess, tempfile
exe = "/x/apps/desktop/release/linux-unpacked/ShellGPT"
...
run_py yes -c '
import subprocess
r = subprocess.run(["C:\\x\\apps\\desktop\\release\\win-unpacked\\ShellGPT.exe"], env={})

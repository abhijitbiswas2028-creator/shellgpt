...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^ShellGPT\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (shellgpt-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "shellgpt@nousresearch.com",
    "shellgpt-audit@example.com",
    "nousbot@nousresearch.com",
    "shellgpt@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "NousResearch/shellgpt-agent",
                "--state", "merged",

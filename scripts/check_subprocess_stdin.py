...
When ShellGPT runs in TUI mode, the gateway child process communicates with
...
# ``get_shellgpt_home() / "plugins"`` (user) and ``./.shellgpt/plugins/`` (project,
# gated behind ``SHELLGPT_ENABLE_PROJECT_PLUGINS``) — see
# ``shellgpt_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``shellgpt_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import shellgpt_constants (this script
    ...
    # plugins in get_shellgpt_home()/plugins/ and project plugins in
    # ./.shellgpt/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_shellgpt_home() / "plugins"]
    if os.environ.get("SHELLGPT_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".shellgpt" / "plugins")

"""Run a real ShellGPT CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_shellgpt_executable(shellgpt_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "shellgpt",
        Path(".venv") / "Scripts" / "shellgpt.exe",
    ):
        candidate = shellgpt_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("shellgpt")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "ShellGPT executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="ShellGPT source checkout containing .venv/bin/shellgpt",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated SHELLGPT_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["shellgpt.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['shellgpt.client.active']}"
        )
    [model] = by_name["shellgpt.model_route.count"]
    expected_model = {
        "name": "shellgpt.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['shellgpt.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["shellgpt.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['shellgpt.task_run.started']}"
        )
    [terminal] = by_name["shellgpt.task_run.finished"]
    ...
    [tool] = by_name["shellgpt.tool_call.count"]
    ...
    lifecycle = by_name["shellgpt.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["shellgpt.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The ShellGPT development environment requires jsonschema"
        ) from exc
    ...
    if metrics["shellgpt.client.active"] != [
        {
            "name": "shellgpt.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['shellgpt.client.active']}"
        )
    [model] = metrics["shellgpt.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['shellgpt.model_route.count']}"
        )
    [terminal] = metrics["shellgpt.task_run.finished"]
    ...
    [tool] = metrics["shellgpt.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["shellgpt.skill.lifecycle.count"]
    ...
    loads = metrics["shellgpt.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    shellgpt_repo = args.shellgpt_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    shellgpt = _resolve_shellgpt_executable(shellgpt_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="shellgpt-relay-shared-metrics-"))
    home = root / "shellgpt-home"
    ...
    try:
        ...
        python_paths = [str(shellgpt_repo)]
    ...
    (root / "shellgpt.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "shellgpt.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"ShellGPT exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("ShellGPT model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("ShellGPT did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("ShellGPT did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "shellgpt.shared_metrics.v2.schema.json",
    )

    print("ShellGPT -> NeMo Relay shared-metrics smoke test passed")

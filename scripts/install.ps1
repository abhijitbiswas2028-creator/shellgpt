# ============================================================================
# ShellGPT Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$ShellGPTHome = $(if ($env:SHELLGPT_HOME) { $env:SHELLGPT_HOME } else { "$env:LOCALAPPDATA\shellgpt" }),
    [string]$InstallDir = $(if ($env:SHELLGPT_HOME) { "$env:SHELLGPT_HOME\shellgpt-agent" } else { "$env:LOCALAPPDATA\shellgpt\shellgpt-agent" }),
    ...
    # builds apps/desktop into a launchable ShellGPT.exe.
    #
    # Why opt-in:
    #   * ShellGPT-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched ShellGPT.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live ShellGPT.exe
    ...
    #     `shellgpt desktop` already builds on demand.
...
# SHELLGPT_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[shellgpt] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'ShellGPTInstall.LongPath').Type) {
            Add-Type -Namespace 'ShellGPTInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [ShellGPTInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -ShellGPTHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('ShellGPTHome')) {
    $ShellGPTHome = ConvertTo-LongPath $ShellGPTHome
} else {
    $ShellGPTHome = ConvertTo-LongPath $(
        if ($env:SHELLGPT_HOME) { $env:SHELLGPT_HOME } else { "$env:LOCALAPPDATA\shellgpt" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SHELLGPT_HOME) { "$env:SHELLGPT_HOME\shellgpt-agent" } else { "$env:LOCALAPPDATA\shellgpt\shellgpt-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: ShellGPTHome=$ShellGPTHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:NousResearch/shellgpt-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/shellgpt-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * ShellGPT Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ShellGPTHome)) {
        New-Item -ItemType Directory -Force -Path $ShellGPTHome | Out-Null
    }
    $envFile = Join-Path $ShellGPTHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ShellGPTHome "node"
...
function Install-Uv {
    # ShellGPT owns its own uv at $ShellGPTHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (shellgpt_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `shellgpt update` stay in sync.
    ...
    Write-Info "Installing managed uv into $ShellGPTHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ShellGPTHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ShellGPTHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ShellGPTHome "bin"
        ...
        # (shellgpt_cli/managed_uv.py looks only at $ShellGPTHome\bin\uv.exe).
...
# Put the ShellGPT-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone shellgpt-setup.exe run, a user typing `npm`) silently
...
# Upgrade the ShellGPT-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "ShellGPT-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("shellgpt-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SHELLGPT_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by ShellGPT belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # shellgpt_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".shellgpt-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first ShellGPT-managed interpreter
    ...
    #
    # Under ShellGPT-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve ShellGPT-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun ShellGPT setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which ShellGPT uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\shellgpt\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    ShellGPT needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the ShellGPT copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\shellgpt\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SHELLGPT_GIT_BASH_PATH`` (User scope) so ShellGPT can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a ShellGPT-managed PortableGit install instead..."
    }

    # Download PortableGit into $ShellGPTHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ShellGPTHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent ShellGPT features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  ShellGPT needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SHELLGPT_GIT_BASH_PATH`` (User env scope) so ShellGPT can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ShellGPTHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ShellGPTHome\git\usr\bin\bash.exe
    $candidates += "$ShellGPTHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ShellGPTHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SHELLGPT_GIT_BASH_PATH", $candidate, "User")
            $env:SHELLGPT_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SHELLGPT_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- ShellGPT may not find Git Bash."
    Write-Info "If needed, set SHELLGPT_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the ShellGPT-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (ShellGPT requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy ShellGPT requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a ShellGPT-managed Node.js installation instead..."

    # Prefer a ShellGPT-managed Node from a previous run over a too-old system one.
    $managedNode = "$ShellGPTHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ShellGPTHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$ShellGPTHome\node"
        Write-Success "Node.js $version found (ShellGPT-managed)"
        ...
        Update-ManagedNpm "$ShellGPTHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing ShellGPT-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ShellGPTHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ShellGPTHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\shellgpt-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$ShellGPTHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$ShellGPTHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$ShellGPTHome\node") {
                    try {
                        Rename-Item "$ShellGPTHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "ShellGPT-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$ShellGPTHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ShellGPTHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\shellgpt-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `shellgpt
                    # update` path (#4735).
                    ...
                    $stashName = "shellgpt-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. shellgpt-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``shellgpt update`` and
                    ...
                    if ($LASTEXITCODE -ne 0) {
                        ...
                        if ($dropped -gt 0) {
                            ...
                            $rescueRef = "refs/shellgpt-update-backups/$rescueKind-$Branch-$rescueStamp-$($preResetSha.Substring(0, [Math]::Min(12, $preResetSha.Length)))"
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if ShellGPT behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running shellgpt processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/shellgpt-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/shellgpt-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/shellgpt-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\shellgpt-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\shellgpt-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `shellgpt update` (see the notes at the shared clone-path
    ...
    # next `shellgpt update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under ShellGPT-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "ShellGPT-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running shellgpt process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running shellgpt processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*ShellGPT_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (shellgpt.exe) plus its child tree.
            & taskkill /F /T /IM shellgpt.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM shellgpt.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m shellgpt_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not shellgpt.exe.
            ...
            # ShellGPT children re-exec through .shellgpt-runtime, so killing only
            ...
            # user (see shellgpt_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-ShellGPT " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `shellgpt update` / the
...
function Restore-VenvBackup {
    ...
    # venv so ShellGPT (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `shellgpt.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install shellgpt-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and shellgpt.exe broken with
    ...
    # here, not 30 seconds later when the user runs `shellgpt`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close ShellGPT processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close ShellGPT processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves ShellGPT and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register shellgpt.exe in dist-info/RECORD but fail to
        ...
        # `shellgpt` command while shellgpt-agent.exe / shellgpt-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m shellgpt_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `shellgpt dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\shellgpt_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `shellgpt dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `shellgpt dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: shellgpt_cli/web_server.py"
        }
    }
...
function Install-ShellGPTCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the shellgpt launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring shellgpt.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "shellgpt.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the shellgpt command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # shellgpt_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("shellgpt", "shellgpt-acp")) {
    ...
    $requiredExe = Join-Path $Destination "shellgpt.exe"
    $requiredCmd = Join-Path $Destination "shellgpt.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the shellgpt command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up shellgpt command..."
    
    if ($NoVenv) {
        $shellgptBin = "$InstallDir"
    } else {
        # $ShellGPTHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `shellgpt update`'s autostash
        ...
        # installer staged under shellgpt-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-ShellGPTCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $shellgptBin = "$ShellGPTHome\bin"
        Install-ShellGPTCommandLaunchers -Root $InstallDir -Destination $shellgptBin | Out-Null
    }
    ...
    #   shellgpt-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The shellgpt-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept shellgpt via $shellgptBin)"
        }
    }
    
    if ($currentPath -notlike "*$shellgptBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$shellgptBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $shellgptBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SHELLGPT_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\shellgpt instead
    # of the Unix default ~/.shellgpt
    $currentShellGPTHome = [Environment]::GetEnvironmentVariable("SHELLGPT_HOME", "User")
    if (-not $currentShellGPTHome -or $currentShellGPTHome -ne $ShellGPTHome) {
        [Environment]::SetEnvironmentVariable("SHELLGPT_HOME", $ShellGPTHome, "User")
        Write-Success "Set SHELLGPT_HOME=$ShellGPTHome"
    }
    $env:SHELLGPT_HOME = $ShellGPTHome
    
    # Update current session
    $env:Path = "$shellgptBin;$env:Path"
    
    Write-Success "shellgpt command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.shellgpt-bootstrap-complete which tells the ShellGPT
    ...
    # ShellGPT-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `shellgpt update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".shellgpt-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SHELLGPT_HOME directory structure ($ShellGPTHome, default %LOCALAPPDATA%\shellgpt)
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShellGPTHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ShellGPTHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ShellGPTHome\config.yaml"
    ...
    # (the default PS5 behaviour), and ShellGPT's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ShellGPTHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in shellgpt_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ShellGPTHome"
    
    # Seed bundled skills into $ShellGPTHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ShellGPTHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $ShellGPTHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ShellGPTHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ShellGPTHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (ShellGPT-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'shellgpt setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\shellgpt-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\shellgpt-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\shellgpt-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `shellgpt tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $ShellGPTHome "bin"
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside ShellGPT' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'shellgpt tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `shellgpt
...
# paths still lazy-install via install_cua_driver() (shellgpt_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `shellgpt computer-use install` runs. Bounded
        ...
        # ShellGPT' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'shellgpt tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: shellgpt computer-use install"
            }
        } else {
...
# the final `electron` -> `ShellGPT` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable ShellGPT.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_shellgpt_desktop command
    # resolves apps/desktop/release/win-unpacked/ShellGPT.exe directly,
    ...
    # enforces the supported Node lines and prepends the ShellGPT-managed Node to
    ...
    # The --dir mode produces an unpacked ShellGPT.exe in
    ...
    # are NOT trying to work around). The ShellGPT icon + product name are
    # stamped onto ShellGPT.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\shellgpt-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\ShellGPT.exe",
        "$desktopDir\release\win-arm64-unpacked\ShellGPT.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no ShellGPT.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The ShellGPT icon + identity are stamped onto ShellGPT.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, shellgpt-agent#38216).
    ...
    #    ShellGPT.exe. We deliberately do NOT point them at `shellgpt desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'ShellGPT.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'ShellGPT.lnk')
        )
        ...
        # with the ShellGPT icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.shellgpt/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'shellgpt setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m shellgpt_cli.main setup
    } else {
        python -m shellgpt_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $shellgptCmd = "$InstallDir\venv\Scripts\shellgpt.exe"
    if (-not (Test-Path $shellgptCmd)) {
        $shellgptCmd = "shellgpt"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ShellGPTHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'shellgpt whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: shellgpt gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ShellGPTHome\logs\gateway.log"
            Start-Process -FilePath $shellgptCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ShellGPTHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: shellgpt gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: shellgpt gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ShellGPTHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShellGPTHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShellGPTHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShellGPTHome\shellgpt-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   shellgpt              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   shellgpt setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   shellgpt config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   shellgpt config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   shellgpt gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   shellgpt update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning ShellGPT repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.shellgpt-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (ShellGPT-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding ShellGPT to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://shellgpt-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow

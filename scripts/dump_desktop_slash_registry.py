...
Run after changing any ``desktop=`` value or alias in ``shellgpt_cli/commands.py``;
``tests/shellgpt_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from shellgpt_cli.commands import desktop_surface_registry

 ...
 *   The artifact published on the website right now -- ShellGPT-Setup.exe has
 ...
 * @typedef {InstallMethod | 'shellgpt-update' | 'open-app-update' | 'shellgpt-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; shellgpt-desktop-app-update starts the app
 *   via `shellgpt desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://shellgpt.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds ShellGPT.exe AND
      ...
      // Website ShellGPT-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `shellgpt desktop`.
      { method: 'shellgpt-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published ShellGPT-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `shellgpt desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'shellgpt-desktop-app-update';
}

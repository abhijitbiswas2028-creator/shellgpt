...
# (%SHELLGPT_HOME%\shellgpt-setup.exe). That binary has no self-update path --
# copy_self_to_shellgpt_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `shellgpt update` refreshes
...
#     -InstallRoot <path>   repo checkout (SHELLGPT_HOME\shellgpt-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] ShellGPT.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .shellgpt-update-in-progress in place (tests)
#
...
# .shellgpt-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim SHELLGPT_HOME\.shellgpt-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining SHELLGPT_UPDATE_STARTED_AT from the Desktop hand-off.
# shellgpt_cli/update_lock.py's ancestry rule lets our
# `shellgpt update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new ShellGPT.exe pid.
try {
    Add-Type -Namespace ShellGPTHandoff -Name Win32 -MemberDefinition @'
...
$ShellGPTHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $ShellGPTHome ".shellgpt-update-in-progress"
$LogDir = Join-Path $ShellGPTHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $ShellGPTHome ".shellgpt-update-result.json"
$script:Ui = $null
$script:UiStage = "ShellGPT will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale shellgpt-update-ui-* leftovers from interrupted
    ...
    # update marker (.shellgpt-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "shellgpt-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-ShellGPTStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("shellgpt-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [ShellGPTHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `shellgpt debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"shellgpt debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [ShellGPTHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen ShellGPT themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old ShellGPT.exe before writing the replacement,
    ...
    # DO NOT spawn ShellGPT.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [ShellGPTHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [ShellGPTHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [ShellGPTHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh ShellGPT process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [ShellGPTHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `shellgpt update` deliberately runs its build steps with stdout
# inherited (shellgpt_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:SHELLGPT_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:SHELLGPT_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:SHELLGPT_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:SHELLGPT_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `shellgpt update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (shellgpt_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:SHELLGPT_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:SHELLGPT_UPDATE_PROGRESS_LOG
}
...
if (-not ("ShellGPTUpdateJob" -as [type])) {
...
public static class ShellGPTUpdateJob {
...
function Invoke-ShellGPTStep([string]$Exe, [string[]]$ShellGPTArgs, [string]$Tag) {
    ...
    # afterwards, where `shellgpt debug share` picks it up.
    #
    ...
    # .shellgpt-update-result.json, clearing .shellgpt-update-in-progress,
    ...
    # ShellGPT" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($ShellGPTArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [ShellGPTUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `shellgpt update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [ShellGPTUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
function Resolve-ShellGPTVenvDir([string]$Root) {
    # Match shellgpt_constants.project_venv_dir(): installer-created venv wins,
...
# the page can be QA'd in any browser; SHELLGPT_SELFTEST_FAIL=1 exercises the
# error state, SHELLGPT_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:SHELLGPT_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SHELLGPT_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:SHELLGPT_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-ShellGPTStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no ShellGPT install to
...
#            update log, the shape of every real `shellgpt update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:SHELLGPT_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SHELLGPT_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:SHELLGPT_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:SHELLGPT_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "shellgpt-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "shellgpt-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "shellgpt-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "shellgpt-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "shellgpt-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "shellgpt-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "shellgpt-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "shellgpt-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `shellgpt update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-ShellGPTStep $powershell @(
    ...
    $flood = Invoke-ShellGPTStep $powershell @(
    ...
    $stall = Invoke-ShellGPTStep $powershell @(
    ...
    try {
        $logstall = Invoke-ShellGPTStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-ShellGPTStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-ShellGPTStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:SHELLGPT_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from SHELLGPT_HOME; pin the process directory to the
    ...
    $VenvDir = Resolve-ShellGPTVenvDir $InstallRoot
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-ShellGPTStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for ShellGPT to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the ShellGPT window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close ShellGPT fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing ShellGPT files"
    $shim = Join-Path $VenvDir "Scripts\shellgpt.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the ShellGPT install open ($shim locked after 20s). Nothing was changed. Close other ShellGPT windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the shellgpt.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\shellgpt.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_shellgpt_exe must first rename the running shellgpt.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that shellgpt.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second ShellGPT window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a ShellGPT.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m shellgpt_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the ShellGPT installer or `shellgpt doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    ...
    $updateArgs = @("-m", "shellgpt_cli.main", "update", "--yes") + $gatewayArg + @("--force", "--branch", $Branch)
    ...
    # collides with the "close all ShellGPT windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m shellgpt_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed shellgpt predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-ShellGPTStep $pythonExe $updateArgs "update"
    Write-HandoffLog "shellgpt update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-ShellGPTUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `shellgpt update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "shellgpt update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-ShellGPTStep $pythonExe @("-m", "shellgpt_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import shellgpt_cli.main; from shellgpt_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-ShellGPTStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated ShellGPT runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    # before `shellgpt update` captures its Windows pause inventory, leaving the
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed -and -not $NoGateway) {
        $gatewayRestart = Invoke-ShellGPTStep $pythonExe @("-m", "shellgpt_cli.main", "gateway", "start", "--all") "gateway restart"
        if ($gatewayRestart.Code -ne 0) {
            ...
            $manualAction = $true
            $manualMsg = "Update complete, but ShellGPT could not restart every messaging gateway. Run `shellgpt gateway start --all` in a terminal."
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `shellgpt desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `shellgpt debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "ShellGPT is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. ShellGPT was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen ShellGPT."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening ShellGPT"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen ShellGPT to finish (it could not restart itself)."

#!/bin/bash
# repro.sh -- reproduce desktop-update paths against a sandboxed SHELLGPT_HOME.
#
# Nothing here touches your real ~/.shellgpt or checkout. Each mode builds (or
# reuses) a disposable install under $TMPDIR and drives the REAL code path --
# the actual installer, the actual orchestrator, the actual `shellgpt update`.
#
#   repro.sh shim          shim UI only: success event after 6s
#   repro.sh shim-fail     shim UI only: error event after 6s
#   repro.sh fresh         fresh install into a sandbox SHELLGPT_HOME
...
SANDBOX="${SHELLGPT_UPDATE_REPRO_HOME:-/tmp/shellgpt-update-repro}"
SANDBOX_ROOT="$SANDBOX/shellgpt-agent"

say() { printf '\n\033[1m== %s ==\033[0m\n' "$1"; }

ensure_sandbox_install() {
  if [ -x "$SANDBOX_ROOT/venv/bin/shellgpt" ]; then
  ...
  SHELLGPT_HOME="$SANDBOX" bash "$SANDBOX_ROOT/scripts/install.sh" --non-interactive --skip-setup --shellgpt-home "$SANDBOX"
}

case "$MODE" in
  shim)
    SHELLGPT_SELFTEST_HOLD_SECONDS="${SHELLGPT_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  shim-fail)
    SHELLGPT_SELFTEST_FAIL=1 SHELLGPT_SELFTEST_HOLD_SECONDS="${SHELLGPT_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  fresh)
    rm -rf "$SANDBOX"
    ensure_sandbox_install
    say "fresh install OK: $("$SANDBOX_ROOT/venv/bin/shellgpt" --version 2>/dev/null || echo '?')"
    ;;
  behind)
    N="${2:-25}"
    ensure_sandbox_install
    ...
    SHELLGPT_HOME="$SANDBOX" bash "$SCRIPT_DIR/posix.sh" \
      --install-root "$SANDBOX_ROOT" --branch main --desktop-pid 0 || true
    say "result file:"
    cat "$SANDBOX/.shellgpt-update-result.json" 2>/dev/null || echo "(none written)"
    echo
  ...
  gate)
    ...
    G="$(mktemp -d -t shellgpt-gate-test.XXXXXX)"
    UNPACKED="$G/shellgpt-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    touch "$UNPACKED/shellgpt" && chmod +x "$UNPACKED/shellgpt"
    ...
    decide() { bash "$SCRIPT_DIR/posix.sh" --self-test-gate --install-root "$G/shellgpt-agent" "$@" | cut -d: -f1; }

    expect "appimage (not under unpacked)"      skew     "$(decide --relaunch-target /opt/ShellGPT/shellgpt)"
    expect "sibling-prefix dir not fooled"      skew     "$(decide --relaunch-target "$UNPACKED-evil/shellgpt")"
    expect "no chrome-sandbox (namespace)"      relaunch "$(decide --relaunch-target "$UNPACKED/shellgpt")"

    touch "$UNPACKED/chrome-sandbox"
    expect "sandbox not root/setuid"            manual   "$(decide --relaunch-target "$UNPACKED/shellgpt")"
    expect "opt-out: --sandbox-fallback"        relaunch "$(decide --relaunch-target "$UNPACKED/shellgpt" --sandbox-fallback)"
    expect "opt-out: --no-sandbox launch arg"   relaunch "$(decide --relaunch-target "$UNPACKED/shellgpt" -- --no-sandbox)"
    expect "opt-out: ELECTRON_DISABLE_SANDBOX"  relaunch "$(ELECTRON_DISABLE_SANDBOX=1 decide --relaunch-target "$UNPACKED/shellgpt")"
    ...
    QHOME="$G/qhome"; mkdir -p "$QHOME/shellgpt-agent"
    bash "$SCRIPT_DIR/posix.sh" --no-ui --no-marker-cleanup --desktop-pid 0 \
      --install-root "$QHOME/shellgpt-agent" --branch 'evil"branch\n$(x)' >/dev/null 2>&1 || true
    if python3 -c "import json,sys; d=json.load(open('$QHOME/.shellgpt-update-result.json')); sys.exit(0 if d['branch']=='evil\"branch\\\\n\$(x)' and d['ok']==False else 1)"; then
  ...
  launch)
    ...
    # a fake install whose `shellgpt` stub exits 0 instantly, so the flow
    # reaches finish() with FINAL_CODE=0 and exercises the launch leg.
    L="$(mktemp -d -t shellgpt-launch-test.XXXXXX)"
    fails=0
    expect_msg() { # name python-expr
      if python3 -c "import json,sys; d=json.load(open('$L/.shellgpt-update-result.json')); sys.exit(0 if ($2) else 1)"; then
        printf 'ok   %s\n' "$1"
      else
        printf 'FAIL %s -> %s\n' "$1" "$(cat "$L/.shellgpt-update-result.json" 2>/dev/null)"; fails=$((fails+1))
      fi
    }
    stub_install() { # creates a fake install whose shellgpt update succeeds
      rm -rf "$L"; mkdir -p "$L/shellgpt-agent/venv/bin"
      printf '#!/bin/sh\nexit 0\n' > "$L/shellgpt-agent/venv/bin/shellgpt"
      chmod +x "$L/shellgpt-agent/venv/bin/shellgpt"
    }

    # 1. linux relaunch target dies instantly -> manual downgrade in result
    stub_install
    UNPACKED="$L/shellgpt-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    printf '#!/bin/sh\nexit 1\n' > "$UNPACKED/shellgpt"; chmod +x "$UNPACKED/shellgpt"
    if [ "$(uname)" != "Darwin" ]; then
      bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/shellgpt-agent" \
        --relaunch-target "$UNPACKED/shellgpt" >/dev/null 2>&1 || true
      expect_msg "instant-exit relaunch downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen ShellGPT' in d['message']"
    else
      ...
      expect_msg "missing bundle downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen ShellGPT' in d['message']"
    fi
    ...
    stub_install
    bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/shellgpt-agent" \
      --relaunch-target /opt/ShellGPT/shellgpt >/dev/null 2>&1 || true
    if [ "$(uname)" != "Darwin" ]; then

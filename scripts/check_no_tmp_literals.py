...
distributions it is a RAM-backed tmpfs that fills under ShellGPT load. ShellGPT resolves scratch
space through one helper (``shellgpt_constants.get_scratch_dir()`` → ``SHELLGPT_HOME/cache/scratch``,
which every ShellGPT process also exports as ``TMPDIR``/``TMP``/``TEMP``), and prompts + skills
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"shellgpt_constants.get_scratch_dir() (or $TMPDIR / tempfile, which ShellGPT points there), tell the "

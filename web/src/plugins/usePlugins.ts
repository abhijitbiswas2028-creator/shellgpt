...
import { api, SHELLGPT_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "shellgpt:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SHELLGPT_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SHELLGPT_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?shellgpt_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-shellgpt-plugin", manifest.name);

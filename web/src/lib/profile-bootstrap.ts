declare global {
  interface Window {
    __SHELLGPT_DASHBOARD_PROFILE__?: string;
    __SHELLGPT_INITIAL_PROFILE__?: string;
  }
}

export function dashboardInitialProfile(): string {
  if (typeof window === "undefined") return "";
  return window.__SHELLGPT_INITIAL_PROFILE__ ?? "";
}

/**
 * The profile this backend process itself serves, injected by the server and
 * empty when it cannot be named unambiguously (custom SHELLGPT_HOME).
 *
...
export function dashboardServingProfile(): string {
  if (typeof window === "undefined") return "";
  return window.__SHELLGPT_DASHBOARD_PROFILE__ ?? "";
}

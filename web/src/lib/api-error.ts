...
export const API_UNREACHABLE_MESSAGE =
  "ShellGPT dashboard cannot reach the ShellGPT service. Is `shellgpt dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The ShellGPT service hit an internal error.",
  502: "The dashboard proxy could not reach the ShellGPT service.",
  503: "The ShellGPT service is not ready yet. Try again in a moment.",
  504: "The ShellGPT service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The ShellGPT service answered with an unexpected error (${status}).`;
}

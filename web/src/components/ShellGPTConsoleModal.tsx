...
interface ShellGPTConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ShellGPTConsoleModal({ open, onClose }: ShellGPTConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("shellgpt> ");
  const inputPromptRef = useRef("shellgpt> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "shellgpt> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to ShellGPT Console...\x1b[0m");
    ...
    void (async () => {
      try {
        ...
        ws.onclose = (ev) => {
          ...
          const message =
            ev.code === 1006 && !hasReadyFrameRef.current
              ? "Console could not connect to the dashboard server. Check that `shellgpt dashboard` is running, then click Reconnect."
              : ev.code === 1000
                ? "Console closed."
        ...
        writeLine(
          term,
          "\x1b[31mConsole could not connect to the dashboard server. Check that `shellgpt dashboard` is running, then click Reconnect.\x1b[0m",
        );
      }
    })();
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="shellgpt-console-title"
    >
      <div
        className={cn(
          themedBody,

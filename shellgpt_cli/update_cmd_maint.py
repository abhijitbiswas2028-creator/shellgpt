"""Post-update maintenance for ``shellgpt update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary.

Split out of ``update_cmd.py``, which re-imports every name so ``shellgpt_cli.update_cmd.<name>``
...
def _sqlite_partial_completion_lines(sqlite_version: str) -> list[str]:
    ...
    return [
        ...
        "corruption bug. ShellGPT works, but sessions could be damaged.",
        f"  Fix: run the installer again ({_REINSTALL_ONE_LINER[bool(_m()._is_windows())]}) "
        "which installs a safe Python, then run `shellgpt doctor` to confirm.",
    ]
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  shellgpt curator run --dry-run")
    print("  Pause it:     shellgpt curator pause")
    print("  Docs:         https://shellgpt-agent.nousresearch.com/docs/user-guide/features/curator")
...
def _print_fts_optimize_available_notice() -> None:
    ...
    if interrupted:
        ...
        print(
            "  A previous `shellgpt sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    shellgpt sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  shellgpt sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: shellgpt curator status)")
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    try:
        ...
        stop_result = _m()._kill_stale_dashboard_processes(
            ...
            scope_home=str(get_shellgpt_home()),
        )
        ...
        print("    shellgpt dashboard --port <port>   (or: systemctl --user restart shellgpt-dashboard)")
        return
    ...
    print("    shellgpt dashboard --port <port>")
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from shellgpt_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("SHELLGPT_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== shellgpt-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from shellgpt_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next ShellGPT runtime."""
    from shellgpt_cli.update_cmd import _post_update_sqlite_runtime_status
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from shellgpt_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `shellgpt desktop` to retry the desktop rebuild.")
    ...
    with suppress(Exception):
        from shellgpt_cli.gateway_multiplex_mode import consume_rewritten_notice, recorded_standalone_warning_lines
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from shellgpt_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    ...
    if holders:
        print(
            ...
            "state.db or its WAL open. Stop them (shellgpt gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart ShellGPT) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from shellgpt_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from shellgpt_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: shellgpt skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v shellgpt'`` resolves."""
    from shellgpt_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/shellgpt).
    fhs_link = Path("/usr/local/bin/shellgpt")
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `shellgpt gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``shellgpt-acp`` launcher next to ``shellgpt`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``shellgpt acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$ShellGPTHome\bin``, never
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        shellgpt_cmd = bin_dir / "shellgpt"
        acp_cmd = bin_dir / "shellgpt-acp"
        try:
            if not (shellgpt_cmd.is_file() or shellgpt_cmd.is_symlink()):
                continue
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_shellgpt_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_shellgpt_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from shellgpt_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from shellgpt_cli.backup import create_pre_update_snapshots_all_profiles
...
def _run_full_backup() -> None:
    """Zip SHELLGPT_HOME under ``backups/`` (restorable via ``shellgpt import``). Never raises."""
    try:
    ...
    # display_shellgpt_home so the user sees ~/.shellgpt/...
    try:
        from shellgpt_constants import get_shellgpt_home, display_shellgpt_home
        display_path = f"{display_shellgpt_home()}/{out_path.relative_to(get_shellgpt_home())}"
    except Exception:
        display_path = str(out_path)

    print(f"  Saved:    {display_path} ({format_bytes(size_bytes)}, {elapsed:.1f}s)")
    print(f"  Restore:  shellgpt import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of SHELLGPT_HOME under ``backups/`` (``shellgpt import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in shellgpt_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # SHELLGPT_HOME, so sync_skills()'s module-level SHELLGPT_HOME cache can't skew it.
    with suppress(Exception):
        from shellgpt_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from shellgpt_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from shellgpt_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from shellgpt_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from shellgpt_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from shellgpt_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('shellgpt-acp launcher self-heal failed: %s', _ensure_acp_launcher),
        ...
        # Legacy SHELLGPT_NEMO_RELAY_ATIF_*/ATOF_* vars produce no traces since the Relay cutover;
...
def _migrate_relay_exporter_env() -> None:
    from shellgpt_cli.relay_plugin_migrate import run_relay_migration_after_update
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from shellgpt_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if ShellGPT re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.nousresearch.shellgpt` (repeat "
    ...
    try:
        # See #95596.
        from shellgpt_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from shellgpt_cli.model_catalog import seed_cache_from_checkout

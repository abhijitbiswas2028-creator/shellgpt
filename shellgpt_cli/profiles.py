"""Profile management for multiple isolated ShellGPT instances."""
...
from shellgpt_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
...
_PROFILE_ID_RE = PROFILE_ID_RE  # legacy alias; shellgpt_constants.PROFILE_ID_RE is canonical
...
# stay visible; containers persist HOME state here). See shellgpt_constants.get_subprocess_home().
...
# (``~/.shellgpt``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# ``shellgpt_cli.backup._EXCLUDED_ROOT_DIRS`` so the two lists cannot drift.
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "shellgpt-agent", ".worktrees", "profiles", "bin", "node_modules",
}) | LOCAL_RUNTIME_ROOT_DIRS

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `shellgpt backup` archives, quick-backup
...
# Marker written by `shellgpt profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `shellgpt update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this ShellGPT profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_shellgpt_home().resolve():
...
# Allow-list for ``export_profile("default")``: when SHELLGPT_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known ShellGPT profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``shellgpt_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"shellgpt", "default", "test", "tmp", "root", "sudo"})

# ShellGPT subcommands that cannot be used as profile names/aliases
_SHELLGPT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the shellgpt root (NOT the current SHELLGPT_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_shellgpt_home() / "profiles"


def _get_default_shellgpt_home() -> Path:
    """Default (pre-profile) SHELLGPT_HOME: ``~/.shellgpt``, or SHELLGPT_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from shellgpt_constants import get_default_shellgpt_root
    return get_default_shellgpt_root()


def _get_active_profile_path() -> Path:
    return _get_default_shellgpt_home() / "active_profile"
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a ShellGPT-generated wrapper (contains ``shellgpt -p``)."""
    try:
        return "shellgpt -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: shellgpt profile create {canon}")
...
def _unknown_profile_error(canon: str) -> FileNotFoundError:
    ...
    return FileNotFoundError(f"No profile named '{canon}'. See your profiles with: shellgpt profile list")


def _profile_exists_error(canon: str) -> FileExistsError:
    return FileExistsError(
        f"A profile named '{canon}' already exists. Switch to it with `shellgpt profile use {canon}`, "
        "see all profiles with `shellgpt profile list`, or choose a different name."
    )
...
def _invalid_profile_name_error(name: str) -> ValueError:
    ...
    return ValueError(
        ...
        f"Then run `shellgpt profile create {suggestion}`."
    )
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.shellgpt
    if not _PROFILE_ID_RE.match(name):
        raise _invalid_profile_name_error(name)
    if name in _RESERVED_NAMES:
        raise ValueError(
            f"Profile name {name!r} is reserved — it collides with either "
            f"the ShellGPT installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SHELLGPT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_shellgpt_home()
    ...
    # profiles/shellgpt may still exist and must keep resolving.
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    shell into a "real" profile on the next ``shellgpt update`` (#95188, #94823, #99392)."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _SHELLGPT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a shellgpt subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            shellgpt_exe = shutil.which("shellgpt") or "shellgpt"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(shellgpt_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `shellgpt doctor --fix` surfaces
    ...
    with contextlib.suppress(Exception):
        ...
        from shellgpt_cli.config import check_config_version, migrate_config
        token = set_shellgpt_home_override(str(profile_dir))
        try:
...
# wrappers are a few hundred bytes with the ``shellgpt -p X`` needle near the top; the wrapper
...
class ProfileInfo:
    ...
    display_name: str = ""
    # Bot Mode title (``profile.yaml`` ``ui_meta['shellgpt-bots'].title``) — the name
    # the Bots roster shows. Presentation-only, like ``display_name``.
    bot_title: str = ""
    # Canonical ids this profile was previously known by (``shellgpt profile rename``
    # appends here). Lets Bot Mode group chats re-link persisted member
    # descriptors to the renamed live profile (#110200).
    previous_names: List[str] = field(default_factory=list)
    # Backend-assigned role (``SETUP_ROLE`` or None). Only ``shellgpt_cli.setup_profile`` writes it.
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `shellgpt model` sets it later
        from shellgpt_constants import get_shellgpt_home
        from shellgpt_cli.config import atomic_config_write, read_user_config_raw
        source = get_shellgpt_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating SHELLGPT_HOME.
...
def _count_skills(profile_dir: Path) -> int:
    ...
    synchronously when stale — detail surfaces (``shellgpt profile info``, ``profiles.describe``)
...
def _cached_skill_count(profile_dir: Path) -> int:
    ...
    if due:
        threading.Thread(target=_count_skills, args=(profile_dir,),
                         name="shellgpt-skill-count", daemon=True).start()
    cached = _SKILL_COUNT_CACHE.get(key)
    return cached[2] if cached is not None else 0


# profile.yaml — per-profile metadata (description, role, etc.)
# Deliberately tiny and separate from ``config.yaml`` (user-facing ShellGPT config, ~5000
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    corrupt file on one profile must not break ``shellgpt profile list``."""
    def _read() -> dict:
        data = _load_yaml_dict(profile_dir / "profile.yaml") or {}
        ui_meta = data.get("ui_meta")
        bot_title = ""
        if isinstance(ui_meta, dict):
            shellgpt_bots = ui_meta.get("shellgpt-bots")
            if isinstance(shellgpt_bots, dict):
                bot_title = str(shellgpt_bots.get("title") or "").strip()
        return {
...
def list_profiles(*, lazy_skill_count: bool = False) -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_shellgpt_home()
...
def profile_is_standalone(home: Path) -> bool:
    ...
    if home == _get_default_shellgpt_home():
...
def profiles_to_serve(multiplex: bool, *, include_standalone: bool = False,
    ...
    """``(profile_name, shellgpt_home)`` pairs a gateway should serve — the single chokepoint
    ...
    out of the host multiplexer; a ``gateway.parked`` marker (``shellgpt -p X gateway stop``) skips
    ...
    default = _get_default_shellgpt_home()
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from shellgpt_cli.auth import strip_cloned_single_use_oauth_grants
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    ``sync_imports`` also copies the source's ``import-sync.json`` (the ``shellgpt import-agent``
    manifest) so the clone stays registered against the same external Claude Code / Codex trees
    and ``shellgpt -p <clone> import-agent --sync`` keeps pulling from them. The link is to the
    ...
    for relpath in _CLONE_SUBDIR_FILES:
        _clone_file(source_dir, profile_dir, relpath)
    from shellgpt_cli.profile_memory_config import active_memory_provider, clone_memory_provider_config
    clone_memory_provider_config(source_dir, profile_dir,
                                 active_memory_provider(_load_yaml_dict(source_dir / "config.yaml")))
    if sync_imports:
        from shellgpt_cli.agent_import_sync import SYNC_MANIFEST_NAME  # lazy: keeps yaml/utils off the hot startup path
...
def create_profile(
    ...
    makes two gateways fight over one bot (``shellgpt_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``shellgpt update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.shellgpt).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from shellgpt_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from shellgpt_cli.profile_channels import strip_channel_settings
    ...
    # `shellgpt -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _finish_profile_layout(profile_dir: Path, *, no_skills: bool, clone_all: bool,
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `shellgpt -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from shellgpt_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `shellgpt update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`shellgpt profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `shellgpt profile describe` works later
...
def _notify_multiplexer(canon: str) -> None:
    from shellgpt_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _purge_identity(canon: str) -> bool:
    ...
    from shellgpt_cli.profile_identity import purge_profile_identity
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from shellgpt_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches SHELLGPT_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SHELLGPT_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_shellgpt_home() / ".env"
...
_SHELLGPT_ARGV_MARKERS = ("shellgpt_cli.main", "shellgpt-gateway", "tui_gateway")
...
# `findOnPath('shellgpt')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "shellgpt".
...
# argv[1] is matched against exact names, not ``startswith("shellgpt")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("shellgpt-notes.py").
_SHELLGPT_CONSOLE_SCRIPT_NAMES = frozenset({"shellgpt", "shellgpt-agent", "shellgpt-acp"})


def _is_shellgpt_argv(argv: list) -> bool:
    """True for a ShellGPT process: entrypoint marker in argv, executable named ``shellgpt*``,
    or a python interpreter directly exec'ing a known ``shellgpt`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _SHELLGPT_ARGV_MARKERS) or exe_name.startswith("shellgpt"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _SHELLGPT_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running ShellGPT *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`shellgpt -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_shellgpt_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by SHELLGPT_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("SHELLGPT_HOME", "")
...
def _stop_bot_desktop(profile_dir: Path) -> None:
    ...
    gateway shutdown does not reach it (its own session, its own pid file). Scoped through the shellgpt-home
    ...
    if not runtime.is_supported_host():
        return
    token = set_shellgpt_home_override(profile_dir)
    try:
        if runtime.stop():
...
class ProfileIdentitySettlementPending(RuntimeError):
    ...
    settled (``shellgpt_cli.profile_identity.purge_profile_identity`` returned False).

    Subclasses ``RuntimeError`` so delete-failure handling that treats the error as fatal (the
    CLI's ``shellgpt profile delete``) keeps working unchanged; surfaces that can report a partial
    ...
    def __init__(self, profile: str, path: Path):
        ...
        self.path = path
        self.retry_command = f"shellgpt profile purge-identity {profile}"
        super().__init__(
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.shellgpt).\nTo remove everything, use: shellgpt uninstall")
    ...
    shutdown_mcp_servers(scope=shellgpt_home_key(profile_dir))
    ...
    with contextlib.suppress(Exception):
        from shellgpt_state_registry import close_all_under as _close_session_dbs_under
    ...
    with contextlib.suppress(Exception):
        from shellgpt_logging import release_profile_log_handlers
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``SHELLGPT_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # The service name follows get_shellgpt_home(): bind the override (the seam a multiplexed
    ...
    from shellgpt_constants import reset_shellgpt_home_override, set_shellgpt_home_override
    old_home = os.environ.get("SHELLGPT_HOME")
    home_token = set_shellgpt_home_override(str(profile_dir))
    try:
        os.environ["SHELLGPT_HOME"] = str(profile_dir)
        from shellgpt_cli.gateway import get_service_name, get_launchd_plist_path, user_systemd_unit_dir
        ...
        if old_home is not None:
            os.environ["SHELLGPT_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's shellgpt_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded SHELLGPT_HOME "
...
def get_active_profile(root: Path | None = None) -> str:
    """Read the sticky active profile name (of *root*, default: this process's ShellGPT root)."""
...
def get_active_profile_name() -> str:
    """Profile name inferred from SHELLGPT_HOME: ``"default"`` when unset or ``~/.shellgpt``, the
    name under ``~/.shellgpt/profiles/<name>``, ``"custom"`` for any other path."""
    from shellgpt_constants import get_shellgpt_home
    resolved = get_shellgpt_home().resolve()
    if resolved == _get_default_shellgpt_home().resolve():
        return "default"
...
def current_profile_name(default: str | None = None) -> str | None:
    """Identity of the profile the current task runs FOR: the ``SHELLGPT_HOME`` override when one is
    bound (a multiplexed cron tick, a routed gateway turn), else a launcher-pinned
    ``SHELLGPT_PROFILE_NAME``/``SHELLGPT_PROFILE`` (the kanban dispatcher pins it on its workers), else
    the name derived from the process's ``SHELLGPT_HOME``; *default* when nothing names a profile.
    ...
    if get_shellgpt_home_override() is None:
        for env_name in ("SHELLGPT_PROFILE_NAME", "SHELLGPT_PROFILE"):
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when SHELLGPT_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_shellgpt_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point SHELLGPT_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/shellgpt-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".shellgpt-profile-exports", Path(tempfile.gettempdir()) / f"shellgpt-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    SHELLGPT_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-ShellGPT file the user may have lying alongside SHELLGPT_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``shellgpt
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.shellgpt (dir name ".shellgpt"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: shellgpt profile import <archive> --name <name>"
        )
    ...
    # target ~/.shellgpt itself.
    canon = _canon_valid(inferred_name)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.shellgpt). "
            "Specify a different name: shellgpt profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise _profile_exists_error(canon)
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="shellgpt_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"shellgpt_{old_name}"
    legacy_old_host = f"shellgpt.{old_name}"
    new_host = f"shellgpt_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_shellgpt_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``shellgpt_<old>`` or legacy ``shellgpt.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_shellgpt_home`` does not refuse it) and the periodic reconcile re-adopts the
    ...
    from shellgpt_constants import shellgpt_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=shellgpt_home_key(old_dir))
    ...
    from shellgpt_cli.profile_identity import _migrate_profile_identity
...
def profile_root_for_env_home(env_home: str, default_root: Path) -> Path:
    """ShellGPT root named by an exported ``SHELLGPT_HOME``: the grandparent of a profile-shaped value
    (``<root>/profiles/<name>``, mirrors ``get_default_shellgpt_root()``), the value itself otherwise,
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SHELLGPT_HOME path string. Called early in the CLI entry
    point, before shellgpt modules are imported, to set SHELLGPT_HOME.

    When SHELLGPT_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match ShellGPT-owned PYTHONPATH
    ...
    root = profile_root_for_env_home(os.environ.get("SHELLGPT_HOME", ""), _get_default_shellgpt_home())
    if canon == "default":
        return str(root)

...
Every ShellGPT process that may need the free tier runs this once at boot (``shellgpt serve`` on a
...
(:func:`shellgpt_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
def reconcile_record() -> Optional[SetupRecord]:
    ...
    writes this process never saw (``shellgpt setup`` / ``shellgpt model`` from a shell, a hand edit).
    ...
    if shellgpt_home_key() != shellgpt_home_key(get_process_shellgpt_home()) or _inventory_stamp == _config_stamp():
        return record
...
def start_background_bootstrap() -> threading.Thread:
    """``shellgpt serve`` entry: run on a daemon thread so a slow portal never delays the socket."""

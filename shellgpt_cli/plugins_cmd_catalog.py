"""``shellgpt plugins`` catalog surface: resolution, provenance sidecar, search/browse/info/validate,
catalog-aware update, plus the dashboard/TUI-facing catalog payload helpers.

Sibling of :mod:`shellgpt_cli.plugins_cmd` (the installer core, enable/disable state and console helpers
...
from shellgpt_cli.plugin_catalog import (
...
CATALOG_SIDECAR = ".shellgpt-catalog.json"
...
def looks_like_catalog_name(identifier: str) -> bool:
    ...
    from shellgpt_cli.plugins_cmd import _URL_SCHEMES
...
def raise_if_removed(*candidates: str) -> None:
    ...
    from shellgpt_cli.plugins_cmd import PluginOperationError
    for candidate in candidates:
        ...
        if removed is not None:
            ...
            raise PluginOperationError(
                f"Plugin '{removed.name}' was removed from the ShellGPT plugin catalog and is blocked from "
...
def resolve_catalog_name(identifier: str, console) -> PluginCatalogEntry:
    ...
    from shellgpt_cli.plugins_cmd import _fail
    entry = get_live_catalog_entry(identifier)
    if entry is None:
        _fail(console, (
            f"[red]Error:[/red] '{identifier}' is not in the ShellGPT plugin catalog and is not a Git URL or "
            "owner/repo shorthand. Browse entries with `shellgpt plugins search`."))
...
def write_catalog_sidecar(target: Path, entry: PluginCatalogEntry, sha: Optional[str] = None) -> None:
    """Human/Desktop-readable ``.shellgpt-catalog.json`` inside the install dir. It is a CONVENIENCE COPY:
...
def _install_record(plugin_dir: Path) -> Optional[dict]:
    ...
    from shellgpt_cli.plugins_cmd import PluginOperationError, _plugins_dir, _read_install_metadata
...
def _adopt_legacy_sidecar(plugin_dir: Path, record: dict) -> Optional[dict]:
    ...
    from shellgpt_cli.plugins_cmd import _update_install_record
...
def read_catalog_sidecar(plugin_dir) -> Optional[dict]:
    ...
    tree: a URL-installed repo that ships its own ``.shellgpt-catalog.json`` must not render as a reviewed
...
def _refuse_unsupported_catalog_platform(entry: PluginCatalogEntry) -> None:
    if not entry.platforms:
        return
    from shellgpt_cli.plugins_cmd import PluginOperationError
    from shellgpt_platform.host.facts import os_family
...
def install_catalog_entry(entry: PluginCatalogEntry, *, force: bool, ref: Optional[str] = None,
    ...
    from shellgpt_cli.plugins_cmd import _install_plugin_core, pinned_revision
...
def refuse_if_installed_removed(name: str, plugin_dir) -> None:
    ...
    if removed is not None:
        raise PluginOperationError(
            f"Plugin '{name}' was removed from the ShellGPT plugin catalog: "
            f"{removed.reason or 'no reason recorded'}. Remove it with `shellgpt plugins remove {name}`, "
            "or reinstall with `shellgpt plugins install <source> --force --allow-removed` if you trust it.")
...
def _local_changes(target: Path) -> tuple[list[str], list[str]]:
    ...
    from shellgpt_cli.plugins_cmd import _resolve_git_executable, _run_plugin_git
...
def plugin_surface(manifest: dict, tree: Path) -> Dict[str, set]:
    ...
    from shellgpt_cli.plugins_cmd import _declared_capabilities_from_manifest
...
def repin_catalog_plugin(target: Path, sidecar: dict, *, consent_cb=None) -> RepinResult:
    ...
    ``<SHELLGPT_HOME>/plugins-backup/<name>-<sha8>/`` with a warning. A manifest rename between pins moves the
    ...
    if entry is None:
        raise PluginOperationError(
            ...
            "See `shellgpt plugins info` and the removed blocklist.")
...
def cmd_update_catalog(name: str, target: Path, sidecar: dict, console) -> None:
    ...
    def _confirm_widening(delta: Dict[str, List[str]]) -> bool:
        ...
        if not _is_tty():
            console.print("  [yellow]Non-interactive session: update NOT applied (fail closed). "
                          "Re-run `shellgpt plugins update` in a terminal to review and confirm.[/yellow]")
            return False
    ...
    if result.changed:
        from shellgpt_cli.plugins_cmd import _install_python_dependencies
...
def _render_entries(entries: List[PluginCatalogEntry], console) -> None:
    from shellgpt_cli.plugins_cmd import _table
    table = _table(((("Name", "bold")), ("Category", None), ("Tier", None), ("Description", None),
                    ("Pinned", "dim"), ("Capabilities", "dim")), title="ShellGPT Plugin Catalog (curated)")
    ...
    console.print(table)
    console.print()
    console.print("[dim]Details:[/dim] shellgpt plugins info <name>    [dim]Install:[/dim] shellgpt plugins install <name>")
...
def cmd_search(term: str = "", *, json_output: bool = False) -> None:
    ...
    from shellgpt_cli.plugins_cmd import _console
...
def cmd_info(name: str) -> None:
    ...
    from shellgpt_cli.plugins_cmd import _console, cmd_show
    ...
    rows = [("Repo", entry.repo), ("Subdir", entry.subdir), ("Version", entry.version), ("Pinned SHA", entry.sha),
            ("Image", entry.image),
            ("Maintainer", entry.maintainer), ("Requires", f"shellgpt {entry.requires_shellgpt}" if entry.requires_shellgpt else ""),
    ...
    if removed is not None:
        ...
        console.print()
    console.print(f"[dim]Install:[/dim]     shellgpt plugins install {entry.name}")
    console.print()
...
def cmd_validate(path: str, as_json: bool = False, install_deps: bool = False) -> None:
    ...
    from shellgpt_cli.plugin_validate import validate_plugin_dir
    from shellgpt_cli.plugins_cmd import _console
    if install_deps:
...
def installed_catalog_state(installed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    equals the manifest name (``shellgpt-plugin-x`` vs ``x``), so installs are matched through the

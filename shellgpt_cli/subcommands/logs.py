"""``shellgpt logs`` subcommand parser."""
...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter ShellGPT log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    shellgpt logs                    Show last 50 lines of agent.log
    shellgpt logs -f                 Follow agent.log in real time
    shellgpt logs errors             Show last 50 lines of errors.log
    shellgpt logs gateway -n 100     Show last 100 lines of gateway.log
    shellgpt logs gui -f             Follow gui.log in real time
    shellgpt logs desktop -f         Follow desktop.log (Electron app boot/backend)
    shellgpt logs --level WARNING    Only show WARNING and above
    shellgpt logs --session abc123   Filter by session ID
    shellgpt logs --component tools  Only show tool-related lines
    shellgpt logs --since 1h         Lines from the last hour
    shellgpt logs --since 30m -f     Follow, starting from 30 min ago
    shellgpt logs list               List available log files with sizes
""")

"""``shellgpt debug`` subcommand parser."""
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ShellGPT Agent. Use 'shellgpt debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    shellgpt debug share              Upload debug report (asks for confirmation)
    shellgpt debug share --yes        Skip confirmation (for scripts/CI)
    shellgpt debug share --lines 500  Include more log lines
    shellgpt debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    shellgpt debug share --local      Print report locally (no upload)
    shellgpt debug share --no-redact  Disable upload-time secret redaction
    shellgpt debug share --nous       Upload to Nous-internal storage (private)
    shellgpt debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'shellgpt debug share'")

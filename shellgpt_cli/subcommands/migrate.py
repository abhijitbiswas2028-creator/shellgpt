"""``shellgpt migrate`` subcommand parser."""
...
def build_migrate_parser(subparsers) -> None:
    ...
    from shellgpt_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    migrate_relay = migrate_subparsers.add_parser(
        "relay", help="Convert legacy SHELLGPT_NEMO_RELAY_ATIF_*/ATOF_* exporter vars into relay-plugins.toml",
        description="The NeMo Relay cutover stopped reading the legacy exporter variables; a .env that still "
            "carries them (and no SHELLGPT_NEMO_RELAY_PLUGINS_TOML) exports nothing. Generate "
            "<shellgpt home>/relay-plugins.toml from them, point SHELLGPT_NEMO_RELAY_PLUGINS_TOML at it, "
            "and comment the legacy lines out. `shellgpt update` runs this for every profile automatically.")
    migrate_relay.add_argument(
        "--all-profiles", action="store_true",
        help="Migrate the default home and every named profile (what `shellgpt update` does)")
    migrate_relay.add_argument(
...
def _cmd_migrate_relay(args) -> None:
    from shellgpt_cli.relay_plugin_migrate import cmd_migrate_relay

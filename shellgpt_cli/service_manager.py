...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``shellgpt gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from shellgpt_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `shellgpt gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged shellgpt user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every ShellGPT runtime call
    inside the container drops to shellgpt via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``shellgpt_cli.gateway`` (systemd/launchd) and
# ``shellgpt_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``shellgpt -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``shellgpt_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import shellgpt_cli
        importlib.import_module(f"shellgpt_cli.{self._backend}")
        return getattr(shellgpt_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in shellgpt_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in shellgpt_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``shellgpt_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `shellgpt profile create` inside the container. Static services (main-shellgpt, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's SHELLGPT_HOME blindly: derive the shared profile root and map the suffix to the root
    default profile or ``<root>/profiles/<profile>``.
    """
    profile = _profile_from_service(name)
    validate_profile_name(profile)
    shellgpt_home = Path(os.environ.get("SHELLGPT_HOME", "/opt/data"))
    root = shellgpt_home.parent.parent if shellgpt_home.parent.name == "profiles" else shellgpt_home
...
def register_unregistered_profile_gateway(mgr: ServiceManager, profile: str) -> bool:
    ...
    `shellgpt profile create` can only register a slot when it runs inside the container; created
...
# UID/GID of the in-image ``shellgpt`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_SHELLGPT_UID = 10000
_SHELLGPT_GID = 10000


def _chown_shellgpt(path: Path) -> None:
    try:
        os.chown(path, _SHELLGPT_UID, _SHELLGPT_GID)
    except PermissionError:
        # Already running as shellgpt → the dir is shellgpt-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create shellgpt-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    shellgpt user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
    ...
    dispatched as the shellgpt user.
...
class GatewayNotRegisteredError(S6Error):
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`shellgpt profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources SHELLGPT_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to shellgpt.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root SHELLGPT_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``SHELLGPT_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``shellgpt_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/shellgpt/.venv/bin/activate",
        ]
        ...
        lines.append("export SHELLGPT_S6_SUPERVISED_CHILD=1")
        # Generalized supervisor marker — same meaning for the profile-redirect guard in
        # shellgpt_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export SHELLGPT_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its SHELLGPT_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``shellgpt gateway run``, an agent action, the Open
        ...
        if profile == "default":
            gateway_cmd = "shellgpt gateway run --replace"
        else:
            gateway_cmd = f"shellgpt -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID → s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid shellgpt {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{SHELLGPT_HOME:=/opt/data}}"\n'
            f'log_dir="$SHELLGPT_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock AS SHELLGPT when starting as root. Never
            # chown/unlink shellgpt-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded shellgpt-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid shellgpt mkdir -p "$log_dir"\n'
            f'  s6-setuidgid shellgpt rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid shellgpt s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed shellgpt-owned supervise/ BEFORE publishing so the shellgpt-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is shellgpt-owned (see ``_seed_supervise_skeleton``)

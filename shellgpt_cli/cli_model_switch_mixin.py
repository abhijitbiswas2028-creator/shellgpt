...
Mixin split out of ``cli.py``; bound onto ``ShellGPTCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``ShellGPTCLI.<name>(self, ...)``.
"""
...
def stored_session_route(session_meta, *, current_model, current_provider):
    ...
    if not stored_model:
        return None
    from shellgpt_state import SessionDB as _SessionDB
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from shellgpt_cli.model_switch import format_model_for_display, resolve_display_context_length
...
def _commit_model_switch(
    ...
    from cli import ShellGPTCLI, _cprint
    old_model = cli.model
    ...
    if not one_turn:
        ShellGPTCLI._persist_model_switch_to_session(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from shellgpt_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from shellgpt_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if opencode_provider_family(resolved_provider) is not None:
            from shellgpt_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
        ...
        if self._model_is_default:
            from shellgpt_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current ShellGPT session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `shellgpt model` / gateway pickers); live catalog only when
            ...
            if not model_list:
                try:
                    from shellgpt_cli.models import cached_provider_model_ids
                    model_list = cached_provider_model_ids(provider_data["slug"]) or model_list
                except Exception:
                    pass
            from shellgpt_cli.models_validate import offered_model_ids
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — ShellGPT default (chat_completions)

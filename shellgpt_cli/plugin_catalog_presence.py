...
A portable plugin declares its app in ``plugin.json`` (``extensions.com.nousresearch.shellgpt.servers``).
The catalog pins the commit, so the declaration is read once per pin from the repo at that commit and
judged by the same ``shellgpt_platform`` resolver the installer and the Plugins-tab pill use. Nothing is
...
def _server_presence(decl: Any, liveness_raw: Any, title: str) -> Presence:
    ...
    from shellgpt_platform.resolver.base import Effort
...
def presence(entry: Any) -> Presence:
    ...
    from shellgpt_cli.agent_plugins import _server_declarations

    manifest = _pinned_manifest(entry.repo, entry.sha, entry.subdir)
    if not manifest:
        return UNKNOWN
    raw_servers = (manifest.get("extensions") or {}).get("com.nousresearch.shellgpt", {}).get("servers") or {}
...
def onboarding_entries() -> list[Dict[str, Any]]:
    ...
    from shellgpt_cli.plugins_cmd_catalog import normalized_platforms

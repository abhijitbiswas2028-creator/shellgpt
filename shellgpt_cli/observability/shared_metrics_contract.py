"""Bounded product contract for the first ShellGPT shared-metrics slice."""
...
SCHEMA_KEY = "shellgpt.metrics.schema_version"
SCHEMA_VERSION = "shellgpt.metrics.event.v2"
MODEL_CALL_SCOPE = "shellgpt.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "shellgpt.task_run"
TOOL_CALL_SCOPE = "shellgpt.tool_call"
CLIENT_ACTIVE_MARK = "shellgpt.client.active"
TOOL_APPROVAL_MARK = "shellgpt.tool_approval"
SKILL_LIFECYCLE_MARK = "shellgpt.skill.lifecycle"
SKILL_LOAD_MARK = "shellgpt.skill.load"
SUBSCRIBER_NAME = "shellgpt.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "shellgpt.client.active"
LEGACY_MODEL_CALL_METRIC = "shellgpt.model_call.count"
MODEL_ROUTE_METRIC = "shellgpt.model_route.count"
TASK_STARTED_METRIC = "shellgpt.task_run.started"
TASK_FINISHED_METRIC = "shellgpt.task_run.finished"
TOOL_CALL_METRIC = "shellgpt.tool_call.count"
TOOL_APPROVAL_METRIC = "shellgpt.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "shellgpt.skill.lifecycle.count"
SKILL_LOAD_METRIC = "shellgpt.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "shellgpt_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted ShellGPT installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    shellgpt_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(shellgpt_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "shellgpt_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("shellgpt_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its ShellGPT logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "shellgpt.call_role"
    )
    call_role = (metadata or {}).get("shellgpt.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from shellgpt_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map ShellGPT terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "shellgpt-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map ShellGPT registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal ShellGPT tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to ShellGPT."""

"""Top-level argparse construction for the shellgpt CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``shellgpt_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``SHELLGPT_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/shellgpt_cli/
...
def _cfg_path() -> str:
    """``~/.shellgpt/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_shellgpt_home()`` would emit the
    "[SHELLGPT_HOME fallback] ... wrong profile" warning on every ``shellgpt`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from shellgpt_constants import display_shellgpt_home, get_process_shellgpt_home

    return f"{display_shellgpt_home(get_process_shellgpt_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``shellgpt --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``shellgpt_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    shellgpt                        Start interactive chat
    shellgpt chat -q "Hello"        Single query mode
    shellgpt --tui                  Launch the modern TUI (or set display.interface: tui)
    shellgpt --cli                  Force the classic REPL (overrides display.interface: tui)
    shellgpt -c                     Resume the most recent session
    shellgpt -c "my project"        Resume a session by name (latest in lineage)
    shellgpt --resume <session_id>  Resume a specific session by ID
    shellgpt --resume latest        Resume the most recent session (same as -c)
    shellgpt --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    shellgpt setup                  Run setup wizard
    shellgpt logout                 Clear stored authentication
    shellgpt auth add <provider>    Add a pooled credential
    shellgpt auth list              List pooled credentials
    shellgpt auth remove <p> <t>    Remove pooled credential by index, id, or label
    shellgpt auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    shellgpt auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    shellgpt auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    shellgpt model                  Select default model
    shellgpt fallback [list]        Show fallback provider chain
    shellgpt fallback add           Add a fallback provider (same picker as `shellgpt model`)
    shellgpt fallback remove        Remove a fallback provider from the chain
    shellgpt config                 View configuration
    shellgpt config edit            Edit config in $EDITOR
    shellgpt config set model gpt-4 Set a config value
    shellgpt gateway                Run messaging gateway
    shellgpt gateway install        Install gateway background service
    shellgpt gateway start          Start the installed gateway service
    shellgpt gateway stop           Stop the gateway service
    shellgpt gateway status         Show gateway status
    shellgpt -p <profile> <cmd>     Run any command against a named profile's
                                  home (also --profile) — e.g. shellgpt -p coder gateway stop
    shellgpt -s shellgpt-agent-dev,github-auth
    shellgpt -w                     Start in isolated git worktree
    shellgpt sessions list          List past sessions
    shellgpt sessions browse        Interactive session picker
    shellgpt sessions rename ID T   Rename/title a session
    shellgpt logs                   View agent.log (last 50 lines)
    shellgpt logs -f                Follow agent.log in real time
    shellgpt logs errors            View errors.log
    shellgpt logs --since 1h        Lines from the last hour
    shellgpt debug share             Upload debug report for support
    shellgpt console                Open the safe ShellGPT command console
    shellgpt update                 Update to latest version
    shellgpt dashboard              Start web UI dashboard (port 9119)
    shellgpt dashboard --stop       Stop running dashboard processes
    shellgpt dashboard --status     List running dashboard processes

For more help on a command:
    shellgpt <command> --help
"""
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via SHELLGPT_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `shellgpt setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to SHELLGPT_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``shellgpt -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/shellgpt_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with ShellGPT Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also SHELLGPT_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from ShellGPT itself.")
...
class ShellGPTArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "shellgpt" at the top level and "shellgpt gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = ShellGPTArgumentParser(
        prog="shellgpt", description="ShellGPT Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``shellgpt [...] <command>`` instead of the brace list of
    # every subcommand name; ``shellgpt --help`` still lists each command with its help row.

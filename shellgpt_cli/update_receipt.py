...
# ``shellgpt update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``shellgpt update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``shellgpt serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``shellgpt_constants`` (stdlib-only), never ``shellgpt_cli.config``: the receipt must be
    # writable from the refused/failed paths where config loading itself may be what broke
    # (#112465, #112558).
    from shellgpt_constants import get_shellgpt_home

    return get_shellgpt_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``shellgpt update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def settle_latest_receipt_fleet(fleet: list[dict[str, Any]], *, discharges) -> bool:
    ...
    pre-pull SHAs) keeps ``shellgpt update`` exiting 1 and every CLI start warning about mixed
    modules, long after the operator's ``shellgpt gateway restart`` fixed the fleet (#117051). The
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from shellgpt_cli.profiles import _get_default_shellgpt_home, _get_profiles_root, _PROFILE_ID_RE
...
def _code_root_for_path(raw: Any) -> Optional[Path]:
    """Return the ShellGPT checkout containing an absolute process path."""
    ...
    with suppress(Exception):
        ...
        for parent in [candidate, *candidate.parents][:_CODE_ROOT_MAX_DEPTH]:
            if (parent / "shellgpt_cli" / "main.py").is_file():
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `shellgpt gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if RESTART_PENDING_STATE in states:
        ...
        print("    verify afterwards with `shellgpt gateway status`.")
    ...
    if stale_or_down:
        ...
        print("  Run `shellgpt gateway restart` (or `shellgpt -p <profile> gateway restart` for a named")
        print("  profile), then `shellgpt gateway status` to confirm.")

# shellgpt_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `ShellGPTCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`shellgpt_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`shellgpt_cli/gateway.py` is the `shellgpt gateway` facade (process discovery, systemd backend, command
...
(`shellgpt gateway setup`: `_PLATFORMS` registry, status table, per-platform prompts, service offer),
...
(late binding on `shellgpt_cli.gateway`), so monkeypatch on the facade; mutable state such as
...
`ShellGPTCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.shellgpt/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`shellgpt_cli/curses_ui.py`; example
`shellgpt_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`shellgpt_cli/commands.py`)
...
`shellgpt_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`shellgpt_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `shellgpt config
...
- **One writer.** Every write of a `config.yaml` (main or profile) goes through
  `shellgpt_cli.config.atomic_config_write` (→ `utils.atomic_roundtrip_yaml_save`, ruamel
  ...
  (CI lint) rejects it, and `tests/shellgpt_cli/test_config_yaml_comment_preservation.py` guards each
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`shellgpt tools/setup`, most subcommands, `shellgpt_cli/config.py`, merges `DEFAULT_CONFIG`);
  `shellgpt_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `shellgpt send`, doctor,
  `shellgpt_time`/`shellgpt_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' shellgpt_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`shellgpt_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.shellgpt/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`shellgpt update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `shellgpt update --plan`): read-only inventory — install kind, all
...
- **Apply**: git pull, or the Windows ZIP fallback — which fires ONLY when git itself failed
  ...
  root entry the ZIP does not ship (`.bytecode-fingerprint`, `.shellgpt-bootstrap-complete`,
  `shellgpt_agent.egg-info/`; tracked root entries stand in for the ZIP set before the download, the
  ...
  `shellgpt_cli/web_dist`, `ui-tui/{dist,node_modules,packages/shellgpt-ink/dist}`, `web/node_modules`,
  ...
  rebuild decision also trusts the build stamp under SHELLGPT_HOME, so an install that already lost
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE within the updating install (every
  `shellgpt-gateway*` unit / `ai.shellgpt.gateway*` LaunchAgent whose home is the updating root or one of its
  ...
  `shellgpt_cli/update_fleet_scope.py` judges every unit/label/process by the home it actually runs on
  (live environ, unit `Environment=`, plist `SHELLGPT_HOME`), and a runtime of another `SHELLGPT_HOME` on the
  same account — a sibling install, the real `shellgpt-gateway.service` seen from a scratch home — is named and
...
- **Report**: every run writes a machine-readable receipt to `~/.shellgpt/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `shellgpt update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `shellgpt update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `shellgpt_cli/main.py` sets `SHELLGPT_HOME` before any module import for
single-profile commands (`shellgpt -p x <cmd>`), so there `get_shellgpt_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["SHELLGPT_HOME"]` keeps the launch
...
carries an identity marker (`shellgpt_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_shellgpt_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `shellgpt_constants.shellgpt_home_key()`, never a single flag.
...
Migration from per-profile gateways: `shellgpt_cli/gateway_migrate.py` (`shellgpt gateway migrate
...
other service domain / UNIX user / SHELLGPT_HOME outside `profiles/` — notices for the explicit command,
...
`shellgpt gateway restart` for a gateway ShellGPT did not install (custom launchd agent / unit running
...
sibling and read facade helpers late-bound through `shellgpt_cli.gateway`, so patch them on the facade; user unit AND `--system` unit with `User=`; an unresolvable `User=` is a blocker,
...
profile's `SHELLGPT_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nous free tier (`shellgpt_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `shellgpt` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`shellgpt auth upgrade`), the CLI `/login` handler and the desktop

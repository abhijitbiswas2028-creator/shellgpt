...
Split out of ``shellgpt_cli.kanban_db``; origin-resident helpers are reached
...
from shellgpt_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER

if TYPE_CHECKING:
    from shellgpt_cli.kanban_db import Task
...
# ``SHELLGPT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a ShellGPT profile. Expected steady-state
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``shellgpt kanban tail`` is the only place the
...
# children): a per-tick ``shellgpt kanban dispatch`` process finds it empty, so
...
def _worker_log_exit_code(task_id: str, board: Optional[str] = None) -> Optional[int]:
    ...
    (``shellgpt_cli.quiet_single_query.exit_single_query``), so it is there whether
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|☤\s*ShellGPT")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``shellgpt kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``shellgpt_cli.profiles.profile_exists``, or ``None`` when it cannot be
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several ShellGPT homes),
...
def dispatch_profile_allowlist_summary() -> str:
    ...
    Surfaced by ``shellgpt kanban diagnostics`` so an operator on a shared board
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real ShellGPT profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (ShellGPT ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``SHELLGPT_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``shellgpt -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``shellgpt kanban tail`` shows why the task looks stuck.
...
def _module_shellgpt_argv() -> list[str]:
    """Interpreter-bound ShellGPT CLI invocation (``shellgpt_cli.main`` is the
    console-script target — there is no top-level ``shellgpt`` package)."""
    return [sys.executable, "-m", "shellgpt_cli.main"]


def _absolute_shellgpt_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved ShellGPT shim."""
...
def _shellgpt_path_argv(path: str) -> list[str]:
    """argv for a resolved ShellGPT executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_shellgpt_argv()
    return [_absolute_shellgpt_path(path)]


def _resolve_shellgpt_argv() -> list[str]:
    """Resolve the ``shellgpt`` invocation as argv for ``Popen``: ``$SHELLGPT_BIN``
    ...
    shellgpt_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("shellgpt")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``shellgpt_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``shellgpt`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_shellgpt_bin``; local because ``shellgpt_cli``
    sits below ``gateway`` in the dependency order.
    """
    import importlib.util
    import shutil

    env_bin = os.environ.get("SHELLGPT_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _shellgpt_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _shellgpt_path_argv(resolved_env_bin)
        return _module_shellgpt_argv()
    ...
    shellgpt_bin = _safe_which_no_cwd("shellgpt") if _kb._IS_WINDOWS else shutil.which("shellgpt")
    if shellgpt_bin:
        return _shellgpt_path_argv(shellgpt_bin)
    return _module_shellgpt_argv()
...
def _worker_profile_scope(shellgpt_home: str, *, bind_home: bool = True):
    ...
    from shellgpt_constants import get_process_shellgpt_home, reset_shellgpt_home_override, set_shellgpt_home_override
    ...
    is_launch_home = str(home.resolve()) == str(Path(get_process_shellgpt_home()).resolve())
    home_token = secret_token = terminal_token = None
    try:
        home_token = set_shellgpt_home_override(str(home)) if bind_home else None
...
def _resolve_worker_cli_toolsets(shellgpt_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``SHELLGPT_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        with _worker_profile_scope(shellgpt_home):
            cfg = load_config()
            toolsets = sorted(_get_platform_tools(cfg, "cli"))
        return toolsets or None
    except Exception as exc:
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for SHELLGPT_HOME=%r (%s)",
            shellgpt_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, shellgpt_home: Optional[str]) -> list[str]:
    """Build the ``shellgpt -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_shellgpt_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped SHELLGPT_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(shellgpt_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `shellgpt kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``shellgpt -p <profile> chat -q ...`` subprocess.
    ...
    ``SHELLGPT_KANBAN_DB`` / ``SHELLGPT_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from shellgpt_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # SHELLGPT_PROFILE (set below) instead.
        profile_home = None
    ...
    # Inject SHELLGPT_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_shellgpt_home() falls back to the DEFAULT
    # profile root because `shellgpt -p` applies its override before
    # shellgpt_constants is imported.
    if profile_home:
        env["SHELLGPT_HOME"] = profile_home
    ...
    if task.tenant:
        env["SHELLGPT_TENANT"] = task.tenant
    env["SHELLGPT_KANBAN_TASK"] = task.id
    env["SHELLGPT_KANBAN_WORKSPACE"] = workspace
    ...
    env["SHELLGPT_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["SHELLGPT_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["SHELLGPT_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SHELLGPT_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["SHELLGPT_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["SHELLGPT_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `shellgpt -p` rewrites SHELLGPT_HOME (symlink / Docker layouts).
    env["SHELLGPT_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["SHELLGPT_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["SHELLGPT_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["SHELLGPT_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads SHELLGPT_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["SHELLGPT_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping SHELLGPT_TUI covers
    # older shellgpt builds on PATH that predate the flag's precedence.
    env.pop("SHELLGPT_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("SHELLGPT_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`shellgpt` executable not found on PATH. "
            "Install ShellGPT Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``shellgpt kanban dispatch`` — the standalone
...
from shellgpt_cli import kanban_db as _kb  # noqa: E402
from shellgpt_cli import kanban_db_connect as _kbc  # noqa: E402
from shellgpt_cli import kanban_db_workspace as _kbw  # noqa: E402

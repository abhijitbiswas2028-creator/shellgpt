"""On-demand supply-chain audit for ShellGPT Agent installs.
...
def _discover_plugins(shellgpt_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.shellgpt/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, shellgpt_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = shellgpt_home or Path(get_shellgpt_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `shellgpt security audit`."""
    ...
    components = _discover_components(shellgpt_home=home, **skips)
    ...
    try:
        findings = run_audit(shellgpt_home=home, components=components)

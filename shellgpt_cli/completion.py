"""Shell completion script generation for shellgpt CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_shellgpt_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# ShellGPT Agent bash completion
# Add to ~/.bashrc:
#   eval "$(shellgpt completion bash)"

_shellgpt_profiles() {{
    local profiles_dir="$HOME/.shellgpt/profiles"
...
_shellgpt_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_shellgpt_profiles)" -- "$cur"))
        return
    fi
...
complete -F _shellgpt_completion shellgpt
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef shellgpt
# ShellGPT Agent zsh completion
# Add to ~/.zshrc:
...
_shellgpt_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.shellgpt/profiles" ]]; then
        profiles+=($HOME/.shellgpt/profiles/*(N/:t))
    fi
...
_shellgpt() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_shellgpt_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'shellgpt command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ShellGPT Agent fish completion",
        "# Add to your config:",
        "#   shellgpt completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __shellgpt_profiles",
        "    echo default",
        "    if test -d $HOME/.shellgpt/profiles",
        "        for d in $HOME/.shellgpt/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c shellgpt -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c shellgpt -f -s p -l profile"
        " -d 'Profile name' -xa '(__shellgpt_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c shellgpt -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__shellgpt_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)

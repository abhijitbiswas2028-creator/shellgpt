"""``shellgpt logs`` — view and filter ShellGPT log files.

``shellgpt logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``shellgpt logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_shellgpt_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when ShellGPT runs — try 'shellgpt chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_shellgpt_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_shellgpt_home()}/logs/")
        return

    print(f"Log files in {display_shellgpt_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'shellgpt chat' to generate logs)")

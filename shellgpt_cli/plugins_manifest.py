...
Split out of :mod:`shellgpt_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``shellgpt``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "shellgpt", "depends",
    "requires_shellgpt", "python_runtime",
}

# Highest manifest schema version this ShellGPT understands.
...
_CONFIG_SCHEMA_TYPES: Dict[str, tuple] = {
    ...
    # ``secret`` values live in ``.env`` (see shellgpt_cli.plugins_settings), so a config.yaml copy is
...
def _plugins_debug() -> bool:
    from shellgpt_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this ShellGPT "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `shellgpt plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `shellgpt plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # ShellGPT version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_shellgpt_error``.
    requires_shellgpt: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — ShellGPT never auto-installs these (isolation design for the install seam
...
# ── requires_shellgpt version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_shellgpt_version() -> str:
    """Version of the ShellGPT code that is running: ``shellgpt_cli.__version__``. Distribution metadata is only
    ...
    try:
        ...
        pass
    return importlib.metadata.version("shellgpt-agent")
...
def requires_shellgpt_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_shellgpt`` rejects the running version."""
    if not manifest.requires_shellgpt:
        return None
    current = running_shellgpt_version()
    if version_satisfies(manifest.requires_shellgpt, current):
        return None
    return f"requires shellgpt {manifest.requires_shellgpt}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_shellgpt=str(data.get("requires_shellgpt") or "").strip(),

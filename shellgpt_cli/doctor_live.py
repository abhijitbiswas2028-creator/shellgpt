"""``shellgpt doctor --live`` — opt-in bounded real-call tool-backend probes.

Opt-in only: these probes make real (cheap, metadata/read-only) network calls and may spend a
trivial amount of quota. They run ONLY when the user passes ``shellgpt doctor --live``.
"""
...
from shellgpt_cli.doctor import _section, check_info
from shellgpt_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from shellgpt_cli.doctor import SHELLGPT_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (SHELLGPT_HOME / "node" / "bin", SHELLGPT_HOME / "node", SHELLGPT_HOME / "node_modules" / ".bin"):
    ...
    # probes above. Mirror the rung shellgpt_cli.doctor uses so this probe can't diverge from it, including

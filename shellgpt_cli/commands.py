"""Slash command registry for the ShellGPT CLI and gateway.
...
class CommandDef:
    ...
    # Key in ``shellgpt_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of ShellGPT config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal ShellGPT works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .shellgpt/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey ShellGPT' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while ShellGPT is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.shellgpt/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update ShellGPT Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show ShellGPT Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``shellgpt skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('shellgpt_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('shellgpt_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('shellgpt_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('shellgpt_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('shellgpt_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('shellgpt_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('shellgpt_cli.commands_platforms', 'telegram_menu_max_commands'),
}

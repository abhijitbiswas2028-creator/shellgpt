...
from shellgpt_constants import get_shellgpt_home, get_default_shellgpt_root
from shellgpt_cli.profiles import parked_marker_path, profile_is_parked, profile_is_standalone, profiles_to_serve
...
def profile_lifecycle(command: str, args) -> bool:
    ...
    if command == "start":
        ...
        host_home = owner.home if owner is not None else get_default_shellgpt_root()
    ...
    if command == "stop":
        ...
        if _confirmed(answer, "unserved", name):
            print(f"Profile '{name}' parked; its bots and cron are stopped. "
                  f"Start again with: shellgpt -p {name} gateway start")
        else:
...
def print_parked_status() -> bool:
    ...
    from shellgpt_cli import gateway as gw
    name = gw._current_profile_name()
    if name and name != "default" and profile_is_parked(get_shellgpt_home()):
        print(f"Profile '{name}': parked (shellgpt -p {name} gateway start)")
        return True
    if not name or name == "default":
        ...
        if parked:
            ...
            for profile in parked:
                print(f"Profile '{profile}': parked (shellgpt -p {profile} gateway start)")
    return False

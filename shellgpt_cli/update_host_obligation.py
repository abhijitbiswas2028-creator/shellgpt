"""Host-scoped update→restart obligation for ``shellgpt update``.
...
profile's ``SHELLGPT_HOME``. The legacy ``$SHELLGPT_HOME/fleet_restart_pending`` marker was
per-home: ``shellgpt -p coder update`` armed and cleared coder's copy while restarting the
...
:func:`gateway.host_rendezvous.host_state_dir` (``$SHELLGPT_GATEWAY_LOCK_DIR`` else
``$XDG_STATE_HOME/shellgpt/gateway-locks``) — the one cross-profile, per-OS-user state root the
...
per-profile ``shellgpt-gateway-<p>.service`` units on a multiplexed host all point at the same
...
def host_restart_already_completed(sha: Optional[str]) -> bool:
    ...
    ``shellgpt update`` must attach to the first restart's outcome, never kill the shared
    multiplexer again.
    """
    record = read_host_obligation()

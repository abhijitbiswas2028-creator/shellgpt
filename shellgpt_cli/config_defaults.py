"""Default configuration data for ShellGPT Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from shellgpt_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    # journal_mode: SQLite journal mode for every ShellGPT DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `shellgpt -c`: each CLI session writes a breadcrumb under
        # $SHELLGPT_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `shellgpt
        ...
        # ShellGPT-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping ShellGPT (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env SHELLGPT_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `shellgpt doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`shellgpt status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # SHELLGPT_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else SHELLGPT_HOME/cache/terminal (auto-pruned
        ...
        # (bytes); raise it for a ~/.shellgpt tree that legitimately exceeds 2 GiB.
        "sync_back_max_bytes": 2 * 1024 * 1024 * 1024,
        # HOME for host tool subprocesses: "auto" = host keeps the real OS-user HOME, containers use
        # SHELLGPT_HOME/home; "real" = force real HOME; "profile" = force SHELLGPT_HOME/home when it
        ...
        # ["/home/user/.shellgpt/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled ShellGPT image,
        # which drops to `shellgpt` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `shellgpt tools`, not a ring
        # member). Set by the `shellgpt tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a ShellGPT-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by ShellGPT' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.shellgpt/browser-profile/ so credentials
        ...
        # runs `shellgpt browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `shellgpt chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.shellgpt/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `shellgpt checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .shellgpt.md,
    ...
    # Same bound for single-query mode (``shellgpt -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # ShellGPT' summarizer cannot shrink it. native = codex decides; shellgpt = ShellGPT' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `shellgpt model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `shellgpt`/`shellgpt chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / SHELLGPT_TUI=1 forces the TUI.
        "interface": "cli",
        # `shellgpt --tui` auto-resumes the most recent human-facing session (like `shellgpt -c`).
        # SHELLGPT_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = SHELLGPT_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/shellgpt`.
        ...
        "runtime_footer": {
            ...
            # deployment a routing proxy reported / ShellGPT' fallback route).
        ...
        # CLI/TUI/desktop, managed with `shellgpt pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_shellgpt_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: SHELLGPT_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than SHELLGPT_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env SHELLGPT_DASHBOARD_OAUTH_CLIENT_ID / SHELLGPT_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env SHELLGPT_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # SHELLGPT_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env SHELLGPT_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/shellgpt) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.shellgpt/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → ShellGPT turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to ShellGPT (any model / provider you have
    ...
    # "Hey ShellGPT" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_shellgpt" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.shellgpt/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.shellgpt/cache/delegation/ with a head+tail window + read_file offset footer, nothing
        ...
        # Total subagents a finite one-shot run (shellgpt chat -q / --oneshot) may spawn; 0 = unlimited.
    ...
    "moa": {
        ...
        # <shellgpt_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.shellgpt/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # SHELLGPT_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.shellgpt/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `shellgpt skills trust` / `untrust`.
        ...
        # skip; SHELLGPT_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${SHELLGPT_SKILL_DIR} / ${SHELLGPT_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `shellgpt skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.shellgpt/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.shellgpt/.curator_backups/blobs/); powers `shellgpt
    ...
    # daemon. `shellgpt curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `shellgpt curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `shellgpt update` restoring them);
        ...
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `shellgpt curator purge`
        ...
        # ~/.shellgpt/skills/ to ~/.shellgpt/skills/.curator_backups/<utc-iso>/skills.tar.gz (`shellgpt curator
    ...
    # peerName, sessions, enabled); shellgpt-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *ShellGPT Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (SHELLGPT_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export SHELLGPT_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. SHELLGPT_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `shellgpt plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `shellgpt plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.shellgpt/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # SHELLGPT_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "auth": {  # Login policy (credentials themselves live in auth.json / .env).
        ...
        # logins automatically when ShellGPT has no usable login of its own. Their refresh tokens are single-use
        # and rotate, so two programs on one login can log each other out; set false to make ShellGPT use only
        # its own logins (`shellgpt auth add <provider>`). `shellgpt auth add openai-codex` still offers the import
        # interactively.
        "adopt_external_logins": True,
        # How `shellgpt auth add openai-codex` / `shellgpt model` sign in to OpenAI Codex.
        ...
        # to device code when that port is busy. `shellgpt auth add openai-codex --browser` opts in
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .shellgpt config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `shellgpt doctor --ack <id>`; remove by editing the list. Catalog:
        # shellgpt_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $SHELLGPT_HOME/plugins/ <name>/),
        ...
        # Env override: SHELLGPT_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: SHELLGPT_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: SHELLGPT_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: SHELLGPT_CRON_MEDIA_SEND_TIMEOUT.
        ...
        # before one reminder ping (the run is still recorded; `shellgpt cron incidents` shows it).
        # A green run or a different error alerts again immediately; `shellgpt cron incidents ack`
    ...
    # promotes dependency-satisfied todos to ready, and fires `shellgpt -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across ShellGPT homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `shellgpt kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with shellgpt-agent's own python
    ...
    "logging": {  # File logging to ~/.shellgpt/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `shellgpt model` once the cache is older. Network
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: ShellGPT
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "shellgpt-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to SHELLGPT_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to SHELLGPT_STARTUP_WATCHDOG / SHELLGPT_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.shellgpt/sessions/sessions.json mirror of the routing index
        ...
        # `shellgpt gateway migrate --multiplex` folds any per-profile fleet that is left.
        ...
        # adapter is parked; `shellgpt profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `shellgpt update` fold this install onto a multiplexed default gateway by itself?
        ...
        # AUTOMATIC path reads this: `shellgpt gateway migrate --multiplex` is explicit and proceeds.
        ...
        # (SHELLGPT_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # SHELLGPT_GATEWAY_MAX_STARTS / SHELLGPT_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.shellgpt/.env, auth.json). True:
        # files must be under the ShellGPT cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to SHELLGPT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the ShellGPT cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to SHELLGPT_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to SHELLGPT_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # SHELLGPT_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.shellgpt/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `shellgpt sessions prune`).
        ...
        # stay until `shellgpt sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `shellgpt update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.shellgpt/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to SHELLGPT_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`shellgpt sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `shellgpt doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `shellgpt update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <SHELLGPT_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``shellgpt backup`` zip in
        # <SHELLGPT_HOME>/backups/ (``shellgpt import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `shellgpt update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `shellgpt update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        # skipped. 0 = for the process lifetime (until `shellgpt lsp restart`); N = retried after N
        ...
        "exclude_roots": [],
        # Missing server binaries: auto = install via npm/go/pip into <SHELLGPT_HOME>/lsp/bin/ on
        ...
        # <SHELLGPT_HOME>/lsp/node_modules; a configured manager that is not installed, or an unknown
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `shellgpt tools`.
    "x_search": {
    ...
    # them in ~/.shellgpt/.env.
    # Browser credential vault: which login sources browser_vault_list/fill may draw from. The local
    # encrypted vault (`shellgpt vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.shellgpt/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.shellgpt/cache/,
            ...
            # Auto-download bws into ~/.shellgpt/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    # Bot Desktop: a headless Xfce screen per profile on the gateway host (Linux), streamed to ShellGPT
    # Desktop where a human can watch, take over (logins, 2FA, CAPTCHAs) and hand back. `shellgpt computer-use screen`.
    "bot_desktop": {
        "geometry": "1440x900",
        ...
        # for; ShellGPT Desktop's Screen pane offers Start and this toggle.
        "auto_start": False,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; ShellGPT sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; ShellGPT also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.shellgpt/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # ShellGPT Desktop (Electron) launch options; only affect `shellgpt desktop`.
        ...
        # Bridged to SHELLGPT_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to SHELLGPT_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        "anthropic_wire": "chat",
        # Nous free tier: with no other provider configured, ShellGPT sets up a free Nous identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `shellgpt auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (ShellGPT still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by ShellGPT releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials ShellGPT mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'shellgpt model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the ShellGPT auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey ShellGPT' wake word engine (optional; "
    ...
    "SHELLGPT_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "SHELLGPT_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "SHELLGPT_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use ShellGPT, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @shellgpt:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. SHELLGPT_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #shellgpt)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: shellgpt-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'shellgpt-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote ShellGPT API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote ShellGPT API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote ShellGPT API server (proxy mode). Must "
    ...
    # SHELLGPT_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. SHELLGPT_TOOL_PROGRESS is unsupported.
    "SHELLGPT_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "SHELLGPT_EPHEMERAL_SYSTEM_PROMPT": _setting(

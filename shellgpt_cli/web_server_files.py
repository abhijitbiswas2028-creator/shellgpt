...
_MANAGED_FILES_ROOT_ENV = "SHELLGPT_DASHBOARD_FILES_ROOT"
...
def _default_shellgpt_root_is_opt_data() -> bool:
    raw = os.environ.get("SHELLGPT_HOME", "").strip()
    if not raw:
        return False
    try:
        from shellgpt_constants import get_default_shellgpt_root

        root = get_default_shellgpt_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``shellgpt update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the shellgpt-webui image), where
    ...
    if _default_shellgpt_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the ShellGPT
    # root actually IS /opt/data or SHELLGPT_DASHBOARD_FILES_ROOT is set.
    if _default_shellgpt_root_is_opt_data():

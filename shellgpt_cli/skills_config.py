"""Skills configuration for ShellGPT Agent. `shellgpt skills` enters this module."""
from typing import List, Optional, Set

from shellgpt_cli.config import cfg_get, load_config, save_config
from shellgpt_cli.colors import Colors, color
from shellgpt_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    """Persist disabled skill names to config; essential skills (e.g. ``shellgpt-agent``) are
...
def skills_command(args=None):
    """Entry point for `shellgpt skills`."""
    from shellgpt_cli.curses_ui import curses_checklist
    config = load_config()

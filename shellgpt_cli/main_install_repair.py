...
# Install-scoped breadcrumbs live next to the venv (not under $SHELLGPT_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``shellgpt acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``shellgpt update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    if attempts >= max_attempts:
        print(
            f"✗ Finishing an interrupted `shellgpt update` failed {attempts} times; automatic "
            "retries are paused so ShellGPT stops reinstalling on every launch.")
        for line in _manual_core_recovery_lines(PROJECT_ROOT, marker, windows=_is_windows()):
            print(line)
        return
    print(
        "⚠ A previous `shellgpt update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``shellgpt.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_shellgpt_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from shellgpt.exe; applying package-only first aid, "
    ...
    from shellgpt_cli import _install_repair as _ir
    try:
...
def _manual_core_recovery_lines(root: Path, marker: Path, *, windows: bool) -> tuple[str, ...]:
    ...
    On Windows another ShellGPT process (Desktop backend, gateway, a second terminal) mapping a
    venv ``.pyd`` is what keeps the automatic install failing, so they must be closed first.
    """
    if windows:
        return (
            "  Close every ShellGPT window (Desktop app, gateways, other terminals), then from a "
            "new terminal run:",
            f'    cd /d "{root}"',
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\shellgpt.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``shellgpt.exe`` never matches.
...
def _venv_shim_matcher():
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _shellgpt_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_shim_holder_pid() -> int:
    """Pid a detached child must outwait before touching the venv: the ``shellgpt.exe`` launcher
...
def _windows_running_shellgpt_launcher_locked() -> bool:
    """True when a venv ``shellgpt*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "SHELLGPT_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim(gateway_resume: dict | None = None) -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``shellgpt update`` so the sync and its tail happen
    ...
    ``venv\\Scripts\\shellgpt.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    from shellgpt_cli.update_handoff import detached_shim_child_env
    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "shellgpt_cli.main", *sys.argv[1:]]
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `shellgpt update` starts, so an update running
...
def _venv_scripts_dir() -> Path | None:
    ...
    from shellgpt_cli.main import PROJECT_ROOT
    from shellgpt_constants import project_venv_dir, venv_bin_dir
...
def _shellgpt_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    names = set(_load_console_script_names()) or {"shellgpt", "shellgpt-agent", "shellgpt-acp"}
    # Not a [project.scripts] entry point, but older update/install paths still
    # rewrite and quarantine it.
    names.add("shellgpt-gateway")
...
def _quarantine_running_shellgpt_exe(
    scripts_dir: Path, *, max_attempts: int = 4, failed_out: list[str] | None = None
) -> list[tuple[Path, Path]]:
    """Pre-empt the Windows file lock on the running ``shellgpt.exe``.
    ...
    transient; ShellGPT Desktop backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _shellgpt_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close ShellGPT Desktop, exit other `shellgpt` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `shellgpt update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older ShellGPT versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _shellgpt_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_shellgpt_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``shellgpt``
...
class ShimQuarantineError(RuntimeError):
    """A live ``shellgpt*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``shellgpt.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``shellgpt.exe``
    can be neither deleted nor overwritten, so without quarantine ``shellgpt`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_shellgpt_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `shellgpt` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``shellgpt.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``shellgpt.exe`` missing while ``shellgpt.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``shellgpt.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``shellgpt.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `shellgpt update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from shellgpt_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``shellgpt*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves shellgpt.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``shellgpt.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``shellgpt`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``shellgpt-agent.exe`` and ``shellgpt-acp.exe`` present but ``shellgpt.exe`` missing, so
    ``shellgpt`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `shellgpt update --force` after "
            "closing other shellgpt processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m shellgpt_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``shellgpt update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `shellgpt update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `shellgpt update --force` after closing other shellgpt processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `shellgpt update --force` after closing other shellgpt processes.",

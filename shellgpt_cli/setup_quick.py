"""Streamlined setup flows: the Nous Portal one-shot (`shellgpt portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``shellgpt_cli.setup`` take effect."""
...
logger = logging.getLogger("shellgpt_cli.setup")
...
def _blank_slate_done(config: dict, shellgpt_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from shellgpt_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         shellgpt skills opt-in --sync",
          "  Add MCP servers:     shellgpt mcp add", *extra, "  Tune agent settings: shellgpt setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from shellgpt_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``shellgpt model``. False when cancelled or failed (the
    message is already printed)."""
    from shellgpt_cli.setup import _info
    try:
        from shellgpt_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``shellgpt setup --portal`` / ``shellgpt portal``)."""
    from shellgpt_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ☤ ShellGPT Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print()
        lead, *rest = provider_setup_failure_lines(exc, retry_command="shellgpt portal")
        print_error(f"  {lead}")
        for line in rest:
            print_info(f"  {line}")

    if not _run_nous_flow(config, context="`shellgpt portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `shellgpt portal`."),
    ...
    _info("  Run `shellgpt portal info` to inspect routing.", "  Run `shellgpt` to start chatting.")


def _run_first_time_quick_setup(config: dict, shellgpt_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'shellgpt setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `shellgpt import`).
        from shellgpt_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    shellgpt setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  shellgpt setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and ShellGPT.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every ShellGPT update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``shellgpt-agent`` skill
    ...
    try:
        ...
        from shellgpt_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "shellgpt-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("shellgpt-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``shellgpt setup agent``."""
...
def _run_blank_slate_setup(config: dict, shellgpt_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `shellgpt-agent` skill is always kept so the agent",
          "can help you drive and configure ShellGPT itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, shellgpt_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `shellgpt update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, shellgpt_home, "  Enable tools:        shellgpt tools", "  Enable plugins:      shellgpt plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, shellgpt_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `shellgpt-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`shellgpt update` runs from re-injecting them. Opt back in any",
              "time with `shellgpt skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `shellgpt tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `shellgpt plugins list` / `shellgpt plugins install`.",
         "No plugins enabled. Add later with `shellgpt plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `shellgpt mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `shellgpt mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, shellgpt_home, "  Enable more tools:   shellgpt tools")


def _run_quick_setup(config: dict, shellgpt_home):
    ...
    from shellgpt_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'shellgpt setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect ShellGPT to messaging apps to chat from anywhere.",
              "You can configure these later with 'shellgpt setup gateway'.")

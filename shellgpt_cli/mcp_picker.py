"""MCP picker — interactive `shellgpt mcp picker` (also the default `shellgpt mcp`)."""
...
from shellgpt_cli.colors import Colors
from shellgpt_cli.cli_output import prompt_yes_no
from shellgpt_cli.curses_ui import curses_single_select
from shellgpt_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new ShellGPT session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from shellgpt_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `shellgpt mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: shellgpt mcp install <name>    Picker: shellgpt mcp", Colors.DIM)
    # Manifest-version warnings: the user's ShellGPT is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer ShellGPT — run `shellgpt update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`shellgpt mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`shellgpt mcp picker` (and default `shellgpt mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`shellgpt mcp install <name>` — non-interactive entry-point."""
    from shellgpt_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `shellgpt mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
...
_PLUGIN_COMPAT_LAZY = {
    'color': ('shellgpt_cli.colors', 'color'),
}

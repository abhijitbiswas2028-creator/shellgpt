"""Runtime-backed validation behind ``shellgpt plugins doctor``: every manifest/import/registration
...
def _doctor_runtime(plugin_path: Path):
    ...
    SHELLGPT_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        # The temp dir enters the stack FIRST so any failure below (ENOSPC / KeyboardInterrupt in
        # copytree) removes it instead of stranding a shellgpt-plugin-doctor-* directory.
        home = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="shellgpt-plugin-doctor-")))
        ...
        stack.enter_context(patch.dict(os.environ, {
            ...
            "SHELLGPT_BUNDLED_PLUGINS": str(bundled),
            "SHELLGPT_ENABLE_PROJECT_PLUGINS": "0",
        }, clear=False))
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(f"ShellGPT discovery found no valid plugin manifest under {copied}")
        if len(manifests) != 1:
        ...
        # SHELLGPT_HOME still exists. This runs the host-owned ctx.on_unload(...)
...
def _is_plugin_module(name: str) -> bool:
    return name == "shellgpt_plugins" or name.startswith("shellgpt_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        candidates.append(get_shellgpt_home() / "plugins" / raw)
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".shellgpt" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    import re as _re
    from shellgpt_cli.plugins import SUPPORTED_MANIFEST_VERSION
    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this ShellGPT supports "
    ...
    if missing:
        report.warning(
            ...
            + " — ShellGPT never auto-installs plugin dependencies; install manually: pip install "
            + " ".join(f"'{m}'" for m in missing))

    schema = getattr(manifest, "config_schema", {}) or {}
    if schema:
        from shellgpt_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through ShellGPT' real scanner and registration path."""
    try:

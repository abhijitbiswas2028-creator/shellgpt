...
from shellgpt_cli.local_runtime.context_policy import (
...
from shellgpt_cli.local_runtime.gguf import model_id_from_stem, read_gguf_header
...
def preset_for_model(gguf: Path, budget: HardwareBudget,
    ...
    from shellgpt_cli.local_runtime.catalog import entry_for_model
    from shellgpt_cli.local_runtime.growth import load_window_overrides
...
def admitted_residency_count(models_dir: Path, budget: HardwareBudget, configured: int) -> int:
    ...
    """
    from shellgpt_cli.local_runtime.bootstrap import staged_in
...
def generate_presets(models_dir: Path, budget: HardwareBudget, preset_path: Path,
    ...
    for gguf in staged_in(models_dir):
        ...
        sections.append("# shellgpt-decision: " + json.dumps({
...
def read_preset_decisions(preset_path: Path | None = None) -> dict[str, PresetEntry]:
    ...
    try:
        ...
        for line in text.splitlines():
            if line.startswith("# shellgpt-decision: "):
                fact = json.loads(line.removeprefix("# shellgpt-decision: "))

...
class CLITuiRuntimeMixin:
    ...
    def _tui_print_startup(self):
        ...
        _welcome_text = "Welcome to ShellGPT Agent! Type your message or /help for commands."
    ...
    def _tui_startup_prewarm_and_warnings(self, _welcome_skin):
        ...
        with suppress(Exception):
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(SHELLGPT_REDACT_SECRETS={_redact_raw}). "
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from shellgpt_cli.config import get_config_path as _get_cfg_path_resid
        ...
        if self._session_db and self.agent:
            ...
            if not self._delete_session_on_exit:
                ...
                try:
                    ...
                    if self._session_db.delete_session(_sid, sessions_dir=get_shellgpt_home() / "sessions"):

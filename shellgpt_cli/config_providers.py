...
Split out of ``shellgpt_cli/config.py``; every name is re-imported there, so
``shellgpt_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``shellgpt_cli.config``
...
logger = logging.getLogger("shellgpt_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but ShellGPT'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by ShellGPT, not hand-curated.
...
def get_custom_provider_session_affinity_header(
    ...
    Opt-in per provider (default off): ShellGPT never ships a session identifier to an endpoint

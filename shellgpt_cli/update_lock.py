"""Cross-process mutual exclusion for in-flight ShellGPT updates.
...
MARKER_NAME = ".shellgpt-update-in-progress"

# Set by an orchestrating updater (Tauri `shellgpt-setup --update`) to its own pid before
# spawning `shellgpt update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "SHELLGPT_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "ShellGPT is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* ShellGPT home (never the context-local profile override): the Rust
    updater resolves ``$SHELLGPT_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_shellgpt_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``shellgpt update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another ShellGPT update is already running (started {elapsed} ago, "
        f"process {holder.pid}).\n"
        "\n"
        "  Running two at once would corrupt the install. Wait for it to finish\n"
        "  (watch `shellgpt logs`), or close the Desktop/dashboard window that\n"
        "  started it, then run `shellgpt update` again."
    )
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        # nothing pre-writes a marker for `shellgpt update` (it always runs under a parent's claim).

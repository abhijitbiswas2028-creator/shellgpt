...
Imported by ``shellgpt_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
shellgpt_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
__all__ = [
    "project_root_str", "normalize_shellgpt_home_env", "ensure_project_root_on_path", "is_termux_env",
...
def normalize_shellgpt_home_env() -> None:
    """Expand ``~``/``$VAR`` in ``SHELLGPT_HOME`` once, at process entry, and write it back.

    fish does not expand ``~`` inside ``VAR=~/...`` and every shell passes a quoted value
    through verbatim, so a literal tilde reaches the process. ``Path("~/.shellgpt")`` is
    *relative*: the many raw ``os.environ["SHELLGPT_HOME"]`` readers (this fast path, the
    active_profile probe, profile re-home, the dotenv loader) would each resolve it against
    cwd and scaffold a full home under ``<cwd>/~/.shellgpt``. One expansion here gives every
    reader the same absolute spelling; ``shellgpt_constants`` expands as well for non-CLI
    entry points. A relative value that is not tilde/variable-shaped is left alone.
    """
    raw = os.environ.get("SHELLGPT_HOME", "")
    if not raw.strip():
        return
    expanded = os.path.expanduser(os.path.expandvars(raw.strip()))
    if expanded != raw:
        os.environ["SHELLGPT_HOME"] = expanded
...
def is_desktop_ssh_backend_argv(argv: list[str]) -> bool:
    ...
    ``SHELLGPT_DASHBOARD_SESSION_TOKEN`` env var the local pool spawn uses. Every reader of
...
def active_profile_may_override_home(shellgpt_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect SHELLGPT_HOME?"""
    active = (_read_text(os.path.join(shellgpt_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".shellgpt")


def _resolved_home() -> str:
    return os.environ.get("SHELLGPT_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("SHELLGPT_DEV") == "1" or is_container_startup_environment():
        return False
    shellgpt_home = os.environ.get("SHELLGPT_HOME", "").strip()
    if shellgpt_home:
        if os.path.exists(os.path.join(shellgpt_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(shellgpt_home)))
        return parent_name != "profiles" and active_profile_may_override_home(shellgpt_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``shellgpt --version`` output (also used by /version).
    ...
    try:
        from shellgpt_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from shellgpt_cli import __release_date__, __version__

        print(f"ShellGPT Agent v{__version__} ({__release_date__})")
    ...
    try:
        from shellgpt_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``shellgpt --version`` before the heavy import wall.
    ...
    SHELLGPT_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("SHELLGPT_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:

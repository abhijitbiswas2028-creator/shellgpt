"""Post-``shellgpt update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``shellgpt_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
from shellgpt_constants import project_venv_dir, venv_python_path
...
def _editable_finder_files(venv: Path) -> list[Path]:
    ...
    return [
        ...
        for site in sites
        if site.is_dir()
        for finder in site.glob("__editable__*shellgpt_agent*finder.py")
    ]
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    quarantines the running ``shellgpt.exe``, and a lost race is the ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "shellgpt_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from shellgpt_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from shellgpt_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__SHELLGPT_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        "import importlib, json, sys\n"
        # Importing shellgpt_cli.main runs the startup dotenv load, which pulls external secret
        ...
        # only our own packages count. Roots come from shellgpt_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('shellgpt_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``shellgpt tools``."""
    try:
        from shellgpt_cli import tools_config
        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active ShellGPT Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``shellgpt tools`` dependencies (from a pre-rebuild probe) into a rebuilt
    ...
    try:
        ...
        logger.debug("ShellGPT Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} ShellGPT Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `shellgpt update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `shellgpt update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not ShellGPT extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from shellgpt_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _reapply_plugin_python_dependencies() -> None:
    ...
    ``uv sync``/reinstall strips anything ShellGPT' own lock does not know). Non-memory plugins whose
    deps no longer resolve are disabled loudly, memory providers last. Never raises."""
    from shellgpt_cli.plugin_python_deps import reapply_all
    from shellgpt_cli.update_cmd import _m
    ...
    for name, reason in report.dropped:
        print(f"  ✗ Plugin '{name}' DISABLED: {reason}. Fix the plugin's declared dependencies, "
              f"then `shellgpt plugins enable {name}`.")
    if report.failed:
...
def _migrate_removed_memory_providers() -> None:
    ...
    try:
        from shellgpt_cli.memory_provider_migration import migrate_all_homes
        migrate_all_homes()
...
def _install_psutil_android_compat(
    ...
    from shellgpt_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    from shellgpt_cli.update_cmd import _m
    from shellgpt_cli.managed_uv import resolve_uv
    existing = resolve_uv()
    if existing:
        return existing
...
def _npm_lockfile_changed(shellgpt_root: Path) -> bool:
    from shellgpt_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    return not _npm_stamp_matches(shellgpt_root, current)


def _npm_lock_cache_file(shellgpt_root: Path, scope: str = "") -> Path:
    ...
    *scope* separates install closures that share the digest (workspace-scoped vs. full desktop)."""
    ...
    return shellgpt_root / f".npm_lock_hash_{cache_key}{scope}"


def _npm_stamp_matches(shellgpt_root: Path, current: str, scope: str = "") -> bool:
    """True when the recorded digest for *scope* equals *current*; a missing/unreadable stamp never matches."""
    try:
        return _npm_lock_cache_file(shellgpt_root, scope).read_text(encoding="utf-8").strip() == current
    except OSError:
        return False


def _clear_npm_lockfile_hash(shellgpt_root: Path, scope: str = "") -> None:
    ...
    with suppress(OSError):
        _npm_lock_cache_file(shellgpt_root, scope).unlink()


def _record_npm_lockfile_hash(shellgpt_root: Path, scope: str = "") -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(shellgpt_root, scope).write_text(digest, encoding="utf-8")
    except OSError:
...
def _desktop_deps_changed(shellgpt_root: Path) -> bool:
    ...
    current = _npm_manifests_digest()
    return current is None or not _npm_stamp_matches(shellgpt_root, current, DESKTOP_NPM_SCOPE)
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run shellgpt
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `shellgpt update`.")
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `shellgpt update`.")
            has_workspace = any(
    ...
    shared_shellgpt_root = get_default_shellgpt_root()
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `shellgpt update` case.
    # See #43564.
    ...
    if not _m()._npm_lockfile_changed(shared_shellgpt_root):
    ...
    nixos_env = with_shellgpt_node_path(_m()._nixos_build_env())
    ...
    # The chatty npm-deprecation noise during `shellgpt update` comes from the *desktop* build, not this step;
    # that one is captured to update.log. See #18840.
    _clear_npm_lockfile_hash(shared_shellgpt_root)
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_shellgpt_root)
    ...
    print("    deps). Fix npm and re-run `shellgpt update`.")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".shellgpt-bootstrap-complete", _m()._update_marker_path())
...
def _venv_dependency_set_stale() -> tuple[bool, str]:
    """Whether the venv's ``shellgpt-agent`` distribution was installed from an OLDER checkout than
    ...
    from shellgpt_cli.update_cmd import _m, _read_project_version
    ...
    try:
        result = subprocess.run(
            [str(venv_python), "-c", "import importlib.metadata as m; print(m.version('shellgpt-agent'))"],
    ...
    return stale, f"installed shellgpt-agent {installed}, checkout is {expected}" if stale else ""
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``shellgpt.exe`` shim
    ...
    if _m()._reexec_dependency_sync_off_windows_shim(gateway_resume):
        ...
        # (and shellgpt.exe locked) exactly while the child needs it gone (#101600).
        sys.exit(0)
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `shellgpt` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    # build stamp under SHELLGPT_HOME: it outlives a swap that lost the artifacts in an earlier run, and
    ...
    desktop_build_cmd = [sys.executable, "-m", "shellgpt_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put ShellGPT-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from shellgpt_constants import with_shellgpt_node_path
    build_env = with_shellgpt_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `shellgpt desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from shellgpt_constants import display_shellgpt_home as _dhh
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo shellgpt`` dies mid-update with ``venv/bin/shellgpt`` already
    ...
    See #83529.
    A later normal ``shellgpt update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/shellgpt`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from shellgpt_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave ShellGPT broken.")
    print("  This usually happens after running shellgpt or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    shellgpt update")
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `shellgpt` again "
    ...
    if not import_ok:
        ...
        print("    Run `shellgpt update` again — if it persists, reinstall:")
        print("    https://shellgpt-agent.nousresearch.com")

"""Dependency-light venv recovery that runs BEFORE shellgpt_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``shellgpt_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``shellgpt_cli.main`` to become importable again (force-reinstall of
...
# ``shellgpt update`` renames the live ``shellgpt*.exe`` shims aside (``shellgpt.exe.old.<unix-ms>``) so
# uv can write replacements. Putting them BACK is the safety-critical direction: losing that rename
# leaves no ``shellgpt`` on PATH, and the command that would repair it IS ``shellgpt update``. The
...
def _should_skip_external_secret_sources() -> bool:
    """True inside any ``shellgpt update`` process (and its import probes).

    Every dotenv load in the process — ``shellgpt_cli.main``, ``run_agent``, ``cli`` — consults
    ...
    Profile flags are stripped before ``shellgpt_cli.main`` loads dotenv, so ``argv[1]`` is
...
# ``shellgpt update`` writes this into the git dir right before git moves the checkout and removes it
...
# files already new; that mixed tree fails at import in every entry point, ``shellgpt update`` included.
INTERRUPTED_PULL_MARKER = "shellgpt-update-pull"
...
_RESTORE_CLAIM = "shellgpt-update-pull.claim"
...
def _held_open(path: Path) -> bool:
    ...
    macOS/BSD have no portable check at all. The claim only orders ShellGPT launches, so on those paths a
...
def restore_interrupted_pull(project_root: Path | None = None) -> bool:
    """Put back the files a killed ``shellgpt update`` had half-moved to the new commit.
    ...
    for), so the install is whole again and ``shellgpt update`` redoes the update from the start. Local
    ...
    Limits, by design: a torn ``shellgpt_cli/__init__.py``, ``shellgpt_bootstrap.py``, ``agent/__init__.py``
    or ``agent/jiter_preload.py`` (imported before the ``shellgpt-agent`` hook) fails before this runs. A file git also changes that the user deleted, emptied or cut to a prefix of git's version
    ...
    try:
        ...
        with _restore_claim(marker.parent) as claimed:
            if not claimed:
                print("⚠ Another ShellGPT launch is still repairing the checkout after an interrupted "
                      "`shellgpt update`; if this one fails, launch again in a moment.", file=sys.stderr)
                return False
        ...
        print(f"⚠ Could not check for an interrupted `shellgpt update`: {exc}", file=sys.stderr)
    return False
...
def _restore_holding_claim(root: Path, marker: Path) -> bool:
    ...
    if any((git_dir / name).exists() for name in _GIT_OPERATION_IN_PROGRESS):
        ...
        if (not _merge_advice_shown and merge_head.is_file()
            ...
            print(f"⚠ A killed `shellgpt update` left its merge unfinished. Run `git -C {root} merge --abort`, "
    ...
    if not _release_dead_index_lock(git_dir):
        print("⚠ A running git holds the index after an interrupted `shellgpt update`; the next launch "
    ...
    if restore or added:
        print("⚠ A previous `shellgpt update` was killed while git was writing the new code — "
    ...
    print("  ✓ Checkout restored; `shellgpt update` updates it again.", file=sys.stderr)
    if stash:
...
def relaunch_after_restore() -> None:
    ...
    Everything imported so far (this package, ``shellgpt_bootstrap``, ``shellgpt_cli.main`` itself) may
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which ShellGPT
    vendors (``~/.shellgpt/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".shellgpt", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``shellgpt acp`` speaks JSON-RPC on stdout). uv is preferred when the
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``shellgpt_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    """Repair wiped core packages so ``shellgpt_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls. Only acts when a recovery marker from a
    prior ``shellgpt update`` exists AND an import probe confirms a core package is actually broken.
    ...
    try:
        ...
        # recovered even when this launch is itself `shellgpt update`: CLI and Desktop retries keep
...
# recovery: a persistently failing install (network down, a .pyd another ShellGPT process maps) must
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``recover_if_needed`` invokes this when ``.update-incomplete`` exists — a prior ``shellgpt update`` (or
    ...
    venv extension mapped; this process, running before ``shellgpt_cli.main``'s third-party imports, maps
    nothing yet, so the installer can replace ``.pyd`` files without hitting the lock.
    """
    try:
        from shellgpt_cli import _install_repair as ir
        ...
        if not _claim_recovery_lock(root):
            return False
        try:
            print("⚠ A previous `shellgpt update` was interrupted mid-install — "
            ...
            if new_attempts < _CORE_INSTALL_MAX_ATTEMPTS:
                print("  The next launch will retry; shellgpt will keep working from "

...
The provider list shown by ``shellgpt model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`shellgpt_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `shellgpt model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``shellgpt model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("shellgpt_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("shellgpt_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    SHELLGPT_OVERLAYS = _safe_import("shellgpt_cli.providers", "SHELLGPT_OVERLAYS", {})
    try:
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = SHELLGPT_OVERLAYS.get(slug)

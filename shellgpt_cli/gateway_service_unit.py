...
Extracted from ``shellgpt_cli/gateway.py``. Bodies read facade helpers through ``_gw()`` (late
binding on ``shellgpt_cli.gateway``) so the seams tests and callers patch on the facade keep
...
def _gw():
    from shellgpt_cli import gateway  # late: the facade imports this module
    return gateway
...
def _ld_library_path_line(system: bool, target_home_dir: str | None = None) -> str:
    ...
    ~/.shellgpt/.env is too late for CUDA libs — #14613); system units remap caller-home components.
...
def _shellgpt_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SHELLGPT_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.shellgpt[/profiles/x]`` → ``/home/alice/.shellgpt[/profiles/x]``; custom paths kept as-is."""
    current_shellgpt_raw = os.environ.get("SHELLGPT_HOME", "").strip()
    current_shellgpt = Path(current_shellgpt_raw).expanduser() if current_shellgpt_raw else _gw().get_shellgpt_home()
    # Keep paths lexical: resolving a non-existent path can bake a different SHELLGPT_HOME into the unit.
    current_default = Path.home() / ".shellgpt"
    target_default = Path(target_home_dir) / ".shellgpt"
    try:
        # Default ~/.shellgpt or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_shellgpt.relative_to(current_default))
    except ValueError:
        return str(current_shellgpt)  # Completely custom path (not under ~/.shellgpt) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    shellgpt_home = _gw().get_shellgpt_home()
    extras = (project_root / "node_modules" / ".bin", shellgpt_home / "node" / "bin", shellgpt_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (SHELLGPT_HOME, else _gw().PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(shellgpt_home: str | Path | None = None) -> int:
    ...
    if shellgpt_home is not None:
        ...
        override_token = set_shellgpt_home_override(shellgpt_home)
        reset_home_override = reset_shellgpt_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], shellgpt_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<shellgpt_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from shellgpt_constants import (shellgpt_managed_node_tree_present, iter_shellgpt_node_dirs)
    managed_node_present = shellgpt_managed_node_tree_present(shellgpt_root)
    for directory in iter_shellgpt_node_dirs(shellgpt_root) if managed_node_present else ():
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        shellgpt_home = _gw()._shellgpt_home_for_target_user(home_dir)
        # Profile arg relative to the TARGET user's ~/.shellgpt when shellgpt_home lives under it.
        target_root = Path(home_dir) / ".shellgpt"
        try:
            Path(shellgpt_home).resolve().relative_to(target_root.resolve())
            profile_arg = _gw()._profile_arg(shellgpt_home, default_root=target_root)
        except ValueError:
            profile_arg = _gw()._profile_arg(shellgpt_home)
        ...
        working_dir = str(shellgpt_home) if shellgpt_home else _gw()._remap_path_for_user(working_dir, home_dir)
        ...
        _gw()._append_node_dir_for_service(_target_node_entries, Path(shellgpt_home) if shellgpt_home else None)
        ...
        shellgpt_home = str(_gw().get_shellgpt_home().resolve())
        profile_arg = _gw()._profile_arg(shellgpt_home)
        user_home = Path.home()
    ...
    watchdog_seconds = _gw()._systemd_watchdog_seconds(shellgpt_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m shellgpt_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="SHELLGPT_HOME={shellgpt_home}"
Environment="SHELLGPT_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__SHELLGPT_PATH__\3",
        _gw()._normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # SHELLGPT_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.shellgpt, so refresh would rewrite a correct unit and status warn forever.
    # Idempotent; the os.environ mutation persists for later runtime reads (restart's PID/drain).
    _gw()._sync_shellgpt_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir SHELLGPT_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'SHELLGPT_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>SHELLGPT_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
        Path(tempfile.gettempdir()).resolve(),
        Path("/tmp"), Path("/var/tmp"), Path("/private/tmp"), Path("/private/var/tmp"),  # no-tmp: ok — detects a temp SHELLGPT_HOME in service definitions
    }
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp SHELLGPT_HOME."""
    temp_home = _gw()._temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: SHELLGPT_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported SHELLGPT_HOME. "
        "Unset it (or run from a clean shell) and retry."
    )
    return True


def _retire_shellgpt_replace_dropin(system: bool = False) -> bool:
    """Unlink the ``20-replace.conf`` drop-in an older ShellGPT wrote to end a respawn storm; True if removed.
    ...
    start (#119467). Only the ShellGPT-authored file (recognised by its own comment) is touched.
    """
    unit_path = _gw().get_systemd_unit_path(system=system)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # _gw().systemd_unit_is_current is the SHELLGPT_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    current = _gw().systemd_unit_is_current(system=system)
    if _retire_shellgpt_replace_dropin(system=system):
        _gw()._run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
        print(f"↻ Removed the stale ShellGPT --replace drop-in from the gateway {_gw()._service_scope_label(system)} service")
        if current:
            return True
    elif current:
        return False
    ...
    # pytest-tmp SHELLGPT_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/shellgpt_test"', "/shellgpt_test/")):
        return False

    # Structural variant: refuse ANY temp-dir SHELLGPT_HOME (manual E2E homes lack the pytest markers).
    if _gw()._refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _gw()._run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_gw()._service_scope_label(system)} service definition to match the current ShellGPT install")
    return True

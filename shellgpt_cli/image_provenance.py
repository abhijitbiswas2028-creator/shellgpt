"""Image-authored deployment provenance for immutable ShellGPT runtimes.

The image bakes ``/etc/shellgpt/image-provenance.json`` outside ``$SHELLGPT_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/shellgpt/image-provenance.json")

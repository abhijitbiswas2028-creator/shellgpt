...
_CA_BUNDLE_ENV_VARS = ("SHELLGPT_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE")
...
# Building the context parses the whole CA bundle (~4 ms for certifi) on EVERY ShellGPT-owned request,
...
def _resolved_https_context() -> ssl.SSLContext | None:
    """Return the shared explicit-CA context for ShellGPT-owned urllib openers.
...
def _secure_opener_from_installed_policy(original_url: str, *, ssl_context=None):
    ...
    (``ssl_ca_cert``/``ssl_verify``) apply; with None a ShellGPT-owned opener gets the explicit CA
    ...
    secured._shellgpt_initial_addheaders = list(getattr(installed, "addheaders", ()))
    secured.addheaders = []
    return secured
...
def open_credentialed_url(
    ...
    *,
    ...
    if opener_factory is None:
        ...
        for name, value in getattr(opener, "_shellgpt_initial_addheaders", ()):

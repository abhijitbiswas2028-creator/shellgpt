...
def activate_plugin_now(name: str, *, in_process: bool = True) -> Dict[str, Any]:
    ...
    in another process (``shellgpt plugins install``) passes ``in_process=False``; the running Desktop /
    ...
    if not in_process:
        activation = (notify_serve_backend(name, Path(get_shellgpt_home())) or {}).get("activation")
    answer = None
    try:
        ...
        if answer is None:
            root = Path(get_default_shellgpt_root())
...
def _go_live(name: str) -> Optional[Dict[str, Any]]:
    from shellgpt_cli.plugins_activation_live import connect_plugin_mcp, live_notice, plugin_skills
    try:
        from shellgpt_cli.plugins import _join_background_discovery, get_plugin_manager
    ...
    if server is not None and (servers or note):
        from shellgpt_constants import get_shellgpt_home
        try:
            server.refresh_plugin_sessions(Path(get_shellgpt_home()), note)
        except Exception:
...
def notify_serve_backend(name: str, home: Path) -> Optional[Dict[str, Any]]:
    """Ask the running dashboard / Desktop backend (``shellgpt serve``, found through its host record) to
    ...
    try:
        ...
        request = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json", "X-ShellGPT-Session-Token": token})

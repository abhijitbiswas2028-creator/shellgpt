"""Directory initialization and storage diagnostics for the active ShellGPT home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool, home: Path) -> None:
    from shellgpt_cli.config import _secure_dir
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize ShellGPT directory {path}: {exc}. "
            ...
            "ShellGPT has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from shellgpt_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize ShellGPT home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from shellgpt_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"ShellGPT storage is unavailable: {exc}",
            "Check the reported path, link target, mount and permissions; keep config.yaml unchanged.",
        )
    return ConfigIssue("error", "Could not load config.yaml", "Run 'shellgpt setup' to create a valid config")

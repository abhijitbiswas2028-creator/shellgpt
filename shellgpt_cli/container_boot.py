...
01-shellgpt-setup (the stage2 hook) has chowned the volume and seeded $SHELLGPT_HOME, but
...
from shellgpt_cli.gateway_multiplex_s6 import AUTOSTART_STATES as _AUTOSTART_STATES, fold_named_slot_intent
...
def _named_profile_dirs(shellgpt_home: Path) -> list[tuple[str, Path]]:
    """Every real named profile under ``$SHELLGPT_HOME/profiles`` (``SOUL.md`` is the marker)."""
    profiles_root = shellgpt_home / "profiles"
...
def reconcile_profile_gateways(
    *, shellgpt_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$SHELLGPT_HOME``): ``shellgpt_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``shellgpt gateway start`` (no ``-p``) targets.

    Without it, bare ``shellgpt gateway start`` inside the container would land on ``s6-svc -u
    /run/service/gateway-default`` → uncaught ``CalledProcessError`` → traceback to the user (PR #30136
    review).
    """
    actions: list[ReconcileAction] = []
    # ONE gateway per container: named slots are registered (so `shellgpt -p X gateway start` has a
    ...
    # as a topology switch (shellgpt_cli/gateway_multiplex_mode.py), so there is nothing to read.
    named = [(name, entry, _read_desired_state(entry)) for name, entry in _named_profile_dirs(shellgpt_home)]
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        shellgpt_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(shellgpt_home)
    # The root slot INHERITS every named slot's autostart intent, because it is the one process
    # that serves them. Without this an image only ever driven as `shellgpt -p coder gateway start`
    ...
    if not dry_run:
        _cleanup_stale_runtime_files(shellgpt_home)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", shellgpt_home, default_prior_state, default_should_start,
    ...
    if not dry_run:
        _write_reconcile_log(shellgpt_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    shellgpt_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = shellgpt_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the shellgpt args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "shellgpt":  # the wrapper re-execs `shellgpt <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `shellgpt -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with shellgpt ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the shellgpt user won't EACCES.
...
def _write_reconcile_log(shellgpt_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $SHELLGPT_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted SHELLGPT_HOME both
    ...
    actions = reconcile_profile_gateways(shellgpt_home=shellgpt_home, scandir=scandir)

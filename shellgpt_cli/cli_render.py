...
def _detect_light_mode_uncached() -> bool:
    ...
    for var in ("SHELLGPT_LIGHT", "SHELLGPT_TUI_LIGHT"):
        v = (os.environ.get(var) or "").strip().lower()
        if _TRUE_RE.match(v):
            return True
        if _FALSE_RE.match(v):
            return False
    theme = (os.environ.get("SHELLGPT_TUI_THEME") or "").strip().lower()
    if theme == "light":
        return True
    if theme == "dark":
        return False
    bg_lum = _luminance_from_hex(os.environ.get("SHELLGPT_TUI_BACKGROUND") or "")
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from shellgpt_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_shellgpt_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._shellgpt_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _build_compact_banner() -> str:
    ...
    if (getattr(_skin, "name", "default") if _skin else "default") == "default":
        tiny_line = "☤ NOUS SHELLGPT"
    else:
        tiny_line = _skin.get_branding("agent_name", "ShellGPT Agent") if _skin else "ShellGPT Agent"
    line1 = f"{tiny_line} - AI Agent Framework"

    if os.environ.get("SHELLGPT_FAST_STARTUP_BANNER") == "1":
        from shellgpt_cli import __release_date__ as _release_date
        from shellgpt_cli import __version__ as _version

        version_line = f"ShellGPT Agent v{_version} ({_release_date})"
    else:

"""Migrate ShellGPT MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end shellgpt-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in ShellGPT config.")
        ...
        if self.preserved_user_servers:
            lines.append(
                ...
                f"config.toml (ShellGPT projection skipped): {', '.join(self.preserved_user_servers)}")
        lines.extend(f"⚠ {err}" for err in self.errors)
        return "\n".join(lines)


# ShellGPT MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown ShellGPT keys.
_KNOWN_SHELLGPT_KEYS = {
...
# (shellgpt key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, shellgpt_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one ShellGPT MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. ShellGPT' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(shellgpt_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if shellgpt_cfg.get("command"):
        if shellgpt_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(shellgpt_cfg["command"])
        if shellgpt_cfg.get("args"):
            out["args"] = [str(a) for a in shellgpt_cfg["args"]]
        if shellgpt_cfg.get("env"):
            out["env"] = _str_map(shellgpt_cfg["env"])
        if shellgpt_cfg.get("cwd"):
            out["cwd"] = str(shellgpt_cfg["cwd"])
    elif shellgpt_cfg.get("url"):
        out["url"] = str(shellgpt_cfg["url"])
        if shellgpt_cfg.get("headers"):
            out["http_headers"] = _str_map(shellgpt_cfg["headers"])
        if shellgpt_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for shellgpt_key, codex_key, note in _TIMEOUT_KEYS:
        if shellgpt_key in shellgpt_cfg:
            try:
                out[codex_key] = float(shellgpt_cfg[shellgpt_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if shellgpt_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in shellgpt_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_SHELLGPT_KEYS:
            skipped.append(f"{key} (unknown ShellGPT key)")
    return out, skipped
...
# env-var passthrough (SHELLGPT_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by ShellGPT)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _unmanaged_mcp_server_names(toml_text: str) -> set[str]:
    ...
    block is the user's. A ShellGPT server whose name is already declared by the user is therefore
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_bin=codex_bin, codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="shellgpt-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a SHELLGPT_HOME pointing there burned into
...
def _build_shellgpt_tools_mcp_entry() -> dict:
    """Codex stdio entry launching ShellGPT' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    SHELLGPT_HOME passes through only IF SET, read from os.environ (not get_shellgpt_home()): when
    unset the codex subprocess must inherit its launcher's runtime SHELLGPT_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched SHELLGPT_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched shellgpt finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # SHELLGPT_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_shellgpt_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever SHELLGPT_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's SHELLGPT_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("SHELLGPT_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    shellgpt_home = os.environ.get("SHELLGPT_HOME") or ""
    if shellgpt_home and not _looks_like_test_tempdir(shellgpt_home):
        env["SHELLGPT_HOME"] = shellgpt_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["SHELLGPT_QUIET"] = "1"
    env["SHELLGPT_REDACT_SECRETS"] = env.get("SHELLGPT_REDACT_SECRETS", "true")
    return {
...
def migrate(
    shellgpt_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_shellgpt_tools: bool = True) -> MigrationReport:
    """Translate ShellGPT mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_shellgpt_tools`` registers ShellGPT' own tool surface
    (agent/transports/shellgpt_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    shellgpt_servers = (shellgpt_config or {}).get("mcp_servers") or {}
    if not isinstance(shellgpt_servers, dict):
        report.errors.append("mcp_servers in ShellGPT config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in shellgpt_servers.items():
        name = str(raw_name)
    ...
    if discover_plugins and not dry_run:
        from shellgpt_cli.codex_runtime_switch import get_configured_codex_binary
        plugins, plugin_err = _query_codex_plugins(
            codex_home=codex_home, codex_bin=get_configured_codex_binary(shellgpt_config))
        if plugin_err:
    ...
    if expose_shellgpt_tools:
        translated[SHELLGPT_TOOLS_MCP_SERVER_NAME] = _build_shellgpt_tools_mcp_entry()
        if SHELLGPT_TOOLS_MCP_SERVER_NAME not in report.migrated:
            report.migrated.append(SHELLGPT_TOOLS_MCP_SERVER_NAME)
    without_managed = ""
    if target.exists():
        try:
        ...
        if report.plugin_query_error:
            try:
                ...
                report.plugin_query_error += (
                    "; existing config.toml was unloadable — re-run `shellgpt codex-runtime migrate` "
                    "to migrate plugins")

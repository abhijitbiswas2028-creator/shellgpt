...
Split out of ``shellgpt_cli/auth.py``; origin helpers are imported lazily inside each function
so ``shellgpt_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from shellgpt_cli.auth_codex import _pool_entries
from shellgpt_cli.auth_constants import (
...
# Log-record parity with the origin module (caplog tests pin "shellgpt_cli.auth").
logger = logging.getLogger("shellgpt_cli.auth")

_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: shellgpt auth add nous"
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("SHELLGPT_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    from shellgpt_cli.nous_account import (
...
def _scoped_operator_override(*names: str) -> Optional[str]:
    ...
    ``SHELLGPT_PORTAL_BASE_URL`` / its ``NOUS_PORTAL_BASE_URL`` alias), resolved through the profile
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    """
    from shellgpt_cli.auth import _optional_base_url
    return _optional_base_url(_scoped_operator_override("SHELLGPT_PORTAL_BASE_URL", "NOUS_PORTAL_BASE_URL"))
...
def _nous_invoke_jwt_status(
    ...
    from shellgpt_cli.auth import _is_expiring
...
def _nous_invoke_jwt_is_usable(
    ...
    from shellgpt_cli.auth import _nous_invoke_jwt_status
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from shellgpt_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nous_agent_key_from_invoke_jwt(
    ...
    from shellgpt_cli.auth import _nonempty_str
...
def _nous_shared_auth_dir() -> Path:
    """Directory of the shared Nous token store: ``SHELLGPT_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``shellgpt --profile X auth add nous --type
    ...
    """
    override = os.getenv("SHELLGPT_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from shellgpt_constants import get_default_shellgpt_root
    return get_default_shellgpt_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt (mirrors the _auth_file_path() guard): under pytest, refuse a path under the real
    # user's ShellGPT root so a test that forgot SHELLGPT_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from shellgpt_constants import get_default_shellgpt_root
        real_home_shared = (
            get_default_shellgpt_root() / "shared" / NOUS_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set SHELLGPT_SHARED_AUTH_DIR to a tmp_path in your test fixture.")
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        yield  # No SHELLGPT_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    from shellgpt_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nous_state
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    """
    from shellgpt_cli.auth import _nonempty_str, _save_private_json
...
def _refresh_access_token(
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    from shellgpt_cli.auth import _OAUTH_GRANT_DEAD_CODES
    try:
    ...
    # self-heal hook) redeemed ShellGPT's refresh_token without persisting the rotated token, so the
    ...
    if code == "refresh_token_reused" or "reuse" in description.lower():
        description = (
            ...
            "custom self-heal hook, or another ShellGPT install sharing "
            "~/.shellgpt/auth.json) called POST /api/oauth/token with ShellGPT's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only ShellGPT may call the "
            "refresh endpoint. For health checks, use `shellgpt auth status` "
            "instead.\n"
            "Re-authenticate with: shellgpt auth add nous")
        relogin = True
...
def _refresh_nous_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from shellgpt_cli.auth import _refresh_access_token, _is_terminal_nous_refresh_error
    try:
...
def _apply_nous_refreshed_tokens(
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from shellgpt_cli.auth import _coerce_ttl_seconds
...
def fetch_nous_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    ...
    for item in data:
        ...
        # ShellGPT models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "shellgpt" not in model_id.lower():
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    ...
    it.
    """
    from shellgpt_cli.auth import _save_active_provider_state, _write_shared_nous_state
...
def _nous_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    """
    from shellgpt_cli.auth import _NOUS_PORTAL_ALLOWED_HOSTS, _optional_base_url
...
class _NousRuntimeResolve:
    ...
    def persist(self, reason: str) -> None:
        from shellgpt_cli.auth import _save_provider_state_to_source, _write_shared_nous_state
        state = self.state
...
def resolve_nous_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    """
    from shellgpt_cli.anon_auth import AnonCredentialDead, clear_dead_guest, ensure_portal_identity
    try:
        ...
        from shellgpt_cli.anon_auth import ANON_ACCOUNT_LOCKED
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    from shellgpt_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _pool_first_oauth_status(
    ...
    Pool first (where `shellgpt auth` / `shellgpt model` store device_code tokens), then
...
def _nous_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    portal_base_url = (
        portal_base_url or os.getenv("SHELLGPT_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    ...
    if _is_remote_session():
        open_browser = False
    print(f"Starting ShellGPT login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `shellgpt model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _mirror_nous_state_best_effort(auth_state: Dict[str, Any]) -> None:
    ...
    from shellgpt_cli.auth import _sync_nous_pool_from_auth_store, _write_shared_nous_state
    with suppress(Exception):
...
def _pick_nous_model_after_login(
    ...
    from shellgpt_cli.models_pricing import (
...
def _offer_shared_nous_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    ...
    """
    from shellgpt_cli.auth import _prompt_yes_no, _read_shared_nous_state
    from shellgpt_cli.anon_auth import is_guest_state
    shared = _read_shared_nous_state()
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("SHELLGPT_CA_BUNDLE")
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `shellgpt model` again to switch to Nous Portal.")
            return
        ...
        from shellgpt_cli.auth_error_copy import sign_in_failure_lines

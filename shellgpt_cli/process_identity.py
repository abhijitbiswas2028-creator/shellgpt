...
Three layers make every long-lived ShellGPT process positively identifiable, so reapers (``shellgpt
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``SHELLGPT_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from shellgpt_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``SHELLGPT_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``SHELLGPT_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_shellgpt_root()) / LEDGER_FILENAME
    except Exception:
        from shellgpt_cli.config import get_shellgpt_home

        return Path(get_shellgpt_home()) / LEDGER_FILENAME
...
def register_self(purpose: str, *, project_root: Optional[Path] = None, detail: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        # 10 tokens: enough for `shellgpt serve --host X --port N --profile P` while bounding
...
def is_desktop_owned_backend(argv: Optional[Sequence[str]] = None) -> bool:
    ...
    ``SHELLGPT_DESKTOP=1`` is inherited by every shell and agent child the app launches, so the
    ...
    strips from agent children): the local pool spawn mints ``SHELLGPT_DASHBOARD_SESSION_TOKEN``,
    the SSH spawn passes a 0600 token FILE on argv and deliberately sets no token env var.
    ``argv`` defaults to this process's own.
    """
    if os.environ.get("SHELLGPT_DESKTOP") != "1":
        return False
    if os.environ.get("SHELLGPT_DASHBOARD_SESSION_TOKEN"):
        return True
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's SHELLGPT_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("SHELLGPT_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("SHELLGPT_PARENT_START_MARKER", "")
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import ShellGPT code). Records the child's

"""Dispatcher and maintenance verbs for ``shellgpt kanban``: ``dispatch``,
...
from shellgpt_cli.kanban_output import _err, _fmt_ts, _print_json
...
_DAEMON_DEPRECATED = (
    "shellgpt kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    shellgpt gateway start       # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"successfully.{held} Check profile health (venv, PATH, credentials) and `shellgpt "
                    f"kanban list --status ready` / `shellgpt kanban list --status blocked` for "

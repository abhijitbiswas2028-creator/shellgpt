...
def get_launchd_label() -> str:
    ...
    return f"ai.shellgpt.gateway-{suffix}" if suffix else "ai.shellgpt.gateway"
...
def _launchd_domain() -> str:
    ...
    See #40831, #23387.
    """
    # The cache lives on the facade: tests and callers reset ``shellgpt_cli.gateway._resolved_launchd_domain``.
    gw = _gw()
...
# at all and we degrade to a detached background process (the `nohup shellgpt gateway run` workaround). See
# #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return _gw().get_shellgpt_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return _gw().get_shellgpt_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m shellgpt_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [_gw().get_python_path(), "-m", "shellgpt_cli.main", *_gw()._profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``shellgpt update`` hands back to
    ...
    ``--external-supervisor`` so ``shellgpt update`` sees the flag on the live grandchild argv and hands the
    ...
    return [_gw().get_python_path(), "-m", "shellgpt_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup shellgpt gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
    ...
    log_dir = _gw().get_shellgpt_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"  Try manually: nohup shellgpt gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def _launchd_degrade_or_raise(exc: subprocess.CalledProcessError, what: str) -> None:
    ...
    if _gw()._launchctl_label_supervising_process(label):
        ...
        print("  Apply the definition with: shellgpt gateway stop && shellgpt gateway install --force")
        raise exc
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from shellgpt_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <string>{shellgpt_home}</string>
        <key>SHELLGPT_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if not _gw()._retry_launchctl_bootstrap_until_registered(domain, plist_path, label, deadline=_deadline):
        ...
        return False
    print("↻ Updated gateway launchd service definition to match the current ShellGPT install")
    return True
...
def launchd_install(force: bool = False, *, start_now: bool = True):
        ...
        if load:
            ...
            if _gw().refresh_launchd_plist_if_needed():
                ...
                print(
                    ...
                    "Run 'shellgpt gateway install --force' or check "
                    f"{display_shellgpt_home()}/logs/launchd-reload.log for details."
                )
            return
    ...
    if not load:
        ...
        # what `shellgpt gateway start` would kickstart instead of loading this plist.
        subprocess.run(
        ...
        print()
        print("Next steps:")
        print("  shellgpt gateway start              # Start it now")
        print("  shellgpt gateway status             # Check status")
        return
...
def launchd_stop():
    ...
    # bootout unloads the definition so KeepAlive doesn't respawn; `shellgpt gateway start` re-bootstraps.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and _gw().probe_gateway_loop_liveness(pid) == _gw().GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `shellgpt update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # ShellGPT PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if _gw().launchd_plist_is_current():
        print("✓ Service definition matches the current ShellGPT install")
    else:
        print("⚠ Service definition is stale relative to the current ShellGPT install")
        print("  Run: shellgpt gateway start")
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: shellgpt gateway stop")
        else:
    ...
    if deep:
        log_file = _gw().get_shellgpt_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")

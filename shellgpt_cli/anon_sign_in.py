...
UPGRADE_UNAVAILABLE = "The free tier is not available right now; run `shellgpt auth add nous` to sign in."
UPGRADE_REASON_COPY = {
...
UPGRADE_NO_DEFAULT_TERMINAL = "No default model is set yet; run `shellgpt model` to pick one."
...
LOGIN_DM_ONLY = "Sign in from a direct message with ShellGPT."
LOGIN_BUSY_ELSEWHERE = "Another sign-in is already running on this ShellGPT. Try again in a few minutes."
LOGIN_NOT_ALLOWED = "Only an operator of this ShellGPT can sign it in."
...
# the in-chat form (never a raw exception, a URL or a ``shellgpt`` verb) and ``.copy_terminal`` the
...
class Failed(SignInState):
    ...
    def retryable(self) -> bool:
        from shellgpt_cli import anon_auth as _core
...
def run_sign_in(
    ...
    *client_factory* is the HTTP client seam, ``client_factory(timeout_seconds, verify)``.
    """
    from shellgpt_cli import anon_auth as _core
    from shellgpt_cli.auth import PROVIDER_REGISTRY, _resolve_verify
    from shellgpt_cli.auth_device_flow import _request_device_code
    from shellgpt_cli.auth_nous import _nous_http_client

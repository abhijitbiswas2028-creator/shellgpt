...
def _strip_shellgpt_path_lines(content: str) -> str:
    """Drop the ``# ShellGPT Agent`` marker (+ its PATH line) and any shellgpt PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# ShellGPT Agent' in line or '# shellgpt-agent' in line:
            skip_next = True
            continue
        if skip_next and ('shellgpt' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'shellgpt' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove ShellGPT PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_shellgpt_path_lines(content)
...
def remove_wrapper_script():
    """Remove the shellgpt wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a shellgpt_cli / shellgpt-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('shellgpt_cli' in content or 'shellgpt-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("shellgpt", "shellgpt-acp", "shellgpt-agent"))
...
def remove_node_symlinks(shellgpt_home: Path) -> list:
    ...
    node_dir = (shellgpt_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from shellgpt_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from shellgpt_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from shellgpt_cli import gateway_windows as gw
...
# SHELLGPT_HOME / SHELLGPT_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\shellgpt\git\{cmd,bin,usr\bin}, ...\shellgpt\node), PortableGit + Node copies
...
def _shellgpt_path_markers(shellgpt_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying ShellGPT-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(shellgpt_home).rstrip("\\/")
    subs = ("shellgpt-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(shellgpt_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip ShellGPT-owned entries from User-scope PATH in the registry (see ``_shellgpt_path_markers``)."""
    markers = tuple(m.lower() for m in _shellgpt_path_markers(shellgpt_home, include_managed_bin=include_managed_bin))
...
def remove_shellgpt_env_vars_windows() -> list[str]:
    """Delete SHELLGPT_HOME and SHELLGPT_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("SHELLGPT_HOME", "SHELLGPT_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(shellgpt_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``shellgpt_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (shellgpt_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``shellgpt`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from shellgpt_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from shellgpt_constants import get_default_shellgpt_root
        bin_dir = get_default_shellgpt_root() / "bin"
    except Exception as e:
...
def _is_default_shellgpt_home(shellgpt_home: Path) -> bool:
    """Return True when ``shellgpt_home`` points at the default (non-profile) root."""
    try:
        from shellgpt_constants import get_default_shellgpt_root
        return shellgpt_home.resolve() == get_default_shellgpt_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``shellgpt -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current SHELLGPT_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m shellgpt_cli.main` (the `shellgpt` wrapper may be half-gone).
    shellgpt_invocation = [sys.executable, "-m", "shellgpt_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                shellgpt_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's SHELLGPT_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``shellgpt uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from shellgpt_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    shellgpt_home = get_shellgpt_home()
    summary = gui_install_summary(shellgpt_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ ShellGPT Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No ShellGPT Chat GUI installation was found.")
        print(f"  Checked: {shellgpt_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The ShellGPT agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(shellgpt_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The ShellGPT agent at {shellgpt_home / 'shellgpt-agent'}")
        print(f"  • Your config, sessions, and secrets under {shellgpt_home}")
        print()
    ...
    print()
    uninstall_gui(shellgpt_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The ShellGPT agent is still installed. Run 'shellgpt' to use the CLI,")
    print("or 'shellgpt uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.shellgpt/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, shellgpt_home=shellgpt_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie SHELLGPT_HOMEs and systemd units behind.
    is_default_profile = _is_default_shellgpt_home(shellgpt_home)
    ...
    print()
    _print_box("│            ☤ ShellGPT Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {shellgpt_home / 'config.yaml'}")
    print(f"  Secrets: {shellgpt_home / '.env'}")
    print(f"  Data:    {shellgpt_home / 'cron/'}, {shellgpt_home / 'sessions/'}, {shellgpt_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, SHELLGPT_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL ShellGPT data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the ShellGPT code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, shellgpt_home=shellgpt_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, shellgpt_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • ShellGPT PATH entries from shell configs / Windows User PATH")
    print("  • ShellGPT wrapper scripts and ShellGPT-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep ShellGPT config/data: {shellgpt_home}")
    else:
        print(f"  • ShellGPT config/data: {shellgpt_home}")
        profiles = _discover_named_profiles() if _is_default_shellgpt_home(shellgpt_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$SHELLGPT_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: shellgpt_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; shellgpt\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_shellgpt_home(shellgpt_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(shellgpt_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No ShellGPT-owned PATH entries in User environment"),
        (windows, "Removing SHELLGPT_HOME / SHELLGPT_GIT_BASH_PATH User env vars...",
         remove_shellgpt_env_vars_windows, "Removed User env var: {}", "No ShellGPT-set User env vars to remove"),
        (True, "Removing shellgpt command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows shellgpt launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows shellgpt launchers found"),
        (True, "Removing ShellGPT-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(shellgpt_home), "Removed {}", "No ShellGPT-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE SHELLGPT_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from shellgpt_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(shellgpt_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(shellgpt_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.shellgpt/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(shellgpt_home)
    else:
        log_info(f"Keeping configuration and data in {shellgpt_home}")
    ...
    print()
    print("Thank you for using ShellGPT Agent! ☤")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://shellgpt-agent.nousresearch.com/install.ps1)",
    False: "  curl -fsSL https://shellgpt-agent.nousresearch.com/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m shellgpt_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``shellgpt_constants`` + ``shellgpt_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m shellgpt_cli.uninstall")
    parser.add_argument(

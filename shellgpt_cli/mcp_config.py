"""MCP Server Management CLI — ``shellgpt mcp`` subcommand."""
...
from shellgpt_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_shellgpt_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.shellgpt/.env`` into ``os.environ`` and
...
def _probe_single_server(
    ...
    # an OAuth browser flow (e.g. `shellgpt mcp login`'s 315s) left that inner bound at the
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return ShellGPTTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see shellgpt_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  shellgpt mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  shellgpt mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  shellgpt mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: shellgpt mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_shellgpt_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `shellgpt mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  shellgpt mcp add <name> --url <endpoint>')
        _info('  shellgpt mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _probe_failure_next_step(name: str, exc: BaseException) -> str:
    ...
    if _is_auth_error(root) or getattr(getattr(root, "response", None), "status_code", None) in (401, 403):
        return f"The server rejected the sign-in. Run: shellgpt mcp login {name}"
    if "missing executable" in _format_connect_error(exc):
        return (f"Install that command, or set mcp_servers.{name}.command in {display_shellgpt_home()}/config.yaml "
                "to its full path.")
    return f"Check the server is running and the URL/command in its config, then run: shellgpt mcp test {name}"
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `shellgpt mcp remove` + `shellgpt mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if selected_flow == "browser":
            ...
            ShellGPTTokenStorage(name).remove(keep_metadata=True)
    except Exception as exc:
    ...
    # web_server_mcp.py and tui_gateway/mcp_oauth_sessions.py. force_interactive_oauth: `shellgpt mcp
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `shellgpt mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: shellgpt mcp reauth <name>   |   shellgpt mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'shellgpt mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "shellgpt mcp                                    Open the catalog picker (default)",
    "shellgpt mcp catalog                            List Nous-approved MCPs",
    "shellgpt mcp install <name>                     Install a catalog MCP",
    "shellgpt mcp serve                              Run as MCP server",
    "shellgpt mcp add <name> --url <endpoint>        Add a custom MCP server",
    "shellgpt mcp add <name> --command <cmd>         Add a stdio server",
    "shellgpt mcp add <name> --preset <preset>       Add from a known preset",
    "shellgpt mcp remove <name>                      Remove a server",
    "shellgpt mcp list                               List configured servers",
    "shellgpt mcp test <name>                        Test connection",
    "shellgpt mcp configure <name>                   Toggle tools",
    "shellgpt mcp login <name>                       Re-authenticate OAuth",
    "shellgpt mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``shellgpt mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from shellgpt_cli import mcp_picker
    ...
    if handler:
        ...
        return handler(args)
    # No subcommand — drop the user into the catalog picker (same UX as `shellgpt plugin`).
    from shellgpt_cli.mcp_picker import run_picker
    run_picker()

...
caller runs in. Under ``shellgpt serve`` (the Desktop backend, where plugin
...
child rides the host pipe to ``shellgpt serve`` and fans out there. A process
with no Desktop client at all (``shellgpt gateway run``, ``shellgpt chat``, cron)
...
def broadcast_plugin_event(plugin_id: str, event: str, payload: Optional[dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        # A plugin-only process (``shellgpt plugins validate`` importing the backend, a trimmed

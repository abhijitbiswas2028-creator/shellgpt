...
resolved through the module object so test patches on ``shellgpt_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from shellgpt_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'shellgpt setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'shellgpt setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'shellgpt tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("shellgpt setup", "          Re-run the full wizard"), ("shellgpt setup model", "    Change model/provider"),
    ("shellgpt setup terminal", " Change terminal backend"), ("shellgpt setup gateway", "  Configure messaging"),
    ("shellgpt setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("shellgpt config", "         View current settings"), ("shellgpt config edit", "    Open config in your editor"),
    ("shellgpt config set <key> <value>", ""))
_READY_ROWS = (
    ("shellgpt", "              Start chatting"), ("shellgpt gateway", "      Start messaging gateway"),
    ("shellgpt doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'shellgpt setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `shellgpt tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'shellgpt setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via shellgpt auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, shellgpt_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — ShellGPT cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    shellgpt model            (pick any provider/model)",
              "    shellgpt setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'shellgpt setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {shellgpt_home}/cron/, sessions/, logs/")):
...
import shellgpt_cli.setup as _setup  # noqa: E402  (bottom: shellgpt_cli.setup imports this module)

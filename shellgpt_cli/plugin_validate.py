"""``shellgpt plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``SHELLGPT_HOME``), so a crashing or
...
from shellgpt_cli.plugin_validate_desktop import check_desktop_surface
from shellgpt_cli.plugins_manifest import _CONFIG_SCHEMA_TYPES
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "SHELLGPT_VALIDATE_JSON:"
...
def _requires_shellgpt_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_shellgpt`` spec.

    Unlike :func:`shellgpt_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from shellgpt_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_shellgpt(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_shellgpt") or "").strip()
    if not spec:
        report.add("requires_shellgpt", True, "not declared")
        return
    if _requires_shellgpt_spec_valid(spec):
        report.add("requires_shellgpt", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_shellgpt",
            False,
            f"requires_shellgpt spec {spec!r} does not parse "
...
# from shellgpt so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "shellgpt_validate_probe_plugin"
...
try:
    spec = importlib.util.spec_from_file_location(
        "shellgpt_validate_probe_plugin",
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="shellgpt-validate-") as scratch:
        env = dict(os.environ)
        env["SHELLGPT_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_shellgpt(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that ShellGPT can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from shellgpt_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from shellgpt_cli.plugin_python_deps import applicable_specs, unsupported_specs

"""API connectivity probes for ``shellgpt doctor`` (split out of ``doctor.py``).
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from shellgpt_cli.providers import normalize_provider as _normalize_provider
...
_OPENROUTER_STATUS = {
    ...
    402: ("(out of credits — payment required)",
          ...
          "Fix: run 'shellgpt config set model.provider <provider>' "
...
def _anthropic_messages_probe(base: str, key: str):
    ...
    headers = {"anthropic-version": "2023-06-01", "User-Agent": _SHELLGPT_USER_AGENT, **auth}
...
def _apikey_request(key: str, base_env, default_url) -> tuple:
    ...
    headers = {"Authorization": f"Bearer {key}", "User-Agent": _SHELLGPT_USER_AGENT}
...
def _probe_github_token() -> ProbeResult:
    ...
    """
    name = "GitHub token"
    from shellgpt_cli.config import get_env_value, load_env
    ...
    try:
        ...
        r = httpx.get(GITHUB_API_PROBE_URL, timeout=10, headers={
            "Authorization": f"Bearer {get_env_value(var)}", "User-Agent": _SHELLGPT_USER_AGENT,

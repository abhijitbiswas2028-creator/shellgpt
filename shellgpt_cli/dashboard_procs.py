"""Dashboard process-hygiene helpers — extracted from ``shellgpt_cli/main.py``.

Helpers defined in ``shellgpt_cli.main_dashboard`` / ``shellgpt_cli.main_install_repair`` are imported at
...
from shellgpt_cli._startup_fast import is_desktop_ssh_backend_argv
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("shellgpt", "shellgpt_cli.main", "shellgpt_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "shellgpt.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``shellgpt update`` (every API call 401s). *exclude_pids* (Desktop's SHELLGPT_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the ShellGPT
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``shellgpt serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``SHELLGPT_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`shellgpt --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from shellgpt_cli.process_identity import ledger_entries
...
def _shellgpt_home_for_pid(pid: int) -> str | None:
    """The ShellGPT home *pid* runs on, tri-state: ``None`` ONLY when its environment is unreadable
    ...
    exec-time env + argv (``shellgpt -p X serve`` rewrites ``SHELLGPT_HOME`` in ``os.environ`` AFTER
    startup, which ``/proc/<pid>/environ`` never reflects): a profile-shaped ``SHELLGPT_HOME``
    without a flag is the home; otherwise the root is ``SHELLGPT_HOME`` (its grandparent when
    ...
    if env is None:
        return None
    from shellgpt_cli.main_dashboard import _dashboard_cmdline_for_pid
    from shellgpt_cli.profiles import get_active_profile, normalize_profile_name, profile_root_for_env_home
    argv = _dashboard_cmdline_for_pid(pid) or []
    env_home = env.get("SHELLGPT_HOME", "").strip()
    profile = _profile_flag_value(argv)
    if profile is None and env_home and (
        Path(env_home).parent.name == "profiles" or env.get("SHELLGPT_UPDATE_POST_SWAP") == "1"
    ):
        return env_home
    if sys.platform == "win32":
        ...
        default_home = base / "shellgpt"
    else:
        default_home = Path(env.get("HOME") or _pid_passwd_home(pid) or Path.home()) / ".shellgpt"
    root = profile_root_for_env_home(env_home, default_home)
    fixed_identity = any(env.get(k) for k in ("SHELLGPT_SUPERVISED_CHILD", "SHELLGPT_S6_SUPERVISED_CHILD",
                                               "SHELLGPT_GATEWAY_EXTERNAL_SUPERVISOR")) or is_desktop_ssh_backend_argv(argv)
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``shellgpt update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _pids_owned_by_shellgpt_home(pids: list[int], home: str) -> list[int]:
    """Return only *pids* whose resolved ShellGPT home (``_shellgpt_home_for_pid``) is ``home``.

    Dashboard argv is discovery-only: it is not an ownership proof because
    several ShellGPT installs and profiles can run the same command on one
    ...
    return [
        pid for pid in pids
        if (pid_home := _shellgpt_home_for_pid(pid))
        and _normalized_home_for_compare(pid_home) == target
    ]


def _profile_key_for_respawn(argv: list[str], shellgpt_home: str | None = None) -> str:
    """Stable owner key: ``SHELLGPT_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if shellgpt_home:
        parts = _resolved_home(shellgpt_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(shellgpt_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, shellgpt_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``SHELLGPT_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``SHELLGPT_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``shellgpt update`` respawn detaches
    ...
    for _pid, argv, shellgpt_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and shellgpt_home and _normalized_home_for_compare(shellgpt_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, shellgpt_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``SHELLGPT_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("SHELLGPT_DESKTOP_CHILD_PID", "").split(","):
        with contextlib.suppress(ValueError):
            out.add(int(part))
    return out


#: Executables that only *carry* a shellgpt command line. A process headed by one of these
#: never serves traffic itself; when its argv matches the dashboard patterns it is a
#: wrapper around the command (``bash -c 'shellgpt dashboard --stop'``), not a backend.
...
def _is_caller_wrapper_shell(pid: int, ancestors: set[int]) -> bool:
    ...
    ``bash -c 'shellgpt dashboard --stop'`` wrapper) matches on its own argv. Ancestor alone
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_shellgpt(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not shellgpt-owned or process identity changed"))
            else:
...
# shellgpt_cli/web_server.py::_lifespan: stop_hosted_room_service(timeout=5.0) + the startup-thread
...
# and keep the deleted state.db-wal inode open — the next shellgpt start refuses with a FATAL
...
def _is_detached_session_leader(pid: int, tty: str) -> bool:
    ...
    ``shellgpt --tui`` child is a session leader too (``pty.fork``) but owns the pts whose master the
...
def _posix_descendants(roots: list[int]) -> dict[int, tuple[int, int | None]]:
    ...
    subtrees. So is the calling process with its subtree and its ancestor chain: ``shellgpt dashboard
    --stop`` / ``shellgpt update`` run from a shell escape inside the hosted Chat TUI are same-session
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``shellgpt dashboard`` / ``shellgpt serve`` processes (update end, ``--stop``).
    ...
    ``SHELLGPT_HOME`` are candidates; unknown ownership fails closed. This is
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``shellgpt update``'s systemd fleet-restart loop, which restarts ``shellgpt-serve*`` units before this
    ...
    """
    from shellgpt_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. shellgpt-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "shellgpt-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if launchd_jobs and (job := _launchd_owner(pid, cmdline)):
                ...
                # Manual process: exact argv + SHELLGPT_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot SHELLGPT_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _shellgpt_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (shellgpt serve) under Restart=on-failure never comes
...
def _detect_concurrent_shellgpt_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``shellgpt.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from shellgpt_cli.main_install_repair import _shellgpt_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _shellgpt_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``shellgpt serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    from shellgpt_cli.update_cmd_windows import _shellgpt_holder_subcommand
    # Canonical token matcher, never argv substrings: ``kanban --preserve-cache`` contains "serve" and
    # ``vim notes about shellgpt serve`` contains both markers — this predicate decides a kill.
    if _shellgpt_holder_subcommand(command) != "serve":
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``shellgpt serve`` it spawns. Such a backend
...
def _shellgpt_home_dir() -> Path:
    """The process's ShellGPT home: remote-backend locks are a process-level asset, so a request scoped
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "shellgptPath", "shellgptHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated SHELLGPT_HOME can't reject a legitimate backend.
...
def _remote_lock_roots(base_dir: Path | None) -> list[Path]:
    ...
    The Desktop writes SSH locks beneath the ROOT home (``~/.shellgpt/desktop-ssh``), but a profile
    backend (``shellgpt --profile X serve``) runs with ``SHELLGPT_HOME=<root>/profiles/X`` — scanning only
    ...
    roots: list[Path] = []
    for home in (_shellgpt_home_dir(), get_default_shellgpt_root()):
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{shellgpt_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local shellgpt serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``shellgpt serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / SHELLGPT_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after SHELLGPT_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time

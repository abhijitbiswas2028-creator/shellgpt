...
Shared helpers are reached via the late-binding seam in :mod:`shellgpt_cli.web_deps`
...
import copy
import functools
from shellgpt_cli.web_read_coalescing import coalesced_read
import inspect
import json
import logging
import re
...
from shellgpt_cli.config import get_process_shellgpt_home
from shellgpt_cli.profiles import ProfileIdentitySettlementPending
from shellgpt_cli.web_server_config import (
    _apply_main_model_assignment, _normalize_main_model_assignment, _validated_main_model_selection,
)
from shellgpt_cli.web_server_gateway import _strip_session_list_rows
from shellgpt_cli.web_routers._common import _CONFIG_MUTATION_LOCK
from shellgpt_cli.web_server_profiles import (
...
from shellgpt_state_health import STORAGE_CORRUPT, note_storage_error, storage_state
...
from shellgpt_cli.web_models import (
    ...
    SessionPrScanBody)
from shellgpt_cli.web_server_profiles import _config_profile_scope, _shellgpt_home_scope
...
_cron_profile_home = late("_cron_profile_home", "shellgpt_cli.web_server_cron")
...
def _profile_setup_command(name: str) -> str:
    ...
    return "shellgpt setup" if name == "default" else f"{name} setup"
...
def _scope_profile_name(path: Path) -> Optional[str]:
    ...
    ``"default"`` for the default root (its basename -- ``.shellgpt`` or a custom root -- is not a
    ...
    if resolved == get_process_shellgpt_home().resolve():
        return None
    if resolved == get_default_shellgpt_root().resolve():
        return "default"
    return path.name


def _write_profile_model(profile_dir: Path, provider: str, model: str, validate_in: Optional[Path] = None) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (SHELLGPT_HOME-scoped)
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from shellgpt_cli.skills_config import get_disabled_skills, save_disabled_skills
    keep_set = {s.strip() for s in keep if s and s.strip()}
    with _shellgpt_home_scope(profile_dir), _CONFIG_MUTATION_LOCK:  # RMW span
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map shellgpt_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
def _corrupt_profile_stores(targets) -> Dict[str, str]:
    ...
    as structurally corrupt (``shellgpt_state_health``). Lets Desktop tell an empty or partial list
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    for name, home in _profile_targets("GET /api/profiles/projects/tree"):
        def _read(db, name=name, home=home):
            with _shellgpt_home_scope(home):
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _set_model() -> bool:
        ...
        try:
            _write_profile_model(path, provider, model, validate_in=get_process_shellgpt_home())
    ...
    def _spawn_install(ident: str):
        return _spawn_shellgpt_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``shellgpt profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``shellgpt profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the SHELLGPT_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``shellgpt profile
    ...
    def _run():
        from shellgpt_cli import profile_describer
...
# ── Export / Import ── wraps shellgpt_cli.profiles.export_profile / import_profile. Paths are

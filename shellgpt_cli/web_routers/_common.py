...
late-binding seam in :mod:`shellgpt_cli.web_deps` (web_server owns helpers/state;
...
log = logging.getLogger("shellgpt_cli.web_server")
...
def destructive_profile(profile: Optional[str], route: str) -> Optional[str]:
    ...
    and `curl` against a plain ``shellgpt serve`` is unchanged.
...
def spawn_profile_action(
    ...
    """Spawn a background ``shellgpt -p <profile> <argv>`` action; a spawn
...
# instead of a 500. The file is never quarantined or renamed from here — that is `shellgpt doctor`'s job.
...
DELETED_WAL_DETAIL = {
    ...
    "message": "another ShellGPT process still holds an old copy of the session database's write-ahead log — "
               "quit every ShellGPT process on this profile, run `shellgpt doctor` (it names the holders), "
               "then start ShellGPT again. Do not run `shellgpt doctor --fix` while they run.",
}
STATE_DB_REPLACED_DETAIL = {
    "error": "state_db_replaced",
    "message": "state.db was replaced while ShellGPT was running — stop ShellGPT, run `shellgpt doctor`, "
               "then start it again. Do not run `shellgpt doctor --fix`, which would repair the wrong file in place.",
}
...
def corrupt_store_as_status(db_path):
    ...
    from shellgpt_state_errors import StateDbReplacedError, classify_persistence_error, is_malformed_db_error
    ...
    try:
        ...
        if last is None or now - last >= _CORRUPT_STORE_WARN_INTERVAL_S:
            ...
            log.warning("state.db at %s is unreadable (%s); dashboard reads return a status payload until it is "
                        "repaired — run `shellgpt doctor`", db_path, exc)
        else:

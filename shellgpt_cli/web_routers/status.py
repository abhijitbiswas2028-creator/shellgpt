...
Extracted from ``shellgpt_cli.web_server``; app state and helpers are late-bound through
:mod:`shellgpt_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from shellgpt_cli.web_deps import LateState, late
from shellgpt_cli.web_server_gateway import _display_system_platform
...
from shellgpt_cli.config import get_config_path, get_env_path
from shellgpt_constants import get_process_shellgpt_home, profile_name_for_home
from shellgpt_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from shellgpt_cli.web_routers._common import config_scoped_to_thread, destructive_profile, scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "shellgpt_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "shellgpt_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "shellgpt_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "shellgpt_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "shellgpt_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "shellgpt_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "shellgpt_cli.web_server_lifecycle")
_spawn_shellgpt_action = late("_spawn_shellgpt_action", "shellgpt_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "shellgpt_cli.config")
get_shellgpt_home = late("get_shellgpt_home", "shellgpt_cli.config")
_profile_cli_args = late("_profile_cli_args", "shellgpt_cli.web_server_profiles")
...
load_config = late("load_config", "shellgpt_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "shellgpt_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another ShellGPT process uses it."""
...
async def get_ssh_ownership(request: Request):
    from shellgpt_cli.web_server import _SSH_OWNER_NONCE
...
async def get_host_identity(request: Request):
    """Prove to an attaching `shellgpt serve`/`dashboard` WHO owns this port.
    ...
    headless ``serve``, so a `shellgpt dashboard` user is never routed to a backend with no UI.
    """
    _require_token(request)
    # ``role`` is the host ROLE this process published (gateway/host_rendezvous.ROLE_SERVE, or
    # ROLE_DESKTOP_SERVE for a Desktop-owned child), not the launch mode: `shellgpt serve` and
    # `shellgpt dashboard` are one host role that differ in SPA.
...
# Profile segment mirrors shellgpt_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level SHELLGPT_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``shellgpt --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_shellgpt_home())
    ...
    if runtime:
        ...
        if not gateway_running:
            ...
            # as ``shellgpt gateway status`` so the sidebar strip and the CLI agree.
...
def _nous_session_validity() -> str:
    ...
    try:
        from shellgpt_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from shellgpt_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_shellgpt_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from shellgpt_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from shellgpt_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of shellgpt_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_shellgpt": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_shellgpt_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (ShellGPT Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "shellgpt_home": str(get_shellgpt_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond shellgpt home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "shellgpt_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_shellgpt_home()))
...
async def set_curator_paused(body: CuratorPause, profile: Optional[str] = None):
    from agent import curator
    # ``_state_file()`` is ``get_shellgpt_home()/skills/.curator_state`` resolved at call
...
def _spawn_action(argv: list, name: str, prefix: str, profile: Optional[str] = None) -> dict:
    """Spawn a background ``shellgpt -p <profile> <argv>`` action; a spawn failure is
    ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_shellgpt_action(_profile_cli_args(profile) + argv, name)
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from shellgpt_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from shellgpt_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    profile: Optional[str] = None):
    from shellgpt_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    with _config_profile_scope(profile):
        log_path = get_shellgpt_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from shellgpt_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}

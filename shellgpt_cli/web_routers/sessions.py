...
from shellgpt_cli.web_server_sessions import _maybe_auto_archive_for_profile, _session_latest_descendant
...
from shellgpt_state_errors import StateDbReplacedError, is_transient_sqlite_error
...
_cron_default_profile = late("_cron_default_profile", "shellgpt_cli.web_server_cron")
...
_session_db_path_for_profile = late("_session_db_path_for_profile", "shellgpt_cli.web_server_sessions")
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `shellgpt sessions prune`)."""
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_shellgpt_home()
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics (mirrors `shellgpt sessions stats`)."""
    def _stats(db):
        out = {
...
def _history_profile_home(profile):
    ...
    # An omitted profile reads this process's DB (including custom SHELLGPT_HOME),
...
async def get_session_timeline(
    ...
    """
    from shellgpt_state_timeline import get_session_timeline as read_timeline
...
async def get_session_messages_around(
    ...
    from shellgpt_state_timeline import get_session_messages_around as read_around

"""Plugin catalog — curated, Nous-approved ShellGPT plugins shipped with the repo.
...
an installed ShellGPT sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://shellgpt-agent.nousresearch.com/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_shellgpt": self.requires_shellgpt,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_shellgpt=str(data.get("requires_shellgpt") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_shellgpt_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``SHELLGPT_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_shellgpt_home(cache.parent)
...
def load_catalog_live() -> List[PluginCatalogEntry]:
    ...
    different pins the NEWER source supplies it — right after ``shellgpt update`` bumps an in-tree pin,
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_shellgpt:
        bits.append(f"Requires ShellGPT {entry.requires_shellgpt}.")
    return " ".join(bits)

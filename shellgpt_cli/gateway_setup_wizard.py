"""Gateway platform setup wizard (shellgpt gateway setup): platform registry/status table, per-platform setup prompts, service offer.
...
import subprocess
import sys
from shellgpt_cli.setup import print_success  # def-time binding (table value)
from shellgpt_cli.setup import print_warning  # def-time binding (table value)
...
_PLATFORMS = [
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. shellgpt) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where ShellGPT delivers cron results and notifications."},
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "5. ShellGPT connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: shellgpt pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and ShellGPT will connect automatically over WebSocket",
        ],
        "vars": [
...
_UNAUTHORIZED_ACCESS_CHOICES = {
    ...
    False: (1,
        ...
        "Use DM pairing (unknown users request access, you approve with 'shellgpt pairing approve')",
        "Politely decline unknown senders (one-time message, then silence)",
...
def _prompt_unauthorized_access(platform_key: str) -> None:
    ...
    if access_idx == 0:
        ...
        _gw().print_info("  Approve with: shellgpt pairing approve <platform> <code>")
    elif access_idx == 2:
        ...
        _gw().print_info("  Skipped — configure later with 'shellgpt gateway setup'")
...
def _telegram_auto_setup(token_var: str) -> tuple[bool, object]:
    ...
    if _gw().prompt("  Choice [1/2]", default="1").strip() != "1":
        return False, None
    try:
        from shellgpt_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    print()
    _gw()._print_info_lines(
        "  1. ShellGPT will open Tencent iLink QR login in this terminal.",
        "  2. Use WeChat to scan and confirm the QR code.",
        "  3. ShellGPT will store the returned account_id/token in ~/.shellgpt/.env.",
    ...
    if not check_weixin_requirements():
        ...
        _gw().print_info("  Install them, then rerun `shellgpt gateway setup`.")
        return
    ...
    try:
        credentials = _gw().asyncio.run(qr_login(str(_gw().get_shellgpt_home())))
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            _gw().print_info(
                "  Unknown DM users can request access and you approve them with `shellgpt pairing approve`."
            )
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        _gw().print_info("  Unknown users can request access; approve with `shellgpt pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        _gw()._print_info_lines(
            ...
            '    signal-cli link -n "ShellGPTAgent"',
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the shellgpt_cli.setup cycle."""
    from shellgpt_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        _gw().print_info(f"  Set these env vars in ~/.shellgpt/.env: {', '.join(required)}")
    else:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            _gw().print_info("Start manually: shellgpt gateway")
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: shellgpt gateway run",
        "  For persistence:   tmux new -s shellgpt 'shellgpt gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: shellgpt gateway run",
        "  Or start it manually in the background (best effort): nohup shellgpt gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: shellgpt gateway run",
    ),
}
...
def _wizard_service_status_block() -> None:
    ...
    if _gw().supports_systemd_services() and _gw().has_legacy_shellgpt_units():
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        _gw().print_info("  You can install later: shellgpt gateway install")
        if _gw().supports_systemd_services():
            _gw().print_info("  Or as a boot-time service: sudo shellgpt gateway install --system")
        _gw().print_info("  Or run in foreground:  shellgpt gateway run")
        return
    try:
        ...
        _gw().print_info("  You can try manually: shellgpt gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_gw()._platform_status(p)) for p in _gw()._all_platforms()):
        ...
        print()
        _gw().print_info("No platforms configured. Run 'shellgpt gateway setup' when ready.")

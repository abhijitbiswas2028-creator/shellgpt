...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or shellgpt -c.",
    ...
    "Ctrl+Z suspends ShellGPT to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "shellgpt -c resumes your most recent CLI session. shellgpt -c \"project name\" resumes by title.",
    "shellgpt -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "shellgpt -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "shellgpt chat -t web,terminal enables only specific toolsets for a focused session.",
    "shellgpt chat -s github-pr-workflow preloads a skill at launch.",
    "shellgpt chat -q \"query\" runs a single non-interactive query and exits.",
    "shellgpt chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "shellgpt chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "shellgpt --yolo bypasses all dangerous command approval prompts for the entire session.",
    "shellgpt chat --source telegram tags the session for filtering in shellgpt sessions list.",
    "shellgpt -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "shellgpt doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "shellgpt dump outputs a compact setup summary — great for bug reports.",
    "shellgpt config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "shellgpt config edit opens config.yaml in your default editor.",
    "shellgpt config check scans for missing or stale configuration options.",
    "shellgpt sessions browse opens an interactive session picker with search.",
    "shellgpt sessions stats shows session counts by platform and database size.",
    "shellgpt sessions prune --older-than 30 cleans up old sessions.",
    "shellgpt skills search react --source skills-sh searches the skills.sh public directory.",
    "shellgpt skills check scans installed hub skills for upstream updates.",
    "shellgpt skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "shellgpt skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "shellgpt mcp add github --command npx adds MCP servers from the command line.",
    "shellgpt mcp serve runs ShellGPT itself as an MCP server for other agents.",
    "shellgpt auth add lets you add multiple API keys for credential pool rotation.",
    "shellgpt completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "shellgpt logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "shellgpt backup creates a zip backup of your entire ShellGPT home directory.",
    "shellgpt profile create coder creates an isolated profile that becomes its own command.",
    "shellgpt profile create work --clone copies your current config and keys to a new profile.",
    "shellgpt update syncs new bundled skills to ALL profiles automatically.",
    "shellgpt gateway install sets up ShellGPT as a system service (systemd/launchd).",
    "shellgpt memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "shellgpt webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: shellgpt tools disables unused tools, shellgpt skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "shellgpt models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as shellgpt -w).",
    ...
    "SHELLGPT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call ShellGPT tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'shellgpt profile create coder' creates the 'coder' command.",
    "shellgpt profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, ShellGPT prints a resume command with session ID and stats.",
    "shellgpt sessions export backup.jsonl exports all sessions for backup or analysis.",
    "shellgpt -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "shellgpt skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .shellgpt/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: shellgpt cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "ShellGPT runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "shellgpt gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to ShellGPT via a local macOS server.",
    ...
    "Context files (.shellgpt.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "ShellGPT loads project context from .shellgpt.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "shellgpt mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "shellgpt mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "shellgpt mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — shellgpt tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.shellgpt/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "shellgpt chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "shellgpt plugins install owner/repo installs plugins directly from GitHub.",
    "External memory providers: Honcho, OpenViking, Mem0 bundled; Hindsight and more via shellgpt plugins install.",
    ...
    "shellgpt acp runs ShellGPT as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SHELLGPT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "shellgpt auth add nous or shellgpt auth add openai-codex sets up OAuth-based providers.",
    ...
    "shellgpt status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.shellgpt/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make ShellGPT your own.",
    ...
    "Before a gateway session reset, ShellGPT auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from shellgpt_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.shellgpt/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope ShellGPT state via SHELLGPT_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "SHELLGPT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SHELLGPT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.shellgpt/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'shellgpt profile create ls' would be rejected.",
    "shellgpt profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "shellgpt claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SHELLGPT_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via shellgpt tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.shellgpt/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.shellgpt/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — ShellGPT auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of ShellGPT config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while ShellGPT is working.',
    ...
    '/reload reloads ~/.shellgpt/.env into the running session — pick up new API keys without restarting.',
    ...
    "SHELLGPT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.shellgpt/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.shellgpt/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'shellgpt curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "shellgpt curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'shellgpt curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'shellgpt auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'SHELLGPT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SHELLGPT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'shellgpt dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'shellgpt dashboard embeds the full ShellGPT TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.shellgpt/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.shellgpt/dashboard-plugins/ — no npm build required.',
    ...
    'SHELLGPT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'SHELLGPT_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'SHELLGPT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SHELLGPT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.shellgpt/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.shellgpt/skills/ so drop-in skills appear without restarting the session.',
    ...
    'shellgpt -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'shellgpt chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'shellgpt chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'shellgpt chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "shellgpt chat --source tool tags programmatic chats so they don't clutter shellgpt sessions list.",
    'shellgpt dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'shellgpt sessions rename <ID> "new title" renames any past session; shellgpt sessions delete <ID> removes one.',
    'shellgpt import restores a full ShellGPT backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'shellgpt fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'shellgpt pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'shellgpt setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'shellgpt status --deep runs the full health sweep across every component; plain shellgpt status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'SHELLGPT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SHELLGPT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.shellgpt/plugins/ — trust-gated by design.',
    "SHELLGPT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SHELLGPT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SHELLGPT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SHELLGPT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SHELLGPT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SHELLGPT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SHELLGPT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'SHELLGPT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SHELLGPT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SHELLGPT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SHELLGPT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes ShellGPT block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from shellgpt sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.shellgpt/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — ShellGPT is

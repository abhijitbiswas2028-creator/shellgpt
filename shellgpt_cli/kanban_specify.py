...
``shellgpt kanban specify [task_id | --all]`` asks the auxiliary LLM for a
tightened title + concrete body for a Triage task, then flips it
``triage -> todo`` via ``kanban_db.specify_triage_task``.

Mirrors ``shellgpt_cli/goals.py``: same aux-client pattern, same "empty config
...
SHELLGPT_KANBAN_SPECIFY_MAX_TOKENS = max(1500, env_int("SHELLGPT_KANBAN_SPECIFY_MAX_TOKENS", 6000))

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the ShellGPT Agent board.
...
def _profile_author(default: str = "specifier") -> str:
    """Same identity contract as ``shellgpt_cli.kanban._profile_author``; ``$USER`` as the last
...
def specify_task(
    ...
    *,
    ...
    raw, reason = _call_aux(
        ...
        max_tokens=SHELLGPT_KANBAN_SPECIFY_MAX_TOKENS, timeout=timeout or 120,
    )
    if raw is None:
        return SpecifyOutcome(task_id, False, reason)
    raw = raw.strip()

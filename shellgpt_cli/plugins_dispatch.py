...
Mixed into :class:`shellgpt_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from shellgpt_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- shellgpt-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- shellgpt-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- shellgpt-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``shellgpt:<event>``.
SHELLGPT_EVENT_NAMESPACE = "shellgpt"
...
class PluginDispatchMixin:
    ...
    def _invoke_hook_callback(cls, callback: Callable, payload: Dict[str, Any]) -> Any:
        ...
        """
        from shellgpt_cli.plugins import resolve_plugin_command_result
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from shellgpt_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"shellgpt-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="shellgpt-plugin-events",
            daemon=True,
        )

"""ACP auth helpers — detect and advertise ShellGPT authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "shellgpt-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active ShellGPT runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for ShellGPT.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no ShellGPT credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate ShellGPT using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure ShellGPT provider", type="terminal", args=["--setup"],
        description=("Open ShellGPT' interactive model/provider setup in a terminal. "
                     "Use this when ShellGPT has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if ShellGPT can resolve any runtime provider credentials."""

...
an unlinked SQLite DB/WAL/SHM generation.  ``shellgpt_state`` supplies only the
...
from shellgpt_state_errors import is_sqlite_lock_error
...
_SHELLGPT_EXECUTABLES = frozenset({"shellgpt", "shellgpt-agent", "shellgpt-acp"})
_SHELLGPT_PYTHON_MODULES = frozenset({"acp_adapter", "shellgpt_cli.main"})
_SHELLGPT_PYTHON_SCRIPTS = frozenset({"shellgpt_cli/main.py", "run_agent.py"})
...
def describe_holder_pid(pid: int) -> str:
    """``PID 123 (shellgpt gateway run)`` for operator-facing holder lists; /proc argv first, psutil elsewhere."""
...
def _looks_like_shellgpt(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported ShellGPT execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _SHELLGPT_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _SHELLGPT_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _SHELLGPT_PYTHON_SCRIPTS
    )
...
def _argv_env_home(argv: Sequence[str]) -> Optional[str]:
    """``SHELLGPT_HOME=<path>`` env-style assignment on the argv (``env SHELLGPT_HOME=… shellgpt …``)."""
    for token in reversed(list(argv)):
        if isinstance(token, str) and token.startswith("SHELLGPT_HOME="):
            return token[len("SHELLGPT_HOME="):]
    return None
...
def _store_install_layout(this_home: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    string test: an arbitrary ``<X>/profiles/<n>/`` tree is not a ShellGPT install, and promoting
    ``<X>`` to "ours" swallows an unrelated instance living under it — the literal two-instance
    shape of #92401. The root store (``~/.shellgpt/state.db``) is its own root with no profile name,
    ...
    try:
        ...
        if os.path.basename(this_home) == ".shellgpt":
...
def _argv_home_selection(
    ...
    serves; its ``--shellgpt-home``/``SHELLGPT_HOME=``/``--profile``/``-p`` selection does. Same
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the SHELLGPT_HOME root, so an absolute-path token
    containing a ``/.shellgpt`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own ShellGPT home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.shellgpt deferred
    ...
    2. The process's own ``--shellgpt-home``/``SHELLGPT_HOME=``/``--profile``/``-p``
    ...
    3. Path tokens. A token under ``<install root>/profiles/<other>`` is another
       ...
       part of an install layout (a custom ``SHELLGPT_HOME`` is served BY the binary
       under ``~/.shellgpt`` — dismissing on it admits maintenance under a live writer).
    """
    ...
    for index, normalized in path_tokens:
        ...
        if index == 0 and not argv0_locates_home:
            continue
        if "/.shellgpt" in normalized or normalized.endswith("/.shellgpt"):
            other_home_seen = True
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # SHELLGPT_HOME would otherwise make every holder invisible and let maintenance proceed.
...
def in_process_state_db_holders(
    ...
    """
    from shellgpt_state_registry import other_generations_for_path
...
def held_store_refusal(db_path: Path, *, command: str, force_hint: Optional[str] = "--force") -> Optional[str]:
    ...
    Running ``shellgpt sessions optimize-storage`` underneath a fleet of live gateways put every agent into
    ...
    lines = [f"Refusing `shellgpt sessions {command}`: another process is using {db_path}."]
    ...
    lines += [
        ...
        f"Stop them first (`shellgpt {profile_arg}gateway stop`, quit the Desktop app, pause cron), then re-run.",
    ]
    if force_hint:

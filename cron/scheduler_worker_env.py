...
``cron.scheduler``, not ``shellgpt_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips ShellGPT-owned PYTHONPATH entries because user
children must not see our tree. This child IS ShellGPT, so the pin is applied *after* the
...
def pin_shellgpt_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:

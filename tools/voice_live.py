"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to ShellGPT.
...
delegates every real request to ShellGPT as its *client-delegation* backend. ShellGPT stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — ShellGPT' answer is what the voice speaks.
...
# backend (ShellGPT) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are ShellGPT, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- ShellGPT agent: a full AI agent with tools — it can run commands, read and edit files, "

...
def _get_default_output_dir() -> str:
    ...
    return str(get_shellgpt_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    SHELLGPT_HOME and later scope requests to a different profile via
    ``shellgpt_constants.set_shellgpt_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped SHELLGPT_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("shellgpt_cli.config not available, using default TTS config")
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `shellgpt setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run shellgpt setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'shellgpt setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'shellgpt tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("SHELLGPT_SESSION_PLATFORM", "").lower()
...
def _tts_schema_overrides() -> dict:
    ...
    params["properties"]["output_path"]["description"] = _output_path_description(display_shellgpt_home())
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": _output_path_description("the profile SHELLGPT_HOME")
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'shellgpt_xai_user_agent': ('tools.xai_http', 'shellgpt_xai_user_agent'),

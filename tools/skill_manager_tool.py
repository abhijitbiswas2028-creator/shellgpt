...
broad, declarative). New skills land in ~/.shellgpt/skills/ (or ``skills.create_dir``);
...
# All skills live in ~/.shellgpt/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    An explicitly patched module-level ``SKILLS_DIR`` (tests) wins over the live SHELLGPT_HOME.

    Long-lived multi-profile runtimes (Dashboard/TUI/Desktop backend, cron, kanban workers) import this
    module once under the launch SHELLGPT_HOME and later bind a different profile per session (#40677).
    """
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    if len(others) == 1:
        ...
        base += (
            ...
            f"it, switch profiles (`shellgpt -p {other_profile}`) or edit the file directly "
            f"(file tools / terminal).")
    elif others:
        names = ", ".join(f"'{p}'" for p, _ in others)
        base += (
            f" Skills by that name exist in other profiles: {names}. Switch profiles (`shellgpt -p "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path, before: Optional[str] = None) -> None:
    ...
    result["lint_hint"] = (
        ...
        "— fix them with skill_manage(action='patch') to match ShellGPT skill standards.")
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    from shellgpt_constants import mkdir_under_shellgpt_home
    mkdir_under_shellgpt_home(skill_dir.parent)
    try:
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if unsafe := _validate_delete_target(skill_dir):  # defense-in-depth before rmtree
        return _err(unsafe)
    # Curator consolidations must be RECOVERABLE (`shellgpt curator restore`): archive instead
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
    ...
    # record as STATE_ARCHIVED (`shellgpt curator status`/`restore`); only a hard delete forgets.
    with suppress(Exception):
        ...
        # ~/.shellgpt/skills/.archive/ is documented as the maximum destructive action the curator may take,
        # and `shellgpt curator restore` promises the skill can be brought back. Route through the recoverable

...
# prefix, the exact-name SHELLGPT_ allowlist, or (Windows) an OS-essential name.
# The broad "SHELLGPT_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (SHELLGPT_BASE_URL, SHELLGPT_KANBAN_DB, *_WEBHOOK).
# SHELLGPT_RPC_SOCKET / SHELLGPT_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. SHELLGPT_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports ShellGPT code loses the Kanban mutation guard
# while still inheriting SHELLGPT_HOME.
_SHELLGPT_CHILD_ALLOWED = frozenset({
    "SHELLGPT_HOME", "SHELLGPT_PROFILE", "SHELLGPT_CONFIG", "SHELLGPT_ENV", "SHELLGPT_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational SHELLGPT_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret SHELLGPT_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _SHELLGPT_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("SHELLGPT_"):
            _dropped_shellgpt.append(k)
    ...
    if _dropped_shellgpt:
        logger.debug(
            "execute_code: dropped %d non-allowlisted SHELLGPT_* var(s) from the "
            ...
            len(_dropped_shellgpt), ", ".join(sorted(_dropped_shellgpt)),
        )
    ...
    # non-allowlisted SHELLGPT_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "SHELLGPT_KANBAN_DB", "SHELLGPT_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from shellgpt_constants import apply_scratch_tmp_env, apply_subprocess_home_env, get_shellgpt_home_override
    child_env = _scrub_child_env(os.environ)
    child_env["SHELLGPT_RPC_SOCKET"] = rpc_endpoint
    child_env["SHELLGPT_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; SHELLGPT_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — shellgpt_time resolves the routed profile's).
    from shellgpt_time import get_timezone_name
    ...
    child_env.pop("SHELLGPT_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # Multiplexed gateway/Desktop (#110303): the server process env carries the machine-default
    # SHELLGPT_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    _home_override = get_shellgpt_home_override()
    if _home_override:
        child_env["SHELLGPT_HOME"] = _home_override
        apply_scratch_tmp_env(child_env)  # TMPDIR follows the routed home, like HOME does
    # PYTHONPATH: the staging dir (shellgpt_tools.py) must always be importable even when project
    # mode changes CWD. ShellGPT's root is added ONLY when the child runs in ShellGPT's Python env —
    # exposing ShellGPT's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited ShellGPT-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip ShellGPT-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-ShellGPT-
    ...
    _strip_shellgpt_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_shellgpt_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import shellgpt_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the ShellGPT "
                    "environment; shellgpt root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the ShellGPT environment.
...
def _uses_shellgpt_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to ShellGPT's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the shellgpt root."""

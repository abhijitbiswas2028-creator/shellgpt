...
One append-only log per child under ``<shellgpt_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.shellgpt)."""
    from shellgpt_constants import get_shellgpt_dir
    return get_shellgpt_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        with _best_effort(f"init ({delegation_id} task {task_index})"):
            ...
            path.write_text(
                "=== ShellGPT subagent live transcript ===\n"

...
def return_to_args(*, op: Optional[str] = None) -> dict[str, Any]:
    ...
    field and keeps the vendor's own done page. The dev build registers ``shellgpt-dev://`` instead, and
    announces itself to its backend with ``SHELLGPT_DESKTOP_DEV_SERVER``."""
    if session_platform() != "desktop":
        return {}
    target = "shellgpt-desktop-dev" if os.environ.get("SHELLGPT_DESKTOP_DEV_SERVER") else "shellgpt-desktop"

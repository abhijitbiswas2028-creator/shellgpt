...
def probe_with_rollback(
        server_name: str, cfg: dict, shellgpt_home: str, flow, reconnect_live: bool, *,
    ...
    from shellgpt_cli.mcp_config import _oauth_tokens_present, _probe_single_server
    from tools.mcp_dashboard_oauth import exception_message
    from tools.mcp_oauth import ShellGPTTokenStorage, login_connect_timeout
    from tools.mcp_oauth_manager import get_manager
    manager = get_manager()
    storage = ShellGPTTokenStorage(server_name)
    ...
    def undo() -> None:
        ...
        if flow is not None and _ACTIVE.get((shellgpt_home, server_name)) not in (None, flow):
            return
        storage.restore(backup)
        manager.restore_entry(server_name, previous_entry, shellgpt_home=shellgpt_home)

    try:
        previous_entry = manager.remove(server_name, shellgpt_home=shellgpt_home)
...
# (shellgpt home, server) -> the newest card attempt. A retry or a new operation replaces an attempt
...
def run_worker(
        shellgpt_home: str, server_name: str, cfg: dict, reconnect_live: bool, *,
    ...
    try:
        ...
        try:
            home_token = set_shellgpt_home_override(shellgpt_home)
            secret_token = set_secret_scope(
                {**build_profile_secret_scope(Path(shellgpt_home)), **(env or {})}, profile_home=shellgpt_home)
            if not (reuse_saved and flow is not None
                ...
                with force_interactive_oauth(), dashboard_oauth_flow(flow):
                    probe_with_rollback(
                        server_name, cfg, shellgpt_home, flow, reconnect_live, on_commit=on_commit)
        finally:
        ...
        if flow is not None:
            ...
            with _COMMIT_GUARD:
                if _ACTIVE.get((shellgpt_home, server_name)) is flow:
                    _ACTIVE.pop((shellgpt_home, server_name), None)
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802
            ...
            if parsed.path.rstrip("/") not in ("/callback", ""):
                ...
                self.end_headers()
                return
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to ShellGPT.</p>"
            status = 200
            try:
...
def start(
    ...
    cfg["auth"] = "oauth"
    shellgpt_home = str(get_shellgpt_home().expanduser().resolve(strict=False))
    flow = DashboardOAuthFlow(
        flow_id=secrets.token_urlsafe(24), server_name=server_name, profile=None,
        shellgpt_home=shellgpt_home, redirect_uri="", reconnect_live=False)
    with _COMMIT_GUARD:
        older = _ACTIVE.get((shellgpt_home, server_name))
        _ACTIVE[(shellgpt_home, server_name)] = flow
    ...
    threading.Thread(
        target=run_worker, args=(shellgpt_home, server_name, cfg, False),

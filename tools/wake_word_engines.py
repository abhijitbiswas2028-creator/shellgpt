...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_shellgpt") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under SHELLGPT_HOME. Keywords are tokenized at RUNTIME.
...
def _sherpa_model_root() -> Path:
    ...
    return get_shellgpt_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        phrase = str(ww._get(cfg, "phrase") or "hey shellgpt").strip()
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="shellgpt-kws-", delete=False,
                                         encoding="utf-8")

"""Secret-scrub policy for ShellGPT child processes: pure data + predicates for which env
names are ShellGPT-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``shellgpt_subprocess_env``) live in ``tools.environments.local``."""
...
_SHELLGPT_PROVIDER_ENV_FORCE_PREFIX = "_SHELLGPT_FORCE_"

# ShellGPT-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "SHELLGPT_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not ShellGPT: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a ShellGPT-managed
    # inference credential — Claude subscription auth is not a working ShellGPT provider path. It arrives via
...
_SHELLGPT_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _is_provider_env_blocklisted(name: str) -> bool:
    ...
    ``OPENAI_API_KEY``; consistent with ``_is_shellgpt_internal_secret``, which
    already folds (``key.upper()``)."""
    return (name in _SHELLGPT_PROVIDER_ENV_BLOCKLIST
            or name.upper() in _SHELLGPT_PROVIDER_ENV_BLOCKLIST)
...
# ``_sanitize_subprocess_env``); execute_code, shellgpt_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``SHELLGPT_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`shellgpt_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a ShellGPT credential; these ARE ShellGPT-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("SHELLGPT_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the ShellGPT venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the ShellGPT venv
# (version-mismatch crashes). PYTHONPATH is handled separately (ShellGPT-owned entries only).
...
# dependencies into the ShellGPT venv path instead of the project's own ``.venv`` — silently clobbering the
# ShellGPT environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the ShellGPT venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). ShellGPT itself
...
# _strip_shellgpt_owned_pythonpath() which removes only ShellGPT-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_shellgpt_internal_secret(key: str) -> bool:
    """True for ShellGPT-internal secrets injected under *dynamic* names the static
...
# is script data, not a ShellGPT gate, and deleting it by name shape broke routed ``no_agent``
# cron scripts (#119539). ``SHELLGPT_*`` never counts (``SHELLGPT_MEDIA_ALLOW_DIRS``,
# ``SHELLGPT_ALLOW_PRIVATE_URLS`` are process settings, not adapter gates).
...
def is_profile_gate_env(name: str, _prefixes: Optional[frozenset] = None) -> bool:
    ...
    upper = name.upper()
    if upper.startswith("SHELLGPT_") or upper.startswith("_"):
        return False
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_shellgpt_internal_secret, but _ID has no secret suffix, so it must be
    # enumerated here to stay stripped on the inherit_credentials=True path.
    "GATEWAY_RELAY_ID", "GATEWAY_RELAY_SECRET", "GATEWAY_RELAY_DELIVERY_KEY",
    "HASS_TOKEN", "EMAIL_PASSWORD", "SHELLGPT_DASHBOARD_SESSION_TOKEN",

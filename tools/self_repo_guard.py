...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(shellgpt_home).expanduser() if shellgpt_home else Path.home() / ".shellgpt") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite ShellGPT's live source checkout "
        ...
        "is pushed. To change this checkout, stop ShellGPT, run the command externally, then restart "
        "ShellGPT.")

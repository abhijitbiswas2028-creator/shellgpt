...
_SHELLGPT_CONFIG_FILES = r'\.shellgpt/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.shellgpt/\.env|\~/\.shellgpt/\.env',
     "shellgpt_env_access", "critical", "exfiltration", "directly references ShellGPT secrets file"),
    ...
    # invocation: any client plugin that relays ShellGPT' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for ShellGPT/other config (setup docs routinely say
    ...
    (_prose_modify_re(_SHELLGPT_CONFIG_FILES),
     "shellgpt_config_mod", "high", "persistence", "modification language aimed at ShellGPT configuration files (verify intent)"),
    (_shell_write_re(_SHELLGPT_CONFIG_FILES),
     "shellgpt_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting ShellGPT configuration files"),
    (r'\.shellgpt/config\.yaml|\.shellgpt/SOUL\.md',
     "shellgpt_config_ref", "low", "persistence", "references ShellGPT configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is ShellGPT-native; `.clawhubignore` is honored for skills published through ClawHub.

...
``metadata.shellgpt.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.shellgpt.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.shellgpt.blueprint`` is a mapping with a
    ...
    blueprint = shellgpt.get("blueprint") if isinstance(shellgpt, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.shellgpt.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.shellgpt.blueprint`` block) ready for ``shellgpt skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"shellgpt": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }

...
The LLM writes a Python script that calls ShellGPT tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform ShellGPT does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_shellgpt_dir("cache/exec", "exec_spill")
...
# ---- shellgpt_tools.py code generator ----
...
def _missing_shellgpt_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
    if missing in {"json_parse", "shell_quote", "retry"}:
        return (f"Import helpers with `from shellgpt_tools import {missing}`. "
                "If that import failed, the generated module may be stale or another "
                "shellgpt_tools may be first on sys.path. Check shellgpt_tools.__file__ "
...
# these as the top execute_code failure classes: shellgpt_tools import misuse, missing helper
# imports, treating tool results as strings, importing third-party packages absent from the sandbox.
_FAILURE_HINT_RULES = (
    (r"cannot import name '(\w+)' from 'shellgpt_tools'", _missing_shellgpt_tools_import_hint),
    (r"NameError: name '(json_parse|shell_quote|retry)' is not defined",
     lambda m, _: f"Import {m.group(1)} before calling it: "
                  f"from shellgpt_tools import {m.group(1)}"),
...
def generate_shellgpt_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """Source of the shellgpt_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
"""Auto-generated ShellGPT tools RPC stubs."""
...
def _connect():
    ...
    SHELLGPT_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SHELLGPT_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SHELLGPT_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (SHELLGPT_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("SHELLGPT_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated ShellGPT tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SHELLGPT_RPC_DIR") or os.path.join(tempfile.gettempdir(), "shellgpt_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage shellgpt_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/shellgpt_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/shellgpt_tools.py",
                             generate_shellgpt_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"SHELLGPT_RPC_DIR={quoted_rpc_dir} SHELLGPT_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (shellgpt-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of ShellGPT tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.shellgpt.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.shellgpt/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "ShellGPT's own python (the common case — stdlib plus ShellGPT's "
    ...
    description = (
        "Run Python that calls ShellGPT tools programmatically. Use when you "
        ...
        f"Available via `from shellgpt_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        f"{cwd_note}\n\n"
        "Helpers require imports: `from shellgpt_tools import json_parse, shell_quote, retry`. "
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from shellgpt_tools import {import_str}` "

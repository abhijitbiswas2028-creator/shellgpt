...
try:
    ...
    pass  # partial `shellgpt update` — only skips the Windows UTF-8 stdio setup

import sys

# `shellgpt-agent` runs this module without shellgpt_cli.main, which repairs a `shellgpt update` killed
# while git wrote the new tree; do it here, before importing anything else from the checkout.
if "shellgpt_cli.main" not in sys.modules:
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``shellgpt -c`` / ``--resume``), or None.
...
# Sources that label the human conversation an interactive UI transport hosts. A finite ``shellgpt chat -q`` /
# one-shot child spawned from such a session inherits SHELLGPT_SESSION_SOURCE (the terminal tool bridges the
# session env into child processes) but is NOT that conversation: labelling it ``tui``/``desktop`` lists it
# in the TUI/WebUI pickers as a resumable chat and lets ``shellgpt -c`` in the TUI continue it (#112550).
# Automation sources (kanban, tool, cron, a2a, ...) are inherited on purpose.
_UI_TRANSPORT_SOURCES = frozenset({"tui", "desktop"})

# Finite non-interactive CLI runs (``shellgpt chat -q``/``--oneshot``, ``shellgpt -z``) get their own source so human
# pickers hide them without title/cwd heuristics; ``shellgpt -c`` still treats them as CLI history.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    ...
    source = str(get_session_env("SHELLGPT_SESSION_SOURCE", "") or "").strip()
    single_query = get_session_env("SHELLGPT_SINGLE_QUERY_SESSION", "") == "1"
    explicit = get_session_env("SHELLGPT_SESSION_SOURCE_EXPLICIT", "") == "1"
...
_shellgpt_home = get_shellgpt_home()  # read by agent_init via _ra()._shellgpt_home
_loaded_env_paths = load_shellgpt_dotenv(shellgpt_home=_shellgpt_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[shellgpt-agent: tool call arguments were corrupted in this session and "
    ...
    def _session_row_model_config(self) -> Any:
        ...
        /yolo toggle for ``shellgpt --resume``.
        """
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from shellgpt_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``SHELLGPT_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("SHELLGPT_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``SHELLGPT_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("SHELLGPT_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("SHELLGPT_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See shellgpt-agent#21444 for symptom history."
        )

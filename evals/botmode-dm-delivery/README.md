...
Real Electron, production Python backend and tool execution, disposable HOME/SHELLGPT_HOME,
...
(cd apps/desktop && DISPLAY=:0 XAUTHORITY=/run/user/1000/xauth_cnpsqU \
  ...
  SHELLGPT_DESKTOP_CDP_PORT=off npx playwright test e2e/probe-dm-delivery.spec.ts --reporter=list)
...
fixture's generated shellgpt shim pins every child to this checkout, not an installed launcher.
...
custom ShellGPT root, and holds the real quiet CLI child at loopback inference.
...
creates a second `.shellgpt` root under HOME.
...
actual Desktop label is ShellGPT. No production failure is claimed for that attempt.

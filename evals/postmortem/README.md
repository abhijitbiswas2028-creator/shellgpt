...
- **`forensics/`** reads a *copy* of a ShellGPT `state.db` (plus rotated `agent.log*` and git) and
...
- A ShellGPT checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.shellgpt/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while ShellGPT is idle) and, optionally, the rotated `~/.shellgpt/logs/agent.log*`.
- For `--live` probes: real credentials in `SHELLGPT_HOME` (they spend cents per run).

## Forensics: recompute the observed numbers for YOUR run

```bash
cd <shellgpt-checkout>
P=.venv/bin/python
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.shellgpt/logs/agent.log*"
...
ShellGPT recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp SHELLGPT_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel ShellGPT subagents (2026-09-04), rewritten here to take `--db`/`--root`

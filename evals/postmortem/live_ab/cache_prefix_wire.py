...
message prefix between call N and N+1. If ShellGPT strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--shellgpt-home DIR]   (default SHELLGPT_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("SHELLGPT_HOME", os.path.expanduser("~/.shellgpt"))
if "--shellgpt-home" in sys.argv:
    os.environ["SHELLGPT_HOME"] = sys.argv[sys.argv.index("--shellgpt-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.shellgpt/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")

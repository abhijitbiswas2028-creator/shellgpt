...
# Temp SHELLGPT_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
home = tempfile.mkdtemp(prefix="dl_home_"); os.environ["SHELLGPT_HOME"] = home
real_home = os.environ.get("SHELLGPT_HOME_SOURCE", os.path.expanduser("~/.shellgpt"))  # credentials are copied from here into a temp home

...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='shellgpt-acp-empty-'))
    shellgpt = home / '.shellgpt'
    shellgpt.mkdir()
    (shellgpt / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), SHELLGPT_HOME=str(shellgpt), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               SHELLGPT_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(shellgpt / 'state.db') as db:

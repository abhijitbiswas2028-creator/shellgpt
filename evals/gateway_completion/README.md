...
Run with a fresh HOME and SHELLGPT_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" SHELLGPT_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `SHELLGPT_EVAL_REPO` selects another checkout for A/B.

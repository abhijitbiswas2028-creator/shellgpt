...
Run with the ShellGPT venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by ShellGPT children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "shellgpt_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="shellgpt-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".shellgpt"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "SHELLGPT_HOME": str(state),

...
sandbox = tempfile.TemporaryDirectory(prefix="shellgpt-104260-")
os.environ.clear()
os.environ.update(
    HOME=sandbox.name,
    SHELLGPT_HOME=sandbox.name + "/shellgpt",
    PATH="/usr/bin:/bin",
    PYTHONDONTWRITEBYTECODE="1",
    SHELLGPT_DISABLE_MODEL_METADATA_FETCH="1",
)
Path(os.environ["SHELLGPT_HOME"]).mkdir()
...
# JSON is valid YAML; only the temporary ShellGPT home is written.

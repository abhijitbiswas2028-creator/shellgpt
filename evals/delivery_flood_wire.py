...
async def setup(url):
    ...
    runner.session_store = SessionStore(Path(os.environ['SHELLGPT_HOME']) / 'sessions', GatewayConfig())
...
def rows():
    with sqlite3.connect(Path(os.environ['SHELLGPT_HOME']) / 'state.db') as conn:

...
batch (tracker: [#77056](https://github.com/NousResearch/shellgpt-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  ShellGPT home, same model, same tasks, same reps.
...
1. Create a dedicated ShellGPT home with credentials for the models under test:
   ...
   integration at it; no ShellGPT observability plugin needs to be enabled.

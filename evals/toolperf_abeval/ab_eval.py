...
measured in the production session DB) through `shellgpt chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    SHELLGPT_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured ShellGPT home with credentials for the
...
(tracker: NousResearch/shellgpt-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "SHELLGPT_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "shellgpt_cli.main", "chat", "--query", q,

...
Each probe runs in a FRESH subprocess with an isolated SHELLGPT_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "SHELLGPT_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "SHELLGPT_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("SHELLGPT_SESSION", "SHELLGPT_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "shellgpt_cli.main", "gateway.run", "tools.registry", "shellgpt_state", "tui_gateway.server", "shellgpt_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "shellgpt-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `shellgpt --version`, `shellgpt --help`, `shellgpt doctor --help`, `shellgpt config get model` (no network)
CLI = {"version": ["shellgpt_cli/main.py", "--version"], "help": ["shellgpt_cli/main.py", "--help"], "config_get": ["shellgpt_cli/main.py", "config", "get", "model"], "tools_list": ["shellgpt_cli/main.py", "tools", "--help"], "skills_help": ["shellgpt_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_shellgpt_default_ms", lambda: resolve_toolset("shellgpt-default"), 200)
...
try:
    import shellgpt_state, tempfile, uuid
    from pathlib import Path
    db=shellgpt_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(shellgpt_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())

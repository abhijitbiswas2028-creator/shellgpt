...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "shellgpt", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="ShellGPT is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("SHELLGPT_HOME", os.path.join(os.path.dirname(a.out), "shellgpt-home-probe"))
    os.makedirs(os.environ["SHELLGPT_HOME"], exist_ok=True)

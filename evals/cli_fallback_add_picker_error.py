"""Drive the real ``shellgpt fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from shellgpt_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="shellgpt_test_fallback_") as home:
        hh = Path(home) / ".shellgpt"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "SHELLGPT_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "shellgpt_cli.main", "fallback", "add"],

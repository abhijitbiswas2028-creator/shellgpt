...
os.environ.update(HOME=str(home), SHELLGPT_HOME=str(home / ".shellgpt"), SESSION_IDLE_MINUTES="1", SESSION_RESET_HOUR="0")
...
Path(os.environ["SHELLGPT_HOME"]).mkdir(exist_ok=True)
Path(os.environ["SHELLGPT_HOME"], "config.yaml").write_text(yaml.safe_dump({"session_reset": {"mode": "both", "idle_minutes": 1}, "gateway": {"session_reset": {"mode": "daily", "at_hour": 0}}}), encoding="utf-8")
...
spec = importlib.util.spec_from_file_location("migration_probe", repo / "optional-skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py")

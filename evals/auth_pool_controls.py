        ...
        for name, provider, command, status in cases:
            ...
            with tempfile.TemporaryDirectory(prefix="auth-pool-pty-") as temp:
                ...
                env = {k: v for k, v in os.environ.items() if not any(t in k for t in
                       ("TOKEN", "API_KEY", "SECRET", "PASSWORD", "SHELLGPT", "PYTEST"))}
                env.update(HOME=temp, SHELLGPT_HOME=temp, SHELLGPT_SHARED_AUTH_DIR=str(home / "shared"), PYTHONPATH=str(Path(args.repo).absolute()), TERM="xterm")

...
os.environ.clear()
os.environ.update(
    HOME=home, SHELLGPT_HOME=home + "/.shellgpt", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["SHELLGPT_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["SHELLGPT_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["SHELLGPT_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from shellgpt_cli.config import get_compatible_custom_providers, load_config
from shellgpt_cli.config_providers import get_custom_provider_context_length
    ...
    json.dumps(
        {
            ...
            "shellgpt_home": os.environ["SHELLGPT_HOME"],
            "captures": captures,
            "rows": rows,

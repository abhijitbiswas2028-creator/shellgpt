...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, SHELLGPT_HOME=home, PATH="/usr/bin:/bin", SHELLGPT_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["shellgpt-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["shellgpt-cli"], quiet_mode=True)

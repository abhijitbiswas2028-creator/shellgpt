# fast-jev-compaction vs ShellGPT compaction — 3-transcript scorecard (2026-09-19)
...
compaction multiplies cache breaks; it is the wrong trade for ShellGPT.
...
4. **The state ceiling does not fit ShellGPT scale.** Jev's 32K window forces the whole
   ...
   all. The plugin is designed for Claude Code's ~200K compaction point; a 1M-window ShellGPT
...
- Transcripts reconstructed with `scripts/reconstruct_lineage.py` from a state.db copy;
  ...
  identical exam.
- The `jev` arm counts rows (ShellGPT has one `role: tool` row per result), so

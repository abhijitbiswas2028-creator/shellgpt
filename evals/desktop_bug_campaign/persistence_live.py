...
def main():
    ...
    env = {k: v for k, v in os.environ.items() if not (k.startswith('SHELLGPT_') or 'API_KEY' in k or 'TOKEN' in k or k in ('PYTHONPATH', 'PYTEST_PLUGINS'))}
    env.update(SHELLGPT_HOME=str(home), HOME=str(home / 'os-home'), SHELLGPT_DASHBOARD_SESSION_TOKEN='persistence-fixture-token', SHELLGPT_IGNORE_RULES='1', PYTHONPATH=str(args.repo), OPENAI_API_KEY='local-fixture')
    log = (args.out / 'serve.log').open('w')
    cmd = [sys.executable, '-m', 'shellgpt_cli.main', 'serve', '--isolated', '--port', str(args.port)]
    if observe:

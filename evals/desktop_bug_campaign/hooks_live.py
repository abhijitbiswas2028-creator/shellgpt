...
env = {k: v for k, v in os.environ.items() if not (k.startswith('SHELLGPT_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), SHELLGPT_HOME=str(home), SHELLGPT_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', SHELLGPT_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'shellgpt_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']

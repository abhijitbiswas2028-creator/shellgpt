...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "SHELLGPT_HOME": str(root / "home/.shellgpt"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "SHELLGPT_NONINTERACTIVE": "1"}
    Path(env["SHELLGPT_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from shellgpt_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'shellgpt')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,shellgpt_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,shellgpt_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=ShellGPT\n", b"Name=ShellGPT custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["SHELLGPT_HOME"]) / "config.yaml"

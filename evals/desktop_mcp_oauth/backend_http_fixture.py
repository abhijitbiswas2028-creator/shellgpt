...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["SHELLGPT_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(ShellGPTTokenStorage("positive", shellgpt_home=owner).get_tokens())
        ...
        override = set_shellgpt_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from shellgpt_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="shellgpt-oauth-http-") as temp:
            home = Path(temp)
            (home / ".shellgpt").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "SHELLGPT_HOME": str(home / ".shellgpt"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}

"""Run the read-tool eval through the REAL ShellGPT AIAgent.

For each task: fresh temp SHELLGPT_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    shellgpt_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".shellgpt"
    ...
    try:
        ...
        shutil.rmtree(shellgpt_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.shellgpt/.env; set +a  — then relaunch."
        )

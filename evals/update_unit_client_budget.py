...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), SHELLGPT_HOME=str(home / "shellgpt"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("shellgpt-gateway*", "shellgpt-serve*")] + [unit + ".service"]
...
unit = f"shellgpt-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):

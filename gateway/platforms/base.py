...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (platform == "slack" and isinstance(raw_message, dict)
            and raw_message.get("_shellgpt_no_thread_response")):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    ``SHELLGPT_SESSION_PLATFORM`` contextvar before the post-handler auto-TTS block runs.
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "shellgpt_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def resolve_proxy_url(
    ...
    *platform_env_var* is a per-adapter, per-profile-configurable setting (each proxy URL can
    ...
    process-env read — those are OS/system-level network settings, not a per-profile ShellGPT concept."""
...
def _config_section(name: str) -> dict:
    ...
    try:
        from shellgpt_cli.config import load_config_readonly as _load_config
...
from shellgpt_constants import get_default_shellgpt_root, get_shellgpt_dir, get_shellgpt_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.shellgpt/.env manually.")
...
# ``shellgpt pairing approve`` command (shellgpt_cli/subcommands/pairing.py) that lets the owner fix it.
...
UNAUTHORIZED_ACTION_NOTICE = (
    ...
    "`shellgpt pairing approve {platform} <request-id>` on the host (`shellgpt pairing list` shows the id).")
...
IMAGE_CACHE_DIR = get_shellgpt_dir("cache/images", "image_cache")
...
def _cache_dir_accessors(kind: str, constant_name: str, new_subpath: str, old_name: str):
    ...
    get_shellgpt_dir (active profile) unless a test monkeypatched the module constant away from
    its import-time default, and creates the directory; ``cleanup(max_age_hours=24)`` deletes
    older files and returns the count."""
    def get_dir() -> Path:
        d = get_shellgpt_dir(new_subpath, old_name)
...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; ShellGPTAgent/1.0)", "Accept": accept}
...
AUDIO_CACHE_DIR = get_shellgpt_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_shellgpt_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_shellgpt_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_shellgpt_dir("cache/screenshots", "browser_screenshots")
...
_SHELLGPT_ROOT = get_default_shellgpt_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "SHELLGPT_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "SHELLGPT_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "SHELLGPT_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "SHELLGPT_MEDIA_DELIVERY_STRICT"
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_SHELLGPT_HOME / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the SHELLGPT_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active SHELLGPT_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (SHELLGPT_HOME symlinked).

    ``SHELLGPT_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when SHELLGPT_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
...
def _profile_dirs() -> List[Path]:
    ...
    try:
        return [p for p in (_SHELLGPT_ROOT / "profiles").iterdir() if p.is_dir()]
    except OSError:
        return []


def _credential_home_roots() -> List[Path]:
    """Every ShellGPT home whose credential stores the denylist must cover: the ACTIVE home
    (the per-turn SHELLGPT_HOME override under ``gateway.multiplex_profiles``), the shared root
    ...
    return list(dict.fromkeys((get_shellgpt_home(), _SHELLGPT_ROOT, *_profile_dirs())))


def _kanban_root() -> Path:
    """Kanban is root-shared across profiles by design (``kanban_db.kanban_home``)."""
    return Path(os.environ.get("SHELLGPT_KANBAN_HOME", "").strip() or _SHELLGPT_ROOT).expanduser()
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("SHELLGPT_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    Credential sub-dirs (``~/.ssh``, ``~/.shellgpt/.env``) stay blocked (more-specific entries)."""
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted ShellGPT cache dirs (``/root/.shellgpt/...`` in
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted cache dirs (``/root/.shellgpt/...``), persistent ``/workspace`` and ``/root``."""
    if not candidate.is_absolute():
        return None
    # In-process gateways (Desktop, `shellgpt serve`) may not have bridged terminal.* config into
    ...
    # prefixes). /root/.shellgpt/* that missed a cache mount is the container's credential surface —
    # translating it via the home mount would dodge the host denylist.
    if "/root" not in mounted and not candidate.as_posix().startswith("/root/.shellgpt"):
        mounts.extend(
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    (``SHELLGPT_MEDIA_DELIVERY_STRICT=1``, public bots where prompt injection must not exfiltrate
    host secrets): MUST be under a ShellGPT cache, an operator root (``SHELLGPT_MEDIA_ALLOW_DIRS``),
    ...
    for root in _media_delivery_allowed_roots():
        ...
        if resolved_root is not None and _path_is_within(resolved, resolved_root):
            return str(resolved)
    # Non-strict (default): anything not denylisted (/etc, /proc, ~/.ssh, ShellGPT-root secrets).
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?shellgpt\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+shellgpt[.!?\s]*$", re.IGNORECASE))
...
class BasePlatformAdapter(ABC):
    ...
    # adapters (API server). Propagated to ``SHELLGPT_SESSION_ASYNC_DELIVERY`` so tools never promise
    # a delivery they can't keep.
    supports_async_delivery: bool = True
    # ``send()`` chunks natively via ``truncate_message()`` -> the router skips its truncation.
    splits_long_messages: bool = False
    # Prefix users can always TYPE for ShellGPT commands ("!" where the client eats a leading "/").
    ...
    # ``shellgpt gateway migrate`` can tell "URL changes" from "this profile would be skipped" as new
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-SHELLGPT_HOME
        ...
        remedy = (f" Stop that gateway first (shellgpt --profile {owner_profile} gateway stop)."
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from shellgpt_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    async def _synthesize_auto_tts(self, text_content: str) -> Tuple[List[str], Optional[str]]:
        ...
        (logged, never raised). Path built platform-aware HERE: SHELLGPT_SESSION_PLATFORM is cleared
        post-handler."""
        paths: List[str] = []
        requested_path = None
        try:
    ...
    def _wants_auto_tts(self, event: MessageEvent, session_key: str, interrupt_event: asyncio.Event,
        ...
        generation = getattr(interrupt_event, "_shellgpt_run_generation", None)
        return bool(
    ...
    async def _fire_post_delivery_callback(self, session_key: str, interrupt_event: asyncio.Event) -> None:
        ...
        _post_cb = self.pop_post_delivery_callback(
            session_key, generation=getattr(interrupt_event, "_shellgpt_run_generation", None))
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            self._streaming_tts_completed_turns.discard(self._streaming_tts_turn_key(
                session_key, getattr(interrupt_event, "_shellgpt_run_generation", None),
                event=event) or "")
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile SHELLGPT_HOME isolation."""

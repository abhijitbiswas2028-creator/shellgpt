...
There are two ways to add a platform to the ShellGPT gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.shellgpt/plugins/` (or under `plugins/platforms/`
...
**zero changes to core ShellGPT code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `shellgpt gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `shellgpt_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the ShellGPT core.
...
```python
"shellgpt-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _SHELLGPT_CORE_TOOLS,
    "includes": []
},
```

And add it to the `shellgpt-gateway` composite:

```python
"shellgpt-gateway": {
    "includes": [..., "shellgpt-your-platform"]
}
```
...
## 12. Status Display (`shellgpt_cli/status.py`)
...
## 13. Gateway Setup Wizard (`shellgpt_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ shellgpt_cli/ toolsets.py \
  --include="*.py" -l | sort -u

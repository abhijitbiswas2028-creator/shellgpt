...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.shellgpt to the active profile home."""
    ...
    if raw == "~/.shellgpt" or raw.startswith("~/.shellgpt/"):
        return shellgpt_home / raw[len("~/.shellgpt/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else shellgpt_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under SHELLGPT_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from shellgpt_constants import get_shellgpt_home
    scripts_root = (get_shellgpt_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.shellgpt" or raw_text.startswith("~/.shellgpt/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__shellgpt_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__shellgpt_ignore__") is True

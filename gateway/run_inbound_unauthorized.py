...
def pairing_code_reply(platform_name: str, code: str, profile_arg: str = "") -> str:
    ...
    approve_cmd = f"shellgpt {profile_arg}pairing approve {platform_name} {code}"
    return (
...
def unauthorized_owner_hint(
    platform_name: str, user_id: str, user_name: str = "", *, shellgpt_home: str,
) -> str:
    ...
    allowlist = (
        f"add the ID to {env_var} in {shellgpt_home}/.env and restart the gateway"
    ...
    return (
        ...
        f"{shellgpt_home}/config.yaml so unknown senders receive a pairing code you can approve with "
        f"`shellgpt pairing approve {platform_name} <code>`."
    )

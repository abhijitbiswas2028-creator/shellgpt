...
Multiplex-only (Teknium ruling): exactly ONE ``shellgpt gateway run`` per host, multiplexing every
...
def _normalized(name: Optional[str]) -> str:
    if not name:
        return ""
    # Late import: ``shellgpt_cli.profiles`` imports gateway modules back.

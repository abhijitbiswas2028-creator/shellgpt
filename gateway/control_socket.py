...
boundary. POSIX: ``$SHELLGPT_HOME/gateway.sock`` (or a temp-dir socket + ``gateway.sock.path`` pointer
file when the home path exceeds ``sun_path``); Windows: named pipe ``\\\\.\\pipe\\shellgpt-gateway-<hash>``.
...
def windows_pipe_name(home: Path) -> str:
    """Per-SHELLGPT_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\shellgpt-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    name = f"shellgpt-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (launchd_job_label(env) or env.get("LAUNCHD_SOCKET")):
        return "launchd"
    if env.get("SHELLGPT_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        **{k: record.get(k) for k in ("kind", "pid", "start_time", "shellgpt_home")},
        "profile": _profile_label_for_home(record.get("shellgpt_home") or ""),
...
class GatewayControlServer:
    ...
    def __init__(self, home: Optional[Path] = None, *,
        ...
        if home is None:
            ...
            home = _get_process_shellgpt_home()

...
class GatewayAdapterLifecycleMixin:
    ...
    def _adapter_disconnect_timeout_secs(self) -> float:
        ...
        override = self._env_timeout_override("SHELLGPT_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT")
    ...
    def _platform_connect_timeout_secs(self, platform=None, *, initial: bool = False) -> float:
        ...
        override = self._env_timeout_override("SHELLGPT_GATEWAY_PLATFORM_CONNECT_TIMEOUT")
    ...
    async def _handoff_watcher(self, interval: float = 2.0, drain_timeout: float = 30.0) -> None:
        ...
        async def _tick(profile_name: Optional[str] = None) -> None:
            ...
            for row in pending:
                ...
                # ``set_shellgpt_home_override`` and ``set_secret_scope`` are ContextVar-based — ensure_future
    ...
    def _flag_reconnect_needs_attention(
        ...
        logger.warning(
            ...
            "credentials, missing intents, broken sidecar). Check `shellgpt status` / `/platform list`.",
    ...
    async def _start_secondary_profile_adapters(self) -> int:
        ...
        Each profile connects under its own SHELLGPT_HOME + secret scope; credential/listener collisions
        ...
        if not self._multiplex_on():
            ...
            # ``served_profiles`` would outlive it into this single-profile run and `shellgpt -p X ...`
            ...
            return 0
        try:
            from shellgpt_cli.profiles import get_active_profile_name, profiles_to_serve, profile_is_parked
        except Exception:
            return 0
    ...
    def _record_served_profiles(self, active: str, profile_homes) -> None:
        ...
        than "has a connected adapter") for `shellgpt status`; seed per-profile PairingStores."""
        with _log_suppressed(logging.DEBUG, "could not record served_profiles", exc_info=True):
            ...
            if owns_host_lock(ROLE_GATEWAY):
                ...
                publish_record(ROLE_GATEWAY, profiles=tuple(served), home=str(get_shellgpt_home()))
    ...
    def _note_unserved_secondary_platform(self, profile_name: str, platform: Platform) -> None:
        ...
        ``<profile>:<platform>`` status entry so ``shellgpt gateway status --profile X`` and the
    ...
    def _configure_profile_adapter(
        ...
        # See #76423.
        adapter._shellgpt_profile_name = profile_name
    ...
    def _make_default_profile_message_handler(self):
        ...
        from gateway.run import _async_profile_runtime_scope, get_shellgpt_home
        default_home = Path(get_shellgpt_home())
    ...
    async def _handle_gateway_platform_event(self, event: dict, source) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "gateway_platform_event hook dispatch failed", exc_info=True):
            from shellgpt_cli.lifecycle import has_hook, invoke_hook
    ...
    def _make_default_profile_platform_event_handler(self):
        ...
        from gateway.run import _profile_runtime_scope, get_shellgpt_home
        default_home = Path(get_shellgpt_home())
    ...
    def _adapter_credential_fingerprint(adapter: Any) -> Optional[str]:
        ...
        for obj, attr in candidates:
            ...
            if isinstance(val, str) and val.strip():
                import hashlib
                return hashlib.sha256(("shellgpt-mux:" + val.strip()).encode("utf-8")).hexdigest()[:16]
        return None
    ...
    def _make_adapter_auth_check(
        ...
        transport_home = Path(get_shellgpt_home()) if self._multiplex_on() and profile_name is None else None

...
Defenses 1-2 (``_SHELLGPT_GATEWAY`` guard on ``shellgpt gateway stop|restart`` /
...
persisted to ``<SHELLGPT_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_shellgpt_home() / "gateway" / "restart_loop.json"

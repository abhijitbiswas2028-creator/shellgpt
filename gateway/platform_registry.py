...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``shellgpt setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``shellgpt gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SHELLGPT_HOME and overlay the
        ...
        # `shellgpt` invocation, so the import happens only when a lookup asks for it.

"""Session-scoped context variables for the ShellGPT gateway.

Replaces the old ``os.environ``-based ``SHELLGPT_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "SHELLGPT_SESSION_PLATFORM", "SHELLGPT_SESSION_SOURCE", "SHELLGPT_SESSION_CHAT_ID",
    "SHELLGPT_SESSION_CHAT_TYPE", "SHELLGPT_SESSION_CHAT_NAME", "SHELLGPT_SESSION_THREAD_ID",
    "SHELLGPT_SESSION_USER_ID", "SHELLGPT_SESSION_USER_ID_ALT", "SHELLGPT_SESSION_USER_NAME",
    "SHELLGPT_SESSION_SCOPE_ID", "SHELLGPT_SESSION_KEY", "SHELLGPT_SESSION_ID",
    "SHELLGPT_UI_SESSION_ID", "SHELLGPT_SESSION_MESSAGE_ID", "SHELLGPT_SESSION_PROFILE",
    "SHELLGPT_BROWSER_CONTROL_PRINCIPAL", "SHELLGPT_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "SHELLGPT_CRON_SESSION", "SHELLGPT_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("SHELLGPT_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("SHELLGPT_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("SHELLGPT_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("SHELLGPT_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("SHELLGPT_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``SHELLGPT_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``SHELLGPT_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds SHELLGPT_SESSION_PLATFORM, CLI/TUI/
# desktop bind SHELLGPT_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``SHELLGPT_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("SHELLGPT_PLATFORM") or get_session_env("SHELLGPT_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("SHELLGPT_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See NousResearch/shellgpt-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``SHELLGPT_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""

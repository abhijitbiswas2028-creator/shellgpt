...
Multiplex-only (Teknium ruling): exactly one MULTIPLEXING ``shellgpt gateway run`` per host, serving
...
def _record_home(record) -> Path:
    ...
    return Path(record.home) if getattr(record, "home", "") else Path(get_default_shellgpt_root())
...
def _identity_matches(identity, record, home: Path) -> bool:
    ...
    if not isinstance(identity, dict) or identity.get("pid") != record.pid:
        return False
    reported = identity.get("shellgpt_home")
    if not reported:
    ...
    try:
        ...
        return bool(_same_shellgpt_home(Path(str(reported)), home))
    except Exception:
...
def attach_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"`shellgpt -p {gateway.profile_label} gateway restart`.")
...
def _unknown_served_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"   Take the host over (only from the home that launched it):  shellgpt gateway run --replace\n"
        f"   Start anyway:  shellgpt gateway run --force")
...
def _refuse_message(gateway: HostGateway, profile: str) -> str:
    ...
    return (
        ...
        f"   Or start one anyway:         shellgpt gateway run --force\n"

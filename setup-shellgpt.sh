...
# 5. Symlinks the 'shellgpt' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ ShellGPT Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "${TMPDIR:-${SHELLGPT_HOME:-$HOME/.shellgpt}}/shellgpt-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "${TMPDIR:-${SHELLGPT_HOME:-$HOME/.shellgpt}}/shellgpt-uv-installer.$$.sh")"
...
if is_termux; then
    # ShellGPT currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; ShellGPT requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink shellgpt into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up shellgpt command..."

SHELLGPT_BIN="$SCRIPT_DIR/venv/bin/shellgpt"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SHELLGPT_BIN" "$COMMAND_LINK_DIR/shellgpt"
echo -e "${GREEN}✓${NC} Symlinked shellgpt → $COMMAND_LINK_DISPLAY_DIR/shellgpt"
...
# ============================================================================
# Seed bundled skills into ~/.shellgpt/skills/
# ============================================================================

SHELLGPT_SKILLS_DIR="${SHELLGPT_HOME:-$HOME/.shellgpt}/skills"
mkdir -p "$SHELLGPT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.shellgpt/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SHELLGPT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  shellgpt status        # Check configuration"
if is_termux; then
    echo "  shellgpt gateway       # Run gateway in foreground"
else
    echo "  shellgpt gateway install # Install gateway service (messaging + cron)"
fi
echo "  shellgpt cron list     # View scheduled jobs"
echo "  shellgpt doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m shellgpt_cli.main setup
fi

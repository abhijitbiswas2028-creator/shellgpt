 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `shellgpt serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. ShellGPT should add
...
export const APPROVAL_COMMAND = 'rm -rf /tmp/shellgpt-e2e-approval-probe'
 ...
 * `E2E_SAY(code-farmer)[{at}shellgpt Reply with B.] E2E_SAY(shellgpt)[B]`.
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `shellgpt-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated SHELLGPT_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `shellgpt-dev-mock-${Date.now()}`))
  const shellgptHome = nodePath.join(root, 'shellgpt-home')
...
function writeMockConfig(shellgptHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(shellgptHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(shellgptHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.shellgptHome, mock.url)
  console.log(`  SHELLGPT_HOME: ${sandbox.shellgptHome}`)
  ...
  const env: Record<string, string> = {
    ...
    SHELLGPT_DESKTOP_SHELLGPT_ROOT: repoRoot,
    SHELLGPT_DESKTOP_APP_NAME: `ShellGPTDevMock-${Date.now()}`,
  }

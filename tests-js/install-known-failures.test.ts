...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/shellgpt.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/shellgpt.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'shellgpt-update',
  error: 'E2E ASSERTION FAILED: shellgpt update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('shellgpt.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'shellgpt-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured shellgpt desktop spec; update completed',
      logs: { desktop: '[shellgpt] [updates] no staged updater; surfacing manual `shellgpt update` for CLI install at C:/install\n[shellgpt] [updates] manual: shellgpt update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: shellgpt update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> shellgpt-update (v2026.3.12 -> HEAD)'

...
timeout. Config is env-only (``SHELLGPT_STARTUP_WATCHDOG=0``,
``SHELLGPT_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "SHELLGPT_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "SHELLGPT_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (shellgpt_cli.main / shellgpt_cli.gateway /
...
def _process_shellgpt_home() -> Path:
    """SHELLGPT_HOME for diagnostic files — stdlib-only replica of the shellgpt_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<SHELLGPT_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_shellgpt_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``SHELLGPT_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``SHELLGPT_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:

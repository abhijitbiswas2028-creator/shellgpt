"""Timezone-aware clock for ShellGPT.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``SHELLGPT_TIMEZONE`` env var, then ``timezone`` in ``~/.shellgpt/config.yaml``, else server-local
...
# SHELLGPT_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``SHELLGPT_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("SHELLGPT_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without shellgpt_cli importable.
        try:

...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with shellgpt-agent)
2. User plugins: ``$SHELLGPT_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``shellgpt_agent.plugins``
...
process; the ``$SHELLGPT_HOME`` plugins of the profile home bound at lookup time
...
# ``$SHELLGPT_HOME`` plugin dir) / "runtime" (entry point, legacy module, direct call).
...
@dataclass
class _HomeLayer:
    """The ``$SHELLGPT_HOME/plugins`` providers of ONE profile home.
    ...
    ``stamps`` are the plugin dirs' mtimes: a directory added by ``shellgpt plugins install`` while the
...
# The layer a ``$SHELLGPT_HOME`` plugin import registers into. A ContextVar, not a module global:
# two turn threads scanning two profile homes at once must not cross-register. Never a lock held
# across the import itself — a thread mid-``import shellgpt_cli.auth`` (whose import calls
...
def _sync_auth_registry() -> None:
    """Mirror profiles into the ``shellgpt_cli`` snapshots (auth registry, picker catalog) that are loaded.

    ``shellgpt_cli.auth`` takes its own snapshot of ``list_providers()`` when it is imported. If a
    ...
    ``sys.modules`` on purpose: this layer must never import ``shellgpt_cli`` (that would run auth's
    top-level code mid-scan and risk a circular import). Never raises: registration must not fail
    because of the auth mirror.
    """
    for module, attr in (
        ("shellgpt_cli.auth", "sync_plugin_provider_registry"),
        ("shellgpt_cli.models_catalog_static", "sync_plugin_provider_catalog"),
    ):
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SHELLGPT_HOME/plugins/model-providers/`` can override
    bundled profiles without editing repo code. A registration made while a
    ``$SHELLGPT_HOME`` plugin is being imported lands in that home's layer.
    """
...
def provider_source(name: str) -> str | None:
    ...
    ``"user"`` is what lets a ``$SHELLGPT_HOME`` plugin re-registering a bundled name win in
    ``shellgpt_cli.auth.PROVIDER_REGISTRY`` too — a bundled profile never rewrites a built-in row.
    """
    layer = _home_layer()
...
def routed_model_rejects_vision_tool_messages(provider: str, model: str) -> bool:
    ...
    from shellgpt_cli.providers import is_routing_aggregator
...
def list_providers() -> list[ProviderProfile]:
    ...
    ``$SHELLGPT_HOME`` plugins shadow process-wide profiles of the same name."""
    global _PROVIDER_LIST_CACHE
    if not _discovered:
        _discover_providers()
    layer = _home_layer()
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SHELLGPT_HOME/plugins/model-providers/`` if it exists."""
    try:
        from shellgpt_constants import get_shellgpt_home

        d = get_shellgpt_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$SHELLGPT_HOME/plugins/`` if it exists.

    This is where ``shellgpt plugins install`` clones a plugin — flat, one
...
def _scan_home_layer(layer: _HomeLayer, key: str) -> None:
    ...
    ``$SHELLGPT_HOME/plugins/model-providers/<name>/`` first, then plugins cloned flat by
    ``shellgpt plugins install`` into ``$SHELLGPT_HOME/plugins/<name>/`` that declare
...
def _user_module_name(plugin_dir: Path, home_key: str) -> str:
    ...
    return f"_shellgpt_user_provider_{digest}_{plugin_dir.name.replace('-', '_')}"
...
def _import_plugin_dir(plugin_dir: Path, source: str, *, home_key: str = "") -> None:
    ...
    # multiple SHELLGPT_HOME profiles don't alias each other.
    if source == "bundled":
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``shellgpt_agent.plugins``
    ...
    * **Provider targets only.** The ``shellgpt_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SHELLGPT_HOME``) keep their
    ...
    try:
        from shellgpt_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    module-level. Later steps win on name collision. ``$SHELLGPT_HOME`` plugins are
    ...
    try:
        ...
        _discovering = False
        # shellgpt_cli.auth may have been imported by a plugin during discovery and snapshotted a
...
def _run_discovery_steps() -> None:
    ...
    # 0. Pip-installed plugins — entry points in the ``shellgpt_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SHELLGPT_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with shellgpt-agent.

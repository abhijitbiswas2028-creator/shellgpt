...
def _cwd_prefix_filter(value: str) -> Tuple[List[str], list]:
    from shellgpt_state_sessions import _cwd_prefix_clause
...
class SessionMaintenanceMixin:
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # PASSIVE, not TRUNCATE: a manual `shellgpt sessions vacuum` runs in a transient CLI
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        """
        from shellgpt_state_repair import _release_auto_maintenance_lock, _try_acquire_auto_maintenance_lock
        ...
        try:
            ...
            if vacuum and pruned > 0 and vacuum_due:
                ...
                # Same admission `shellgpt sessions optimize` runs: VACUUM plus the TRUNCATE checkpoint
                ...
                from shellgpt_state_holders import (

...
description: "Run ShellGPT Agent directly on an Android phone with Termux"
---

# ShellGPT on Android with Termux
...
ShellGPT Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop ShellGPT from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed ShellGPT installs are marked with install method `apt`. ShellGPT therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade shellgpt-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. ShellGPT runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

ShellGPT now ships a Termux-aware installer path:
...
- links `shellgpt` into `$PREFIX/bin` so it stays on your Termux PATH
...
ShellGPT requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `shellgpt` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/shellgpt" "$PREFIX/bin/shellgpt"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `shellgpt` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.shellgpt/.env`.
...
`uvloop` bundles libuv, whose `./configure` does not run on Android. ShellGPT keeps it out of the core install and of the `termux` / `termux-all` extras — `uvicorn` falls back to the stdlib asyncio loop, which the dashboard and gateway run fine on. If you see this error, you are installing an extra that includes `uvloop` (for example `[all]`): install `.[termux]` or `.[termux-all]` instead.

### `shellgpt doctor` says ripgrep or Node is missing

...
description: "Frequently asked questions and solutions to common issues with ShellGPT Agent"
---
...
### What LLM providers work with ShellGPT?

ShellGPT Agent works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `shellgpt auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `shellgpt model` or by editing `~/.shellgpt/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run ShellGPT in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run ShellGPT inside WSL2
...
- let ShellGPT use the resulting MCP browser tools

This is more reliable than trying to force ShellGPT core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with ShellGPT](../guides/use-mcp-with-shellgpt.md#wsl2-bridge-shellgpt-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). ShellGPT Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.shellgpt/`.

### Can I use it offline / with local models?

Yes. Run `shellgpt model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← ShellGPT minimum; set this to match your server's actual context window
```
...
ShellGPT persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in ShellGPT — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
ShellGPT auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `SHELLGPT_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

ShellGPT Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same ShellGPT Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use ShellGPT programmatically:
...
#### `shellgpt: command not found` after installation
...
**Cause:** ShellGPT requires Python 3.11 or newer.
...
**Cause:** ShellGPT builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when ShellGPT runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** ShellGPT auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.shellgpt/config.yaml`:
...
Independently of the init files, every terminal command's `PATH` is completed with the standard system directories (`/usr/local/bin`, `/opt/homebrew/bin`, …), the ShellGPT-managed runtime dirs, and `~/.local/bin` when it exists (the `pip --user` / `pipx` / `uv tool` install target) — appended after your own entries, so precedence is unchanged. This covers backends started with a thin non-interactive PATH (systemd, GUI launchers, the Desktop SSH remote backend) without any configuration.
...
sudo rm /usr/local/bin/shellgpt
...
#### The agent says "ShellGPT policy" or "ShellGPT guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden ShellGPT runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

ShellGPT enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. ShellGPT does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual ShellGPT tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a ShellGPT execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](../user-guide/security.md) for ShellGPT' documented execution controls and [Providers](../integrations/providers.md) for provider configuration.

#### "…refused this request because of a policy on your account"

**Meaning:** the provider rejected the request for an account-level reason that retrying cannot change — an aggregator's data/privacy settings excluded every endpoint for the model, or the model's upstream provider has blocked the account (for example `this user has been blocked for a previous policy violation`, which OpenRouter can relay inside an otherwise successful HTTP 200 stream). ShellGPT sends the request once, does not retry it or rotate credentials, and moves to your fallback chain if one is configured.

**Solution:** check the account's status and data/privacy settings with the provider named in the reply, or switch to another model or provider with `/model`. `shellgpt fallback add` routes future blocks to a backup automatically.
...
| "The AI model service isn't reachable right now — the configured model endpoint is **not running or is unreachable**." | Nothing accepted the connection (`Connection refused`, no route to host, DNS failure). | Start the model server / check `base_url`, then `/retry`; `shellgpt doctor` on the host. |
| "ShellGPT **could not reach** the AI model service (no further detail from the SDK)." | The SDK reported a generic `APIConnectionError` and kept no cause; neither of the above is certain. | `/retry`; `shellgpt doctor` if it persists. The raw exception is in `shellgpt logs`. |
...
**Solution:** Exit your session and use `shellgpt model` from your terminal to add new providers:

```bash
# Exit the ShellGPT chat session first (Ctrl+C or /quit)
...
After adding a new provider via `shellgpt model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `shellgpt model` (from terminal) |
| Enter/change API keys | `shellgpt model` (from terminal) |
...
shellgpt config show

# Re-configure your provider
shellgpt model

# Or set directly
shellgpt config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.shellgpt/.env` for conflicting entries.
:::
...
# Set a valid model
shellgpt config set model.default anthropic/claude-opus-4.7

# Or specify per-session
shellgpt chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `shellgpt chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or ShellGPT detected the wrong context length for your model.
...
shellgpt chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, ShellGPT may have the wrong context length for your model. Check what it detected:
...
**Cause:** ShellGPT detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — ShellGPT never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `shellgpt chat`
...
shellgpt gateway status

# Start the gateway
shellgpt gateway start

# Check logs for errors
cat ~/.shellgpt/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `shellgpt gateway setup`
- Check gateway logs: `cat ~/.shellgpt/logs/gateway.log | tail -50`
...
Configure in `~/.shellgpt/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.shellgpt/shellgpt-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `shellgpt gateway start` fails
...
tmux new -s shellgpt 'shellgpt gateway run'
# Reattach later: tmux attach -t shellgpt

# Option 3: Background via nohup
nohup shellgpt gateway run > ~/.shellgpt/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'shellgpt gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `shellgpt gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
shellgpt gateway install    # Re-snapshots your current PATH
shellgpt gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.shellgpt.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `shellgpt chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `shellgpt chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`shellgpt prompt-size`](./cli-commands.md#shellgpt-prompt-size) to measure it, then trim: disable toolsets you don't use (`shellgpt tools`) and uninstall or disable skills you don't need (`shellgpt skills`).
...
cd ~/.shellgpt/shellgpt-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.shellgpt/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
shellgpt config show | grep -A 12 mcp_servers

# Restart ShellGPT or reload MCP after config changes
shellgpt chat
```
...
:::warning
If an MCP server crashes mid-request, ShellGPT will report a timeout. Check the server's own logs (not just ShellGPT logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `shellgpt profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or ShellGPT home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](../user-guide/features/memory-providers.md).

### What happens when I run `shellgpt update`?

`shellgpt update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `shellgpt update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is a directory under `~/.shellgpt/profiles/` that carries at least one identity file (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json` or `state.db`); a bare directory without one (a leftover from a log rotation or cron tick) is not a profile — it is not listed or served, `-p <name>` reports it as missing, and `shellgpt profile create <name>` refuses to overwrite it until you move or remove it. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** ShellGPT can route subagents to a different model automatically. Set this in `~/.shellgpt/config.yaml`:
...
Now when you tell ShellGPT "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can ShellGPT do this?

**Current limitation:** ShellGPT profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, ShellGPT reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `shellgpt skills config` settings don't seem to take effect.

**Solution:** Use `shellgpt skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`shellgpt gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** ShellGPT creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting ShellGPT to another machine
...
1. Install ShellGPT Agent on the new machine:
   ```bash
   curl -fsSL https://shellgpt-agent.nousresearch.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   shellgpt backup
   ```
   This creates a zip of your entire `~/.shellgpt/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/shellgpt-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/shellgpt-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   shellgpt import ~/shellgpt-backup-<timestamp>.zip
   ```

4. On the new machine, run `shellgpt setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
shellgpt profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
shellgpt profile import ./work-backup.tar.gz work
```
...
### `shellgpt backup` vs `shellgpt profile export`

| Feature | `shellgpt backup` | `shellgpt profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.shellgpt` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='shellgpt-agent' ~/.shellgpt/ newmachine:~/.shellgpt/
```

:::tip
`shellgpt backup` produces a consistent snapshot even while ShellGPT is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the ShellGPT installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a ShellGPT-specific issue — it's a shell config permissions problem.
...
shellgpt config show | head -20

# Re-run model selection
shellgpt model

# Or test with a known-good model
shellgpt chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/shellgpt-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), ShellGPT version (`shellgpt --version`), and the full error message

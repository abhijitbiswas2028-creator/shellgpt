...
This page covers all commands related to [ShellGPT profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `shellgpt profile` without a subcommand shows help.
...
## `shellgpt profile use`

```bash
shellgpt profile use <name>
```

Sets `<name>` as the active profile. All subsequent `shellgpt` commands (without `-p`) will use this profile.
...
shellgpt profile use default
```

## `shellgpt profile create`

```bash
shellgpt profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `shellgpt profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `shellgpt update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.shellgpt`), use `shellgpt skills opt-out` / `shellgpt skills opt-in`. |
...
shellgpt profile create work2 --clone-from work

# Clone everything from a specific profile
shellgpt profile create work2-backup --clone-from work --clone-all
```

## `shellgpt profile describe`

```bash
shellgpt profile describe [<name>] [options]
```
...
shellgpt profile describe researcher

# Set it explicitly
shellgpt profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
shellgpt profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
shellgpt profile describe --all --auto
```

## `shellgpt profile delete`

```bash
shellgpt profile delete <name> [options]
```
...
```bash
shellgpt profile delete mybot
shellgpt profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.shellgpt`) cannot be deleted — use `shellgpt uninstall` to remove everything.
:::

## `shellgpt profile show`

```bash
shellgpt profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

The skills count here (and in `shellgpt profile list`) is counted on the spot. The Desktop and dashboard profile lists are polled every few seconds, so they show the last known count instead and refresh it in the background — a freshly started backend may briefly show `0` skills for a profile until the first background count lands, and a skill you just installed appears in those lists within about a minute.

This shows the profile's ShellGPT home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ shellgpt profile show work
Profile: work
Path:    ~/.shellgpt/profiles/work
...
## `shellgpt profile alias`

```bash
shellgpt profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your ShellGPT installation.
...
```bash
shellgpt profile alias work
# Creates/updates ~/.local/bin/work

shellgpt profile alias work --name mywork
# Creates ~/.local/bin/mywork

shellgpt profile alias work --remove
# Removes the wrapper script
```

## `shellgpt profile rename`

```bash
shellgpt profile rename <old-name> <new-name>
```
...
```bash
shellgpt profile rename mybot assistant
# ~/.shellgpt/profiles/mybot → ~/.shellgpt/profiles/assistant
...
after the rename; `shellgpt profile migrate-identity` retries that step too if it was reported as
failed.

## `shellgpt profile migrate-identity`

```bash
shellgpt profile migrate-identity <old-name> <new-name>
```

Retries the identity migration of a rename that already completed. Run it if `shellgpt profile
...
#     shellgpt profile migrate-identity mybot assistant

shellgpt profile migrate-identity mybot assistant
# ✓ Session/routing identity migrated: mybot → assistant
```

## `shellgpt profile purge-identity`

```bash
shellgpt profile purge-identity <name>
```

Retries the identity purge of a delete that already completed. Run it if `shellgpt profile delete`
...
is decided by `shellgpt profile delete`, which removes the profile's own `profiles/<name>/`, its
...
#   Restart the gateway, then run:
#     shellgpt profile purge-identity mybot

shellgpt profile purge-identity mybot
# ✓ Session/routing identity purged: mybot
```

## `shellgpt profile export`

```bash
shellgpt profile export <name> [options]
```
...
```bash
shellgpt profile export work
# Creates work.tar.gz in the current directory

shellgpt profile export work -o ./work-2026-03-29.tar.gz
```
...
## `shellgpt profile import`

```bash
shellgpt profile import <archive> [options]
```
...
```bash
shellgpt profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

shellgpt profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `shellgpt -p telemetry`). |
...
shellgpt profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
shellgpt profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
shellgpt profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
shellgpt profile install ./telemetry/
```

### `shellgpt profile update`

```bash
shellgpt profile update <name> [--force-config] [--yes]
```
...
### `shellgpt profile info`

```bash
shellgpt profile info <name>
```

Prints the profile's distribution manifest — name, version, required
ShellGPT version, author, env var requirements, the source URL/path, and
...
`shellgpt profile list` also shows the distribution name and version in a
`Distribution` column, and `shellgpt profile show <name>` / `delete <name>`
...
shellgpt profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
shellgpt profile install https://github.com/your-org/internal-assistant.git
```
...
`shellgpt_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
ShellGPT version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host ShellGPT can clone from.
3. Tell recipients to run `shellgpt profile install <your-repo-url>`.
...
## `shellgpt -p` / `shellgpt --profile`

```bash
shellgpt -p <name> <command> [options]
shellgpt --profile <name> <command> [options]
```

Global flag to run any ShellGPT command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
shellgpt -p work chat -q "Check the server status"
shellgpt --profile dev gateway start
shellgpt -p personal skills list
shellgpt -p work config edit
```

## `shellgpt completion`

```bash
shellgpt completion <shell>
```
...
- `shellgpt profile <TAB>` — subcommands (list, use, create, etc.)
- `shellgpt profile use <TAB>` — profile names
- `shellgpt -p <TAB>` — profile names

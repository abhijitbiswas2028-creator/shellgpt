...
Copy-paste blueprints for common automation patterns. Each blueprint uses ShellGPT's built-in [cron scheduler](../user-guide/features/cron.md) for time-based triggers and [webhook platform](../user-guide/messaging/webhooks.md) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`shellgpt webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `shellgpt webhook subscribe`) |
...
```bash
shellgpt cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/shellgpt-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/shellgpt-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
shellgpt webhook subscribe github-pr-review \
  --events "pull_request" \
...
shellgpt cron create "0 9 * * 1" \
  "Scan the NousResearch/shellgpt-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/shellgpt-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (shellgpt_cli/commands.py, shellgpt_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (shellgpt_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
shellgpt cron create "0 6 * * *" \
  "Run a dependency security audit on the shellgpt-agent project.

1. cd ~/.shellgpt/shellgpt-agent && source .venv/bin/activate
...
```bash
shellgpt webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
shellgpt webhook subscribe alert-triage \
...
```python title="~/.shellgpt/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "ShellGPT-Monitor/1.0"})
...
```bash
shellgpt cron create "every 30m" \
  ...
  --script ~/.shellgpt/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
shellgpt cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to ShellGPT Agent development." \
...
```bash
shellgpt webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
shellgpt webhook subscribe stripe-payments \
...
```bash
shellgpt cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the shellgpt-agent codebase.

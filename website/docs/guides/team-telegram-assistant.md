...
This tutorial walks you through setting up a Telegram bot powered by ShellGPT Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **ShellGPT Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](../getting-started/installation.md) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.shellgpt/.env`

:::tip
A $5/month VPS is plenty for running the gateway. ShellGPT itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team ShellGPT Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_shellgpt_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by ShellGPT Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.shellgpt/.env`:
...
```
[Gateway] Starting ShellGPT Gateway...
...
# View live logs
journalctl --user -u shellgpt-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `shellgpt gateway install` to update the plist.
:::
...
```bash
shellgpt gateway stop && shellgpt gateway start
```
...
shellgpt pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.shellgpt/.env`:
...
Control how much detail the bot shows when using tools. In `~/.shellgpt/config.yaml`:
...
Customize how the bot communicates by editing `~/.shellgpt/SOUL.md`:

For a full guide, see [Use SOUL.md with ShellGPT](./use-soul-with-shellgpt.md).
...
```bash
# From the CLI
shellgpt cron list          # View all scheduled jobs
shellgpt cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u shellgpt-gateway` (Linux) or `~/.shellgpt/logs/gateway.log` (macOS) |
| Cron job output | `~/.shellgpt/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.shellgpt/cron/jobs.json` |
| Pairing data | `~/.shellgpt/pairing/` |
| Session history | `~/.shellgpt/sessions/` |

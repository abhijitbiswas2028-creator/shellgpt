...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `shellgpt send`."
---

# Pipe Script Output to Messaging Platforms

`shellgpt send` is a small, scriptable CLI that pushes a message to any
messaging platform ShellGPT is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | shellgpt send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `shellgpt
...
shellgpt send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
shellgpt send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
shellgpt send --list

# Filter by platform
shellgpt send --list telegram
```
...
Any platform ShellGPT ships adapters for works as a target:
...
`shellgpt send` resolves the message body in this order:

1. **Positional argument** — `shellgpt send --to telegram "hi"`
2. **`--file PATH`** — `shellgpt send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | shellgpt send --to telegram`

When stdin is a TTY (no pipe), ShellGPT does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  shellgpt send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `shellgpt send` reuses your ShellGPT config, the same script works on
any host where ShellGPT is installed — no need to export bot tokens into
...
`shellgpt send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  shellgpt send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | shellgpt send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.shellgpt/bin/shellgpt send \
...
```bash
./train.py --epochs 200 && \
  shellgpt send --to telegram "training done" || \
  shellgpt send --to telegram "training failed (exit $?)"
```
...
shellgpt send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(shellgpt send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `shellgpt send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `shellgpt send` calls
the platform's REST endpoint directly using credentials from
`~/.shellgpt/.env` and `~/.shellgpt/config.yaml` (or the equivalent files under
your resolved ShellGPT home — `%LOCALAPPDATA%\shellgpt` on Windows, or the profile
directory when `SHELLGPT_HOME` / `-p` is set). It's a standalone subprocess
...
`Looked in: C:\Users\me\AppData\Local\shellgpt\.env (no DISCORD_BOT_TOKEN),
C:\Users\me\AppData\Local\shellgpt\config.yaml (no platforms.discord block),
...
`.env` so `shellgpt send` can use it. When your shell is scoped to a profile home
(`SHELLGPT_HOME=<root>/profiles/<name>`) but the connected gateway runs from the
default root, the error says so — the gateway never read the profile's `.env`,
and `shellgpt send --list` points at the root's `channel_directory.json`.
...
pointing at the gateway; start it with `shellgpt gateway start` and retry.
...
# Machine-readable
shellgpt send --list --json
```

The listing is built from `~/.shellgpt/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`shellgpt gateway
...
| Approach | Multi-platform | Reuses ShellGPT creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `shellgpt send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`shellgpt send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `shellgpt send`.
...
- [Gateway Internals](../developer-guide/gateway-internals.md) —
  the delivery router that `shellgpt send` shares with cron delivery.

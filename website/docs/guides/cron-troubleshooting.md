...
description: "Diagnose and fix common ShellGPT cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`shellgpt gateway` for foreground, or `shellgpt gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `shellgpt cron tick`.
...
```bash
date
shellgpt cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.shellgpt/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.shellgpt/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.shellgpt/.env` |
...
| `local` | Write access to `~/.shellgpt/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `shellgpt cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (ShellGPT Cloud / Team Gateway)
...
```bash
shellgpt skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `shellgpt skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `shellgpt skills list`.
...
2. `~/.shellgpt/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `shellgpt cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the ShellGPT config directory). Verify:
```bash
ls ~/.shellgpt/scripts/your-script.py   # Must exist
shellgpt cron edit <job_id> --script ~/.shellgpt/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `shellgpt skills install <skill-name>`.
...
```bash
ps aux | grep shellgpt
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.shellgpt/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.shellgpt/cron/jobs.json
chmod 600 ~/.shellgpt/cron/jobs.json   # Your user should own it
```
...
```bash
shellgpt cron list                    # Show all jobs, states, next_run times
shellgpt cron run <job_id>            # Schedule for next tick (for testing)
shellgpt cron edit <job_id>           # Fix configuration issues
shellgpt logs                         # View recent ShellGPT logs
shellgpt skills list                  # Verify installed skills
```
...
1. Run the job with `shellgpt cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.shellgpt/logs/agent.log` for scheduler messages and `~/.shellgpt/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/shellgpt-agent](https://github.com/NousResearch/shellgpt-agent) with:

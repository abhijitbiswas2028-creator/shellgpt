...
```bash
shellgpt teams-pipeline validate
```
...
```bash
shellgpt teams-pipeline token-health
shellgpt teams-pipeline token-health --force-refresh
```
...
```bash
shellgpt teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
shellgpt teams-pipeline maintain-subscriptions
shellgpt teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: ShellGPT cron (recommended if you already run the ShellGPT gateway)

ShellGPT ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.shellgpt/scripts/`. First create the script:

```bash
mkdir -p ~/.shellgpt/scripts
cat > ~/.shellgpt/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec shellgpt teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.shellgpt/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
shellgpt cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/shellgpt-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=ShellGPT Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=shellgpt
EnvironmentFile=/etc/shellgpt/env
ExecStart=/usr/local/bin/shellgpt teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/shellgpt-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run ShellGPT Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now shellgpt-teams-pipeline-maintain.timer
systemctl list-timers shellgpt-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/shellgpt teams-pipeline maintain-subscriptions >> /var/log/shellgpt/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.shellgpt/.env` at the top of a wrapper script that crontab calls.
...
```bash
shellgpt teams-pipeline subscriptions   # should show expirationDateTime advanced
shellgpt teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
shellgpt teams-pipeline list
shellgpt teams-pipeline list --status failed
shellgpt teams-pipeline show <job-id>
```

### Replay a stored job

```bash
shellgpt teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
shellgpt teams-pipeline fetch --meeting-id <meeting-id>
shellgpt teams-pipeline fetch --join-web-url "<join-url>"
shellgpt teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `shellgpt teams-pipeline maintain-subscriptions --dry-run`
- inspect `shellgpt teams-pipeline list --status failed`
...
- run `shellgpt teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `shellgpt teams-pipeline run`
...
- [ ] `shellgpt teams-pipeline validate` returns an OK snapshot
- [ ] `shellgpt teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (ShellGPT cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

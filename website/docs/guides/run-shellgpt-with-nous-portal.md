...
3. Sets `model.provider: nous` in `~/.shellgpt/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where ShellGPT is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
shellgpt setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
shellgpt auth add nous --type oauth
# Then re-run `shellgpt setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "ShellGPT Agent release notes" and summarize the top 3 hits.
```

You should see ShellGPT call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`shellgpt setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
shellgpt config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick ShellGPT-4 for agent work

ShellGPT-4-70B and ShellGPT-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](../user-guide/features/subscription-proxy.md) from non-agent tools. For ShellGPT Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a ShellGPT-side opinion.
...
These rows appear in `shellgpt tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, ShellGPT runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
shellgpt portal tools
```
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and ShellGPT will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [ShellGPT profiles](../user-guide/profiles.md) (e.g. a separate config per project), each profile is an independent credential island: a profile that has never signed in to the Portal fails closed instead of adopting another profile's session. Sign in once per profile with `shellgpt -p <name> portal` — when a shared Portal session already exists on the machine it offers to import it without a browser round-trip, and from then on the shared token store keeps that profile's token current. See [Profile setup](../integrations/nous-portal.md#profile-setup).

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.shellgpt/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `shellgpt portal info` shows "not logged in" after `shellgpt setup --portal`
...
```bash
shellgpt config set model.provider nous
```
...
Re-verify with `shellgpt portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so ShellGPT doesn't replay it endlessly. Just log in again:

```bash
shellgpt auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/shellgpt-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`shellgpt portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple ShellGPT profiles where you're using the wrong one (check `shellgpt profile list`)

### Want to revoke and start clean

```bash
shellgpt auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](../user-guide/features/subscription-proxy.md)** — Use your Portal subscription from non-ShellGPT tools
...
- **[Profiles](../user-guide/profiles.md)** — Share one Portal login across multiple ShellGPT configurations

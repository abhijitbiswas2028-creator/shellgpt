...
| Related skills | [`claude-code`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code.md), [`shellgpt-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-shellgpt-agent.md) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the ShellGPT terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For ShellGPT itself, `model.provider: openai-codex` uses ShellGPT-managed Codex
OAuth from `~/.shellgpt/auth.json` after `shellgpt auth add openai-codex`. For the
...
## ShellGPT Gateway Caveat

When invoking the Codex CLI from a ShellGPT gateway/service context (for example,
...
terminal(command="codex --sandbox workspace-write exec 'Fix issue #78: <description>. Commit when done.'", workdir="~/.shellgpt/cache/scratch/issue-78", background=true, pty=true)
terminal(command="codex --sandbox workspace-write exec 'Fix issue #99: <description>. Commit when done.'", workdir="~/.shellgpt/cache/scratch/issue-99", background=true, pty=true)

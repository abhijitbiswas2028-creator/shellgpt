...
| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the ShellGPT MCP catalog. The
...
Two halves, in this order: the editor side must be up before ShellGPT connects.
...
### One-time, ShellGPT side

    shellgpt mcp install unreal-engine
...
`~/.shellgpt/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for ShellGPT — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. ShellGPT connects
...
2. Start the ShellGPT session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through ShellGPT
they appear as:

| ShellGPT tool | Purpose |
|---|---|
...
then happens at `shellgpt mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — ShellGPT runs batched calls
...
- **Start order matters.** Editor + server up first, then the ShellGPT
...
- **Timeouts on long operations.** ShellGPT' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.shellgpt/config.yaml` for

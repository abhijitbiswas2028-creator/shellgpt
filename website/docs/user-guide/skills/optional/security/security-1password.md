...
| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by ShellGPT Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during ShellGPT terminal calls (desktop app flow only)
...
### Service Account (recommended for ShellGPT)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SHELLGPT_HOME:-~/.shellgpt}/.env` (the skill will prompt for this on first load).
...
## ShellGPT Execution Pattern (desktop app flow)

ShellGPT terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-${SHELLGPT_HOME:-$HOME/.shellgpt}/cache/scratch}/shellgpt-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/shellgpt-op.sock"

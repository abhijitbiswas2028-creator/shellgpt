...
| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SHELLGPT_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SHELLGPT_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `shellgpt`)    |

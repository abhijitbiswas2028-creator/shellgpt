...
| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), ShellGPT Agent |
| License | MIT |
...
import upstream classes into ShellGPT itself.
...
```bash
mkdir -p ~/.shellgpt/cache/darwinian-evolver && cd ~/.shellgpt/cache/darwinian-evolver
...
uv run darwinian_evolver parrot \
  ...
  --output_dir ~/.shellgpt/cache/scratch/parrot_demo
```

Outputs:
- `~/.shellgpt/cache/scratch/parrot_demo/snapshots/iteration_N.pkl` — pickled population per iteration
- `~/.shellgpt/cache/scratch/parrot_demo/<jsonl>` — per-iteration JSON log (path printed at end)

Open `~/.shellgpt/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.shellgpt/skills/research/darwinian-evolver
DE_DIR=~/.shellgpt/cache/darwinian-evolver/darwinian_evolver

cd "$DE_DIR" && \
  EVOLVER_MODEL='openai/gpt-4o-mini' \
  uv run --with openai python "$SKILL_DIR/scripts/parrot_openrouter.py" \
    --num_iterations 3 --num_parents_per_iteration 2 \
    --output_dir ~/.shellgpt/cache/scratch/parrot_or
```
...
uv run --with openai python "$SKILL_DIR/scripts/show_snapshot.py" \
  ~/.shellgpt/cache/scratch/parrot_or/snapshots/iteration_3.pkl
```
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside ShellGPT core.
   Custom driver scripts under `~/.shellgpt/skills/...` are user-side and fine.

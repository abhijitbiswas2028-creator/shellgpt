---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into ShellGPT"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into ShellGPT"
---
...
Import an OpenClaw setup (memories, skills) into ShellGPT.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | ShellGPT Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `ShellGPT`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> ShellGPT Migration

Use this skill when a user wants to move their OpenClaw setup into ShellGPT Agent with minimal manual cleanup.
...
```bash
shellgpt claw migrate              # Full interactive migration
...
shellgpt claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `shellgpt setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_shellgpt.py` to:

- import `SOUL.md` into the ShellGPT home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into ShellGPT memory entries
- merge OpenClaw command approval patterns into ShellGPT `command_allowlist`
- migrate ShellGPT-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to ShellGPT working-directory configuration
- copy OpenClaw skills into `~/.shellgpt/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen ShellGPT workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.shellgpt/tts/`
- archive non-secret docs that do not have a direct ShellGPT destination
...
- `scripts/openclaw_to_shellgpt.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py`

Do not guess a shorter path like `~/.shellgpt/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.shellgpt/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of ShellGPT-compatible secrets, currently:
...
ShellGPT CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in ShellGPT. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a ShellGPT workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing ShellGPT skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.shellgpt/skills/migration/openclaw-migration/scripts/openclaw_to_shellgpt.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of ShellGPT unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty ShellGPT targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean ShellGPT destination. Unsupported auth blobs must still be reported as skipped.
...
- ShellGPT persona state imported
- ShellGPT memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.shellgpt/skills/openclaw-imports/`

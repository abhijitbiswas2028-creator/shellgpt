...
| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/mcp/fastmcp` |
...
| Related skills | [`shellgpt-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-shellgpt-agent.md), [`mcporter`](../../optional/mcp/mcp-mcporter.md) |
...
- smoke-test a server with the FastMCP CLI before wiring it into ShellGPT or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to ShellGPT. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.shellgpt/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.shellgpt/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is ShellGPT integration, either:

- configure the server in `~/.shellgpt/config.yaml` using the `native-mcp` skill, or
...
### ShellGPT cannot see the deployed server

The server-building part may be correct while the ShellGPT config is not. Load the `native-mcp` skill and configure the server in `~/.shellgpt/config.yaml`, then restart ShellGPT.

...
| | |
|---|---|
| Source | Optional — install with `shellgpt skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp/mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), ShellGPT Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`shellgpt-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-shellgpt-agent.md), [`mcporter`](../../optional/mcp/mcp-mcporter.md), [`fastmcp`](../../optional/mcp/mcp-fastmcp.md) |
...
# MCP OAuth on a Remote ShellGPT Gateway

## Overview

ShellGPT' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the ShellGPT process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when ShellGPT runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches ShellGPT.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files ShellGPT' token storage expects, so a subsequent `/reload-mcp` finds
...
2. ShellGPT is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI ShellGPT** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
ShellGPT' native MCP OAuth client (`tools/mcp_oauth.py`):

1. Picks a free local port `P`.
2. Registers a dynamic OAuth client with the AS, sending `redirect_uri = http://127.0.0.1:P/callback`.
3. Starts an HTTP server on `127.0.0.1:P` **inside the ShellGPT process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When ShellGPT runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the ShellGPT process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the shellgpt user, an
empty or missing tokens directory (`$SHELLGPT_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When ShellGPT detects a remote session it prints two
...
Both require an interactive terminal to the ShellGPT host. The rest of this skill
is for when there is NO interactive TTY — ShellGPT running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the ShellGPT Dashboard (try this BEFORE manual token surgery)

A remote ShellGPT gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `shellgpt dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'shellgpt dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "SHELLGPT_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`SHELLGPT_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$SHELLGPT_HOME/.env`. Those
...
ShellGPT' `ShellGPTTokenStorage` would have written, so on `/reload-mcp` ShellGPT finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "SHELLGPT|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if ShellGPT' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find SHELLGPT_HOME and the config path

```bash
SHELLGPT_HOME=$(python3 -c 'from shellgpt_constants import get_shellgpt_home; print(get_shellgpt_home())')
echo "config: $SHELLGPT_HOME/config.yaml"
echo "tokens: $SHELLGPT_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "ShellGPT Agent (manual OAuth)",
...
**Stash `code_verifier` and `state` to disk** (e.g. `~/.shellgpt/cache/scratch/.mcp-oauth-work/<server>.json`,
...
### 8. Write tokens in ShellGPT' exact schema

`tools/mcp_oauth.py::ShellGPTTokenStorage` expects two files under
`$SHELLGPT_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "shellgpt-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though ShellGPT (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, ShellGPT sees `auth: oauth`, calls `ShellGPTTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the ShellGPT process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** ShellGPT needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
13. **Security: the stash file contains the `code_verifier`.** Delete `~/.shellgpt/cache/scratch/.mcp-oauth-work/<server>.json` immediately after successful token exchange. There's no reason to keep a proof-of-identity secret around once it's consumed.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (ShellGPT does natively).
...
- **Don't claim ShellGPT doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in ShellGPT. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of ShellGPT' config.

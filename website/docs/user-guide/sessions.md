...
ShellGPT Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.shellgpt/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
ShellGPT stores session history so it can resume conversations, but it does not
...
ShellGPT explicitly injects for that turn.
...
For example, if a user sends an image and asks ShellGPT to make a meme from it,
ShellGPT may inspect that image once with vision and run an image-processing
...
`shellgpt sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `shellgpt sessions optimize` merges FTS5 index segments and
VACUUMs the database without touching any session data. Both `optimize` and `prune` refuse
while another ShellGPT process (gateway, Desktop, dashboard, cron) holds `state.db` — stop it
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`shellgpt` or `shellgpt chat`) |
| `oneshot` | Finite non-interactive runs: `shellgpt chat --oneshot -q`, `-Q`, `shellgpt -z`, and `-q` on non-TTY stdio. Hidden from the TUI, Desktop and dashboard session pickers (like `kanban` and `tool`), even when launched from inside a TUI or Desktop session — the run inherits that transport's environment but is not that conversation. Still counts as CLI history: `shellgpt -c` / `--resume latest` continue the last one-shot, and `shellgpt sessions list` shows it. An explicit `--source <tag>` always wins (`shellgpt chat -q --source tui` is stored as `tui`). |
...
shellgpt -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
shellgpt -c "my project"   # → resumes "my project #3"
```
...
shellgpt --resume 20250305_091523_a1b2c3d4
shellgpt -r 20250305_091523_a1b2c3d4

# Resume by title
shellgpt --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
shellgpt --resume latest

# Or with the chat subcommand
shellgpt chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `shellgpt sessions list`.
...
shellgpt --resume latest --in ./my-project

# Works with the TUI too
shellgpt --tui --resume latest --in ./my-project
```
...
```bash
shellgpt --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`shellgpt sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
shellgpt sessions list --workspace my-project
shellgpt sessions list --workspace ~/code/shellgpt-agent
```

### Conversation Recap on Resume

When you resume a session, ShellGPT displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a ShellGPT session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.shellgpt/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `shellgpt -r "<title>"` from the shell) and pick up where the platform left off.
...
ShellGPT automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `shellgpt sessions list` or `shellgpt sessions browse`.
...
```bash
shellgpt sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), ShellGPT creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`shellgpt -c "my project"`), it automatically picks the most recent session in the lineage.
...
ShellGPT provides a full set of session management commands via `shellgpt sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
shellgpt sessions list

# Filter by platform
shellgpt sessions list --source telegram

# Show more sessions
shellgpt sessions list --limit 50
```
...
`shellgpt sessions export` is one surface for every export format, selected with `--format`:
...
shellgpt sessions export backup.jsonl

# Export sessions from a specific platform
shellgpt sessions export telegram-history.jsonl --source telegram

# Export a single session
shellgpt sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Point at a directory (existing, or ending in /) and the file is named for you:
# ~/exports/shellgpt_session_20250305_091523_a1b2c3d4.jsonl
shellgpt sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
shellgpt sessions export backup.jsonl --redact
```

Exported files contain one JSON object per line with full session metadata and all messages.

Each record also carries a `timings` block derived from the message timestamps, so a reader of an export attached to a bug report can tell a single long model gap from many small tool round-trips without reconstructing it by hand. It holds only ids, roles, counts and durations — `wall_clock_ms`, `largest_gap_ms`, `role_counts`, `tool_calls_emitted` and per-message `intervals` — never prompt text, tool arguments or results, so it survives `--redact` unchanged. ShellGPT does not persist a model/tool stopwatch, so `complete` is always `false`; when a session has no timestamped messages, `available` is `false` and `unavailable_reason` says why. The block is rebuilt on every export and ignored (and not counted toward size limits) on import.
...
shellgpt sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
shellgpt sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
shellgpt sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
shellgpt sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `shellgpt-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
shellgpt sessions export --format trace

# One session to a local trace file
shellgpt sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
shellgpt sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.shellgpt/session-exports`).

```bash
# Export one session to Markdown
shellgpt sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
shellgpt sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
shellgpt sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
shellgpt sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
shellgpt sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
shellgpt sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
shellgpt sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
shellgpt sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
shellgpt sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
shellgpt sessions pin 20250305_091523_a1b2c3d4
shellgpt sessions pin 20250305 20250306

# Remove the pin
shellgpt sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
shellgpt sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
shellgpt sessions pinned --json > pinned-sessions.json
```
...
shellgpt sessions prune

# Custom age threshold — bare numbers are days
shellgpt sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
shellgpt sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
shellgpt sessions prune --newer-than 5h

# Explicit window with absolute timestamps
shellgpt sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
shellgpt sessions prune --source telegram
shellgpt sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
shellgpt sessions prune --newer-than 5h --title "smoke test"   # title substring
shellgpt sessions prune --older-than 30 --max-messages 3        # tiny sessions
shellgpt sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
shellgpt sessions prune --model gpt-5 --older-than 1w           # by model (substring)
shellgpt sessions prune --provider openrouter --older-than 60   # by billing provider
shellgpt sessions prune --branch feature/old-experiment         # by git branch
shellgpt sessions prune --user 12345678 --chat-type group       # by messaging origin
shellgpt sessions prune --max-tokens 500 --older-than 7         # by token usage
shellgpt sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
shellgpt sessions prune --newer-than 5h --dry-run

# Skip confirmation
shellgpt sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `shellgpt sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `shellgpt sessions prune` keeps the 90-day cutoff. Every
...
`shellgpt sessions archive` takes the same filters as `prune` but soft-hides
...
shellgpt sessions archive --newer-than 5h

# Archive by title substring, preview first
shellgpt sessions archive --title "dry run" --dry-run
shellgpt sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `shellgpt sessions archive` refuses to
...
`shellgpt sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
shellgpt sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`shellgpt insights`](../reference/cli-commands.md#shellgpt-insights).
...
`shellgpt sessions repair-routing` finds message-bearing session rows with no
...
shellgpt sessions repair-routing
...
shellgpt sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
shellgpt sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.shellgpt/state.db ~/.shellgpt/state.db.bak`).
...
profile. Current versions put new state in the right place; `shellgpt sessions
repair-profiles` settles what is already crossed.

```bash
# Report only — every store is scanned, nothing is written
shellgpt sessions repair-profiles

# Perform the repairs (stop the gateway first; a snapshot of every store is taken)
shellgpt sessions repair-profiles --apply

# Machine-readable report
shellgpt sessions repair-profiles --json
```
...
`shellgpt profile migrate-identity <old> <new>`), and `agent:main:…` rows inside
...
`database.journal_mode: delete` only applies to databases ShellGPT creates. An
...
commits, and a downgrade underneath them destroys those commits — so ShellGPT
...
shellgpt sessions set-journal-mode delete     # WAL -> rollback journal
shellgpt sessions set-journal-mode wal        # back to WAL
shellgpt sessions set-journal-mode delete --db ~/.shellgpt/kanban.db   # another ShellGPT store
```
...
quiet; `--force` waives only that case after you have stopped every ShellGPT
...
Started a conversation in another agent CLI? You can pull it into ShellGPT and
continue it here. ShellGPT reads Claude Code's session logs
...
shellgpt sessions import

# Limit to one tool, or point at a specific file
shellgpt sessions import --from claude
shellgpt sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
shellgpt --resume @claude
shellgpt --resume @codex
```

`shellgpt sessions import` creates a new ShellGPT session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `shellgpt --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**ShellGPT Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in ShellGPT** copies the conversation into the
...
When ShellGPT cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, ShellGPT uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to ShellGPT in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.shellgpt/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.shellgpt/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.shellgpt/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.shellgpt/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.shellgpt/sessions/sessions.json` is a **legacy mirror** of it,
...
`shellgpt sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.shellgpt/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `shellgpt sessions list`, the cause is
`state.db` not receiving them — run `shellgpt sessions repair` and watch for a
...
`*.jsonl` files in `~/.shellgpt/sessions/`. They are no longer written or
read by ShellGPT. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every ShellGPT process in the same `SHELLGPT_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.shellgpt/config.yaml`:
...
`shellgpt sessions recover` synthesizes for orphaned messages) whose last
...
`in its tip segment`). `shellgpt sessions export` still works for such sessions.
...
shellgpt sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `shellgpt sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::

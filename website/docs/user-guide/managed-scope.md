...
`~/.shellgpt/config.yaml`, `~/.shellgpt/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/shellgpt`:

```text
/etc/shellgpt/
├── config.yaml     # managed config layer (wins over ~/.shellgpt/config.yaml)
└── .env            # managed env layer (wins over ~/.shellgpt/.env + shell)
```
...
The location can be relocated with the `SHELLGPT_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SHELLGPT_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by ShellGPT.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SHELLGPT_MANAGED_DIR=/opt/org/shellgpt-policy
```

:::warning
A user who can set `SHELLGPT_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `shellgpt doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/shellgpt/config.yaml` (managed) | `/etc/shellgpt/.env` (managed) |
| 2 | `~/.shellgpt/config.yaml` (user) | `~/.shellgpt/.env` (user) |
...
```bash
shellgpt config        # shows a header naming the managed source + the pinned keys
shellgpt doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, ShellGPT refuses and names the source:

```bash
$ shellgpt config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/shellgpt/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `shellgpt config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/shellgpt

# Pin some config values for every user on this machine
sudo tee /etc/shellgpt/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/shellgpt/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/shellgpt
sudo chmod 0644 /etc/shellgpt/config.yaml /etc/shellgpt/.env
```

Changes take effect on the next ShellGPT start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`shellgpt doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs ShellGPT as `root`), managed scope is advisory.

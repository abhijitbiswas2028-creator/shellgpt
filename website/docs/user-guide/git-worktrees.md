...
title: "Git Worktrees"
description: "Run multiple ShellGPT agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ShellGPT Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with ShellGPT so each session has a clean, isolated working directory.

## Why Use Worktrees with ShellGPT?

ShellGPT treats the **current working directory** as the project root:

- CLI: the directory where you run `shellgpt` or `shellgpt chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.shellgpt/config.yaml`
...
ShellGPT creates `.worktrees/my-experiment/` inside the repo (branch
`shellgpt/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `shellgpt-<id>`
...
git worktree add ../repo-feature feature/shellgpt-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/shellgpt-experiment` checked out in that directory

Now you can `cd` into the new worktree and run ShellGPT there:

```bash
cd ../repo-feature

# Start ShellGPT in the worktree
shellgpt
```
...
git worktree add ../repo-experiment-a feature/shellgpt-a
git worktree add ../repo-experiment-b feature/shellgpt-b
```
...
Each ShellGPT process:

- Works on its own branch (`feature/shellgpt-a` vs `feature/shellgpt-b`).
...
- ShellGPT checkpoint data under `~/.shellgpt/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per ShellGPT experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/shellgpt-checkpoints-docs`, `feature/shellgpt-refactor-tests`.
- **Commit frequently**
...
- **Avoid running ShellGPT from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `shellgpt -w` (Automatic Worktree Mode)

ShellGPT has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `shellgpt/shellgpt-<hash>`).
...
```bash
shellgpt -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `shellgpt -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each ShellGPT session its own clean checkout.

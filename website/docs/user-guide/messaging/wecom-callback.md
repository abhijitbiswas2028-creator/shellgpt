...
Connect ShellGPT to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
ShellGPT supports two WeCom integration modes:
...
> Run `shellgpt gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. ShellGPT decrypts the message, queues it for the agent
...
(Use `shellgpt gateway start` only after `shellgpt gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in ShellGPT and the token the
...
in `~/.shellgpt/.env` values around `=` will also break signature checks. After
fixing, restart `shellgpt gateway run`.
...
Check `shellgpt gateway run` logs for the bound host/port. If the adapter bound to

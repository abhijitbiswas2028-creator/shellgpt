---
sidebar_position: 30
title: "ShellGPT Relay"
description: "Connect ShellGPT to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# ShellGPT Relay (Connector)
...
ShellGPT Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.shellgpt/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.shellgpt/.env`:
...
`shellgpt gateway enroll` refuses to run in managed/hosted installs — there the
...
`shellgpt gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.shellgpt/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `shellgpt auth add nous` (or `shellgpt setup`) and retry.
...
startup. Restart it (`shellgpt gateway restart`).

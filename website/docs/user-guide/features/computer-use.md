...
ShellGPT Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended ShellGPT integration. It
...
**Fresh installs already have the driver.** The ShellGPT installer
...
- **`shellgpt tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
shellgpt computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `shellgpt computer-use
status` to verify the install.

Already have cua-driver? ShellGPT reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `shellgpt update`, and the first
`computer_use` call of a session, ShellGPT checks the local version and
...
selected with `SHELLGPT_CUA_DRIVER_CMD` stays
under your control, so ShellGPT reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. ShellGPT autodetection is a
planned cua-driver follow-up, so currently point ShellGPT at that directory or
...
interface. The built-in toolset provides ShellGPT actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `shellgpt computer-use doctor` (CuaDriver, `com.trycua.driver`, in every permission mode — the driver daemon always launches through `CuaDriver.app`). |
...
```
shellgpt -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.shellgpt/config.yaml`.

## Permission modes and logged-in browser profiles

ShellGPT maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| ShellGPT session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal ShellGPT approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit ShellGPT risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.shellgpt/cua-manifest.yaml
```
...
for the format). ShellGPT launches a private runtime with
...
identity instead of resetting with every ShellGPT build), and ShellGPT verifies
...
resetting or closing the ShellGPT session, cancellation cleanup, or process exit
closes that transport session. ShellGPT also stops private runtimes that it
launched for bounded or unrestricted access. One ShellGPT
...
## `shellgpt computer-use doctor` — your first triage stop

`shellgpt computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ shellgpt computer-use doctor
...
`shellgpt computer-use status` prints the same dead-daemon line and exits 1.
...
ShellGPT run declares a public cua-driver **session name** (something like
`shellgpt-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. ShellGPT disables it automatically where it is a known failure
...
ShellGPT keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the ShellGPT-side `computer_use` workflow and action vocabulary. For
...
The command links the pack into `~/.shellgpt/skills/cua-driver` (ShellGPT is one
...
These are **platform deep dives, not duplicates of the ShellGPT skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **ShellGPT
...
image capture also saves a bounded, shareable copy under ShellGPT' image cache
...
ShellGPT applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.shellgpt/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. ShellGPT applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  ShellGPT agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the ShellGPT agent itself at High integrity (launch from an
  elevated terminal); otherwise target non-elevated windows.
- **Windows: `shellgpt computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the ShellGPT venv
  ...
  `SHELLGPT_CUA_DRIVER_CMD` to a copy outside `WindowsApps`.
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. ShellGPT warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive ShellGPT from inside the RDP / console
  ...
  - **Linux** requires a reachable display server. Headless servers
    ...
    desktop over TigerVNC, streamed into ShellGPT Desktop, where you can
    take over for logins and 2FA. You start it from the Desktop's
    Screen pane or `shellgpt computer-use screen start`; it starts on
...
On Linux, native Wayland support remains an explicit opt-in. ShellGPT passes the
...
```
SHELLGPT_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SHELLGPT_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **ShellGPT disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) ShellGPT sets
...
When it's on, `shellgpt computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point ShellGPT at a binary you built from source instead
of the published release. ShellGPT resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SHELLGPT_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for ShellGPT testing (see notes).
...
### Option B — point ShellGPT straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SHELLGPT_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SHELLGPT_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SHELLGPT_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm ShellGPT is using your build

- `shellgpt computer-use status` prints the resolved binary path and
  version.
- `shellgpt computer-use doctor` confirms the binary is reachable and
...
- **ShellGPT spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  ShellGPT YOLO, ShellGPT instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no ShellGPT
  change needed either way.
- **Local builds skip the version check.** ShellGPT warns when the
...
**First action when anything's off: run `shellgpt computer-use doctor`.**
...
Run `shellgpt computer-use install` to fetch the cua-driver binary, or
run `shellgpt tools` and enable the Computer Use toolset.

**Clicks seem to have no effect** — Capture and verify. A modal you
didn't see may be blocking input. Dismiss it with `escape` or the close
button.

**macOS: System Settings shows CuaDriver ON, but `shellgpt computer-use
...
shellgpt computer-use permissions grant
```
...
Wayland without an XWayland bridge. `shellgpt computer-use doctor` will
...
- **ShellGPT-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  ShellGPT `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. ShellGPT autodetection is a
  planned follow-up; currently point ShellGPT at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (ShellGPT among them)

...
description: "Plugins shipped with ShellGPT Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

ShellGPT ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.shellgpt/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](./plugins.md) page for the general plugin system, and [Build a ShellGPT Plugin](../../developer-guide/plugins/index.md) to write your own.
...
2. **User** — `~/.shellgpt/plugins/<name>/`
3. **Project** — `./.shellgpt/plugins/<name>/` (requires `SHELLGPT_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `shellgpt_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `shellgpt memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `shellgpt plugins list` and the interactive `shellgpt plugins` UI), but none load until you explicitly enable them:

```bash
shellgpt plugins enable disk-cleanup
```

Or via `~/.shellgpt/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer ShellGPT. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
shellgpt plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `shellgpt plugins enable <name>`.
...
| `shellgpt-achievements` | dashboard tab | Steam-style collectible badges generated from your real ShellGPT session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `shellgpt memory` and `shellgpt plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SHELLGPT_HOME` or `/tmp/shellgpt-*`, track it silently as `test` / `temp` / `cron-output`. | <!-- no-tmp: ok — documents the plugin's scope guard -->
...
**State** — everything lives at `$SHELLGPT_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SHELLGPT_HOME` or `/tmp/shellgpt-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end. User project trees (`workspace/`, `projects/`, `plans/`, `home/`) are never tracked or swept at all: a `test_*.py` or `tmp_*` file inside your project is source code, not scratch. `kanban/` (task attachments and workspaces) is never tracked either, and a tracked *directory* under a protected top level such as `cache/` is never removed — only the files inside it age out.

**Enabling:** `shellgpt plugins enable disk-cleanup` (or check the box in `shellgpt plugins`).

**Disabling again:** `shellgpt plugins disable disk-cleanup`.
...
**Enabling:** `shellgpt plugins enable security-guidance` (or check the box in `shellgpt plugins`).

**Disabling again:** `shellgpt plugins disable security-guidance`.
...
Traces ShellGPT turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of ShellGPT' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `shellgpt logs`.
...
```bash
shellgpt tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart ShellGPT and the next turn ships a trace.
...
Then put the credentials in `~/.shellgpt/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "ShellGPT turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `SHELLGPT_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the ShellGPT session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `shellgpt chat` session lives under one Langfuse session.

**Verify:**

```bash
shellgpt plugins list                 # observability/langfuse should show "enabled"
shellgpt chat -q "hello"              # check the Langfuse UI for a "ShellGPT turn" trace
```
...
| `SHELLGPT_LANGFUSE_RELEASE` | — | Release/version tag |
| `SHELLGPT_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `SHELLGPT_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `SHELLGPT_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

ShellGPT-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — ShellGPT-prefixed wins when both are set.
...
**Disabling:** `shellgpt plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled ShellGPT plugin. Do not run `shellgpt plugins enable observability/nemo_relay`; ShellGPT core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `SHELLGPT_NEMO_RELAY_PLUGINS_TOML` to that file before starting ShellGPT. The policy is process-wide for every profile hosted by that ShellGPT process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `SHELLGPT_NEMO_RELAY_ATOF_*` and `SHELLGPT_NEMO_RELAY_ATIF_*` settings no longer activate exporters — a `.env` that still carries them (and no `SHELLGPT_NEMO_RELAY_PLUGINS_TOML`) exports **nothing**, and the gateway logs one warning saying so. `shellgpt doctor` reports these stale settings when no replacement `plugins.toml` is selected.

**Automatic migration.** `shellgpt update` (and `shellgpt migrate relay`, or `shellgpt migrate relay --all-profiles` for every profile home) converts the legacy variables into `<shellgpt home>/relay-plugins.toml`, sets `SHELLGPT_NEMO_RELAY_PLUGINS_TOML` in that profile's `.env`, and comments the legacy lines out (nothing is deleted). Under a multiplexed gateway every profile home gets its own file. The generated file is validated through Relay before it is written; this is the shape it produces (note the `type = "file"` sink discriminator — a sink without it is rejected):
...
type = "file"
output_directory = "/home/you/.shellgpt/telemetry/nemo-relay/atof"
filename = "shellgpt-atof.jsonl"
mode = "append"

[components.config.atif]
enabled = true
agent_name = "ShellGPT Agent"
model_name = "unknown"
output_directory = "/home/you/.shellgpt/telemetry/nemo-relay/atif"
...
Then add `SHELLGPT_NEMO_RELAY_PLUGINS_TOML=/home/you/.shellgpt/relay-plugins.toml` to `.env` and restart the gateway.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `shellgpt.session.segment` plus `shellgpt.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.shellgpt/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
shellgpt plugins enable google_meet
shellgpt meet setup   # preflight: playwright, chromium, auth file
shellgpt meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `shellgpt plugins disable google_meet`. Any saved transcripts stay in `~/.shellgpt/workspace/meetings/` until you remove them.

### shellgpt-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real ShellGPT session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with ShellGPT feature changes.

**How it works:**

- Scans your entire `~/.shellgpt/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SHELLGPT_HOME/plugins/shellgpt-achievements/state.json`
...
| Secret | Hidden until ShellGPT detects the first related signal in your history |

**API** — routes mount under `/api/plugins/shellgpt-achievements/`:
...
**State files** — live under `$SHELLGPT_HOME/plugins/shellgpt-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across ShellGPT updates. |
...
**Enabling:** Nothing to enable — `shellgpt-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `shellgpt dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/shellgpt-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.shellgpt/plugins/shellgpt-achievements/` that ships no dashboard. The plugin's state files under `$SHELLGPT_HOME/plugins/shellgpt-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other ShellGPT plugin — see [Build a ShellGPT Plugin](../../developer-guide/plugins/index.md). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.shellgpt/plugins/<name>/`
- Manifest source is reported as `bundled` in `shellgpt plugins list`

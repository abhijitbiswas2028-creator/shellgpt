...
chat completions — use your ShellGPT-managed provider subscription as their
...
| Use case | "Use ShellGPT as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. ShellGPT stores
the refresh token in `~/.shellgpt/auth.json` — the same place all ShellGPT
...
Model:      ShellGPT-4-70B    # or ShellGPT-4.3-36B, ShellGPT-4-405B
```
...
interface in `shellgpt_cli/proxy/adapters/`.
...
If you see `not logged in`, run `shellgpt portal`. If you see
...
`shellgpt_cli/proxy/adapters/<provider>.py` and registering it in

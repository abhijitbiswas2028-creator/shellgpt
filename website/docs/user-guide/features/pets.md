...
ShellGPT can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SHELLGPT_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), ShellGPT renders the sprite at full fidelity when your
...
shellgpt pets list
shellgpt pets list cat

# Install a pet and make it active in one step
shellgpt pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
shellgpt pets show

# Check your setup
shellgpt pets doctor
```

## `shellgpt pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `shellgpt pets list [query] [--limit N]` |
| List installed pets | `shellgpt pets list --installed` |
| Install a pet | `shellgpt pets install <slug> [--select] [--force]` |
...
| Preview/animate | `shellgpt pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `shellgpt pets off` |
| Remove an installed pet | `shellgpt pets remove <slug>` |
| Diagnose setup | `shellgpt pets doctor` |

`shellgpt pets show` flags:
...
Beyond installing pre-made pets from the gallery, ShellGPT can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `shellgpt pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `shellgpt tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SHELLGPT_PET_IMAGE_PROVIDER` env var (e.g. `SHELLGPT_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while ShellGPT is
...
Run `shellgpt pets doctor` — it reports:
...
- The [`shellgpt-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-shellgpt-agent.md)

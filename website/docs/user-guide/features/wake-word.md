...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey ShellGPT' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey ShellGPT")

The wake word turns ShellGPT into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, ShellGPT listens in the background for a spoken
trigger phrase. Say it, and ShellGPT starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** ShellGPT backend (for example a
...
ShellGPT supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey shellgpt"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey shellgpt"** — a model for it ships with ShellGPT, so
...
```bash
cd ~/.shellgpt/shellgpt-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `shellgpt` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.shellgpt/config.yaml`,
...
wake_word:
  ...
  phrase: "hey shellgpt"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_shellgpt         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey shellgpt" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey shellgpt" utterances are missed.
...
default) to let ShellGPT choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which ShellGPT installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three ShellGPT surfaces, and `surface` picks which one
...
| `cli` | Only the classic `shellgpt` CLI. |
| `tui` | Only `shellgpt --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when ShellGPT surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. ShellGPT does not silently fail over to another open surface.
...
"Hey ShellGPT" works out of the box — the bundled openWakeWord model
(`model: hey_shellgpt`) is the default. To wake on something else, the easiest
...
command (`shellgpt -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.shellgpt/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("shellgpt" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey ShellGPT" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey shellgpt"
  porcupine:
    keyword: ~/.shellgpt/wakewords/hey_shellgpt.ppn
```

Set your access key in `~/.shellgpt/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `shellgpt tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `shellgpt-agent[wake]`).
...
but the phrase never fires. ShellGPT detects this (`/wake status` shows
"mic delivers only silence"; the desktop's folded voice menu carries the same
hint on its trigger).
Fix: System Settings → Privacy & Security → Microphone → enable the ShellGPT
backend (it may appear as your terminal, `python`, or ShellGPT), then toggle the
...
```bash
shellgpt config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
shellgpt config set wake_word.input_device null
```

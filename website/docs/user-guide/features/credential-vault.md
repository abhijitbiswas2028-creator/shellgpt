...
fills the password through ShellGPT. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): ShellGPT generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `shellgpt vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, ShellGPT
...
in, ShellGPT picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). ShellGPT hands the master password to the manager's
...
Prefer not to use a detected manager? `shellgpt vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `shellgpt vault add`), bound to the checkout site,
...
- **CLI**: `shellgpt vault list`, `shellgpt vault add`, `shellgpt vault rm <handle>`,
  `shellgpt vault sources`.

Items live encrypted under `~/.shellgpt/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `shellgpt chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through ShellGPT: not in

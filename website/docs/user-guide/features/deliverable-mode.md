...
When ShellGPT Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
2. **The gateway scans agent responses for file paths.** Any absolute path
   (`~/.shellgpt/cache/scratch/...`) or home-relative path (`~/...`) ending in a supported
...
persona in `~/.shellgpt/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.shellgpt/config.yaml` (switchable per session
via `/personality`).

The mechanic the agent has to use is simple: render the file to an
absolute path (e.g. `~/.shellgpt/cache/scratch/q3-revenue.png`) and mention that path as
...
If you use ShellGPT' kanban multi-agent workflow, workers can attach
...
kanban_complete(
    ...
    artifacts=[
        "~/.shellgpt/cache/scratch/q3-revenue.png",
        "~/.shellgpt/cache/scratch/q3-report.pdf",
    ],
)
```
...
Install MCP servers via `~/.shellgpt/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. ShellGPT Agent's deliverable

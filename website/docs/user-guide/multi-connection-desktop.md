...
# Connecting Desktop to Many ShellGPT Instances

Register every ShellGPT backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and ShellGPT Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another ShellGPT gateway…"**) deep-links straight to
...
list of ShellGPT gateways. Its intro says it plainly: *"Manage this device and
every ShellGPT gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The ShellGPT runtime managed by this app." | automatic |
| **Remote gateway** | "A ShellGPT gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A ShellGPT install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **ShellGPT Cloud** | "A hosted instance discovered through your ShellGPT Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the ShellGPT Cloud sign-in/discovery flow at
  the top of the Gateways page — the **ShellGPT Cloud** kind in the add-connection
...
The ShellGPT Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **ShellGPT Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **ShellGPT Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `shellgpt serve` backend,
   ...
   - *SSH only:*
     ...
     - **ShellGPT path (optional)** — full path to the `shellgpt` executable on
       the remote (for example `/opt/shellgpt/bin/shellgpt`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `shellgpt` on its `PATH` and **Test** reports *"ShellGPT is not
...
Gateways are machines or hosted backends; profiles are isolated ShellGPT agents
...
connection is registered) dispatches `shellgpt update` to every eligible
...
- **ShellGPT Cloud** instances are skipped with a *"Managed by ShellGPT Cloud"*
...
panel, ⌘K **Update ShellGPT**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `shellgpt serve` is running on the remote host, the
...
- **"Update ShellGPT Desktop to chat with agents on other connections"** — the

---
title: "Bot Mode"
description: "Turn your ShellGPT profiles into a roster of named Bots — each with its own chat, role, model, memory, skills, and avatar. Bots run routines, share group chats, and message each other."
---

# Bot Mode

**Bot Mode** turns your [ShellGPT profiles](./profiles.md) into a roster of named **Bots**. Each Bot has its own role, model, memory, skills, and avatar; Bots run recurring routines, deliberate together in group chats, and message each other directly. Build a specialist Bot once and it is there forever, one click away.
...
There is no new primitive to learn: a Bot **is** a ShellGPT profile — isolated config, memory, skills, credentials, and chat history under `~/.shellgpt/profiles/<name>/`. Bot Mode is a UI over that primitive, so everything you do in it is visible from the CLI too: `shellgpt -p <bot> chat` opens the same agent, and Bot routines appear in `shellgpt cron list`. No core patches, no background daemons, no extra storage.
...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair ShellGPT knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile. Picking a model from the Bot Chat's composer sticks to that chat (it survives reopening the app) until you change the Bot's profile model, which takes over again.
...
- **Copy API keys from the main profile** — on by default. Each Bot gets its own credential store: static API keys are copied in, while single-use OAuth logins (Anthropic, OpenAI Codex, xAI) are not copied — sign the Bot in itself with `shellgpt -p <name> auth add <provider>`. See [Every profile owns its credentials](./profiles.md#every-profile-owns-its-credentials).
...
- **A pixel pet** — a companion from the [petdex gallery](./features/pets.md) that bounces beside the avatar while the Bot is busy. Run `shellgpt pets` in a terminal to explore the gallery.
...
- Teammates can hand off to the primary Bot with `@shellgpt`, including in older saved rooms; Bots on other gateways keep their device-qualified tags (for example, `@default-vera`).
...
1. **The session is titled exactly `Bot Chat`.** That exact title is the canonical chat's identity (it is what the desktop plugin's createCanonicalChat uses and what `shellgpt -p <bot> chat -c "Bot Chat"` resumes); any other title, or an untitled scratch session, gets neither the section nor the tool.
2. **At least one profile on the install carries a `ui_meta: { shellgpt-bots: … }` block in its `profile.yaml`.** This is the "Bot-Mode-managed" marker the desktop plugin writes for every Bot it owns; the gate scans every profile, so one marked profile marks the whole install. There is no CLI command that writes it.
...
shellgpt -p <bot> chat -c "Bot Chat" --create-if-missing
```

```yaml
# ~/.shellgpt/profiles/<any-bot>/profile.yaml — an empty block is enough to mark the install
ui_meta:
  shellgpt-bots: {}
```

From the next turn in that Bot Chat the teammate roster, the protocol section, and `message_agent` are all picked up — including over `shellgpt -p <bot> chat` on a purely terminal box.
...
the ShellGPT entrypoint beside the sending runtime's Python interpreter, so an
unrelated or older `shellgpt` on a service's `PATH` cannot take precedence when
...
Every gateway you register in **Settings → Connections** — local, remote URL, SSH, ShellGPT Cloud, docker — is a persistent line the Desktop holds open, and Bot Mode uses those lines for messaging automatically. No extra setup:
...
### Bot-initiated DMs across machines (`shellgpt peer`)
...
```bash
shellgpt peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
shellgpt peer list
shellgpt peer dm spark < ~/.shellgpt/cache/scratch/dm.txt        # message body from a file (nothing shell-interpreted)
shellgpt peer dm spark/researcher < ~/.shellgpt/cache/scratch/dm.txt   # named profile on a multiplexed peer
shellgpt peer run spark --idempotency-key ticket-123 < ~/.shellgpt/cache/scratch/long-task.txt
shellgpt peer status spark run_abc123
shellgpt peer stop spark run_abc123
```

`shellgpt peer dm` delivers into the remote agent's canonical Bot Chat over the peer's existing API server, runs one agent turn there, and prints the reply on stdout — the exact cross-machine twin of the local `shellgpt -p <bot> chat` command.
...
Requirements: the peer machine runs the `api_server` gateway platform with a strong `API_SERVER_KEY`; reachability is your network's business (LAN, Tailscale, VPN). The key is a credential and lives in `~/.shellgpt/.env` as `SHELLGPT_PEER_<NAME>_KEY`; peer names/URLs live in `config.yaml` under `bot_peers`.
...
| --- | --- |
| Chat with a Bot | `shellgpt -p <bot> chat` |
| A Bot's files, skills, memory | `~/.shellgpt/profiles/<bot>/` |
| Routines | `shellgpt cron list` (jobs named `[bot:<name>] …`) |
| Create / inspect profiles | `shellgpt profile create`, `shellgpt profile list` |

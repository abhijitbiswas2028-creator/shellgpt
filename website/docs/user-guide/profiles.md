...
Run multiple independent ShellGPT agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate ShellGPT home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. ShellGPT recognises a directory under `~/.shellgpt/profiles/` as a profile only when it carries one of those identity files (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json`, `state.db`); a bare directory left behind by logging or cron is ignored by `profile list`, gateways and `-p`. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up ShellGPT state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same ShellGPT home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](./features/memory-providers.md) instead.
:::
...
These terms describe different parts of ShellGPT:
...
- **Agent** is the running ShellGPT assistant that uses that configuration and
  state. "ShellGPT Agent" also names the product.
...
- **Messaging bot** is an account on a platform such as Telegram, Discord, or
  Slack, connected to ShellGPT through the gateway. Its
...
```bash
shellgpt profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own ShellGPT profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `shellgpt setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](../integrations/nous-portal.md).
:::

### Blank profile

```bash
shellgpt profile create mybot
```
...
```bash
shellgpt profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `shellgpt profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
shellgpt profile create backup --clone-all
```

Copies **everything** — config, API keys, personality, all memories, skills, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. When cloning from the default profile, the local-model runtime trees (`models/`, `runtimes/`, `node/` — downloaded weights and managed binaries, re-fetched on demand) are skipped too, as `shellgpt backup` already does. **Cron jobs are not cloned** either: they are scheduled work bound to the source profile and its delivery channel, and a clone that inherited them would run every job twice (two gateways, same job ids). The new profile starts with an empty `cron/`. For a full backup including history and cron jobs, use `shellgpt profile export` or `shellgpt backup` instead.

:::note OAuth logins are shared, not copied
Anthropic (Claude Pro/Max), OpenAI Codex, and xAI OAuth logins use **single-use refresh tokens** — a copy of one is not a second credential, it is the same credential with two owners, and the first profile to refresh it revokes it for every other copy. `--clone-all` (and the dashboard's credential mirroring) therefore drops those OAuth rows from the clone. The new profile keeps reading the login from the root `~/.shellgpt/auth.json`, and a token refresh performed inside any profile is written back to root, so all profiles stay signed in. Static API keys are copied as usual. To give a profile its own separate OAuth login, run `shellgpt -p <name> auth add <provider>` inside it.
:::

### Clone from a specific profile

```bash
shellgpt profile create work --clone-from coder
```
...
```bash
shellgpt profile create work-backup --clone-from coder --clone-all
```
...
parks the duplicate adapter (and `shellgpt gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`shellgpt -p <name> setup` or the dashboard Messaging page.

```bash
shellgpt profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `shellgpt profile list` prints the same warning for any existing
...
The alias works with every shellgpt subcommand — it's just `shellgpt -p <name>` under the hood.
...
```bash
shellgpt -p coder chat
shellgpt --profile=coder doctor
shellgpt chat -p coder -q "hello"    # works in any position
```

### Sticky default (`shellgpt profile use`)

```bash
shellgpt profile use coder
shellgpt chat                   # now targets coder
shellgpt tools                  # configures coder's tools
shellgpt profile use default    # switch back
```

Sets a default so plain `shellgpt` commands target that profile. Like `kubectl config use-context`.
...
- **`shellgpt profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives ShellGPT its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory ShellGPT was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.shellgpt/profiles/coder/.env

# Edit assistant's tokens
nano ~/.shellgpt/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates shellgpt-gateway-coder systemd/launchd service
assistant gateway install     # creates shellgpt-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `shellgpt profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `shellgpt -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](./docker.md#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.shellgpt/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `shellgpt profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`shellgpt update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
shellgpt profile list           # show all profiles with status
shellgpt profile show coder     # detailed info for one profile
shellgpt profile rename coder dev-bot   # rename (updates alias + service)
shellgpt profile migrate-identity coder dev-bot   # retry a rename's identity migration
shellgpt profile purge-identity dev-bot   # retry a delete's identity purge
shellgpt profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
shellgpt profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.shellgpt` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
shellgpt profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `shellgpt profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.shellgpt/profile.yaml`; remove that line to
...
```bash
shellgpt profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `shellgpt profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.shellgpt`). To remove everything, use `shellgpt uninstall`.
:::
...
```bash
# Bash
eval "$(shellgpt completion bash)"

# Zsh
eval "$(shellgpt completion zsh)"
```
...
Profiles use the `SHELLGPT_HOME` environment variable. When you run `coder chat`, the wrapper script sets `SHELLGPT_HOME=~/.shellgpt/profiles/coder` before launching shellgpt. Since 119+ files in the codebase resolve paths via `get_shellgpt_home()`, ShellGPT state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `SHELLGPT_HOME`.
...
isolated by `SHELLGPT_HOME`, not by changing `HOME`. Container backends still use
`{SHELLGPT_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `SHELLGPT_HOME` is the profile boundary. It controls ShellGPT config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other ShellGPT
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, ShellGPT keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode ShellGPT launches tool
subprocesses with `HOME={SHELLGPT_HOME}/home`; you then need to initialize or link
...
ShellGPT also exposes `SHELLGPT_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.shellgpt` itself. No migration needed — existing installs work identically.
...
shellgpt profile export coder
shellgpt profile import ./coder.tar.gz --name coder
```
...
shellgpt profile install github.com/you/research-bot --alias

# Update later when the author ships a new version (keeps your memories + .env)
shellgpt profile update research-bot
```

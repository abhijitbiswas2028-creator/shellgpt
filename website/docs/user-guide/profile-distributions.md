...
A **profile distribution** packages a complete ShellGPT agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

ShellGPT has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `shellgpt profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `shellgpt profile update` pulls new versions | Re-send the file |
...
| Made with | `shellgpt profile install` / `update` | `/export` and `/import`, or `shellgpt profile export` / `import` |
...
Before distributions, sharing a ShellGPT agent meant sending someone:
...
```bash
shellgpt profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `shellgpt profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running ShellGPT in 2026, that's already true.
...
:::caution
**ShellGPT does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `shellgpt profile install` or `shellgpt profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
shellgpt profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.shellgpt/profiles/research-bot/SOUL.md
...
Create `~/.shellgpt/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
shellgpt_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.shellgpt/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `shellgpt profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.shellgpt/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
shellgpt profile install github.com/you/research-bot

# Full HTTPS
shellgpt profile install https://github.com/you/research-bot.git

# SSH
shellgpt profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
shellgpt profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
shellgpt profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
shellgpt profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
shellgpt profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
shellgpt profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this ShellGPT distribution.
...
```bash
cp ~/.shellgpt/profiles/research-bot/.env.EXAMPLE ~/.shellgpt/profiles/research-bot/.env
...
```bash
shellgpt profile info research-bot
```
...
`shellgpt profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
shellgpt profile delete research-bot
```
...
Path:    ~/.shellgpt/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `shellgpt profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.shellgpt/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
shellgpt profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `shellgpt profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.shellgpt/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/shellgpt-polymarket-trader.git
git push -u origin main --tags

# Anyone
shellgpt profile install github.com/you/shellgpt-polymarket-trader --alias
```
...
You built ShellGPT-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
shellgpt profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
shellgpt profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `shellgpt profile info <name>` and hold off on updates until you're ready.
:::
...
shellgpt profile info research-bot | grep Version
...
```yaml
# ~/.shellgpt/profiles/research-bot/local/my-overrides.yaml
...
shellgpt profile delete research-bot --yes
shellgpt profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
shellgpt profile update research-bot --force-config --yes
```
...
shellgpt profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.shellgpt/profiles/forked-research-bot/
...
shellgpt profile install ~/.shellgpt/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
shellgpt profile delete research-bot-test --yes
shellgpt profile install ~/.shellgpt/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in ShellGPT's managed
`profile-exports/` directory under the default ShellGPT home, not in the current
...
source file. If the ShellGPT home itself lives inside a Git checkout (some
Docker/custom deployments), the archive goes to `~/.shellgpt-profile-exports/`
...
```bash
shellgpt profile export research-bot
shellgpt profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
shellgpt profile import ./research-bot.tar.gz
shellgpt profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `shellgpt -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.shellgpt`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.shellgpt`) is exported through an allow-list — only known ShellGPT artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.shellgpt/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — newly shipped jobs are installed paused. Review them with `shellgpt -p <name> cron list` and resume only the jobs you trust. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `shellgpt profile` — not a parallel command tree.
...
- Reserved profile names (`shellgpt`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`shellgpt profile export` / `import`](../reference/profile-commands.md#shellgpt-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with ShellGPT](../guides/use-soul-with-shellgpt.md) — authoring personalities

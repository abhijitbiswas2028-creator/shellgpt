...
ShellGPT can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; ShellGPT does the rest.
...
2. Click **Install runtime**. ShellGPT downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with ShellGPT, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How ShellGPT picks what to download
...
Models ship in several quality grades (quantizations). ShellGPT picks the
...
guarantees. Below 4-bit the quality loss is too severe, so ShellGPT never
...
Local models live or die by memory placement, so ShellGPT manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, ShellGPT deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), ShellGPT uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  ShellGPT compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — ShellGPT
...
If a llama-server is already running on your machine, ShellGPT detects it
...
[Run ShellGPT Locally with Ollama](../guides/local-ollama-setup.md) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with ShellGPT.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; ShellGPT updates it with
...
Models and runtime builds live under the ShellGPT home directory

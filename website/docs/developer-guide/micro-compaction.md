...
has to be thrown away or summarized. ShellGPT has always done this in one batch:
...
ShellGPT folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. ShellGPT's normal session flush is
...
With `micro_compact` unset or `false` ShellGPT behaves exactly as it always has:
...
Defaults to `$SHELLGPT_HOME/logs/agent.log`. It reports passes, outcome mix, net

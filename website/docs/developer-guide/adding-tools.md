...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to ShellGPT Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in ShellGPT tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying ShellGPT core, use the plugin route instead:

- [Plugins](../user-guide/features/plugins.md)
- [Build a ShellGPT Plugin](./plugins/index.md)
...
2. **`toolsets.py`** — add tool name to `_SHELLGPT_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `shellgpt_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `shellgpt_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `shellgpt chat -q "Use the weather tool for London"`

...
ShellGPT runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.shellgpt/plugins/`, registering a backend the user selects
...
| `shellgpt setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `shellgpt status` / `shellgpt doctor` | `doctor_checks()` |
...
```python title="~/.shellgpt/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.shellgpt"    # where synced cache files land, or None
...
```yaml title="~/.shellgpt/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
shellgpt plugins enable acmebox
shellgpt config set terminal.backend acmebox
```

...
description: "Build wrapper CLIs that extend the ShellGPT TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

ShellGPT exposes protected extension hooks on `ShellGPTCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `shellgpt_cli/cli_tui_mixin.py` (where these hooks are defined; `ShellGPTCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends ShellGPT."""
...
Called after ShellGPT registers its own keybindings and before the layout is built. Add your keybindings to `kb`.

...
# ShellGPT Observer Hooks

ShellGPT observer hooks are the read-only telemetry contract for plugins that
...
They do not replace ShellGPT' planner, model providers, memory, tool registry,
...
ShellGPT also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "shellgpt.observer.v1"
```

Hook callbacks are fail-open. ShellGPT catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so ShellGPT only builds
...
The native NeMo Relay SDK integration maps ShellGPT session, turn, LLM, and tool

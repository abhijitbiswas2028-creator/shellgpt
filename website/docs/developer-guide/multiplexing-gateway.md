...
by `shellgpt_cli/gateway_multiplex_mode.py::resolve_multiplex_mode`, which runs
the `shellgpt gateway migrate` preflight and keeps the gateway standalone when a
...
- At startup, `GatewayRunner.__init__` calls
  ...
  `get_secret()`.
- The dashboard/Desktop backend (`shellgpt serve`) has no such flag, so
  `shellgpt_cli/web_server.py::start_server` calls
...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_shellgpt_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`SHELLGPT_HOME`, `SHELLGPT_PROFILE`, proxy settings,
...
reload helper: `shellgpt_cli.env_loader.load_shellgpt_dotenv` skips the
...
## The SHELLGPT_HOME override

`shellgpt_constants.py` holds a context-local override consulted by
`get_shellgpt_home()` before the `SHELLGPT_HOME` env var. Everything that resolves
...
automatically. `get_process_shellgpt_home()` exists for the few machine-level
assets that must not follow the override. `shellgpt_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `shellgpt_cli/profiles.py`
...
DB handles are resolved at call time through the active SHELLGPT_HOME override —
...
SHELLGPT_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `shellgpt_home_key()`. |
...
- `agent/secret_scope.py`, `shellgpt_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `shellgpt_cli/profiles.py`

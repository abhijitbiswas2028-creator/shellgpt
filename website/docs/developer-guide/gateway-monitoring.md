...
ShellGPT gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by ShellGPT's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `shellgpt.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `shellgpt.platform.up/degraded` with bounded `error_code` attributes |
...
`shellgpt.gateway.active_agents`, `shellgpt.gateway.background_work`, and
`shellgpt.gateway.background_delegations` are complementary. `active_agents`
...
```bash
shellgpt monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'shellgpt-agent[otlp]'`),
...
a plain `shellgpt gateway run`: the exporter lives in the gateway process, so
...
`shellgpt.gateway.up`, `shellgpt.platform.up`, and `shellgpt.platform.degraded`.
...
absent_over_time(shellgpt_gateway_up[5m])

# Locally owned bridge is explicitly down.
shellgpt_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
shellgpt_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
shellgpt_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
shellgpt_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(shellgpt_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `shellgpt.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
shellgpt.status = failed|unknown
shellgpt.delivery_outcome = failed|not_configured
shellgpt.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `shellgpt.job_key`;
...
ShellGPT Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log ~/.shellgpt/cache/scratch/shellgpt_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log ~/.shellgpt/cache/scratch/shellgpt_otel_capture.jsonl --wait 8
...
`shellgpt.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
shellgpt monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
  --log ~/.shellgpt/cache/scratch/cap.jsonl --wait 8          # drive the real exporter
```
...
The `shellgpt monitoring` CLI intentionally exposes `status` only. This first
release covers only ShellGPT Agent-owned service-health and operational-diagnostic
signals, including ShellGPT Agent-owned Relay transport health. Team Gateway's

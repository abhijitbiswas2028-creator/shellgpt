...
description: "Wire contract between the ShellGPT gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **ShellGPT gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `shellgpt gateway enroll --wake-url <url>` (or
...
the inbound (`scope_id`, `user_id`) and, on a multiplexed gateway, the ShellGPT
...
**per-gateway secret** (`shellgpt gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `shellgpt` CLI.

...
sidebar_position: 1
title: "Architecture"
description: "ShellGPT Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of ShellGPT Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ shellgpt_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ShellGPTCLI facade — mixins in shellgpt_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── shellgpt_state.py           # SQLite session/state database facade (+ shellgpt_state_*.py siblings)
├── shellgpt_constants.py       # SHELLGPT_HOME, profile-aware paths
...
│
├── shellgpt_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `shellgpt` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # shellgpt skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # shellgpt tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # shellgpt gateway start/stop
│
...
```text
User input → ShellGPTCLI.process_input()
...
Three discovery sources: `~/.shellgpt/plugins/` (user), `.shellgpt/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `shellgpt plugins` or `config.yaml`.
...
Exposes ShellGPT as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`shellgpt -p <name>`) gets its own SHELLGPT_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

...
ShellGPT middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing ShellGPT'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before ShellGPT calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving ShellGPT retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving ShellGPT guardrails,
...
- `telemetry_schema_version`: currently `shellgpt.observer.v1`
- `middleware_schema_version`: currently `shellgpt.middleware.v1`
...
ShellGPT stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, ShellGPT runs
them as a nested chain in registration order. Middleware failures are fail-open:
ShellGPT logs a warning and continues with the next middleware or the base
...
For each provider request, ShellGPT applies middleware in this order:
...
For each tool call, ShellGPT applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal ShellGPT pre-execution path against the effective arguments:
...
For isolated local testing, use one `SHELLGPT_HOME` for plugin enablement and the
agent run:

```bash
export SHELLGPT_HOME=$HOME/.shellgpt/cache/scratch/shellgpt-middleware-test
mkdir -p "$SHELLGPT_HOME"
shellgpt plugins enable <plugin-name>
shellgpt chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run shellgpt plugins enable <plugin-name>
uv run shellgpt chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["shellgpt_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", str(Path.home() / ".shellgpt" / "cache" / "scratch" / "shellgpt-middleware-demo"))
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape ShellGPT expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, ShellGPT treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, ShellGPT preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. ShellGPT does not convert downstream

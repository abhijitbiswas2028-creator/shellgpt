...
Plugins can launch and supervise fresh ShellGPT child sessions without importing
...
active agent turn fails closed with `No active ShellGPT parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until ShellGPT can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; ShellGPT's existing unsafe-tool block remains enforced.

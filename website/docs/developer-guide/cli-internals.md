...
sidebar_position: 15
title: "CLI Internals"
description: "How shellgpt_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `shellgpt_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `shellgpt_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `shellgpt_cli.update_cmd._shellgpt_holder_subcommand` (top-level subcommand of any ShellGPT argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `shellgpt_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.shellgpt/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

ShellGPT supports profiles — fully isolated instances, each with its own `SHELLGPT_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`shellgpt -p x <cmd>`),
`_apply_profile_override()` in `shellgpt_cli/main.py` sets `SHELLGPT_HOME` before any module imports, so
every `get_shellgpt_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["SHELLGPT_HOME"]` stays the launch
...
`Path.home() / ".shellgpt" / "profiles"`, not `get_shellgpt_home() / "profiles"`) so
`shellgpt -p coder profile list` sees all profiles regardless of which one is active — intentional.

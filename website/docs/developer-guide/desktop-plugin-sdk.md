...
title: "Desktop Plugin SDK (@shellgpt/plugin-sdk)"
description: "Extend the native ShellGPT Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [ShellGPT Desktop](../user-guide/desktop.md) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ShellGPTPlugin`.
It imports one module — `@shellgpt/plugin-sdk` — and gets everything: the app's
...
`$SHELLGPT_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across ShellGPT. This page is the **native
desktop app** (`shellgpt desktop`) SDK — the `@shellgpt/plugin-sdk` module and
`$SHELLGPT_HOME/desktop-plugins/`. The **web dashboard** (`shellgpt dashboard`) has
its own, unrelated plugin system on `window.__SHELLGPT_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](../user-guide/features/extending-the-dashboard.md). Python
CLI/gateway plugins are documented at [Build a ShellGPT Plugin](./plugins/index.md).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SHELLGPT_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SHELLGPT_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `ShellGPTPlugin` contract, appear in **Capabilities → Plugins**,
...
Create `$SHELLGPT_HOME/desktop-plugins/hello/plugin.js` (that's `~/.shellgpt/...`
...
```javascript
// ~/.shellgpt/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@shellgpt/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, ShellGPT' }),
      jsx('div', {
...
The only importable specifiers are `@shellgpt/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ShellGPTPlugin`:

```ts
interface ShellGPTPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `shellgpt.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@shellgpt/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@shellgpt/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@shellgpt/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@shellgpt/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@shellgpt/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@shellgpt/plugin-sdk'
...
| Plugin | Was | Now |
|---|---|---|
| next-prompt (#120660) | `window.dispatchEvent(new CustomEvent('shellgpt:composer-insert', …))` + a `setTimeout` `shellgpt:composer-focus`; `[data-composer-target]`/`[data-pane-hidden]` scan for the visible target | `await host.composer.insertText(null, suggestion.text, { mode: 'block' })`, then `host.composer.focus(null)` if the pill lost the caret |
| prompt-snippets (#116030) | same `shellgpt:composer-insert` event; `[data-slot="composer-input"]`/ProseMirror `textContent` + synthetic `InputEvent` fallback; `surfaceEditorEl().focus()` | `host.composer.insertText(sid, text, { mode: 'block' })`; `setDraft(sid, (await getDraft(sid) ?? '') + '\n' + text)` replaces the fallback; `host.composer.focus(sid)` — `sid = host.state.focusedSessionId.get()` |
...
```ts
import { SESSION_ROW_AREAS, type SessionRowSlotContribution } from '@shellgpt/plugin-sdk'
...
- **better-session-appearance** — replace the `localStorage`
  `shellgpt.desktop.sessionColors` write and the fiber-harvested `onChange` with
...
```ts
import { COMPOSER_AREAS, type ComposerModelPillContext, type ComposerModelPillProvider } from '@shellgpt/plugin-sdk'
...
* **shellgpt-appearance-hub** — mount its paper-texture / font / intro-copy
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@shellgpt/plugin-sdk'
...
`window.shellgpt.send('get-price eth')` (or a declarative
`<button data-shellgpt-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, WORKSPACE_PAGE_HEADER_AREA } from '@shellgpt/plugin-sdk'
...
```ts
import { SIDEBAR_NAV_PREFS_AREA, type SidebarNavPrefsContribution } from '@shellgpt/plugin-sdk'
...
for requests; `targetProfile` is the backend ShellGPT profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from ShellGPT
...
ctx.os.notify({
  ...
  // Body click → focus ShellGPT + navigate. Same vocabulary as OS deep links:
  activate: 'shellgpt://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'shellgpt://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `shellgpt://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `shellgpt://…` URL works as an OS deep link). Action buttons only render on
...
|--------|-------------|-------------------|
| keybind map (`shellgpt.desktop.keybinds`) | `KEYBINDS_AREA` contribution | a raw map write rebinds every other plugin's shortcuts; the area merges per plugin and is torn down with it |
...
| `toolView.technical`, `embed-mode`, `titlebarAppActions`, `translucency.v2`, `user-bubble-transparency.v1`, `shellgptDesktop.zoom.*` | follow-up keys after each store is audited | some drive the main process or window chrome; each needs its own guard and ownership review before it becomes plugin-writable |

Migration — `shellgpt-appearance-hub`, which today does
`localStorage.setItem('shellgpt.desktop.sessionListDensity', id)` followed by
...
```ts
// before
localStorage.setItem('shellgpt.desktop.backdrop.v1', String(on))
window.dispatchEvent(new StorageEvent('storage', { key: 'shellgpt.desktop.backdrop.v1', newValue: String(on) }))
// after — the store notifies its own subscribers; no synthetic StorageEvent
host.settings.set('backdrop.v1', on)
host.settings.set('sessionListDensity', id)          // was shellgpt.desktop.sessionListDensity
host.settings.set('tabStripDefault', id)             // was shellgpt.desktop.tabStripDefault
host.settings.set('reasoning.collapsedByDefault', on) // was shellgpt.desktop.reasoning.collapsedByDefault
...
`localStorage.getItem('shellgpt.desktop.keybinds')` to back up its shortcut — that
...
calling `window.shellgptDesktop.api` raw.
...
JSON.parse(localStorage.getItem(                           host.pluginDecisions.get()
  'shellgpt.desktop.pluginDecisions.v2'))                    ctx.onDispose(host.pluginDecisions.subscribe(fn))
localStorage.setItem('shellgpt.desktop.pluginDecisions.v2')  // declined — host.navigate('/capabilities?tab=plugins')
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@shellgpt/plugin-sdk'
...
```javascript
import { queryClient } from '@shellgpt/plugin-sdk'
...
copies that half into `$SHELLGPT_HOME/desktop-plugins/<id>/` beside a
`.shellgpt-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.shellgpt/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`shellgpt plugins update`, or
...
a package into `~/.shellgpt/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.shellgpt/plugins` is not reachable as a filesystem —
...
the `shellgpt://` scheme — a plain anchor on your website or README:
...
`shellgpt-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular ShellGPT plugin and declare it in a
`manifest.json`:
...
shellgpt-agent codebase directly (`shellgpt_state`, `shellgpt_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.shellgpt/`)
...
```python
from shellgpt_cli.plugin_events import broadcast_plugin_event
...
|---|---|
| `shellgpt serve` (the Desktop backend): `plugin_api.py` routers, plugin slash commands, tools and hooks in the agent turn | every connected Desktop window |
| the `dashboard.turn_isolation` compute-host child (tools/hooks of an isolated turn) | relayed over the host pipe to `shellgpt serve`, then every window |
| the stdio TUI (`shellgpt` in a terminal) | that terminal's client |
| `shellgpt gateway run` (messaging platforms), `shellgpt chat`, cron, `shellgpt plugins validate` | nobody — no Desktop client is attached to that process; the call is a logged no-op |
...
Migration (rss-reader): drop the `~/.shellgpt/rss-reader/commands.jsonl` queue,
...
(`shellgpt.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ShellGPTPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@shellgpt/plugin-sdk` alias.
2. It's still lint-fenced to `@shellgpt/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
`ctx.rest`, storage, `navigate`) and the `window.shellgptDesktop` native bridge
...
the loader's import allowlist (`@shellgpt/plugin-sdk` and `react*` only; a static
...
- **Only three specifiers resolve:** `@shellgpt/plugin-sdk`, `react`,
...
| Plugin contract | `ShellGPTPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `ShellGPTOpenTarget`, `Contribution` |
...
### Agents: the `shellgpt-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`shellgpt-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SHELLGPT_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `shellgpt logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@shellgpt/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.shellgpt/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.shellgpt/logs/errors.log` for

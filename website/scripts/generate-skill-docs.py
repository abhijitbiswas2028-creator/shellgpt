...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/shellgpt-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    shellgpt_meta = (fm.get("metadata") or {}).get("shellgpt") or {}
    tags = shellgpt_meta.get("tags") or []
    related = shellgpt_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `shellgpt skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ShellGPT Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "ShellGPT ships with a large built-in skill library copied into `~/.shellgpt/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "ShellGPT also syncs bundled skills on `shellgpt update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.shellgpt/skills/` tree, it is still shipped with ShellGPT; restore it with `shellgpt skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with shellgpt-agent — install via shellgpt skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with shellgpt-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",

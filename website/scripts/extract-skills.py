...
2. The unified ShellGPT Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                shellgpt_meta = metadata.get("shellgpt", {})
                if isinstance(shellgpt_meta, dict):
                    tags = shellgpt_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])

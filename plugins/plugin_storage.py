"""Per-plugin persistent storage: ``<shellgpt home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<shellgpt home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `shellgpt plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<shellgpt home>/plugin-data/<name>/``; resolves ``get_shellgpt_home()`` on
    every call so it follows the active profile — don't cache across profile switches."""
    from shellgpt_constants import get_shellgpt_home
    root = get_shellgpt_home() / "plugin-data" / _validate_name(name)

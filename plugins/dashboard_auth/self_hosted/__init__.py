...
``dashboard.oauth.self_hosted.{issuer,client_id,scopes,client_secret}`` or ``SHELLGPT_DASHBOARD_OIDC_*``.
"""
...
from shellgpt_cli.dashboard_auth import LoginStart, ProviderError, Session
...
def _settings() -> dict:
    ...
    issuer = setting("SHELLGPT_DASHBOARD_OIDC_ISSUER", "issuer")
    client_id = setting("SHELLGPT_DASHBOARD_OIDC_CLIENT_ID", "client_id")
    if not issuer or not client_id:
        raise SkipRegistration(
            "Self-hosted OIDC dashboard auth is not configured. Set both an issuer and "
            "a client_id — either as env vars (SHELLGPT_DASHBOARD_OIDC_ISSUER + "
            "SHELLGPT_DASHBOARD_OIDC_CLIENT_ID) or under "
    ...
    return {
        ...
        "scopes": setting("SHELLGPT_DASHBOARD_OIDC_SCOPES", "scopes") or _DEFAULT_SCOPES,
        # Credential: canonical home is the env var / ~/.shellgpt/.env. Empty ⇒ public client.
        "client_secret": setting("SHELLGPT_DASHBOARD_OIDC_CLIENT_SECRET", "client_secret")}
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('shellgpt_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('shellgpt_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('shellgpt_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('shellgpt_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}

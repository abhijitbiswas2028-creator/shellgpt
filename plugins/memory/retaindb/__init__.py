...
class _Client:
    ...
    def _headers(self, path: str, json_body: bool = True) -> dict:
        ...
        return {"Authorization": f"Bearer {token}", "x-sdk-runtime": "shellgpt-plugin",
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._user_id, self._session_id, self._agent_id = "default", "", "shellgpt"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project: RETAINDB_PROJECT > config.yaml > shellgpt-<profile> > "default" (API auto-creates "default").
        ...
        if not project:
            profile_name = os.path.basename(str(kwargs.get("shellgpt_home", "")))
            project = f"shellgpt-{profile_name}" if profile_name not in {"", ".shellgpt"} else "default"
        ...
        self._agent_id = kwargs.get("agent_id", "shellgpt") or "shellgpt"

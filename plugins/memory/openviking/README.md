...
- OpenViking server running and reachable from ShellGPT

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
ShellGPT can identify older servers that expose the legacy status-only health
...
connection values into ShellGPT, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.shellgpt/.env`; for a named profile use
...
OpenViking's server config is separate from ShellGPT:
...
ShellGPT-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, ShellGPT lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
ShellGPT sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
ShellGPT also sends `User-Agent: openviking-memory-shellgpt/<version>` on
OpenViking requests. This standard harness identifier contains the ShellGPT
...
does not ask for a peer ID. Without a configured peer, ShellGPT sends neither
...
order remains environment, linked OpenViking config, then ShellGPT YAML. To use
...
on the old implicit `shellgpt` peer now use user memory for new writes. Without
...
Set `agent: shellgpt` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `shellgpt-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live ShellGPT
...
Use the same OpenViking profile and credentials as ShellGPT for manual recovery.
...
ShellGPT built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| ShellGPT action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because ShellGPT
...
file URIs, such as
`viking://user/default/peers/shellgpt/memories/preferences/mem_abc123.md` (any

...
- Hosted: API key from [app.supermemory.ai/integrations?connect=shellgpt](http://app.supermemory.ai/integrations?connect=shellgpt)
...
Before running `shellgpt memory setup`, add the local endpoint to
...
Then run `shellgpt memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SHELLGPT_HOME/supermemory.json`
...
| `container_tag` | `shellgpt` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `shellgpt-{identity}` → `shellgpt-coder`). |
...
`https://api.supermemory.ai`. ShellGPT resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: shellgpt`, and document writes stamp
`metadata.sm_source: shellgpt`. This is a **functional routing key, not telemetry**:
it groups ShellGPT-written memories into a dedicated "ShellGPT" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per ShellGPT profile:

```json
{
  "container_tag": "shellgpt-{identity}"
}
```

For a profile named `coder`, this resolves to `shellgpt-coder`. The default profile resolves to `shellgpt-default`. Without `{identity}`, all profiles share the same container.

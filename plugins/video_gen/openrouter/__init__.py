...
class OpenRouterVideoGenProvider(VideoGenProvider):
    ...
    def _headers(self, api_key: str) -> Dict[str, str]:
        return {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json",
                "HTTP-Referer": "https://github.com/NousResearch/shellgpt-agent", "X-Title": "ShellGPT Agent"}
    ...
    def generate(
        ...
        if not api_key:
            return fail("No OpenRouter credential: set OPENROUTER_API_KEY or run `shellgpt auth add openrouter`",
                        "missing_credentials")

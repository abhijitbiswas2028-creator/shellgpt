"""disk-cleanup plugin — auto-cleanup of ephemeral ShellGPT session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/shellgpt-x/test_foo.py`
...
All operations are scoped to SHELLGPT_HOME and /tmp/shellgpt-*.  # no-tmp: ok — legacy scratch scope this plugin cleans up
...
def _cmd_track(argv: List[str]) -> str:
    ...
    return f"Not tracked (already present, missing, or outside SHELLGPT_HOME): {path_arg}"
...
def register(ctx) -> None:
    ...
    ctx.register_command("disk-cleanup", handler=_handle_slash,
                         description="Track and clean up ephemeral ShellGPT session files.")

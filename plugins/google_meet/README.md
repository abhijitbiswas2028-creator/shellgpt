...
Let the shellgpt agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where shellgpt runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `shellgpt meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `shellgpt meet` CLI |
...
| `cli.py` | `shellgpt meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SHELLGPT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `shellgpt meet node {run,list,approve,remove,status,ping}` |
...
shellgpt meet install                                      # pip + Chromium
shellgpt meet setup                                        # preflight
shellgpt meet auth                                         # optional
shellgpt meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
shellgpt meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.shellgpt/.env
shellgpt meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
shellgpt meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
shellgpt meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.shellgpt/.env
shellgpt meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, shellgpt will **not** switch your system audio input automatically — the
...
admission the bot unmutes itself if Meet seated it muted; `status.json` / `shellgpt meet
...
shellgpt plugins enable google_meet
shellgpt meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
shellgpt meet node ping my-mac

...
def ensure_google_chat_deps() -> bool:
    ...
    ``SHELLGPT_LAZY_INSTALL_TARGET`` instead of the read-only venv. Resets the
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # The profile scope this adapter was connected under (multiplex: SHELLGPT_HOME override + secret
        ...
        from shellgpt_constants import get_shellgpt_home as _get_shellgpt_home
        self._thread_count_store = _ThreadCountStore(_get_shellgpt_home() / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from shellgpt_constants import get_shellgpt_home as _get_shellgpt_home
        return _get_shellgpt_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a shellgpt MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "shellgpt_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'ShellGPT is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "ShellGPT is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. shellgpt-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``shellgpt setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.shellgpt/.env``."""
    from shellgpt_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.shellgpt/.env")
    print_info("Restart the gateway: shellgpt gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``shellgpt cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `shellgpt setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'ShellGPT is thinking…' marker message "

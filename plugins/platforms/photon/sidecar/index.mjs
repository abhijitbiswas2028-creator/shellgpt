// ShellGPT Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> ShellGPT): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (ShellGPT -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-ShellGPT-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `shellgpt photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so ShellGPT can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply ShellGPT'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local ShellGPT allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of ShellGPT, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-shellgpt-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when ShellGPT supplied them so a known-good

...
Small Node helper that bridges ShellGPT Agent to Photon's Spectrum SDK
(`spectrum-ts`).  ShellGPT is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-ShellGPT-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and ShellGPT can receive inbound messages
...
The ShellGPT plugin's `shellgpt photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. ShellGPT is Python, so replies go through this sidecar

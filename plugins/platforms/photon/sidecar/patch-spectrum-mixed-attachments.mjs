...
// `message.content.text` before ShellGPT can see it. We rewrite the two inbound
...
const MARKER = "ShellGPT patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // ShellGPT to patch; keep the legacy v8 path below for older pinned installs.
    if (

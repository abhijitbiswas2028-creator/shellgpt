# A2A — Agent-to-Agent protocol for ShellGPT
...
peer (another ShellGPT, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
shellgpt gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, ShellGPT serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.shellgpt/a2a_audit.jsonl`.
- Conversations persist to `~/.shellgpt/a2a_conversations/` — they survive context

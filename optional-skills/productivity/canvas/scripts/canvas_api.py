#!/usr/bin/env python3
"""Canvas LMS API CLI for ShellGPT Agent.
...
def _check_config():
    ...
    if missing:
        shellgpt_env = os.path.join(
            os.environ.get("SHELLGPT_HOME", os.path.expanduser("~/.shellgpt")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {shellgpt_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for ShellGPT Agent"
    )

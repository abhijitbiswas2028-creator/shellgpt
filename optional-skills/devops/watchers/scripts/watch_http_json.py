...
Usage (via cron with --no-agent):
    ...
    shellgpt cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SHELLGPT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "ShellGPT-Watcher/1.0"})

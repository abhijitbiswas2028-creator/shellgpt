...
Usage (via cron with --no-agent):
    ...
    shellgpt cron create shellgpt-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SHELLGPT_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name shellgpt-issues --repo NousResearch/shellgpt-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the ShellGPT .env file
(``${SHELLGPT_HOME:-~/.shellgpt}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "ShellGPT-Watcher/1.0",
    }

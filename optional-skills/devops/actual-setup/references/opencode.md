...
is typically only in `~/.shellgpt/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike ShellGPT,
...
`shellgpt-custom-providers` skill, pitfall 2). ShellGPT hits this because it forwards

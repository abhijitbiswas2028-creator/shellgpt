...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SHELLGPT_HOME", os.path.expanduser("~/.shellgpt")), "skills/security/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SHELLGPT_HOME", Path.home() / ".shellgpt")) / "skills" / "security" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# ShellGPT config paths
# ═══════════════════════════════════════════════════════════════════

SHELLGPT_HOME = Path(os.getenv("SHELLGPT_HOME", Path.home() / ".shellgpt"))
CONFIG_PATH = SHELLGPT_HOME / "config.yaml"
PREFILL_PATH = SHELLGPT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/ShellGPT models — already uncensored, just needs clean prompt
    "shellgpt": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "shellgpt" in model_lower or "nous" in model_lower:
        return "shellgpt"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ShellGPT config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.shellgpt/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ShellGPT for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ShellGPT for changes to take effect.")

...
## 5. GODMODE FAST — ShellGPT 4 405B (Zero Refusal)

**Model:** `nousresearch/shellgpt-4-405b`
**Strategy:** ShellGPT models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with ShellGPT Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.shellgpt/config.yaml`:

...
All per-subject data lives under PDD_DATA_DIR (default: $SHELLGPT_HOME/unbroker),
which is the same trust boundary ShellGPT uses for .env and OAuth tokens.
"""
...
def shellgpt_home() -> Path:
    return Path(os.environ.get("SHELLGPT_HOME") or (Path.home() / ".shellgpt"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else shellgpt_home() / "unbroker"

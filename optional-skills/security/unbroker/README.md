...
The design is **ShellGPT-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
shellgpt skills install official/security/unbroker
```

Then start a new ShellGPT session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a ShellGPT session:
...
- **PII handling.** Dossiers live under the ShellGPT home directory (`0600`, optionally

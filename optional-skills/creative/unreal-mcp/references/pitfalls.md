...
### 1. Start order: editor first, ShellGPT second

ShellGPT probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW ShellGPT session.
...
restart the editor, restart the ShellGPT session.
...
server fails to bind (Output Log) or ShellGPT' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.shellgpt/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for ShellGPT

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. ShellGPT' connection
lives in `~/.shellgpt/config.yaml` via `shellgpt mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms ShellGPT — just don't
mistake it for the ShellGPT setup step.
...
explicitly warns against overlapping calls. ShellGPT executes same-turn tool
...
### 9. Timeouts: ShellGPT gives up before Unreal does

ShellGPT' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If ShellGPT runs elsewhere (SSH backend,

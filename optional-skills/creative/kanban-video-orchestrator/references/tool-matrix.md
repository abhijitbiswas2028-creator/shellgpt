...
Maps each role archetype to the ShellGPT skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public shellgpt-agent
...
## ShellGPT skills relevant to video production

### Visual / rendering skills (`shellgpt-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `shellgpt plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`shellgpt-agent/skills/creative/`)
...
### Audio / media skills (`shellgpt-agent/skills/creative/` + `skills/media/`)
...
These are **not** ShellGPT skills but external CLIs / APIs that profiles invoke.
...
## Built-in ShellGPT tools for media review

These are native ShellGPT tools — not invoked via terminal but through their own
...
key is present in `${SHELLGPT_HOME:-~/.shellgpt}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every ShellGPT profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SHELLGPT_HOME:-~/.shellgpt}/.env` → environment variable.
...
When a new ShellGPT-public video skill ships:

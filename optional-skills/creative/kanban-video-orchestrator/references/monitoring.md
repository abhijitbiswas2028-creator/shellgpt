...
shellgpt kanban watch --tenant <project-slug>

# Snapshot of the board
shellgpt kanban list --tenant <project-slug>
shellgpt kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
shellgpt kanban stats --tenant <project-slug>

# Visual dashboard (browser)
shellgpt dashboard

# Inspect a specific task (includes comments + events)
shellgpt kanban show <task-id>

# Follow a single task's event stream
shellgpt kanban tail <task-id>
```

Verify available subcommands with `shellgpt kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `shellgpt kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `shellgpt kanban block <id>` then `shellgpt kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `shellgpt kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `shellgpt kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `shellgpt profile list` (profile exists?), `shellgpt status` (gateway/dispatcher up?), and verify tenant. |
...
shellgpt kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
shellgpt kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(shellgpt kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
shellgpt kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
shellgpt kanban block <task-id>
shellgpt kanban archive <task-id>

# Diagnose what happened
shellgpt kanban show <task-id>      # task body, comments, recent events
shellgpt kanban tail <task-id>      # follow the live event stream
shellgpt kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `shellgpt kanban create` for the director
...
while true; do
    clear
    shellgpt kanban list --tenant <slug>
    echo "---"
    shellgpt kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `shellgpt kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `shellgpt profile list` and start

...
the actual `setup.sh` that creates the project workspace, configures ShellGPT
...
2. **Create profiles** — `shellgpt profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.shellgpt/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `shellgpt kanban create` assigned to the director
...
```bash
shellgpt profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.shellgpt/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.shellgpt/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most ShellGPT
...
Each profile gets a `SOUL.md` at `~/.shellgpt/profiles/<name>/SOUL.md` that
...
```bash
shellgpt kanban create "Direct production of <video title>" \
    --assignee director \
...
the ShellGPT `.env` (`${SHELLGPT_HOME:-$HOME/.shellgpt}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _shellgpt_env="${SHELLGPT_HOME:-$HOME/.shellgpt}/.env"
    if grep -q "^${var}=" "$_shellgpt_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_shellgpt_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_shellgpt_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY shellgpt ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY shellgpt OPENROUTER_API_KEY || exit 1
# ...
```

...
Usage:
  ...
  <server_name> matches the files in $SHELLGPT_HOME/mcp-tokens/<server>.json etc.
...
def _shellgpt_home():
    # Prefer ShellGPT' own resolver (profile-safe); fall back to env then ~/.shellgpt.
    try:
        from shellgpt_constants import get_shellgpt_home
        return str(get_shellgpt_home())
    except Exception:
        return os.environ.get("SHELLGPT_HOME") or os.path.expanduser("~/.shellgpt")


def _tokens_dir():
    return os.path.join(_shellgpt_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "shellgpt-diag", "version": "1.0"}},
        },
        headers={
